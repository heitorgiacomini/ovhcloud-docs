...
description: Aprenda a elegir la clase adecuada de Block Storage de OVHcloud. Compare rendimiento, costos y casos de uso para optimizar su almacenamiento en términos de precio y eficacia.
...
Para garantizar la continuidad del servicio incluso en caso de fallo de una zona de disponibilidad, esta clase también admite [Multi-Attach](/guides/public-cloud/compute/storage-classic-multi-attach-3az). Esto permite que varias instancias situadas en diferentes zonas de disponibilidad se conecten simultáneamente y usen el mismo volumen.
...
La clase **High Speed Volume Gen2** ofrece prestaciones evolutivas que aumentan con el tamaño del volumen, lo que la convierte en una solución ideal para las cargas de trabajo intensivas:

- **IOPS**: 30 IOPS/Go (base 3 000 IOPS para 10–100 Go, hasta 20 000 IOPS)
- **Rendimiento**: 0,5 Mo/s/Go (base 50 Mo/s para 10–100 Go, hasta 512 Mo/s)
- **Tamaño máximo**: 12 To

Esta clase se recomienda para cargas de trabajo pesadas como la analítica, grandes bases de datos y el cálculo de alto rendimiento.

### Tabla comparativa

| Clase de almacenamiento | Caso de uso | Rendimiento | Regiones disponibles | SLA de disponibilidad | Replicación | Notas |
| --- | --- | --- | --- | --- | --- | --- |
| **High Speed Volume Gen2** | Cargas de trabajo de alto rendimiento, análisis, grandes bases de datos | 30 IOPS/Go (base 3 000 IOPS, hasta 20 000 IOPS), 0,5 Mo/s/Go (base 50 Mo/s, hasta 512 Mo/s), max 12 To | 3-AZ, 1-AZ, Local Zones | 99,9 % | Zonal | NVMe optimizado, rendimiento escalable |
...
Aunque el uso estándar de los volúmenes se factura por Go/mes, las [instantáneas](/guides/public-cloud/compute/storage-create-volume-snapshot) almacenadas en **Block Storage** y las [copias de seguridad](/guides/public-cloud/compute/volume-backup) almacenadas en **Object Storage** pueden generar costos adicionales. Las instantáneas permiten capturar el estado de un volumen en un momento dado, y las copias de seguridad garantizan la protección y recuperación de los datos.

#### Gestión del ciclo de vida y redimensionamiento

Los volúmenes Block Storage son completamente flexibles: puede [aumentar su tamaño](/guides/public-cloud/compute/increase-the-size-of-an-additional-disk) en cualquier momento, ampliar las particiones y conectar los volúmenes a diferentes instancias. Estas operaciones se gestionan a nivel de volumen, permitiéndole optimizar capacidad y rendimiento sin interrupciones.
...
[Modificar un Volume Block Storage](/guides/public-cloud/compute/switch-volume-type)
