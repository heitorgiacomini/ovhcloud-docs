...
description: This guide provides details on how to migrate from an S3-compatible object storage provider to OVHcloud Object Storage using Rclone
...
If this is your first time creating an Object Storage bucket, read our guide [Getting Started with Object Storage](/guides/storage-and-backup/object-storage/s3-getting-started-with-object-storage).
...
Similar to your source bucket, you will need your `access key`, `secret key` but also the `region ID` for your destination bucket. Log in to the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> and navigate to the <code className="action">Object Storage</code> section to collect those details.
...
To get the list of OVHcloud region endpoints, see [Object Storage - Endpoints and Object Storage geoavailability](/guides/storage-and-backup/object-storage/s3-location).
