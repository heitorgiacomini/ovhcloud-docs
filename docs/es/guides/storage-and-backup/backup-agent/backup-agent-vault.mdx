...
description: Descubra cómo funciona el sistema de Vault y la ubicación de sus datos de copia de seguridad
...
- Haber pedido un servicio Backup Agent en el momento de la compra de su servidor Bare Metal o posteriormente a través del menú <code className="action">Agente de copia de seguridad</code> de su espacio cliente.
...
Esto se basa en nuestros buckets de Object Storage, que puede encontrar en [este enlace](https://www.ovhcloud.com/es-es/public-cloud/object-storage/).

Para encontrar sus Vaults, haga clic en <ManagerLink to="/#/bmc-backup-agent-baremetal/service">este enlace</ManagerLink> para acceder a la sección <code className="action">Backup Agent</code> y, a continuación, haga clic en la pestaña <code className="action">Vaults</code>.

<img className="thumbnail" alt="Lista de Vaults del Agente de copia de seguridad" src="/images/storage-and-backup/backup-agent/backup-agent-vault/01-backup-agent-vault-list.png" loading="lazy" />
...
<img className="thumbnail" alt="Casos de uso del Vault del Agente de copia de seguridad" src="/images/storage-and-backup/backup-agent/backup-agent-vault/01-backup-agent-vault-use-cases.png" loading="lazy" />
