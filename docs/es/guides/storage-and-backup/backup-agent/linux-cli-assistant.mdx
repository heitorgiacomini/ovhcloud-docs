...
description: Descubra cómo utilizar el script ovh-ba-install.sh proporcionado por OVHcloud para instalar y gestionar el agente de backup Veeam en un servidor Linux
...
| Elemento | Detalle |
|--------|--------|
| **Sistema** | Linux **compatible** con el agente Veeam para Linux (consulte los [requisitos del sistema Veeam](https://helpcenter.veeam.com/docs/agentforlinux/userguide/system_requirements.html?ver=13) y las [restricciones de Backup Agent](/guides/storage-and-backup/backup-agent/restrictions)). |
...
En el área de cliente de OVHcloud, abra su <ManagerLink to="/#/bmc-backup-agent-baremetal/service">Backup Agent</ManagerLink>, vaya a la pestaña <code className="action">Agents</code> y haga clic en el botón <code className="action">Descargar</code>. En la ventana que se abre, seleccione **Linux** para mostrar el comando con las dos URL (script y paquete Linux).

<img className="thumbnail" alt="Descargar el agente — comandos de instalación Linux" src="/images/storage-and-backup/backup-agent/backup-agent-linux-cli-assistant/01-backup-agent-download-linux-fr.png" loading="lazy" />
...
Para profundizar en sus diagnósticos, consulte nuestra [guía de diagnóstico y resolución de problemas de Backup Agent](/guides/storage-and-backup/backup-agent/troubleshooting).
...
- [Configurar su primer backup](/guides/storage-and-backup/backup-agent/first-configuration)
