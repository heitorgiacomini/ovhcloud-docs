...
description: "Descubra los fundamentos del Public Cloud Compute: funcionamiento de las instancias, familias y tamaños disponibles, despliegues multi-AZ, gestión de imágenes, seguridad SSH, mecanismos de copia de seguridad, red pública/privada, y ventajas de los Savings Plans."
...
Las instancias de Public Cloud de OVHcloud se despliegan en [varios centros de datos a través del mundo](https://www.ovhcloud.com/es-es/about-us/global-infrastructure/regions/), garantizando una alta disponibilidad y proximidad con sus usuarios. Ejemplos de regiones:
...
:::info
**Buena práctica**: Para cargas de trabajo críticas, elija un [despliegue multi-AZ](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture) para garantizar la resiliencia y la continuidad del servicio.
:::
...
- **[Imágenes personalizadas](/guides/public-cloud/compute/upload-own-image)**: Puede importar sus propias imágenes en formato QCOW2 o RAW, ofreciendo un control total sobre su entorno y permitiendo migraciones, modelos estándar o configuraciones especializadas.

**Ciclo de vida y soporte**: OVHcloud actualiza regularmente el catálogo de imágenes. Siempre consulte las anuncios sobre el ciclo de vida y el fin del soporte para asegurarse de que sus imágenes siguen seguras y soportadas. Ver [aquí](/guides/public-cloud/compute/image-life-cycle).
...
Para instrucciones detalladas sobre la creación y uso de claves SSH, consulte la guía [OVHcloud sobre SSH](/guides/public-cloud/compute/creating-ssh-keys).
...
:::info
**Buena práctica**: Las copias de seguridad no sustituyen una [arquitectura resistente](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture). Para entornos críticos, combine las copias de seguridad con la replicación multi-AZ para garantizar una protección máxima de los datos y una disponibilidad óptima del servicio.
:::
...
Para más detalles sobre la configuración de redes de Public Cloud, consulte la guía oficial [OVHcloud sobre redes](/guides/public-cloud/network-services/concepts).
...
Los Savings Plans le ayudan a optimizar su presupuesto manteniendo el rendimiento y la fiabilidad de su entorno en la nube. Para más información, consulte la guía oficial [OVHcloud sobre Savings Plans](/guides/public-cloud/cross-functional/savings-plans).
...
- [Cómo crear una instancia de Public Cloud y acceder a ella](/guides/public-cloud/compute/getting-started)
- [Gestionar sus instancias de Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance)
- [Iniciar una instancia en un volumen arrancable](/guides/public-cloud/compute/start-instance-on-attached-volume)
- [Poner en suspensión o pausar una instancia](/guides/public-cloud/compute/suspend-or-pause-an-instance)
