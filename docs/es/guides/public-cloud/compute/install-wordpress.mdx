...
description: Cómo utilizar una instancia de Public Cloud para alojar sitios web WordPress
...
- Tener una [instancia de Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance) con Debian o Ubuntu instalado.
...
Para acceder a la instalación a través de un dominio, deberá asociarlo a su instancia. Para ello, edite la zona DNS accesible desde el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, siempre que OVHcloud sea su agente registrador **y** que el dominio utilice los servidores DNS de OVHcloud.

Para más información, consulte la guía [Editar una zona DNS](/guides/web-cloud/domains/dns-zone-edit). Si el dominio está en uso, solo podrá configurar los DNS después de instalar el nuevo WordPress y de iniciar el sitio web.
...
Abra un navegador web y conéctese al sitio WordPress introduciendo la dirección IP de su instancia (o el nombre de dominio si ya ha [asociado uno a la instancia](/guides/web-cloud/domains/dns-zone-edit)). Seleccione un idioma en la primera página.
