...
- Si desea preparar el entorno para utilizar [Terraform](/guides/public-cloud/cross-functional/how-to-use-terraform), debe preparar el entorno.

**Esta guía explica cómo crear un disco adicional y configurarlo en una instancia.**

## Requisitos

- Disponer de una instancia de [Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance) en su cuenta de OVHcloud.
- Tener acceso de administrador (sudo) a su instancia a través de SSH.
- Preparar el entorno si desea utilizar [Terraform](/guides/public-cloud/cross-functional/how-to-use-terraform).
...
<summary>**Classic - 500 IOPS garantizadas**</summary>
...
En las regiones 3AZ, los volúmenes Classic son servicios regionales que utilizan un Erasure Coding distribuido entre varias zonas de disponibilidad. Esto garantiza la disponibilidad de los datos sin impacto ni interrupción en caso de fallo de una zona, siempre que se cumplan los requisitos de la arquitectura resiliente con conexión múltiple. Para más información, consulte nuestra guía "[Uso correcto y limitaciones del almacenamiento Classic Multi-Attach en las regiones 3AZ](/guides/public-cloud/compute/storage-classic-multi-attach-3az)".
...
<Tabs>
  <Tab label="Desde el área de cliente de OVHcloud">
    Abra <code className="action">Block Storage</code> en el menú de la izquierda, en **Storage & Backup**.
    
    En esta sección, haga clic en el botón <code className="action">Crear un volumen</code>.
    
    <img className="thumbnail" alt="seleccionar proyecto" src="/images/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance/avolume01.png" loading="lazy" />

Siga los pasos de configuración para seleccionar las opciones de ubicación, tipo de disco, cifrado y capacidad de disco. Introduzca un nombre para el volumen y acepte haciendo clic en <code className="action">Crear el volumen</code>.
...
A la derecha del volumen, haga clic en el botón <code className="action">...</code> y seleccione <code className="action">Asociar a la instancia</code>.
...
En la ventana que aparece, seleccione una instancia de la lista y haga clic en <code className="action">Confirmar</code> para asociar el disco.
...
El proceso de asociar el disco a su instancia comenzará. La operación puede tardar unos minutos.
  ...
  <Tab label="A través de Horizon">
    Acceda a la sección desplegable <code className="action">Volumes</code>, haga clic en <code className="action">Volumes</code> y luego en <code className="action">Create Volume</code>.
...
Introduzca el campo <code className="action">Volume Name</code> y seleccione el tipo de volumen que desee. A continuación, haga clic en <code className="action">Create Volume</code>.
...
Para asociar este volumen a una instancia, en la línea de su volumen, haga clic en la flecha situada al final de la línea, junto a `Edit Volume`. Haga clic en <code className="action">Manage Attachments</code>
    
    <img className="thumbnail" alt="Asociar un volumen de almacenamiento de bloques a una instancia" src="/images/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance/horizon_manage_attachments.png" loading="lazy" />

Seleccione la instancia a la que quiere asociar el volumen y haga clic en <code className="action">Attach Volume</code>.
  ...
  <Tab label="A través de la CLI OpenStack">
    :::warning
...
Abra una [conexión SSH a su instancia](/guides/public-cloud/compute/getting-started#connect-instance) y utilice el siguiente comando para mostrar los discos asociados.
...
Una vez que se haya conectado, haga clic derecho en el botón <code className="action">Iniciar</code> y abra <code className="action">Gestión de discos</code>.
...
<img className="thumbnail" alt="volumen desconocido" src="/images/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance/disk-management-01.png" loading="lazy" />
...
Haga clic derecho en el disco y seleccione <code className="action">En línea</code>.

Si el disco está indicado aquí como desconectado, probablemente se deba a una política en la instancia. Para solucionar el problema, haga clic derecho en el disco y seleccione <code className="action">On-line</code>.
...
Haga clic derecho de nuevo y seleccione <code className="action">Inicializar disco</code>.
...
A continuación, seleccione <code className="action">MBR</code> si su disco adicional es inferior a 2TB, o <code className="action">GPT</code> si es superior a 2TB, y haga clic en <code className="action">OK</code>.
...
Haga clic derecho en el botón <code className="action">Iniciar</code> y abra <code className="action">Ejecutar</code>.
...
<img className="thumbnail" alt="run rápido" src="/images/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance/run-prompt.png" loading="lazy" />
...
En la herramienta de <code className="action">gestión de discos</code>, haga clic derecho en el nuevo disco y seleccione <code className="action">Nuevo volumen simple...</code>.
...
En el asistente, haga clic en <code className="action">Siguiente</code> para especificar el tamaño del volumen. Por defecto, debe estar al máximo. Haga clic en <code className="action">Siguiente</code> para continuar.
...
Deje la nueva letra predeterminada o seleccione otra y haga clic en <code className="action">Siguiente</code>.
...
Asigne un nombre al volumen (opcional) y confirme las opciones de formato haciendo clic en <code className="action">Siguiente</code>.
...
En la última ventana, haga clic en <code className="action">Finalizar</code> para dar formato al disco.
...
<Tabs>
  <Tab label="En Linux">
    Abra una [conexión SSH a su instancia](/guides/public-cloud/compute/getting-started#3-crear-una-instancia) y utilice el siguiente comando para mostrar los discos asociados.
  ...
  <Tab label="En Windows">
    ...
    Una vez que se haya conectado, haga clic derecho en el menú <code className="action">Iniciar</code> y abra <code className="action">Administración de discos</code>.
    
    <img className="thumbnail" alt="gestión de discos" src="/images/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance/start-menu.png" loading="lazy" />

Haga clic derecho en el volumen que desee desmontar y seleccione <code className="action">Cambiar la letra y rutas de acceso de unidad...</code>.
...
Haga clic en <code className="action">Quitar</code> para eliminar el disco.
...
Haga clic en <code className="action">Sí</code> para confirmar la eliminación de la letra de la unidad de disco.
    
    <img className="thumbnail" alt="confirm remove disk" src="/images/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance/confirmunmounting.png" loading="lazy" />
...
<Tabs>
  <Tab label="Desde el área de cliente de OVHcloud">
    Acceda a la sección <code className="action">Public Cloud</code> de su área de cliente de OVHcloud y haga clic en <code className="action">Block Storage</code> en el menú de la izquierda bajo **Backup Storage**.
    
    Haga clic en el botón <code className="action">...</code> junto al volumen correspondiente y seleccione <code className="action">Desvincular de la instancia</code>.
...
Haga clic en <code className="action">Confirmar</code> en la nueva ventana para iniciar el proceso.
