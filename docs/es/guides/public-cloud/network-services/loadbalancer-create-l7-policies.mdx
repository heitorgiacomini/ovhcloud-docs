...
description: Learn how to configure and manage Level 7 (L7) Policies and Rules for OVHcloud Public Cloud Load Balancers
...
- Understanding of the [Load Balancer concepts](/guides/public-cloud/network-services/load-balancer-concepts)
- An active [Public Cloud project](/guides/public-cloud/cross-functional/create-a-public-cloud-project)
- A previously configured [Load Balancer](/guides/public-cloud/network-services/getting-started-load-balancer) within your OVHcloud project space
...
- **OpenStack CLI:** For command line management. More information on preparing your OpenStack API environment is available [here](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment).
- **Horizon Interface:** Offers graphical management via OpenStack for users familiar with this platform. Additional details [here](/guides/public-cloud/cross-functional/introducing-horizon).
...
In your Public Cloud project, select <code className="action">Load Balancer</code> in the `Network`section or use the quick search function. You will see a list of your Load Balancers. Click on the one you wish to configure.

#### Step 3: Manage L7 Policies

After selecting your Load Balancer, you will access its management interface. Find and click on the <code className="action">Listeners</code> tab to display the list of listeners.

To access the management options for L7 policies of a specific listener, click  the <code className="action">...</code> button to the right of the listener you want to configure.

Select the <code className="action">View L7 policies</code> option from the dropdown menu.
...
Once in the L7 policies section, you can view existing policies or click on <code className="action">Add an L7 policy</code> to create a new policy, depending on the options available in the interface.
...
After filling in all the necessary fields, click on <code className="action">Add</code> to create your L7 policy.
...
On the L7 policies page of your listener, locate the policy to which you want to add rules. Click the <code className="action">...</code> button next to the relevant policy and select <code className="action">Manage L7 rules</code>.
...
On the L7 rules page, click the <code className="action">Add an L7 rule</code> button.
...
Once all fields are filled in according to your criteria, click <code className="action">Add</code> to save the rule to your L7 policy.
...
In the Horizon dashboard, select your <code className="action">Project</code><br/>
In the left menu, click on the <code className="action">Network</code> tab and select <code className="action">Load Balancers</code>.
...
In the listener's details, locate and click on the <code className="action">L7 policies</code> tab. This tab lists all existing policies.

To add a new policy, click the <code className="action">Create L7 Policy</code> button.
...
Click on <code className="action">Add Rule</code> to set up the rules that will determine the specific conditions under which the policy will apply.
