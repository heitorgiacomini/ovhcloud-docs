...
- in [Horizon](/guides/public-cloud/cross-functional/introducing-horizon).
- using the [OpenStack CLI](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) by doing `openstack floating ip list` and `openstack loadbalancer list`.
- using the OVHcloud API:

<Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/loadbalancing/loadbalancer/\{loadBalancerId\}"} />
...
Are you on Discord? Connect to our channel at [https://discord.gg/ovhcloud](https://discord.gg/ovhcloud) and interact directly with the team that builds our services!
