...
<details>
<summary>How to install the ovhai CLI?</summary>


To install the ovhai CLI, you can follow the installation instructions provided by OVHcloud in the official [ovhai installation documentation](/guides/public-cloud/ai-machine-learning/ai-cli-install-client).

</details>


<details>
<summary>When I try to use the ovhai CLI, I get a `AI Training error: not authenticated` error.</summary>
...
<details>
<summary>I can't login with the `--token` parameter inside ovhai CLI</summary>
...
You can use the `-h` or `--help` flag after the command. This will provide you with a list of possible commands and their usage instructions (available options, arguments, …). The basic command is: `ovhai -h`. Additionally, you can refer to the [ovhai CLI cheat sheet](/guides/public-cloud/ai-machine-learning/ai-cheat-sheet) for a comprehensive overview of available commands.
...
For more information on these products, please read the [comparative guide for AI Solutions](/guides/public-cloud/ai-machine-learning/ai-comparative-tables).
...
<details>
<summary>How to attach and use storage?</summary>


Read this [OVHcloud documentation](/guides/public-cloud/ai-machine-learning/ai-data) on data for more information. You can also check the [S3* compatible compliance documentation](/guides/public-cloud/ai-machine-learning/ai-s3-compliance) if you are interested in S3* compatible Object Storage buckets.
...
<details>
<summary>How to fix file access permission errors?</summary>
...
While AI Notebooks does not offer this possibility, it is possible with AI Training & AI Deploy. Your image needs to be either on the shared registry, a private managed registry, DockerHub, or GitHub packages. You will find more information [here](/guides/public-cloud/ai-machine-learning/ai-manage-registries). Keep in mind that your image must respect the **linux/AMD64** architecture.
...
<details>
<summary>How to build my custom Docker image?</summary>


Essential information about building a custom Docker image can be found in the [Build & use custom Docker image](/guides/public-cloud/ai-machine-learning/ai-training-build-use-custom-image) guide. Advanced information can be found in the [official Docker documentation](https://docs.docker.com/engine/reference/builder/).
...
<details>
<summary>Why did my notebook or job stop after 7 consecutive running days?</summary>


To keep the platform secure and up‑to‑date, hosts are periodically restarted. Any AI Notebook and AI Training job that runs continuously for more than 7 consecutive days in a RUNNING state is automatically stopped. If you need a longer runtime, you can contact [our support](https://help.ovhcloud.com/csm?id=csm_get_help) to extend the limit up to **28 days**.
...
Make sure your AI Task is in a `RUNNING` state and that your UI is exposed either on the default port or that you specified the correct port in your URL (see [public ports](/guides/public-cloud/ai-machine-learning/ai-training-capabilities#quotas-per-public-cloud-project)).
...
Each notebook, job, and app exposes a public URL that defaults to port `8080`. This default port can be configured when you submit your AI Training job or your AI Deploy app, but is fixed to `8080` for AI Notebooks and cannot be changed.

However, you can access other ports than the default one by appending their number to your notebook, job or app public URL. For example, if you want to access port `8501` from your notebook, you will have to add `-8501` to your notebook URL (starting with the notebook's ID, filled with 0 here). The URL for accessing the `8501` port will be https://00000000-0000-0000-0000-000000000000-8501.job.gra.ai.cloud.ovh.net/

The same applies to AI Training jobs and AI Deploy apps, where other ports can be accessed even after the job or app has been launched.

For Notebooks, only the HTTP layer is accessible.
