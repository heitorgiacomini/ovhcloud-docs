...
description: Discover AI Deploy and unfold your first application
...
This guide explains how to get started with OVHcloud AI Deploy, covering the deployment of your first application using either the Control Panel (UI) or the [ovhai Command-Line Interface](/guides/public-cloud/ai-machine-learning/ai-cli-install-client).
...
Click <ManagerLink to="/#/pci/projects">this link</ManagerLink> to access your Public Cloud project, then go to the <code className="action">AI & Machine Learning</code> category in the left menu and choose <code className="action">AI Deploy</code>.

Click on the <code className="action">Deploy an app</code> button and accept the terms and conditions if any.

Once clicked, you will be redirected to the creation process detailed below.

<img className="thumbnail" alt="AI Deploy app subscribe" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/ai_app_subscribe.png" loading="lazy" />
...
<img className="thumbnail" alt="AI Deploy app name" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-1-name-app.png" loading="lazy" />
...
:::info
OVHcloud provides multiple datacenters. You can find the capabilities for AI Deploy in the guide [AI Deploy capabilities](/guides/public-cloud/ai-machine-learning/ai-deploy-capabilities).

:::


<img className="thumbnail" alt="AI Deploy app location" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-2-select-location.png" loading="lazy" />
...
<img className="thumbnail" alt="AI Deploy app resources" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-3-app-resources.png" loading="lazy" />

#### Step 4: Select the application to deploy

AI Deploy allows a user to deploy applications from two sources:

- From your own Docker image, giving you the full flexibility to deploy what you want. This image can be stored on many types of registry ([OVHcloud Managed Private Registry](https://www.ovhcloud.com/es-es/public-cloud/managed-private-registry/), [Docker Hub](https://hub.docker.com/), [GitHub packages](https://github.com/features/packages), ...) and the expected format is `<registry-address>/<image-identifier>:<tag-name>`.
...
:::info
If you want to deploy your own image, you need to comply with a few rules like adding a specific user. Follow our [Build and use custom images](/guides/public-cloud/ai-machine-learning/ai-training-build-use-custom-image) guide. You might also be interested in [Using and managing your public and private registries](/guides/public-cloud/ai-machine-learning/ai-manage-registries).

:::


To use this demonstration OVHcloud docker image, enter the following name as the Custom Docker image: `ovhcom/ai-deploy-hello-world`. Then click on the <code className="action">+</code> button to confirm.
...
<img className="thumbnail" alt="AI Deploy app docker image" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-4-app-image.png" loading="lazy" />
...
<img className="thumbnail" alt="AI Deploy app scaling strategies" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-5-scaling-strategies.png" loading="lazy" />
...
For more detailed information about scaling strategies, please refer to our dedicated guide: [AI Deploy - Scaling strategies](/guides/public-cloud/ai-machine-learning/ai-deploy-apps-deployments).
...
<img className="thumbnail" alt="AI Deploy app port" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-6-app-port.png" loading="lazy" />
...
<img className="thumbnail" alt="AI Deploy app confidentiality" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-7-app-confidentiality.png" loading="lazy" />
...
To learn more about data, volumes and permissions, check out the [data page](/guides/public-cloud/ai-machine-learning/ai-data).

<img className="thumbnail" alt="AI Deploy app volumes" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-8-app-volumes.png" loading="lazy" />
...
<img className="thumbnail" alt="AI Deploy app labels" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-8-app-labels.png" loading="lazy" />
...
Learn more about this feature in the [dedicated tutorial](/guides/public-cloud/ai-machine-learning/ai-deploy-tokens).
...
You can also generate the equivalent `ovhai` CLI command, which enables you to deploy the same application using the command line interface. This CLI can be downloaded [here](/guides/public-cloud/ai-machine-learning/ai-cli-install-client). For more information, consult the [CLI - Launch an AI Deploy app](/guides/public-cloud/ai-machine-learning/ai-cli-deploy-app) documentation.

To launch your AI Deploy app, click on <code className="action">Order now</code>. Please note that your app will not be immediately available, as it requires some time to:
...
<img className="thumbnail" alt="AI Deploy app details" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/ai_app_details.png" loading="lazy" />

For your information, you can access your deployed application by clicking the <code className="action">HTTP access</code> blue button, which will expose the default HTTP port of your app. However, since we have deployed a Flask API in this tutorial, you won't be able to access it through the <code className="action">HTTP access</code> button as no interface was deployed.
...
In your OVHcloud Control Panel left menu, go to the <code className="action">AI Dashboard</code> in the <code className="action">AI & Machine Learning</code> section. Select the <code className="action">Tokens</code> tab.

Click on the <code className="action">+ Create a token</code> button then fill in a name, a label selector, a role and region as below:

<img className="thumbnail" alt="AI Deploy token creation" src="/images/public-cloud/ai-machine-learning/deploy-guide-02-getting-started/step-10-token-creation.png" loading="lazy" />
...
Once stopped, your AI Deploy app will free up the previously allocated compute resources. Your endpoint is kept and if you restart your AI Deploy app, the same endpoint can be reused seamlessly.
    ...
    If you want to completely **delete** your AI Deploy app, select the <code className="action">Delete</code> action.
    Be sure to also delete your Object Storage data if you don't need it anymore, by going in the <code className="action">Object Storage</code> section (in the Storage category).
  </Tab>
...
- You can imagine deploying an AI model for sketch recognition thanks to **AI Deploy**. Refer to this [tutorial](/guides/public-cloud/ai-machine-learning/ai-deploy-gradio-sketch-recognition).
- Do you want to use **Streamlit** in order to create an app? [Here it is](/guides/public-cloud/ai-machine-learning/ai-deploy-build-use-flask-image).
