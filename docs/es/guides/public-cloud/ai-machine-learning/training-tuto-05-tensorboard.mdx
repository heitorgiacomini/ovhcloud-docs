...
description: How to use a tensorboard inside an AI Training Job
lastUpdated: 2022-09-01
---

## Objective

The purpose of this tutorial is to show how it is possible to launch a [TensorBoard](https://www.tensorflow.org/tensorboard) with `AI Training`.
...
Alternatively, you can have a job already `RUNNING` that is connected to that object store container and is writing metric logs inside it (example: `my_tf_metrics@GRA:/runs:RW:cache`). In that last case, don't forget the `cache` parameter indicating that the volume is cached and sharable among jobs. More information about volumes configuration in jobs can be found [here](/guides/public-cloud/ai-machine-learning/ai-cli-run-job#attaching-volumes), information about volume caching can be found [here](/guides/public-cloud/ai-machine-learning/ai-data#capabilities).
