...
:::warning
This guide assumes you are familiar with the <ApiLink>OVHcloud API</ApiLink>. If you have never used it, you can find the basics here: [First steps with the OVHcloud API](/guides/manage-and-operate/api/first-steps).
:::
...
We will use the following endpoint to create the private networks:

<Api version="v1" section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/network/private"} />

For example, to create `priv-net-01`, set the *request body* to:
...
And then we assign them the subnets using this endpoint, with the network ID returned by the previous call as the `networkId` *path parameter*:
