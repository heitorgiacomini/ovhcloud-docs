...
The purpose of this guide is to help OVHcloud Managed Kubernetes Service (MKS) users to migrate from an existing [Load Balancer for Managed Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) to a [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/).

[Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/) is the default Load Balancer for MKS clusters using Kubernetes versions >1.30.

[Load Balancer for Managed Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) is deprecated for MKS clusters using Kubernetes versions >1.31.
...
Below is a matching of existing annotations supported on [Load Balancer for Managed Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) and their equivalent on [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/).

:::warning
If you are using legacy annotations from [Load Balancer for Managed Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/), you must update them to the new format supported by [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/) during migration.
...
:::warning
Starting from MKS cluster using Kubernetes version **1.31**, any cluster upgrade attempt will be blocked if a service of type Load Balancer relying on [Load Balancer for Managed Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) (IOLB) is still present in the cluster.
...
There are two methods to move from a [Load Balancer for Managed Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) to a [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/), you can either **[Migrate](#migration)** or **[Replace](#replacement)** your Load Balancer.

Your existing Load Balancer Service using [Load Balancer for Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) should have the following annotation:
...
Migrating from an existing [Load Balancer for Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) to a [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/) involves creating a new Load Balancer service using Public Cloud Load Balancer with the same label selector to expose your application. For a short period of time, your application will be accessible using both Load Balancers.

At this time you can perform a DNS switch and then delete the old Load Balancer.

To migrate from an existing [Load Balancer for Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) to a [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/), follow these steps:
...
Replacing an existing [Load Balancer for Kubernetes](https://www.ovhcloud.com/es-es/public-cloud/load-balancer-kubernetes/) with a [Public Cloud Load Balancer](https://www.ovhcloud.com/es-es/public-cloud/load-balancer/) involves modifying the existing service and changing the loadbalancer class from `iolb` to `octavia`. This will lead to Kubernetes reconciling the loadbalancer class by deleting the old one and creating a new loadbalancer.
