...
:::info
Para más información sobre nuestra solución anti-DDoS, [haga clic aquí](https://www.ovhcloud.com/es-es/security/anti-ddos/).
...
- Un servicio de OVHcloud expuesto y que utilice una dirección IP pública dedicada ([Servidor Dedicado](https://www.ovhcloud.com/es-es/bare-metal/), [VPS](https://www.ovhcloud.com/es-es/vps/), [instancia de Public Cloud](https://www.ovhcloud.com/es-es/public-cloud/), [Hosted Private Cloud](https://www.ovhcloud.com/es-es/hosted-private-cloud/vmware/), [Additional IP](https://www.ovhcloud.com/es-es/network/additional-ip/), etc.)
...
<img className="thumbnail" alt="filtrar los servicios" src="/images/bare-metal-cloud/dedicated-servers/firewall-network/selectservice_cut_new.png" loading="lazy" />

A continuación, haga clic en el botón <code className="action">⁝</code> situado a la derecha de la IPv4 correspondiente y seleccione <code className="action">Configurar el Edge Network Firewall</code> (o haga clic en el icono de estado en la columna **Edge Firewall**).

<img className="thumbnail" alt="Activación del Edge Network Firewall" src="/images/bare-metal-cloud/dedicated-servers/firewall-network/firewall_config_new.png" loading="lazy" />
...
Para más información, consulte las siguientes guías: [Configuración del firewall en Windows](/guides/bare-metal-cloud/dedicated-servers/activate-port-firewall-soft-win) y [Configuración del firewall en Linux con iptables](/guides/bare-metal-cloud/dedicated-servers/firewall-linux-iptable).

:::


**Para añadir una regla**, haga clic en el botón <code className="action">+ Añadir una regla</code>, en la parte superior izquierda de la página.

| ![add-rule-btn](/images/bare-metal-cloud/dedicated-servers/firewall-network/enf_add_rule_new.png) |
|:--:|
| Haga clic en <code className="action">+ Añadir una regla</code>. |
...
|:--|
| - Una prioridad (de 0 a 19, siendo 0 la primera regla que se aplica, seguida de las demás) <br/> - Una acción (<code className="action">Aceptar</code> o <code className="action">Denegar</code>) <br/> - El protocolo <br/> - La dirección IP de origen (opcional) |
...
|:--|
| - Una prioridad (de 0 a 19, siendo 0 la primera regla que se aplica, seguida de las demás) <br/> - Una acción (<code className="action">Aceptar</code> o <code className="action">Denegar</code>) <br/> - El protocolo <br/> - La dirección IP de origen (opcional) <br/> - Puerto o rango de puertos de origen (opcional) <br/> - Puerto o rango de puertos de destino (opcional) <br/> - El estado TCP (opcional) <br/> - Fragmentos (opcional) |
...
<img className="thumbnail" alt="Ejemplo de configuración" src="/images/bare-metal-cloud/dedicated-servers/firewall-network/exemple.png" loading="lazy" />
...
Para obtener información detallada sobre los ataques detectados y los resultados de las actividades del Scrubbing Center, le recomendamos que consulte nuestra guía sobre el [Network Security Dashboard](/guides/bare-metal-cloud/dedicated-servers/network-security-dashboard).
...
- [Proteger un servidor GAME con el firewall de aplicación](/guides/bare-metal-cloud/dedicated-servers/firewall-game-ddos)
