...
description: Cómo configurar el OVHcloud Game DDoS Protection Firewall
...
:::info
Para más información, consulte nuestra página web [Protección anti-DDoS Game](https://www.ovhcloud.com/es-es/security/game-ddos-protection/).
...
- Un [servidor dedicado OVHcloud **Game**](https://www.ovhcloud.com/es-es/bare-metal/game/)
...
- **Enlace directo:** <ManagerLink to="/#/network/ip">Public IP</ManagerLink>
...
:::warning
Esta funcionalidad puede no estar disponible o estar limitada en los servidores de la [**gama Eco**](https://eco.ovhcloud.com/es-es/about/).

Consulte nuestra [página de comparación](https://eco.ovhcloud.com/es-es/compare/) para obtener más información.
...
:::warning
El *Game Firewall* protege las direcciones IP asociadas a un servidor. Por lo tanto, si tiene un servidor con varias direcciones IP (p. ej., [direcciones Additional IP](https://www.ovhcloud.com/es-es/network/additional-ip/)), deberá configurar cada una de ellas por separado.

Cada una de las direcciones que desee proteger con el Game Firewall **debe mostrar** el estado `Configurada` en la columna Game Firewall para que se apliquen las reglas de protección.
...
- Haga clic en <code className="action">Network</code> en el menú situado a la izquierda de la pantalla.
- Haga clic en <code className="action">Direcciones IP públicas</code>.

Puede filtrar las direcciones IP utilizando el menú desplegable <code className="action">Todos los tipos de servicio</code> o introduciendo directamente la dirección IP deseada en la barra de búsqueda. Introduzca el nombre o la categoría del servidor correspondiente:

#### Listar las direcciones IP vinculadas a su servidor Game

<Tabs>
  <Tab label="Desde la página Servidores dedicados">
    - Abra la sección <code className="action">Bare Metal Cloud</code> en la barra lateral izquierda.
    - Seleccione <code className="action">Servidores dedicados</code>.
    - Haga clic en el servidor Game que desee configurar.
    - En la sección `Red` de la pestaña `Información general`, busque la sección "Game DDoS Protection".
    - Haga clic en el botón <code className="action">...</code> y seleccione <code className="action">Configurar Game Protection</code>. Se le redirigirá a la lista de direcciones IP asignadas a su servidor.
  </Tab>
  <Tab label="Desde la página Direcciones IP públicas">
    ...
    - En el menú desplegable <code className="action">Todos los tipos de servicio</code>, busque y seleccione el servidor Bare Metal Game que desee configurar.
    - Aparecerá una lista de las direcciones IP asignadas a su servidor.
  </Tab>
</Tabs>
...
Para cada dirección vinculada a su servidor que requiera protección, compruebe que el estado del *Game Firewall* sea <code className="action">Disponible</code>. Deberá configurar las reglas del *Game Firewall* individualmente para cada una de ellas.

- Haga clic en el botón <code className="action">⁝</code> a la derecha de la tabla y seleccione <code className="action">Configurar el firewall Game</code>.
- Añada reglas especificando el protocolo y el rango de puertos para cada aplicación de juego que sea accesible en la dirección IP seleccionada. Consulte la sección [Menciones específicas para determinados juegos](#game_specific) para obtener más información.
- Por razones de seguridad, le recomendamos encarecidamente activar la opción <code className="action">Aplicar la política de «Denegación por omisión»</code>, en la parte superior derecha de la tabla de reglas. Esta opción bloquea todo el tráfico que no coincida con las reglas que ha definido para el Game Firewall, es decir, que todas las aplicaciones de juego enumeradas estarán protegidas y ninguna otra conexión podrá llegar a su servidor. Esta opción reduce considerablemente la superficie de ataque expuesta a posibles actores maliciosos.
...
Tenga en cuenta que los protocolos de juego compatibles (títulos y versiones de juegos que se pueden proteger) pueden cambiar con el tiempo. Además, pueden ser diferentes entre las antiguas gamas de servidores Bare Metal Game y las más recientes. La lista más reciente de perfiles de juegos compatibles está disponible en la [página Game DDoS Protection](https://www.ovhcloud.com/es-es/security/game-ddos-protection/).
...
:::warning
La protección anti-DDoS Game se aplica después de las reglas definidas en el [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network). Para que ambos funcionen correctamente, el Edge Network Firewall no puede ser demasiado estricto y debe dejar pasar el tráfico apropiado a la protección anti-DDoS Game.
...
### Menciones específicas para determinados juegos <a name="game_specific"></a>
...
Todos los protocolos de protección anti-DDoS Game admitidos para un servidor específico están visibles en la página de configuración <code className="action">GAME firewall</code> para cualquier dirección IP que apunte a ese servidor, en el menú desplegable <code className="action">Game protocol</code>:
...
Si prefiere la automatización, los detalles del protocolo pueden recuperarse mediante [la APIv6 de OVHcloud](/guides/manage-and-operate/api/first-steps):
...
<summary>**¿Puedo utilizar la protección anti-DDoS Game en otras gamas distintas de los servidores Bare Metal Game?**</summary>
...
<summary>**¿Cómo puedo asegurarme de que la automatización funcione para una Additional IP entre una nueva y una antigua generación de servidores Bare Metal Game?**</summary>
...
<summary>**¿Puedo desactivar la protección anti-DDoS Game?**</summary>
...
<summary>**Mi juego no está en la lista de protocolos compatibles, ¿qué puedo hacer?**</summary>
...
<summary>**Después de configurar el juego con los puertos adecuados y la directiva predeterminada que se va a eliminar, sigo recibiendo ataques que afectan a mi servidor Game. ¿Qué hago?**</summary>
