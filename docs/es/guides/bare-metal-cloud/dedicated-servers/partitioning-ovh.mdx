...
description: Find out what the OVHcloud API provides in order to customize the disks, hardware/software RAID and partitioning configuration during the server OS reinstallation
...
With [OVHcloud Dedicated Servers](https://www.ovhcloud.com/es-es/bare-metal/), you can configure Disks, [hardware RAID](/guides/bare-metal-cloud/dedicated-servers/raid-hard), Partitions, [software RAID](/guides/bare-metal-cloud/dedicated-servers/raid-soft), LVM, ZFS, etc. during [OS reinstallation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server) from the [OVHcloud API](https://eu.api.ovh.com/) as well as the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. In this article, we will focus on the [OVHcloud API](https://eu.api.ovh.com/). This will give us more details about the engine that is running in the background in order to create the storage customisation on the dedicated server from the input data passed on to the OVHcloud API.
...
* A [dedicated server](https://www.ovhcloud.com/es-es/bare-metal/) **ready to be installed/reinstalled** in your OVHcloud account.
...
Some [dedicated servers](https://www.ovhcloud.com/es-es/bare-metal/) have multiple groups of disks. For example, one group with SATA disks and another group with SSD disks. Those servers are sometimes also called **hybrid servers**.
...
² The RAID level for swap can only be set to 1 within the [OVHcloud API](https://eu.api.ovh.com/). In reality, the swap partitions will not use RAID. When a swap partition with size `s` is defined on a server with `n` disks, this will create `n` partitions of size `s` on every disk without any software RAID device underneath.<br />
...
From the [OVHcloud API](https://eu.api.ovh.com/), this status can be obtained with the following API call:
