...
- Tener acceso al <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>.
...
En la línea que empieza por ```link ether``, puede comprobar que esta interfaz se corresponde con la interfaz **privada** que haya introducido en el <ManagerLink to="/">Panel de configuración de OVHcloud</ManagerLink>. Utilice este nombre de interfaz para reemplazar `NETWORK_INTERFACE` en las configuraciones siguientes (por ejemplo: `eno2`).
