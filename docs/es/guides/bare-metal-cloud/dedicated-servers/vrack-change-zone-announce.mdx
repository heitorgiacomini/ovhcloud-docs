...
description: Cómo modificar el anuncio de un bloque de IP en el vRack
...
El [vRack](https://www.ovhcloud.com/es-es/network/vrack/) es una red privada que permite configurar el direccionamiento entre dos o más [servidores dedicados](https://www.ovhcloud.com/es-es/bare-metal/) de OVHcloud.

**Esta guía explica cómo definir la zona de anuncio de un bloque de IP en el vRack.**

## Requisitos

- Tener un [vRack](https://www.ovhcloud.com/es-es/network/vrack/).
- Haber [configurado un bloque de IP en el vRack](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack).
...
Conéctese a la [API de OVHcloud](https://eu.api.ovh.com/) con su ID de cliente y contraseña. Utilice las siguientes llamadas a la API para modificar el anuncio del bloque de IP:

<Api version="v1" section="/vrack" method="GET" route={"/vrack"} />

Esta llamada a la API permite mostrar la lista de vRacks. Si el nombre mostrado en la API no le permite identificar el vRack, puede consultarlo en el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, en la sección <code className="action">Bare Metal Cloud</code>. Haga clic en <code className="action">Network</code> y seleccione <code className="action">vRack</code> en la columna izquierda y se desplegarán los vRacks existentes.

<Api version="v1" section="/vrack" method="POST" route={"/vrack/\{serviceName\}/ip"} />

Esta llamada a la API permite modificar el anuncio de un bloque de IP. Complete la información solicitada:

|Campo|Descripción|
|---|---|
|serviceName|Introduzca el nombre del servicio vRack correspondiente. Por ejemplo, `pn-12345`.|
|block|Introduzca el nombre del bloque de IP correspondiente (no se trata de la dirección IP probada en el paso anterior, sino el bloque de IP, por ejemplo, `192.0.2.0/24`).|
|region|Seleccione la nueva zona donde quiera anunciar el bloque de IP (no se trata de la zona obtenida en el paso anterior). Por ejemplo, `bexxx.gra-d1-a75.fr.eu`.|

<img className="thumbnail" alt="vrack zone" src="/images/bare-metal-cloud/dedicated-servers/vrack-change-zone-announce/vrack_announcement_zone.png" loading="lazy" />
