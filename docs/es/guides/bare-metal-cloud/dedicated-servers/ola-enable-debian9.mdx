...
Si la configuración de red de su sistema utiliza `Netplan`, consulte [esta guía](/guides/bare-metal-cloud/dedicated-servers/lacp-enable-netplan).
...
<br/>Para ello, en primer lugar, conéctese al <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>. En la pestaña <code className="action">Bare Metal Cloud</code>, haga clic en <code className="action">Servidores dedicados</code> y seleccione el servidor. A continuación, abra la pestaña <code className="action">IPMI</code> (1).

Haga clic en el botón <code className="action">Desde un applet Java (KVM)</code> (2).
