...
- Un servicio de OVHcloud expuesto en una dirección IP pública dedicada ([Servidor Dedicado](https://www.ovhcloud.com/es-es/bare-metal/), [VPS](https://www.ovhcloud.com/es-es/vps/), [instancia de Public Cloud](https://www.ovhcloud.com/es-es/public-cloud/), [Hosted Private Cloud](https://www.ovhcloud.com/es-es/hosted-private-cloud/), [Additional IP](https://www.ovhcloud.com/es-es/network/additional-ip/), etc.)
...
- **Enlace directo:** <ManagerLink to="/#/dedicated/network-security/scrubbing-center">Security Dashboard</ManagerLink>
- **Ruta de navegación:** <code className="action">Network</code> > <code className="action">Network Security Dashboard</code>
...
Durante un ataque, una acción de mitigación activa se indica mediante un icono de advertencia en la página de lista de IP (en la sección <code className="action">Gestionar las direcciones IP</code> del área de cliente).
...
:::info
Puede encontrar más información sobre cómo mitigar los ataques DDoS en OVHcloud [aquí](https://www.ovhcloud.com/es-es/security/anti-ddos/ddos-attack-mitigation/).
...
Conéctese a su <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, haga clic en <code className="action">Network</code> en el menú situado a la izquierda de la pantalla y seleccione <code className="action">Direcciones IP públicas</code>. Asegúrese de que la opción `Modo avanzado` esté activada para ver el estado de la infraestructura anti-DDoS y la configuración de sus componentes.
...
- La columna **Firewall** indica el estado del firewall de red perimetral que puede ser:
    ...
    - **(sin estado)** - No se ha creado la configuración del cortafuegos. Para configurar las reglas, haga clic en el botón <code className="action">...</code> y seleccione <code className="action">Crear firewall</code>.
...
Para obtener más información sobre cómo configurar las reglas de Edge Network Firewall, consulte nuestra guía [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network).
...
[Activación y configuración del Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network)

[Protección de un servidor GAME con el firewall de aplicaciones](/guides/bare-metal-cloud/dedicated-servers/firewall-game-ddos)
