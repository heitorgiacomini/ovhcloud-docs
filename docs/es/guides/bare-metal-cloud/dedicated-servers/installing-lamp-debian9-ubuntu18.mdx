...
description: Cómo configurar un servidor web LAMP
...
Si ya tiene instalada una distribución Debian o Ubuntu en el servidor, realice la reinstalación desde el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>. Es la mejor manera de tener un sistema propio para su servidor web y las aplicaciones que se ejecutan en él.

Consulte la guía correspondiente para instalar una distribución en su servicio OVHcloud y conecte a él por [SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction):
...
- [Instancia Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance)
...
Se recomienda utilizar el método de autenticación propuesto (*unix_socket*) en lugar del acceso con contraseña root. Pulse <code className="action">y</code> y, a continuación, <code className="action">Intro</code>. Si decide utilizar el acceso de usuario root en su lugar, elija <code className="action">n</code> y establezca una contraseña root en la siguiente imagen.
...
Para instalar WordPress en un servidor, puede seguir [este tutorial](/guides/public-cloud/compute/install-wordpress).
...
[La configuración de un cortafuegos](/guides/bare-metal-cloud/dedicated-servers/firewall-linux-iptable) (*iptables*) mejorará la seguridad del servidor. Este proceso puede simplificarse utilizando el frontend "Uncomplicated Firewall" (UFW) y su conjunto de perfiles predefinidos. 
...
Para acceder a la instalación de su servidor web a través de un dominio, es necesario asociarlo al servicio. Para ello, edite la zona DNS accesible desde el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, siempre que OVHcloud sea su agente registrador **y que** el dominio utilice los servidores DNS de OVHcloud.

Para más información, consulte la guía [Editar una zona DNS](/guides/web-cloud/domains/dns-zone-edit). Si el dominio está en uso, configure los DNS únicamente después de que el sitio web o la aplicación estén listos.
...
Como alternativa, OVHcloud le ofrece la solución [SSL Gateway](https://www.ovhcloud.com/es-es/security/ssl-gateway/). Para más información, consulte [nuestra guía](/guides/web-cloud/ssl-gateway/order).
...
[Cómo descargar y recuperar datos en un servidor mediante SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp)
