...
description: Reconstruya la configuración del espejo RAID por software en un servidor dedicado Windows tras un reemplazo de disco
...
Una vez que se haya conectado, haga clic derecho en el botón del menú <code className="action">Iniciar</code> y abra <code className="action">Ejecutar</code>.
...
Introduzca `cmd` y haga clic en <code className="action">OK</code>.
...
En este ejemplo, el `Disk 1` es un disco sustitutivo que se ha instalado para sustituir el `Disk M0` defectuoso que había sido [físicamente retirado](/guides/bare-metal-cloud/dedicated-servers/disk-replacement) anteriormente.
