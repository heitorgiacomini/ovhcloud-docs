...
description: Descubra nuestras buenas prácticas antispam y cómo desbloquear una dirección IP bloqueada para spam.
...
- Si envía spam o mensajes de correo ilegítimos, le recomendamos encarecidamente que resuelva el problema **antes** de desbloquear la dirección IP. Consulte esta guía para conocer las [mejores prácticas (EN)](/guides/bare-metal-cloud/dedicated-servers/antispam-best-practices#bestpractices) al enviar correos electrónicos. 
...
Haga clic en el botón <code className="action">⁝</code> junto a la IP o al servicio correspondiente y seleccione <code className="action">Desbloqueo de correo antispam</code>.
...
En la ventana que aparece, haga clic en <code className="action">Desbloquear la IP</code> en la parte inferior y confirme.
...
Conéctese a la interfaz de la [API de OVHcloud](https://eu.api.ovh.com/) y siga los pasos que se indican a continuación. Para más información sobre el uso de las API de OVHcloud, consulte nuestra guía [Primeros pasos con las API de OVHcloud](/guides/manage-and-operate/api/first-steps).
...
- configurar correctamente los [Reverses](/guides/bare-metal-cloud/dedicated-servers/mail-sending-optimization#configurer-le-reverse-ip) en todas las direcciones IP. 
