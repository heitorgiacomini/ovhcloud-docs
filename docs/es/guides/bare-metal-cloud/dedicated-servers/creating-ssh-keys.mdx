---
title: Crear claves SSH para acceder a un servidor dedicado
description: Cree pares de claves OpenSSH en su dispositivo local para conexiones seguras sin contraseña a su servidor dedicado
...
:::info
`Ed25519` se considera el método más seguro, pero `RSA` es una alternativa válida. Ambas son compatibles con el área de cliente de OVHcloud si desea [almacenar las claves públicas en su cuenta de cliente](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
:::
...
Si confirma con <code className="action">Entrada</code> sin escribir un nombre, se utilizará el nombre de archivo estándar (`id_rsa` en este ejemplo).
...
Copie esta cadena de clave para [añadirla a un nuevo servidor](#getstarted) o para [importarla al área de cliente](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
...
Si confirma con la tecla <code className="action">Intro</code> sin introducir un nombre, se utilizará el nombre de archivo estándar (`id_rsa` en este ejemplo).
...
Al trabajar desde una línea de comandos **Windows**, haga clic con el botón secundario del mouse para **pegar** el contenido del Portapapeles en la ventana de la línea de comandos. Para **copiar*** una cadena de la ventana de la línea de comandos, resáltela y pulse <code className="action">Intro</code>. También puede encontrar estas funciones haciendo clic derecho en la barra de menús de la ventana de la línea de comandos.
:::
...
[Introducción al protocolo SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)

[Modo de rescate en servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)

[Modo de rescate en VPS](/guides/bare-metal-cloud/virtual-private-servers/rescue)
