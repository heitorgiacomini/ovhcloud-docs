---
title: Migrar los datos de un servidor dedicado a otro
description: Migre los datos entre servidores dedicados OVHcloud con rsync, SFTP o el backup storage para una transición fluida
...
Esta guía le ayudará en la mayor medida posible a realizar las tareas habituales. No obstante, si tiene dificultades o dudas con respecto a la administración, el uso o la ejecución de los servicios en un servidor, le recomendamos que contacte con un [proveedor de servicios especializado](https://partner.ovhcloud.com/es-es/directory/).
:::
...
- La forma más sencilla es utilizar un programa adecuado como [SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp).
- La otra opción es [sincronizar los dos servidores entre sí](/guides/bare-metal-cloud/dedicated-servers/how-to-copy-data-from-one-dedicated-server-to-another-using-rsync).
...
Para más información, consulte nuestra guía sobre [utilizar Backup Storage en un servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage).
...
Si la reputación de sus direcciones IP es importante, le recomendamos encarecidamente que utilice las direcciones [Additional IP](https://www.ovhcloud.com/es-es/network/additional-ip/), ya que pueden conservarse si se migra.

Una vez que disponga de estas direcciones Additional IP, solo tiene que moverlas al nuevo servidor.
Para ello, consulte nuestra guía: [Mover una Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip).
...
Para más información, consulte nuestra documentación sobre los dominios y DNS.
