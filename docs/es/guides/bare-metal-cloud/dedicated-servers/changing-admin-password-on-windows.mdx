...
Consulte [esta guía](/guides/bare-metal-cloud/dedicated-servers/rcw-changing-admin-password-on-windows) al utilizar la opción `Windows Customer Rescue System (Windows2022-based)` en el área de cliente de OVHcloud.
...
- Tener un [servidor dedicado](https://www.ovhcloud.com/es-es/bare-metal/) con Windows instalado.
...
Para más información sobre el modo de rescate, consulte [esta guía](/guides/bare-metal-cloud/dedicated-servers/rescue-mode).

### 2. montar la partición del sistema

Conéctese al servidor por SSH. Si fuera necesario, consulte la guía [Introducción al SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction).
...
En primer lugar, sustituya el netboot por **Arrancar en el disco duro** en el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> (ver [Etapa 1](#1-reiniciar-el-servidor-en-modo-de-rescate)). 
...
En el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, acceda a la pestaña <code className="action">IPMI</code> para abrir una sesión de KVM.
...
Haga clic en <code className="action">Aceptar</code> y estará conectado.
...
En esta interfaz puede manipular el archivo *SAM* para borrar la contraseña del usuario admin. La ruta de acceso predeterminada del directorio *WINDOWS* se autocompletará. Abra el archivo para ver la lista de usuarios haciendo clic en <code className="action">Abrir</code>.

El usuario en cuestión será "admin" o "Administrator", según la versión de Windows. Si ambos están presentes, seleccione "admin". Haga clic en <code className="action">Cambiar la contraseña</code>.
...
En la nueva ventana, deje los campos vacíos y haga clic en <code className="action">Aceptar</code>. Finalice haciendo clic en <code className="action">Guardar los cambios</code> y luego en <code className="action">Salir</code>.
...
Al volver a la ventana de KVM, seleccione la opción de apagado <code className="action">Reiniciar</code> con el botón Windows "Iniciar" en la parte inferior izquierda.
...
[Utilizar IPMI en un servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/ipmi)
