...
description: Cómo crear backups de sus datos en cinco sencillos pasos
...
- Un [servidor de almacenamiento](https://www.ovhcloud.com/es-es/bare-metal/storage/) de OVHcloud.
- Una infraestructura de producción ([VPS](https://www.ovhcloud.com/es-es/vps/), [servidor dedicado](https://www.ovhcloud.com/es-es/bare-metal/), [instancia de Public Cloud](https://www.ovhcloud.com/es-es/public-cloud/), etc.).
- Conexión SSH configurada entre el servidor de almacenamiento y la infraestructura de producción.
- Una [red privada](https://www.ovhcloud.com/es-es/network/vrack/) entre los servidores (recomendado).

## Procedimiento

### 1. Elegir el nivel de RAID adecuado

Los [servidores de almacenamiento](https://www.ovhcloud.com/es-es/bare-metal/storage/) de OVHcloud incluyen varios discos duros en su configuración de hardware. Para este tutorial, vamos a utilizar un servidor con RAID por software (o softRAID) y cuatro discos con una capacidad de 6 TB cada uno.
...
Conéctese al <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> e instale el servidor.
 Como indicamos al principio, utilizaremos Debian 9.4. Para más información, consulte nuestra guía [Primeros pasos con un servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#instalar-o-reinstalar-el-servidor-dedicado).

Una vez seleccionado el sistema operativo, marque la casilla <code className="action">Personalizar la configuración de las particiones</code> y haga clic en <code className="action">Siguiente</code>.

<img className="thumbnail" alt="Personalizar la configuración de las particiones" src="/images/bare-metal-cloud/dedicated-servers/save-datas-database-on-storage-ds/custompartition.png" loading="lazy" />

Aquí podrá cambiar el tipo de RAID de su **/home** (1) y, si lo desea, aumentar el tamaño de la partición (2).

<img className="thumbnail" alt="Edición de la /home" src="/images/bare-metal-cloud/dedicated-servers/save-datas-database-on-storage-ds/partition_customization_menu.png" loading="lazy" />
