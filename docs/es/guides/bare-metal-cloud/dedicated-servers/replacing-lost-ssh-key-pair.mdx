...
description: Cómo restaurar el acceso al servidor en caso de pérdida de la clave privada generando un nuevo par de claves SSH
...
Si [utiliza llaves SSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys) para conectarse a su servidor, la pérdida de su llave SSH privada podría significar la pérdida total de acceso a su servidor.

No obstante, podrá conectarse al servidor utilizando el [modo de rescate de OVHcloud](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) con una contraseña provisional que le permitirá modificar sus archivos.
...
Cree un nuevo par de llaves SSH en su puesto de trabajo, tal y como se describe en la primera parte de la guía ["Crear claves SSH"](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys).
...
- [Modo de rescate del servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)
- [VPS modo de rescate](/guides/bare-metal-cloud/virtual-private-servers/rescue)
...
Cambie al modo de arranque "normal" y reinicie el servidor desde el <ManagerLink to="/">Panel de configuración de OVHcloud</ManagerLink>. Consulte la guía "[Activar y utilizar el modo de rescate](#step2)" si es necesario.
