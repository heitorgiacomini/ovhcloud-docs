...
description: Descubra cómo enviar SMS desde n8n utilizando la API de OVHcloud
...
- Tener una [cuenta SMS de OVHcloud](https://www.ovhcloud.com/es-es/sms/) y un [remitente SMS](/guides/web-cloud/messaging/sms/sms-senders) válido.
- Tener un [VPS de OVHcloud](https://www.ovhcloud.com/es-es/vps/), un servidor o una máquina local con [n8n](https://n8n.io/) instalado y accesible.

## Procedimiento

Si aún no ha instalado [n8n](https://n8n.io/) en su VPS, siga las instrucciones de nuestro tutorial "[Instalar n8n en un VPS de OVHcloud](/guides/bare-metal-cloud/virtual-private-servers/install-n8n-on-vps)".
...
Para ello, consulte la sección **Uso avanzado: combinar las API de OVHcloud con una aplicación** de nuestro tutorial "[Primeros pasos con las API de OVHcloud](/guides/manage-and-operate/api/first-steps)", y copie y guarde los tres identificadores `Application key`, `Application secret` y `Consumer key`.
...
- [El remitente debe estar definido en su cuenta de OVHcloud](/guides/web-cloud/messaging/sms/sms-senders). Para realizar una prueba sin declarar un remitente y utilizar un número corto, reemplace `"sender": "Your_sender"` por `"senderForResponse": true`.
:::
