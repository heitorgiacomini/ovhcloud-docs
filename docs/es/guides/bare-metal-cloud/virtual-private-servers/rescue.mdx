...
description: Descubra cómo utilizar el modo de rescate de OVHcloud para solucionar los problemas de su VPS y realizar comprobaciones del sistema
...
- [Restablecer la contraseña del usuario para recuperar el acceso](/guides/bare-metal-cloud/dedicated-servers/replacing-user-password)
...
Esta guía está diseñada para ayudarle en las tareas comunes tanto como sea posible. No obstante, le recomendamos que se ponga en contacto con un [proveedor de servicios especializados](https://partner.ovhcloud.com/es-es/directory/) o con [nuestra comunidad](https://community.ovhcloud.com/community/en) si tiene algún problema.
...
En la pestaña <code className="action">Inicio</code>, haga clic en <code className="action">...</code> junto a « Boot » en la zona **Su VPS**.
...
Seleccione <code className="action">Reiniciar en modo de rescate</code> en el menú.
...
:::info
Recibirá un mensaje de correo electrónico automático con las claves SSH para acceder al modo de rescate. Por favor, espere la recepción del email antes de continuar con cualquier acción. Este mensaje de correo electrónico también puede consultarse en el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>. Para localizarlo, haga clic en el nombre asociado a su identificador de OVHcloud en la barra de menús situada en la esquina superior derecha y seleccione <code className="action">Correo electrónico del servicio</code>.

:::


:::warning
Tenga en cuenta que si ya no es el contacto técnico del servidor, no recibirá el correo electrónico. Para obtener más información, consulte nuestra guía: «[Gestionar los contactos de los servicios](/guides/account-and-service-management/account-information/managing-contacts)».

:::


A continuación, deberá [acceder al servidor por SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction), utilizando la contraseña temporal generada para el modo de rescate.
...
<img className="thumbnail" alt="rescue modo control panel" src="/images/assets/screens/control-panel/product-selection/bare-metal-cloud/vps/cp-reboot.png" loading="lazy" />

### Resolver problemas de inicio

Si se produce un error al reiniciar un VPS, siga estos pasos:

- Compruebe el KVM en el área de cliente para obtener información sobre los motivos por los que el VPS no puede iniciarse. Para más información, consulte nuestra [guía KVM](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps).
- Si el KVM indica que el VPS está bloqueado al inicio o no puede encontrar el disco, asegúrese de que [los logs de inicio están activados](/guides/bare-metal-cloud/virtual-private-servers/bootlog-display-kvm). Envíe los logs pertinentes a nuestro equipo de soporte para más investigaciones [creando una solicitud de asistencia](https://help.ovhcloud.com/csm?id=csm_get_help).
...
[Recuperar el acceso al servidor en caso de pérdida de la contraseña de usuario](/guides/bare-metal-cloud/dedicated-servers/replacing-user-password)

[Sustituir un par de claves SSH](/guides/bare-metal-cloud/dedicated-servers/replacing-lost-ssh-key-pair)
