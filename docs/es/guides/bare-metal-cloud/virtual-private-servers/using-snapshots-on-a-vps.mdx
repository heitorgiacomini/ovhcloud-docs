...
description: Cómo activar y usar la opción Instantáneas  en el panel de control de OVHcloud
...
Acceda a la pestaña <code className="action">Inicio</code> y desplácese hacia abajo hasta el cuadro **Copia de seguridad**  
. Si el estado de "Snapshot" ya es "Activado", vaya directamente al paso 2. Si "Snapshot" todavía no está activado, haga clic en <code className="action">...</code> junto a la opción "Snapshot" y, seguidamente, en <code className="action">Contratar</code> (en el menú contextual).
...
En el siguiente paso, preste atención a la información sobre los precios y, a continuación, haga clic en <code className="action">Contratar</code>. Se le guiará por el proceso de contratación y recibirá un mensaje de correo electrónico de confirmación.

Para dar de baja la opción de servicio, haga clic en el botón <code className="action">...</code> situado junto a "Snapshot" y seleccione <code className="action">Dar de baja</code> en el menú contextual.
...
Una vez activada la opción, haga clic en <code className="action">...</code> junto a la opción «Instantáneas» y, seguidamente, en «Tomar una instantánea» (en el menú contextual). Puede escribir una descripción que se asociará al snapshot. El tiempo de creación del snapshot depende del espacio de almacenamiento utilizado. Después, aparecerá la fecha y hora de su creación en el cuadro «Copia de seguridad».

### Paso 3: Eliminar o restaurar una instantánea

Puesto que las instantáneas solo se pueden activar de una en una, se debe eliminar la instantánea existente antes de crear una nueva. En el menú contextual, seleccione <code className="action">Eliminar la instantánea</code>.
...
Si está seguro de que desea restablecer su servidor virtual privado (VPS) al estado de la instantánea, haga clic en <code className="action">Restaurar la instantánea</code> y confirme la acción de restauración en la ventana emergente.
...
Si la función snapshot es demasiado limitada para su proyecto, la opción [Backup automatizado](/guides/bare-metal-cloud/virtual-private-servers/using-automated-backups-on-a-vps) es una alternativa.
...
El snapshot actual puede recuperarse mediante un enlace de descarga. Haga clic en el botón <code className="action">...</code> situado junto a la opción `Snapshot` y seleccione <code className="action">Descargar el snapshot</code> en el menú contextual.
...
El archivo cargado puede importarse en un proyecto de Public Cloud como imagen (QCOW2) mediante [OpenStack](/guides/public-cloud/compute/overview). (Consulte un ejemplo de uso en [esta guía](/guides/public-cloud/compute/upload-own-image).)
...
Puede instalar el software mediante un archivo MSI disponible en el sitio web del proyecto Fedora: [https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/latest-qemu-ga/](https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/latest-qemu-ga/)
