...
description: Cómo diagnosticar un VPS utilizando los logs de arranque (boot logs)
...
Si su VPS no responde, siempre debería poder acceder a él desde el área de cliente a través del [KVM](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps). La forma más rápida de diagnosticar el problema es comprobar los logs de arranque (boot logs) del servidor. No obstante, la configuración GRUB debe modificarse para que aparezcan estos logs. 
...
Si necesita ayuda para realizar estas acciones, póngase en contacto con un proveedor de servicios especializado o intercambie información con nuestra comunidad de usuarios en [https://community.ovh.com/en/](https://community.ovh.com/en/). OVHcloud no podrá ofrecerle soporte técnico.
:::
...
Reinicie el servidor en modo de rescate desde el área de cliente. Si lo necesita, consulte nuestra [guía sobre el modo de rescate](/guides/bare-metal-cloud/virtual-private-servers/rescue).
...
Una vez realizados los cambios, reinicie su VPS en modo "normal" desde el <ManagerLink to="/">Panel de configuración de OVHcloud</ManagerLink>. Los logs de arranque deben aparecer al utilizar la [consola KVM](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps).

## Más información

[Utilizar el KVM en un VPS](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps)
