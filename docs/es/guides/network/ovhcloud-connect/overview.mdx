---
pageType: overview
title: OVHcloud Connect
description: "Conecte su red local a OVHcloud de forma privada y con gran ancho de banda - configuraciones físicas, virtuales y basadas en POP de OVHcloud Connect."
text: "Explore las guías de OVHcloud Connect"
essentialsTitle: "Guías esenciales para empezar"
essentials:
  - title: Installation of OVHcloud Connect Direct from the OVHcloud Control Panel
    link: /guides/network/ovhcloud-connect/occ-direct-control-panel-setup
  - title: Installation of OVHcloud Connect Provider from the OVHcloud Control Panel
    link: /guides/network/ovhcloud-connect/provider-control-panel-setup
  - title: How to initiate a diagnostic for OVHcloud Connect from the OVHcloud Control Panel
    link: /guides/network/ovhcloud-connect/diagnostics
  - title: OVHcloud Connect Logs Forwarding
    link: /guides/network/ovhcloud-connect/logs-to-customers
  - title: FAQ OVHcloud Connect
    link: /guides/network/ovhcloud-connect/faq
goFurther:
  title: "Más información"
  items:
    - title: Visite la comunidad de OVHcloud
      link: https://community.ovh.com/
    - title: "Consulte la roadmap & changelog"
      link: https://www.ovhcloud.com/es/roadmap-changelog/
    - title: "Únase a Discord"
      link: https://discord.gg/ovhcloud
cta:
  title: "¿Preguntas? Acceda al soporte de OVHcloud"
  description: "Encuentre la respuesta a sus preguntas en nuestro centro de ayuda."
  actions:
    - text: Acceder al soporte
      link: https://help.ovhcloud.com/csm/es-es-home
      theme: brand
---
