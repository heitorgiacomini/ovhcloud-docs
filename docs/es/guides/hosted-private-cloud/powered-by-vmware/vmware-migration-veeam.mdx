...
description: Learn how to migrate your on-prem VMware workloads to an OVHcloud Hosted Private Cloud environment using Veeam Replication
...
For SecNumCloud environments, please refer to the dedicated guide: [Move2Cloud - Migrating VMware Workloads to SecNumCloud Hosted Private Cloud with Veeam Replication](/guides/hosted-private-cloud/powered-by-vmware/vmware-migration-veeam-secnumcloud)
...
<img className="thumbnail" alt="Move2Cloud" src="/images/hosted-private-cloud/powered-by-vmware/vmware-migration-veeam/Move2PCC_Veam.png" loading="lazy" />
...
With [OVHcloud vRack](https://www.ovhcloud.com/es-es/network/vrack/), you can create up to 4,000 VLANs, which allows you to replicate your current IP address plan without any re-IP.
...
For internet-exposed services, allocate additional public IPs or use the [Bring Your Own IP (BYOIP)](https://www.ovhcloud.com/es-es/network/byoip/) feature to retain your existing IP addresses.
...
2. Navigate to the <code className="action">Hosted Private Cloud</code> section and open the Secure SSL Gateway.
3. Add the IP addresses that should have access to vCenter.

You can find detailed instructions in our [IP whitelisting guide](/guides/hosted-private-cloud/powered-by-vmware/authorise-ip-vcenter-access).

### Step 4: Configure roles and permissions

#### Step 4.1: Use OVHcloud IAM

Set up roles and permissions in your <code className="action">Hosted Private Cloud</code> using `OVHcloud IAM`.
...
For instructions, refer to the [IAM setup guide](/guides/hosted-private-cloud/powered-by-vmware/vmware-iam-getting-started).
...
Follow the steps in [this guide](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-adfs).
...
Refer to the [NSX first steps guide](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps) for more details.
...
Activate its license using the [setup guide](/guides/storage-and-backup/backup-and-disaster-recovery-solutions/veeam/veeam-veeam-backup-replication).
...
- [NSX VPN setup](/guides/hosted-private-cloud/powered-by-vmware/nsx-configure-ipsec)
...
For higher bandwidth and lower latency, you can subscribe to [OVHcloud Connect](https://www.ovhcloud.com/es-es/network/ovhcloud-connect/), which provides an MPLS-like link between your on-prem infrastructure and your OVHcloud tenant.

With [OVHcloud Connect](https://www.ovhcloud.com/es-es/network/ovhcloud-connect/), latency is controlled, and you can choose the bandwidth you need, from 200 Mbps up to several Gbps.
...
1. Open the <code className="action">Replication Jobs</code> section in the Veeam Console.
2. Add the <code className="action">source VMs</code> and configure the target HPC environment.
...
Run your replication jobs and test their functionality using Veeam’s <code className="action">Failover Now</code> feature. Revert to the original state using <code className="action">Undo Failover</code>.
...
Perform a <code className="action">Planned Failover</code> on migration day to synchronize final changes, power off the source VMs, and activate the replicas in HPC.
...
Use the <code className="action">Permanent Failover</code> option in Veeam to finalize your migration. This will make the replicas in HPC the primary production VMs.
...
Move VMs to target storage using [Storage vMotion](/guides/hosted-private-cloud/powered-by-vmware/storage-vmotion) and configure backup jobs in [OVHcloud Object Storage](https://www.ovhcloud.com/es-es/public-cloud/object-storage/) to protect your workloads.
...
2. **Use Storage vMotion:**
    - Open the <code className="action">vSphere Client</code> and navigate to the VM you wish to move.
    - Right-click the VM and select <code className="action">Migrate</code>.
    - Choose the <code className="action">Change storage only</code> option and select the target datastore (vSAN or NFS).
...
For more information, refer to the [Storage vMotion guide](/guides/hosted-private-cloud/powered-by-vmware/storage-vmotion).
...
1. **Define Backup Storage:**
    - Use OVHcloud **[S3*-compatible Object Storage](https://www.ovhcloud.com/es-es/public-cloud/object-storage/)** as a backup repository for scalability and cost efficiency.
2. **Create a Backup Job in Veeam:**
    - Open the **Veeam Console** and navigate to the <code className="action">Home</code> tab.
    - Click <code className="action">Backup Job</code> > <code className="action">Virtual Machine</code> and follow the wizard:
...
Setting up regular backups guarantees that your workloads are protected against data loss or corruption. For detailed instructions, refer to the [Veeam S3 backup guide](/guides/storage-and-backup/object-storage/s3-veeam).
...
- [Veeam Managed Backup](https://www.ovhcloud.com/es-es/hosted-private-cloud/vmware/veeam-managed-backup/): A fully managed backup solution by OVHcloud.
- [Zerto for VMware on OVHcloud](https://www.ovhcloud.com/es-es/hosted-private-cloud/vmware/zerto/): Disaster recovery and replication across regions.
