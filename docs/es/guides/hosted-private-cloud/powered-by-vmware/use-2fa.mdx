...
Una vez que se haya conectado a la interfaz de gestión, haga clic en <code className="action">cambiar contraseña —<i>Change Passwrod</i>—</code>
...
Acceda a la sección <code className="action">de validación de la operación —<i>Operation Validation</i>—</code>, cargue la operación recibida por SMS y confirme con el token recibido en ese mismo SMS.
...
Es preciso autorizar estas aplicaciones previamente a través de la política de acceso a la utilidad vCenter, lo cual se puede configurar en el [área de cliente](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud).
