...
description: Find out how to prepare a migration from managed VMware vSphere on OVHcloud to a solution based on a Public VCF as-a-Service environnement
...
- A managed [VMware vSphere on OVHcloud](https://www.ovhcloud.com/es-es/hosted-private-cloud/vmware/) offer.
- You must have access to the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> and be technical administrator of the managed VMware vSphere on OVHcloud infrastructure.
...
- [OVHcloud.com - Public VCF as-a-Service](https://www.ovhcloud.com/es-es/lp/vmware-vcd-evolution/)
...
We recommend reading our guide [Public VCF as-a-Service - The fundamentals of Public VCF as-a-Service](/guides/hosted-private-cloud/powered-by-vmware/vcd-get-concepts) to see which features are included in each migration wave of your environments.
...
|      3️⃣      | 🔐 `Encryption VMs (KMS/OKMS or vNKP)`               | Decrypt or disable the encryption policy for VMs | - It is not possible to migrate with VMs, vApps encrypted in VMware vSphere on OVHcloud.                                                                                                                                | [Enabling Virtual Machine Encryption (VM Encryption)](/guides/hosted-private-cloud/powered-by-vmware/vm-encrypt)                                                                                                                                                                                                                                                                                                                                                                    |
|      4️⃣      | 💾 `Zerto`                                           | No solution for now                              | - If you are using Zerto solutions (data continous replication for disaster recovery), you cannot make Zerto work with Public VCF as-a-Service (for now).                                                               | [Setting up Zerto Virtual Replication between two OVHcloud datacentres](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-vmware-vsphere-drp)                                                                                                                                                                                                                                                                                                                      |
...
| **Checklist**  | **Use cases**                               | **Goals**                                                                                                                                        | **Additional information**                                                                                                                                                                           | **Help and references**                                                                                                                                                                                |
|:--------------:|:--------------------------------------------|:-----------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|      5️⃣       | 🚫 `FT (fault tolerance)`                     | Disable VM Fault Tolerance in VMware vSphere on OVHcloud                                                                           | - Right-click your VMs and select <code className="action">Fault Tolerance</code> > <code className="action">Disable Fault Tolerance</code> in managed VMware vSphere on OVHcloud                                                           | [VMware fault tolerance](/guides/hosted-private-cloud/powered-by-vmware/fault-tolerance-continuity)                                                                                                       |
|      6️⃣       | ⚠️ `Affinity/anti-affinity rules DRS`       | Reconstruction of affinity/anti-affinity rules in Public VCF as-a-Service                                                                              | - To be retained, DRS affinity/anti-affinity rules will have to be manually recreated by you in Public VCF as-a-Service after migration (for now).                                                           | [VMware DRS distributed resource scheduler](/guides/hosted-private-cloud/powered-by-vmware/drs-distributed-ressource-scheduler)                                                                      |
|      7️⃣       | 📀 `Special devices (CD, DVD, etc...)`      | Unplug all special equipment in VMware vSphere on OVHcloud                                                                             | - All special devices (CDs, DVDs, etc.) must be removed prior to migration, otherwise they will be removed by the migration process (for now).                                                       | [Modify virtual machine resources](/guides/hosted-private-cloud/powered-by-vmware/modify-virtual-machine-hardware-configuration)                                              |
|      8️⃣       | 🛢 `Datastore clusters`                     | Delete all clustering rules in VMware vSphere on OVHcloud                                                                       | - Clustering rules must be removed before migration, as this concept no longer exists with Public VCF as-a-Service                                                                                           | [Cluster creation and EVC activation](/guides/hosted-private-cloud/powered-by-vmware/create-cluster-enable-evc)                                                                  |
|      9️⃣       | 🔄 `Over-committed memory`                  | Plan or scale your resource requirements in Public VCF as-a-Service.<br/>Or optimize your requirements before migrating (vSphere control panel side) | - Because you cannot over-commit resources within Public VCF as-a-Service. This concept does not exist.                                                                                                      | [Modify virtual machine resources](/guides/hosted-private-cloud/powered-by-vmware/modify-virtual-machine-hardware-configuration)                                          |
...
|       ⏸️       | 🆓 `Hosts + Datastore`                      | Free up resources (hosts + datastore) in VMware vSphere on OVHcloud                                                                        | - Free "Freespare" and "Hourly" resources (hosts + datastores) must be released before the migration, or converted into monthly resources ("Monthly").                                               | [Hosted Private Cloud billing information](/guides/account-and-service-management/managing-billing-payments-and-services/facturation-private-cloud)                                             |
...
For more information, please refer to our guide: [Public VCF as-a-Service - Veeam Data Platform backup](/guides/hosted-private-cloud/powered-by-vmware/vcd-backup)
...
- [Public VCF as-a-Service - Getting started](/guides/hosted-private-cloud/powered-by-vmware/vcd-getting-started-dashboard-overview)
- [Public VCF as-a-Service - Public VCF as-a-Service fundamental concepts](/guides/hosted-private-cloud/powered-by-vmware/vcd-get-concepts)
- [Public VCF as-a-Service - Frequently Asked Questions](/guides/hosted-private-cloud/powered-by-vmware/vcd-faq)
- [Public VCF as-a-Service - Log in from the Public VCF as-a-Service control panel](/guides/hosted-private-cloud/powered-by-vmware/vcd-logging)
- [Public VCF as-a-Service - Network concepts and best practices](/guides/hosted-private-cloud/powered-by-vmware/vcd-network-concepts)
...
If you need training or technical assistance to implement our solutions, please contact your Technical Account Manager or click on [this link](https://www.ovhcloud.com/es-es/professional-services/) to get a quote and ask our Professional Services experts for a custom analysis of your project.
