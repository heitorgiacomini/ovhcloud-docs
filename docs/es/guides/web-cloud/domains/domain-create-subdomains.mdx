...
- Tener una <Tooltip term="dnsZone">zona DNS</Tooltip> activa para el nombre de dominio. Si lo necesita, consulte nuestra guía "[Crear una zona DNS en OVHcloud](/guides/web-cloud/domains/dns-zone-create)".
- Disponer de los derechos necesarios sobre todos los servicios afectados. Para más información, consulte nuestra guía [Gestionar los contactos de sus servicios](/guides/account-and-service-management/account-information/managing-contacts).
...
- [Asociar, autorizar y configurar su subdominio con un servicio de OVHcloud](#link-subdomain)
    ...
    <Region zones={['eu']}>
    - [Caso 3 - Crear direcciones Email Pro con un subdominio](#link-subdomain-case-3)
    </Region>
...
<summary>¿Cuál es la composición de un nombre de dominio?</summary>
...
<summary>¿Qué es un subdominio?</summary>
...
- *dns1.example.com* y *dns2.example.com* para personalizar sus servidores DNS utilizando los [Glue Records](/guides/web-cloud/domains/glue-registry);
...
Para más información sobre las zonas DNS, consulte nuestra guía "[Crear una zona DNS de OVHcloud](/guides/web-cloud/domains/dns-zone-create) " y "[Editar una zona DNS de OVHcloud](/guides/web-cloud/domains/dns-zone-edit) ".
...
1: Para identificar la zona DNS activa de un nombre de dominio registrado en OVHcloud, consulte nuestra guía "[Cambiar los servidores DNS de un nombre de dominio en OVHcloud](/guides/web-cloud/domains/dns-server-edit)".

2: Si su nombre de dominio no está registrado con OVHcloud, contacte con el actual agente registrador* para conocer el lugar en el que está alojada su zona DNS activa. Puede consultar nuestra guía "[Transferir un nombre de dominio a OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain)" si desea realizar esta acción.
...
Para añadir sus subdominios a la zona DNS activa de su nombre de dominio, consulte nuestra guía "[Editar una zona DNS de OVHcloud](/guides/web-cloud/domains/dns-zone-edit) ".
...
- Los servidores de correo (registros DNS de tipo <Tooltip term="mxRecord">*MX*</Tooltip>) a los que su subdominio debe redirigir los emails que recibe. Esto le permite consultarlas en su dirección de correo personalizada con su subdominio.
- Información relativa a la seguridad y la autenticación de los servicios (alojamiento web, servidor web, correo electrónico...) asociados a uno de sus subdominios (registros DNS de tipo <Tooltip term="spf">*SPF*</Tooltip>, <Tooltip term="dkim">*DKIM*</Tooltip>, <Tooltip term="dmarc">*DMARC*</Tooltip>, etc.).

:::info
La modificación de una zona DNS asociada a un nombre de dominio conlleva un retraso de <Tooltip term="dnsPropagation">propagación</Tooltip> de **4** a **24** horas como máximo para que sea efectiva.
...
Al igual que para un nombre de dominio y para autorizar a un subdominio a mostrar el contenido de una carpeta "destino" presente en un alojamiento web, conéctese a su <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> y seleccione <code className="action">Web Cloud</code>. En la columna izquierda, haga clic en <code className="action">Alojamientos</code>, seleccione el alojamiento correspondiente y abra la pestaña <code className="action">Multisitio</code>.

Aquí es donde autoriza el acceso de su subdominio al alojamiento web en el que se encuentra su sitio web.

Para más información sobre la configuración de un nombre de dominio o subdominio en un alojamiento web, consulte nuestra guía "[Alojar varios sitios web en un mismo hosting](/guides/web-cloud/web-hosting/multisites-configure-multisite)". El procedimiento es el mismo para un nombre de dominio o subdominio.
...
Para desbloquear la creación de direcciones de correo Exchange personalizadas con un subdominio, conéctese al <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> y seleccione <code className="action">Web Cloud</code>. Haga clic en <code className="action">Microsoft</code> en la columna de la izquierda y, a continuación, en <code className="action">Exchange</code>. A continuación, seleccione la plataforma Exchange que quiera utilizar con su subdominio. A continuación, abra la pestaña <code className="action">Dominios asociados</code> y haga clic en el botón <code className="action">Añadir un dominio</code> situado a la derecha.
...
- [Añadir un dominio a una plataforma de correo](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-adding-domain)
- [Añadir un registro CNAME para validar su dominio en su servicio de correo](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname)

</Region>

<Region zones={['eu']}>

#### Caso 3 - Crear direcciones Email Pro con un subdominio <a name="link-subdomain-case-3"></a>

Para desbloquear la creación de direcciones Email Pro personalizadas con un subdominio, conéctese al <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> y seleccione <code className="action">Web Cloud</code>. Haga clic en <code className="action">Email Pro</code> y seleccione la plataforma Email Pro que quiera utilizar con su subdominio. En la nueva página, abra la pestaña <code className="action">Dominios asociados</code> y haga clic en el botón <code className="action">Añadir un dominio</code> situado a la derecha.
...
[Añadir un nombre de dominio a una plataforma de correo](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-adding-domain)

[Añadir un registro CNAME para validar su nombre de dominio en su solución de correo](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname)
