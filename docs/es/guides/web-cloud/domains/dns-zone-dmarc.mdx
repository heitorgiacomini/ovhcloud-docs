...
El registro **D**omain-based **M**essage **A**uthentication, **R**eporting, and **C**onformance (DMARC) es un mecanismo de seguridad del correo electrónico. Se basa en los resultados de las comprobaciones [SPF](/guides/web-cloud/domains/dns-zone-spf) y [DKIM](/guides/web-cloud/domains/dns-zone-dkim).
...
- Uno de los mecanismos de autenticación, [SPF](/guides/web-cloud/domains/dns-zone-spf) y/o [DKIM](/guides/web-cloud/domains/dns-zone-dkim), debe configurarse en la zona DNS del nombre de dominio de la solución de correo.
...
El DMARC permite al titular de un nombre de dominio gestionar la seguridad de los mensajes de correo electrónico enviados con su nombre de dominio. Su objetivo es:
...
- Mejorar el control del uso del nombre de dominio y detectar los intentos de usurpación mediante los informes enviados en caso de fallo de autenticación del correo. Además, también mejora la seguridad al crear el enlace entre los protocolos SPF y DKIM.

El registro DMARC contiene información sobre la política que debe aplicarse a los mensajes de correo electrónico maliciosos que intentan usurpar su nombre de dominio.<br/>
DMARC consulta los mecanismos de autenticación [SPF](/guides/web-cloud/domains/dns-zone-spf) y [DKIM](/guides/web-cloud/domains/dns-zone-dkim) para comprobar los mensajes de correo electrónico entrantes.<br/>
...
El registro DMARC comunica la política que debe adoptarse en función del resultado de las pruebas SPF y DKIM. También puede introducir una o varias direcciones de correo electrónico (representadas en nuestro ejemplo por la dirección **report@mydomain.ovh**) que sirven para recibir los informes de fallos de correo enviados desde el nombre de dominio **mydomain.ovh**.

Tras leer las instrucciones del registro DMARC del nombre de dominio **mydomain.ovh** por el servidor de recepción de **«otherdomain.ovh»**, los mensajes de correo se enviarán a la dirección **recipient@otherdomain.ovh**, o bien se marcarán como «SPAM» o se rechazarán.

<img className="thumbnail" alt="dmarc" src="/images/assets/schemas/emails/dns-dmarc-diagram.png" loading="lazy" />
...
Puede añadir el registro DMARC a su zona DNS desde el área de cliente de OVHcloud. Para ello, conéctese al <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> y acceda a la sección <code className="action">Web Cloud</code>. En la columna izquierda, seleccione el nombre de dominio en la sección <code className="action">Nombres de dominio</code> y abra la pestaña <code className="action">Zona DNS</code> para acceder a su zona DNS.

Una vez que haya visualizado la zona DNS, haga clic en el botón <code className="action">Añadir un registro</code> y seleccione «Campos de correo» <code className="action">DMARC</code>.

- **Subdominio**: esta entrada debe **empezar por** `_dmarc`. Si aplica su DMARC a todo el nombre de dominio, no introduzca nada más que `_dmarc` en esta casilla. Si define su DMARC a un subdominio de su nombre de dominio principal, añada su subdominio después de `_dmarc`. Por ejemplo, si se debe aplicar el DMARC a un subdominio.*mydomain.ovh*, hay que introducir `_dmarc.subdomain` en la casilla «subdominio» para el nombre de dominio *mydomain.ovh*.
...
- **Regla para el dominio (p=)**: política que debe adoptar el destinatario a petición del titular del nombre de dominio remitente. La política se aplica al nombre de dominio consultado y a los subdominios, a menos que la etiqueta de subdominio **sp=** indique instrucciones diferentes. Los valores posibles son los siguientes:
    - *none*: el titular del nombre de dominio no solicita ninguna acción específica relativa a la entrega de mensajes.
...
:::warning
La configuración del parámetro `p=` puede afectar considerablemente a la entrega de los mensajes de correo electrónico del nombre de dominio. Es recomendable configurar `p=none` y realizar un análisis de los informes de fallos durante varias semanas para corregir cualquier anomalía. Pasar a `p=quarantine` o `p=reject` requiere un control total de los parámetros de seguridad del correo electrónico, relativos al [SPF](/guides/web-cloud/domains/dns-zone-spf) y al [DKIM](/guides/web-cloud/domains/dns-zone-dkim). El uso del factor `pct=`, que se muestra a continuación, permite realizar una transición gradual.

:::


- **Porcentaje de mensajes filtrados (pct=)** (valor entre 0 y 100, el valor predeterminado es 100): porcentaje del flujo de mensajes al que se debe aplicar la política DMARC. El objetivo de la etiqueta «pct» es permitir a los titulares de nombres de dominio adoptar una implementación lenta del mecanismo DMARC.
...
- **Regla para los subdominios (sp=)**: política que debe adoptar el destinatario para todos los subdominios. Se aplica únicamente a los subdominios del nombre de dominio consultado y no al nombre de dominio en sí. Su sintaxis es idéntica a la de la etiqueta «p» definida anteriormente. Si esta etiqueta no está presente, la política especificada por la etiqueta 'p' se aplica a los subdominios.
...
- **Modo de alineación para SPF (aspf=)** (el valor predeterminado es `r`): especifica el modo de alineación SPF. Los valores son los siguientes:
    - `R`(Relaxed) para el modo flexible: los mensajes de correo electrónico pueden enviarse, por ejemplo, desde un subdominio del nombre de dominio declarado. Esto se conoce como alineación parcial.
    - `s`(strict) para el modo strict: los mensajes de correo deben enviarse desde el nombre de dominio declarado y únicamente desde este. El resultado es «alineado».
...
- **Alineado**: cuando la dirección *john.smith@mydomain.ovh* transmite un mensaje desde el servicio de correo asociado al nombre de dominio *mydomain.ovh* y se han configurado los mecanismos de autenticación SPF y DKIM, se obtiene un resultado alineado.
- **Parcialmente alineado**: cuando la dirección *john.smith@subdomain.mydomain.ovh* transmite un mensaje desde el servicio de correo asociado al nombre de dominio *mydomain.ovh*, pero los mecanismos de autenticación SPF y DKIM se han configurado únicamente en el nombre de dominio principal (es decir, *mydomain.ovh*), se obtiene un resultado parcialmente alineado.
...
<img className="thumbnail" alt="dmarc" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/dns-dmarc-01.png" loading="lazy" />

#### Registro TXT <a name="txt-record"></a>

Puede añadir el registro TXT a su zona DNS desde el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink> y accediendo a la sección <code className="action">Web Cloud</code>. Haga clic en <code className="action">Dominios</code> y seleccione el nombre de dominio correspondiente. A continuación, abra la pestaña <code className="action">Zona DNS</code>.

Una vez que haya visualizado la zona DNS, haga clic en el botón <code className="action">Añadir un registro</code> y seleccione «Campos extendidos» <code className="action">TXT</code>.
...
La configuración del parámetro `p=` puede afectar considerablemente a la entrega de los mensajes de correo electrónico del nombre de dominio. Se recomienda configurar `p=none` y realizar un análisis de los informes de errores durante varias semanas para corregir cualquier anomalía. Pasar a `p=quarantine` o `p=reject` requiere un control total de los parámetros de seguridad del correo electrónico, relativos al [SPF](/guides/web-cloud/domains/dns-zone-spf) y al [DKIM](/guides/web-cloud/domains/dns-zone-dkim). El uso del factor `pct=`, que se muestra a continuación, permite realizar una transición gradual.
...
<img className="thumbnail" alt="dmarc" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/dns-dmarc-03.png" loading="lazy" />
...
<img className="thumbnail" alt="dmarc" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/dns-dmarc-04.png" loading="lazy" />
...
- **pct=100**: La política DMARC se aplica al 50% de los mensajes procedentes del flujo de correo del titular del nombre de dominio.
