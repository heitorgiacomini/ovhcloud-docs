...
Consulte nuestras guías "[Todo sobre los servidores DNS](/guides/web-cloud/domains/dns-server-general-information)" y "[Todo sobre la zona DNS](/guides/web-cloud/domains/dns-zone-general-information)" para más información.
...
- Ser titular de un [dominio](https://www.ovhcloud.com/es-es/domains/) registrado en OVHcloud.
- Disponer de los permisos [necesarios para gestionar](/guides/account-and-service-management/account-information/managing-contacts) el dominio.
...
- **Ruta de navegación:** <code className="action">Web Cloud</code> > <code className="action">Dominios</code> > Seleccione su dominio
...
- Si no desea modificar los servidores DNS sino uno o varios registros de su configuración/zona DNS actual, consulte nuestra guía: "[Editar una zona DNS de OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
...
:::info
Cuando utiliza los servidores DNS de OVHcloud, los números presentes en los nombres de los servidores no tienen ninguna relación con el servicio o servicios que utiliza. Solo la opción [DNS anycast](https://www.ovhcloud.com/es-es/domains/options/) utiliza servidores DNS específicos (`ns200.anycast.me` y `dns200.anycast.me`). Cuando los contrata, se le asignan automáticamente.
...
<summary>Opción 1 - Utilizar los DNS por defecto de OVHcloud</summary>
...
:::info
Si es necesario, consulte las guías "[Editar una zona DNS de OVHcloud](/guides/web-cloud/domains/dns-zone-edit)" y/o "[Crear una zona DNS en OVHcloud](/guides/web-cloud/domains/dns-zone-create)" para comprobar si existe una zona DNS de OVHcloud para su dominio.
...
<Tabs>
  ...
  <Tab label="Paso 2">
    Seleccione la pestaña <code className="action">Servidores DNS</code> una vez situado en el dominio.
  </Tab>
  <Tab label="Paso 3">
...
Haga clic en el botón <code className="action">Modificar los servidores DNS</code> situado a la derecha de la tabla "servidores DNS". Según la resolución de su pantalla, el botón puede encontrarse debajo de la tabla.
  </Tab>
  <Tab label="Paso 4">
    <img className="thumbnail" alt="Modificar servidores DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-servers/modify-my-dns-servers-1.png" loading="lazy" />

Para utilizar los servidores DNS por defecto de OVHcloud, haga clic en <code className="action">Aplicar la configuración</code>. Aparecerá la siguiente ventana:
    
    <img className="thumbnail" alt="Modificar servidores DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-servers/modify-my-dns-servers-1-apply-configuration.png" loading="lazy" />
...
Muestra los nombres de los 2 servidores DNS que se aplicarán a su dominio. Deben tener una de las 3 formas siguientes:
    ...
    Si corresponden a los que desea aplicar, haga clic en <code className="action">Aplicar</code>.
...
Haga clic en el botón <code className="action">Modificar los servidores DNS</code> situado a la derecha de la tabla "servidores DNS". Según la resolución de su pantalla, el botón puede encontrarse debajo de la tabla.
  ...
  <Tab label="Paso 4">
    <img className="thumbnail" alt="Modificar servidores DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-servers/modify-my-dns-servers-2.png" loading="lazy" />
...
Para introducir uno de sus propios servidores DNS, rellene los 2 campos del formulario del recuadro como se indica a continuación:
  ...
  <Tab label="Paso 5">
    Una vez introducida la información, haga clic en el botón <code className="action">+</code> situado a la derecha de los 2 campos del formulario. Esto añade el servidor DNS y muestra un nuevo recuadro de entrada debajo del anterior.
    
    Repita la operación para cada servidor DNS que desee añadir, respetando los límites indicados en la nota informativa.
    Haga clic en el botón <code className="action">+</code> para cada servidor DNS con el fin de validar su entrada y adición.
    
    Una vez añadidos todos sus servidores DNS, haga clic en <code className="action">Aplicar la configuración</code>. Aparecerá la siguiente ventana:
    
    <img className="thumbnail" alt="Modificar servidores DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-servers/modify-my-dns-servers-2-apply-configuration.png" loading="lazy" />
...
2. La opción [DNSSEC](/guides/web-cloud/domains/dns-dnssec) debe estar desactivada para combinar el uso de sus propios servidores DNS con los DNS de OVHcloud.
3. No mezcle un grupo de servidores DNS de OVHcloud con otro grupo de servidores DNS de OVHcloud. En OVHcloud, los grupos de servidores DNS se identifican por el número presente en los nombres de los servidores. Dos servidores DNS de OVHcloud forman parte del mismo grupo de servidores DNS cuando comparten el mismo número.

<Region zones={['eu']}>

Por ejemplo, *dns19.ovh.net* y *ns19.ovh.net* corresponden a un grupo de servidores DNS de OVHcloud, van a la par y están sincronizados. Forman parte del mismo grupo porque comparten el mismo número.

</Region>

<Region zones={['ca', 'apac']}>

Por ejemplo, *dnsXX.ovh.ca* y *nsXX.ovh.ca* corresponden a un grupo de servidores DNS de OVHcloud, van a la par y están sincronizados (por ejemplo, *dns10.ovh.ca* y *ns10.ovh.ca*). Forman parte del mismo grupo porque comparten el mismo número. Los servidores exactos asignados a su dominio se indican en el área de cliente de OVHcloud.
...
Por ejemplo: desea declarar los servidores DNS *ns1.dns-server.tld*, *dnsXX.ovh.ca* y *nsXX.ovh.ca* para su dominio. Deberá entonces comprobar que los tres registros de tipo "NS" siguientes están presentes en las 3 zonas DNS alojadas en estos 3 servidores DNS:

- "Your own domain (or just an @)" IN NS ns1.dns-server.tld.
- "Your own domain (or just an @)" IN NS dnsXX.ovh.ca.
- "Your own domain (or just an @)" IN NS nsXX.ovh.ca.
...
Haga clic en el botón <code className="action">Modificar los servidores DNS</code> situado a la derecha de la tabla "servidores DNS". Según la resolución de su pantalla, el botón puede encontrarse debajo de la tabla.
    ...
    <img className="thumbnail" alt="Modificar servidores DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-servers/modify-my-dns-servers-3.png" loading="lazy" />
  </Tab>
  <Tab label="Paso 4">
    Para introducir uno de sus propios servidores DNS, rellene los 2 campos del formulario del recuadro como se indica a continuación:
  ...
  <Tab label="Paso 5">
    ...
    <img className="thumbnail" alt="Modificar servidores DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-servers/modify-my-dns-servers-3-apply-configuration.png" loading="lazy" />
...
- El *registro* que gestiona la extensión de su dominio (por ejemplo, el registro de las extensiones *.fr*) debe ser informado de la modificación DNS realizada en OVHcloud. Siga la progresión de esta operación en la página <ManagerLink to="/#/web/ongoing-operations">Operaciones en curso</ManagerLink>.
...
Si quiere disfrutar de ayuda para utilizar y configurar sus soluciones de OVHcloud, puede consultar nuestras distintas [soluciones en materia de soporte](https://www.ovhcloud.com/es-es/support-levels/).
