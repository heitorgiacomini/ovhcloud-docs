...
Necesitará el nombre de cluster de su alojamiento web que encontrará en su <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, así como el nombre de usuario FTP y la contraseña. Consulte [la guía correspondiente](/guides/web-cloud/web-hosting/ftp-connection) si necesita más información sobre este método de acceso.
...
Necesitará la dirección IP de su servidor que encontrará en su <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, así como el nombre de la cuenta de usuario que desea utilizar para esta sesión de conexión. Si desea más información sobre este tema, consulte nuestras guías Primeros pasos:
...
:::info
PuTTY almacena los archivos de claves en un formato específico que los hace incompatibles con los archivos de claves SSH creados con el cliente **OpenSSH**. Si desea utilizar una **clave privada** creada previamente con el cliente SSH en línea de comandos (por ejemplo, para un [servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys) o una [instancia Public Cloud](/guides/public-cloud/compute/creating-ssh-keys)), deberá [convertirla al formato PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/faq.html#faq-ssh2-keyfmt).
...
- [Cómo sustituir un par de claves SSH en una instancia Public Cloud](/guides/public-cloud/compute/replacing-lost-ssh-key-pair)
...
|  |  |
|:--|:--|
| <img className="thumbnail" alt="putty" src="/images/assets/screens/other/web-tools/putty/putty3.png" loading="lazy" /> | 1\. Abra PuTTY.<br/> 2\. Expanda el nodo `SSH` en `Connection` en el árbol `Category`.<br/> 3\. Expanda el nodo `Auth`.<br/> 4\. Haga clic en `Credentials` para ver los parámetros correspondientes.<br/> 5\. Pulse el botón <code className="action">Browse</code>.<br/> 6\. Seleccione el archivo de clave privada (`keyfile.ppk`) en la carpeta en la que lo guardó. |
...
[Cómo crear llaves SSH con OpenSSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)

[Cómo crear llaves SSH con OpenSSH para instancias Public Cloud](/guides/public-cloud/compute/creating-ssh-keys)
