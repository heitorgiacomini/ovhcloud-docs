...
Declare su nuevo nombre de dominio utilizando nuestra documentación sobre la [adición de un sitio web en su alojamiento compartido](/guides/web-cloud/web-hosting/multisites-configure-multisite). Declare también su subdominio en `www` si desea, por ejemplo, que `www.NewDomain.tld` muestre también su sitio además de `NewDomain.tld`.
...
- Compruebe que su nuevo dominio apunta correctamente a la dirección IP correcta de su alojamiento compartido. Para obtener la dirección IP, conéctese a su <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>, acceda a la sección <code className="action">Web cloud</code>, haga clic en <code className="action">Alojamientos</code>, seleccione su alojamiento y recupere **l IPv4** en la pestaña <code className="action">Información general</code>.

:::warning
Si activa las opciones **IP del país** o **CDN** con su nuevo dominio, utilice la dirección IP correcta con nuestra documentación que recoge [todas las direcciones IP de nuestros alojamientos compartidos](/guides/web-cloud/web-hosting/multisites-configure-multisite).

Para consultar el número del cluster en el que se encuentra el alojamiento, acceda a la sección <code className="action">Web cloud</code>, haga clic en <code className="action">Alojamientos</code>, seleccione el alojamiento y abra la pestaña <code className="action">FTP-SSH</code>. Podrá ver el número del cluster en el formulario **Servidor FTP y SFTP** : `ftp.cluster0XX.ovh.net` (donde los `X` representan el número de cluster).
...
<img className="thumbnail" alt="multisitios" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/multisite/all-domain-same-config-enable.png" loading="lazy" />
...
Antes de realizar cualquier acción, le recomendamos que obtenga una [copia de seguridad de su espacio de almacenamiento FTP](/guides/web-cloud/web-hosting/ftp-save-and-backup) y una [copia de seguridad de su base de datos](/guides/web-cloud/web-hosting/sql-database-export). Para restaurar el sitio web en caso de manipulación incorrecta.
...
Tenga en cuenta que en estos CMS también es posible realizar sus modificaciones directamente [en la base de datos](/guides/web-cloud/web-hosting/sql-create-database). Deberá cambiar la URL de acceso al sitio web en la tabla que se indica a tal efecto.
...
Para reescribir sus URL con su nuevo dominio, [conéctese a la base de datos de su sitio web](/guides/web-cloud/web-hosting/sql-create-database) y, a continuación, sustituya su antiguo dominio por el nuevo en la tabla correspondiente. 
...
Para evitar el "*Duplicate-content*" y cuando su nuevo nombre de dominio esté completamente operativo con su sitio web, deberá eliminar la declaración de su antiguo nombre de dominio en su sitio web utilizando el guía sobre la gestión de [sitios web en su alojamiento compartido](/guides/web-cloud/web-hosting/multisites-configure-multisite).
...
[Lista de las IP de nuestros alojamientos compartidos](/guides/web-cloud/web-hosting/multisites-configure-multisite)

[Gestionar un certificado SSL en un alojamiento web](/guides/web-cloud/web-hosting/ssl-on-webhosting)

[Habilitar HTTPS en un sitio web con certificado SSL](/guides/web-cloud/web-hosting/ssl-activate-https-website)

[Redirigir un dominio](/guides/web-cloud/domains/redirect-domain-name)
