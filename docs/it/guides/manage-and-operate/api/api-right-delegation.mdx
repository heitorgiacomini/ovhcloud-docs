...
description: Find out how to create an application and manage services for your customers
...
To do so, go to the <ManagerLink urls={{ eu: 'https://auth.eu.ovhcloud.com/api/createApp', ca: 'https://auth.ca.ovhcloud.com/api/createApp' }}>OVHcloud API application creation page</ManagerLink>.

You will need to log in and set an application name and description.

![image](/images/manage-and-operate/api/api-right-delegation/createapp.png)

Once logged in, you will get your `Application Key` and `Application Secret`, later refered as `AK` and `AS`.

![image](/images/manage-and-operate/api/api-right-delegation/ak-as.png)
...
![sequence diagram](/images/manage-and-operate/api/api-right-delegation/sequence01.png)
...
To do so, your application will request credentials by calling <ApiLink section="/auth" method="POST" route={"/auth/credential"}>`POST /auth/credential`</ApiLink> and passing as argument a list of endpoints the application needs access to.
...
![rights confirmation](/images/manage-and-operate/api/api-right-delegation/validate-ck.png)
...
![sequence diagram](/images/manage-and-operate/api/api-right-delegation/sequence02.png)
