...
- `serviceName`: Inserisci il nome interno della tua licenza (vedi la sezione `Licenze` nel tuo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>).
...
- Scarica via SFTP: Connetti il tuo VPS con un client software in grado di utilizzare SFTP (ad esempio [FileZilla](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp)), quindi trasferisci tutti i file nella directory radice del sistema.
...
Non è possibile scaricare un file VM di un VPS di OVHcloud. Tuttavia, puoi utilizzare l'opzione **Snapshot** del tuo VPS nel <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> per recuperare un file immagine.
