...
description: Come configurare IPv6 sul tuo VPS OVHcloud
...
:::info
Questo articolo riguarda la configurazione di indirizzi IPv6 principali su un'interfaccia pubblica. È inoltre possibile configurare gli indirizzi Additional IP sul VPS utilizzando [questa guida](/guides/bare-metal-cloud/virtual-private-servers/configuring-ip-aliasing).
...
- Disporre di un [server VPS OVHcloud](https://www.ovhcloud.com/it/vps/)
- Essere connesso al tuo VPS in SSH (accesso root) o tramite desktop remoto (Windows)
- Possedere conoscenze base di rete
- Accesso all'[API OVHcloud](https://eu.api.ovh.com/) (facoltativo)
...
<Tabs>
  <Tab label="Dallo Spazio Cliente OVHcloud">
    ...
    <img className="thumbnail" alt="configureipv6" src="/images/bare-metal-cloud/virtual-private-servers/configure-ipv6/vps_ipv6_information.png" loading="lazy" />
  </Tab>
  <Tab label="Tramite le API OVHcloud">
    Accedi alla pagina delle [API OVHcloud](https://eu.api.ovh.com/console/):
    ...
    <Api version="v1" section="/vps" method="GET" route={"/vps/\\{serviceName\\}/ips"} />

Per ottenere il gateway IPv6 assegnato al VPS:
    
    <Api version="v1" section="/vps" method="GET" route={"/vps/\\{serviceName\\}/ips/\\{ipAddress\\}"} />
  </Tab>
</Tabs>
...
Una volta che disponi dei dati necessari per la configurazione dell’IPv6, accedi al VPS in SSH. Se hai bisogno di aiuto per questa operazione, consulta la guida [Introduzione a SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction).
...
- [Applicazione persistente su Debian e derivati (Ubuntu, Crunchbang, SteamOS, ecc.)](#persistentdebian)
- [Applicazione persistente su Red Hat e derivati (CentOS, Rocky Linux, AlmaLinux, ecc.)](#persistentredhat)
- [Applicazione persistente su Fedora 42 e versioni successive](#persistentfedora)
...
#### Applicazione persistente su Debian e derivati (Ubuntu, Crunchbang, SteamOS, ecc.) <a name="persistentdebian"></a>
...
- **Per Debian 11**: utilizza il metodo basato sul file *interfaces*.

- **Per Debian 12, Ubuntu 22.04 e versioni successive**: utilizza il metodo basato sulla funzione *Netplan*.

In alcuni casi, il metodo da utilizzare potrebbe non essere quello indicato sopra. Verifica il metodo attivo nel tuo caso e, per verificarlo, accedi al tuo sistema. Se necessario, consulta il sito [https://netplan.io/](https://netplan.io/) per maggiori informazioni.
...
<Tabs>
  <Tab label="Configurazione dei file *interfaces*">
  ...
  <Tab label="Configurazione tramite *Netplan*">
    I file di configurazione di rete si trovano nella directory `/etc/netplan/`. Di default, il file di configurazione principale è `50-cloud-init.yaml`. Prima di continuare, controllare il file per verificare se l'indirizzo IPv6 è già stato configurato. In questo caso, non è necessario configurare nuovamente l'indirizzo IPv6 in quanto con il server VPS è disponibile un solo indirizzo IPv6.
    
    Se l'indirizzo IPv6 non è stato configurato, l'approccio migliore consiste nel creare un file di configurazione separato per configurare l'indirizzo IPv6 nella directory `/etc/netplan/`. In questo modo è possibile annullare le modifiche in caso di errore.
    
    È inoltre consigliabile modificare i permessi per il file appena creato. Per maggiori informazioni sui permessi dei file, consulta la [documentazione ufficiale di ubuntu](https://help.ubuntu.com/community/FilePermissions).
    ...
    **Esempio di configurazione** (con prefisso /128):
...
#### Applicazione persistente su Red Hat e suoi derivati (CentOS, Rocky Linux, AlmaLinux, ecc.) <a name="persistentredhat"></a>

Esistono due metodi per configurare la tua rete in base al sistema operativo installato sul tuo server:

- **per CentOS 7, Rocky Linux e AlmaLinux (8/9)**: utilizzare il metodo basato sul file *network-scripts*;

- **per Rocky Linux e AlmaLinux (10)**: utilizzare il metodo basato sul file *system-connections*.

<Tabs>
  <Tab label="Configurazione dei file *network-scripts*">
  ...
  <Tab label="Configurazione dei file *system-connections*">
...
#### Applicazione persistente su Fedora 42 e versioni successive <a name="persistentfedora"></a>
...
Di default, IPv6 non è configurato sui server Windows. Per attivarlo, apri il <code className="action">Pannello di configurazione</code> e clicca su <code className="action">Visualizza lo stato e le operazioni della rete</code>, poi su <code className="action">Modifica le impostazioni della scheda</code>.

<img className="thumbnail" alt="configureipv6" src="/images/bare-metal-cloud/virtual-private-servers/configure-ipv6/configure-ipv6-step2.png" loading="lazy" />

Clicca su <code className="action">Ethernet</code> per aprire le impostazioni e clicca sul pulsante <code className="action">Proprietà</code> per visualizzare `Proprietà Ethernet`.

Seleziona <code className="action">Protocol Internet version 6 (TCP/IPv6)</code> e clicca sul pulsante <code className="action">Proprietà</code>.

<img className="thumbnail" alt="configureipv6" src="/images/bare-metal-cloud/virtual-private-servers/configure-ipv6/configure-ipv6-step3.png" loading="lazy" />

Nella finestra Proprietà IPv6 seleziona <code className="action">Utilizza il seguente indirizzo IPv6</code>. Inserisci gli indirizzi IP recuperati nel primo step.

Puoi anche inserire le risoluzioni DNS IPv6 che preferisci utilizzando <code className="action">Utilizza il seguente indirizzo server DNS IPv6</code>. Ciò non è obbligatorio se i resolver DNS della configurazione IPv4 sono già operativi.

Seleziona la casella <code className="action">Conferma le impostazioni all'uscita</code> e clicca su <code className="action">OK</code> per confermare le modifiche. Se il gateway specificato non si trova sulla stessa sottorete IPv6 (ad esempio, /128 e /64), può essere visualizzato un messaggio di errore. Puoi ignorare questo messaggio e passare allo step successivo.

<img className="thumbnail" alt="configureipv6" src="/images/bare-metal-cloud/virtual-private-servers/configure-ipv6/configure-ipv6-step4.png" loading="lazy" />
...
<summary>**Step 4: Disattiva la gestione della rete Cloud-init (in opzione)**</summary>
