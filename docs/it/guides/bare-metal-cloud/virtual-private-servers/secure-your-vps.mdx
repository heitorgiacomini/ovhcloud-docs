...
description: Come impostare misure di sicurezza di base per proteggere il VPS da attacchi e accessi non autorizzati
...
Per configurare il tuo primo VPS OVHcloud, consulta la nostra guida [sulla messa in servizio di un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps).
:::


Questi esempi presuppongono la connessione come [utente con elevate autorizzazioni](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).
...
- [Come creare e utilizzare chiavi di autenticazione per le connessioni SSH ai server OVHcloud](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)
- [Guida - Come utilizzare PuTTY per le connessioni SSH e l'autenticazione](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows)
...
Ricordati di indicare la nuova porta ad ogni richiesta di [connessione SSH al tuo server](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction):
...
Se sei bloccato fuori dal tuo sistema, puoi utilizzare il nostro ambiente [Rescue mode](/guides/bare-metal-cloud/virtual-private-servers/rescue) per annullare le modifiche apportate.

### Crea un account con diritti utente limitati <a name="createuser"></a>

In genere, i compiti che non richiedono privilegi root devono essere eseguiti tramite un utente standard. Per maggiori informazioni, consulta [questa guida](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).
...
Per maggiori informazioni su iptables, consulta la nostra [guida dedicata](/guides/bare-metal-cloud/virtual-private-servers/firewall-linux-iptable).
...
Per attivare il Network Firewall, consulta la guida [Configurare il Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network).
...
Tutte le informazioni sulle soluzioni di backup disponibili per il tuo servizio sono disponibili sulla [pagina del prodotto](https://www.ovhcloud.com/it/vps/options/) e nelle [rispettive](/guides/bare-metal-cloud/virtual-private-servers/overview) guide.

## Per saperne di più

[Iniziare a utilizzare un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps) 

[Creare e utilizzare chiavi SSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)

[Configura il firewall su Windows](/guides/bare-metal-cloud/virtual-private-servers/activate-port-firewall-soft-win)

[Configura il firewall su Linux con iptables](/guides/bare-metal-cloud/virtual-private-servers/firewall-linux-iptable)
