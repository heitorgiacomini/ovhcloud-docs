...
description: Identifica un disco difettoso sul tuo server dedicato OVHcloud e richiedi la sostituzione tramite lo Spazio Cliente
...
Se possiedi un server che dispone di un RAID software, fai riferimento alla guida [Software RAID](/guides/bare-metal-cloud/dedicated-servers/raid-soft) per trovare i dischi installati sul tuo server.
...
Se il tuo server dispone di un RAID hardware, fai riferimento alla guida (in inglese) [Hardware RAID - EN](/guides/bare-metal-cloud/dedicated-servers/raid-hard) e segui la procedura relativa al tuo tipo di controller RAID per trovare i percorsi di accesso ai tuoi dischi.
...
Nel caso di un disco NVME, è necessario mettere il server in modalità [Rescue mode](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) e installare lo strumento `nvme-cli`.
...
Questo tipo di sostituzione è possibile solo per i [server dedicati](https://www.ovhcloud.com/it/bare-metal/) con una scheda RAID.
> 
...
Se possiedi un server con RAID software, ti consigliamo di risincronizzare i dischi manualmente. Per maggiori informazioni, consulta la documentazione in inglese sul [RAID software ](/guides/bare-metal-cloud/dedicated-servers/raid-soft).

## Per saperne di più

[RAID software](/guides/bare-metal-cloud/dedicated-servers/raid-soft)

[RAID hardware - EN](/guides/bare-metal-cloud/dedicated-servers/raid-hard)

[Rescue Mode](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)
