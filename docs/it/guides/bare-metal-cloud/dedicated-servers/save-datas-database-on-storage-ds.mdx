...
description: Come proteggere i tuoi dati in 5 step
...
- Un’infrastruttura di produzione ([VPS](https://www.ovhcloud.com/it/vps/), [server dedicati](https://www.ovhcloud.com/it/bare-metal/), [Public Cloud](https://www.ovhcloud.com/it/public-cloud/), ecc.)
...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e installa il tuo server (come già detto nella parte iniziale di questa guida, utilizzeremo Debian 9.4). Per maggiori informazioni, consulta la nostra guida [Iniziare a utilizzare un server dedicato](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#installa-o-reinstalla-il-tuo-server-dedicato).

Una volta selezionato il sistema operativo, spunta la casella <code className="action">Personalizza la configurazione delle partizioni</code> e clicca su <code className="action">Seguente</code>.

<img className="thumbnail" alt="Personalizzare la configurazione delle partizioni" src="/images/bare-metal-cloud/dedicated-servers/save-datas-database-on-storage-ds/custompartition.png" loading="lazy" />

Nella nuova finestra, è possibile modificare il tipo di RAID dalla tua `/home` (1) ed eventualmente estendere la partizione (2).

<img className="thumbnail" alt="Modifica della /home" src="/images/bare-metal-cloud/dedicated-servers/save-datas-database-on-storage-ds/partition_customization_menu.png" loading="lazy" />
