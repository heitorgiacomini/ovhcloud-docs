...
Questa guida ti mostra come eseguire le operazioni necessarie per eseguire l'operazione. Tuttavia, in caso di difficoltà o dubbi relativamente all'amministrazione, all'utilizzo o alla realizzazione dei servizi su un server, ti consigliamo di contattare un [provider specializzato](https://partner.ovhcloud.com/it/directory/).
:::
...
Puoi spingerti oltre, configurando il record SPF per un dominio specifico o utilizzando l'indirizzo IPv6. Per una corretta comprensione del record SPF, consulta la nostra guida sulla [configurazione di un record SPF](/guides/web-cloud/domains/dns-zone-spf).
...
Per maggiori informazioni, consulta la nostra guida sulla [configurazione di un record DKIM](/guides/web-cloud/domains/dns-zone-dkim).
...
Per maggiori informazioni, consulta la nostra guida sulla [configurazione di un record DMARC](/guides/web-cloud/domains/dns-zone-dmarc).
...
Se i server DNS sono gestiti da OVHcloud, consulta la nostra guida [sulla modifica di una zona DNS OVHcloud dallo Spazio Cliente](/guides/web-cloud/domains/dns-zone-edit).

Una volta modificata la zona DNS del dominio, la propagazione delle modifiche potrebbe richiedere fino a 24 ore.

Per configurare il percorso del reverse DNS nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, consulta la [nostra guida dedicata](/guides/bare-metal-cloud/virtual-private-servers/configuring-reverse-dns).
...
[https://postmaster.live.com/snds/JMRP.aspx?wa=wsignin1.0](https://postmaster.live.com/snds/JMRP.aspx?wa=wsignin1.0)
...
[Migliorare la sicurezza delle email tramite un record DKIM](/guides/web-cloud/domains/dns-zone-dkim)

[Migliorare la sicurezza delle email tramite un record SPF](/guides/web-cloud/domains/dns-zone-spf)

[Migliorare la sicurezza delle email tramite un record DMARC](/guides/web-cloud/domains/dns-zone-dmarc)
