...
description: Scopri come configurare il servizio OVHcloud Game DDoS Protection firewall
...
:::info
Per maggiori informazioni, consulta la nostra [protezione anti-DDoS Game sul nostro sito](https://www.ovhcloud.com/it/security/game-ddos-protection/).
...
- Un [server dedicato OVHcloud **Game**](https://www.ovhcloud.com/it/bare-metal/game/)
...
- **Link diretto:** <ManagerLink to="/#/network/ip">Public IP</ManagerLink>
...
Per ulteriori informazioni, consulta la nostra [pagina di confronto](https://eco.ovhcloud.com/it/compare/).
...
:::warning
Il *Game Firewall* protegge gli indirizzi IP associati a un server. Di conseguenza, se possiedi un server con diversi indirizzi IP (ad esempio [indirizzi Additional IP](https://www.ovhcloud.com/it/network/additional-ip/)), devi configurarli separatamente.
...
- Clicca su <code className="action">Network</code> nel menu a sinistra dello schermo.
- Clicca su <code className="action">Indirizzi IP pubblici</code>.

È possibile filtrare gli indirizzi IP utilizzando il menu a tendina <code className="action">Tutti i tipi di servizio</code> oppure inserendo direttamente l’indirizzo IP desiderato nella barra di ricerca. Inserisci il nome o la categoria del server corrispondente:

#### Elenco degli indirizzi IP associati al server Game

<Tabs>
  <Tab label="Dalla pagina Server dedicati">
    - Apri la sezione <code className="action">Bare Metal Cloud</code> nella barra laterale sinistra.
    - Seleziona <code className="action">Server dedicati</code>.
    ...
    - Clicca sul pulsante <code className="action">...</code> e seleziona <code className="action">Configurare Game Protection</code>. Verrai reindirizzato all’elenco degli indirizzi IP assegnati al tuo server.
  </Tab>
  <Tab label="Dalla pagina Indirizzi IP pubblici">
    ...
    - Nel menu a tendina <code className="action">Tutti i tipi di servizio</code>, cerca e seleziona il server Bare Metal Game che desideri configurare.
...
Per ogni indirizzo associato al tuo server che necessita di protezione, verifica che lo stato del *Game Firewall* sia <code className="action">Disponibile</code>. Dovrai configurare le regole del *Game Firewall* singolarmente per ciascuno di essi.

- Clicca sul pulsante <code className="action">⁝</code> a destra della tabella e seleziona <code className="action">Configurare il Game Firewall</code>.
...
:::warning
La protezione anti-DDoS Game viene attivata dopo le regole definite nel [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network). Per il corretto funzionamento di entrambi, l’Edge Network Firewall non può essere troppo rigido e deve lasciare passare il traffico appropriato alla protezione anti-DDoS Game.
...
Tutti i protocolli di protezione anti-DDoS Game supportati per un server specifico sono visibili nella pagina di configurazione <code className="action">GAME firewall</code> per ogni indirizzo IP che punta a questo server, nel menu a tendina <code className="action">Game protocol</code>:
...
Se preferisci l'automazione, puoi recuperare i dettagli del protocollo utilizzando [l'APIv6 OVHcloud](/guides/manage-and-operate/api/first-steps):
...
<summary>**È possibile utilizzare la protezione anti-DDoS Game su gamme diverse dai server Bare Metal Game?**</summary>
...
<summary>**In che modo l'automazione funziona per un Additional IP tra una nuova e una vecchia generazione di server Bare Metal Game?**</summary>
...
<summary>**È possibile disattivare la protezione anti-DDoS Game?**</summary>
...
<summary>**Il gioco non è presente nell'elenco dei protocolli supportati, cosa posso fare?**</summary>
...
<summary>**Dopo aver configurato il gioco con le porte appropriate e il criterio predefinito da eliminare, continuo a ricevere attacchi che colpiscono il mio server Game. Cosa fare?**</summary>
