...
- Un servizio OVHcloud esposto su un indirizzo IP pubblico dedicato ([Server Dedicato](https://www.ovhcloud.com/it/bare-metal/), [VPS](https://www.ovhcloud.com/it/vps/), [Istanza Public Cloud](https://www.ovhcloud.com/it/public-cloud/), [Hosted Private Cloud](https://www.ovhcloud.com/it/hosted-private-cloud/)), [Additional IP](https://www.ovhcloud.com/it/network/additional-ip/), ecc.
...
- **Link diretto:** <ManagerLink to="/#/dedicated/network-security/scrubbing-center">Security Dashboard</ManagerLink>
- **Percorso di navigazione:** <code className="action">Network</code> > <code className="action">Network Security Dashboard</code>
...
L’infrastruttura anti-DDoS di OVHcloud è un sistema di difesa distribuito e multilivello per combattere gli attacchi informatici. È costituito da molteplici localizzazioni e centri di pulitura che possono analizzare e ripulire il traffico malevolo. Insieme a [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network) e alla [GAME DDoS protection](/guides/bare-metal-cloud/dedicated-servers/firewall-game-ddos), offre servizi di protezione della qualità in diversi casi.
...
Durante un attacco, un'azione di mitigazione attiva viene indicata da un'icona di avviso nella pagina dell'elenco degli IP (nella sezione <code className="action">Gestisci gli indirizzi IP</code> del tuo Spazio Cliente OVHcloud).
...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, clicca su <code className="action">Network</code> nel menu a sinistra dello schermo e poi su <code className="action">Indirizzi IP Pubblici</code>. Verifica che la `Modalità avanzata` sia abilitata per visualizzare lo stato dell'infrastruttura anti-DDoS e la configurazione dei suoi componenti.
...
- La colonna **Firewall** indica lo stato di Edge Network Firewall, che può essere:
    ...
    - **(nessuno stato)** - Configurazione del firewall non creata. Per configurare le regole, clicca sul pulsante <code className="action">...</code>, quindi seleziona <code className="action">Crea Firewall</code>.
...
Per ulteriori informazioni su come configurare le regole Edge Network Firewall, consulta la nostra guida [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network).
...
[Abilitazione e configurazione di Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network)

[Proteggere un server GAME con il firewall dell’applicazione](/guides/bare-metal-cloud/dedicated-servers/firewall-game-ddos)
