...
description: Come cambiare l’annuncio di un blocco IP nella rete privata vRack
...
La [vRack](https://www.ovhcloud.com/it/network/vrack/) è una rete privata che permette di configurare l’indirizzamento tra due o più [server dedicati](https://www.ovhcloud.com/it/bare-metal/) OVHcloud.

**Questa guida ti mostra come definire la zona annunciata nella vRack per un blocco IP.**

## Prerequisiti

- Disporre di una [vRack](https://www.ovhcloud.com/it/network/vrack/)
- Aver [configurato un blocco IP nella vRack](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack)
...
Questa chiamata API consente di recuperare l'elenco di tutte le vRack create. Se i riferimenti mostrati non ti permettono di identificare la vRack, recupera l’informazione dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>: nella sezione <code className="action">Bare Metal Cloud</code> è sufficiente cliccare su <code className="action">Network</code>, poi su <code className="action">vRack</code> nella colonna a sinistra per visualizzare tutte le vRack attive.
...
[Configurare un blocco di indirizzi IP nella vRack](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack)
