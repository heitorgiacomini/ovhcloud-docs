...
description: Crea coppie di chiavi OpenSSH sul tuo dispositivo locale per connessioni sicure senza password al tuo server dedicato
...
:::info
`Ed25519` è considerato il metodo più sicuro, ma `RSA` è un’alternativa valida. Entrambi sono compatibili con lo Spazio Cliente OVHcloud se desideri [salvare le chiavi pubbliche nel tuo account cliente](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
:::
...
Se si conferma con <code className="action">Entrata</code> senza immettere un nome, verrà utilizzato il nome file standard (in questo esempio, `id_rsa`).
...
Copiare questa stringa di chiave per [aggiungerla a un nuovo server](#getstarted) o per [importarla nello Spazio Cliente](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
...
Se si conferma con il tasto <code className="action">Invio</code> senza immettere un nome, verrà utilizzato il nome file standard (in questo esempio, `id_rsa`).
...
Quando si lavora da riga di comando **Windows**, utilizzare il pulsante destro del mouse per **incollare** il contenuto degli Appunti nella finestra della riga di comando. Per **copiare** una stringa dalla finestra della riga di comando, evidenziarla e premere <code className="action">Invio</code>. Queste funzioni sono disponibili anche facendo clic con il pulsante destro del mouse sulla barra dei menu della finestra della riga di comando.
:::
...
[Introduzione al protocollo SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)

[Modalità Rescue su server dedicato](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)

[Modalità Rescue su VPS](/guides/bare-metal-cloud/virtual-private-servers/rescue)
