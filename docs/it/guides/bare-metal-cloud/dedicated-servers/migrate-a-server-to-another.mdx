...
description: Migra i dati tra server dedicati OVHcloud con rsync, SFTP o il backup storage per una transizione fluida
...
- Il modo più semplice è utilizzare un software adeguato come [SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp).
- L'altra opzione consiste nel [sincronizzare i due server](/guides/bare-metal-cloud/dedicated-servers/how-to-copy-data-from-one-dedicated-server-to-another-using-rsync).
...
Per maggiori informazioni, consulta la nostra guida su [utilizzare il Backup Storage su un server dedicato](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage).
...
Se la reputazione dei tuoi indirizzi IP è importante, ti consigliamo vivamente di utilizzare indirizzi [Additional IP](https://www.ovhcloud.com/it/network/additional-ip/) perché, in caso di migrazione, possono essere conservati.

Una volta creati questi indirizzi IP, dovete semplicemente spostarli sul nuovo server.
Per farlo, consulta la nostra guida: [Trasferisci un Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip).
