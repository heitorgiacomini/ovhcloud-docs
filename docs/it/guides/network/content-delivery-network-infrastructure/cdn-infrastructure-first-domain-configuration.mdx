...
description: Come configurare un dominio sulla CDN OVH
...
- Usufruire della [Content Delivery Network (CDN) OVH](https://www.ovhcloud.com/it/network/cdn/).
- Avere accesso alla gestione della zona DNS del dominio
- Avere accesso allo <ManagerLink to="/">Spazio Cliente OVH</ManagerLink>.

## Procedura

### Aggiungi il dominio sulla CDN

Per prima cosa, è necessario aggiungere un sottodominio alla CDN in modo che possa accettare richieste HTTP(S) per questo dominio: accedi alla sezione <code className="action">Dedicato</code> dello <ManagerLink to="/">Spazio Cliente OVH</ManagerLink> e clicca su <code className="action">NAS e CDN</code> nel menu a sinistra. 

Clicca su <code className="action">Aggiungi un dominio alla CDN</code>:

<img className="thumbnail" alt="Spazio Cliente CDN" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/cdn_customer_panel.png" loading="lazy" />

Nella nuova finestra, seleziona il sottodominio da aggiungere alla CDN.

<img className="thumbnail" alt="Aggiungere un sottodominio alla CDN" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/add_cdn_domain_step_1.png" loading="lazy" />
...
<img className="thumbnail" alt="Aggiungere un back-end" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/add_cdn_domain_step_2.png" loading="lazy" />
...
<img className="thumbnail" alt="Nome della foto" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/cname_field.png" loading="lazy" />
