...
description: "Scopri come utilizzare lo script ovh-ba-install.sh fornito da OVHcloud per installare e gestire l'agente di backup Veeam su un server Linux"
...
| Elemento | Dettagli |
|--------|--------|
| **Sistema** | Linux **compatibile** con l'agente Veeam per Linux (consulta i [requisiti di sistema Veeam](https://helpcenter.veeam.com/docs/agentforlinux/userguide/system_requirements.html?ver=13) e le [restrizioni Backup Agent](/guides/storage-and-backup/backup-agent/restrictions)). |
...
Nello Spazio Cliente OVHcloud, apri il tuo <ManagerLink to="/#/bmc-backup-agent-baremetal/service">Backup Agent</ManagerLink>, vai nella scheda <code className="action">Agents</code>, quindi clicca sul pulsante <code className="action">Scarica</code>. Nella finestra che si apre, seleziona **Linux** per visualizzare il comando contenente i due URL (script e pacchetto Linux).

<img className="thumbnail" alt="Scaricare l'agente — comandi di installazione Linux" src="/images/storage-and-backup/backup-agent/backup-agent-linux-cli-assistant/01-backup-agent-download-linux-fr.png" loading="lazy" />
...
Per approfondire la diagnostica, consulta la nostra [guida alla diagnostica e alla risoluzione dei problemi Backup Agent](/guides/storage-and-backup/backup-agent/troubleshooting).
...
- [Configurare il primo backup](/guides/storage-and-backup/backup-agent/first-configuration)
