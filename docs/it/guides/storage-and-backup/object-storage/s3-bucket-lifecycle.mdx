...
For more information about the differences between the 2 endpoints, see [this documentation](/guides/storage-and-backup/object-storage/s3-location).
:::
...
<details>
<summary>Delete all objects in a non-versioned bucket</summary>
...
<details>
<summary>Abort incomplete multipart uploads</summary>
...
<details>
<summary>Expire objects in a non-versioned bucket with overlapping prefixes</summary>
...
<details>
<summary>Expire objects in a non-versioned bucket with overlapping tags</summary>
...
<details>
<summary>Empty a non-versioned bucket</summary>
...
<details>
<summary>Empty a versioned bucket.</summary>
...
The minimum duration for transition rules between storage classes is **1 day** i.e your lifecycle configuration will not be valid and you will get an error if the number of days for your transition rule is less than 1 day. In practice, with multiple transitions, the minimum duration for the nth transition must be at least n days.

Examples:

- with a single transition from High Performance to Standard, the minimum duration is 1 day.
- you have 3 transitions: 1st from High Performance to Standard, 2nd from Standard to Infrequent Access, and 3rd from Infrequent Access to Active Archive. The minimum durations are 1 day (1st), 2 days (2nd), and 3 days (3rd).
...
| Transitions.Days                                     | yes*     | Indicates the duration in days after which the objects are to be transitioned. The value must be an integer equal to or greater than 1. <br/><br/> ⚠️ This attribute is mandatory if Date is not present. <br/> ⚠️ this attribute is mutually exclusive with Date i.e you either have Date or Days but you cannot specify both. |
...
<details>
<summary>Transition objects in a non-versioned bucket</summary>
...
<details>
<summary>Transition objects in a versioned bucket</summary>
...
<details>
<summary>Mix of expiration and transition</summary>
...
<details>
<summary>Mix of expiration and transition rules with conflicting rules</summary>
...
<details>
<summary>Create a lifecycle configuration file using your favorite editor.</summary>
...
First, ensure you have a bucket with data to apply the lifecycle rule to.
...
- **Direct link:** <ManagerLink to="/#/public-cloud/pci/projects">Public Cloud</ManagerLink>
- **Navigation path:** <code className="action">Public Cloud</code> > Select your project > <code className="action">Object Storage</code>

---
{/* CP-NAV-END:storage-object-storage */}

Click the name of the target bucket, go to the <code className="action">Lifecycle</code> tab, and click <code className="action">Create rule</code>.

#### Step 2 — Configure the rule identifier

In **Rule identifier**, enter a unique name.
...
- **Transition the current version of objects**: automate transition of current versions to a lower-cost storage tier.
    - **Days after creation**: number of days before the transition is triggered (minimum 30).
...
- **Delete incomplete multipart uploads**: stop and remove orphan upload parts.
...
Click <code className="action">Create rule</code>. The rule appears in the list and applies asynchronously within 24 hours.

#### Managing existing rules

In the rules list, click <code className="action">...</code> at the end of a row to:
...
You can use the [Server Access Logging](/guides/storage-and-backup/object-storage/s3-server-access-logging) feature to monitor actions taken by your lifecycle rules on your S3 bucket. Server Access Logging provides detailed records for the requests that are made to a bucket including expiration operations, transitions of objects to another storage tier,etc.
...
Delete operations resulting from application of lifecycle rules are not replicated. See the documentation for [asynchronous replication](/guides/storage-and-backup/object-storage/s3-asynchronous-replication) for further details on what is replicated and what is not.
