...
![Move2CloudZerto](/images/assets/screens/other/zerto/move2cloud-zerto.png)
...
2. Select your `Hosted Private Cloud`.
3. Navigate to the `Security` tab.
4. Click `Add a new IP address range` to authorize your source infrastructure IPs and Zerto components.
...
In your `Hosted Private Cloud` interface:

1. Go to `Disaster Recovery`.
2. Select `Enable Zerto Virtual Replication`.
...
Use `Storage vMotion` via the `vSphere Client`:
