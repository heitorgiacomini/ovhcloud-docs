...
description: Questa guida ti mostra come impostare una manutenzione programmata sul tuo servizio VMware on OVHcloud
...
- Avere accesso allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> o all'[interfaccia di gestione dei servizi via API](https://eu.api.ovh.com/).

## Procedura

:::tip
Le email inviate da OVHcloud sono accessibili anche dal tuo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>.<br/>
Clicca sul tuo nome in alto a destra e poi su <code className="action">Email di servizio</code> nel menu di destra.
...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> con un account amministratore.

Nel menu <code className="action">Hosted Private Cloud</code>, clicca sulla scheda <code className="action">Operazioni</code>. Seleziona <code className="action">Da fare</code> nel menu a tendina che permette di filtrare le operazioni.

Clicca sui tre puntini in corrispondenza del servizio <code className="action">...</code> e seleziona <code className="action">Modifica la data di elaborazione</code>.

<img className="thumbnail" alt="modifica oraria" src="/images/hosted-private-cloud/powered-by-vmware/maintenance-rescheduling/maintenance-date-edition01.png" loading="lazy" />

:::info
Se il pulsante <code className="action">Modifica la data di gestione</code> è grigio, significa che la manutenzione non può essere differita.
...
Clicca sul pulsante <code className="action">Modifica</code> per confermare le modifiche.

<img className="thumbnail" alt="modifica oraria" src="/images/hosted-private-cloud/powered-by-vmware/maintenance-rescheduling/maintenance-date-edition02.png" loading="lazy" />

### Via API OVHcloud

Accedi all'[interfaccia di gestione dei tuoi servizi via API](https://eu.api.ovh.com/). Per maggiori informazioni, consulta la guida Iniziare a [utilizzare le API OVHcloud](/guides/manage-and-operate/api/first-steps).
