...
3. Assicurati di non mescolare un gruppo di server DNS OVHcloud con un altro gruppo di server DNS OVHcloud. In OVHcloud, i gruppi di server DNS sono identificabili tramite il numero presente nei nomi dei server. Due server DNS OVHcloud fanno parte dello stesso gruppo di server DNS quando condividono lo stesso numero.

<Region zones={['eu']}>

Ad esempio, *dns19.ovh.net* e *ns19.ovh.net* corrispondono a un gruppo di server DNS OVHcloud, vanno di pari passo e sono sincronizzati. Fanno parte dello stesso gruppo perché condividono lo stesso numero.

</Region>

<Region zones={['ca', 'apac']}>

Ad esempio, *dnsXX.ovh.ca* e *nsXX.ovh.ca* corrispondono a un gruppo di server DNS OVHcloud, vanno di pari passo e sono sincronizzati (ad esempio, *dns10.ovh.ca* e *ns10.ovh.ca*). Fanno parte dello stesso gruppo perché condividono lo stesso numero. I server esatti assegnati al tuo nome di dominio sono indicati nello Spazio Cliente OVHcloud.
...
Ad esempio: vuoi dichiarare i server DNS *ns1.dns-server.tld*, *dnsXX.ovh.ca* e *nsXX.ovh.ca* per il tuo nome di dominio. Dovrai quindi verificare che i tre record di tipo "NS" seguenti siano presenti nelle 3 zone DNS ospitate su questi 3 server DNS:
...
- Il *Registry* che gestisce l'estensione del tuo nome di dominio (ad esempio, il Registry delle estensioni *.fr*) deve essere informato della modifica DNS apportata lato OVHcloud. Segui l'avanzamento dell'operazione sulla pagina <ManagerLink to="/#/web/ongoing-operations">Operazioni in corso</ManagerLink>.
