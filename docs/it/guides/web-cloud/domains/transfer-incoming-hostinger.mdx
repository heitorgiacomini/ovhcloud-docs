...
description: Questa guida ti mostra tutte le informazioni relative al trasferimento di un nome di dominio Hostinger in OVHcloud
...
Per i nomi di dominio registrati in Hostinger, è possibile gestire lo stato di blocco del nome di dominio aprendo la sezione <code className="action">Domini</code> e selezionando il nome di dominio da trasferire.
...
Per familiarizzare con la modifica dell'intestatario di un nome di dominio, consulta questa [guida](/guides/web-cloud/domains/trade-domain).
