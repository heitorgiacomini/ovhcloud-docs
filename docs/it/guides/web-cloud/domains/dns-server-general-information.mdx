...
description: Questa guida ti mostra il ruolo dei server DNS, cosa contengono e come funzionano con un dominio
...
In particolare, consentono l'utilizzo di [domini](https://www.ovhcloud.com/it/domains/) per accedere al sito Web preferito senza dover ricordare l'indirizzo IP del server in cui è ospitato il sito Web.
...
Per effettuare questa operazione su un dominio registrato in OVHcloud, consulta la nostra guida "[Modificare i server DNS di un dominio OVHcloud](/guides/web-cloud/domains/dns-server-edit)".
...
:::tip[Success]
- Per maggiori informazioni sulle zone DNS, consulta la nostra guida "[Sapere tutto sulla zona DNS](/guides/web-cloud/domains/dns-zone-general-information)".
- Consulta la nostra guida su [Scopri tutto sui record DNS](/guides/web-cloud/domains/dns-zone-records) per una migliore comprensione dell'insieme.
:::
