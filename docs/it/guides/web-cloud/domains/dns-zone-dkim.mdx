...
Se il tuo dominio è registrato presso OVHcloud, verifica che utilizzi la configurazione OVHcloud nel tuo <ManagerLink to="/">Spazio Cliente</ManagerLink> utilizzando la scheda <code className="action">Zona DNS</code>.
:::
...
<details>
<summary>La macinazione <a name="hash"></a></summary>
...
<details>
<summary>Crittografia asimmetrica <a name="encrypt"></a></summary>
...
<details>
<summary>Come vengono utilizzati la classificazione e la crittografia asimmetrica per il DKIM? <a name="encrypt-and-hash"></a></summary>


Dalla piattaforma email, il DKIM utilizzerà l'hash per creare una firma a partire da alcuni elementi [dell'intestazione dell'email](/guides/web-cloud/email-and-collaborative-solutions/troubleshooting/diagnostic-headers) e del corpo dell'email (contenuto dell'email).
...
<details>
<summary>Perché è necessario configurare i server DNS? <a name="dns-and-dkim"></a></summary>


Affinché il destinatario possa verificare la firma DKIM del mittente, avrà bisogno dei parametri DKIM e soprattutto della chiave pubblica per decifrarla. La [zona DNS](/guides/web-cloud/domains/dns-zone-general-information) di un dominio è pubblica, motivo per cui è stato aggiunto un record DNS per trasmettere la chiave pubblica e i parametri DKIM al destinatario.

</details>


<details>
<summary>Cos'è un selettore DKIM <a name="selector"></a></summary>
...
<details>
<summary>Esempio di un'email inviata utilizzando il DKIM <a name="example"></a></summary>
...
<Region zones={['eu']}>
- [E-mail Pro](https://www.ovhcloud.com/it/emails/email-pro/).
</Region>
<Region zones={['eu']}>
- [Zimbra](https://www.ovhcloud.com/it/emails/zimbra-emails/).
</Region>
...
<ZoneTabs>
  <Tab label="Email (MX Plan)">
    ...
    1. Infine, clicca sulla scheda <code className="action">Informazioni generali</code>.
    ...
    Per attivare il DKIM, basta cliccare sul tasto `DKIM` rosso e poi su <code className="action">Attivare</code> dalla finestra di attivazione che appare.
    ...
    Questa vi invita a inserire due valori CNAME nella zona DNS del nome a dominio, il che permette di collegare questo nome a dominio ai selettori DKIM del vostro servizio e-mail. È necessario inserire questi valori e assicurarvi che siano propagati prima di cliccare su <code className="action">Attivare</code>.
  </Tab>
  <Tab label="Exchange" availableIn={['eu', 'ca']}>
    1. Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>.
    1. Accedi alla sezione <code className="action">Web Cloud</code>.
    1. Nella sezione `MICROSOFT`, clicca su <code className="action">Exchange</code>.
    1. Seleziona la piattaforma interessata.
    1. Infine, clicca sulla scheda <code className="action">Domini associati</code>.
  ...
  <Tab label="Email Pro" availableIn={['eu']}>
    1. Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>.
    1. Accedi alla sezione <code className="action">Web Cloud</code>.
    1. Clicca su <code className="action">Email Pro</code>.
    1. Seleziona la piattaforma interessata.
    1. Infine, clicca sulla scheda <code className="action">Domini associati</code>.
  ...
  <Tab label="Zimbra" availableIn={['eu']}>
    1. Accedete al vostro <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>.
    1. Recatevi nella sezione <code className="action">Web Cloud</code>.
    1. Cliccate su <code className="action">Zimbra Mail</code>.
    1. Infine, andate sull'onghetto <code className="action">Domini</code>.
    1. A destra del dominio interessato, cliccate su <code className="action">⁝</code>, quindi su <code className="action">Diagnostica</code>.
    ...
    A destra della dicitura `DKIM` sull'onghetto corrispondente, dovreste osservare un avviso che indica che il DKIM non è conforme. Cliccate sull'onghetto <code className="action">DKIM</code> per accedere allo stato della configurazione DKIM. Per correggere l'errore, dovrete aggiungere o modificare due voci DNS di tipo CNAME nella zona DNS del nome a dominio associato, in base alle informazioni visibili da questo onggetto.
    ...
    **Suggerimento per creare un record CNAME**
    ...
    Dal <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> dove è ospitato il nome a dominio del vostro servizio e-mail, nella sezione <code className="action">Web Cloud</code>, cliccate su <code className="action">Nomi di dominio</code> nella colonna di sinistra e selezionate il nome a dominio interessato.<br/>
    Selezionate l'onghetto <code className="action">Zona DNS</code> e cliccate su <code className="action">Aggiungi un'entrata</code> nella finestra che appare. Scegliete `CNAME` e completate in base ai valori che avete notati.
    :::
  </Tab>
</ZoneTabs>
...
<Region zones={['ca']}>
Per una piattaforma Exchange, è necessario prima recuperare il riferimento della piattaforma per configurare il DKIM.
</Region>
...
Assicurati anche che il dominio da utilizzare per le tue email sia attivo nella sezione <code className="action">Domini associati</code>.
...
Per configurare il DKIM, recatevi sulla [pagina delle API OVHcloud](/links/api) e accedete:

1. Cliccate su <code className="action">Authentication</code> in alto a sinistra.
1. Cliccate quindi su <code className="action">Login with OVHcloud SSO</code>.
1. Inserite i vostri identificativi OVHcloud.
1. Cliccate sul pulsante <code className="action">Authorize</code> per autorizzare le chiamate alle API da questo sito.
...
<Tabs>
  <Tab label="1. Attiva il DKIM sul tuo dominio">
    ...
    Clicca su <code className="action">EXECUTE</code> per avviare l’attivazione.<br/>
  ...
  <Tab label="2. Verifica lo stato dell'operazione DKIM">
    ...
    <br/>
    Clicca su <code className="action">EXECUTE</code> per visualizzare il risultato.<br/>
  ...
  <Tab label="3. Recupera il record DNS">
    ...
    Clicca su <code className="action">EXECUTE</code> per visualizzare il risultato.
  ...
  <Tab label="4. Configura record DNS">
    Dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> in cui è ospitato il dominio del tuo servizio di posta, nella scheda <code className="action">Web Cloud</code>, clicca su <code className="action">Domini</code> nella colonna di sinistra e seleziona il dominio interessato.<br/>
    Clicca sulla scheda <code className="action">Zona DNS</code> e poi su <code className="action">Aggiungi un record</code> nella finestra che appare. Seleziona `CNAME` e completa con i valori inseriti.
    ...
    Una volta inseriti i valori, clicca su <code className="action">Seguente</code> e poi su <code className="action">Conferma</code>.
...
<Tabs>
  <Tab label="1. Elenco degli elettori">
  ...
  <Tab label="2. Creare un selettore">
    ...
    *Esempio di inserimento:*
    ...
    Clicca su <code className="action">EXECUTE</code> per avviare la creazione del selettore.<br/>
  ...
  <Tab label="3. Recuperare il record DNS">
  ...
  <Tab label="4. Configura il record DNS">
    ...
    Clicca sulla scheda <code className="action">Zona DNS</code> e poi su <code className="action">Aggiungi un record</code> nella finestra che appare. Scegli `CNAME` e completa in base ai valori rilevati.<br/>
    ...
    Una volta inseriti i valori, clicca su <code className="action">Seguente</code> e poi <code className="action">Conferma</code>.
  ...
  <Tab label="5. Attivazione del DKIM">
    :::warning
...
<Tabs>
  ...
  <Tab label="4. Configura il record DNS">
    ...
    Una volta inseriti i valori, clicca su <code className="action">Seguente</code> e poi <code className="action">Conferma</code>.<br/>
...
Dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, clicca sulla scheda <code className="action">Web Cloud</code> e poi su <code className="action">Domini</code> nella colonna di sinistra.

Clicca sulla scheda <code className="action">Zona DNS</code> e poi su <code className="action">Aggiungi un record</code>. Per configurare il DKIM nella tua zona DNS, è possibile aggiungere un record in 3 modi:
...
<ZoneTabs>
  <Tab label="Exchange" availableIn={['eu', 'ca']}>
    ...
    Nella sezione <code className="action">Domini associati</code>, verifica il colore dell’icona `DKIM` a destra del dominio interessato (vedi l’immagine qui sotto).
  ...
  <Tab label="Email Pro" availableIn={['eu']}>
    ...
    Nella sezione <code className="action">Domini associati</code>, verifica il colore dell’icona `DKIM` a destra del nome di dominio interessato (vedi l’immagine qui sotto).
...
<ZoneTabs>
  <Tab label="Exchange e Email Pro" availableIn={['eu']}>
