...
If you also need to change the **domain name holder**, you should do so **before** you change the domain name contacts. To do this, use our [change of ownership for domain names](/guides/web-cloud/domains/trade-domain) instructions.
:::
...
### Step 3: Request a domain name transfer to OVHcloud <a id="step3"></a>
...
When you are asked to provide your transfer code, enter it in the box next to your domain name. If you do not yet have the transfer code, you can tick the box labelled `Enter transfer code at a later stage`. However, we strongly advise ensuring that you have the code to hand before you continue. Remember that the transfer will not start until a valid code has been provided.

You can also complete your order with a [web hosting plan](/links/web/hosting)  and other OVHcloud solutions. This may be of interest to you, if you would also like to migrate your services to OVHcloud. Our guide to [Migrating your website to OVHcloud](/guides/web-cloud/web-hosting/hosting-migrating-to-ovh) provides instructions on how to do this.
...
- By clicking `Modify configuration` in this step, you can enter the names of the DNS servers that the domain name currently uses. This way, the domain name will already be associated with these DNS servers in the OVHcloud configuration.
...
- In some cases, the transfer process may require additional information about the domain name owner. To add this information, click `Manage Contacts/Owner`.

![domain](/images/assets/screens/website/order/order-summary.png)

Once the order has been confirmed, you will receive a free purchase order. The transfer process will only start once you have confirmed this free purchase order. Once this is done, you can track the transfer progress via the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. Once you have logged in, go to the `Web Cloud` section, then click on `Ongoing operations`.
...
Once the transfer procedure is complete, you can manage your domain name from the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. To do this, select `Web Cloud`, click `Domain names`, then click on the domain name concerned.
