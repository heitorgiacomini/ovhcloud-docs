...
- Disporre di almeno un [nome di dominio](https://www.ovhcloud.com/it/domains/);
- Disporre di una zona DNS attiva per il nome di dominio. Se necessario, consulta la nostra guida "[Creare una zona DNS in OVHcloud](/guides/web-cloud/domains/dns-zone-create)";
...
Un [nome di dominio](https://www.ovhcloud.com/it/domains/) può essere associato a diversi tipi di servizi (email, sito Web, ecc...).
...
Per funzionare correttamente, tutti i [nomi di dominio](https://www.ovhcloud.com/it/domains/) hanno bisogno di una **zona DNS**. La zona DNS è composta da informazioni tecniche chiamate *record DNS*. È come un centro di scambio.
