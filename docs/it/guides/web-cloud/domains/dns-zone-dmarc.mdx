...
Il record **D**omain-based **M**essage **A**uthentication, **R**eporting, and **C**onformance (DMARC) è un meccanismo di sicurezza e-mail. Si basa sui risultati delle verifiche [SPF](/guides/web-cloud/domains/dns-zone-spf) e [DKIM](/guides/web-cloud/domains/dns-zone-dkim).
...
- Uno dei meccanismi di autenticazione, [SPF](/guides/web-cloud/domains/dns-zone-spf) e/o [DKIM](/guides/web-cloud/domains/dns-zone-dkim) deve essere configurato nella zona DNS del nome di dominio del servizio di posta.
...
Il DMARC permette all'intestatario di un nome di dominio di gestire la sicurezza delle email emesse con il suo nome di dominio. Il suo obiettivo è:
...
- Controllare meglio l'utilizzo del nome di dominio e rilevare i tentativi di furto tramite i report inviati in caso di mancata autenticazione delle email. Inoltre, migliora la sicurezza creando un collegamento tra i protocolli SPF e DKIM.
...
DMARC interroga i meccanismi di autenticazione [SPF](/guides/web-cloud/domains/dns-zone-spf) e [DKIM](/guides/web-cloud/domains/dns-zone-dkim) per verificare le email in arrivo.<br/>
...
Il record DMARC comunica la politica da adottare in funzione dei risultati dei test SPF e DKIM. Può inoltre indicare uno o più indirizzi email (rappresentati nel nostro esempio dall’indirizzo **report@mydomain.ovh**) che servono a ricevere le notifiche di errori inviati dal nome di dominio **mydomain.ovh**.

Una volta lette le istruzioni del record DMARC del nome di dominio **mydomain.ovh** dal server di posta in arrivo di **"otherdomain.ovh"**, le email saranno consegnate verso l'indirizzo **recipient@otherdomain.ovh**, contrassegnate come "SPAM" o rifiutate.
...
Il record DMARC può essere aggiunto alla zona DNS dallo Spazio Cliente OVHcloud. Per farlo, accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e seleziona la sezione <code className="action">Web Cloud</code>. Nella colonna di sinistra, seleziona il nome di dominio nella sezione <code className="action">Domini</code> e clicca sulla scheda <code className="action">Zona DNS</code> per accedere alla zona DNS.

Una volta visualizzata la zona DNS, clicca sul pulsante <code className="action">Aggiungi un record</code> e poi su "Record email" <code className="action">DMARC</code>.

- **Sottodominio**: questa voce deve **obbligatoriamente iniziare con** `_dmarc`. Se si applica il DMARC a tutto il nome di dominio, non inserire altro che `_dmarc` in questa casella. Se definisci il tuo DMARC in un sottodominio del tuo nome di dominio principale, aggiungi il tuo sottodominio dopo `_dmarc`. Ad esempio, se vuoi applicare il DMARC a un sottodominio *subdomain.mydomain.ovh*, è necessario inserire `_dmarc.subdomain` nella casella "sottodominio" per il nome di dominio *mydomain.ovh*.
...
- **Regola per il nome di dominio (p=)**: policy che deve essere adottata dal destinatario su richiesta dell'intestatario del nome di dominio del mittente. Il criterio viene applicato al nome di dominio richiesto e ai sottodomini, a meno che il tag sottodominio **sp=** non indichi istruzioni diverse. I valori possibili sono:
    - *none* : l'intestatario del nome di dominio non richiede alcuna azione specifica riguardante la consegna dei messaggi.
...
:::warning
L’impostazione del parametro `p=` può avere un impatto importante sull’evasione delle email del nome di dominio. È consigliabile configurare `p=none` ed eseguire un'analisi dei rapporti di errore per diverse settimane, per correggere eventuali anomalie. L’upgrade a `p=quarantine` o `p=reject` richiede il pieno controllo delle impostazioni di sicurezza email relative a [SPF](/guides/web-cloud/domains/dns-zone-spf) e [DKIM](/guides/web-cloud/domains/dns-zone-dkim). L’uso del fattore `pct=`, presentato di seguito, consente una transizione graduale.

:::


- **Percentuale di messaggi filtrati (pct=)** (valore compreso tra 0 e 100, valore predefinito 100): percentuale del flusso di messaggi a cui applicare la politica DMARC. Lo scopo del tag pct è consentire ai proprietari di nomi di dominio di adottare un'implementazione lenta del meccanismo DMARC.
...
- **Regola per i sottodomini (sp=)**: criterio che deve essere adottato dal destinatario per tutti i sottodomini. Si applica solo ai sottodomini del nome di dominio richiesto e non al nome di dominio stesso. La sua sintassi è identica a quella del tag "p" definito sopra. Se il tag non è presente, per i sottodomini viene applicato il criterio specificato dal tag "p".
...
- **Modalità di allineamento SPF (aspf=)** (il valore predefinito è `r`): indica la modalità di allineamento SPF. I valori sono i seguenti:
    ...
    - `s`(strict) per la modalità strict: le email devono essere inviate dal nome di dominio dichiarato e solo da questo. Il risultato è quindi "allineato".

:::info
Per i meccanismi di autenticazione SPF e DKIM, l'**allineamento** fa riferimento alla corrispondenza tra il nome di dominio (e/o la firma del nome di dominio) utilizzato durante l'invio **e** il nome di dominio registrato in questi meccanismi.

**Esempi**

- **Allineato**: quando l’indirizzo *john.smith@mydomain.ovh* trasmette un messaggio dal servizio email associato al nome di dominio *mydomain.ovh* e i meccanismi di autenticazione SPF e DKIM sono stati configurati, si ottiene un risultato allineato.
- **Allineamento parziale**: quando l’indirizzo *john.smith@subdomain.mydomain.ovh* trasmette un messaggio dal servizio email associato al nome di dominio *mydomain.ovh*, ma i meccanismi di autenticazione SPF e DKIM sono stati configurati esclusivamente sul nome di dominio principale (ad esempio *mydomain.ovh*), si ottiene un risultato parzialmente allineato.
...
È possibile aggiungere il record TXT alla zona DNS dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, sezione <code className="action">Web Cloud</code>. Clicca su <code className="action">Domini</code> e poi seleziona il nome di dominio interessato. e clicca sulla scheda <code className="action">Zona DNS</code>.

Una volta visualizzata la zona DNS, clicca sul pulsante <code className="action">Aggiungi un record</code> e poi su "Record estesi" <code className="action">TXT</code>.
...
- **ruf** (elenco in testo normale separato da virgole): indirizzi a cui devono essere segnalate le informazioni specifiche sull'errore del messaggio. Se questo tag è presente, l'intestatario del nome di dominio di invio richiede ai destinatari di inviare rapporti dettagliati sugli errori dei messaggi di posta elettronica che non superano la valutazione DMARC in modo specifico (vedere il tag `fo` di seguito). Il formato del messaggio da generare deve essere il formato specificato per il tag `rf`. La dicitura "mailto:" deve precedere il destinatario email (esempio: `mailto:address@example.com`).
...
L’impostazione del parametro `p=` può avere un impatto importante sull’evasione delle email del nome di dominio. Si consiglia di configurare `p=none` ed eseguire un'analisi dei rapporti di errore per diverse settimane, per correggere eventuali anomalie. L’upgrade a `p=quarantine` o `p=reject` richiede il pieno controllo delle impostazioni di sicurezza email relative a [SPF](/guides/web-cloud/domains/dns-zone-spf) e [DKIM](/guides/web-cloud/domains/dns-zone-dkim). L’uso del fattore `pct=`, presentato di seguito, consente una transizione graduale.
...
- **pct=100**: la politica DMARC si applica al 50% dei messaggi provenienti dal flusso di posta elettronica dell'intestatario del nome di dominio.
