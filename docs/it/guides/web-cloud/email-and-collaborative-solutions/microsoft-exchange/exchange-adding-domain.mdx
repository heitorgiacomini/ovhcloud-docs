...
description: Come aggiungere un dominio alla piattaforma Exchange o Email Pro
...
- Poter modificare la configurazione del dominio ([zona DNS](/guides/web-cloud/domains/dns-zone-edit)).
...
1. Clicca sulla scheda <code className="action">Domini associati</code> della tua piattaforma Exchange o Email Pro.
1. Visualizzi una tabella con tutti i domini associati al tuo servizio.
1. Clicca sul pulsante <code className="action">Aggiungi un dominio</code>.
...
:::warning
Di default, tutti gli account email di una piattaforma sono interconnessi. Tutti gli indirizzi creati sul servizio di posta elettronica saranno all’interno della stessa directory degli altri indirizzi email, inclusi quelli con un dominio diverso. Per dissociare la visualizzazione dei domini, è necessario ordinare un'altra piattaforma [Exchange o Email Pro](https://www.ovhcloud.com/it/mail/) per il o i domini in questione.
...
Una volta effettuata la scelta, clicca sul pulsante <code className="action">Avanti</code>.
...
Alla fine del processo di configurazione, verifica il riepilogo delle informazioni e poi clicca sul pulsante <code className="action">Conferma</code> per confermare l’aggiunta del dominio.
...
- **se il dominio inserito non è gestito dal tuo account OVHcloud**:
    ...
    - Per una casellina `CNAME` rossa, consulta la nostra guida che spiega come [creare un record CNAME quando si aggiunge un dominio associato](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname).
...
1. Clicca sulla scheda <code className="action">Domini associati</code>.
1. Clicca sul pulsante <code className="action">...</code> nella riga del dominio corrispondente.
1. Clicca su <code className="action">Configurazione</code>.
...
Ora che i domini sono stati aggiunti al servizio, è possibile utilizzarli per configurare gli account di posta. Per effettuare questa operazione, clicca sulla scheda <code className="action">Account email</code>. Se necessario, è possibile ordinare account aggiuntivi cliccando sul pulsante <code className="action">Action</code>/<code className="action">Ordina account</code> o <code className="action">Aggiungi account</code>.
...
Una volta completata la configurazione degli account, è possibile iniziare a utilizzarli. OVHcloud mette a disposizione il servizio **webmail**, accessibile [qui](https://www.ovhcloud.com/it/mail/). Per un utilizzo ottimale del tuo indirizzo su un client, assicurati che sia compatibile con il servizio.

Per configurare l’indirizzo email su un client di posta o un dispositivo come smartphone o tablet o se hai bisogno di assistenza sulle funzionalità del tuo servizio di posta, consulta le nostre guide relative alle funzionalità del servizio di posta elettronica [Exchange](https://www.ovhcloud.com/it/emails-exchange/) e [Email Pro](https://www.ovhcloud.com/it/emails/email-pro/).

È possibile acquistare licenze Outlook nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e licenze Office 365 nella pagina [Microsoft 365](https://www.ovhcloud.com/it/collaborative-tools/microsoft-365/). Per utilizzare un client di posta Outlook o più programmi della suite Office, ti consigliamo una di queste soluzioni.
...
:::warning
Prima di eliminare un account email, assicurati che non sia in uso. Può essere necessario effettuare un backup di questi account. Se necessario, consulta la guida [Migrare manualmente l’indirizzo email](/guides/web-cloud/email-and-collaborative-solutions/migrating/manual-email-migration) che ti descriverà come esportare i dati di un account dal tuo Spazio Cliente o da un client di posta.

:::


Clicca sulla scheda <code className="action">Domini associati</code> della tua piattaforma. Dalla tabella, la colonna `Account` ti indica il numero di account associati ai domini della tua lista.
...
1. Accedi alla scheda <code className="action">Account email</code>.
1. A destra degli account da modificare, clicca sul pulsante <code className="action">...</code>.
1. Clicca su <code className="action">Modifica</code>.
...
1. Accedi alla scheda <code className="action">Account email</code>.
1. A destra dell’account da eliminare, clicca sul pulsante <code className="action">...</code>.
1. Clicca su <code className="action">Ripristina questo account</code> o <code className="action">Ripristina</code>.
...
Dalla scheda <code className="action">Domini associati</code> della tua piattaforma, clicca sul pulsante <code className="action">...</code> a destra del dominio in questione e poi su <code className="action">Elimina questo dominio</code>.
...
[Creare un record CNAME per aggiungere un dominio associato](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname)
