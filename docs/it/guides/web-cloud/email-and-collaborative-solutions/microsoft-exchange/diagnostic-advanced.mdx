...
description: Come eseguire la diagnostica automatizzata degli errori sugli account Exchange
...
- Essere aggiornato nei [pagamenti](/guides/account-and-service-management/managing-billing-payments-and-services/invoice-management#pay-bills) di questo servizio e del dominio associato
...
<img className="thumbnail" alt="Diagnostica Exchange" src="/images/web-cloud/email-and-collaborative-solutions/microsoft-exchange/diagnostic-advanced/img_4450.png" loading="lazy" />

Clicca sulla scheda <code className="action">Diagnostica</code> e seleziona l'account Exchange nel menu a tendina. Inserisci la password dell'account nel campo corrispondente e clicca su <code className="action">Avvia la diagnostica</code>.

<img className="thumbnail" alt="Diagnostica Exchange" src="/images/web-cloud/email-and-collaborative-solutions/microsoft-exchange/diagnostic-advanced/img_4451.png" loading="lazy" />

La procedura di diagnostica richiederà circa 3-10 minuti. Ecco un esempio di risultati:

<img className="thumbnail" alt="Diagnostica Exchange" src="/images/web-cloud/email-and-collaborative-solutions/microsoft-exchange/diagnostic-advanced/img_4471.png" loading="lazy" />

La pagina dei risultati propone due azioni per continuare:

- <code className="action">Nuova diagnostica</code>: avvia un'altra diagnostica.

- <code className="action">Crea una richiesta di assistenza</code>: ti permette di creare un ticket presso il nostro supporto tecnico. Il ticket conterrà i risultati della diagnostica.
...
Puoi verificarlo nella scheda <code className="action">Account email</code> del tuo servizio Exchange. L'account visualizzerà una dicitura `SPAM` nella colonna "Stato" della tabella.

Consulta la nostra guida [Cosa fare in caso di account bloccato per Spam?](/guides/web-cloud/email-and-collaborative-solutions/troubleshooting/locked-for-spam) per consentire ai nostri team di sicurezza di riattivare l'account.
...
Per riattivare il tuo abbonamento, è sufficiente riconfigurarne la [periodicità di fatturazione](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/manage-billing-exchange#periodicity) nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>.

### La politica di sicurezza ha bloccato l'account

Se nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> è attiva una politica di sicurezza, l'account può essere temporaneamente bloccato.
...
Per maggiori informazioni su questa funzionalità, consulta la nostra [guida sulla politica di sicurezza](/guides/web-cloud/email-and-collaborative-solutions/common-email-features/security-policy).

### L'autenticazione alla Webmail è fallita <a name="password"></a>

Questa operazione può essere dovuta all'inserimento di una password dell'account non corretta. Verifica che la password sia corretta, accedendo alla [Webmail](/guides/web-cloud/email-and-collaborative-solutions/using-the-outlook-web-app-webmail/email-owa) e riavvia la diagnostica.

Se necessario, modifica la password dell'account nella scheda <code className="action">Account email</code> del tuo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>. Se il problema persiste, crea una richiesta di assistenza.
...
Puoi verificare queste impostazioni nella [zona DNS del tuo dominio](/guides/web-cloud/domains/dns-zone-edit).
