...
description: Questa guida ti mostra come installare manualmente il tuo CMS Joomla!
...
Mettiamo a tua disposizione questo tutorial per supportarti nelle operazioni più frequenti. Tuttavia, in caso di difficoltà o dubbi, ti consigliamo di rivolgerti a uno [specialista del settore](https://partner.ovhcloud.com/it/directory/) o [amministratore del CMS Joomla!](https://www.joomla.org/). OVHcloud non potrà fornirti alcuna assistenza. Per maggiori informazioni, consulta la sezione ["Per saperne di più"](#go-further) di questo tutorial.

:::


:::tip
Per installare Joomla! **automaticamente** dal tuo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, consulta la nostra guida sull'[installazione di un modulo in un click](/guides/web-cloud/web-hosting/cms-install-1-click-modules).
:::
> Per installare **manualmente un altro CMS** (WordPress, Drupal, PrestaShop), consulta la nostra guida sull'[installazione manuale di un CMS](/guides/web-cloud/web-hosting/cms-manual-installation).
>
...
<img className="thumbnail" alt="Joomla Installstep 1" src="/images/assets/screens/other/cms/joomla/install-select-language-1.png" loading="lazy" />

Seleziona la lingua, inserisci il nome del tuo sito Web e clicca su <code className="action">Setup Login Data</code>.
...
<img className="thumbnail" alt="Joomla Installstep 2" src="/images/assets/screens/other/cms/joomla/install-define-admin-2.png" loading="lazy" />
...
Verifica le informazioni inserite e clicca su <code className="action">Setup Database Connection</code>.

#### 2.3 - Collega il tuo database con il tuo Joomla!

Inserisci le informazioni richieste relative al database:

<img className="thumbnail" alt="Joomla Installstep 3" src="/images/assets/screens/other/cms/joomla/install-db-connect-3.png" loading="lazy" />

Per completare i campi seguenti, consulta le informazioni indicate nello **Step 1.4** del tutorial sull'[installazione manuale di un CMS](/guides/web-cloud/web-hosting/cms-manual-installation).
...
Per i database creati su un Web Cloud Databases, consulta le informazioni indicate nello **Step 1.4** della guida sull'[installazione manuale di un CMS](/guides/web-cloud/web-hosting/cms-manual-installation).
...
- *Enter the database name*: questo nome è stato definito durante la creazione del database nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>. Se utilizzi un database incluso con il tuo hosting Web, è lo stesso nome utente del database.

- *Enter a table prefix or use the randomly generated one*: se l'installazione avviene con un nuovo database, inserisci il "prefisso" che preferisci. Se utilizzi un database già utilizzato da un altro sito, consulta **lo Step 1.4** della guida sull'[installazione manuale di un CMS](/guides/web-cloud/web-hosting/cms-manual-installation) per non inserire un "prefisso" di tavolo già utilizzato nel tuo database.

- **Connection Encryption**: lascia il valore **Default**

Clicca su <code className="action">Install Joomla</code>.

Visualizzi questo messaggio:

<img className="thumbnail" alt="Joomla Installstep 3-1" src="/images/assets/screens/other/cms/joomla/install-db-connect-3-1.png" loading="lazy" />
...
Questo file da eliminare si trova nel tuo [spazio di archiviazione FTP](/guides/web-cloud/web-hosting/ftp-connection).
...
Clicca su <code className="action">Install Joomla</code>. e poi torna al browser.

#### 2.4 - Terminare l'installazione

Una volta completata l'installazione, visualizzi la pagina seguente:

<img className="thumbnail" alt="Joomla Installstep 4" src="/images/assets/screens/other/cms/joomla/install-ending-4.png" loading="lazy" />
