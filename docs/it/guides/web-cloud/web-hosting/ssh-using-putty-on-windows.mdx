...
È necessario conoscere il nome del cluster dell’hosting Web presente nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, il nome utente FTP e la password associata. Per maggiori informazioni su questo metodo di accesso, consulta la [guida corrispondente](/guides/web-cloud/web-hosting/ftp-connection).
...
È necessario indicare l’indirizzo IP del server presente nello <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e il nome dell’account utente da utilizzare per la sessione di connessione. Per maggiori informazioni, consulta le nostre guide "Primi passi":
...
- [Come sostituire una coppia di chiavi SSH su un’istanza Public Cloud](/guides/public-cloud/compute/replacing-lost-ssh-key-pair)
...
|  |  |
|:--|:--|
| <img className="thumbnail" alt="putty" src="/images/assets/screens/other/web-tools/putty/putty3.png" loading="lazy" /> | 4\. Espandere il nodo `SSH` in `Connection` nella struttura `Category`.<br/> 5\. Espandere il nodo `Auth` nella struttura `Category`.<br/> 6\. Fare clic su `Credentials` per visualizzare le impostazioni corrispondenti.<br/> 7\. Clicca sul pulsante <code className="action">Browse</code>.<br/> 8\. Passare alla cartella in cui sono archiviati i file delle chiavi private.<br/> 9\. Aprire il file di chiave in questione. |
...
[Come creare chiavi SSH con OpenSSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)

[Come creare chiavi SSH con OpenSSH per le istanze Public Cloud](/guides/public-cloud/compute/creating-ssh-keys)
