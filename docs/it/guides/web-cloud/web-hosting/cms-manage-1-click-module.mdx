...
:::info
Se il tuo sito non appare correttamente a seguito di questa operazione, consulta le guide OVHcloud relative agli hosting condivisi nella sezione [Diagnostica](/guides/web-cloud/web-hosting/landing-page-web-hosting).
...
<Tabs>
  ...
  <Tab label="Passaggio 3">
    In caso contrario, vai alla scheda <code className="action">I miei siti</code>. Annota il nome della `Cartella di root` presente sulla riga del sito web interessato: si tratta della directory in cui si trovano i file che costituiscono il tuo modulo in 1 click sul server FTP.
  </Tab>
</Tabs>

Accedi allo [spazio FTP del tuo hosting](/guides/web-cloud/web-hosting/ftp-connection). Apri la `Cartella di root` trovata precedentemente nella scheda <code className="action">I miei siti</code> e cerca il file di configurazione del tuo modulo:
...
Se hai già raggiunto il numero massimo di database consentito dal tuo [hosting Web](/links/web/hosting), dovrai eliminare il database identificato nella [parte 1](#step1) **PRIMA** di poter creare un nuovo modulo in 1 click.
