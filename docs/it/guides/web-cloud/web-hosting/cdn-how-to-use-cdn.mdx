...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e seleziona <code className="action">Web Cloud</code>. e seleziona il tuo servizio nella sezione <code className="action">Hosting</code>. Clicca sui tre puntini <code className="action">...</code> a destra di "Opzione CDN" e poi su <code className="action">Ordina una CDN</code> o <code className="action">Attiva l'opzione</code> se l'opzione CDN è già inclusa nel tuo hosting.

:::info
Se hai un'opzione CDN precedente al 19/11/2020, puoi ordinare la nuova offerta Shared CDN cliccando su <code className="action">Aggiorna la CDN verso la versione superiore</code>.
...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e seleziona <code className="action">Web Cloud</code>. E seleziona il tuo servizio nella sezione <code className="action">Hosting</code>. Nella scheda <code className="action">Multisito</code>, clicca sul pulsante <code className="action">...</code> a destra del nome di dominio o sottodominio interessato e quindi clicca su <code className="action">Modifica il dominio</code>.

A questo punto, seleziona la voce "Attiva la CDN", clicca su <code className="action">Seguente</code> e infine su<code className="action">Conferma</code>.
...
:::warning
Nel caso di un nome di dominio esterno ad OVHcloud aggiunto su uno dei siti web del tuo hosting, devi specificare l'indirizzo IP del CDN del tuo hosting nella zona DNS del nome di dominio.

Consulta la [lista degli indirizzi IP di cluster e hosting Web](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip) per recuperare l’indirizzo IP specifico per la CDN del tuo cluster.
...
Clicca sulla scheda <code className="action">Multisito</code> del tuo hosting, clicca su <code className="action">...</code> a destra del nome di dominio o sottodominio interessato e quindi su <code className="action">Svuota la cache della CDN</code>.
...
Dopo aver scelto le opzioni, clicca su <code className="action">Applica la configurazione</code> e poi su <code className="action">Conferma la configurazione</code> nella finestra successiva.
...
Per aggiungere una regola di cache su uno degli elementi del tuo sito, vai alla scheda <code className="action">Multisito</code> del tuo hosting, clicca su <code className="action">...</code> a destra del nome di dominio o sottodominio interessato, quindi su <code className="action">Modifica la CDN</code>.

Seleziona **Regola di cache** e clicca sul pulsante <code className="action">Aggiungi una regola</code>.
...
Una volta effettuate le scelte, clicca sul pulsante <code className="action">Crea la regola</code>.

Le regole figurano in un elenco. Per modificare una regola, clicca sui tre puntini <code className="action">...</code> in corrispondenza della regola in questione e seleziona <code className="action">Modifica la regola</code>. o eliminarla cliccando su <code className="action">Elimina la regola</code>.
...
Una volta configurate le regole e selezionate le opzioni, clicca su <code className="action">Applica la configurazione</code> e poi su <code className="action">Conferma la configurazione</code> nella finestra successiva.
...
- **Cross-Origin Resource Sharing (CORS)**: Indica, nell'elenco, i domini esterni che saranno autorizzati ad accedere alle risorse del tuo sito web per condividerle. 
	...
	Una volta attivata la funzione, clicca su <code className="action">Modifica l'elenco delle risorse esterne</code> per aggiungere i domini autorizzati a condividere le tue risorse.
...
Una volta completata la lista, clicca su <code className="action">Conferma</code>.
...
:::warning
Per l'installazione di un [modulo in 1 click OVHcloud](/guides/web-cloud/web-hosting/cms-install-1-click-modules), il WAF deve essere disattivato per evitare che l'installazione del modulo sia bloccata.
...
- **Purga avanzata**: Personalizza lo spurgo scegliendo gli elementi della cache da svuotare: tutto il sito, una cartella, un'URI, un'estensione di file o tramite un'espressione regolare personalizzata. 
	...
	Nella scheda <code className="action">Multisito</code>, clicca sul pulsante <code className="action">...</code> a destra del nome di dominio o sottodominio interessato e quindi su <code className="action">Svuota la cache della CDN</code>.
...
- **Prewarm**: Costruisci il salvataggio in cache permanente delle tue risorse primordiali. La CDN anticipa e aggiorna automaticamente la cache, senza attendere la richiesta dell'utente. Questa funzione si applica solo su contenuti statici con un TTL superiore a 0 e la risorsa non deve superare 1 GB. Il livello di consumo delle risorse in *Prewarm* è indicato da un misuratore, in base alla lista degli URL. Il totale delle risorse chiamate da questi URL non deve superare la 1GB.
	...
	Per definire la lista degli URL da utilizzare in *Prewarm*, clicca su <code className="action">Modifica la lista degli URL</code>.

	Con l'aiuto dei campi `Protocollo`, `Dominio` e `Percorso della risorsa`, inserisci uno dei link verso una risorsa che vuoi aggiungere alla funzione **Prewarm** e clicca su <code className="action">Aggiungi</code>.

	Una lista si forma nel riquadro inferiore, con tutti i link che hai elencato, potrai eliminare quello che preferisci selezionandolo e poi cliccando su <code className="action">Rimuovi</code>.
...
Seleziona **Regole di cache** e clicca sul pulsante <code className="action">Aggiungi una regola</code>.
...
Le regole figurano in un elenco. Per modificare una regola, clicca sui tre puntini <code className="action">...</code> in corrispondenza della regola in questione e seleziona <code className="action">Modifica la regola</code>. Per eliminarla, clicca su <code className="action">Elimina la regola</code>.
...
Questa azione permette di disattivare il CDN per uno o più dei tuoi nomi di dominio o sottodomini, senza eliminare l'opzione CDN dal tuo hosting web.
...
Questa operazione viene eseguita automaticamente se la zona DNS attiva del dominio è gestita anche dall’<ManagerLink to="/">account OVHcloud</ManagerLink> che contiene l’hosting Web.

In caso contrario, esegui queste due operazioni:

- recupera l’indirizzo IP **di default** del cluster in cui si trova l’hosting Web utilizzando la guida "[Hosting Web: gli indirizzi IP dei nostri cluster](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip)";
...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e seleziona <code className="action">Web Cloud</code>. e seleziona il tuo servizio nella sezione <code className="action">Hosting</code>. Nella scheda <code className="action">Multisito</code>, clicca su <code className="action">...</code> a destra del nome di dominio o sottodominio interessato e quindi su <code className="action">Modifica il dominio</code>.

A questo punto, deseleziona la voce "Attiva la CDN", clicca su <code className="action">Seguente</code> e infine su<code className="action">Conferma</code>.
...
Accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e seleziona <code className="action">Web Cloud</code>. e seleziona il tuo servizio nella sezione <code className="action">Hosting</code>. Clicca sui tre puntini <code className="action">...</code> in corrispondenza di "Opzione CDN" e poi su <code className="action">Disattiva la CDN</code>.
