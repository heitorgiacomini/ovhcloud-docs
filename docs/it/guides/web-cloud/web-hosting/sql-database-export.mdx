...
description: Questa guida ti mostra come ripristinare il backup di un database di un hosting Web OVHcloud
...
- In base al metodo di backup scelto, avere accesso alla gestione dell’hosting Web dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> o disporre dei dati di connessione al database
...
- **Utilizza lo strumento di backup di OVHcloud**: permette di recuperare i backup dei tuoi database dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>. Questo metodo non richiede particolari competenze tecniche.
...
- **Effettua il backup da un comando SSH**: questo metodo richiede l’accesso allo spazio di storage FTP tramite il protocollo SSH e l’utilizzo di comandi per interagire con esso. Questo tipo di accesso richiede conoscenze avanzate e una soluzione [hosting Web OVHcloud](https://www.ovhcloud.com/it/web-hosting/) specifica.

:::tip
Se effettui un backup del tuo database perché è pieno/pieno, consulta il nostro tutorial "[Cosa fare quando il tuo database è pieno?](/guides/web-cloud/web-hosting/sql-overquota-database)".
...
Sempre nella scheda <code className="action">Database</code>, clicca sul pulsante <code className="action">...</code> a destra del database di cui vuoi effettuare il backup e seleziona <code className="action">Crea un backup</code>.
...
Sempre nella scheda <code className="action">Database</code>, clicca sul pulsante <code className="action">...</code> a destra del database di cui vuoi effettuare il backup e seleziona <code className="action">Ripristina un backup</code>.
...
Per scaricare un backup, clicca sul pulsante <code className="action">...</code> a destra del backup che vuoi ripristinare e poi su <code className="action">Scarica il backup</code>. Viene visualizzata una finestra in cui è possibile salvare il file nel computer. Conferma e attendi il completamento dell’operazione.
...
Per effettuare l’operazione, accedi a *phpMyAdmin*. Per conoscere il link di accesso a quest’ultimo, accedi al tuo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, clicca su <code className="action">Web Cloud</code>. Nella colonna di sinistra, clicca su <code className="action">Hosting</code> e seleziona il nome dell’hosting interessato. e clicca sulla scheda <code className="action">Database</code>.

Visualizzi una tabella con tutti i database creati per la tua soluzione di hosting. Clicca sul pulsante <code className="action">...</code> in corrispondenza del database interessato e seleziona <code className="action">Accedi a phpMyAdmin</code>.
...
Accedi all’interfaccia di connessione a *phpMyAdmin*, inserisci le informazioni del database ed effettua l’accesso. Accedi alla scheda <code className="action">Esporta</code>, in cui sono proposti due metodi di esportazione:
...
Una volta creato lo script è necessario caricarlo sullo spazio di storage FTP del tuo hosting Web. Per farlo, consulta le informazioni descritte allo Step 2 della guida intitolata "[Accedere allo spazio di storage](/guides/web-cloud/web-hosting/hosting-how-to-get-my-website-online)".
...
[Tutorial - Cosa fare quando il database è pieno?](/guides/web-cloud/web-hosting/sql-overquota-database)
