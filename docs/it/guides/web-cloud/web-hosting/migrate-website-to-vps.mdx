...
description: Questa guida ti mostra come migrare un sito Web da un hosting condiviso a un VPS OVHcloud
...
- Disporre di una [offerta di hosting Web](https://www.ovhcloud.com/it/web-hosting/) attiva.
- Aver sottoscritto un [VPS](https://www.ovhcloud.com/it/vps/) presente nel proprio account OVHcloud.
...
Se utilizzi WordPress, segui la nostra guida "[Salva il tuo sito WordPress](/guides/web-cloud/web-hosting/how-to-backup-your-wordpress)" per scoprire come salvare i file e il database del tuo sito Web WordPress, quindi passa allo [step 2](#step2).

#### Step 1.1 - Accedere allo spazio di storage FTP dell’hosting Web

Segui gli step della nostra guida "[Accedi allo spazio di storage FTP del tuo hosting Web](/guides/web-cloud/web-hosting/ftp-connection)" per accedere allo spazio di storage FTP del tuo hosting Web.
...
In caso di migrazione del database sul VPS, segui gli step indicati nella nostra guida "[Recuperare il backup del database di un hosting Web](/guides/web-cloud/web-hosting/sql-database-export)" per effettuare il backup del database.

### Step 2 - Configura il tuo VPS <a name="step2"></a>

:::info
Se non hai ancora attivato un VPS, accedi alla [pagina dei prodotti VPS di OVHcloud](https://www.ovhcloud.com/it/vps/) per acquistarne uno. Assicurati di scegliere il VPS che meglio si adatta alle esigenze del sito Web in termini di risorse (RAM, CPU, storage, ecc.) e alle specifiche tecniche del CMS. Per chi non ha dimestichezza con i VPS, consulta la nostra guida "[Iniziare a utilizzare un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)".
:::


#### Step 2.1 - Accedi al tuo VPS

Per accedere al VPS, consulta la sezione "Connettersi al proprio VPS" della nostra guida "[Iniziare a utilizzare un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)".
...
Per installare questo ambiente Web, consulta la nostra guida "[Installare un ambiente di sviluppo Web su un VPS o un server dedicato](/guides/bare-metal-cloud/virtual-private-servers/install-env-web-dev-on-vps)".
...
Consulta la nostra [guida sull’utilizzo di FileZilla](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp) e inserisci le seguenti informazioni:
...
- Create una directory `site` in `/home/debian/` (clic destro → <code className="action">Crea una directory</code>), quindi apritela.
...
Connettetevi al VPS tramite SSH consultando la sezione "Connessione al VPS" della guida "[Primi passi con un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)".
...
1. Connettetevi al VPS tramite SSH consultando la sezione "Connessione al VPS" della guida "[Primi passi con un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)".
...
Per un elenco completo delle best practice di sicurezza, consulta la nostra guida "[Mettere in sicurezza un VPS](/guides/bare-metal-cloud/virtual-private-servers/secure-your-vps)".
