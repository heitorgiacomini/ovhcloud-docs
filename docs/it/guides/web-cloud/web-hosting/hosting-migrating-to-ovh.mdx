...
description: Scopri come migrare il tuo sito Web, il tuo dominio, il tuo database e le tue email in OVHcloud senza interruzione di servizi
...
Diverse [offerte di hosting web OVHcloud](https://www.ovhcloud.com/it/web-hosting/) contengono un'offerta email "[MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities)". Questa offerta email permette di creare indirizzi email con uno spazio di storage massimo di 5 GB per ogni indirizzo. Scegli tra le offerte di hosting qui sotto in base alla versione PHP, alla versione SQL, al numero di indirizzi email di cui hai bisogno e alla dimensione del tuo sito da migrare:

Gamma **Eco**:

- **Starter** — 2 indirizzi email "MX Plan"
- **Personale** — 10 indirizzi email "MX Plan"

Gamma **Business**:

- **Startup** — 10 indirizzi email "MX Plan"
- **Pro** — 100 indirizzi email "MX Plan"
- **Performance** — 1000 indirizzi email "MX Plan"

Gamma **Agencies**:

- **Agency**, **Agency Plus** e **Agency Max** — 1000 indirizzi email "MX Plan" ciascuna

Una volta scelta la soluzione di hosting, se non sei ancora cliente OVHcloud, clicca su <code className="action">Ordine</code> nella pagina commerciale dell'offerta scelta. Segui gli step dell'ordine **senza richiedere il trasferimento del tuo dominio** (questa operazione verrà eseguita nello step 10 di questa guida).
...
<Tabs>
  <Tab label="Passaggio 1">
    Accedi alla pagina <ManagerLink to="/#/web/hosting">Hosting</ManagerLink>.
  ...
  <Tab label="Passaggio 2">
    In alto a sinistra dell'interfaccia, clicca sul pulsante <code className="action">Ordina</code> e poi su <code className="action">Hosting</code>.
  </Tab>
...
<Region zones={['ca']}>
:::info
OVHcloud propone altre offerte email in aggiunta al servizio "MX Plan". È possibile, ad esempio, combinare a degli indirizzi email "MX Plan" degli account ["Exchange"](https://www.ovhcloud.com/it/emails/hosted-exchange/).
...
Una volta creato il tuo hosting, accedi allo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> e crea una zona DNS per il tuo dominio **senza le "www"**. Per maggiori informazioni, consulta la guida sulla [creazione di una zona DNS OVHcloud](/guides/web-cloud/domains/dns-zone-create).

Una volta creata la zona DNS, consulta la nostra guida "[Modificare una zona DNS in OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
...
Per recuperare l'indirizzo IP di destinazione corretto per il tuo hosting OVHcloud, consulta la nostra guida che elenca gli [indirizzi IP dei diversi cluster di hosting condivisi](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip).
...
<Region zones={['ca', 'apac']}>
:::tip
Prendere nota dei due valori di destinazione con tipo di record "NS". Questi valori, di tipo `dnsXX.ovh.ca` e `nsXX.ovh.ca` (o `dns200.anycast.me` e `ns200.anycast.me`), corrispondono ai server DNS associati a questa zona DNS per il tuo dominio. e saranno utilizzati nello [step 9](#step9) di questa guida.
...
Per importare il backup dello spazio di storage FTP del tuo precedente provider, [accedi allo spazio di storage FTP del tuo hosting OVHcloud](/guides/web-cloud/web-hosting/ftp-connection) e trasferisci il backup nella cartella root "www" (o in un'altra cartella root creata precedentemente).

Ti consigliamo di utilizzare il software [FileZilla](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide) per trasferire il tuo backup FTP sul tuo hosting.
...
Per il backup del tuo database, [crea un nuovo database](/guides/web-cloud/web-hosting/sql-create-database) e [importa il backup nel tuo nuovo database](/guides/web-cloud/web-hosting/sql-importing-mysql-database).

:::info
OVHcloud propone server di database Web Cloud Databases. Per utilizzare questa offerta con il tuo sito Web, consulta la nostra documentazione su questo prodotto nella [nostra pagina dedicata](/guides/web-cloud/databases/overview).
...
:::tip
Per collegare il tuo nuovo database se utilizzi un Content Management System (CMS) come WordPress, Joomla!, Drupal o PrestaShop, consulta le informazioni sui file di configurazione nello **Step 2** della guida ["Modificare la password di un database"](/guides/web-cloud/web-hosting/sql-change-password).

:::


Dichiarate/autorizzate il vostro nome di dominio esterno sull'ospedaggio web OVHcloud tramite la nostra guida "[gestione dei siti web di un ospedaggio web OVHcloud](/guides/web-cloud/web-hosting/multisites-configure-multisite)". Dichiarare correttamente il "nome" della cartella di root scelta all'inizio dello [step 4](#step4). Ti ricordiamo che questa è la cartella in cui hai inserito i tuoi file nel tuo spazio di storage FTP.
...
Ricevi allo stesso modo gli indirizzi email presenti presso il tuo provider tramite la nostra guida sulla [creazione di indirizzi email "MX Plan"](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).

<Region zones={['eu']}>
Se hai scelto una soluzione Email Pro o Exchange, consulta la nostra documentazione per creare i tuoi indirizzi email:

- Per [Email Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/first-config)
- Per [Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-starting-hosted)
</Region>

<Region zones={['ca']}>
Se hai scelto una soluzione Exchange, consulta la nostra documentazione per creare i tuoi indirizzi email:
...
<Region zones={['ca', 'apac']}>
- Il tuo dominio (senza "www") verso la destinazione di tipo "MX": "mx1.mail.ovh.ca. ".
- Il tuo dominio (senza "www") verso la destinazione di tipo "MX": "mx2.mail.ovh.ca. ".
- Il tuo dominio (senza "www") verso la destinazione di tipo "MX": "mx3.mail.ovh.ca. ".
</Region>
...
**Opzione 1**: utilizza il nostro tool [OVHcloud Mail Migrator (OMM)](https://omm.ovhcloud.com/) che permette di copiare il contenuto degli indirizzi email registrati presso il tuo precedente provider a quelli creati presso OVHcloud. Per maggiori informazioni, consulta la guida [Migrare account email via OVHcloud Mail Migrator](/guides/web-cloud/email-and-collaborative-solutions/migrating/omm-migrate-email-account-to-ovhcloud).

Ti consigliamo di non utilizzare <code className="action">Tipo di server</code> **POP** nella sezione <code className="action">Account sorgente</code>. Questo protocollo cancella le email del tuo vecchio server per inviarle verso il server OVHcloud di destinazione. In questo modo non è più possibile confrontare il contenuto del vecchio e del nuovo indirizzo email.

Nella sezione <code className="action">Account di destinazione</code>, inserisci solo l'indirizzo email OVHcloud e la password associata. <code className="action">Tipo di server</code> in <code className="action">Hosted by OVH (Autodetect)</code>.
...
- Consulta la guida Per maggiori informazioni sui parametri di configurazione [Gestisci le email MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities#consult-emails-client). e i link alle guide personalizzate per i principali client di posta.

<Region zones={['eu']}>
#### Per gli account "Email Pro":

- Consulta le nostre guide all'aiuto alla configurazione nelle sezioni `Configurazione su computer` e `Configurazione su smartphone` de [la nostra guida sull'offerta Email Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/overview).
</Region>

<Region zones={['eu', 'ca']}>
#### Per gli account email "Exchange":

- Tutte le nostre guide all'aiuto alla configurazione sono disponibili nelle sezioni `Configurazione di Exchange sul computer` e `Configurazione di Exchange su smartphone` di [la nostra guida sull'offerta Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/overview).
</Region>
...
<Region zones={['ca', 'apac']}>
Sostituisci i server DNS correnti (del registrar di origine) con i due server DNS dichiarati nella zona DNS OVHcloud (di tipo `dnsXX.ovh.ca` e `nsXX.ovh.ca` o `dns200.anycast.me` e `ns200.anycast.me`). Questa operazione viene effettuata nell'interfaccia di gestione del registrar di origine.
</Region>
...
In seguito, trasferisci il tuo dominio nella guida su [trasferisci un dominio in OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain).
...
[Generalità sulle email condivise](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities).

[Generalità sui server DNS](/guides/web-cloud/domains/dns-server-general-information).

[Creare un indirizzo email condiviso](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).

[Importa un database MySQL](/guides/web-cloud/web-hosting/sql-importing-mysql-database).
