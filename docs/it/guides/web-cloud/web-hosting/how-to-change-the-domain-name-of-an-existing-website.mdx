...
Dichiara il nuovo nome di dominio seguendo la nostra guida « [Come associare un nome di dominio a un sito web esistente ?](/guides/web-cloud/web-hosting/my-websites-add-domain-existing-website) ». Dovrete inoltre dichiarare il sottodominio in `www` se desiderate, ad esempio, che `www.NewDomain.tld` mostri il vostro sito insieme a `NewDomain.tld`.
...
<Tabs>
  ...
  <Tab label="Passaggio 2">
    Clicca sulla scheda <code className="action">Informazioni generali</code>.
  </Tab>
...
:::warning
Se attivate le opzioni **IP del paese** o **CDN** con il vostro nuovo dominio, utilizzate il giusto indirizzo IP con la nostra documentazione che censisce [l'insieme degli indirizzi IP dei nostri hosting condivisi](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip).

Per visualizzare il numero del cluster in cui si trova il tuo hosting, consulta la nostra guida "[Hosting Web - Come conoscere cluster e filer](/guides/web-cloud/web-hosting/how-to-know-cluster-and-filer)".
...
Per evitare il « *Duplicate-content* » e quando il tuo nuovo nome di dominio è pienamente operativo con il tuo sito web, dovrai eliminare la dichiarazione del vecchio nome di dominio sul tuo sito web utilizzando la guida « [Come separare un dominio da un sito web esistente](/guides/web-cloud/web-hosting/my-websites-detach-domain-existing-website) ».
...
[Elenco degli IP dei nostri hosting condivisi](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip)
