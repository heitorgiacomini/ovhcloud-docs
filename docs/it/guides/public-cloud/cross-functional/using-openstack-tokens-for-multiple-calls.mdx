...
description: Come creare e utilizzare token OpenStack per le tue azioni
...
- Per gli utenti di Windows, consulta questa guida per esportare le tue variabili d'ambiente: [Impostare le variabili d'ambiente OpenStack](/guides/public-cloud/cross-functional/compute-set-openstack-environment-variables).

## Procedura

### Step 1: scarica e verifica il file OpenRC

Clicca su <code className="action">Utenti e ruoli</code> nella sezione **Impostazioni** e poi sul pulsante <code className="action">...</code> a destra del tuo utente OpenStack.<br/>
Scarica il file OpenRC di questo utente e specifica la Region in cui vuoi effettuare le azioni.

<img className="thumbnail" alt="scarica il file openRC" src="/images/public-cloud/cross-functional/using-openstack-tokens-for-multiple-calls/openrc.png" loading="lazy" />
