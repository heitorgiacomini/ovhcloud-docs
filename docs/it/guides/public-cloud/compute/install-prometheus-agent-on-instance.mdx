...
description: "Scopri come installare Prometheus Node Exporter o Windows Exporter su un'istanza Public Cloud OVHcloud per raccogliere metriche"
...
- [Creare un'istanza dallo Spazio Cliente OVHcloud](/guides/public-cloud/cross-functional/create-a-public-cloud-project).
- [Disporre di un accesso amministratore all'istanza](/guides/public-cloud/compute/getting-started).
...
:::info
Su Windows, utilizza PowerShell con SSH o un client SSH come [PuTTY](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows) se preferisci utilizzare un'interfaccia a riga di comando.
...
<Tabs>
  <Tab label="Per Debian / Ubuntu">
    ```bash
    sudo apt update && sudo apt upgrade -y
    ```
  </Tab>
  <Tab label="Per CentOS / RHEL">
    ```bash
    sudo yum update -y
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab label="Per Windows (tramite SSH / PowerShell sulla VM)">
    :::info
...
<Tabs>
  ...
  <Tab label="Per Windows (tramite SSH / PowerShell)">
    ```powershell
...
<Tabs>
  <Tab label="Per Linux (Debian / Ubuntu con UFW)">
    ```bash
