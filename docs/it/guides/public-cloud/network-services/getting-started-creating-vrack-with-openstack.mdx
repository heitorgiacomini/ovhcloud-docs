...
description: Come attivare e gestire una vRack Public Cloud con OpenStack CLI
lastUpdated: 2025-01-13
---

## Obiettivo

La [vRack](https://www.ovhcloud.com/it/network/vrack/) è una rete privata che permette di configurare l'indirizzamento tra diversi server dedicati OVHcloud. Permette anche di aggiungere [istanze Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance) alla tua rete privata per creare un'infrastruttura di risorse fisiche e virtuali.

**Questa guida ti mostra come configurare le tue istanze Public Cloud con OpenStack CLI**

## Prerequisiti

- Disporre di un [progetto Public Cloud](/guides/public-cloud/cross-functional/create-a-public-cloud-project)
- Aver [creato un utente OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user#crea-un-utente-openstack) (facoltativo)
...
:::info
Questo non si applica ai progetti appena creati che vengono consegnati automaticamente con una vRack. Per visualizzare la vRack dopo aver creato il progetto, accedi al menu <code className="action">Network</code> nella scheda a sinistra e clicca su <code className="action">Rete Privata vRack</code> per visualizzare le vRack.
...
Nel menu a sinistra dello schermo, clicca sul pulsante <code className="action">Aggiungi un servizio</code> (icona del carrello degli acquisti). Utilizza il filtro in alto o scorri verso il basso per trovare il servizio <code className="action">vRack</code>.
...
Una volta attivato il servizio, è disponibile nello Spazio Cliente, sezione <code className="action">Network</code> > <code className="action">Rete Privata vRack</code>. Con la denominazione "pn-xxxxxx".

Nella lista dei servizi compatibili, seleziona il progetto che vuoi aggiungere alla vRack e clicca sul pulsante <code className="action">Aggiungi</code>.

<img className="thumbnail" alt="aggiungi il progetto" src="/images/public-cloud/network-services/getting-started-09-creating-vrack-with-openstack/addprojectvrack.png" loading="lazy" />

<a name="ovhcloud-api"></a>

#### Dalle APIv6 OVHcloud

Per attivare e gestire una vRack, clicca [qui](/guides/public-cloud/network-services/vrack-apiv6#step-1-activating-and-managing-a-vrack) (EN), per consultare la guida specifica a questo metodo.
...
Ricorda anche queste informazioni, come indicato nella [guida all'utilizzo dell'API Nova](/guides/public-cloud/compute/starting-with-nova):
...
[Configurazione della vRack Public Cloud da APIv6 OVHcloud](/guides/public-cloud/network-services/vrack-apiv6) (EN).

[Server dedicati - Creare due o più VLAN nella vRack](/guides/bare-metal-cloud/dedicated-servers/creating-multiple-vlans-in-a-vrack).
