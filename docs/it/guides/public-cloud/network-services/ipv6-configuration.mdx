...
description: "Come configurare il protocollo IPv6 su un'istanza Public Cloud"
...
:::info
Al momento, le offerte Floating IP e Gateway non supportano l'IPv6. L'IPv6 è utilizzabile solo con le istanze in [modalità pubblica](/guides/public-cloud/network-services/concepts#publicmode).
...
Nel tuo progetto Public Cloud, clicca su <code className="action">Istanze</code> nella barra di navigazione a sinistra. Poi clicca su <code className="action">...</code> accanto all’istanza corrispondente e clicca su <code className="action">Dettagli dell’istanza</code>.
...
<Tabs>
  <Tab label="Debian (tranne Debian 12)">
  ...
  <Tab label="Red Hat / CloudLinux / Rocky Linux (8 & 9) / AlmaLinux (8 & 9)">
  ...
  <Tab label="Windows">
    ...
    Vai alla sezione <code className="action">Connessione di rete</code> di Windows.
...
Clicca con il tasto destro sulla scheda di rete per accedere a <code className="action">Proprietà</code>.
...
Clicca su <code className="action">Internet Protocol Version 6 (TCP/IPv6)</code> e poi sul pulsante <code className="action">Proprietà</code>.
...
Una volta terminata, spunta la casella `Conferma le impostazioni all’uscita` e clicca sul pulsante <code className="action">OK</code> per confermare le modifiche.
  </Tab>
</Tabs>
...
In un primo momento [riavvia la tua istanza in modalità di ripristino Rescue-pro](/guides/public-cloud/compute/put-an-instance-in-rescue-mode).
