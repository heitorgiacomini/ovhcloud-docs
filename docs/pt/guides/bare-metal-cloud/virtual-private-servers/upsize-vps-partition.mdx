...
description: Saiba como aumentar a quantidade de espaço útil no disco após uma atualização
...
- Ter reiniciado o servidor em [modo rescue](/guides/bare-metal-cloud/virtual-private-servers/rescue) (apenas Linux).
...
Se o VPS ainda não estiver em modo rescue, ative-o graças ao [nosso guia](/guides/bare-metal-cloud/virtual-private-servers/rescue).
...
- `bad magic number in superblock`: não continuar. Para resolver este problema, consulte a secção “[Como corrigir os erros *bad magic number in superblock*](/guides/bare-metal-cloud/virtual-private-servers/upsize-vps-partition#como-corrigir-os-erros-bad-magic-number-in-superblock)” deste manual.
...
Antes de eliminar a antiga partição, recomendamos que anote o número correspondente ao primeiro setor da partição. Para obter esta informação, execute o comando <code className="action">p</code>. O número é o que aparece no campo `Start`. Conserve esta informação para ser usada mais tarde.
...
Em seguida, elimine a partição através do comando <code className="action">d</code>.
...
Agora é necessário criar uma nova partição executando o comando <code className="action">n</code>. Recomendamos a utilização dos valores predefinidos.
...
A seguir, certifique-se de que a partição seja de arranque (bootable). Para tal, utilize o comando <code className="action">a</code>.
...
Guarde as alterações e saia da aplicação com o comando<code className="action">w</code>:
...
Se o comando <code className="action">e2fsck</code> devolver a mensagem de erro`bad magic number in superblock`, deverá verificar e corrigir o sistema de ficheiros utilizando um superbloco de backup. Para ver os superblocos de backups disponíveis, execute este comando:
...
<img className="thumbnail" alt="Serviços de File and Storage" src="/images/bare-metal-cloud/virtual-private-servers/upsize-vps-partition/file-and-storage.png" loading="lazy" />

#### Redimensionar o volume

Clique com o botão direito do rato em C: e selecione <code className="action">Extend Volume...</code>.
...
Introduza o tamanho pretendido e clique em <code className="action">OK</code>. O seu volume será então aumentado.
