...
description: Saiba como diagnosticar um VPS consultando os logs de início (boot logs)
...
Se o seu VPS não responder, deverá sempre poder aceder ao VPS a partir da Área de Cliente através do [KVM](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps). A forma mais rápida de diagnosticar o problema é verificar os logs de arranque (boot logs) do servidor. No entanto, a configuração GRUB deve ser modificada para que estes logs apareçam. 
...
Se encontrar dificuldades para efetuar estas ações, contacte um fornecedor de serviços especializado e/ou troque informações com a nossa comunidade de utilizadores [https://community.ovh.com/en/](https://community.ovh.com/en/). A OVHcloud não poderá fornecer-lhe assistência técnica a este respeito.
:::
...
Lance um reboot ao servidor em modo rescue a partir da Área de Cliente. Se necessário, consulte o nosso [guia sobre o modo rescue](/guides/bare-metal-cloud/virtual-private-servers/rescue).
...
Depois de efetuar as modificações, reinicie o seu VPS em modo "normal" a partir da sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. Os logs de boot devem aparecer aquando da utilização da [consola KVM](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps).

## Quer saber mais?

[Utilizar o KVM para os VPS](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps)
