...
description: Identifique um disco com defeito no seu servidor dedicado OVHcloud e solicite a sua substituição através da área de cliente
...
Se possui um servidor que recorre a RAID por software, consulte o guia [«RAID por software»](/guides/bare-metal-cloud/dedicated-servers/raid-soft) para encontrar os discos instalados no seu servidor.
...
Se possui um servidor que recorre a RAID por hardware, consulte o guia [«RAID por hardware» - EN](/guides/bare-metal-cloud/dedicated-servers/raid-hard) e realize o procedimento adequado ao seu tipo de controlador RAID para encontrar os caminhos de acesso aos discos.
...
No caso de um disco NVMe, será preciso pôr o servidor em [modo rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) e instalar a ferramenta `nvme-cli`.
...
- **O número de série do disco a substituir, bem como de todos os outros discos sem problemas**. Para obter o número de série do disco a substituir, consulte [este guia](/guides/bare-metal-cloud/dedicated-servers/how-to-find-hdd-serial). Se por alguma razão não conseguir obter o número de série do disco, queira mencioná-lo no ticket e comunicar-nos o número de série do(s) disco(s) a não substituir. 
...
Este tipo de substituição só é possível para os [servidores dedicados](https://www.ovhcloud.com/pt/bare-metal/) com uma placa RAID.
> 
...
Se possui um servidor em RAID por software, convém que execute manualmente a ressincronização dos discos. Para isso, consulte a documentação associada ao [«RAID por software»](/guides/bare-metal-cloud/dedicated-servers/raid-soft).

## Quer saber mais?

[RAID por software](/guides/bare-metal-cloud/dedicated-servers/raid-soft)

[RAID por hardware - EN](/guides/bare-metal-cloud/dedicated-servers/raid-hard)

[Modo rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)
