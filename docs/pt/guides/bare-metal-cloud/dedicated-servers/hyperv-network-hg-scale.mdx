...
description: Descubra como configurar a rede no Windows Server com Hyper-V
...
Se pretender utilizar endereços MAC virtuais para configurar a rede com Hyper-V nos seus servidores das gamas High Grade ou Scale, pode consultar [este manual](/guides/bare-metal-cloud/dedicated-servers/network-bridging).
...
Para este tipo de configuração, é possível utilizar a funcionalidade IP como alias descrita no artigo [Como configurar um IP alias](/guides/bare-metal-cloud/dedicated-servers/network-ipaliasing).
...
* Dispor de um endereço [Additional IP](https://www.ovhcloud.com/pt/network/additional-ip/)
...
:::info
Verifique que a sua configuração é semelhante. Dispõe das informações relativas aos MAC e interfaces públicas ou privadas na sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> ou através da API OVHcloud.

:::


Volte ao Server Manager, vá ao <code className="action">Local Server</code> e clique em <code className="action">Disabled</code> junto de "NIC Teaming".
...
Na página seguinte, clique com o botão direito do rato sobre uma das interfaces públicas previamente identificadas e, a seguir, em <code className="action">Add to New Team</code>.
...
Atribua um nome ao seu *teaming* e adicione a segunda interface ao *teaming*. A seguir, abra as propriedades adicionais, defina "Teaming Mode" em "LACP" e clique em <code className="action">OK</code>.
...
Carregue em `Windows Key` + `R` para abrir uma janela Run. Introduza `ncpa.cpl` e clique em <code className="action">OK</code>. Isto abrirá a janela "Ligações de rede".
...
Clique com o botão direito do rato no *teaming* que criou e clique em <code className="action">Properties</code>.
...
Depois de introduzir os endereços, clique em <code className="action">OK</code> para fechar a janela e novamente em <code className="action">OK</code> para fechar a janela das propriedades do adaptador.
...
No Server Manager, selecione o <code className="action">Dashboard</code> e clique em <code className="action">Add roles and feature</code>.
...
Por fim, na secção "Confirmation", selecione `Restart the destination server automatically if required` e clique em <code className="action">Install</code>.
...
Abra a nova aplicação chamada "Routing and Remote Access", clique com o botão direito do rato no seu servidor e clique em <code className="action">Configure and Enable Routing and Remote Access</code>.
...
Escolha `Custom configuration` e clique em <code className="action">Next</code>.
...
De seguida, selecione `LAN Routing` e clique em <code className="action">Next</code>.
...
Por fim, clique em <code className="action">Finish</code> e, a seguir, em <code className="action">Start Service</code>, na janela que aparecerá.
...
Clique com o botão direito do rato no seu cartão vEthernet e clique em <code className="action">Properties</code>.
...
A seguir, clique no botão `Advanced...` e, na nova janela, em <code className="action">Add...</code> e em endereços IP.

Adicione o endereço IP e a máscara de sub-rede correspondente ao seu Additional IP e clique em <code className="action">Add</code>
...
Depois de introduzir todos os endereços, clique em <code className="action">OK</code> para fechar a janela avançada, novamente em <code className="action">OK</code> para fechar os parâmetros TCP/IPv4 e, uma última vez, em <code className="action">OK</code> para fechar a janela das propriedades do cartão.

:::warning
Esta etapa pode provocar uma perda de ligação. Se isso acontecer, ligue-se através do [IPMI](/guides/bare-metal-cloud/dedicated-servers/ipmi) e altere novamente a configuração. Poderá verificar que a gateway predefinida voltou ao estado vazio. Adicione a gateway 100.64.0.1.
...
- Dispor de um [servidor compatível com o vRack](https://www.ovhcloud.com/pt/bare-metal/).
...
Na página seguinte, clique com o botão direito do rato sobre uma das interfaces privadas anteriormente identificadas e, a seguir, em <code className="action">Add to New Team</code>.
