...
A melhor forma de verificar a interface correta para o vRack é verificar o separador <code className="action">Interfaces de rede</code> do seu servidor na sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. Na tabela abaixo, tome nota do endereço MAC, que é também o **nome** da interface **Privada**.
...
Na linha que começa por ```link ether``, pode verificar que esta interface corresponde à interface **Private** indicada na sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. Utilize este nome de interface para substituir `NETWORK_INTERFACE` nas configurações abaixo (exemplo: `eno2`).
