...
description: Reconstrua a configuração do espelho RAID por software num servidor dedicado Windows após a substituição de um disco
...
Depois de se conectar, clique com o botão direito do menu <code className="action">Iniciar</code> e abra a <code className="action">Executar</code>.
...
Introduza `cmd` e clique em <code className="action">OK</code>.
...
Neste exemplo, o `Disk 1` é um disco de substituição que foi instalado para substituir o `Disk M0` defeituoso que tinha sido [fisicamente removido](/guides/bare-metal-cloud/dedicated-servers/disk-replacement) anteriormente.
