...
- IPv4: [Configurar um bloco de endereços IP no vRack](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack).
- IPv6: [Configurar um bloco IPv6 numa vRack (EN)](/guides/bare-metal-cloud/dedicated-servers/configure-ipv6-in-vrack).
...
Este guia fornece as instruções necessárias para realizar as operações mais habituais. Se encontrar dificuldades ou dúvidas relativamente à administração, à utilização ou à segurança de um servidor, deverá contactar um [fornecedor especializado](https://partner.ovhcloud.com/pt/directory/). Para mais informações, consulte a secção ["Quer saber mais"](#go-further) neste guia.
...
- Ter conhecimentos básicos de [SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction) e redes.
...
<Tabs>
  <Tab label="Através da Área de Cliente">
    A gateway IPv6 associada ao seu servidor é apresentada na secção "Rede" do separador <code className="action">Informações gerais</code>. Uma vez copiado, continue para a aplicação de configuração IPv6.
  ...
  <Tab label="Através das API OVHcloud">
    Outra forma de recuperar as informações de rede do seu servidor é [utilizar a API OVHcloud](/guides/manage-and-operate/api/first-steps).
...
O ficheiro está em imparidade. Por predefinição, o NetworkManager não cria novos perfis neste formato. O ficheiro de configuração encontra-se agora em `/etc/NetworkManager/system-connections/`.
  ...
  <Tab label="Debian 12, Ubuntu 22.04 e posteriores">
  ...
  <Tab label="AlmaLinux (8/9) e Rocky Linux (8/9)">
  ...
  <Tab label="Windows Server 2019 e versões posteriores">
    **Passo 1: Usar o RDP para se conectar ao servidor**
    
    [Encontre mais informações neste guia.](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#ligar-se-ao-servidor)
    
    **Passo 2: Abrir a configuração da rede do servidor**
    
    Em primeiro lugar, faça clique com o botão direito no ícone de rede situado na área de notificações. Aceda ao <code className="action">Network and Sharing Center</code>.
    
    <img className="thumbnail" alt="Network and Sharing Center" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_sharing_center.png" loading="lazy" />

Clique em <code className="action">Change adapter settings</code>.
    
    <img className="thumbnail" alt="Change adapter settings" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_change_adapter_settings.png" loading="lazy" />

Faça clique com o botão direito sobre o seu adaptador de rede e, de seguida, clique em <code className="action">Properties</code>.
    
    <img className="thumbnail" alt="Network Adapter Properties" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_adapter_properties.png" loading="lazy" />

Selecione <code className="action">Internet Protocol Version 6</code> e clique em <code className="action">Properties</code>.
    
    <img className="thumbnail" alt="Properties" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_properties.png" loading="lazy" />

**Passo 3: Corrigir a configuração de rede**
    
    Introduza a sua configuração IPv6 (`IPv6 address` e `Default Gateway`), marque a caixa `Validar os parâmetros ao sair` e clique no botão <code className="action">OK</code> para validar as suas alterações.
    
    <img className="thumbnail" alt="Properties" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_configuration.png" loading="lazy" />
  </Tab>
</Tabs>
...
Num primeiro tempo, [coloque o seu servidor em modo rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode).
