...
:::info
Este artigo diz respeito à configuração de endereços Additional IPv4 numa interface pública. Pode igualmente configurar endereços IPv6 nos seus servidores dedicados utilizando [este manual](/guides/bare-metal-cloud/dedicated-servers/network-ipv6).
...
Saiba como configurar o Additional IP num vRack com os nossos manuais para [IPv4](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack) e [IPv6](/guides/bare-metal-cloud/dedicated-servers/configure-ipv6-in-vrack).
:::
...
- Dispor de um ou vários endereços [Additional IP](https://www.ovhcloud.com/pt/network/additional-ip/).
...
<Tabs>
  ...
  <Tab label="cPanel">
    ...
    **1 - Aceder à secção gestão IP da WHM**
    ...
    <img className="thumbnail" alt="Adicionar um novo endereço IP" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/Cpanel-1.png" loading="lazy" />
    ...
    **2 - Adicionar as informações dos Adicionais IP**
    ...
    <img className="thumbnail" alt="indicar novas informações sobre o novo endereço IP" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/Cpanel-2024.png" loading="lazy" />
    ...
    **3 - Verificar a configuração IP atual**
    ...
    De volta para a secção <code className="action">IP Functions</code>, clique em <code className="action">Show or Delete Current IPs</code> para verificar que o endereço Additional IP foi corretamente adicionado.
  ...
  <Tab label="Windows Server">
    ...
    Abra a linha de comando <code className="action">cmd</code> ou o <code className="action">powershell</code> e introduza este comando:
    ...
    **Através da linha de comandos (recomendado)**
    ...
    <details>
    <summary>**Através da interface gráfica**</summary>

    1. Aceda ao menu <code className="action">Start</code> > <code className="action">Control Panel</code> > <code className="action">Network and Internet</code> > <code className="action">Network and Sharing Centre</code> > <code className="action">Change Adapter Settings</code> (no menu à esquerda).
    
    2. Clique com o botão direito do rato na sua ligação de rede, no nosso exemplo <code className="action">Ethernet 2</code>.
    
    3. Clique em <code className="action">Properties</code>.
    
    4. Selecione o <code className="action">Internet Protocol Version 4 (TCP/IPv4)</code>, e clique em <code className="action">Properties</code>.
    
    5. Clique em <code className="action">Use the following IP address</code> e introduza o IP principal do servidor, a máscara de sub-rede e o *gateway* predefinido, apresentados após a execução do comando `ipconfig` (ver exemplo acima). Em `Preferred DNS Server`, introduza 213.186.33.99.
    
    <img className="thumbnail" alt="Internet Protocol Version 4 (TCP/IPv4) Properties" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/guides-network-ipaliasing-windows-2008-2.png" loading="lazy" />
    
    :::warning
    Atenção: se introduzir informação incorreta, o servidor ficará inacessível. Será obrigado a efetuar as correções em modo [WinRescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode#windows) ou através do [KVM](/guides/bare-metal-cloud/dedicated-servers/ipmi).
    :::
    
    Depois, clique em <code className="action">Advanced</code> (nas <code className="action">TCP/IP Settings</code>).
    ...
    Na parte <code className="action">IP Address</code>, clique em <code className="action">Add</code>:
    ...
    Introduza o Additional IP e a máscara de sub-rede **255.255.255.255**. Em seguida, clique em <code className="action">Add</code>.
    ...
    Clique em <code className="action">OK</code> para validar a sua configuração.
  ...
  <Tab label="Plesk">
    ...
    **1 - Aceder à gestão de IP do Plesk**
    ...
    <img className="thumbnail" alt="acesso à gestão dos endereços IP" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/pleskip1.png" loading="lazy" />
    ...
    **2 - Adicionar informações IP suplementares**
    ...
    <img className="thumbnail" alt="adicionar informações IP" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/Plesk-2024.png" loading="lazy" />
    ...
    <img className="thumbnail" alt="adicionar informações IP" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/Plesk-2024-1.png" loading="lazy" />
    
    **3 - Verificar a configuração IP atual**
    
    Na secção `IP Addresses`, verifique se o endereço Additional IP foi adicionado corretamente.
    
    <img className="thumbnail" alt="configuração IP atual" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/Plesk-2024-2.png" loading="lazy" />
  </Tab>
</Tabs>
...
Se não conseguir estabelecer uma ligação entre a rede pública e o seu alias IP e suspeitar de um problema de rede, reinicie o servidor em [modo rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) e configure o alias diretamente no servidor.
