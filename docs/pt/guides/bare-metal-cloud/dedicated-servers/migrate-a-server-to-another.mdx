---
title: Migrar os dados de um servidor dedicado para outro
description: Migre os dados entre servidores dedicados OVHcloud com rsync, SFTP ou o backup storage para uma transição sem problemas
...
Este guia explica como implementar algumas medidas para tornar o seu sistema mais seguro. Se encontrar alguma dificuldade relacionada com o processo, deverá contactar um [serviço especializado](https://partner.ovhcloud.com/pt/directory/). Para mais informações, aceda à secção deste guia intitulada: «Quer saber mais?»
:::
...
- A forma mais simples é utilizar um software adequado como [SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp).
- A outra opção consiste em [sincronizar os dois servidores um com o outro](/guides/bare-metal-cloud/dedicated-servers/how-to-copy-data-from-one-dedicated-server-to-another-using-rsync).
...
Para mais informações, consulte o nosso guia sobre [utilizar o Backup Storage num servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage).
...
Se a reputação dos seus endereços IP é importante, recomendamos vivamente que utilize endereços [Additional IP](https://www.ovhcloud.com/pt/network/additional-ip/) migração, pois estes podem ser conservados se efetuar uma migração.

Quando tiver estes endereços IP, basta migrá-los para o novo servidor.
Para isso, consulte o nosso guia sobre: [Migrar um Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip).
...
Para mais informações queira ler a nossa documentação sobre domínios e DNS.
