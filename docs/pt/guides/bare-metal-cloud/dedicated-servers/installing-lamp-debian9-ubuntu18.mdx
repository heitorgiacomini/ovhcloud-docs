...
description: Saiba como configurar um servidor web LAMP
...
Se uma distribuição Debian ou Ubuntu não estiver já instalada no seu servidor, efetue uma reinstalação a partir da sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. É a melhor forma de ter um sistema limpo para o seu servidor web e as aplicações que se executam.

Siga o guia correspondente para instalar uma distribuição no seu serviço OVHcloud e aceder-lhe através de [SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction):
...
Confirme a primeira chamada apoiando-se na <code className="action">Entrada</code>.
...
Recomenda-se a utilização do método de autenticação proposto (*unix_socket*) em vez do acesso por palavra-passe root. Carregue <code className="action">y</code> e depois em <code className="action">Enter</code>. Se optar por utilizar o acesso root ao utilizador, escolha <code className="action">n</code> e defina uma palavra-passe root ao seguinte convite.
...
Os convites seguintes relativos às medidas de segurança, confirme-as com <code className="action">y</code> até ao fim do script.
...
Para instalar o WordPress num servidor, pode seguir [este tutorial](/guides/public-cloud/compute/install-wordpress).
...
[A configuração de uma firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-linux-iptable) (*iptables*) permitirá melhorar a segurança do seu servidor. Este processo pode ser simplificado utilizando o Frontend "Uncomplated Firewall" (UFW) e o seu conjunto de perfis pré-definidos. 
...
O acesso à instalação do seu servidor web através de um nome de domínio requer que o associe ao seu serviço. Para isso, deve editar a zona DNS acessível a partir do seu <ManagerLink to="/">Espaço Cliente OVHcloud</ManagerLink>, na condição de a OVHcloud ser o seu ambiente de registo **e** o nome de domínio utilizar os servidores DNS da OVHcloud.

Para saber mais, consulte o guia "[Editar uma zona DNS](/guides/web-cloud/domains/dns-zone-edit)". Se o domínio estiver a ser utilizado atualmente, só poderá configurar os DNS depois de o website ou a aplicação estarem prontos.
...
Em alternativa, a OVHcloud oferece-lhe a solução [SSL Gateway](https://www.ovhcloud.com/pt/security/ssl-gateway/). Para mais informações, consulte o [nosso manual](/guides/web-cloud/ssl-gateway/order).
...
[Como transferir e recuperar dados para um servidor através de SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp)
