...
![OVHcloud Load Balancer Service](/images/guides/network/load-balancer/create-http-https/iplb_service.png)
...
![Adding a server farm via the OVHcloud Control Panel](/images/guides/network/load-balancer/create-http-https/add_backend.png)

Click the `Add` button once you have filled in the fields.

Your server farm should appear in the list, under the `Server Farms` tab.

![Details of the created server farm](/images/guides/network/load-balancer/create-http-https/resume_backend.png)
...
### Add a server <a id="server"></a>

We will now add a server to our server farm.

#### From the OVHcloud Control Panel

Still in the `Server Farms` tab, select the farm to which you want to add a server by clicking on the corresponding line. The list of servers already configured in the farm appears below the list of farms, as well as the `Add a server` button. Click on this button to add a new server.
...
![Adding a server to a Farm.](/images/guides/network/load-balancer/create-http-https/add_server.png)

Click the `Add` button once you have filled in the fields.

Your server should appear in the list of servers, in the `Server Farms` tab, just below the list of farms.

![Details of the created server.](/images/guides/network/load-balancer/create-http-https/resume_server.png)
...
### Add a frontend <a id="frontend"></a>
...
In the `Frontends` tab, click the `Add a frontend` button.
...
![Adding a frontend](/images/guides/network/load-balancer/create-http-https/add_frontend.png)

Click the `Add` button once you have filled in the fields.

Your frontend should appear in the list, under the `Frontends` tab.

![Details of the created frontend](/images/guides/network/load-balancer/create-http-https/resume_frontend.png)
...
### Add an SSL/TLS certificate <a id="certificate"></a>
...
The list of SSL/TLS certificates configured on your OVHcloud Load Balancer service is in the `SSL Certificates` tab. This interface offers the 2 options mentioned above, namely importing an existing certificate (via the `Add an SSL certificate` button) and `Order an SSL certificate` managed automatically by your OVHcloud Load Balancer service.

![Certificate addition buttons](/images/guides/network/load-balancer/create-http-https/certificat.png)

If you choose to import an existing SSL/TLS certificate, click on `Add an SSL certificate`. The *Private Key* and *Certificate* fields are mandatory.

Click the `Add` button once the fields are filled in. Your certificate will then appear in the certificate list.

![Adding an existing certificate](/images/guides/network/load-balancer/create-http-https/add_certificat_custom.png)

To add a Let's Encrypt certificate, click on `Order an SSL certificate`, enter your domain, make sure it points to your OVHcloud Load Balancer service and follow the emails you receive. You will then see it appear in the list of available certificates.

![Ordering a Let's Encrypt certificate](/images/guides/network/load-balancer/create-http-https/add_certificat_letsencrypt.png)
...
### Apply the changes <a id="apply"></a>
...
Go to the home page of your OVHcloud Load Balancer service, click on the `...` dots next to the name of your service and click on `Apply the configuration`.

![Apply the configuration](/images/guides/network/load-balancer/create-http-https/btn_apply_configuration.png)

Then select the list of zones you want to deploy and click on the `Apply the configuration` button.

![Apply the configuration zones](/images/guides/network/load-balancer/create-http-https/btn_apply_configuration_zones.png)
