...
description: Saiba como configurar o protocolo IPv6 numa instância Public Cloud
...
:::info
Atualmente, as ofertas Floating IP e Gateway não tomam a cargo o IPv6. O IPv6 só pode ser utilizado com as instâncias em [modo público](/guides/public-cloud/network-services/concepts#publicmode).
...
No seu projeto Public Cloud, clique em <code className="action">Instâncias</code> no menu à esquerda. Depois clique no <code className="action">...</code> ao lado da instância correspondente e clique em <code className="action">Detalhes da instância</code>.
...
<Tabs>
  <Tab label="Debian (exceto Debian 12)">
  ...
  <Tab label="Windows">
    ...
    Aceda à secção <code className="action">Ligações de rede</code> do seu Windows.
...
Em seguida, clique com o botão direito do rato no seu adaptador de rede para aceder a <code className="action">Propriedades</code>.
...
A seguir, clique em <code className="action">Internet Protocol Version 6 (TCP/IPv6)</code> e, a seguir, no botão <code className="action">Propriedades</code>.
...
Uma vez terminado, selecione a opção `Validar os parâmetros à saída` e clique no botão <code className="action">OK</code> para validar as suas alterações.
  </Tab>
</Tabs>
...
Primeiro, [passe a sua instância para o modo Rescue](/guides/public-cloud/compute/put-an-instance-in-rescue-mode).
