...
description: Understand how you can easily benchmark your APIs and find their limits
...
For this tutorial, we will load test a spam classifier API from the [AI Deploy app portfolio](/guides/public-cloud/ai-machine-learning/ai-deploy-apps-portfolio).
This API takes sentences (emails) as input text, and outputs a spam probability score. 

You can deploy this API easily from the <ManagerLink to="/">OVHcloud control panel</ManagerLink> or [OVHcloud CLI](/guides/public-cloud/ai-machine-learning/ai-cli-install-client). A good strategy is to deploy with autoscaling, with minimum and maximum replicas. This way we will monitor the growth of used replicas.
...
:::info
To be able to connect to your AI Deploy app, you have to [create a token](/guides/public-cloud/ai-machine-learning/ai-cli-app-token) bearer for your OVHcloud AI user.
...
- We put the token to access our API, generated via the OVHcloud Control Panel or `ovhai` CLI. We specify it in the header of the request. If you want to know more about generation and use of tokens, you can follow [this tutorial](/guides/public-cloud/ai-machine-learning/ai-deploy-tokens).
...
Once your `locustfile.py` is ready, open the Locust web interface on `[http://your_IP:8089](http://your_IP:8089)`. 
...
Go in the OVHcloud Control Panel and get the detail of your deployed application. Click on the <code className="action">Access Dashboards</code> button.
