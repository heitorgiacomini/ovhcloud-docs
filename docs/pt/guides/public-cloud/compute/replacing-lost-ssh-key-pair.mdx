...
description: Saiba como restaurar o acesso ao servidor substituindo um par de chaves SSH por uma nova em caso de perda da sua chave privada
...
Crie um novo par de chaves SSH no seu dispositivo local, seguindo as instruções da primeira parte do [guia de criação de uma chave SSH](/guides/public-cloud/compute/creating-ssh-keys).

### Etapa 2: aceder à sua instância em modo rescue

Siga os passos indicados no [manual do modo rescue](/guides/public-cloud/compute/put-an-instance-in-rescue-mode) para reiniciar a instância em modo rescue, aceder à mesma e montar as suas partições.
...
Reinicie a instância no modo "normal" a partir da <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. Consulte as instruções do [manual sobre o modo rescue](/guides/public-cloud/compute/put-an-instance-in-rescue-mode), se necessário.
