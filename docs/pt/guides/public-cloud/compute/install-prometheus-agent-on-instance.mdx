...
description: Descubra como instalar Prometheus Node Exporter ou Windows Exporter numa instância Public Cloud OVHcloud para recolher métricas
...
- [Criar uma instância a partir da Área de Cliente OVHcloud](/guides/public-cloud/cross-functional/create-a-public-cloud-project).
- [Dispor de um acesso administrador à instância](/guides/public-cloud/compute/getting-started).
...
:::info
Em Windows, utilize o PowerShell com SSH ou um cliente SSH como [PuTTY](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows) se preferir utilizar uma interface de linha de comandos.
...
<Tabs>
  ...
  <Tab label="Para Windows (através de SSH / PowerShell na VM)">
    :::info
...
<Tabs>
  ...
  <Tab label="Para Windows (através de SSH / PowerShell)">
    ```powershell
...
<Tabs>
  <Tab label="Para Linux (Debian / Ubuntu com UFW)">
    ```bash
