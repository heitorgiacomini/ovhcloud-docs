...
description: Saiba como configurar chaves SSH adicionais para contas de utilizadores e adicioná-las à sua instância Public Cloud
...
Este guia fornece as instruções necessárias para realizar as operações mais habituais. Entretanto, recomendamos que você entre em contacto com um [provedor de serviços especializado](https://partner.ovhcloud.com/pt/directory/) ou entre em contacto com a [nossa comunidade](https://community.ovhcloud.com/community/en) se houver algum problema.
:::
...
- [Acesso administrativo à sua instância via SSH](/guides/public-cloud/compute/creating-ssh-keys#login-linux)
...
Se necessário, utilize o nosso [guia sobre as chaves SSH](/guides/public-cloud/compute/creating-ssh-keys) para criar um novo par de chaves SSH.  
Encontrará também informações sobre [a gestão de várias chaves](/guides/public-cloud/compute/creating-ssh-keys#multiplekeys) na sua estação de trabalho local, se a sua instalação o exigir.

### Etapa 2: configurar uma nova conta de utilizador

[Ligue-se à sua instância](/guides/public-cloud/compute/getting-started#connect-instance) e utilize os comandos abaixo para criar uma nova conta de utilizador e uma pasta `.ssh`:
...
Pode obter mais informações sobre as permissões dos utilizadores e tópicos relacionados no [guia da conta de utilizador](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).
...
:::info
Por razões de segurança e de boas práticas, um par de chaves não deve ser utilizado por vários utilizadores. Uma vez que cada utilizador nos sistemas GNU/Linux tem o seu próprio ficheiro `authorized_keys` em `~/.ssh/`, pode utilizar o comando `ssh-copy-id` como indicado acima e adaptar `KeyFileName` e `user` depois de ter [criado o par de chaves](/guides/public-cloud/compute/creating-ssh-keys#create-ssh-key).
:::


#### Adição manual de chaves públicas à instância

[Ligue-se à sua instância](/guides/public-cloud/compute/getting-started#connect-instance) e abra, com o seu editor de texto preferido (`nano` é utilizado neste exemplo), o ficheiro `authorized_keys` na pasta pessoal do novo utilizador:
...
Consulte [manual sobre as chaves SSH](/guides/public-cloud/compute/creating-ssh-keys) i para saber mais sobre a utilização das chaves SSH com as instâncias Public Cloud.

## Quer saber mais?

[Como criar instâncias Public Cloud](/guides/public-cloud/compute/getting-started)
