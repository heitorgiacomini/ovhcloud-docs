...
Encontre mais informações no nosso [guia sobre a criação de uma instância Public Cloud](/guides/public-cloud/compute/getting-started).
:::
...
:::info
`Ed25519` é considerado o mais seguro mas `RSA` é uma alternativa válida. Ambos os métodos são compatíveis com a [Área de Cliente OVHcloud](/guides/public-cloud/compute/getting-started).
:::
...
Se confirmar com o botão <code className="action">Enter</code> sem introduzir um nome, será utilizado o nome de ficheiro padrão (`id_rsa` neste exemplo).
...
Copie esta cadeia de chaves para [adicioná-la a uma nova instância ou importá-la para a sua Área de Cliente](/guides/public-cloud/compute/getting-started).
...
Abra a aplicação `Prompt de comandos` escrevendo "cmd" na barra de pesquisa (ou abra o PowerShell a partir do menu <code className="action">Iniciar</code>).
...
Se confirmar com a tecla <code className="action">Enter</code> sem introduzir um nome, será utilizado o nome de ficheiro padrão (`id_rsa` neste exemplo).
...
Ao trabalhar com base numa linha de comandos **Windows**, clique com o botão direito do rato para **colar** o conteúdo da área de transferência na janela da linha de comandos. Para **copiar** um canal a partir da janela da linha de comandos, realce-o e prima <code className="action">Enter</code>. Pode igualmente consultar estas funções através de um clique com o botão direito do rato na barra de menu da janela da linha de comandos.
:::
...
Consulte o nosso [guia específico](/guides/public-cloud/compute/configuring-additional-ssh-keys) para uma explicação pormenorizada destas etapas.

## Quer saber mais?

[Como criar e interligar-se a uma instância Public Cloud](/guides/public-cloud/compute/getting-started)

[Como começar com as ligações SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)
