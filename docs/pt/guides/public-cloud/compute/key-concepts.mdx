...
description: "Descubra os fundamentos do Public Cloud Compute: funcionamento das instâncias, famílias e tamanhos disponíveis, implantações multi-AZ, gestão de imagens, segurança SSH, mecanismos de backup, rede pública/privada e vantagens dos Savings Plans."
...
As instâncias Public Cloud da OVHcloud são implantadas em [vários centros de dados espalhados pelo mundo](https://www.ovhcloud.com/pt/about-us/global-infrastructure/regions/), assegurando uma alta disponibilidade e proximidade com os seus utilizadores. Exemplos de regiões:
...
:::info
**Boa prática**: Para cargas de trabalho críticas, prefira uma [implantação multi-AZ](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture) para assegurar a resiliência e a continuidade do serviço.
:::
...
Para instruções detalhadas sobre a criação e utilização de chaves SSH, consulte o guia [OVHcloud sobre SSH](/guides/public-cloud/compute/creating-ssh-keys).
...
:::info
**Boa prática**: Os backups não substituem uma [arquitetura resiliente](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture). Para ambientes críticos, combine os backups com a replicação multi-AZ para assegurar uma proteção máxima dos dados e uma disponibilidade otimizada do serviço.
:::
...
Para mais detalhes sobre a configuração das redes Public Cloud, consulte o guia oficial [OVHcloud sobre redes](/guides/public-cloud/network-services/concepts).
...
Os Savings Plans ajudam-no a otimizar o seu orçamento enquanto mantém o desempenho e a fiabilidade do seu ambiente cloud. Para mais informações, consulte o guia oficial [OVHcloud sobre Savings Plans](/guides/public-cloud/cross-functional/savings-plans).
...
- [Como criar uma instância Public Cloud e aceder a ela](/guides/public-cloud/compute/getting-started)
- [Gerir as suas instâncias Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance)
- [Iniciar uma instância num volume inicializável](/guides/public-cloud/compute/start-instance-on-attached-volume)
- [Colocar uma instância em standby ou suspender](/guides/public-cloud/compute/suspend-or-pause-an-instance)
