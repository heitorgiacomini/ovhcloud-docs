...
description: Saiba como criar uma instância GPU em Linux ou Windows
...
- [Uma chave SSH](/guides/public-cloud/compute/creating-ssh-keys) criada para criar uma instância GPU Linux.
...
:::info
Se não se sentir à vontade para compilar manualmente um módulo do kernel, recomendamos que utilize uma distribuição oficialmente suportada pela Nvidia e para a qual a Nvidia forneça drivers *prontos a usar*: [https://developer.nvidia.com/cuda-downloads](https://developer.nvidia.com/cuda-downloads).

:::


Clique em <code className="action">Instâncias</code> no menu à esquerda no separador **Compute**. De seguida, clique em <code className="action">Criar instância</code> e escolha uma instância GPU compatível:
...
A seguir, siga os passos restantes, conforme descrito em [este guia](/guides/public-cloud/compute/getting-started#create-instance). Este processo pode levar alguns minutos.
...
Clique em <code className="action">Instâncias</code> no menu à esquerda no separador **Compute**. De seguida, clique em <code className="action">Criar uma instância</code> e escolha uma instância GPU compatível:
...
Depois de a instância ser criada, é preciso finalizar a instalação Windows (_sysprep_). Para isso, clique em <code className="action">...</code> e em <code className="action">Dados da instância</code>. Passe para o separador <code className="action">Consola VNC</code>. A consola já deverá exibir a interface pós-instalação.
...
No primeiro passo, configure a sua localização selecionando uma região, uma língua e um tipo de teclado. Para prosseguir, clique em <code className="action">Avançar</code>.
...
Introduza o endereço IPv4 da instância e indique «Administrator» como utilizador. Então, digite a frase de acesso. É possível que receba uma mensagem de aviso a pedir-lhe para confirmar a conexão devido a uma certificado desconhecido. Clique em <code className="action">Sim</code> para se conectar à instância.
