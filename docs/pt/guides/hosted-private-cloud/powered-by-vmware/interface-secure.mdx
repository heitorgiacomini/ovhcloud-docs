...
description: Utilização da interface segura em infraestruturas HDS ou PCI-DSS
...
Tenha em conta que também é possível atribuir esta autorização a outros utilizadores através da Área de Cliente OVHcloud. Se necessário, consulte a nossa [Apresentação da Área de Cliente Private Cloud OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud)
...
<img className="thumbnail" alt="Primeira SMS" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/SMS1.png" loading="lazy" />
...
- uma ligação que permite validar a operação (atenção: se o seu telefone não estiver ligado a uma rede cujo [o endereço IP](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud#seguranca) não foi autorizado, a página não aparecerá).

Para validar a operação, ligue-se através da ligação apresentada na mensagem. A seguir, consulte a secção <code className="action">Operation Validation</code>.

<img className="thumbnail" alt="Validação da operação" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/operationValidation.png" loading="lazy" />
...
Carregue a operação introduzindo o seu ID no campo “Operation id” e clicando no botão <code className="action">Load operation</code>. A seguir, introduza o token que acabou de receber por SMS e clique em <code className="action">Confirm operation</code>.

<img className="thumbnail" alt="Token da operação" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/operationIdAndToken.png" loading="lazy" />

Será enviada uma SMS de confirmação da operação aos utilizadores com a autorização **token validator**. Exemplo:

<img className="thumbnail" alt="Segundo SMS" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/SMS2.png" loading="lazy" />
...
Para alterar a palavra-passe de utilizador, aceda à interface segura (`https://pcc-xxx-xxx-xxx-xxx.ovh.com/secure/`) e clique no botão <code className="action">Change Password</code>.

<img className="thumbnail" alt="Alterar palavra-passe" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/changePassword.png" loading="lazy" />
...
<img className="thumbnail" alt="Definir a palavra-passe" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/defineNewPassword.png" loading="lazy" />
...
Para repor a palavra-passe de utilizador, aceda à interface segura (`https://pcc-xxx-xxx-xxx-xxx.ovh.com/secure/`) e clique no botão <code className="action">Password lost</code>.

<img className="thumbnail" alt="Palavra-passe perdida" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/passwordLost.png" loading="lazy" />

Uma mensagem indicará que, para continuar, deve poder receber SMS. Se tal for o caso, indique as informações solicitadas (incluindo o utilizador que requer uma reinicialização) e clique em <code className="action">Next step</code>.

<img className="thumbnail" alt="Informações utilizador" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/infoUser.png" loading="lazy" />
...
<img className="thumbnail" alt="Token e palavra-passe" src="/images/hosted-private-cloud/powered-by-vmware/interface-secure/tokenAndPassword.png" loading="lazy" />
