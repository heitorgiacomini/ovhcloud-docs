...
description: Compreender o funcionamento das snapshots horárias da OVHcloud
...
Os ficheiros estão presentes agora basta adicionar esta máquina ao seu **inventário** clicando no ficheiro `.vmx` e, a seguir, no <code className="action">register VM</code> acima.
...
Recomendamos a utilização de uma solução completa de backup, como o nosso serviço [Veeam Backup](/guides/hosted-private-cloud/powered-by-vmware/veeam-backup-as-a-service) ou qualquer outro sistema que realize um backup completo das suas máquinas virtuais.
