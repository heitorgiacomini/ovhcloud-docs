...
description: Saiba como transferir uma manutenção programada para o seu serviço VMware on OVHcloud
...
- Ter acesso à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> ou à [interface de administração dos seus serviços através da API](https://eu.api.ovh.com/).

## Instruções

:::tip
Os e-mails enviados pela OVHcloud também estão acessíveis a partir do seu <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.<br/>
Clique no seu nome no canto superior direito do ecrã e, a seguir, em <code className="action">E-mails de serviço</code> no menu à direita.
...
Aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> com uma conta de administrador.

No menu <code className="action">Hosted Private Cloud</code>, clique no separador <code className="action">Operações</code>. Selecione <code className="action">Por realizar</code> no menu pendente que permite filtrar as operações.

A seguir, clique no botão <code className="action">...</code> e em <code className="action">Modificar a data de tratamento</code>.

<img className="thumbnail" alt="alteração horária" src="/images/hosted-private-cloud/powered-by-vmware/maintenance-rescheduling/maintenance-date-edition01.png" loading="lazy" />

:::info
Se o botão <code className="action">Modificar a data de tratamento</code> estiver cinzento, isto significa que esta manutenção não pode ser adiada.
...
Finalmente, clique no botão <code className="action">Alterar</code> para validar as suas modificações.

<img className="thumbnail" alt="alteração horária" src="/images/hosted-private-cloud/powered-by-vmware/maintenance-rescheduling/maintenance-date-edition02.png" loading="lazy" />

### A partir da API OVHcloud

Ligue-se à [interface de administração dos seus serviços através da API](https://eu.api.ovh.com/). Pode consultar o nosso manual "[Primeiros passos com as API OVHcloud](/guides/manage-and-operate/api/first-steps)".
