...
description: This guide provides instructions for deploying the SLES 15 for SAP image on an OVHcloud dedicated server and preparing it for SAP HANA
lastUpdated: 2025-04-29
routes: &#123;"canonical":"/pages/hosted_private_cloud/sap_on_ovhcloud/cookbook_install_sles_sap_hana_dedicated_server"&#125;
---
...
From the OVHcloud Control Panel, you can launch the deployment of the SLES 15 for SAP image provided by OVHcloud. Click the <code className="action">Install</code> button to launch it.
...
&ensp;&thinsp;a. In the drop-down menu <code className="action">Type of OS</code>, select <code className="action">ERP</code>.<br/>
&ensp;&thinsp;b. In the drop-down menu <code className="action">LINUX</code>, select <code className="action">SUSE Linux Enterprise Server 15 SP3 for SAP Applications - sles-sap15sp3</code>.

We advise you to do this installation on the target disk array <code className="action">2 X Disk SSD 480 GB, JBOD</code>.
...
You can set a hostname which will be displayed with the command `hostname`. You have also the possibility to [add your SSH key](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#adding-an-ssh-key-optional).
...
Once these last parameters are configured, click <code className="action">Confirm</code> to launch the installation.
...
Once the SLES 15 for SAP installation is done, you can [connect to your dedicated server](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#logging-on-to-your-server).
