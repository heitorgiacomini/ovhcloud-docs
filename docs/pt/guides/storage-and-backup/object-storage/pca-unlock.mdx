...
description: Saiba como descongelar os seus arquivos
...
- Degel através de python-swiftclient:
    - [Preparar o ambiente para utilizar a API OpenStack](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) instalando python-swiftclient.
...
Clique na secção <code className="action">Cloud Archive</code> no menu à esquerda.

Para descongelar um arquivo, clique no botão <code className="action">...</code> à direita desta e, a seguir, em <code className="action">Descongelar</code> a fim de dar início ao processo de recuperação.

<img className="thumbnail" alt="desgel" src="/images/storage-and-backup/object-storage/pca-unlock/unfreeze.png" loading="lazy" />

Uma vez o processo iniciado, a data e a hora de disponibilidade do seu arquivo são apresentadas na coluna `Disponibilidade`.

<img className="thumbnail" alt="prazo antes do descongelamento" src="/images/storage-and-backup/object-storage/pca-unlock/unfreeze_result.png" loading="lazy" />

O seu ficheiro estará disponível para download após este prazo. Poderá então iniciar o download diretamente através do browser ou através de [um cliente Swift/SFTP/SCP](/guides/storage-and-backup/object-storage/pca-sftp).
