...
description: Saiba como utilizar o script ovh-ba-install.sh fornecido pela OVHcloud para instalar e gerir o agente de cópia de segurança Veeam num servidor Linux
...
- **Link direto:** <ManagerLink to="/#/bmc-backup-agent-baremetal/service">Backup Agent</ManagerLink>
...
| Elemento | Detalhe |
|--------|--------|
| **Sistema** | Linux **compatível** com o agente Veeam para Linux (consulte os [requisitos de sistema Veeam](https://helpcenter.veeam.com/docs/agentforlinux/userguide/system_requirements.html?ver=13) e as [restrições Backup Agent](/guides/storage-and-backup/backup-agent/restrictions)). |
...
Na área de cliente OVHcloud, abra o seu <ManagerLink to="/#/bmc-backup-agent-baremetal/service">Backup Agent</ManagerLink>, aceda ao separador <code className="action">Agents</code> e clique no botão <code className="action">Descarregar</code>. Na janela que se abre, selecione **Linux** para apresentar o comando que contém os dois URL (script e pacote Linux).

<img className="thumbnail" alt="Descarregar o agente — comandos de instalação Linux" src="/images/storage-and-backup/backup-agent/backup-agent-linux-cli-assistant/01-backup-agent-download-linux-fr.png" loading="lazy" />
...
Para aprofundar os seus diagnósticos, consulte o nosso [guia de diagnóstico e resolução de problemas Backup Agent](/guides/storage-and-backup/backup-agent/troubleshooting).
...
- [Configurar a sua primeira cópia de segurança](/guides/storage-and-backup/backup-agent/first-configuration)
