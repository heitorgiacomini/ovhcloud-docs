...
description: Saiba como conectar-se ao NAS-HA através de NFS
...
- [Ter criado uma partição no seu serviço com o protocolo NFS ativado](/guides/storage-and-backup/file-storage/ha-nas/nas-get-started#partition)
- [Ter uma entrada ACL para o endereço IP do servidor](/guides/storage-and-backup/file-storage/ha-nas/nas-get-started#addaccess)
...
Também precisará do **nome interno** e **do endereço IP** do serviço NAS-HA que poderá encontrar no e-mail recebido após a instalação ou na <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.
...
Na interface de administração Proxmox, clique em <code className="action">Storage</code> no menu vertical.

<img className="thumbnail" alt="proxmox" src="/images/storage-and-backup/file-storage/ha-nas/nas-nfs/proxmox1.png" loading="lazy" />

Clique no botão <code className="action">Add</code> e selecione <code className="action">NFS</code>.
...
Clique em <code className="action">Add</code> para montar a sua partição.

### VMware ESXI

Na interface de administração VMware ESXI, clique em <code className="action">Storage</code> no menu à esquerda.

A seguir, clique no botão <code className="action">New datastore</code> para abrir o assistente.

<img className="thumbnail" alt="ESXI" src="/images/storage-and-backup/file-storage/ha-nas/nas-nfs/esxi1.png" loading="lazy" />

Na nova janela, selecione <code className="action">Mount NFS datastore</code> e clique em <code className="action">Next</code>.
...
Depois, clique em <code className="action">Next</code>. Clique em <code className="action">Finish</code> na última etapa.
...
- Se aloja na sua partilha NFS uma base de dados, tenha em conta que, em caso de desconexões de rede, o mecanismo de bloqueios específico do protocolo NFS v4.x pode provocar a interrupção da sua aplicação (consulte este rfc para mais pormenores: [https://datatracker.ietf.org/doc/rfc3530/](https://datatracker.ietf.org/doc/rfc3530/)).
- Se aloja máquinas virtuais VMware na sua partilha NFS, tenha em conta que o mecanismo de bloqueios integrado na versão NFSv4.x não é compatível com o modo de clustering implementado no seu NAS-HA (cluster em modo ativo/passivo explicado em [esta página](https://www.ovhcloud.com/pt/storage-solutions/nas-ha/)). É imperativo que utilize o protocolo NFSv3 para que não perca o acesso ao seu datastore durante um incidente que afete o servidor principal ou durante uma operação de manutenção programada.
