...
description: Iniciar a sua oferta Enterprise File Storage com recurso às API OVHcloud
...
- Ter acesso à página das [API OVHcloud](https://eu.api.ovh.com/)
- Saber [montar um NAS através de NFS](/guides/storage-and-backup/file-storage/ha-nas/nas-nfs)
...
Todas as API utilizadas para este guia estão disponíveis na secção */armazenamento*: [https://api.ovh.com/console/#/storage](https://api.ovh.com/console/#/storage).
...
| `NetAppShare.description` |  | Descrição do volume |
| `NetAppShare.name` |  | Nome do volume |
| `NetAppShare.protocol` | Yes | Protocolo utilizado pelo volume |
| `NetAppShare.size` |  | Tamanho em gigabytes do volume |
...
| `NetAppShareACLRule.accessLevel` | Yes | Nível de acesso ACL. Pode ser **rw** (leitura e escrita) ou **ro** (leitura apenas). |
| `NetAppShareACLRule.accessTo` | Yes | Endereço IP ou intervalo de endereços IP em notação CIDR. |
