...
description: Saiba como utilizar um módulo Terraform para implementar a stack de infraestrutura do NetApp Trident CSI com o Enterprise File Storage e o Managed Kubernetes Service.
...
<img className="thumbnail" alt="Arquitetura do módulo Terraform" src="/images/storage-and-backup/file-storage/enterprise-file-storage/netapp-trident-terraform/terraform-module-architecture.png" loading="lazy" />
...
| Gateway obrigatória | O cluster MKS necessita de uma gateway para alcançar a API OVHcloud |
...
Siga o guia [Primeiros passos com as APIs OVHcloud](/guides/manage-and-operate/api/first-steps/) para gerar o seu token de API.
...
:::info
Sobre a variável `ovh.endpoint`: por predefinição, `ovh-eu` está definido, dado que efetuamos chamadas à API OVHcloud Europa.
...
Depois de implementada a infraestrutura com este módulo, configure o Trident CSI para utilizar o Enterprise File Storage. Siga o guia [Primeiros passos com o Trident CSI](/guides/storage-and-backup/file-storage/enterprise-file-storage/netapp-trident-csi/) para obter instruções detalhadas.
...
- [Primeiros passos com o Trident CSI](/guides/storage-and-backup/file-storage/enterprise-file-storage/netapp-trident-csi/)
- [Gerir o Enterprise File Storage com o provider Terraform OVHcloud](/guides/storage-and-backup/file-storage/enterprise-file-storage/netapp-terraform/)
...
Se precisar de formação ou de assistência técnica para implementar as nossas soluções, contacte o seu representante comercial ou aceda à página [Serviços Profissionais](/links/professional-services) para obter um orçamento e solicitar uma análise personalizada do seu projeto aos nossos especialistas da equipa de Serviços Profissionais.
