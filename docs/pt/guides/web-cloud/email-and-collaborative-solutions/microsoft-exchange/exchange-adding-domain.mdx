...
description: Saiba como adicionar um domínio à sua plataforma Exchange ou E-mail Pro
...
- Ter a possibilidade de alterar a configuração do domínio [zona DNS](/guides/web-cloud/domains/dns-zone-edit).
...
1. Clique no separador <code className="action">Domínios associados</code> da sua plataforma Exchange ou E-mail Pro.
1. Aparecerá uma tabela com os domínios atualmente associados ao serviço.
1. Clique no botão <code className="action">Adicionar um domínio</code>.
...
Depois de escolher, clique no botão <code className="action">Seguinte</code>.
...
No final do processo, verifique as informações apresentadas e clique no botão <code className="action">Confirmar</code> para validar a adição do domínio.
...
- **Se indicou um domínio não gerido pela sua conta OVHcloud**:
    ...
    - No âmbito de uma etiqueta `CNAME` vermelha, queira consultar o nosso guia explicando como [criar um registo CNAME ao adicionar um domínio associado](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname).
...
Para verificar se a configuração de um domínio está correta, aceda à tabela <code className="action">Domínios associados</code> do seu serviço. A etiqueta verde indica que o domínio está configurado corretamente. Caso contrário, é possível que a propagação ainda não esteja concluída.
...
1. Clique no separador <code className="action">Domínios associados</code>.
1. Clique no botão <code className="action">...</code> na linha do domínio em questão.
1. Clique em <code className="action">Configuração</code>.
...
Agora que adicionou os nomes de domínio desejados ao seu serviço, pode configurar as suas contas de e-mail com estes últimos. Esta operação é realizada no separador <code className="action">Contas de e-mail</code>. Se necessário, pode encomendar contas suplementares graças ao botão <code className="action">Action</code>/<code className="action">Encomendar contas</code> ou <code className="action">Adicionar uma conta</code>.
...
Depois de configurar as contas, pode começar a utilizá-las. Para isso, a OVHcloud disponibiliza-lhe o **webmail**, acessível [aqui](https://www.ovhcloud.com/pt/mail/). Para uma utilização ótima do seu endereço num software, certifique-se de que é compatível com o serviço.

Se pretender configurar o seu endereço de e-mail num software de mensagens ou num dispositivo como um smartphone ou um tablet, ou obter ajuda relativamente às funcionalidades do seu serviço de e-mail, consulte a nossa documentação, acessível a partir das páginas [Exchange](https://www.ovhcloud.com/pt/emails-exchange/) e [E-mail Pro](https://www.ovhcloud.com/pt/emails/email-pro/).

Pode adquirir licenças Outlook na <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e licenças Office 365 na página [Microsoft 365](https://www.ovhcloud.com/pt/collaborative-tools/microsoft-365/). Caso pretenda usufruir do software de correio eletrónico Outlook ou de outros programas da suite Office, recomendamos uma destas soluções.
...
Aceda ao separador <code className="action">Domínios associados</code> da plataforma. A partir da tabela, a coluna `Contas` indica-lhe o número de contas associadas aos nomes de domínio da sua lista.
...
1. À direita das contas a modificar, clique no botão <code className="action">...</code>.
1. Clique em <code className="action">Alterar</code>.
...
1. Aceda ao separador <code className="action">Contas de e-mail</code>.
1. À direita da conta a eliminar, clique no botão <code className="action">...</code>.
1. Clique em <code className="action">Reiniciar esta conta</code> ou em <code className="action">Reiniciar</code>.
...
No separador <code className="action">Domínios associados</code> da sua plataforma, clique no botão <code className="action">...</code> à direita do nome de domínio em causa e, a seguir, em <code className="action">Eliminar este domínio</code>.
...
[Criar um registo CNAME para adicionar um domínio associado](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname)
