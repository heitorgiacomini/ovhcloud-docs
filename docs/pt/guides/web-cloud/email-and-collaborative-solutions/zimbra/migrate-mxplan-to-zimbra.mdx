...
<Tabs>
  <Tab label="Etapa 1">
    **Criar um projeto de migração**
    ...
    Aceda a [OVHcloud Mail Migrator](/links/web/omm) e clique em <code className="action">Nova migração</code>.
    ...
    Clique em <code className="action">Criar o meu projeto</code>. Receberá um e-mail de confirmação com o identificador único do projeto.
  </Tab>
  <Tab label="Etapa 2">
    **Ligar-se ao projeto e criar a migração**

    A partir da página inicial do [OMM](/links/web/omm), clique em <code className="action">Acompanhar uma migração</code>, introduza o `Identificador do projeto` e a `Palavra-passe do projeto` e clique em <code className="action">Ligar-se ao projeto</code>.

    A seguir, clique em <code className="action">Nova migração</code> para configurar a sua migração:
    ...
    - **Conta de origem**:
        - **Tipo de conta**: Selecione `OVHcloud` e, em seguida, escolha `MX Plan` ou `Deteção automática`. Clique em <code className="action">Ligar-se</code> para se identificar com a sua conta OVHcloud e selecionar automaticamente o serviço e o endereço a migrar (exemplo: `john.smith@mydomain.ovh`). Introduza depois a palavra-passe desta conta de e-mail.
    - **Conta de destino**:
        - **Tipo de conta**: Selecione `OVHcloud` e, em seguida, escolha `Zimbra`. Clique em <code className="action">Ligar-se</code> para se identificar com a sua conta OVHcloud e selecionar o serviço Zimbra e o endereço de destino (exemplo: `zimbra2@mydomain.ovh`). Introduza depois a palavra-passe desta conta de e-mail.
    ...
    Clique em <code className="action">Migrar a minha conta</code> para iniciar a migração.
  ...
  <Tab label="Etapa 3">
    **Acompanhar a migração**
    ...
    - <code className="action">Ver mais detalhes</code>: Acompanhe o progresso da migração e consulte o relatório quando estiver concluída.
    - <code className="action">Cancelar a migração</code>: Cancela a migração em curso. Os elementos já migrados são conservados na conta de destino.
    - <code className="action">Eliminar os meus dados de migração (RGPD)</code>: Desencadeia a eliminação de todos os dados relativos à migração. As informações sobre os eventos de migração são conservadas.
