...
- Ter acesso às informações de acesso ao endereço de e-mail MX Plan que pretende consultar. Para mais informações, consulte o nosso guia [Primeiros passos com a oferta MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities).
...
Aceda à página [Webmail](https://www.ovhcloud.com/pt/mail/). Introduza um endereço de e-mail e a password, e depois clique em <code className="action">Ligação</code>. 
...
Para gerir com mais precisão as pastas, clique na roda dentada na parte inferior da coluna e, a seguir, em <code className="action">Gerir pastas.</code>
...
Para eliminar uma pasta, selecione a pasta em causa, clique na roda dentada no fundo da coluna `Pastas` e, a seguir, em <code className="action">Eliminar</code>. Para apagar o conteúdo mas guardar a pasta, clique em <code className="action">Esvaziar</code>.
...
- <code className="action">Responder</code>: responder diretamente ao expedidor.
...
- <code className="action">Reencaminhar</code>: transferir o e-mail selecionado para um ou vários destinatários.
- <code className="action">Eliminar</code>: colocar o e-mail selecionado no "Corbeille".
- <code className="action">Spam</code>: colocar o e-mail selecionado diretamente na caixa de correio indesejável (Junk), qualificá-lo como **spam**.
- <code className="action">Marcar</code>: determinar manualmente o estado de um e-mail.
- <code className="action">Mais</code> 
    - <code className="action">Imprimir</code>.
    - <code className="action">Guardar como (.eml)</code>: recuperar o cabeçalho do e-mail e o seu conteúdo.
    - <code className="action">Editar como novo</code>: criar um novo e-mail com base no e-mail selecionado.
    - <code className="action">Mostrar código fonte</code>: mostrar o e-mail na sua forma bruta com o cabeçalho.
    - <code className="action">Migrar para</code>: mover o e-mail para uma pasta.
...
Os capítulos seguintes deste guia correspondem aos separadores que compõem a parte <code className="action">Preferências</code> dos <code className="action">Definições</code> de Roundcube. A sua descrição não é exaustiva.
...
> Na prática, eu crio uma sub-pasta « Rascunhos de e-mails de clientes ». Vou a <code className="action">As minhas preferências</code> / <code className="action">Pastas especiais</code> e escolho a opção "Rascunhos". No menu suspenso, seleciono a pasta "Rascunhos de e-mails de clientes" para substituir "Drafts". Os e-mails redigidos nesta pasta serão considerados rascunhos.
...
Para mais informações, consulte a FAQ da "Maildeve" em [https://mailvelope.com/faq](https://mailvelope.com/faq).

### Gerir as identidades e a sua assinatura <a name="identity-signature"></a>

A partir do Roundcube, clique em <code className="action">Definições</code> na barra superior e, a seguir, em <code className="action">Identidades</code> na coluna da esquerda. A "identidade" permite personalizar as informações enviadas aos destinatários como, por exemplo, o nome do ecrã ou a assinatura.
...
:::warning
Assim, se a assinatura estiver em formato HTML, será necessário passar para HTML para a redação de um e-mail. Pode ativar esta opção por predefinição para cada redação de e-mail, a partir da secção <code className="action">Definições</code> da interface Roundcube.
Clique em <code className="action">Preferências</code> na coluna da esquerda e, a seguir, em <code className="action">Composição de mensagens</code>. Para a menção **Redigir e-mails HTML**, selecione `Sempre`.
...
Clique no botão <code className="action">&lt; ></code> na barra de ferramentas HTML e insira o seguinte código, substituindo `your-image-url` pelo endereço (URL) da imagem e `text-if-image-is-not-displayed` por um texto que substitui a imagem se esta não puder ser apresentada.
...
Clique em <code className="action">Contactos</code>, na barra superior, para aceder ao diretório de contactos. Esta é dividida em **3 colunas**:
...
Para criar um grupo, clique no botão <code className="action">+</code> no fundo da coluna `Grupos`. Defina o nome do grupo e clique em <code className="action">Guardar</code> para validar.
...
Para afetar um contacto a um dos grupos, selecione um contacto na coluna `Contactos` e, na janela que aparecer, clique no separador <code className="action">Grupos</code>. Selecione o grupo que deseja atribuir ao contacto.
...
:::info
Pode adicionar campos adicionais através do menu pendente <code className="action">Adicionar campo...</code>, situado sob os campos `Primeiro nome` e `Endereço`.
...
Na janela <code className="action">Contatos</code>, na barra superior, clique em <code className="action">importar</code> para abrir a janela de importação.
...
Na janela <code className="action">Contatos</code>, no canto superior direito, clique no botão <code className="action">Exportar</code>, no botão virado para baixo.

Pode escolher entre:

- <code className="action">Exportar todos</code> e o conjunto dos contactos será então exportado num ficheiro **.vcf**.
- <code className="action">Exportar a selecionados</code> para exportar unicamente os elementos que escolheu na coluna <code className="action">Contatos</code>.
...
No Roundcube, clique em <code className="action">Definiçõe</code> na barra superior e, a seguir, em <code className="action">Respostas</code> na coluna da esquerda.
...
Deseja adicionar uma resposta automática ao seu endereço de e-mail quando estiver ausente ou indisponível. Esta função não pode ser ativada a partir do webmail, mas sim a partir da <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>, na interface de gestão dos seus endereços de e-mail. Consulte o guia "[Criar uma resposta automática para o endereço de e-mail](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/feature-auto-responses)".

### Alterar a palavra-passe do endereço de e-mail <a name="password"></a>

Para alterar a palavra-passe do seu endereço de e-mail, deve ligar-se à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>, na interface de gestão dos seus endereços de e-mail. Consulte o guia "[Alterar a palavra-passe de um endereço de e-mail](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-change-password)".

### Redação de um e-mail <a name="email-writing"></a>

No separador <code className="action">E-mail</code> na barra superior, clique em <code className="action">Ediger</code>.
...
- <code className="action"> Corretor ortográfico</code>, para verificar o texto, com um menu que permite a escolha da língua.
...
- <code className="action">Respostas</code>: adiciona um template pré-registado na parte [Respostas](#responses).
...
[Criar uma resposta automática para o endereço de e-mail](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/feature-auto-responses)
