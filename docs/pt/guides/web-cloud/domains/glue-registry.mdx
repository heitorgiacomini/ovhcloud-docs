...
description: Saiba como personalizar os servidores DNS do seu domínio OVHcloud
...
Siga cuidadosamente os passos descritos abaixo ou contacte um [fornecedor especializado](https://partner.ovhcloud.com/pt/directory/) em caso de dúvida.
...
Para isso, aceda à ligação [https://zonemaster.net](https://zonemaster.net/pt), introduza o seu domínio sem as *www* (exemplo: *domain.tld*) e assinale o botão <code className="action">Options</code> situado logo abaixo do formulário de introdução do domínio.

Nas opções disponíveis, clique diretamente no botão <code className="action">Obter os NS a partir da zona-mãe</code>.
...
Se necessário, consulte o nosso tutorial sobre a ferramenta [Zonemaster](/guides/web-cloud/domains/dns-zonemaster) para mais informações.
...
<Tabs>
  ...
  <Tab label="Passo 2">
    Depois de aceder ao domínio correspondente, clique no separador <code className="action">Hosts</code>.
    
    Na tabela apresentada, se existirem, encontrará os registos host atualmente configurados na OVHcloud para o seu domínio. Para adicionar um novo registo, clique no botão <code className="action">Adicionar</code>.
...
Na imagem acima, retomando o exemplo do [passo 2](#step2), o host que se pretende adicionar aqui (a partir do domínio *domain.tld*) é **dns1.domain.tld**.
    ...
    Depois de preencher as informações, clique no botão <code className="action">Adicionar</code>. Tome conhecimento das informações apresentadas e clique em <code className="action">Validar</code>. Repita esta operação tantas vezes quantas necessário, consoante o número de servidores DNS utilizados pelo seu domínio.
...
<summary>O seu domínio não utiliza uma zona DNS ativa na OVHcloud</summary>
...
<summary>O seu domínio utiliza uma zona DNS ativa na OVHcloud</summary>
...
<Tabs>
  <Tab label="Passo 1">
    Aceda à página <ManagerLink to="/#/web/zone">Zonas DNS</ManagerLink> e escolha o domínio que utilizou para criar os hosts na [parte 3](#step3).
  ...
  <Tab label="Passo 2">
    Clique em <code className="action">Adicionar um registo</code>.
  </Tab>
  <Tab label="Passo 3">
  ...
  <Tab label="Passo 4">
    Introduza o *subdomínio* e o endereço *IPv4* (A) ou *IPv6* (AAAA) e prossiga até à validação da adição. Se necessário, consulte as instruções descritas na nossa documentação "[Editar uma zona DNS da OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
  </Tab>
</Tabs>
...
<Tabs>
  <Tab label="Passo 1">
    Aceda à página <ManagerLink to="/#/web/zone">Zonas DNS</ManagerLink> e escolha o domínio para o qual personalizou os servidores DNS.
    
    <img className="thumbnail" alt="Zonas DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/dns-zones.png" loading="lazy" />
  </Tab>
  <Tab label="Passo 2">
    Clique em <code className="action">Modificar em modo de texto</code>.
  ...
  <Tab label="Passo 3">
    Nesta janela, substitua **unicamente nos registos de tipo *NS*** os nomes dos servidores DNS pelos seus próprios nomes de servidores DNS personalizados **sem esquecer** de incrementar em "1" o primeiro valor numérico da linha *SOA*. Após efetuar as modificações, clique em <code className="action">Seguinte</code> e em <code className="action">Validar</code>.
...
Se necessário, consulte as instruções descritas na nossa documentação "[Editar uma zona DNS da OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
...
<Tabs>
  <Tab label="Passo 1">
    Aceda à página <ManagerLink to="/#/web-domains/domain">Nomes de domínio</ManagerLink> e escolha *o domínio para o qual pretende personalizar os servidores DNS*.
    
    <img className="thumbnail" alt="Nomes de domínio" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-names.png" loading="lazy" />
  </Tab>
  <Tab label="Passo 2">
    Selecione o separador <code className="action">Servidores DNS</code> e clique em <code className="action">Modificar os servidores DNS</code>.
  ...
  <Tab label="Passo 4">
    Finalize os passos e, se necessário, consulte as instruções descritas na nossa documentação "[Modificar os servidores DNS de um domínio OVHcloud](/guides/web-cloud/domains/dns-server-edit)".
...
[Informações gerais sobre os servidores DNS da OVHcloud](/guides/web-cloud/domains/dns-server-general-information)
...
Se pretender usufruir de ajuda na utilização e na configuração das suas soluções OVHcloud, consulte as nossas diferentes [ofertas de suporte](https://www.ovhcloud.com/pt/support-levels/).
