...
Em caso de dúvida, contacte um [fornecedor especializado](https://partner.ovhcloud.com/pt/directory/).
...
<Tabs>
  ...
  <Tab label="Passo 3">
    Na janela que aparecer, selecione o campo de apontamento do tipo <code className="action">CNAME</code>.
  </Tab>
  <Tab label="Passo 4">
    Introduza no campo `Subdomínio` o subdomínio correspondente (por exemplo: `www` para o subdomínio `www.domain.tld`) e, no campo `Alvo *`, o nome de domínio ou subdomínio (por exemplo: `domain.tld`) que pretende visar através do registo do tipo CNAME. Clique em <code className="action">Seguinte</code>.
  </Tab>
  <Tab label="Passo 5">
