...
description: Saiba como importar e instalar um certificado SSL personalizado no seu alojamento Web OVHcloud
...
Este guia fornece as instruções necessárias para realizar as operações mais habituais. Se encontrar dificuldades, recomendamos que recorra a um [fornecedor especializado](https://partner.ovhcloud.com/pt/directory/). De facto, não poderemos fornecer-lhe assistência no **instalação ou subscrição de um certificado SSL que não seja o [certificado SSL proposto pela OVHcloud](https://www.ovhcloud.com/pt/web-hosting/options/ssl/)**. Mais informações na secção "[Quer saber mais?](#go-further)" deste guia.
...
Uma vez iniciada a encomenda, o terminal solicitará cada uma das seguintes informações (para si, para a sua empresa ou para a sua associação). Depois de responder à pergunta, prima a tecla <code className="action">ENTRAE</code> no teclado para apresentar a seguinte pergunta:
...
- `Organization Name (eg, company) [Internet Widgits Pty Ltd]`: introduza o nome da sua organização, empresa ou associação. **Se for um utilizador doméstico, não responda a esta pergunta e prima diretamente a tecla <code className="action">ENTRAE</code> no teclado para apresentar a seguinte pergunta**.
- `Organizational Unit Name (eg, section) []` : introduza o nome do seu departamento ou do seu serviço na sua organização, empresa ou associação. **Se for um utilizador doméstico, não responda a esta pergunta e prima diretamente a tecla <code className="action">ENTRAE</code> no teclado para apresentar a seguinte pergunta**.
...
As perguntas seguintes são opcionais e destinam-se principalmente a utilizadores experientes. Em caso de dúvida, recomendamos-lhe vivamente que as passe pressionando a tecla <code className="action">ENTRAE</code> no teclado até que o terminal deixe de fazer perguntas.
...
Abra um programa de processamento de texto (bloco de notas, LibreOffice, etc.) e, em seguida, <code className="action">copie/cole</code> a chave privada completa, incluindo os termos `-----BEGIN PRIVATE KEY-----` e `-----END PRIVATE KEY-----`.
...
Abra um programa de processamento de texto e, em seguida, <code className="action">copie/cole</code> a totalidade da CSR, incluindo os termos `-----BEGIN CERTIFICATE REQUEST-----` e `-----END CERTIFICATE REQUEST-----`.
...
<Tabs>
  ...
  <Tab label="Etapa 3">
    Quando o conteúdo do separador aparecer, clique no botão <code className="action">Importação do seu próprio certificado SSL</code>.
  ...
  <Tab label="Etapa 4">
    ...
    - <code className="action">Copiar conteúdo do seu certificado (apenas RSA)</code>: introduza o conteúdo do ficheiro **certificate.crt** emitido pelo seu fornecedor SSL, incluindo os termos `-----BEGIN CERTIFICATE-----` e `-----END CERTIFICATE-----` (ou seus equivalentes). A encriptação RSA é a encriptação standard para os certificados SSL.
    - <code className="action">Copiar o conteúdo da sua chave privada (não encriptada)</code>: introduza o conteúdo do ficheiro **private.key** fornecido pelo seu fornecedor SSL, incluindo os termos `-----BEGIN RSA PRIVATE KEY-----` e `-----END RSA PRIVATE KEY-----` (ou os seus equivalentes). Se *não encriptada* significa que a chave privada não necessita de estar protegida com uma palavra-passe ou frase. Caso contrário, a instalação do certificado falhará.
    - <code className="action">Copiar o conteúdo da sua cadeia de certificados</code>: introduza o conteúdo do ficheiro **ca_bundle.crt** emitido pelo seu fornecedor SSL, incluindo os termos `-----BEGIN CERTIFICATE-----` e `-----END CERTIFICATE-----` (ou seus equivalentes).
...
Depois de preencher os 3 formulários, clique em <code className="action">Validar</code> para terminar a importação do certificado SSL personalizado para o seu alojamento web.
  </Tab>
</Tabs>
...
Consulte os nossos manuais "[Partilhar o alojamento entre vários sites](/guides/web-cloud/web-hosting/multisites-configure-multisite)" e "[Editar uma zona DNS da OVHcloud](/guides/web-cloud/domains/dns-zone-edit)" para resolver esta situação.
...
<Tabs>
  ...
  <Tab label="Etapa 3">
    ...
    <img className="thumbnail" alt="Quadro de gestão dos certificados SSL" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/ssl-certificates/tab-custom.png" loading="lazy" />
  </Tab>
</Tabs>


O seu certificado SSL personalizado já está instalado e ativo. Pode desde já utilizá-lo com o seu website passando, por exemplo, pelo seu [website em HTTPS](/guides/web-cloud/web-hosting/ssl-activate-https-website).
