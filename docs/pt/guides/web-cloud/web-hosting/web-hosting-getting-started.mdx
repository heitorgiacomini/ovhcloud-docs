---
title: Como começar bem com o seu alojamento web
description: "Saiba como publicar um novo site através das nossas opções de \"Módulos 1 clique\", como criar um novo endereço de e-mail personalizado com o seu nome de domínio, tudo graças à nossa solução de alojamento web"
lastUpdated: 2026-06-17
availableIn: [eu, ca, apac]
---

import { Region } from '@components/Zone';
import { Tab, Tabs } from '@rspress/core/theme';

## Objetivo

Pretende criar um site para a sua empresa ou um blogue pessoal? Precisa de uma loja online para vender os seus produtos? Este guia dedicado aos primeiros passos com o alojamento web OVHcloud indicar-lhe-á as etapas chave para o configurar. Além disso, também encontrará explicações para criar endereços de e-mail profissionais associados ao seu domínio. Permite-lhe colocar online o seu projeto de forma fácil e rápida, a fim de comunicar com o seu público de forma eficaz.

:::info
- Deseja migrar um site já existente para fornecedor de alojamento? Consulte diretamente o nosso guia dedicado: [Migrar o seu website e os seus serviços associados para a OVHcloud](/guides/web-cloud/web-hosting/hosting-migrating-to-ovh).
<Region zones={['eu']}>
- Deseja apenas uma página inicial simples para a sua atividade profissional? Consulte o nosso guia dedicado: [Alojamento web - Ativar alojamento gratuito](/guides/web-cloud/web-hosting/activate-start10m).
</Region>

:::

:::info
Alguns procedimentos deste guia aplicam-se à <ManagerLink to="/beta/#/">nova versão da Área de Cliente OVHcloud</ManagerLink>.
Existe um lembrete no início de cada secção afetada.

:::

## Requisitos

<Region zones={['eu']}>
- Ter um serviço de [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/) com, pelo menos, uma base de dados disponível (exceto a oferta de alojamento gratuita).
</Region>
<Region zones={['ca','apac']}>
- Ter um serviço de [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/) com, pelo menos, uma base de dados disponível.
</Region>
- Ter recebido o e-mail com a confirmação da instalação do alojamento web.
- Dispor de um [nome de domínio](https://www.ovhcloud.com/pt/domains/) e de uma zona DNS associada na OVHcloud.
- Todos os serviços (Alojamento web, Nome de domínio, zona DNS) devem estar acessíveis a partir de uma única conta OVHcloud.

{/* CP-NAV-START:web-hosting */}
---

### Acesso à Área de Cliente OVHcloud

- **Ligação direta:** <ManagerLink to="/#/web/hosting">Alojamentos</ManagerLink>
- **Caminho de navegação:** <code className="action">Web Cloud</code> > <code className="action">Alojamentos</code> > Selecione o seu alojamento web

---
{/* CP-NAV-END:web-hosting */}

## Instruções

### 1 - Atribuir o seu nome de domínio a um site no seu alojamento web <a name="part-1"></a>

:::tip
Se subscreveu o seu nome de domínio e o seu alojamento web na mesma encomenda, estes dois serviços já estão associados. Passe diretamente para [Parte 2](#part-2) deste manual.

:::

Clique nos separadores abaixo para visualizar cada uma das **4** etapas.

:::info
Esta secção aplica-se à <ManagerLink to="/beta/#/">nova versão da Área de Cliente OVHcloud</ManagerLink>.

:::

<Tabs>
  <Tab label="Etapa 1">
    Aceda à página <ManagerLink to="/beta/#/web-cloud/hosting/sites">Alojamentos</ManagerLink> e escolha o alojamento web em causa.

    <img className="thumbnail" alt="Alojamentos" src="/images/assets/screens/control-panel/product-selection/web-cloud/hosting-sites.png" loading="lazy" />
  </Tab>
  <Tab label="Etapa 2">
    Selecione o separador <code className="action">Meus sites</code> quando posicionado no alojamento web em causa.
  </Tab>
  <Tab label="Etapa 3">
    No quadro que aparece, clique no botão <code className="action">⁝</code> à direita do site criado automaticamente no momento da criação do seu alojamento web, e depois em <code className="action">Adicionar um domínio</code>.
  </Tab>
  <Tab label="Etapa 4">
    Na nova página que se abre, selecione e preencha os elementos solicitados até à sua validação.
  </Tab>
</Tabs>

<details>

<summary>Clique aqui para mais informações.</summary>

Consulte os nossos guias detalhados:

- [Como associar um nome de domínio a um site web existente?](/guides/web-cloud/web-hosting/my-websites-add-domain-existing-website)
- [Partilhar o alojamento entre vários sites](/guides/web-cloud/web-hosting/multisites-configure-multisite).
- [Alojamento web - Alterar um nome de domínio já associado a um alojamento](/guides/web-cloud/web-hosting/multisites-modify-domain).

</details>

### 2 - Instalar um "módulo 1 clique" para o seu website <a name="part-2"></a>

Nos seus alojamentos web, a OVHcloud propõe instalar gratuitamente os CMS WordPress, Joomla!, PrestaShop e Drupal graças à opção "módulo 1 clique".

Clique nos separadores abaixo para visualizar cada uma das **4** etapas.

<Tabs>
  <Tab label="Etapa 1">
    Aceda à página <ManagerLink to="/#/web/hosting">Alojamentos</ManagerLink> e escolha o alojamento web em causa.

    <img className="thumbnail" alt="Web Hosting" src="/images/assets/screens/control-panel/product-selection/web-cloud/hosting-plans.png" loading="lazy" />
  </Tab>
  <Tab label="Etapa 2">
    Selecione o separador <code className="action">Módulos "1 clique"</code> quando posicionado no alojamento web em causa.
  </Tab>
  <Tab label="Etapa 3">
    Na página que se abrir, clique no botão <code className="action">Adicionar um módulo</code>.
  </Tab>
  <Tab label="Etapa 4">
    Na nova janela, selecione o CMS que deseja instalar. De seguida, escolha o domínio no qual deseja instalar o módulo, selecionando o nome de domínio desejado **sem os "www"** à frente (por exemplo: `domain.tld` e não `www.domain.tld`), e clique diretamente em <code className="action">Instalar</code>.
  </Tab>
</Tabs>

<details>

<summary>Clique aqui para mais informações.</summary>

Consulte os nossos guias detalhados:

- [Instalar o seu website com um 'módulo 1 clique' (CMS)](/guides/web-cloud/web-hosting/cms-install-1-click-modules).
- [Como gerir o seu módulo em 1 clique?](/guides/web-cloud/web-hosting/cms-manage-1-click-module).

</details>

### 3 - Ativar os endereços de e-mail incluídos com o seu alojamento web <a name="part-3"></a>

:::tip
Se subscreveu o seu domínio e o seu alojamento web numa única encomenda, os endereços de e-mail incluídos com o alojamento web já estão associados ao seu domínio. Passe diretamente para [Secção 4](#part-4) deste manual.

:::

Clique nos separadores abaixo para visualizar cada uma das **3** etapas.

<Tabs>
  <Tab label="Etapa 1">
    Aceda à página <ManagerLink to="/#/web/hosting">Alojamentos</ManagerLink> e escolha o alojamento web em causa.

    <img className="thumbnail" alt="Web Hosting" src="/images/assets/screens/control-panel/product-selection/web-cloud/hosting-plans.png" loading="lazy" />
  </Tab>
  <Tab label="Etapa 2">
    Na página que se abrir, na caixa **Configuração**, clique no botão <code className="action">...</code> à direita da menção <code className="action">Endereços de e-mail</code>, e depois em <code className="action">Ativar a minha oferta de e-mail</code>.
  </Tab>
  <Tab label="Etapa 3">
    Na nova página que surgir, selecione o domínio em causa na secção `(1)` e, em seguida, prossiga até à ativação dos endereços de e-mail.
  </Tab>
</Tabs>

<details>

<summary>Clique aqui para mais informações.</summary>

Consulte o nosso guia detalhado "[Alojamento web - Ativar os endereços de e-mail incluídos](/guides/web-cloud/web-hosting/activate-email-hosting)".

</details>

### 4 - Criar um endereço de e-mail personalizado com o seu nome de domínio <a name="part-4"></a>

Clique nos separadores abaixo para visualizar cada uma das **4** etapas.

<Tabs>
  <Tab label="Etapa 1">
    Aceda à página <ManagerLink to="/#/web/email_domain">MX Plan</ManagerLink> e escolha o domínio em causa.
  </Tab>
  <Tab label="Etapa 2">
    Na página que se abrir, clique no separador <code className="action">E-mails</code>.
  </Tab>
  <Tab label="Etapa 3">
    Na nova página que aparece, clique no botão <code className="action">Criar um endereço de e-mail</code>.
  </Tab>
  <Tab label="Etapa 4">
    Na nova janela, preencha os elementos solicitados até à sua validação.
  </Tab>
</Tabs>

Repita esta operação para cada endereço de e-mail que deseja criar (no limite da sua oferta de alojamento web).

<details>

<summary>Clique aqui para mais informações.</summary>

Consulte o nosso guia detalhado "[Criar um endereço de e-mail](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation)".

</details>

### 5 - Opções adicionais disponíveis com o seu alojamento web <a name="part-5"></a>

O seu alojamento web não está limitado à instalação de um "módulo 1 clique". Pode alojar websites criados por si ou por um programador web (blogue, CMS, loja online, etc.). Se pretender saber mais sobre as possibilidades do seu alojamento web, não hesite em consultar o nosso guia mais pormenorizado "[Como criar um website - Realizar o seu projeto em 5 etapas](/guides/web-cloud/web-hosting/website-project)".

## Quer saber mais?

Encontre aqui uma seleção dos nossos guias que detalham as principais funcionalidades propostas nos nossos alojamentos web:

- [Alojamento web - Gerir um certificado SSL](/guides/web-cloud/web-hosting/ssl-on-webhosting).
- [Guia de utilização do acelerador CDN num alojamento web](/guides/web-cloud/web-hosting/cdn-how-to-use-cdn).
- [Alojamento web - Ambiente, versão PHP, .ovhconfig](/guides/web-cloud/web-hosting/configure-your-web-hosting).
- [Alojamento web - Consultar as estatísticas e logs de um website](/guides/web-cloud/web-hosting/logs-and-statistics).
- [Alojamento partilhado Seguimento dos emails automatizados](/guides/web-cloud/web-hosting/mail-function-script-records).
- [Como geolocalizar um website num país específico?](/guides/web-cloud/web-hosting/multisites-geolocation).
- [Partilhado: ativação da firewall](/guides/web-cloud/web-hosting/multisites-activating-application-firewall).
- [Configurar e utilizar o Git com o seu alojamento web OVHcloud](/guides/web-cloud/web-hosting/git-integration-webhosting).
- [Aceder ao espaço de armazenamento FTP do alojamento web](/guides/web-cloud/web-hosting/ftp-connection).
- [Criar tarefas automatizadas (CRON) no seu alojamento Web](/guides/web-cloud/web-hosting/cron-tasks).

Para serviços especializados (referenciamento, desenvolvimento, etc.), contacte os [parceiros OVHcloud](https://partner.ovhcloud.com/pt/directory/).

Se pretender usufruir de uma assistência na utilização e na configuração das suas soluções OVHcloud, consulte as nossas diferentes [ofertas de suporte](https://www.ovhcloud.com/pt/support-levels/).

Fale com a nossa [comunidade de utilizadores](/links/community).
