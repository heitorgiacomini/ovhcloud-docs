...
<Region zones={['eu']}>
- Dispor de um serviço de [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/), com exceção de um serviço de alojamento Cloud Web.
</Region>
...
**Caso 1 - utiliza um Content Management System (CMS)** tal como *WordPress*, *Joomla!*, *PrestaShop* ou *Drupal* : 
...
**Caso 2 - utiliza um site baseado numa solução personalizada** : 

- Aproxime-se do webmaster que criou o website.
- Consulte a [documentação oficial PHP](https://php.net/manual/en/appendices.php) para obter mais informações sobre as migrações da versão.
...
**Clique nos 2 títulos abaixo para visualizar o conteúdo.**

<details>
<summary>Através da Área de Cliente OVHcloud</summary>
...
<Tabs>
  ...
  <Tab label="Etapa 2">
    No separador <code className="action">Informações gerais</code>, localize a versão abaixo de *Versão global de PHP*.
...
<details>
<summary>Através de um script</summary>

Crie um script **.php** que contenha apenas o seguinte código:
...
Consulte o guia "[Ativação da firewall da aplicação](/guides/web-cloud/web-hosting/multisites-activating-application-firewall)" para mais informações sobre o assunto.
...
<Tabs>
  ...
  <Tab label="Etapa 3">
    Aparecerá uma janela com duas opções possíveis. Selecione a que corresponde à ação que deseja realizar e clique em <code className="action">Seguinte</code>.

    |Escolha|Detalhes|
    |---|---|
    |`Voltar a uma configuração anterior`|Depois de selecionar esta opção, escolha a configuração a restaurar ao lado de `Escolha histórica`. Esta opção não estará disponível se não tiver efetuado nenhuma alteração anteriormente.|
