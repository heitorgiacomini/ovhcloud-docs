...
:::info
Se o website não for apresentado corretamente após esta operação, consulte os guias OVHcloud relativos aos alojamentos partilhados na secção [Diagnóstico](/guides/web-cloud/web-hosting/landing-page-web-hosting).
...
<Tabs>
  ...
  <Tab label="Etapa 3">
    Caso contrário, dirija-se ao separador <code className="action">Meus sites</code>. Registe o nome da `Pasta raiz` presente na linha do site em questão: trata-se do diretório onde se encontram os ficheiros que constituem o seu módulo 1 clique no servidor FTP.
  </Tab>
</Tabs>

Aceda ao [espaço FTP](/guides/web-cloud/web-hosting/ftp-connection) do seu alojamento. Abra a `Pasta raiz` encontrada anteriormente no separador <code className="action">Meus sites</code> e procure o ficheiro de configuração do seu módulo:
...
Se já atingiu o número máximo de bases de dados permitido pela sua [solução de alojamento web](/links/web/hosting), deverá eliminar a base de dados identificada [no passo 1](#step1) **ANTES** de poder criar um novo módulo 1 clique.
