...
description: Descubra como migrar o seu website, o seu nome de domínio, a sua base de dados e os seus e-mails para a OVHcloud sem interrupção de serviços
...
Várias ofertas de [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/) incluem uma oferta de e-mail "[MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities)". Esta oferta de e-mail permite criar endereços de e-mail com um espaço de armazenamento de 5 GB no máximo para cada endereço. Escolha a seguir as ofertas de alojamento em função da versão PHP, da versão SQL, do número de endereços de e-mail que precisa e do tamanho do seu site a migrar:

Gama **Eco**:

- **Starter** — 2 endereços de e-mail "MX Plan"
- **Perso** — 10 endereços de e-mail "MX Plan"

Gama **Business**:

- **Startup** — 10 endereços de e-mail "MX Plan"
- **Pro** — 100 endereços de e-mail "MX Plan"
- **Performance** — 1000 endereços de e-mail "MX Plan"

Gama **Agencies**:

- **Agency**, **Agency Plus** e **Agency Max** — 1000 endereços de e-mail "MX Plan" cada uma

Depois de escolher a oferta de alojamento, se ainda não é cliente da OVHcloud, clique no botão <code className="action">Encomendar</code> presente na página comercial da oferta escolhida. Siga os passos da encomenda **sem solicitar a transferência do seu nome de domínio** (esta ação será realizada na etapa 10 deste manual).
...
<Tabs>
  <Tab label="Etapa 1">
    Aceda à página <ManagerLink to="/#/web/hosting">Alojamentos</ManagerLink>.
  ...
  <Tab label="Etapa 2">
    No canto superior esquerdo da interface, clique no botão <code className="action">Encomendar</code> e, a seguir, em <code className="action">Alojamentos</code>.
  </Tab>
  <Tab label="Etapa 3">
...
<Region zones={['ca']}>
:::info
A OVHcloud oferece outras ofertas de e-mail para além da oferta "MX Plan". Por exemplo, pode combinar endereços de e-mail "MX Plan" de contas ["Exchange"](https://www.ovhcloud.com/pt/emails/hosted-exchange/).
...
Quando o alojamento estiver criado, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e crie uma zona DNS para o seu domínio **sem "www"**. Para mais informações, consulte o nosso manual sobre a [criação de uma zona DNS na OVHcloud](/guides/web-cloud/domains/dns-zone-create).

Depois de criar a zona DNS, aceda à sua gestão através do nosso guia "[Editar uma zona DNS da OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
...
Para obter o endereço IP certo de destino do seu alojamento OVHcloud, consulte o guia que apresenta os [endereços IP dos diferentes clusters de alojamento partilhados](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip).
...
<Region zones={['ca', 'apac']}>
:::tip
Anote os dois valores alvo que têm como tipo de registo "NS". Estes valores, do tipo `dnsXX.ovh.ca` e `nsXX.ovh.ca` (ou `dns200.anycast.me` e `ns200.anycast.me`), correspondem aos servidores DNS associados a esta zona DNS para o seu domínio. Estas instruções serão utilizadas na [etapa 9](#step9) deste manual.
...
Para importar o backup do espaço de armazenamento FTP do antigo fornecedor, [aceda ao espaço de armazenamento FTP do seu alojamento OVHcloud](/guides/web-cloud/web-hosting/ftp-connection) e elimine o backup da pasta raiz "www" (ou de outra pasta raiz que tenha criado).

Recomendamos que utilize o software [FileZilla](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide) para transferir o seu backup FTP para o seu alojamento.
...
Para o backup da sua base de dados, [crie uma nova base de dados](/guides/web-cloud/web-hosting/sql-create-database) e depois [importe o seu backup para a sua nova base de dados](/guides/web-cloud/web-hosting/sql-importing-mysql-database).

:::info
A OVHcloud oferece servidores de base de dados Web Cloud Databases. Se pretender utilizar esta oferta com o seu website, consulte toda a nossa documentação sobre este produto na [nossa página dedicada](/guides/web-cloud/databases/overview).
...
:::tip
Para ligar a sua nova base de dados se utiliza um Content Management System (CMS) como WordPress, Joomla!, Drupal ou PrestaShop, encontre as informações sobre os seus ficheiros de configuração a partir do **etapa 2** do guia "[modificação da palavra-passe de uma base de dados](/guides/web-cloud/web-hosting/sql-change-password)".

:::


Declare/autorize o seu nome de domínio externo no seu alojamento web OVHcloud através do nosso guia "[gestão dos websites de um alojamento web OVHcloud](/guides/web-cloud/web-hosting/multisites-configure-multisite)". Declare o "nome" da pasta raiz que escolheu no início da [etapa 4](#step4). Relembramos que esta é a pasta na qual colocou os seus ficheiros no seu espaço de armazenamento FTP.
...
Recrie de forma idêntica os endereços de e-mail presentes no seu fornecedor de e-mail através do nosso guia sobre a [criação de endereços de e-mail "MX Plan"](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).
...
<Region zones={['ca']}>
Se optou por uma solução "Exchange", consulte o nosso manual para criar os seus endereços de e-mail:
...
<Region zones={['ca', 'apac']}>
- O seu domínio (sem "www") para o destino do tipo "MX": mx1.mail.ovh.ca ".
- O seu domínio (sem "www") para o destino do tipo "MX": mx2.mail.ovh.ca ".
- O seu domínio (sem "www") para o destino do tipo "MX": mx3.mail.ovh.ca ".
</Region>
...
**Opção 1**: utilize a nossa ferramenta [OVHcloud Mail Migrator (OMM)](https://omm.ovhcloud.com/) que permite copiar o conteúdo dos endereços de e-mail registados no seu antigo fornecedor para os criados na OVHcloud. Para mais informações, consulte o nosso manual "[Migrar contas de e-mail via OVHcloud Mail Migrator](/guides/web-cloud/email-and-collaborative-solutions/migrating/omm-migrate-email-account-to-ovhcloud)".

Sugerimos que não utilize o <code className="action">Tipo de servidor</code> **POP** na parte <code className="action">Conta source</code>. Este protocolo apaga os e-mails do seu antigo servidor para os enviar para o servidor OVHcloud de destino. Neste caso, já não será possível comparar o conteúdo do endereço de e-mail antigo e novo.

Para a parte <code className="action">Conta de destino</code>, introduza apenas o endereço de e-mail OVHcloud em questão e a respetiva palavra-passe. Isto permitindo o <code className="action">Tipo de serviço</code> em <code className="action">Hosted by OVH (Autodetect)</code>.
...
- Encontre os parâmetros de configuração no nosso guia "[Generalidades sobre os e-mails MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities#consult-emails-client)". Além disso, poderá encontrar links para os guias de configuração personalizados para os principais softwares de e-mail.

<Region zones={['eu']}>
#### Para as contas "Email Pro"

- Encontre os nossos guias de ajuda à configuração nas secções `Configuration on computador` e `Configuration no smartphone` de [o nosso manual na oferta Email Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/overview).
</Region>

<Region zones={['eu', 'ca']}>
#### Para as contas de e-mail "Exchange" :

- Encontre os nossos guias de ajuda à configuração nas secções `Configuration Exchange no computador` e `Configuration Exchange no smartphone` de [a nossa documentação sobre a oferta Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/overview).
</Region>
...
<Region zones={['ca', 'apac']}>
Substitua os servidores DNS atuais (do registar de origem) pelos dois servidores DNS declarados na zona DNS da OVHcloud (de tipo `dnsXX.ovh.ca` e `nsXX.ovh.ca` ou `dns200.anycast.me` e `ns200.anycast.me`). Esta operação faz-se na interface de gestão do registar de origem.
</Region>
...
De seguida, transfira o domínio para a OVHcloud através do nosso manual sobre a [transferência de um domínio](/guides/web-cloud/domains/transfer-incoming-generic-domain).
...
[Generalidades sobre os servidores DNS](/guides/web-cloud/domains/dns-server-general-information).

[Criar um endereço de e-mail partilhado](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).

[Importação de uma base de dados MySQL](/guides/web-cloud/web-hosting/sql-importing-mysql-database).
