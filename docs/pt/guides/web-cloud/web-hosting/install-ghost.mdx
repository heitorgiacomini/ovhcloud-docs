...
description: Saiba como instalar um blogue com a plataforma Ghost num alojamento Cloud Web
...
O [alojamento Cloud Web da OVHcloud](https://www.ovhcloud.com/pt/web-hosting/cloud-web-offer/) permite utilizar Node.js como motor de execução para os seus websites e, assim, instalar e alojar o Ghost ou qualquer outra aplicação concebida para Node.js.
...
- Ter um serviço de [alojamento Cloud Web da OVHcloud](https://www.ovhcloud.com/pt/web-hosting/cloud-web-offer/).
...
Para aceder aos motores de execução do seu alojamento Cloud Web, aceda à <ManagerLink to="/">Área de Cliente</ManagerLink>. Clique em <code className="action">Alojamentos</code> na barra à esquerda e, em seguida, selecione o alojamento Cloud Web correspondente. Por fim, clique no separador <code className="action">Motores de execução</code>.
...
- **Se pretender adicionar um motor**: clique em <code className="action">Ações</code> na parte superior da tabela e em <code className="action">Adicionar um tempo de execução para o aplicativo de software</code>.
- **Se pretender alterar um motor**: clique no botão <code className="action">...</code> à direita do motor e, a seguir, em <code className="action">Alterar</code>.
...
Concluída esta etapa, clique no botão <code className="action">Validar</code>. Para mais informações sobre os motores de execução, consulte o nosso manual [Gerir os motores de execução de um alojamento Cloud Web](/guides/web-cloud/web-hosting/manage-runtime-software-applications).
...
Agora que o Node.js está ativado como motor de execução, já o pode associar a um multisite. Para isso, clique no separador <code className="action">Multisite</code>. Aparecerá uma tabela com todos os domínios adicionados enquanto multisite. 
...
- **Se pretender adicionar um multisite**: clique em <code className="action">Adicionar um domínio ou subdomínio</code> à direita da tabela.
- **Se pretender alterar um multisite**: clique no botão <code className="action">...</code> à direita do nome de domínio em causa, e depois em <code className="action">Modificar</code>.
...
No que diz respeito às opções adicionais, selecione as que pretende ativar. Uma vez as informações introduzidas, clique em <code className="action">Seguinte</code> e finalize a operação. Esta operação pode demorar até uma hora. No entanto, a alteração da configuração DNS pode demorar até 24 horas antes de ficar totalmente efetiva. Se pretender obter mais informações sobre a gestão dos multisites, consulte o nosso manual “[Partilhar o alojamento entre vários sites](/guides/web-cloud/web-hosting/multisites-configure-multisite)”.
...
Clique no separador <code className="action">Base de dados</code> (se não o conseguir ver, clique no botão com três barras). A tabela apresenta as bases de dados já criadas no seu alojamento. Para iniciar a criação de uma nova base de dados, existem duas possibilidades:

- **Se ainda não tiver criado uma base de dados**: clique no botão <code className="action">Criar uma base de dados</code>.

- **Se já tiver criado uma base de dados**: clique no botão <code className="action">Ações</code> acima da tabela e, a seguir, <code className="action">Criar uma base de dados</code>.
...
Na nova janela, apenas "MySQL", em versão "5.6", está disponível. A seguir, selecione a opção “Armazenado na sua instância Cloud Web” e, por fim, <code className="action">Seguinte</code>.

Personalize um nome de utilizador e defina uma palavra-passe. Depois clique em <code className="action">Seguinte</code>. Confirme a criação da base de dados clicando em <code className="action">Validar</code>. Aguarde alguns minutos até a operação ser finalizada.
...
Para isso, clique no separador <code className="action">Variáveis de ambiente</code>. A tabela apresenta as variáveis já criadas. Para adicionar uma nova, clique no botão <code className="action">Ações</code> acima da tabela e, depois, em <code className="action">Adicionar uma variável de ambiente</code>.
...
Na nova janela que aparecerá, preencha as informações necessárias em função da sua situação e, depois, clique em <code className="action">Confirmar</code> para criar a variável. Estas são as variáveis que criámos para este tutorial:
...
Primeiro, certifique-se de que tem todas as informações que precisa para se ligar. De seguida, clique no separador <code className="action">FTP - SSH</code>. (se não o conseguir ver, clique no botão com três barras). Aparecerá a informação relativa ao seu espaço de armazenamento. Consulte os seguintes elementos:
...
Se já não se lembra da palavra-passe SSH, clique no botão <code className="action">...</code> situado no final da linha correspondente e selecione <code className="action">Alterar a palavra-passe</code>.
...
Para reiniciar o *daemon* Node.js, aceda à sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. No separador <code className="action">Multi-site</code>, clique no botão <code className="action">...</code> no final da linha correspondente ao domínio à direita, em <code className="action">Reiniciar</code>.
...
[Transferir o meu site para a OVHcloud](/guides/web-cloud/web-hosting/hosting-migrating-to-ovh) (Versão PT disponível em breve)
