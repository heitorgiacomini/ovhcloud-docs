...
description: Saiba como recuperar a cópia de segurança de uma base de dados de um alojamento web da OVHcloud
...
- Consoante o método de backup utilizado, ter acesso à gestão do alojamento web a partir da <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> ou às informações que lhe permitem aceder à base de dados.
...
- **Utilizar a ferramenta de backup da OVHcloud** : esta solução permite recuperar backups das suas bases de dados a partir da <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. Este método não requer conhecimentos técnicos especiais.
...
- **Efetuar o backup através de SSH** : este método requer que se ligue ao espaço de armazenamento FTP através do protocolo SSH e que utilize comandos para interagir com este. Este tipo de acesso requer conhecimentos mais avançados, bem como uma oferta de [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/) específica.

:::tip
Se efetuar um backup da sua base de dados, pois esta está saturada / cheia, não hesite em consultar o nosso tutorial "[O que fazer quando a minha base de dados estiver saturada?](/guides/web-cloud/web-hosting/sql-overquota-database)".
...
Ainda no separador <code className="action">Bases de dados</code>, clique no botão <code className="action">...</code> à direita da base de dados a salvaguardar e, a seguir, em <code className="action">Criar uma cópia de segurança</code>.
...
Na janela que surgir, selecione a data desejada para o backup e clique no botão <code className="action">Seguinte</code>. Certifique-se de que as informações no resumo estão corretas e clique em <code className="action">Validar</code> para dar início à operação.
...
Ainda no separador <code className="action">Bases de dados</code>, clique no botão <code className="action">...</code> à direita da base de dados a salvaguardar e, a seguir, em <code className="action">Restaurar um backup</code>.
...
Para descarregar um backup, clique no botão <code className="action">...</code> à direita daquele que deseja recuperar e, a seguir, em <code className="action">Transferir backup</code>. Surgirá uma janela que lhe solicitará para guardar o ficheiro na sua máquina. Aceite e espere enquanto o backup é descarregado.
...
Para realizar a operação, aceda a *phpMyAdmin*. Para saber a ligação de acesso a esta última, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e aceda à secção <code className="action">Web Cloud</code>. Na coluna da esquerda, clique em <code className="action">Alojamentos</code> e escolha o nome do alojamento correspondente. Por fim, clique no separador <code className="action">Bases de dados</code>.

Aparecerá uma tabela com todas as bases de dados criadas com o plano de alojamento web. Clique no botão <code className="action">...</code> à direita da base de dados em causa e depois em <code className="action">Aceder ao phpMyAdmin</code>.
...
Uma vez na interface de ligação a *phpMyAdmin*, introduza as informações da base de dados e ligue-se. Depois de iniciar sessão, aceda ao separador <code className="action">Exportar</code> onde existem dois métodos de exportação:
...
Depois de criar o script de backup, deverá carregá-lo para o espaço de armazenamento FTP do seu alojamento web. Para isso, consulte as informações descritas no passo 2 da documentação intitulada "[Ligar-se ao espaço de armazenamento](/guides/web-cloud/web-hosting/hosting-how-to-get-my-website-online)".
...
:::info
A utilização de um script de backup com o nosso sistema de tarefas planificadas (tarefas « CRON ») pode permitir automatizar backups com a frequência que desejar. Saiba mais sobre as tarefas planificadas no manual: "[Criar uma tarefa planificada (CRON) no alojamento web](/guides/web-cloud/web-hosting/cron-tasks)".
...
[Tutorial - O que fazer quando o meu banco de dados estiver cheio?](/guides/web-cloud/web-hosting/sql-overquota-database)
