...
Declare o seu novo nome de domínio com a nossa documentação sobre [como adicionar um site web ao seu alojamento partilhado](/guides/web-cloud/web-hosting/multisites-configure-multisite). Declare também o seu subdomínio em `www` se quiser, por exemplo, que `www.NewDomain.tld` também mostre o seu site, em adição a `NewDomain.tld`.
...
- Verifique que o seu novo domínio aponta para o endereço IP correto do seu alojamento partilhado. Para obter o endereço IP, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>, vá à secção <code className="action">Web cloud</code>, clique nos <code className="action">Alojamentos</code>, selecione o seu alojamento e recupere **o IPv4** no separador <code className="action">Informações gerais</code>.
...
Para encontrar o número do cluster onde está o seu alojamento, aceda à parte <code className="action">Web cloud</code>, clique em <code className="action">Alojamentos</code>, selecione o separador <code className="action">FTP-SSH</code>. Visualizará o número do cluster no formulário **Servidor FTP e SFTP**: `ftp.cluster0XX.ovh.net` (onde os `X` representam o número de cluster).
...
<img className="thumbnail" alt="multi-sites" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/multisite/all-domain-same-config-enable.png" loading="lazy" />
