...
:::info
Este guia diz respeito principalmente aos e-mails enviados a partir de scripts localizados no seu [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/) com a ajuda da função "mail()" de PHP.

Se deseja gerir os endereços de e-mail incluídos na sua oferta MX Plan ou na sua oferta de [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/), consulte a nossa documentação sobre os [E-mails partilhados - MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/overview).
:::
...
<Tabs>
  ...
  <Tab label="Etapa 2">
    Na página que se apresenta, clique no separador <code className="action">Mais</code> e depois em <code className="action">Scripts e-mails</code>.
  ...
  <Tab label="Etapa 3">
    Esta página permite-lhe seguir e gerir os e-mails automáticos enviados a partir do seu [alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/).
    ...
    - **E-mails em erro**: permite o acesso aos logs dos últimos e-mails que falharam no envio. Encontrará os endereços de e-mail em causa com o erro associado. Atenção, este histórico não será reiniciado, mesmo que decida <code className="action">Purgar os e-mails</code> ou <code className="action">Desbloquear o envio</code>.
    ...
    Para realizar a ação pretendida, clique no botão correspondente e depois em <code className="action">Validar</code>. Em certos casos, a ação pretendida pode levar vários minutos para ser plenamente efetiva.
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab label="Etapa 3">
    ...
    - Se clicar em <code className="action">Desbloquear o envio</code>, o estado do serviço passará para *«Force»*. O rácio **e-mails que voltaram com erro / número total de envios de e-mails enviados** autorizado antes de um bloqueio será duplicado. O envio voltará a estar operacional alguns minutos após o desbloqueio.
...
No entanto, recomendamos que recorra a um [fornecedor especializado](https://partner.ovhcloud.com/pt/directory/) se encontrar dificuldades. Não lhe poderemos prestar assistência. Para mais informações, aceda à secção [«Quer saber mais?»](#go-further) deste guia.
:::
...
:::warning
Os e-mails emitidos com um script que utilize uma configuração SMTP não poderão ser geridos e seguidos a partir da sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.
:::
...
Se pretender usufruir de uma assistência na utilização e na configuração das suas soluções OVHcloud, consulte as nossas [ofertas de suporte](https://www.ovhcloud.com/pt/support-levels/).
