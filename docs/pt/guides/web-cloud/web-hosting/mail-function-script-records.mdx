...
Se pretender gerir os endereços de e-mail incluídos na sua oferta MX Plan ou na sua oferta de[alojamento web OVHcloud](https://www.ovhcloud.com/pt/web-hosting/), consulte a nossa documentação sobre os [E-mails partilhados - MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/overview).
...
O acompanhamento e a gestão dos e-mails automatizados do seu alojamento web OVHcloud efetuam-se a partir do seu <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.
...
<Tabs>
  ...
  <Tab label="Etapa 2">
    Na página que se abrir, clique no separador <code className="action">Mais</code> e clique em <code className="action">Scripts e-mails</code>.
...
### Apresentação da parte "Scripts e-mails"
...
- **Relatório de erros a**: receba-o diariamente no endereço de e-mail à sua escolha. Defina-a graças ao botão <code className="action">Alterar o destinatário</code>. Este relatório inclui os e-mails enviados a partir do seu alojamento web com rendimentos de erro na OVHcloud. Um botão <code className="action">Emails em erro</code> também permite consultar estes relatórios a qualquer momento, à direita da página <code className="action">Scripts emails</code>.
...
- **E-mails em erro**: permite o acesso aos logs dos últimos e-mails com erro de envio. Encontrará os endereços de e-mail afetados com o erro associado. Atenção, este histórico não será reiniciado, mesmo se decidir <code className="action">Purgar os e-mails</code> ou <code className="action">desbloquear o envio</code>.
...
Para realizar a ação desejada, clique no botão correspondente e depois em <code className="action">Valider</code>. Em certos casos, a ação desejada pode demorar dezenas de minutos para ser plenamente efetiva.
...
- A configuração de [o registo SPF](/guides/web-cloud/domains/dns-zone-spf) na zona DNS do nome de domínio.
- A configuração de [o registo DMARC](/guides/web-cloud/domains/dns-zone-dmarc) na zona DNS do nome de domínio, **apenas se o servidor de destino o exigir**.
- Verifique a reputação do endereço IP na origem do envio ([a do seu alojamento web](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip) no seu caso), com a ajuda de uma ferramenta tal como [MXtoolbox](https://mxtoolbox.com/) ou [Spamhaus](https://check.spamhaus.org/).
...
- desativou a funcionalidade a partir do seu <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.

Para desbloquear a situação, aceda à secção <code className="action">Web Cloud</code>, clique em <code className="action">Alojamentos</code> e escolha o alojamento em causa na lista. A seguir, clique no separador <code className="action">Plus</code> e depois em <code className="action">Scripts emails</code>.

Finalmente, clique em <code className="action">Libertar o envio</code> e aguarde alguns minutos para que o serviço de envio fique novamente ativo.
...
- Se clicar em <code className="action">Libertar o envio</code>, o estado do serviço passará para *"Force"*. O rácio **e-mails devolvidos em erro / número total de envios de e-mails enviados** autorizado antes de um bloqueio será duplicado. O envio estará de novo operacional alguns minutos após o desbloqueio.
- Se clicar em <code className="action">Purger os e-mails</code>, este sistema apagará todos os e-mails da fila de espera e o estado do serviço passará a *"Ativo"* sem duplicar o rácio.
...
De seguida, aceda à secção <code className="action">Web Cloud</code>, clique em <code className="action">Alojamentos</code> e escolha o alojamento em causa na lista. A seguir, clique no separador <code className="action">Plus</code> e depois em <code className="action">Scripts emails</code>.

A seguir, clique em <code className="action">Purger os e-mails</code>, e o estado do serviço eliminará todos os e-mails da fila de espera. O estado do serviço será novamente indicado em *"Ativo"*.
...
Depois de proteger o alojamento, aceda à secção <code className="action">Web Cloud</code>, clique em <code className="action">Alojamentos</code> e escolha o alojamento em causa na lista. A seguir, clique no separador <code className="action">Plus</code> e depois em <code className="action">Scripts emails</code>.
...
:::warning
Os e-mails emitidos com um script utilizando uma configuração SMTP não poderão ser geridos e acompanhados a partir do seu <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.
