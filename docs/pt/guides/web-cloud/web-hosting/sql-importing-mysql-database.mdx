...
description: Saiba como importar um backup para a base de dados de um alojamento web da OVHcloud
...
- Possuir uma base de dados criada no âmbito de uma oferta de [alojamento web da OVHcloud](https://www.ovhcloud.com/pt/web-hosting/).
...
- **Restaurar a base de dados numa data anterior**: esta solução permite restaurar o conteúdo da base de dados utilizando as cópias de segurança armazenadas na ferramenta de backup da OVHcloud. Esta operação não requer conhecimentos técnicos especiais e realiza-se a partir da <ManagerLink to="/">Área de Cliente</ManagerLink>.

- **Importar o seu próprio ficheiro de backup**: esta solução permite importar os dados do seu próprio ficheiro de backup numa das suas bases de dados. Esta operação pode ser realizada a partir da <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>.
...
Aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>, clique em <code className="action">Alojamentos</code> e escolha o nome do alojamento correspondente. Por fim, clique no separador <code className="action">Base de dados</code>.
...
Aparecerá uma tabela com todas as bases de dados criadas com o plano de alojamento. Clique nos três pontos à direita da linha correspondente à base de dados que pretende restaurar numa data anterior e selecione <code className="action">Restaurar um backup</code>. Tenha em atenção que esta ação substituirá o conteúdo atual da base de dados pelo backup.
...
Clique nos três pontos à direita da linha correspondente à base de dados que pretende restaurar e selecione <code className="action">Restaurar um backup</code>. Na janela que aparece, certifique-se de que as informações estão corretas e clique em <code className="action">Confirmar</code>. Aguarde uns instantes para que a restauração seja realizada. 
...
Aparecerá uma tabela com todas as bases de dados criadas com o plano de alojamento. Clique nos três pontos à direita da linha correspondente à base de dados na qual pretende importar dados e selecione <code className="action">Importar ficheiro</code>.
...
Na nova janela, selecione <code className="action">Importar um novo ficheiro</code> e clique em <code className="action">Seguinte</code>.

:::info
A opção <code className="action">Utilizar um ficheiro existente</code> permite importar novamente os dados de um ficheiro que já foi enviado na ferramenta de importação.
...
Introduza o nome do ficheiro (que lhe permitirá identificar este backup mais tarde, caso pretenda voltar a restaurá-lo) e selecione o ficheiro de backup no seu computador. Clique em <code className="action">Enviar</code>.

Aguarde a confirmação de envio e, de seguida, clique em <code className="action">Seguinte</code>.
...
Uma vez selecionada a opção, clique em <code className="action">Confirmar</code> e aguarde até que a importação seja concluída.
...
Em primeiro lugar, deverá aceder a phpMyAdmin. Para isso, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>, clique em <code className="action">Alojamentos</code> e selecione o nome do alojamento correspondente. Por fim, clique no separador <code className="action">Base de dados</code>.

<img className="thumbnail" alt="Databases" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/databases.png" loading="lazy" />

Aparecerá uma tabela com todas as bases de dados criadas com o plano de alojamento. Clique nos três pontos à direita da linha correspondente à base de dados e selecione <code className="action">Aceder ao phpMyAdmin</code>.
...
Uma vez na página de phpMyAdmin, introduza as informações da base de dados, selecione a opção de aceder aos dados atuais da base de dados no menu pendente e, em seguida, conecte-se.  Depois de se conectar, na barra superior direita, selecione a opção <code className="action">Importar</code> e introduza a informação que lhe é solicitada. Recordamos que existe um limite para o tamanho do ficheiro de configuração.
