...
Aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e aceda à secção <code className="action">Web Cloud</code>. Clique em <code className="action">Alojamentos</code> na coluna da esquerda, escolha o alojamento que pretende criar e clique no separador <code className="action">Bases de dados</code>.
...
De seguida, clique em <code className="action">Seguinte</code>.
...
:::info
Quando clicar em <code className="action">Validar</code>, a criação da base de dados pode demorar até **15 minutos**. Recarregue a página web do seu <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> se a base de dados não aparecer automaticamente no quadro que lista as suas bases de dados.
...
:::info
Se lhe for pedido e qualquer que seja a base de dados [Start SQL](https://www.ovhcloud.com/pt/web-hosting/options/start-sql/) adicionada ou incluída no seu alojamento web OVHcloud, o número de **port** a utilizar é o **3306**.
...
Se necessário, para recuperar estas informações de ligação, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e aceda à secção <code className="action">Web Cloud</code>. Clique em <code className="action">Alojamentos</code> na coluna da esquerda, escolha a oferta de alojamento para a qual pretende recuperar as informações de ligação à base de dados e clique no separador <code className="action">Bases de dados</code>.
...
A OVHcloud fornece uma ferramenta online para a gestão das bases de dados, "phpMyAdmin". Para encontrar o link de acesso a esta aplicação, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e aceda à secção <code className="action">Web Cloud</code>. Clique em <code className="action">Alojamentos</code> na coluna da esquerda, escolha a oferta de alojamento para a qual pretende recuperar as informações de ligação à base de dados e clique no separador <code className="action">Bases de dados</code>. No quadro que se abrir, clique no botão <code className="action">...</code> à direita da base de dados em causa e, a seguir, clique em <code className="action">Aceder ao phpMyAdmin</code> no menu pendente.
...
Para verificar as snapshots disponíveis, bem como a data e a hora de criação, aceda à <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink> e aceda à secção <code className="action">Web Cloud</code>. Clique em <code className="action">Alojamentos</code> na coluna da esquerda, escolha a oferta de alojamento sobre a qual deseja consultar as snapshots disponíveis para a base de dados e, a seguir, clique no separador <code className="action">Bases de dados</code>. Na tabela que vai aparecer, clique no símbolo junto do círculo verde. Pode igualmente descarregar cada backup de uma base de dados a partir desse mesmo local. Encontre mais informações sobre este assumpto no nosso guia "[Obter a cópia de segurança da base de dados de um alojamento web](/guides/web-cloud/web-hosting/sql-database-export)".
...
Depois de a base de dados ter sido otimizada (eliminada), recalcule a sua quota na Área de Cliente OVHcloud para a desbloquear novamente. Encontre mais informações sobre este assumpto no nosso guia "[O que fazer quando a quota de armazenamento da minha base de dados for ultrapassada?](/guides/web-cloud/web-hosting/sql-overquota-database)"
