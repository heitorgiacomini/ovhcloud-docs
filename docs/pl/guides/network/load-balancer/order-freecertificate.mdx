...
You can configure your SSL termination from the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> in the `Bare Metal Cloud` section, then `Load Balancer`.

After selecting the Load Balancer you wish to modify, create a new frontend or edit an existing one.

In the `Frontends` section, click on the `Add Frontend` button to create a new frontend. <br />
...
![Configuring the SSL termination of a Frontend](/images/guides/network/load-balancer/order-freecertificate/enable_ssl_terminaison.png)
...
![Add a free SSL certificate](/images/guides/network/load-balancer/order-freecertificate/iplb-order-ssl.png)
