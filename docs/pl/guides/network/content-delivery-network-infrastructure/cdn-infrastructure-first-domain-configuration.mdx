...
description: Poznaj dobre praktyki konfiguracji domeny w Twojej usłudze CDN
...
- Posiadanie usługi [CDN (Content Delivery Network) od OVH](https://www.ovhcloud.com/pl/network/cdn/)
- Dostęp do interfejsu zarządzania strefą DNS Twojej domeny
- Dostęp do <ManagerLink to="/">Panelu klienta OVH</ManagerLink>
...
W tym celu przejdź do <ManagerLink to="/">Panelu klienta</ManagerLink> > zakładka <code className="action">Serwery dedykowane</code> > <code className="action">NAS i CDN</code>.

Następnie kliknij <code className="action">Dodanie domeny do CDN</code>:

<img className="thumbnail" alt="Panel klienta CDN" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/cdn_customer_panel.png" loading="lazy" />

W pierwszy kroku wybierz subdomenę, którą chcesz dodać do usługi CDN:

<img className="thumbnail" alt="Dodanie subdomeny do CDN" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/add_cdn_domain_step_1.png" loading="lazy" />

W sekcji *backend* możesz wybrać *backend* już istniejący lub też możesz dodać nowy adres IP:

<img className="thumbnail" alt="Dodanie backendu" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/add_cdn_domain_step_2.png" loading="lazy" />
...
<img className="thumbnail" alt="Szablon OVF Windows" src="/images/network/content-delivery-network-infrastructure/first-domain-name-configuration/cname_field.png" loading="lazy" />
