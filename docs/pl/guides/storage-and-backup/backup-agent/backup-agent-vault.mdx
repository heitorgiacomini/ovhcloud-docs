...
description: Dowiedz się, jak działa system Vault i gdzie przechowywane są Twoje dane kopii zapasowych
...
- Usługa Backup Agent zamówiona w momencie zakupu serwera Bare Metal lub później za pomocą menu <code className="action">Backup Agent</code> w Panelu klienta OVHcloud.
...
Jest to oparte na naszych obiektach magazynu, które możesz znaleźć [tutaj](https://www.ovhcloud.com/pl/public-cloud/object-storage/).

Aby znaleźć swoje Vault, kliknij <ManagerLink to="/#/bmc-backup-agent-baremetal/service">ten link</ManagerLink>, aby uzyskać dostęp do sekcji <code className="action">Backup Agent</code>, a następnie kliknij kartę <code className="action">Vaults</code>.

<img className="thumbnail" alt="Lista Vault Backup Agent" src="/images/storage-and-backup/backup-agent/backup-agent-vault/01-backup-agent-vault-list.png" loading="lazy" />
...
<img className="thumbnail" alt="Przypadki użycia Vault Backup Agent" src="/images/storage-and-backup/backup-agent/backup-agent-vault/01-backup-agent-vault-use-cases.png" loading="lazy" />
