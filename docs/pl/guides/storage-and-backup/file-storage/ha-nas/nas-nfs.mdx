...
description: Dowiedz się, jak się zalogować do usługi NAS-HA przy użyciu protokołu NFS
...
- [Utworzenie partycji dla Twojej usługi przy użyciu protokołu NFS](/guides/storage-and-backup/file-storage/ha-nas/nas-get-started#partition)
- [Posiadanie wpisu ACL dla adresu IP serwera](/guides/storage-and-backup/file-storage/ha-nas/nas-get-started#addaccess)
...
- **Link bezpośredni:** <ManagerLink to="/#/dedicated/nasha">NAS-HA</ManagerLink>
...
Potrzebna będzie również **nazwa wewnętrzna** oraz **adres IP** usługi NAS-HA, które można znaleźć w e-mailu otrzymanym po instalacji usługi lub w <ManagerLink to="/">Panelu client OVHcloud</ManagerLink>.
...
W interfejsie administracyjnym Proxmox kliknij <code className="action">Storage</code> w menu pionowym.

<img className="thumbnail" alt="proxmox" src="/images/storage-and-backup/file-storage/ha-nas/nas-nfs/proxmox1.png" loading="lazy" />

Kliknij przycisk <code className="action">Add</code> i wybierz <code className="action">NFS</code>.
...
Kliknij <code className="action">Add</code>, aby zamontować partycję.

### VMware ESXI

W interfejsie VMware ESXI kliknij <code className="action">Storage</code> w menu po lewej stronie.

Następnie kliknij przycisk <code className="action">New datastore</code>, aby otworzyć asystenta.

<img className="thumbnail" alt="ESXI" src="/images/storage-and-backup/file-storage/ha-nas/nas-nfs/esxi1.png" loading="lazy" />

W nowym oknie wybierz <code className="action">Mount NFS datastore</code> i kliknij <code className="action">Next</code>.
...
Następnie kliknij <code className="action">Next</code>. Kliknij <code className="action">Finish</code> w ostatnim etapie.
...
- Jeśli na udziale NFS zainstalowana jest baza danych, należy pamiętać, że w przypadku rozłączenia sieci specyficzny mechanizm blokowania NFS v4.x może spowodować wyłączenie aplikacji (więcej informacji można znaleźć w rfc: [https://datatracker.ietf.org/doc/rfc3530/](https://datatracker.ietf.org/doc/rfc3530/)).
- Jeśli instalujesz wirtualne maszyny VMware na swoim koncie zasobów NFS, pamiętaj, że mechanizm blokowania zintegrowany z wersją NFSv4.x nie jest kompatybilny z trybem klastrowania zaimplementowanym na NAS-HA (klaster w trybie aktywnym/pasywnym, wyjaśniony na [tej stronie](https://www.ovhcloud.com/pl/storage-solutions/nas-ha/)). Konieczne jest zatem użycie protokołu NFSv3, ponieważ w przeciwnym razie dojdzie do utraty dostępu do datastore w przypadku incydentu dotykającego serwer główny lub w przypadku planowanych prac konserwacyjnych.
