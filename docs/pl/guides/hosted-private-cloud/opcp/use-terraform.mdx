---
title: OPCP - How to use Terraform
description: Find out how to generate an Application Credential from Horizon and automate the deployment of your OPCP resources with Terraform
...
- An active [OPCP](https://www.ovhcloud.com/en-gb/hosted-private-cloud/onprem-cloud-platform/) service.
...
- A **private network** previously created in your OPCP project (see the guide [How to install an instance from the Horizon interface](/guides/hosted-private-cloud/opcp/how-to-setup-instance)).

## Table of Contents

- [1. Creating an Application Credential from Horizon](#appcred)
- [2. Preparing the Terraform environment](#tfenv)
- [3. Creating a server](#server)
- [4. Additional node-level configurations (RAID, LACP)](#nodeconfig)
- [5. Removing the infrastructure](#destroy)
- [6. Troubleshooting](#troubleshooting)
- [7. References](#references)

## Instructions

### 1. Creating an Application Credential from Horizon <a name="appcred"></a>
...
Log in to the **Horizon** interface of your OPCP environment, then select the **project** in which you want to deploy your resources via Terraform. For more information, refer to the [Getting started with your OPCP](/guides/hosted-private-cloud/opcp/getting-started) guide.

#### Creating the Application Credential

In the left-hand menu, click on <code className="action">Identity</code>, then on <code className="action">Application Credentials</code>.

<img className="thumbnail" alt="horizon-identity-application-credentials" src="/images/hosted-private-cloud/opcp/use-terraform/01-application-credential-step01.png" loading="lazy" />

Click on <code className="action">+ Create Application Credential</code>.

<img className="thumbnail" alt="horizon-create-application-credential" src="/images/hosted-private-cloud/opcp/use-terraform/01-application-credential-step02.png" loading="lazy" />
...
<img className="thumbnail" alt="horizon-application-credential-download" src="/images/hosted-private-cloud/opcp/use-terraform/01-application-credential-step03.png" loading="lazy" />
...
### 2. Preparing the Terraform environment <a name="tfenv"></a>
...
No parameters are required in the <code className="action">provider</code> block: the OpenStack provider automatically retrieves the authentication information from the `OS_*` environment variables.
...
### 3. Creating a server <a name="server"></a>
...
Confirm with `yes` when Terraform asks you to. Once the creation is complete, the instance is visible in the Horizon interface, in the <code className="action">Compute</code> > <code className="action">Instances</code> section.

### 4. Additional node-level configurations (RAID, LACP) <a name="nodeconfig"></a>
...
|-----------|----------------|-----------|
| `Authentication failed` | Wrong `application_credential_id` or `secret` | Check the credentials in Horizon (<code className="action">Identity</code> > <code className="action">Application Credentials</code>). |
| `Could not find image` | The image name does not match | List the available images via `openstack image list` or from Horizon. |
| `No suitable endpoint could be found` | Wrong `auth_url` URL or non-existent region | Check the Keystone URL and the region in <code className="action">Project</code> > <code className="action">API Access</code>. |
| `Network not found` | Private network missing in the project | Create a private network beforehand from Horizon (<code className="action">Network</code> > <code className="action">Networks</code>). |
