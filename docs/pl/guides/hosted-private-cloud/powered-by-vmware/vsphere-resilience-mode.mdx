---
title: Test tymczasowej utraty hosta poprzez aktywację trybu Resilience (EN)
description: Find out how to test a temporary host loss with resilience mode on your VMware on OVHcloud infrastructure
...
- A [VMware on OVHcloud solution](https://www.ovhcloud.com/pl/hosted-private-cloud/vmware/)
...
<Api version="v1" section="/dedicatedCloud" method="GET" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/host/\{hostId\}/resilience/canBeEnabled"} />

If the test is feasible, the result is: `true`.

To start the test, you can use the following call:

<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/host/\{hostId\}/resilience/enable"} />

The host will then be disconnected and switch to "No response" mode until the test is complete:

<img className="thumbnail" alt="vsphere" src="/images/hosted-private-cloud/powered-by-vmware/vsphere-resilience-mode/resilience_mode.png" loading="lazy" />

You can check the status of the action, using the following call:

<Api version="v1" section="/dedicatedCloud" method="GET" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/host/\{hostId\}/resilience"} />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/host/\{hostId\}/resilience/disable"} />
