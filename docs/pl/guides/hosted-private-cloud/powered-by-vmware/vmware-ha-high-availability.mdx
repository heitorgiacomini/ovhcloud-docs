...
description: Zarządzanie polityką restartowania przy użyciu funkcji HA
...
Kliknij polecenie <code className="action">Edytuj</code> i zaznacz pole, aby włączyć funkcję HA.
...
Wyłączenie tej funkcji jest konieczne na przykład do przeprowadzenia aktualizacji przy użyciu narzędzia [Update Manager](/guides/hosted-private-cloud/powered-by-vmware/use-vmware-update-manager). W tym konkretnym przypadku host jest izolowany.
...
Monitoring maszyn wirtualnych jest dostępny po zainstalowaniu narzędzia [VMware tools](/guides/hosted-private-cloud/powered-by-vmware/vmware-tools-install).
