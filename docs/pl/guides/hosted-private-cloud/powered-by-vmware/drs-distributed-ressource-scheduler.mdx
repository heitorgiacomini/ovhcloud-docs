...
description: Dowiedz się, jak kontrolować równoważenie obciążenia dzięki funkcji DRS
...
Jeśli funkcja DRS nie jest aktywna w klastrze, wybierz ją, a następnie przejdź do karty <code className="action">Konfiguruj</code>.

Kliknij menu <code className="action">Usługi</code> i wybierz podmenu <code className="action">vSphere DRS</code>.

Kliknij <code className="action">Edytuj</code> i przesuń przycisk `vSphere DRS`, aby włączyć funkcję DRS.
...
Czwarta reguła — Maszyny wirtualne do maszyn wirtualnych — została wyjaśniona w przewodniku dotyczącym [funkcji HA](/guides/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability).
