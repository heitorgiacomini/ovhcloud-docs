...
:::info
Jeśli Twój serwer należy do linii produktów **Eco**, przejdź do [tego przewodnika](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server-eco).
...
Możesz w prosty sposób przeprowadzić reinstalację serwera lub wybrać inny obraz systemu operacyjnego do zainstalowania w <ManagerLink to="/">Panelu klienta</ManagerLink>. W zakładce <code className="action">Informacje ogólne</code> kliknij <code className="action">...</code> naprzeciwko systemu operacyjnego, a następnie wybierz <code className="action">Zainstaluj</code>.
...
- Jeśli wcześniej przechowywałeś klucz publiczny w Twoim <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>, wybierz odpowiedni klucz z rozwijanego menu `Klucz(y) SSH, który chcesz wstępnie zainstalować`.
...
Możesz włączyć lub wyłączyć monitoring serwera dedykowanego w zakładce <code className="action">Informacje ogólne</code> w Twoim <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>. Wariant ten znajduje się w sekcji `Status usług`.
...
W przypadku każdego rodzaju problemu pierwszym krokiem do rozwiązania problemu jest uruchomienie serwera w trybie Rescue w Panelu <ManagerLink to="/">klienta OVHcloud</ManagerLink>. Przed skontaktowaniem się z zespołami pomocy ważne jest zidentyfikowanie problemów z serwerem w tym trybie.
