...
:::info
`Ed25519` jest uważany za najbezpieczniejszą metodę, ale `RSA` jest wartościową alternatywą. Oba są kompatybilne z Panelem klienta OVHcloud, jeśli chcesz [przechowywać klucze publiczne na Twoim koncie klienta](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
:::
...
Jeśli potwierdzisz parametrem <code className="action">Enter</code> bez wpisania nazwy, zostanie użyta standardowa nazwa pliku (`id_rsa` w tym przykładzie).
...
Skopiuj ten ciąg klucza, aby [dodać do nowego serwera](#getstarted) lub [zaimportować do panelu klienta](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
...
[Wprowadzenie do protokołu SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)

[Dedicated Server Mode](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)

[Tryb Rescue na serwerze VPS](/guides/bare-metal-cloud/virtual-private-servers/rescue)
