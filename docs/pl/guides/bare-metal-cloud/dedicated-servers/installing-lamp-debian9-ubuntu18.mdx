...
description: Dowiedz się, jak skonfigurować serwer WWW LAMP
...
Jeśli dystrybucja Debian lub Ubuntu nie jest już zainstalowana na Twoim serwerze, przeprowadź wstępną instalację w <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>. Jest to najlepszy sposób na posiadanie własnego systemu dla Twojego serwera www i aplikacji, które go wykonują.

Zapoznaj się z przewodnikiem dotyczącym instalacji dystrybucji w Twojej usłudze OVHcloud i połącz się z nią przez [SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction):
...
Potwierdź pierwszą prośbę, naciskając <code className="action">Enter</code>.
...
Zaleca się stosowanie proponowanej metody uwierzytelniania (*unix_socket*) zamiast dostępu za pomocą hasła root. Kliknij <code className="action">y</code>, a następnie <code className="action">Enter</code>. Jeśli zdecydujesz się na korzystanie z dostępu użytkownika root zamiast tego, wybierz <code className="action">n</code> i zdefiniuj hasło root na następnej stronie.
...
Poniższe zaproszenia dotyczące środków bezpieczeństwa, potwierdź je wszystkie za pomocą <code className="action">y</code> do końca skryptu.
...
Aby zainstalować system WordPress na serwerze, zapoznaj się [z tym tutorialu](/guides/public-cloud/compute/install-wordpress).
...
[Konfiguracja zapory ogniowej](/guides/bare-metal-cloud/dedicated-servers/firewall-linux-iptable) (*iptables*) poprawi bezpieczeństwo Twojego serwera. Proces ten można uprościć, korzystając z front-endu "Uncomplicated Firewall" (UFW) oraz zestawu wstępnie zdefiniowanych profili. 
...
Aby uzyskać dostęp do instalacji serwera www za pomocą domeny, należy przypisać go do Twojej usługi. W tym celu edytuj strefę DNS dostępną w <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>, pod warunkiem, że OVHcloud jest Twoim operatorem rekordu **i** że nazwa domeny wykorzystuje serwery DNS OVHcloud.

Aby dowiedzieć się więcej, zapoznaj się z przewodnikiem "[Modyfikacja strefy DNS](/guides/web-cloud/domains/dns-zone-edit)". Jeśli nazwa domeny jest aktualnie używana, skonfiguruj serwery DNS tylko po tym, jak Twoja strona WWW lub aplikacja są gotowe.
...
OVHcloud oferuje również rozwiązanie [SSL Gateway](https://www.ovhcloud.com/pl/security/ssl-gateway/). Więcej informacji znajdziesz w [dokumentacji](/guides/web-cloud/ssl-gateway/order).
...
[Jak pobierać i pobierać dane z serwera za pomocą SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp)
