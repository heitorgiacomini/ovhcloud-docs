...
- **Link bezpośredni:** <ManagerLink to="/#/dedicated/network-security/scrubbing-center">Security Dashboard</ManagerLink>
- **Ścieżka nawigacji:** <code className="action">Network</code> > <code className="action">Network Security Dashboard</code>
...
W trakcie ataku, aktywne filtrowanie jest wskazane przez ikonę ostrzegawczą na stronie z listą adresów IP (w sekcji <code className="action">Zarządzaj adresami IP</code> w Panelu klienta).
...
Zaloguj się do <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>, kliknij <code className="action">Network</code> w menu po lewej stronie ekranu, a następnie <code className="action">Publiczne adresy IP</code>. Upewnij się, że `Tryb zaawansowany` jest włączony, aby sprawdzić stan infrastruktury anty-DDoS i konfigurację jej komponentów
...
- Kolumna **Firewall** wskazuje stan Edge Network Firewall, który może być:
    ...
    - **(brak stanu)** - Konfiguracja firewalla nie została utworzona. Aby skonfigurować reguły, kliknij <code className="action">...</code> Przycisk wybierz <code className="action">Utwórz zaporę</code>.
...
Więcej informacji na temat konfigurowania reguł firewalla Edge Network znajdziesz w przewodniku [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network).
