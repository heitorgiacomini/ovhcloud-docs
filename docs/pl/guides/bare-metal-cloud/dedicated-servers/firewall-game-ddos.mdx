...
description: Dowiedz się, jak skonfigurować OVHcloud Game DDoS Protection firewall
...
:::info
Więcej informacji na temat naszej [ochrony anty-DDoS Game na naszej stronie](https://www.ovhcloud.com/pl/security/game-ddos-protection/).
...
- An [OVHcloud **Game** dedicated server](https://www.ovhcloud.com/pl/bare-metal/game/)
...
- **Link bezpośredni:** <ManagerLink to="/#/network/ip">Public IP</ManagerLink>
...
:::warning
Funkcja ta może być niedostępna lub ograniczona na serwerach z [**gamy Eco**](https://eco.ovhcloud.com/pl/about/).
...
:::warning
*Game Firewall* chroni adresy IP powiązane z serwerem. Jeśli posiadasz serwer z kilkoma adresami IP (np. [adresy Additional IP](https://www.ovhcloud.com/pl/network/additional-ip/)), musisz skonfigurować każdy z nich oddzielnie.
...
- Kliknij <code className="action">Network</code> w menu po lewej stronie ekranu.
- Kliknij <code className="action">Publiczne adresy IP</code>.

Możesz filtrować adresy IP, korzystając z menu rozwijanego <code className="action">Wszystkie typy usług</code>, lub bezpośrednio wpisać żądany adres IP w pasku wyszukiwania. Wpisz nazwę lub kategorię odpowiedniego serwera:

#### Lista adresów IP przypisanych do serwera Game

<Tabs>
  <Tab label="Ze strony Serwery dedykowane">
    - Otwórz sekcję <code className="action">Bare Metal Cloud</code> w menu po lewej stronie.
    - Wybierz <code className="action">Serwery dedykowane</code>.
    ...
    - Kliknij przycisk <code className="action">...</code> i wybierz <code className="action">Skonfiguruj Anty-DDoS Game</code>. Zostaniesz przekierowany do listy adresów IP przypisanych do Twojego serwera.
  </Tab>
  <Tab label="Ze strony Publiczne adresy IP">
    - Otwórz sekcję <code className="action">Network</code> w menu po lewej stronie.
    - Wybierz <code className="action">Publiczne adresy IP</code>.
    - W menu rozwijanym <code className="action">Wszystkie typy usług</code> znajdź i wybierz serwer Bare Metal Game, który chcesz skonfigurować.
...
Dla każdego adresu przypisanego do Twojego serwera wymagającego ochrony sprawdź, czy status *Game Firewall* jest ustawiony na <code className="action">Dostępny</code>. Reguły *Game Firewall* musisz skonfigurować osobno dla każdego adresu.

- Kliknij przycisk <code className="action">⁝</code> po prawej stronie tabeli i wybierz <code className="action">Skonfiguruj Game Firewall</code>.
...
- Ze względów bezpieczeństwa zdecydowanie zalecamy włączenie opcji <code className="action">Zastosuj politykę „Blokuj pozostałe"</code> w prawym górnym rogu tabeli reguł. Opcja ta blokuje cały ruch, który nie odpowiada regułom zdefiniowanym dla Game Firewall — wszystkie wymienione aplikacje gier będą chronione i żadne inne połączenia nie będą mogły dotrzeć do Twojego serwera. Opcja ta znacząco zmniejsza powierzchnię ataku narażoną na potencjalnych złośliwych aktorów.
...
Obsługiwane protokoły gier (tytuły i wersje gier, które mogą być chronione) mogą ulec zmianie w czasie. Ponadto mogą się one różnić między starszymi i nowszymi gamami serwerów Bare Metal Game. Najnowsza lista obsługiwanych profilów gier jest dostępna na [stronie Game DDoS Protection](https://www.ovhcloud.com/pl/security/game-ddos-protection/).
...
:::warning
Ochrona anty-DDoS Game obowiązuje po zastosowaniu reguł określonych w [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network). Aby oba działały poprawnie, Edge Network Firewall nie może być zbyt rygorystyczny i musi przepuszczać ruch do ochrony anty-DDoS Game.
...
Wszystkie protokoły ochrony anty-DDoS Game obsługiwane dla danego serwera można znaleźć na stronie konfiguracyjnej <code className="action">GAME firewall</code> dla każdego adresu IP wskazującego na ten serwer w rozwijanym menu <code className="action">Game protocol</code>:
...
Jeśli wybierzesz automatyzację, szczegóły protokołu można pobrać za pomocą [APIv6 OVHcloud](/guides/manage-and-operate/api/first-steps):
...
<summary>**Czy mogę korzystać z ochrony anty-DDoS Game w ofercie serwerów innych niż Bare Metal Game?**</summary>
...
<summary>**Jak mogę się upewnić, że automatyzacja działa dla adresu Additional IP między nową i starą generacją serwerów Bare Metal Game?**</summary>
...
<summary>**Czy mogę wyłączyć ochronę DDoS Game?**</summary>
...
<summary>**Moja gra nie znajduje się na liście obsługiwanych protokołów, co mogę zrobić?**</summary>
...
<summary>**Po skonfigurowaniu mojej gry z odpowiednimi portami i domyślną zasadą do usunięcia, nadal otrzymuję ataki, które wpływają na mój serwer Game. Co zrobić?**</summary>
