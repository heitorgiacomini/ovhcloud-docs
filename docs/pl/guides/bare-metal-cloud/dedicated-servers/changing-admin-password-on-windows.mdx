...
description: Dowiedz się, jak zmienić hasło administratora na serwerze dedykowanym z systemem Windows
lastUpdated: 2024-06-26
---

## Wprowadzenie

Podczas instalacji lub reinstalacji systemu operacyjnego Windows otrzymujesz hasło administratora. Rekomendujemy jego zmianę zgodnie z instrukcją zawartą w przewodniku o bezpieczeństwie [serwera dedykowanego](/guides/bare-metal-cloud/dedicated-servers/securing-a-dedicated-server). Jeśli utraciłeś hasło administratora, zresetuj je w trybie Rescue.
...
Sprawdź [ten przewodnik](/guides/bare-metal-cloud/dedicated-servers/rcw-changing-admin-password-on-windows), gdy w Panelu klienta OVHcloud jest używana opcja `Windows Customer Rescue System (Windows2022-based)`.
...
- Posiadanie [serwera dedykowanego](https://www.ovhcloud.com/pl/bare-metal/) z zainstalowanym systemem Windows
...
Więcej informacji o trybie Rescue znajdziesz w [tym przewodniku](/guides/bare-metal-cloud/dedicated-servers/rescue-mode).
...
Zacznij od zastąpienia netbootu **Uruchom z dysku twardego** w Panelu <ManagerLink to="/">klienta OVHcloud</ManagerLink> (patrz [Etap 1](#etap-1-restart-serwera-w-trybie-rescue)). 
...
W <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink> przejdź do zakładki <code className="action">IPMI</code>, aby otworzyć sesję KVM.
...
Kliknij <code className="action">OK</code> i zostaniesz zalogowany.
...
W tym interfejsie możesz manipulować plikiem *SAM*, aby usunąć hasło użytkownika administratora. Domyślna ścieżka dostępu do katalogu *WINDOWS* jest wstępnie wypełniona. Otwórz plik, aby wyświetlić listę użytkowników, klikając przycisk <code className="action">Otwórz</code>.

Użytkownik będzie albo "admin" albo "Administrator", w zależności od wersji systemu Windows. Jeśli oba są obecne, wybierz "admin". Następnie kliknij <code className="action">Zmień hasło</code>.
...
W oknie, które się wyświetli zostaw puste pola i kliknij <code className="action">OK</code>. Zakończ, klikając <code className="action">Zapisz zmiany</code>, a następnie <code className="action">Opuść</code>.
...
W oknie KVM wybierz opcję zatrzymania <code className="action">Wykonaj restart</code> za pomocą przycisku Windows "Uruchom" w lewym dolnym rogu.
...
[Korzystanie z IPMI dla serwerów dedykowanych](/guides/bare-metal-cloud/dedicated-servers/ipmi)
