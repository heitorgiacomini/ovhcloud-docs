...
3. Upewnij się, że nie łączysz jednej grupy serwerów DNS OVHcloud z inną grupą serwerów DNS OVHcloud. W OVHcloud grupy serwerów DNS są identyfikowane na podstawie numeru zawartego w nazwach serwerów. Dwa serwery DNS OVHcloud należą do tej samej grupy, jeśli mają ten sam numer.

<Region zones={['eu']}>

Na przykład *dns19.ovh.net* i *ns19.ovh.net* odpowiadają jednej grupie serwerów DNS OVHcloud, są ze sobą powiązane i zsynchronizowane. Należą do tej samej grupy, ponieważ mają ten sam numer.

</Region>

<Region zones={['ca', 'apac']}>

Na przykład *dnsXX.ovh.ca* i *nsXX.ovh.ca* odpowiadają jednej grupie serwerów DNS OVHcloud, są ze sobą powiązane i zsynchronizowane (na przykład *dns10.ovh.ca* i *ns10.ovh.ca*). Należą do tej samej grupy, ponieważ mają ten sam numer. Dokładne serwery przypisane do Twojej nazwy domeny są wskazane w Panelu klienta OVHcloud.
...
Na przykład: chcesz zadeklarować serwery DNS *ns1.dns-server.tld*, *dnsXX.ovh.ca* i *nsXX.ovh.ca* dla Twojej nazwy domeny. Sprawdź, czy w 3 strefach DNS hostowanych na tych 3 serwerach DNS widoczne są następujące trzy rekordy typu NS:
...
- *Rejestr* zarządzający rozszerzeniem Twojej nazwy domeny (np. rejestr rozszerzeń *.fr*) musi zostać poinformowany o zmianie DNS wprowadzonej po stronie OVHcloud. Postęp operacji można śledzić na stronie <ManagerLink to="/#/web/ongoing-operations">Operacje w toku</ManagerLink>.
