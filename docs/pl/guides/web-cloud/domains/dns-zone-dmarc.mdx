...
Rekord **D**omain-based **M**essage **A**uthentication, **R**eporting, and **C**onformance (DMARC) to mechanizm bezpieczeństwa poczty elektronicznej. Opiera się on na wynikach weryfikacji [SPF](/guides/web-cloud/domains/dns-zone-spf) i [DKIM](/guides/web-cloud/domains/dns-zone-dkim).
...
- Jeden z mechanizmów uwierzytelniania, [SPF](/guides/web-cloud/domains/dns-zone-spf) i/lub [DKIM](/guides/web-cloud/domains/dns-zone-dkim) musi być skonfigurowany w strefie DNS nazwy domeny usługi e-mail.
...
DMARC pozwala abonentowi nazwy domeny na zarządzanie bezpieczeństwem e-maili wysyłanych przy użyciu jego nazwy domeny. Jego celem jest:
...
- Pozwala lepiej kontrolować korzystanie z nazwy domeny i wykrywać próby sfałszowania przy użyciu wysyłanych zgłoszeń w przypadku niepowodzenia uwierzytelnienia wiadomości e-mail. Dodatkowo zwiększa również bezpieczeństwo, tworząc połączenie między protokołami SPF i DKIM.

Rekord DMARC zawiera informacje o zasadach dotyczących złośliwych wiadomości e-mail próbujących sfałszować nazwę Twojej nazwy domeny.<br/>
DMARC wysyła zapytania do mechanizmów uwierzytelniania [SPF](/guides/web-cloud/domains/dns-zone-spf) i [DKIM](/guides/web-cloud/domains/dns-zone-dkim), aby zweryfikować przychodzące e-maile.<br/>
...
Gdy adres **contact@mydomain.ovh** wyśle wiadomość na adres docelowy **recipient@otherdomain.ovh**, serwer poczty przychodzącej odpytuje strefę DNS nazwy domeny nadawcy **mydomain.ovh**, aby zapoznać się z instrukcjami rekordu DMARC.

Rekord DMARC informuje o polityce, jaką należy przyjąć w zależności od wyników testów SPF i DKIM. Może również podać jeden lub kilka adresów e-mail (w naszym przykładzie jako adres **report@mydomain.ovh**) używanych do otrzymywania raportów o błędach e-maili wysłanych z nazwy domeny **mydomain.ovh**.

Po przeczytaniu instrukcji rejestracji DMARC nazwy domeny **mydomain.ovh** przez serwer poczty przychodzącej "**otherdomain.ovh"** wiadomości e-mail będą dostarczane na adres **recipient@otherdomain.ovh** albo oznaczone jako "SPAM" lub odrzucone.
...
Rekord DMARC możesz dodać do strefy DNS w Panelu klienta OVHcloud. W tym celu zaloguj się do <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink> i przejdź do sekcji <code className="action">Web Cloud</code>. W lewej kolumnie wybierz w sekcji <code className="action">Nazwy domen</code> odpowiednią nazwę domeny, następnie kliknij na zakładkę <code className="action">Strefa DNS</code>, aby przejść do strefy DNS.

Po wyświetleniu strefy DNS kliknij przycisk <code className="action">Dodaj rekord</code>, a następnie "Pola e-mail" <code className="action">DMARC</code>.

- **Subdomena**: ten wpis musi **musi zaczynać się od** `_dmarc`. Jeśli stosujesz DMARC do całej nazwy domeny, nie wpisuj w tym polu niczego innego niż `_dmarc`. Jeśli ustawisz DMARC na subdomenę swojej nazwy domeny głównej, dodaj subdomenę po `_dmarc`. Na przykład, jeśli chcesz zastosować DMARC do subdomeny *subdomain.mydomain.ovh*, musisz wpisać `_dmarc.subdomain` w polu „subdomena” dla nazwy domeny *mydomain.ovh*.
...
- **Reguła dla nazwy domeny (p=)**: zasady do przyjęcia przez adresata na wniosek abonenta nazwy domeny nadawcy. Polityka dotyczy badanej nazwy domeny i subdomen, chyba że znacznik subdomeny **sp=** wskazuje na inne instrukcje. Możliwe wartości to:
    - *none*: Abonent nazwy domeny nie żąda żadnych konkretnych działań dotyczących dostarczania wiadomości.
...
:::warning
Konfiguracja parametru `p=` może mieć duży wpływ na dostarczalność e-maili Twojej nazwy domeny. Zalecamy skonfigurowanie `p=none` i przeprowadzenie przez kilka tygodni analizy raportów o niepowodzeniach, aby usunąć ewentualne anomalie. Zmiana `p=quarantine` lub `p=reject` wymaga pełnej kontroli nad ustawieniami bezpieczeństwa e-mail dla [SPF](/guides/web-cloud/domains/dns-zone-spf) i [DKIM](/guides/web-cloud/domains/dns-zone-dkim). Zastosowanie czynnika `pct=`, przedstawionego poniżej, umożliwia stopniowe przechodzenie.

:::


- **Procent filtrowanych wiadomości (pct=)** (wartość z zakresu od 0 do 100, wartość domyślna to 100): Procent przepływu wiadomości, do którego ma być stosowana polityka DMARC. Celem znacznika "pct" jest umożliwienie abonentom nazw domen powolnego wdrażania mechanizmu DMARC.
...
- **Reguła dla subdomen (sp=)**: Polityka dla wszystkich subdomen, jaką ma przyjąć odbiorca. Dotyczy to tylko subdomen nazwy domeny, do której jest wysyłany zapytanie, a nie samej nazwy domeny. Jego składnia jest taka sama jak w tagu "p" zdefiniowanym powyżej. Jeśli ten znacznik nie istnieje, dla subdomen stosowana jest polityka określona przez znacznik "p".
...
- **Wyrównanie**: gdy adres *john.smith@mydomain.ovh* przekazuje wiadomość z usługi e-mail powiązanej z nazwą domeny *mydomain.ovh* i gdy mechanizmy uwierzytelniania SPF i DKIM zostały skonfigurowane, otrzymujemy wyrównany wynik.
- **Częściowo wyrównany**: gdy adres *john.smith@subdomain.mydomain.ovh* przekazuje wiadomość z usługi e-mail powiązanej z nazwą domeny *mydomain.ovh*, ale mechanizmy uwierzytelniania SPF i DKIM zostały skonfigurowane tylko dla nazwy domeny głównej (czyli *mydomain.ovh*), otrzymujemy częściowo wyrównany wynik.
...
Możesz dodać rekord TXT do strefy DNS w <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink> i przejść do sekcji <code className="action">Web Cloud</code>. Kliknij <code className="action">Domeny</code>, następnie nazwę wybranej nazwy domeny. Teraz przejdź do zakładki <code className="action">Strefa DNS</code>.

Po wyświetleniu strefy DNS kliknij przycisk <code className="action">Dodaj rekord</code>, a następnie "Rozszerzone pola" <code className="action">TXT</code>.
...
- **ri** (32-bitowa liczba całkowita niepodpisana jako zwykły tekst; domyślnie 86400): wymagany interwał w sekundach między raportami zagregowanymi. Ten znacznik określa częstotliwość, z jaką odbiorcy wiadomości e-mail mają generować zbiorcze raporty dotyczące wyników oceny DMARC dla danej nazwy domeny.
...
Konfiguracja parametru `p=` może mieć duży wpływ na dostarczalność e-maili Twojej nazwy domeny. Zalecamy skonfigurowanie `p=none` i przeprowadzenie przez kilka tygodni analizy raportów o niepowodzeniach, aby naprawić ewentualne anomalie. Zmiana na `p=quarantine` lub `p=reject` wymaga pełnej kontroli nad ustawieniami bezpieczeństwa e-mail dla [SPF](/guides/web-cloud/domains/dns-zone-spf) i [DKIM](/guides/web-cloud/domains/dns-zone-dkim). Zastosowanie czynnika `pct=`, przedstawionego poniżej, umożliwia stopniowe przechodzenie.
...
- **pct=100**: Polityka DMARC dotyczy 50% wiadomości wysyłanych z kanałów e-mail abonenta nazwy domeny.
