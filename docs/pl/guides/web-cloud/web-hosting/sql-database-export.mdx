...
description: Dowiedz się, jak pobrać kopię zapasową bazy danych Twojego hostingu WWW OVHcloud
...
- W zależności od metody tworzenia kopii zapasowej, jakiej użyjesz, posiadanie dostępu do interfejsu zarządzania usługą hostingu WWW w <ManagerLink to="/">Panelu klienta</ManagerLink> lub posiadanie informacji umożliwiających zalogowanie do bazy danych
...
- **Użycie narzędzia OVHcloud do tworzenia kopii zapasowych** : rozwiązanie to umożliwia pobranie kopii zapasowych baz danych z poziomu <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>. Metoda ta nie wymaga specjalnych umiejętności technicznych.
...
- **Wykonanie kopii zapasowej przy użyciu komendy SSH** : metoda ta wymaga zalogowania się do przestrzeni dyskowej FTP przez protokół SSH, a następnie zastosowania odpowiednich komend pozwalających połączyć się z przestrzenią dyskową FTP. W przypadku tego dostępu konieczne są bardziej zaawansowane umiejętności techniczne oraz posiadanie [hostingu OVHcloud](https://www.ovhcloud.com/pl/web-hosting/).

:::tip
Jeśli tworzysz kopię zapasową bazy danych, ponieważ jest ona zapełniona/pełna, zapoznaj się z tutorialem "[Co zrobić, gdy baza danych jest zapełniona?](/guides/web-cloud/web-hosting/sql-overquota-database)".
...
W zakładce <code className="action">Bazy danych</code> kliknij przycisk <code className="action">...</code> po prawej stronie bazy danych, którą chcesz skopiować, a następnie kliknij <code className="action">Utwórz kopię zapasową</code>.
...
W oknie, które się pojawi wybierz datę utworzenia kopii zapasowej, następnie kliknij przycisk <code className="action">Dalej</code>. Upewnij się, że informacje w podsumowaniu są poprawne, następnie kliknij <code className="action">Zatwierdź</code>, aby rozpocząć operację.
...
W zakładce <code className="action">Bazy danych</code> kliknij przycisk <code className="action">...</code> po prawej stronie bazy danych, którą chcesz skopiować, a następnie kliknij <code className="action">Przywróć kopię zapasową</code>.
...
Aby pobrać kopię zapasową, kliknij przycisk <code className="action">...</code> po prawej stronie kopii, którą chcesz pobrać, a następnie kliknij <code className="action">Pobierz kopię zapasową</code>. Pojawi się okno z prośbą, abyś zapisał ją na Twoim komputerze. Zaakceptuj, po czym odczekaj, aż kopia zapasowa zostanie pobrana.
...
Aby przeprowadzić operację, zaloguj się do *phpMyAdmin*. Aby uzyskać link dostępowy do phpMyAdmin, zaloguj się do <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink> i przejdź do sekcji <code className="action">Web Cloud</code>. W lewej kolumnie kliknij <code className="action">Hosting</code>, następnie wybierz odpowiednią nazwę hostingu. Teraz przejdź do zakładki <code className="action">Bazy danych</code>.

Tabela, która się wyświetla zawiera listę wszystkich baz danych utworzonych w ramach Twojego pakietu hostingowego. Kliknij przycisk <code className="action">...</code> po prawej stronie odpowiedniej bazy danych, a następnie <code className="action">Dostęp do phpMyAdmin</code>.
...
Po zalogowaniu się do *phpMyAdmin* wprowadź informacje dotyczące bazy danych, następnie zaloguj się. Po zalogowaniu przejdź do zakładki <code className="action">Eksportuj</code>, gdzie dostępne są dwie metody eksportu:
...
Po poprawnym utworzeniu skryptu kopii zapasowej pobierz go na przestrzeń dyskową FTP Twojego hostingu. W tym celu zapoznaj się z informacjami zawartymi w opisie etapu 2 dokumentacji "[Logowanie do przestrzeni dyskowej](/guides/web-cloud/web-hosting/hosting-how-to-get-my-website-online)".
...
[Tutorial - Co zrobić, gdy baza danych jest zapełniona?](/guides/web-cloud/web-hosting/sql-overquota-database)
