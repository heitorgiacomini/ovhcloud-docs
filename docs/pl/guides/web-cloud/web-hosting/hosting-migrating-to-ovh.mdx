...
description: Dowiedz się, jak migrować stronę WWW, nazwę domeny, bazę danych oraz konta e-mail do OVHcloud bez przerwy w dostępności usług
...
Kilka [ofert hostingu OVHcloud](https://www.ovhcloud.com/pl/web-hosting/) zawiera ofertę e-mail "[MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities)". Ta oferta e-mail pozwala na tworzenie kont e-mail z przestrzenią dyskową do 5 GB dla każdego konta. Wybierz spośród poniższych pakietów hostingowych w zależności od wersji PHP, wersji SQL, liczby kont e-mail, których potrzebujesz oraz rozmiaru Twojej strony, którą chcesz przenieść:

Seria **Eco**:

- **Starter** — 2 adresy e-mail "MX Plan"
- **Perso** — 10 adresów e-mail "MX Plan"

Seria **Business**:

- **Startup** — 10 adresów e-mail "MX Plan"
- **Pro** — 100 adresów e-mail "MX Plan"
- **Performance** — 1000 adresów e-mail "MX Plan"

Seria **Agencies**:

- **Agency**, **Agency Plus** i **Agency Max** — po 1000 adresów e-mail "MX Plan" każda

Po wybraniu oferty hostingu, jeśli nie jesteś jeszcze klientem OVHcloud, kliknij przycisk <code className="action">Zamów</code> na stronie handlowej wybranej oferty. Postępuj zgodnie z kolejnymi poleceniami **bez konieczności przenoszenia domeny** (Operacja ta zostanie wykonana na etapie 10 niniejszego przewodnika).
...
<Tabs>
  <Tab label="Krok 1">
    Przejdź do strony <ManagerLink to="/#/web/hosting">Hosting</ManagerLink>.
  ...
  <Tab label="Krok 2">
    W lewym górnym rogu interfejsu kliknij przycisk <code className="action">Zamów</code>, a następnie <code className="action">Hosting</code>.
  </Tab>
  <Tab label="Krok 3">
...
<Region zones={['ca']}>
:::info
OVHcloud poza ofertą "MX Plan" proponuje również inne usługi e-mail. Możesz na przykład połączyć konta e-mail "MX Plan" z kontami ["Exchange"](https://www.ovhcloud.com/pl/emails/hosted-exchange/).
...
Po utworzeniu hostingu zaloguj się do <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>, następnie utwórz strefę DNS dla Twojej domeny **bez wpisywania jej na listę "www"**. Zapoznaj się z naszym przewodnikiem [dotyczącym tworzenia strefy DNS w OVHcloud](/guides/web-cloud/domains/dns-zone-create).

Po utworzeniu strefy DNS przejdź do interfejsu zarządzania strefą DNS "[Edycja strefy DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
...
Aby dowiedzieć się, jaki adres IP jest odpowiedni dla Twojego hostingu OVHcloud, zapoznaj się z naszym przewodnikiem zawierającym [adresy IP różnych klastrów hostingu współdzielonego](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip).
...
<Region zones={['ca', 'apac']}>
:::tip
Zanotuj dwie wartości docelowe z typem rekordu "NS". Wartości te, typu `dnsXX.ovh.ca` i `nsXX.ovh.ca` (lub `dns200.anycast.me` i `ns200.anycast.me`), odpowiadają serwerom DNS przypisanym do tej strefy DNS dla Twojej domeny. Zostaną one użyte w [etapie 9](#step9) niniejszego przewodnika.
...
Aby zaimportować kopię zapasową przestrzeni dyskowej FTP poprzedniego dostawcy, [zaloguj się do przestrzeni FTP Twojego hostingu OVHcloud](/guides/web-cloud/web-hosting/ftp-connection) i przesłać kopię zapasową do katalogu głównego "www" (lub innego katalogu głównego, który wcześniej utworzyłeś).

Zalecamy użycie programu [FileZilla](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide) do pobrania kopii zapasowej FTP z Twojego hostingu.
...
Do tworzenia kopii zapasowej bazy danych [utwórz nową bazę danych](/guides/web-cloud/web-hosting/sql-create-database), a następnie [zaimportuj kopię zapasową do nowej bazy danych](/guides/web-cloud/web-hosting/sql-importing-mysql-database).

:::info
OVHcloud oferuje serwery baz danych Web Cloud Databases. Jeśli chcesz korzystać z tej oferty na swojej stronie internetowej, zapoznaj się z naszą dokumentacją dotyczącą tego produktu [na naszej stronie](/guides/web-cloud/databases/overview).
...
:::tip
Aby połączyć nową bazę danych, jeśli korzystasz z Content Management System (CMS), takiego jak WordPress, Joomla!, Drupal lub PrestaShop, znajdziesz informacje dotyczące plików konfiguracyjnych w pliku konfiguracyjnym w **etap 4** w przewodniku ["zmiana hasła do bazy danych"](/guides/web-cloud/web-hosting/sql-change-password).

:::


Deklaruj/autoryzuj swoje zewnętrzne nazwy domeny na swoim OVHcloud Hosting Web za pomocą naszego przewodnika "[konfiguracja wielu witryn na OVHcloud Hosting Web](/guides/web-cloud/web-hosting/multisites-configure-multisite)". Proszę podać nazwę katalogu głównego wybranego na początku [etap 4](#step4). Przypominamy, że jest to folder, w którym umieściłeś pliki na przestrzeni FTP.
...
Skorzystaj z naszego przewodnika dotyczącego [tworzenia kont e-mail "MX Plan"](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation) i przypisz do konta e-mail hostowane u Twojego dostawcy.

<Region zones={['eu']}>
Jeśli wybrałeś rozwiązanie "Email Pro" lub "Exchange", zapoznaj się z naszą dokumentacją dotyczącą tworzenia kont e-mail:

- Dla [Email Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/first-config)
- W przypadku [Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-starting-hosted)
</Region>

<Region zones={['ca']}>
Jeśli wybrałeś rozwiązanie "Exchange", zapoznaj się z naszą dokumentacją dotyczącą tworzenia kont e-mail:
...
<Region zones={['ca', 'apac']}>
- Twoja domena (bez "www") do celu typu "MX": "mx1.mail.ovh.ca. ".
- Twoja domena (bez "www") do celu typu "MX": "mx2.mail.ovh.ca. ".
- Twoja domena (bez "www") do celu typu "MX": "mx3.mail.ovh.ca. ".
</Region>
...
**Wariant 1**: użyj naszego narzędzia [OVHcloud Mail Migrator (OMM)](https://omm.ovhcloud.com/), które pozwala na skopiowanie zawartości kont e-mail zarejestrowanych u dotychczasowego operatora do kont stworzonych w OVHcloud. Zapoznaj się z naszym przewodnikiem "[Przeniesienie kont e-mail poprzez OVHcloud Mail Migrator](/guides/web-cloud/email-and-collaborative-solutions/migrating/omm-migrate-email-account-to-ovhcloud)".

Zalecamy nie używać <code className="action">Typ serwera</code> **POP** w części <code className="action">Konto źródłowe</code>. Protokół usuwa e-maile ze starego serwera i wysyła je do docelowego serwera OVHcloud. Nie będziesz już mógł porównywać zawartości starego i nowego adresu e-mail.

W części <code className="action">Konto docelowe</code> wprowadź tylko adres e-mail OVHcloud i przypisane do niego hasło. Pozostawiając <code className="action">Typ serwera</code> w <code className="action">Hosted by OVH (Autodetect)</code>.
...
- Wszystkie parametry konfiguracji znajdziesz w naszym przewodniku "[Informacje ogólne na temat e-maili MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities#consult-emails-client)". Znajdziesz tam również linki do przewodników dotyczących konfiguracji dla głównych programów pocztowych.

<Region zones={['eu']}>
#### Dla kont "Email Pro":

- Wszystkie nasze przewodniki dotyczące konfiguracji znajdują się w sekcjach `Konfiguracja na komputerze` i `Konfiguracja na smartphonie` [dokumentacja dotycząca oferty Email Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/overview).
</Region>

<Region zones={['eu', 'ca']}>
#### Dla kont e-mail "Exchange":

- Wszystkie nasze przewodniki dotyczące konfiguracji znajdują się w sekcjach `Konfiguracja Exchange na komputerze` oraz `Konfiguracja Exchange na smartphonie` [dokumentacja dotycząca oferty Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/overview).
</Region>
...
<Region zones={['ca', 'apac']}>
Zastąp aktualne serwery DNS (operatora) dwoma serwerami DNS zadeklarowanymi w strefie DNS OVHcloud (typu `dnsXX.ovh.ca` i `nsXX.ovh.ca` lub `dns200.anycast.me` i `ns200.anycast.me`). Operacja ta jest wykonywana w interfejsie zarządzania rejestratora.
</Region>
...
Przenieś domenę w naszym przewodniku dotyczącym [transferu domeny do OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain).
...
[Informacje ogólne na temat serwerów DNS](/guides/web-cloud/domains/dns-server-general-information).

[Tworzenie konta e-mail na hostingu](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).

[Import bazy danych MySQL](/guides/web-cloud/web-hosting/sql-importing-mysql-database)
