...
description: Dowiedz się, jak zabezpieczyć katalog lub dostęp do administrowania stroną WWW za pomocą uwierzytelnienia za pomocą plików .htaccess i .htpasswd
...
Używając dwóch plików konfiguracyjnych (HTTP) Apache do umieszczenia w [przestrzeni FTP](/guides/web-cloud/web-hosting/ftp-connection) Twojego hostingu www: 
...
- Posiadanie danych do logowania do [przestrzeni FTP Twojego hostingu](/guides/web-cloud/web-hosting/ftp-connection)
...
Jeśli korzystasz z CMS WordPress, OVHcloud udostępnia również tutorial, w jaki sposób [używać pliku htaccess przy użyciu WordPress](/guides/web-cloud/web-hosting/htaccess-how-to-protect-wordpress).
...
W tym celu skontaktuj się z naszą [społecznością użytkowników](https://community.ovhcloud.com/community/en) lub naszymi [partnerami OVHcloud](https://partner.ovhcloud.com/pl/directory/).
:::
...
Aby zmienić wersję PHP swoich stron www na Twoim hostingu, zapoznaj się z naszym przewodnikiem "[Hosting WWW : środowisko, wersja PHP, .ovhconfig](/guides/web-cloud/web-hosting/configure-your-web-hosting)".
...
Zaloguj się do [przestrzeni dyskowej FTP](/guides/web-cloud/web-hosting/ftp-connection) Twojego hostingu. Otwórz ["katalog główny"](/guides/web-cloud/web-hosting/multisites-configure-multisite), do którego wskazuje Twoja domena.
...
Przejdź do katalogu głównego, w którym utworzyłeś plik "crypt.php". Kliknij polecenie <code className="action">Edytuj</code> i umieść następujące wiersze:
...
Jeśli dysponujesz hostingiem **Pro**, **Performance**, **Agency**, **Agency Plus** lub **Agency Max**, zaloguj się do hostingu za pomocą [SSH](/guides/web-cloud/web-hosting/ssh-on-webhosting). Przejdź do "**katalog główny**", w którym znajduje się Twój skrypt "crypt.php".
...
:::warning
Ze względów bezpieczeństwa zaleca się korzystanie z SSH. Jeśli jednak dysponujesz usługą **Starter**, **Perso** lub **Startup**, dla której SSH jest niedostępny, możesz również uruchomić plik "crypt.php" za pomocą przeglądarki internetowej.
...
- `your_ftp_login`: login FTP, którego używasz do logowania się do przestrzeni dyskowej FTP. Aby pobrać login FTP, zapoznaj się z naszym przewodnikiem dotyczącym [logowania do przestrzeni FTP](/guides/web-cloud/web-hosting/ftp-connection).
...
[Logowanie do przestrzeni FTP hostingu WWW](/guides/web-cloud/web-hosting/ftp-connection)

[Przewodnik - Operacje możliwe do wykonania za pomocą pliku ".htaccess"](/guides/web-cloud/web-hosting/htaccess-what-else-can-you-do)

[Zablokuj dostęp do mojej strony dla niektórych adresów IP przez plik .htaccess](/guides/web-cloud/web-hosting/htaccess-how-to-block-a-specific-ip-address-from-accessing-your-website)

[Prześlij link dostępowy do mojej strony za pomocą mod_rewrite za pomocą pliku .htaccess](/guides/web-cloud/web-hosting/htaccess-url-rewriting-using-mod-rewrite)
