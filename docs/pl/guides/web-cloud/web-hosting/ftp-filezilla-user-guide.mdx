...
**SFTP** używa domyślnego portu 22 zamiast portu 21, podobnie jak SSH. Jeśli korzystasz z hostingu Cloud Web, użyj portu, który wyświetla się w Twoim <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>. Port 22 jest wyłączony przez bezpieczeństwo w protokole SSH i SFTP dla hostingu Cloud Web.
...
W tym celu wróć do zakładki <code className="action">FTP-SSH</code> w <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>, jak pokazano w [pierwszej części](#part-1) tego przewodnika.
...
Sprawdź dane logowania, aby upewnić się, że nie wystąpiły błędy. [Zmiana hasła dostępu FTP](/guides/web-cloud/web-hosting/ftp-change-password) Twojego hostingu WWW bezpośrednio w <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>.
...
Upewnij się, czy jest ona powiązana z nazwą hosta zadeklarowaną w <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink>.
