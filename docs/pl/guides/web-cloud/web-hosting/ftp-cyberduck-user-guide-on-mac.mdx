...
description: Dowiedz się, jak korzystać z aplikacji Cyberduck, aby zalogować się do hostingu OVHcloud
...
Oddajemy w Twoje ręce niniejszy przewodnik, którego celem jest pomoc w jak najbardziej optymalnym wykonywaniu bieżących zadań. Niemniej jednak, w przypadku trudności zalecamy skontaktowanie się z [wyspecjalizowanym dostawcą](https://partner.ovhcloud.com/pl/directory/) lub skontaktowanie się z wydawcą usługi. Niestety firma OVHcloud nie jest w stanie udzielić Ci wsparcia w tym zakresie. Więcej informacji zawiera sekcja [Sprawdź również](#go-further) w tym przewodniku.
:::


## Wymagania początkowe

- Posiadanie [hostingu web](https://www.ovhcloud.com/pl/web-hosting/).
...
- Poniżej znajduje się zawartość, którą chcesz wyświetlić. Na przykład po kliknięciu ikony <code className="action">History</code> pojawi się lista działań.
...
W menu głównym na górze interfejsu kliknij <code className="action">View</code>, a następnie <code className="action">Customize Toolbar...</code>.
...
W oknie, które się pojawi, przeciągnij żądane elementy na pasek narzędzi. Na przykład, jeśli chcesz dodać ikonę <code className="action">Download</code> do paska narzędzi, przeciągnij i upuść ikonę <code className="action">Download</code> na pasek narzędzi. Aby zatwierdzić zmiany, kliknij <code className="action">Done</code>.
...
**1.** Na pasku narzędzi kliknij <code className="action">Open Connection</code>

**2.** Z rozwijanego menu (pomarańczowa ramka obrazu) wybierz opcję <code className="action">SFTP (SSH File Transfert Protocol)</code>
...
:::tip[Success]
- Możesz zarejestrować hasło w Cyberduck zaznaczając opcję <code className="action">Add to keychain</code>. Jeśli nie zaznaczysz tego pola, wprowadź hasło, aby zalogować się ponownie do Twojej przestrzeni hostingowej.
- Jeśli nie znasz wszystkich swoich danych FTP (serwer, dane do logowania, etc.), zapoznaj się z przewodnikiem "[Logowanie do przestrzeni dyskowej FTP Twojego hostingu WWW](/guides/web-cloud/web-hosting/ftp-connection)".
:::


Po pierwszym zalogowaniu do przestrzeni hostingowej wyświetli się okno z tytułem <code className="action">Modified fingerprint</code>. Zaznacz kratkę <code className="action">Always</code> i zatwierdź. Pozwoli to na definitywną certyfikację hosta połączenia (OVHcloud).
...
:::tip[Success]
- Jeśli nie znasz wszystkich swoich danych FTP (serwer, login, etc.), zapoznaj się z przewodnikiem "[Logowanie do przestrzeni dyskowej FTP Twojego hostingu WWW](/guides/web-cloud/web-hosting/ftp-connection)".
:::
...
- W razie potrzeby skorzystaj z przewodnika "[Logowanie do przestrzeni dyskowej FTP hostingu WWW](/guides/web-cloud/web-hosting/ftp-connection)".
:::
...
2. W górnej części interfejsu, na pasku narzędzi, kliknij zakładkę <code className="action">Bookmarks</code> (pomarańczowa ramka na poniższym obrazie).
3. W lewym dolnym rogu kliknij ikonę<code className="action">+</code>, aby dodać nową zakładkę.
...
W menu Cyberduck kliknij <code className="action">File</code>, a następnie <code className="action">Upload...</code>. Wybierz pliki, które chcesz przesłać na serwer i kliknij na <code className="action">Upload</code>.
...
- klikając na <code className="action">Window</code> (w menu głównym), a następnie <code className="action">Transfers</code>.
...
Kliknij dwukrotnie plik lub folder, a następnie wybierz <code className="action">Info</code>. Pojawi się następujące okno:

<img className="thumbnail" alt="hosting" src="/images/assets/screens/other/web-tools/cyberduck/transfert-files-list.png" loading="lazy" />

Kliknij zakładkę <code className="action">Permissions</code> i wprowadź żądane zmiany:
...
W menu Cyberduck kliknij <code className="action">Go</code>, a następnie <code className="action">Send command...</code>.
...
W nowym oknie wpisz komendę `CHMOD 705 /` i kliknij na <code className="action">Send</code>, aby wykonać komendę. W potwierdzeniu w poniższej ramce powinien pojawić się komunikat `200 Permissions changed on /`.
