...
<Tabs>
  <Tab label="Krok 1">
    **Tworzenie projektu migracji**
    ...
    Przejdź na stronę [OVHcloud Mail Migrator](/links/web/omm) i kliknij <code className="action">Nowa migracja</code>.
    ...
    Kliknij <code className="action">Utwórz mój projekt</code>. Otrzymasz e-mail z potwierdzeniem zawierający unikalny identyfikator projektu.
  </Tab>
  <Tab label="Krok 2">
    **Logowanie do projektu i tworzenie migracji**

    Na stronie głównej [OMM](/links/web/omm) kliknij <code className="action">Śledzić migrację</code>, wpisz `Identyfikator projektu` i `Hasło projektu`, a następnie kliknij <code className="action">Zaloguj się do projektu</code>.

    Następnie kliknij <code className="action">Nowa migracja</code>, aby skonfigurować migrację:
    ...
    - **Konto źródłowe**:
        - **Typ konta**: Wybierz `OVHcloud`, a następnie `MX Plan` lub `Autodetekcja`. Kliknij <code className="action">Zaloguj się</code>, aby zalogować się na swoje konto OVHcloud i automatycznie wybrać usługę oraz adres do migracji (np.: `john.smith@mydomain.ovh`). Następnie wpisz hasło do tego konta e-mail.
    - **Konto docelowe**:
        - **Typ konta**: Wybierz `OVHcloud`, a następnie `Zimbra`. Kliknij <code className="action">Zaloguj się</code>, aby zalogować się na swoje konto OVHcloud i wybrać usługę Zimbra oraz adres docelowy (np.: `zimbra2@mydomain.ovh`). Następnie wpisz hasło do tego konta e-mail.
    ...
    Kliknij <code className="action">Migruj moje konto</code>, aby rozpocząć migrację.
  ...
  <Tab label="Krok 3">
    **Śledzenie migracji**
    ...
    - Ze strony głównej [OMM](/links/web/omm): kliknij <code className="action">Śledzić migrację</code>, wpisz `Identyfikator projektu` i `Hasło projektu`, a następnie kliknij <code className="action">Zaloguj się do projektu</code>.

    Na stronie projektu kliknij przycisk <code className="action">⋮</code> po prawej stronie wiersza migracji, aby wyświetlić opcje:

    - <code className="action">Wyświetl więcej szczegółów</code>: Śledź postępy migracji i przeglądaj raport po jej zakończeniu.
    - <code className="action">Anuluj migrację</code>: Anuluje trwającą migrację. Elementy już zmigrowane są zachowane na koncie docelowym.
    - <code className="action">Usuń moje dane migracji (RODO)</code>: Powoduje usunięcie wszystkich danych związanych z migracją. Informacje o zdarzeniach migracji są zachowywane.
