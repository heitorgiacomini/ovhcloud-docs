...
description: "Dowiedz się podstaw Public Cloud Compute: jak działają instancje, dostępne rodziny i rozmiary, wdrożenia wielozonowe, zarządzanie obrazami, bezpieczeństwo SSH, mechanizmy kopii zapasowych, sieci publiczne/prywatne oraz korzyści z Savings Plans."
...
Instancje OVHcloud Compute są wdrażane w [wielu centrach danych na całym świecie](https://www.ovhcloud.com/pl/about-us/global-infrastructure/regions/), zapewniając wysoką dostępność i bliskość użytkowników. Przykłady regionów obejmują:
...
:::info
**Najlepsze praktyki**: Dla krytycznych obciążeń zaleca się wdrożenie [wielostrefowe](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture), aby zapewnić odporność usługi i ciągłość działania.
:::
...
- **[Własne obrazy](/guides/public-cloud/compute/upload-own-image)**: Możesz zaimportować własne obrazy w formacie QCOW2 lub RAW, zapewniając pełną kontrolę nad swoim środowiskiem i umożliwiając migracje, standardowe szablony lub specjalistyczne konfiguracje.

**Cykl życia i wsparcie**: OVHcloud regularnie aktualizuje katalog obrazów. Zawsze sprawdzaj ogłoszenia dotyczące cyklu życia i końca wsparcia, aby upewnić się, że Twoje obrazy są bezpieczne i wspierane. Zobacz [tutaj](/guides/public-cloud/compute/image-life-cycle).
...
Aby uzyskać szczegółowe instrukcje dotyczące tworzenia i używania kluczy SSH, odwiedź [przewodnik SSH OVHcloud](/guides/public-cloud/compute/creating-ssh-keys).
...
:::info
**Najlepsze praktyki**: Kopia zapasowa nie zastępuje [odporności architektury](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture). Dla krytycznych środowisk połącz kopie zapasowe z replikacją wielostrefową, aby zapewnić maksymalne ochronę danych i dostępność usługi.
:::
...
Aby uzyskać bardziej szczegółowe wskazówki dotyczące konfigurowania sieci Public Cloud, odwiedź oficjalny [przewodnik OVHcloud dotyczący sieci](/guides/public-cloud/network-services/concepts).
...
Savings Plans pomagają Ci zoptymalizować budżet, jednocześnie zachowując wydajność i niezawodność Twojego środowiska chmurowego. Aby uzyskać więcej szczegółów, odwiedź oficjalny [przewodnik OVHcloud dotyczący Savings Plans](/guides/public-cloud/cross-functional/savings-plans).
...
- [Jak utworzyć instancję Public Cloud i połączyć się z nią](/guides/public-cloud/compute/getting-started)
- [Zarządzanie instancjami Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance)
- [Uruchamianie instancji na wolumenie startowym](/guides/public-cloud/compute/start-instance-on-attached-volume)
- [Wstrzymywanie lub zawieszanie instancji](/guides/public-cloud/compute/suspend-or-pause-an-instance)
