...
<details>
<summary>Jak długo ważny jest voucher OVHcloud?</summary>


Vouchery OVHcloud są zazwyczaj ważne przez miesiąc. Należy z nich korzystać jak najszybciej, aby w pełni je wykorzystać. Saldo vouchera jest używane przed zasileniem Public Cloud.
...
Zapoznaj się z przewodnikiem [Zmiana klucza SSH w przypadku utraty](/guides/public-cloud/compute/replacing-lost-ssh-key-pair).
...
**[Opcjonalnie od września 2023]** Użytkownik [OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user) może zostać utworzony do korzystania z interfejsów API Horizon lub OpenStack. Jest to opcjonalne, ponieważ wdrożyliśmy logowanie jednokrotne (SSO: *Single Sign-On*) między Panelem klienta OVHcloud a Horizon.
...
Zapraszamy do zapoznania się z przewodnikiem [Wykupienie adresu Additional IP](/guides/public-cloud/network-services/buy-additional-ip).
...
Zapraszamy do zapoznania się z przewodnikiem [Konfiguracja usługi vRack Public Cloud](/guides/public-cloud/network-services/vrack).
...
Zapraszamy do zapoznania się z przewodnikiem [zarządzanie regułami firewalla i portu security w sieciach wykorzystujących OpenStack CLI](/guides/public-cloud/cross-functional/security-group-private-network).
...
Jeśli e-maile nie są wysyłane lub serwer SMTP nie odpowiada, sprawdź, czy porty SMTP nie są zablokowane. Port 25 jest domyślnie zablokowany na instancjach Public Cloud OVHcloud, aby zapobiec nadużyciom. Zamiast tego użyj portu 587 (STARTTLS) do wysyłki e-maili wychodzących lub, w razie potrzeby, poproś o odblokowanie portu 25, [kontaktując się z naszym zespołem wsparcia](https://help.ovhcloud.com/csm?id=csm_get_help).
