...
description: Erfahren Sie hier, wie Sie das IPv6-Protokoll auf einer Public Cloud-Instanz konfigurieren
...
:::info
Floating IP und Gateway sind derzeit nicht mit IPv6 kompatibel. Sie können IPv6 nur mit Instanzen im [Public Mode](/guides/public-cloud/network-services/concepts#publicmode) verwenden.
...
Klicken Sie in Ihrem Public Cloud Projekt auf <code className="action">Instanzen</code> im linken Menü. Klicken Sie dann auf <code className="action">...</code> und <code className="action">Instanz-Details</code> neben der gewünschten Instanz.
...
<Tabs>
  <Tab label="Debian (außer Debian 12)">
  ...
  <Tab label="Windows">
    ...
    Gehen Sie in den Bereich <code className="action">Netzwerkverbindungen</code> Ihrer Windows-Oberfläche.
...
Klicken Sie dann mit der rechten Maustaste auf Ihre Netzwerkkarte, um auf <code className="action">Eigenschaften</code> zuzugreifen.
...
Klicken Sie dann auf <code className="action">Internet Protocol Version 6 (TCP/IPv6)</code> und dann auf <code className="action">Eigenschaften</code>.
...
Wenn Sie damit fertig sind, setzen Sie einen Haken in dem Feld `Einstellungen ausgehend überprüfen` und klicken Sie auf <code className="action">OK</code>, um Ihre Änderungen zu bestätigen.
  </Tab>
</Tabs>
...
Zunächst [stellen Sie Ihre Instanz auf den Rescuemodus "rescue-pro" um](/guides/public-cloud/compute/put-an-instance-in-rescue-mode).
