...
- [Prepare the environment to use the OpenStack API](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) by installing python-swiftclient.
...
![Keystone](/images/guides/public-cloud/compute/create-server-group-horizon-cli/create_server_groups_openstack_dashboard.png)
...
1. Click on `Compute` in left tab, then on `Server Groups`.
1. Next, cick on `+ Create Server Group`.
...
1. Log in to the [Horizon interface](https://horizon.cloud.ovh.net/auth/login/).
1. Select the appropriate region from the drop down menu at the top left.
1. Click on `Compute` in left tab, then on `Instances`.
1. Click on `Launch Instance`.
1. In the pop-up window, fill in the required fields to create your instance. In the `Server Group` field, click on the drop-down arrow next to `Available` to display the list of available Server Groups. Select a Server Group and it will be moved to `Allocated`.
![create server with server group OpenStack](/images/guides/public-cloud/compute/create-server-group-horizon-cli/create_a_server_with_server_group_openStack_dashboard.png)
1. Once done, click on `Launch Instance`.
...
![List Server Groups OpenStack](/images/guides/public-cloud/compute/create-server-group-horizon-cli/list_server_groups_openStack_dashboard.png)
...
![Server group error OpenStack](/images/guides/public-cloud/compute/create-server-group-horizon-cli/server_group_instances_error_openstack_dashboard.png)
