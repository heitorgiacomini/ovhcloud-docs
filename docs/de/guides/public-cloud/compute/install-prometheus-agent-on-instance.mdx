...
description: Erfahren Sie, wie Sie den Prometheus Node Exporter- oder Windows Exporter-Agent auf einer OVHcloud Public Cloud Instanz installieren, um Metriken zu sammeln
...
- Sie verfügen über eine [Public Cloud Instanz](/guides/public-cloud/cross-functional/create-a-public-cloud-project).
- Sie haben [administrativen Zugriff auf die Instanz](/guides/public-cloud/compute/getting-started).
...
:::info
Auf Windows können Sie PowerShell mit SSH oder einen SSH-Client wie [PuTTY](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows) verwenden, wenn Sie eine Befehlszeile bevorzugen.
...
<Tabs>
  <Tab label="Für Debian/Ubuntu">
    ```bash
    sudo apt update && sudo apt upgrade -y
    ```
  </Tab>
  <Tab label="Für CentOS/RHEL">
    ```bash
    sudo yum update -y
    ```
  </Tab>
  <Tab label="Für Windows">
...
<Tabs>
  ...
  <Tab label="Für Windows (über SSH/PowerShell in der VM)">
    :::info
...
<Tabs>
  ...
  <Tab label="Für Windows (über SSH/PowerShell)">
    ```powershell
...
<Tabs>
  <Tab label="Für Linux (Debian/Ubuntu mit UFW)">
    ```bash
