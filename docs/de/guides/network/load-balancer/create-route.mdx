...
### API presentation <a id="apipresentation"></a>

Route management is only available through the [OVHcloud API](/links/api). It is only valid for the **http** and **tcp** protocols, and the path `/ipLoadbalancing/{serviceName}/{protocol}/route/` exposes the dedicated route API.
...
:::info
To display only the API calls related to routes in the OVHcloud API console, you can use the `filter` field with the keyword "[a-z]*".
:::
...
#### Handling routes <a id="handling-routes"></a>
...
#### Handling rules <a id="handling-rules"></a>
...
#### Available actions <a id="available-actions"></a>
...
#### Available rules <a id="available-rules"></a>
