...
![Result server health status via the API](/images/guides/network/load-balancer/retrieve-servers-state/result_servers_state_api.png)

*The image above illustrates the result of the command in the API.*

### From the OVHcloud Control Panel

In the `Server clusters` tab, after selecting one of them, the status of each of its servers is displayed on the corresponding line.

#### Result

![Result server health status via the OVHcloud Control Panel](/images/guides/network/load-balancer/retrieve-servers-state/farm_server_health.png)

To obtain details on a server's health status, click on the text in the "**Status**" column, or click the `...` button and select `See status`.

![Result server health status via the OVHcloud Control Panel (details)](/images/guides/network/load-balancer/retrieve-servers-state/server_health_detail.png)
