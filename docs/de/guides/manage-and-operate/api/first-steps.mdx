...
Klicken Sie auf der [OVHcloud API Seite](https://eu.api.ovh.com/) auf <code className="action">Explore the OVH API</code>, um die Liste der API-Funktionen anzuzeigen.
...
:::info
Wenn Ihr OVHcloud Account durch [Zwei-Faktor-Authentifizierung](/guides/account-and-service-management/account-information/secure-ovhcloud-account-with-2fa) geschützt ist, müssen Sie auch den per SMS, OTP-Anwendung oder U2F-Schlüssel generierten Code eingeben.
...
Sie haben auch einen Selektor auf der linken Seite der Produktliste, mit dem Sie zwischen den API-Zweigen **/v1** und **/v2** wählen können. Wenn Sie mit dem Prinzip der API-Verzweigung nicht vertraut sind, lesen Sie [unsere Dokumentation zur API v2](/guides/manage-and-operate/api/apiv2).
...
Wenn Sie die Parameter festgelegt haben, können Sie die Ausführung des Aufrufs starten, indem Sie auf <code className="action">EXECUTE</code> klicken. 
...
Wenn Sie auf <code className="action">Create Keys</code> klicken, erhalten Sie drei Schlüssel:
...
Der **CK**-Token kann für die Übertragung von Rechten verwendet werden. Weitere Informationen finden Sie in dieser Anleitung: [How to manage a customer’s account via OVHcloud API](/guides/manage-and-operate/api/first-steps) (EN).
...
Um Schlüssel aufzulisten und zu widerrufen, können Sie die [API-Konsole](https://eu.api.ovh.com/) oder das <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> verwenden:

<Tabs>
  <Tab label="API-Konsole">
...
[Domainnamen über die API verwalten](/guides/web-cloud/domains/api-domain-intro) (EN)
