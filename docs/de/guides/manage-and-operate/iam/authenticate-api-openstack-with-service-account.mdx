...
description: How to log in to OpenStack APIs or command lines with OVHcloud service accounts
...
To automate calls to protected APIs, you need to provide credentials to the code that handles them. With OVHcloud service accounts, it is possible to have a single ID per script for the use of the different APIs offered by OVHcloud products ([OVHcloud API](/guides/manage-and-operate/api/console-preview), [OpenStack API](/guides/public-cloud/compute/starting-with-nova), etc.).
...
- You know [how to use the OpenStack API](/guides/public-cloud/compute/starting-with-nova)
- You have created a [service account via API](/guides/manage-and-operate/api/manage-service-account).
