...
description: So ändern Sie die IP-Block-Ankündigung im vRack
...
Das [vRack](https://www.ovhcloud.com/de/network/vrack/) ist ein privates Netzwerk, mit dem Sie das Routing zwischen zwei oder mehr OVHcloud [Dedicated Servern](https://www.ovhcloud.com/de/bare-metal/) einrichten können.

**In dieser Anleitung erfahren Sie, wie Sie die IP-Block-Ankündigung im vRack ändern.**

## Voraussetzungen

- Sie haben ein [vRack](https://www.ovhcloud.com/de/network/vrack/) in Ihrem Kunden-Account.
- Sie haben [einen IP-Adressblock im vRack eingerichtet](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack).
...
Öffnen Sie die [API-Konsole](https://eu.api.ovh.com/) und loggen Sie sich mit Ihrer OVHcloud Kundenkennung ein. Verwenden Sie die nachstehenden API-Aufrufe, um die IP-Block-Ankündigung zu bearbeiten.

<Api version="v1" section="/vrack" method="GET" route={"/vrack"} />

So können Sie über die API die Liste der vRack Dienste aufrufen. Wenn Sie den betreffenden Dienst über diese Referenzen nicht identifizieren können, finden Sie seinen Namen in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink>. Gehen Sie hierzu in den Bereich <code className="action">Bare Metal Cloud</code>, dann in den Bereich <code className="action">Network</code> und dann "vRack". 
