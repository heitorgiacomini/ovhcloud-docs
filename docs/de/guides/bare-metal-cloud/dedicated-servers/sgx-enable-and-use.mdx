...
- Zugriff auf die <ApiLink>OVHcloud API</ApiLink> (optional)
...
Es erscheint ein Fenster, um zu bestätigen, dass die Aktivierung von Intel SGX einen Neustart Ihres Servers erfordert.
  ...
  <Tab label="Über die OVHcloud API">
    **1 - Anmeldung in der API-Konsole**
    ...
    Klicken Sie auf der <ApiLink>OVHcloud API Seite</ApiLink>:
...
Um zu überprüfen, ob Ihr Dienst über die SGX-Option verfügt, nutzen Sie folgenden Aufruf: 
    ...
    <Api version="v1" section="/dedicated/server" method="GET" route={"/dedicated/server/\\{serviceName\\}/biosSettings/sgx"} />
...
Aktivieren Sie SGX unter Verwendung des Servernamens:
    ...
    <Api version="v1" section="/dedicated/server" method="POST" route={"/dedicated/server/\\{serviceName\\}/biosSettings/sgx/configure"} />

Geben Sie zum Beispiel im Feld *request body* Folgendes ein:
