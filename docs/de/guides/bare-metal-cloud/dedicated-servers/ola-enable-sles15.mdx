...
description: Erfahren Sie hier, wie Sie OLA für Ihren SLES 15 Server aktivieren
...
- Sie haben Ihre [Netzwerkkarte für die OVHcloud Link Aggregation im OVHcloud Kundencenter konfiguriert](/guides/bare-metal-cloud/dedicated-servers/ola-enable-manager).
...
Klicken Sie anschließend auf den Tab <code className="action">IPMI</code> (1) und dann auf den Button <code className="action">Mit einem Java-Applet (KVM)</code> (2).
...
[Konfiguration der OVHcloud Link Aggregation in Ihrem Kundencenter](/guides/bare-metal-cloud/dedicated-servers/ola-enable-manager)
