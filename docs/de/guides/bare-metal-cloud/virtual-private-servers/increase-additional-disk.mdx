...
- Sie haben eine [zusätzliche Disk konfiguriert](/guides/bare-metal-cloud/virtual-private-servers/config-additional-disk).
...
In den folgenden Schritten wird vorausgesetzt, dass Sie bereits eine zusätzliche Disk erstellt haben, wie [in unserer Anleitung beschrieben](/guides/bare-metal-cloud/virtual-private-servers/config-additional-disk).

### Größe der Disk ändern <a name="extend"></a>

Im Tab <code className="action">Start</code> finden Sie im Bereich **Ihre Konfiguration** den Abschnitt mit der Bezeichnung `Zusätzliche Festplatten`. Klicken Sie auf <code className="action">...</code> und im Kontextmenü auf <code className="action">Festplattengröße erhöhen</code>.
...
Wählen Sie im neuen Fenster die gewünschte Größe aus und klicken Sie dann auf <code className="action">Erweitern</code>.
...
Die Erhöhung der Disk-Kapazität wird nach der Bestätigung der Zahlung einige Minuten dauern. Sie können den Fortschritt im Tab <code className="action">Zusätzliche Festplatte</code> überprüfen: Wird die ausgewählte neue Größe angezeigt, ist die Disk bereit.
...
Auf der [erweiterten Disk](#extend) wird die zusätzliche Kapazität als nicht zugewiesener Speicherplatz angezeigt. Klicken Sie mit der rechten Maustaste auf das Volume Ihrer zusätzlichen Disk und wählen Sie <code className="action">Volume erweitern</code> im Kontextmenü.
...
Klicken Sie im Assistenten auf <code className="action">Weiter</code>, um fortzufahren.

Wenn nötig können Sie den Speicherplatz in diesem Schritt ändern. Klicken Sie auf <code className="action">Weiter</code>.
...
Klicken Sie auf <code className="action">Beenden</code>, um den Vorgang abzuschließen.
