...
description: Erfahren Sie hier, wie Sie den verwendbaren Speicherplatz nach einem Upgrade vergrößern
...
- Der Server wurde im [Rescue-Modus](/guides/bare-metal-cloud/virtual-private-servers/rescue) neu gestartet (nur für GNU/Linux-Systeme notwendig).
...
Wenn sich der VPS noch nicht im Rescue-Modus befindet, aktivieren Sie ihn mithilfe von [unserer Anleitung](/guides/bare-metal-cloud/virtual-private-servers/rescue).
...
Es wird empfohlen, vor dem Löschen der vorherigen Partition die dem ersten Sektor entsprechende Zahl der Partition aufzubewahren. Diese Information erhalten Sie über den Befehl <code className="action">p</code>. Sie steht unter dem Feld `Start`. Bewahren Sie diese Angabe für später auf.
...
Löschen Sie anschließend die Partition mit dem Befehl <code className="action">d</code>.
...
Jetzt muss über den Befehl <code className="action">n</code> eine neue Partition erstellt werden. Die Verwendung von Standardwerten wird empfohlen.
...
Jetzt muss sichergestellt werden, dass die Partition bootfähig ist. Das ist über den Befehl <code className="action">a</code> möglich.
...
Speichern Sie Ihre Änderungen und verlassen Sie die Anwendung über den Befehl <code className="action">w</code>:
...
Wenn nach Eingabe des Befehls <code className="action">e2fsck</code> die Fehlermeldung `bad magic number in superblock` erscheint, müssen Sie mithilfe eines Backup-Superblocks das Dateisystem überprüfen und reparieren. Um die verfügbaren Backup-Superblöcke zu sehen, geben Sie folgenden Befehl ein:
...
Klicken Sie mit der rechten Maustaste auf C: und wählen Sie <code className="action">Extend Volume...</code>.
...
Geben Sie die gewünschte Größe des Volumes ein und klicken Sie auf <code className="action">OK</code>. Ihr Volume wird nun erweitert.
