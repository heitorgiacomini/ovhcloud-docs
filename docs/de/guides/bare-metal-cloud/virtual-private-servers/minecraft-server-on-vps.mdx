...
description: Erfahren Sie hier, wie Sie Ihren eigenen Minecraft-Server installieren
...
<br/>Es wird empfohlen, einen neuen VPS zu bestellen oder einen bestehenden Server über Ihr <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> zu [re-installieren](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps#reinstallvps) und dabei die neueste verfügbare Version von Ubuntu oder Debian zu verwenden.

Wenn das Betriebssystem installiert ist, verbinden Sie sich mit Ihrem VPS per SSH, wie in der Anleitung "[Erste Schritte mit einem VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)" erklärt.
...
Bestätigen Sie einfach mit <code className="action">Enter</code>, um das Ausfüllen der üblichen Kontoinformationen zu überspringen.
...
Klicken Sie dazu auf der offiziellen [Website von Minecraft](https://minecraft.net/download/server) mit der rechten Maustaste auf den Download-Link und wählen Sie <code className="action">Linkadresse kopieren</code>.

<img className="thumbnail" alt="Download des Servers" src="/images/bare-metal-cloud/virtual-private-servers/minecraft-server-on-vps/jar_file_download.png" loading="lazy" />
...
Um eine Shell zu verlassen (während diese weiter ausgeführt wird), drücken Sie auf <code className="action">Ctrl</code>, dann auf <code className="action">a</code> und dann auf <code className="action">d</code> Ihrer Tastatur.
...
Sie können alternativ auch <code className="action">Ctrl</code>, dann auf <code className="action">a</code> und dann auf <code className="action">n</code> Ihrer Tastatur drücken.
...
<img className="thumbnail" alt="Verbindung zum Server" src="/images/bare-metal-cloud/virtual-private-servers/minecraft-server-on-vps/login_minecraft.png" loading="lazy" />
...
<img className="thumbnail" alt="Informationen zum Server" src="/images/bare-metal-cloud/virtual-private-servers/minecraft-server-on-vps/minecraft_server_login.png" loading="lazy" />
...
Beachten Sie, dass diese Installationsanleitung auch für einen [OVHcloud Dedicated Server](https://www.ovhcloud.com/de/bare-metal/) oder eine [Public Cloud Instanz](https://www.ovhcloud.com/de/public-cloud/compute/) anwendbar ist. Mit diesen Lösungen profitieren Sie auch von garantierten und stabilen physischen Ressourcen, da die Hardware dediziert ist.
