...
In addition to the [responsibility model between OVHcloud and the customer on the Dedicated Server service](/guides/account-and-service-management/responsibility-sharing/dedicated-servers), this sheet aims to present the specific features and security features of this service. It also highlights best practices that will enable the customer to make the most of it.
...
Other guides are available in [the guide on Dedicated Servers](/guides/bare-metal-cloud/dedicated-servers/overview) to assist the customer with the transfer
...
| --- | --- | --- | --- | --- |
| - FTP backup or Backup Storage is a 500GB storage space made available to the customer following subscription to the service.<br /> - The service must be activated by the customer.<br /> - No backup routine is configured by OVHcloud. | At the customer's choice | Depends on customer choice | N/A | [Use Backup Storage on a dedicated server](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) |
| - Backup Storage is an additional storage option that allows you to have up to 10TB of additional disk space to deposit backups. | At the customer's choice | Depends on customer choice | N/A | [Use Backup Storage on a dedicated server](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) |
...
| --- | --- | --- |
| Control Plane | Logs on all interactions made via API calls, launched by admin, technical or billing contacts, on the services they have access to. |- [https://api.ovh.com/console/#/me](https://api.ovh.com/console/#/me) (see `/me/api/logs` calls)<br />- [List of API calls done with your account](https://api.ovh.com/console/#/me/api/logs/self~GET)<br />- [List of API calls done on services you have access to](https://api.ovh.com/console/#/me/api/logs/services~GET) |
...
Via the OVHcloud Control Panel, the customer can manage the service using [three typical contacts](/guides/account-and-service-management/account-information/managing-contacts#definition).
...
To increase access to the customer account, the customer can enable [two-factor authentication (2FA)](/guides/account-and-service-management/account-information/secure-ovhcloud-account-with-2fa) or [single sign-on (SSO)](/guides/account-and-service-management/account-information/overview) by linking their account to an external Active Directory.

### 7.2 - Data Plane

Once the service has been delivered, at the OS installation stage, the customer has the choice between [using an SSH key](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys) (for Linux distributions) to access their server, or a single password, generated automatically by OVHcloud if they have not configured an SSH key.
...
### 9.1 - Providing OS images and hardening <a id="os-images"></a>
...
| --- | --- |
| Debian | [https://wiki.debian.org/Hardening](https://wiki.debian.org/Hardening) |
| Redhat | [https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/security_hardening/overview-of-security-hardening-security-hardening](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/security_hardening/overview-of-security-hardening-security-hardening) |
| Ubuntu | [https://ubuntu.com/security/certifications/docs/usg](https://ubuntu.com/security/certifications/docs/usg) |
| Windows | [https://docs.microsoft.com/en-gb/windows/security/threat-protection/windows-security-configuration-framework/windows-security-baselines](https://docs.microsoft.com/en-gb/windows/security/threat-protection/windows-security-configuration-framework/windows-security-baselines) |
...
Prerequisites and instructions are available on [this link](/guides/bare-metal-cloud/dedicated-servers/bring-your-own-image).
...
In order to continue to benefit from the OVHcloud monitoring service, the customer must configure [filtering rules](/guides/bare-metal-cloud/dedicated-servers/network-ip-monitoring) on their servers' internal firewall, and select the other services whose status they want to track.

OVHcloud offers a feature called [OVHcloud Link Aggregation](/guides/bare-metal-cloud/dedicated-servers/ola-enable-manager) that can be activated by the customer and which allows them to benefit from a high-speed, redundant private network for their Dedicated Servers.
...
OVHcloud's portability principles are described in its own [portability policy](/guides/account-and-service-management/reversibility/global-reversibility-policy), and those specific to the Dedicated Servers service are set out in its [specific policy](/guides/account-and-service-management/reversibility/dedicated-servers-reversibility-policy).
