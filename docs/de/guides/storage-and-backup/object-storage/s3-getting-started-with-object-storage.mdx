...
- for **Standard object storage - SWIFT API** storage class, follow [this guide](/guides/storage-and-backup/object-storage/pcs-create-container).
- for **Cloud Archive - SWIFT API** storage class, follow [this guide](/guides/storage-and-backup/object-storage/pca-create-container).
...
:::info
If you wish to use the OVHcloud Terraform provider, you can follow [this guide](/guides/storage-and-backup/object-storage/s3-terraform).
...
- You will need your user's *Access key* and *Secret key*. You can access this information in the <code className="action">Object Storage users</code> tab in your OVHcloud Control Panel.
- You will also need your *endpoint_url*. If you have already created your bucket, you can access this information from the <code className="action">My containers</code> tab, then in the details of your bucket. Otherwise, follow this [guide](/guides/storage-and-backup/object-storage/s3-location).
...
<img className="thumbnail" alt="Bucket details" src="/images/storage-and-backup/object-storage/s3-getting-started-with-object-storage/object_storage_information_panel.png" loading="lazy" />
...
For a list of endpoints by region and storage class, refer to [this page](/guides/storage-and-backup/object-storage/s3-location).
...
<Tabs>
  ...
  <Tab label="Via the OVHcloud Control Panel">
    ...
    <img className="thumbnail" alt="Create your bucket part 1" src="/images/storage-and-backup/object-storage/s3-getting-started-with-object-storage/object-storage-bucket-creation-1.png" loading="lazy" />
...
You can enter the name of your bucket (optional) and then **select your offer**.
    ...
    **Select a region.**
    ...
    <img className="thumbnail" alt="Create your bucket part 2" src="/images/storage-and-backup/object-storage/s3-getting-started-with-object-storage/object-storage-bucket-creation-2.png" loading="lazy" />
    ...
    Versioning allows you to keep multiple variants of an object in the same bucket. This feature helps **preserve, retrieve, and restore every version of every object stored in your buckets**, making it easier to recover from unintended user actions or application failures. By default, versioning is disabled on buckets, and you must explicitly enable it. Find more information about versioning on our [dedicated guide](/guides/storage-and-backup/object-storage/s3-versioning).
    
    You can also enable [Object Lock](/guides/storage-and-backup/object-storage/s3-managing-object-lock) to store your objects in WORM (Write Once, Read Many) mode and guarantee their immutability for a defined retention period.
    ...
    **Note:** this option must be enabled when creating a bucket, it cannot be enabled later.
    ...
    You can now decide whether or not you wish to **encrypt your data** using [SSE-OMK (server-side encryption with OVHcloud Managed Keys)](/guides/storage-and-backup/object-storage/s3-encrypt-your-objects-with-sse-c).
...
When uploading objects, you can select a storage class to control availability, redundancy, and cost. To help you in choosing the best class for your requirements, check the documentation [here](/guides/storage-and-backup/object-storage/s3-choosing-the-right-storage-class-for-your-needs).
...
<Tabs>
  <Tab label="Via AWS CLI">
    ...
    **By default, objects are named after files, but they can be renamed.**
    ...
    To learn more about the storage class mapping between OVHcloud storage tiers and AWS storage classes, you can check our documentation [here](/guides/storage-and-backup/object-storage/s3-location).
...
<Tabs>
  <Tab label="Via AWS CLI">
    ...
    **Deleting objects and buckets with versioning enabled**
    ...
    :::info
    If your bucket has Object Lock enabled, you will not be able to permanently delete your objects. See our [documentation](/guides/storage-and-backup/object-storage/s3-managing-object-lock) to learn more about Object Lock.
