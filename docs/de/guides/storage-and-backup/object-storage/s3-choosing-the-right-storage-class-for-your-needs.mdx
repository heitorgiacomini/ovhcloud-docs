...
description: Discover the different OVHcloud Object Storage classes and choose the one that best suits your needs.
...
The Standard storage class offers a scalable object storage service, compatible with the vast majority of use cases, adapted to any type of volume. The solution is based on HDD storage within a resilient architecture in a datacentre. The solution is accessible via an S3-compatible API.
...
This offer is suitable for the following use cases: AI & Analytics, datalake, High Performance Computing, multimedia/content platforms.
...
More details about available regions [here](/guides/storage-and-backup/object-storage/s3-location).

:::info
Before November 2025, Cold Archive only existed with a bucket-level granularity, as [a dedicated product at OVHcloud](/guides/storage-and-backup/object-storage/cold-archive-getting-started). The product has been updated to allow object-level granularity and Cold Archive is now considered as a class of Object Storage compatible with [**lifecycle management feature**](/guides/storage-and-backup/object-storage/s3-bucket-lifecycle).
:::
...
More details about lifecycle management [here](/guides/storage-and-backup/object-storage/s3-bucket-lifecycle).
...
The list of all API endpoints can be found [here](/guides/storage-and-backup/object-storage/s3-location).
