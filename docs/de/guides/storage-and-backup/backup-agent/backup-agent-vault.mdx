...
description: Erfahren Sie, wie das Vault-System funktioniert und wo Ihre Backup-Daten gespeichert werden
...
- Sie haben einen Backup Agent-Dienst, der entweder zum Zeitpunkt des Bestellens Ihres Bare Metal Servers oder später über das Menü <code className="action">Backup Agent</code> in Ihrem OVHcloud Kundencenter bestellt wurde.
...
Dies basiert auf unseren Object Storage-Buckets, die Sie [hier](https://www.ovhcloud.com/de/public-cloud/object-storage/) finden können.

Um Ihre Vaults zu finden, klicken Sie auf <ManagerLink to="/#/bmc-backup-agent-baremetal/service">diesen Link</ManagerLink>, um auf den Bereich <code className="action">Backup Agent</code> zuzugreifen, und klicken Sie dann auf den Tab <code className="action">Vaults</code>.

<img className="thumbnail" alt="Backup Agent Vault Liste" src="/images/storage-and-backup/backup-agent/backup-agent-vault/01-backup-agent-vault-list.png" loading="lazy" />
...
<img className="thumbnail" alt="Backup Agent Vault Anwendungsfälle" src="/images/storage-and-backup/backup-agent/backup-agent-vault/01-backup-agent-vault-use-cases.png" loading="lazy" />
