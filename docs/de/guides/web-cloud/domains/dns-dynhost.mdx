...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Wählen Sie den Tab <code className="action">DNS-Server</code> aus.
...
- `ns200.anycast.me` und `dns200.anycast.me` (wenn die Option [DNS Anycast](https://www.ovhcloud.com/de/domains/options/) aktiviert ist).

Mehr Informationen finden Sie in unserer [Dokumentation zu DNS-Servern](/guides/web-cloud/domains/dns-server-general-information).
...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Wählen Sie den Tab <code className="action">DynHost</code> aus.
  ...
  <Tab label="Schritt 3">
    Klicken Sie auf den Button <code className="action">Zugriff verwalten</code> und dann auf <code className="action">DynHost-Kennung erstellen</code>. 
  ...
  <Tab label="Schritt 4">
    ...
    :::tip
    Um einen DynHost für Ihren Domainnamen direkt einzurichten, geben Sie `*` in das Feld mit dem Namen <code className="action">Subdomain</code> ein.
  ...
  <Tab label="Schritt 5">
    Wenn Sie die Felder ausgefüllt haben, klicken Sie auf den Button <code className="action">Bestätigen</code>. Die Kennung erscheint dann in der Tabelle auf der aktuellen Seite.
...
<Tabs>
  ...
  <Tab label="Schritt 3">
    Klicken Sie auf den Button <code className="action">DynHost hinzufügen</code>.
  ...
  <Tab label="Schritt 4">
    ...
    :::warning
    Für die Einrichtung eines dynamischen DNS-Eintrags (DynHost) ist die Verwendung einer *wildcard* (nur das Zeichen `*`) im Formular <code className="action">Subdomain</code> nicht verfügbar.

    :::

    
    <img className="thumbnail" alt="dynhost" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dynhost/create-a-dynhost.png" loading="lazy" />

Wenn Sie die Felder ausgefüllt haben, klicken Sie auf den Button <code className="action">Bestätigen</code>. Der DynHost-Eintrag erscheint dann in der Tabelle auf der aktuell geöffneten Seite. 
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab label="Schritt 3">
    Überprüfen Sie die in der Spalte <code className="action">Ziel</code> angezeigte IP-Adresse.
