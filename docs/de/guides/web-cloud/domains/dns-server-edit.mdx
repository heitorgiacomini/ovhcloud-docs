...
3. Achten Sie darauf, verschiedene Gruppen von OVHcloud DNS-Servern nicht zu mischen. Bei OVHcloud können DNS-Servergruppen anhand der Nummer in den Servernamen identifiziert werden. Zwei OVHcloud DNS-Server gehören derselben Gruppe an, wenn sie dieselbe Nummer verwenden.

<Region zones={['eu']}>

Zum Beispiel bilden *dns19.ovh.net* und *ns19.ovh.net* eine Gruppe von OVHcloud DNS-Servern; sie gehören zusammen und werden synchronisiert. Sie gehören derselben Gruppe an, weil sie dieselbe Nummer verwenden.

</Region>

<Region zones={['ca', 'apac']}>

Zum Beispiel bilden *dnsXX.ovh.ca* und *nsXX.ovh.ca* eine Gruppe von OVHcloud DNS-Servern; sie gehören zusammen und werden synchronisiert (zum Beispiel *dns10.ovh.ca* und *ns10.ovh.ca*). Sie gehören derselben Gruppe an, weil sie dieselbe Nummer verwenden. Die genauen Server, die Ihrem Domainnamen zugewiesen sind, werden im OVHcloud Kundencenter angezeigt.
...
Beispiel: Sie möchten die DNS-Server *ns1.dns-server.tld*, *dnsXX.ovh.ca* und *nsXX.ovh.ca* für Ihren Domainnamen deklarieren. Überprüfen Sie, ob die folgenden drei NS-Einträge in den 3 DNS-Zonen vorhanden sind, die auf diesen 3 DNS-Servern gehostet werden:

- "Ihr Domainname (oder nur ein @)" IN NS ns1.dns-server.tld.
- "Ihr Domainname (oder nur ein @)" IN NS dnsXX.ovh.ca.
- "Ihr Domainname (oder nur ein @)" IN NS nsXX.ovh.ca.
...
- Die *Registry*, die Ihre Domainendung verwaltet (z.B. die für *.fr*-Endungen zuständige Registry), muss über die bei OVHcloud vorgenommene DNS-Änderung informiert werden. Verfolgen Sie den Fortschritt auf der Seite <ManagerLink to="/#/web/ongoing-operations">Laufende Vorgänge</ManagerLink>.
