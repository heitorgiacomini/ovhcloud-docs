...
Der Eintrag **D**omain-based **M**essage **A**uthentication, **R**eporting, and **C**ompliance (DMARC) ist ein Mechanismus zur E-Mail-Sicherheit. Er basiert auf den Ergebnissen von [SPF](/guides/web-cloud/domains/dns-zone-spf)- und [DKIM](/guides/web-cloud/domains/dns-zone-dkim)-Prüfungen.
...
- Einer der Authentifizierungsmechanismen [SPF](/guides/web-cloud/domains/dns-zone-spf) oder [DKIM](/guides/web-cloud/domains/dns-zone-dkim) muss in der DNS-Zone des Domainnamens des E-Mail-Dienstes konfiguriert werden.
...
DMARC fragt die Authentifizierungsmechanismen [SPF](/guides/web-cloud/domains/dns-zone-spf) und [DKIM](/guides/web-cloud/domains/dns-zone-dkim) ab, um eingehende E-Mails zu überprüfen.<br/>
...
Sie können den DMARC-Eintrag über das OVHcloud Kundencenter zu Ihrer DNS-Zone hinzufügen. Loggen Sie sich hierzu in Ihr <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> ein und gehen Sie in den Bereich <code className="action">Web Cloud</code>. Wählen Sie in der linken Spalte im Bereich <code className="action">Domainnamen</code> die betreffende Domain aus und klicken Sie dann auf den Tab <code className="action">DNS-Zone</code>, um zu Ihrer DNS-Zone zu gelangen.

Klicken Sie auf den Button <code className="action">Eintrag hinzufügen</code> und dann auf <code className="action">DMARC</code> in den E-Mail-Feldern.
...
:::warning
Die Konfiguration der Einstellung `p=` kann einen großen Einfluss auf die E-Mail-Zustellung Ihrer Domain haben. Es wird empfohlen, `p=none` zu konfigurieren und mehrere Wochen lang Fehlerberichte zu analysieren, um Anomalien festzustellen. Der Wechsel zu `p=quarantine` oder `p=reject` erfordert volle Kontrolle über die E-Mail-Sicherheitseinstellungen für [SPF](/guides/web-cloud/domains/dns-zone-spf) und [DKIM](/guides/web-cloud/domains/dns-zone-dkim). Der unten beschriebene Faktor `pct=` ermöglicht Übergangswerte.
...
Sie können den TXT-Eintrag über das <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> im Bereich <code className="action">Web Cloud</code> zu Ihrer DNS-Zone hinzufügen. Klicken Sie auf <code className="action">Domainnamen</code> und wählen Sie die betreffende Domain aus. Gehen Sie dann auf den Tab <code className="action">DNS-Zone</code>.

Klicken Sie auf den Button <code className="action">Eintrag hinzufügen</code> und dann auf <code className="action">TXT</code>.
