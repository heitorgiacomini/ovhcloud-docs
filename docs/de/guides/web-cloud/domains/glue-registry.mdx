...
description: Erfahren Sie hier, wie Sie die DNS-Server Ihres OVHcloud Domainnamens anpassen
...
- Ein bei OVHcloud registrierter [Domainname](https://www.ovhcloud.com/de/domains/).
...
Befolgen Sie die nachstehenden Schritte sorgfältig oder kontaktieren Sie einen [spezialisierten Dienstleister](https://partner.ovhcloud.com/de/directory/), wenn Sie Zweifel haben.
...
Gehen Sie dazu auf [https://zonemaster.net](https://zonemaster.net/en), geben Sie Ihren Domainnamen ohne *www* ein (Beispiel: *domain.tld*), und klicken Sie dann auf den Button <code className="action">Options</code> direkt unter dem Eingabefeld für den Domainnamen.

Klicken Sie in den verfügbaren Optionen auf den Button <code className="action">Fetch NS from parent zone</code>.
...
Weitere Informationen finden Sie in unserem Tutorial zum Tool [Zonemaster](/guides/web-cloud/domains/dns-zonemaster).
...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Klicken Sie auf dem betreffenden Domainnamen auf den Tab <code className="action">Hosts</code>.
    
    In der angezeigten Tabelle werden die aktuell bei OVHcloud konfigurierten Host-Einträge für Ihren Domainnamen aufgeführt, sofern vorhanden. Um einen neuen Eintrag hinzuzufügen, klicken Sie auf den Button <code className="action">Hinzufügen</code>.
...
In der Abbildung oben ist, basierend auf dem Beispiel aus [Schritt 2](#step2), der hier hinzuzufügende Host (für den Domainnamen *domain.tld*) **dns1.domain.tld**.
    ...
    Wenn Sie die Informationen ausgefüllt haben, klicken Sie auf den Button <code className="action">Hinzufügen</code>. Lesen Sie die angezeigten Informationen und klicken Sie dann auf <code className="action">Bestätigen</code>. Wiederholen Sie diesen Vorgang so oft wie nötig, abhängig von der Anzahl der von Ihrem Domainnamen verwendeten DNS-Server.
...
<summary>Ihr Domainname verwendet keine aktive DNS-Zone bei OVHcloud</summary>
...
<summary>Ihr Domainname verwendet eine aktive DNS-Zone bei OVHcloud</summary>
...
<Tabs>
  <Tab label="Schritt 1">
    Gehen Sie auf die Seite <ManagerLink to="/#/web/zone">DNS-Zone</ManagerLink>, und wählen Sie den betreffenden Domainnamen aus, den Sie für die Erstellung der Hosts in [Schritt 3](#step3) verwendet haben.
  ...
  <Tab label="Schritt 2">
    Klicken Sie auf <code className="action">Eintrag hinzufügen</code>.
  </Tab>
  <Tab label="Schritt 3">
  ...
  <Tab label="Schritt 4">
    Geben Sie die *Subdomain* und die *IPv4*-Adresse (A) oder *IPv6*-Adresse (AAAA) ein und fahren Sie fort, bis die Hinzufügung bestätigt wurde. Falls erforderlich, folgen Sie den Anweisungen in unserer Anleitung "[OVHcloud DNS-Zone bearbeiten](/guides/web-cloud/domains/dns-zone-edit)".
  </Tab>
</Tabs>
...
<Tabs>
  <Tab label="Schritt 1">
    Gehen Sie auf die Seite <ManagerLink to="/#/web/zone">DNS-Zone</ManagerLink>, und wählen Sie den betreffenden Domainnamen aus, für den Sie die DNS-Server individualisiert haben.
    
    <img className="thumbnail" alt="DNS zones" src="/images/assets/screens/control-panel/product-selection/web-cloud/dns-zones.png" loading="lazy" />
  </Tab>
  <Tab label="Schritt 2">
    Klicken Sie auf <code className="action">Im Textmodus bearbeiten</code>.
    ...
    Kontaktieren Sie einen [spezialisierten Dienstleister](https://partner.ovhcloud.com/de/directory/), wenn Sie Zweifel haben.

    :::

  </Tab>
  <Tab label="Schritt 3">
    Ersetzen Sie in diesem Fenster **nur in den Einträgen vom Typ *NS*** die DNS-Servernamen durch Ihre eigenen benutzerdefinierten DNS-Servernamen, und **erhöhen Sie dabei** den ersten numerischen Wert der *SOA*-Zeile um "1". Wenn Sie Ihre Änderungen vorgenommen haben, klicken Sie auf <code className="action">Weiter</code> und dann auf <code className="action">Bestätigen</code>.
...
Falls erforderlich, folgen Sie den Anweisungen in unserer Anleitung "[OVHcloud DNS-Zone bearbeiten](/guides/web-cloud/domains/dns-zone-edit)".
...
<Tabs>
  <Tab label="Schritt 1">
    Gehen Sie auf die Seite <ManagerLink to="/#/web-domains/domain">Domainnamen</ManagerLink>, und wählen Sie *den Domainnamen aus, für den Sie die DNS-Server individualisieren möchten*.
    
    <img className="thumbnail" alt="Domain names" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-names.png" loading="lazy" />
  </Tab>
  <Tab label="Schritt 2">
    Gehen Sie auf den Tab <code className="action">DNS-Server</code> und klicken Sie auf <code className="action">DNS-Server ändern</code>.
  ...
  <Tab label="Schritt 4">
    Schließen Sie die Schritte ab und folgen Sie bei Bedarf den Anweisungen in unserer Anleitung "[DNS-Server eines OVHcloud Domainnamens ändern](/guides/web-cloud/domains/dns-server-edit)".
...
[Allgemeine Informationen zu OVHcloud DNS-Servern](/guides/web-cloud/domains/dns-server-general-information)
