...
description: Erfahren Sie hier, wie Sie Ihre Website, Ihren Domainnamen, Ihre Datenbank und E-Mails ohne Dienstunterbrechung zu OVHcloud migrieren
...
Mehrere [OVHcloud Webhosting-Angebote](https://www.ovhcloud.com/de/web-hosting/) enthalten "[MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities)" E-Mail-Dienste. Mit diesem E-Mail-Angebot können E-Mail-Accounts mit jeweils maximal 5 GB Speicherplatz erstellt werden. Wählen Sie aus den unten stehenden Hosting-Angeboten eines aus, das den Anforderungen Ihrer Website an PHP-Version, SQL-Version, Anzahl der benötigten E-Mail-Accounts, sowie der Größe der zu migrierenden Website entspricht:

**Eco**-Reihe:

- **Starter** — 2 MX Plan E-Mail-Accounts
- **Basic** — 10 MX Plan E-Mail-Accounts

**Business**-Reihe:

- **Startup** — 10 MX Plan E-Mail-Accounts
- **Pro** — 100 MX Plan E-Mail-Accounts
- **Performance** — 1.000 MX Plan E-Mail-Accounts

**Agencies**-Reihe:

- **Agency**, **Agency Plus** und **Agency Max** — jeweils 1.000 MX Plan E-Mail-Accounts

Wenn Sie sich für ein passendes Webhosting-Angebot entschieden haben, klicken Sie auf den Button <code className="action">Bestellen</code> auf unserer Webseite. Folgen Sie den Bestellschritten, aber **leiten Sie dabei noch nicht den Transfer Ihres Domainnamens ein**. (Diese Aktion wird in Schritt 10 dieser Anleitung ausgeführt.)
...
<Tabs>
  <Tab label="Schritt 1">
    Gehen Sie auf die Seite <ManagerLink to="/#/web/hosting">Hosting-Pakete</ManagerLink>.
  ...
  <Tab label="Schritt 2">
    Klicken Sie oben links im Interface auf den Button <code className="action">Bestellen</code> und dann auf <code className="action">Hosting-Pakete</code>.
  </Tab>
  <Tab label="Schritt 3">
...
<Region zones={['ca']}>
:::info
OVHcloud bietet neben MX Plan weitere E-Mail-Angebote an. Sie können beispielsweise [Exchange](https://www.ovhcloud.com/de/emails/hosted-exchange/)-Accounts mit E-Mail-Accounts kombinieren.
...
Wenn Ihr Webhosting eingerichtet ist, loggen Sie sich in Ihr <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> ein und erstellen Sie eine DNS-Zone für Ihren Domainnamen. Verwenden Sie dabei nicht "**www**". Weitere Informationen finden Sie in unserer Anleitung zur [Erstellung einer DNS-Zone bei OVHcloud](/guides/web-cloud/domains/dns-zone-create).

Sobald die DNS-Zone eingerichtet ist, kann sie zur Verwendung mit dem Webhosting vorbereitet werden. Verwenden Sie dazu unsere Anleitung zum [Bearbeiten einer OVHcloud DNS-Zone](/guides/web-cloud/domains/dns-zone-edit). 
...
Die richtige Ziel-IP-Adresse Ihres OVHcloud Hostings finden Sie in unserer Anleitung [IP-Adressen der verschiedenen Shared Hosting Cluster](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip).
...
<Region zones={['ca', 'apac']}>
:::tip
Beachten Sie die beiden Zielwerte für den Eintragstyp NS. Die Werte `dnsXX.ovh.ca` und `nsXX.ovh.ca` (oder `dns200.anycast.me` und `ns200.anycast.me`) entsprechen den DNS-Servern, die die DNS-Zone für Ihre Domain bereitstellen. Sie werden im [Schritt 9](#step9) dieser Anleitung verwendet.
...
Um das Backup des FTP-Speicherplatzes Ihrer Website zu importieren, [verbinden Sie sich mit dem FTP-Speicherplatz Ihres OVHcloud Hostings](/guides/web-cloud/web-hosting/ftp-connection) und laden Sie das Backup in den "www"-Wurzelordner hoch (gegebenenfalls in einen anderen Wurzelordner, den Sie zuvor erstellt haben).

Wir empfehlen, das Programm [FileZilla](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide) zu verwenden, um das FTP-Backup auf Ihr Hosting zu übertragen.
...
[Erstellen Sie eine neue Datenbank](/guides/web-cloud/web-hosting/sql-create-database) für Ihr Hosting und [importieren Sie dann Ihr Backup](/guides/web-cloud/web-hosting/sql-importing-mysql-database).

:::info
OVHcloud bietet auch Web Cloud Databases Datenbankserver an. Wenn Sie dieses Angebot mit Ihrer Website verwenden möchten, finden Sie unsere Dokumentation zu diesem Dienst auf [unserer dedizierten Seite](/guides/web-cloud/databases/overview).
...
:::tip
Wenn Sie ein Content Management System (CMS) wie WordPress, Joomla!, Drupal oder PrestaShop verwenden, finden Sie die Informationen zur Datenbank in deren Konfigurationsdateien. Sie finden Details hierzu in **Schritt 2** der Anleitung "[Änderung des Passworts einer Datenbank](/guides/web-cloud/web-hosting/sql-change-password)".

:::


Deklarieren und authorisieren Ihren externen Domainnamen auf Ihrem OVHcloud Webhosting mithilfe der Anleitung "[Mehrere Websites auf einem Webhosting einrichten](/guides/web-cloud/web-hosting/multisites-configure-multisite)". Geben Sie den Namen des Ordners, den Sie zu Beginn von [Schritt 4](#step4) ausgewählt haben, als Wurzelverzeichnis an. Zur Erinnerung: Es handelt sich um den Ordner im FTP-Speicherplatz, in den Sie Ihre Webseiten-Dateien abgelegt haben.
...
Erstellen Sie neue E-Mail-Accounts und verwenden Sie dabei die gleichen E-Mail-Adressen, die Sie aktuell bei Ihrem E-Mail Provider haben. Sie finden mehr Informationen in unserer Anleitung zur [Erstellung von E-Mail-Accounts mit MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).
...
- [E-Mail Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/first-config)
- [Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-starting-hosted)
</Region>

<Region zones={['ca']}>
Wenn Sie sich für das E-Mail-Angebot Exchange entschieden haben, lesen Sie unsere entsprechende Anleitung, um Ihre E-Mail-Adressen zu erstellen:
...
<Region zones={['ca', 'apac']}>
- Ihr Domainname ohne "www" als Eintragstyp "MX", zum Ziel: `mx1.mail.ovh.ca.`
- Ihr Domainname ohne "www" als Eintragstyp "MX", zum Ziel: `mx2.mail.ovh.ca.`
- Ihr Domainname ohne "www" als Eintragstyp "MX", zum Ziel: `mx3.mail.ovh.ca.`
</Region>
...
**Option 1**: Verwenden Sie unser Tool [OVHcloud Mail Migrator (OMM)](https://omm.ovhcloud.com/), um die Inhalte der bei Ihrem bisherigen Anbieter registrierten E-Mail-Accounts auf die bei OVHcloud erstellten Accounts zu kopieren. Weitere Informationen finden Sie in unserer Anleitung "[E-Mail-Accounts über OVHcloud Mail Migrator migrieren](/guides/web-cloud/email-and-collaborative-solutions/migrating/omm-migrate-email-account-to-ovhcloud)".

Wir empfehlen Ihnen, als <code className="action">Server type</code> unter <code className="action">Source account</code> nicht **POP** zu verwenden. Dieses Protokoll löscht E-Mails auf dem Quell-Server, und sendet sie an den Ziel-Server bei OVHcloud. Sie können dann die Inhalte von beiden Accounts nicht mehr vergleichen.

Geben Sie im Bereich <code className="action">Destination account</code> ausschließlich die betreffende OVHcloud E-Mail-Adresse und das zugehörige Passwort ein. Hierbei bleibt der <code className="action">Server type</code> auf <code className="action">Hosted by OVH (Autodetect)</code>.
...
- Alle Konfigurationseinstellungen finden Sie in unserer Anleitung "[Erste Schritte mit MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities#consult-emails-client)". Sie finden dort auch Links zu den Konfigurationsanleitungen für die wichtigsten E-Mail-Clients.

<Region zones={['eu']}>
#### Für E-Mail Pro E-Mail-Accounts:

- Sie finden alle unsere Anleitungen zur Konfigurationsunterstützung unter `Konfiguration des E-Mail-Clients` auf der [Seite für E-Mail Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/overview).
</Region>

<Region zones={['eu', 'ca']}>
#### Für Exchange E-Mail-Accounts:

- Sie finden alle unsere Anleitungen zur Konfigurationsunterstützung in den Abschnitten `Konfiguration des Exchange E-Mail-Clients`und `Konfiguration von Exchange auf kompatiblen Smartphones/Tablets` auf der [Seite für Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/overview).
</Region>
...
<Region zones={['ca', 'apac']}>
Ersetzen Sie die aktuell aktiven DNS-Server (des bisherigen Registrars) durch die beiden in der OVHcloud DNS-Zone deklarierten DNS-Server (Format `dnsXX.ovh.ca` und `nsXX.ovh.ca` oder `dns200.anycast.me` und `ns200.anycast.me`). Dieser Vorgang erfolgt über das Verwaltungsinterface des bisherigen Registrars.
</Region>
...
Transferieren Sie anschließend Ihren Domainnamen mithilfe unserer Anleitung zum [Transfer eines Domainnamens zu OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain).
...
[DNS-Server ändern](/guides/web-cloud/domains/dns-server-general-information)

[E-Mail-Accounts erstellen](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation)

[Backup in eine Webhosting-Datenbank importieren](/guides/web-cloud/web-hosting/sql-importing-mysql-database)

[Verwaltung einer Webhosting-Datenbank](/guides/web-cloud/web-hosting/sql-create-database)
