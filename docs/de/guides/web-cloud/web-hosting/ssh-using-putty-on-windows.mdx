...
Sie benötigen den Cluster-Namen Ihres Webhostings, den Sie in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> finden, sowie den FTP-Benutzernamen und das Passwort.  Weitere Informationen zu dieser Zugriffsmethode finden Sie in der [zugehörigen Anleitung](/guides/web-cloud/web-hosting/ftp-connection).
...
Sie benötigen die IP-Adresse Ihres Servers, die Sie in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> finden, sowie den Namen des Benutzer-Accounts, den Sie für diese Login-*Session* verwenden möchten. Weitere Informationen zu diesem Thema finden Sie in unseren Anleitungen zu den ersten Schritten:
...
:::info
PuTTY speichert Schlüsseldateien in einem bestimmten Format, das sie inkompatibel mit in **OpenSSH**-Clients erstellten SSH-Schlüsseldateien macht. Wenn Sie einen **Private Key** verwenden möchten, der zuvor mit einem SSH-Client in der Kommandozeile erstellt wurde (zum Beispiel für einen [Dedicated Server](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys) oder eine [Public Cloud Instanz](/guides/public-cloud/compute/creating-ssh-keys)), muss er zuerst [in das PuTTY-Format konvertiert werden](https://www.chiark.greenend.org.uk/~sgtatham/putty/faq.html#faq-ssh2-keyfmt).
...
<video className="thumbnail" autoPlay loop muted playsInline aria-label="putty key" src="/images/assets/screens/other/web-tools/putty/puttygen.mp4" />
...
- [Ersetzen eines verlorenen SSH-Schlüsselpaars auf einer Instanz](/guides/public-cloud/compute/replacing-lost-ssh-key-pair)
...
|  |  |
|:--|:--|
| <img className="thumbnail" alt="putty" src="/images/assets/screens/other/web-tools/putty/putty3.png" loading="lazy" /> | 1\. Öffnen Sie PuTTY.<br/> 2\. Erweitern Sie den Knoten `SSH` unter `Connection` in der Struktur `Category`.<br/> 3\. Erweitern Sie den Knoten `Auth`.<br/> 4\. Klicken Sie auf `Credentials`, um die entsprechenden Einstellungen anzuzeigen.<br/> 5\. Klicken Sie auf <code className="action">Browse</code>.<br/> 6\. Wählen Sie die Datei für den privaten Schlüssel (`keyfile.ppk`) aus dem Ordner, in dem Sie sie gespeichert haben. |
...
[SSH-Schlüssel mit OpenSSH erstellen](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)

[SSH-Schlüssel mit OpenSSH für Public Cloud Instanzen erstellen](/guides/public-cloud/compute/creating-ssh-keys)
