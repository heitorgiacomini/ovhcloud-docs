...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Im Tab <code className="action">FTP - SSH</code> ist der Hostname unter `FTP- und SFTP-Server` sichtbar.
...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Im Tab <code className="action">FTP - SSH</code> suchen Sie den Benutzernamen in der Spalte `Login` der Tabelle.
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Im Tab <code className="action">FTP - SSH</code> finden Sie den Pfad unter `Pfad des home-Verzeichnisses`. Wenn jedoch mehrere Benutzer konfiguriert sind, kann der angegebene Pfad abweichen. Ersetzen Sie in diesem Fall den nach `home/` angegebenen Benutzernamen durch den Namen Ihrer Wahl in der Spalte `Login` der Tabelle.
...
[SSH-Zugang Ihres Webhostings verwenden](/guides/web-cloud/web-hosting/ssh-on-webhosting) (nur verfügbar mit **Pro**, **Performance**, **Agency**, **Agency Plus** oder **Agency Max** [Webhostings](https://www.ovhcloud.com/de/web-hosting/))
