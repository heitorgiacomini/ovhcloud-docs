...
description: Eine Website auf einem Webhosting mit Visual Studio Code mithilfe einer SFTP-Erweiterung verwalten
...
:::info
Wenn Sie Ihre Website ohne Visual Studio Code verwalten möchten, können Sie den FileZilla FTP-Client installieren. Weitere Informationen finden Sie in unserer Anleitung „[FileZilla mit Ihrem OVHcloud Hosting verwenden](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide)“. Wenn Sie sich via SSH mit Ihrer Website verbinden möchten, lesen Sie unsere Dokumentation „[SSH-Zugang Ihres Webhostings verwenden](/guides/web-cloud/web-hosting/ssh-on-webhosting)“.
...
Nachdem Sie Visual Studio Code gestartet haben, klicken Sie oben auf der Benutzeroberfläche im horizontalen Menü auf <code className="action">View</code> und dann auf <code className="action">Extensions</code>.
...
Geben Sie oben links im Interface den Namen der Erweiterung „SFTP/FTP sync“ für *Natizyskunk* ein und klicken Sie auf <code className="action">Install</code>.
...
Wechseln Sie zurück zu Visual Studio Code im horizontalen Menü am oberen Rand der Benutzeroberfläche, klicken Sie auf <code className="action">View</code> und dann auf <code className="action">Command Palette</code>, um den Befehlseditor anzuzeigen.
...
Klicken Sie auf <code className="action">Open Folder</code>, navigieren Sie zum gewünschten Speicherort des lokalen Ordners, und klicken Sie zur Bestätigung auf <code className="action">Select a folder</code>.
...
Bevor Sie an Ihrem Projekt arbeiten, laden Sie es in Ihren zuvor erstellten lokalen Ordner herunter. Stellen Sie jedoch zunächst sicher, dass die Datei „sftp.json“ richtig konfiguriert ist. Die relevanten Informationen finden Sie in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink>. Klicken Sie im Bereich <code className="action">Web Cloud</code> auf <code className="action">Hosting-Pakete</code>. Wählen Sie das betreffende Hosting aus und klicken Sie dann auf <code className="action">FTP - SSH</code>.
...
Ebenfalls im Tab <code className="action">FTP-SSH</code> ist der Hostname (`host`) unter <code className="action">FTP- und SFTP-Server</code> sichtbar.
...
Suchen Sie den Benutzernamen (`username`) in der Spalte <code className="action">Login</code> der Tabelle.

#### remotePath

Den Remote-Pfad (`remotePath`) finden Sie unter <code className="action">Pfad des home-Verzeichnisses</code>. Wenn jedoch mehrere Benutzer konfiguriert sind, kann der angegebene Pfad abweichen. Ersetzen Sie in diesem Fall den nach `home/` angegebenen Benutzernamen durch den Namen Ihrer Wahl aus der Liste <code className="action">Login</code> Ihres Webhostings.
...
[FileZilla mit Ihrem OVHcloud Hosting verwenden](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide)

[SSH-Zugang Ihres Webhostings verwenden](/guides/web-cloud/web-hosting/ssh-on-webhosting) (nur verfügbar mit [Pro oder Performance Webhostings](https://www.ovhcloud.com/de/web-hosting/))
