...
description: Erfahren Sie, wie Sie sich mit dem Speicherplatz Ihres OVHcloud Webhostings verbinden und die darin enthaltenen Daten mithilfe der FileZilla Software verwalten
...
Sie können damit Dateien oder Ihre Website online stellen, indem Sie sich [mit dem Speicherplatz Ihres Webhostings verbinden](/guides/web-cloud/web-hosting/ftp-connection).
...
<summary>Klicken Sie hier, um den Inhalt dieses Abschnitts anzuzeigen.</summary>
...
- Die obere Leiste (<code className="action">Quickconnect</code>) ermöglicht die schnelle Verbindung zu Ihrem Hosting, indem Sie den **Hostnamen**, den **Benutzernamen**, das zugehörige **Passwort** und die verwendete **Port-Nummer** eintragen.
...
3. Klicken Sie auf den Tab <code className="action">FTP - SSH</code>.
...
:::info
Aus Sicherheitsgründen wird das Passwort eines Benutzers nicht auf der Seite <code className="action">FTP - SSH</code> angezeigt. Wenn Sie es vergessen haben, lesen Sie [diese Anleitung](/guides/web-cloud/web-hosting/ftp-change-password), um es zu ändern.
...
<summary>Mit FileZilla per SFTP mit dem Speicherplatz Ihres Webhostings verbinden. <a name="sftp"></a></summary>


**SFTP** verwendet wie SSH den Standard-Port 22 anstelle von Port 21. Wenn Sie ein Cloud Web Hosting Angebot verwenden, müssen Sie den Port verwenden, der in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> angezeigt wird. Port 22 ist für Cloud Web Hostings per SSH und SFTP deaktiviert.
...
Gehen Sie hierzu in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> zum Tab <code className="action">FTP - SSH</code> zurück, wie im [ersten Teil](#part-1) dieser Anleitung beschrieben.
...
- Für die Pro und Performance Angebote:
    ...
    - 1: Klicken Sie auf den Button <code className="action">...</code> rechts neben der Zeile für den Benutzer und dann auf <code className="action">Ändern</code>.
    - 2: Wählen Sie im angezeigten Fenster im Bereich `Verbindungsprotokolle` die Option <code className="action">FTP und SFTP</code> aus und klicken Sie auf <code className="action">Weiter</code>.
...
Wenn Sie in der Box **1** im Bild unten alles korrekt eingegeben haben, klicken Sie auf <code className="action">Quickconnect</code>.
...
<summary>Mit FileZilla per FTP mit dem Speicherplatz Ihres Webhostings verbinden.</summary>
...
Überprüfen Sie Ihre Zugangsdaten, um sicherzustellen, dass keine Fehler gemeldet werden. Gegebenenfalls können Sie [Passwort des FTP-Zugangs ändern](/guides/web-cloud/web-hosting/ftp-change-password) Ihres Webhostings direkt im <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink>.
...
Überprüfen Sie diesen anhand des in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> angezeigten Hostnamens.
...
<summary>Warteschlange (Queue)</summary>
...
<summary>Kontextmenü Server</summary>
...
<summary>Zugriffsrechte auf Dateien und Ordner (CHMOD)</summary>


Klicken Sie mit der rechten Maustaste auf eine der auf dem Server vorhandenen Dateien und wählen Sie <code className="action">File permissions...</code>.
...
<summary>Eine blockierte Website wieder freischalten</summary>
...
[Sichern Sie dann Ihre Dienste](/guides/web-cloud/web-hosting/diagnostic-403-forbidden#step-2) und beheben Sie dabei die Sicherheitslücken, die in der per E-Mail erhaltenen Benachrichtigung erwähnt werden.

:::


Klicken Sie anschließend auf <code className="action">Server</code> und wählen Sie <code className="action">Enter custom command</code> (diese Option kann auch als <code className="action">Enter FTP command</code> bezeichnet sein).
...
<summary>Transfer von Binärdateien</summary>


Bei binären Dateien, wie z.B. **CGI**-Dateien, kann es von Interesse sein, wie der Transfer ausgeführt wird.

Um den Transfertyp zu ändern, wählen Sie <code className="action">Transfer</code> im Hauptmenü und dann <code className="action">Transfer type</code>.
...
<summary>Ordnervergleich</summary>
...
Sie finden [hier unsere Anleitung zur Behebung von Fehlern bei der Verwendung eines FTP-Programms](/guides/web-cloud/web-hosting/ftp-recurring-ftp-problems).

Sie finden [hier alle unsere Hilfen zu Webhostings](/guides/web-cloud/web-hosting/overview).
