...
[OVHcloud Webhostings](https://www.ovhcloud.com/de/web-hosting/) erlauben das Hosten von Websites aller Art, solange sie mit der [Konfiguration unserer Shared Hosting Infrastrukturen](https://webhosting-infos.hosting.ovh.net) kompatibel sind.
...
- Im <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink>
...
Änderungen der Konfiguration Ihres Webhostings sind über das <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> oder direkt in der Datei “.ovhconfig“ möglich. Beide Wege haben denselben Effekt.
...
<iframe className="video" width="560" height="315" src="https://www.youtube-nocookie.com/embed/x_XDyA7D4qQ?si=OovLiOQbB45TDlRI" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowFullScreen={true}></iframe>

## Voraussetzungen

<Region zones={['eu']}>
- Sie verfügen über ein [OVHcloud Webhosting](https://www.ovhcloud.com/de/web-hosting/), mit Ausnahme von [Cloud Web Hosting](https://www.ovhcloud.com/de/web-hosting/cloud-web-offer/).
</Region>
...
- **Im OVHcloud Kundencenter**: Verbinden Sie sich mit dem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> und gehen Sie dann in den Bereich <code className="action">Web Cloud</code>. Klicken Sie in der linken Spalte auf <code className="action">Hosting-Pakete</code> und wählen Sie das betreffende Webhosting aus. Im Tab <code className="action">Allgemeine Informationen</code> finden Sie die Version unter *Globale PHP-Version*. 

<img className="thumbnail" alt="phpversion" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/general-information/change-php-version-step1.png" loading="lazy" />
...
Stellen Sie es anschließend auf Ihrem [FTP-Speicherplatz](/guides/web-cloud/web-hosting/ftp-connection) online, und rufen Sie es auf, indem Sie dessen vollständige Adresse/URL öffnen.
...
Weitere Informationen zu diesem Thema finden Sie in unserer Anleitung „[Application Firewall aktivieren](/guides/web-cloud/web-hosting/multisites-activating-application-firewall)“.
...
<Tabs>
  <Tab label="Schritt 1">
    Loggen Sie sich in Ihr <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> ein und gehen Sie dann in den Bereich <code className="action">Web Cloud</code>.
...
Klicken Sie bei **Globale PHP-Version** auf <code className="action">...</code> und dann auf <code className="action">Konfiguration ändern</code>.
...
Im Konfigurationsfenster haben Sie zwei Möglichkeiten. Wählen Sie die Aktion aus, die Sie durchführen möchten, und klicken Sie auf <code className="action">Weiter</code>.
...
Wenn Sie bereit sind, klicken Sie auf <code className="action">Bestätigen</code>, um die Änderung zu übernehmen. Warten Sie einen Moment, bis die Operation abgeschlossen ist.

<img className="thumbnail" alt="hostingConfiguration" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/general-information/modify-hosting-configuration-step-1-and-2.png" loading="lazy" />
...
<Tabs>
  <Tab label="Schritt 1">
    Loggen Sie sich in Ihr <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> ein und gehen Sie in den Bereich <code className="action">Web Cloud</code>.
  ...
  <Tab label="Schritt 4">
    ...
    <img className="thumbnail" alt="ovhcoFTP - SSH tab" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/ftp-ssh/tab-perso.png" loading="lazy" />

Bezüglich des Passworts des FTP-Benutzers folgen Sie bei Bedarf den Anweisungen in unserer Dokumentation [„Passwort eines FTP-Benutzers ändern“](/guides/web-cloud/web-hosting/ftp-change-password).
  </Tab>
</Tabs>
...
Wenn Sie sich in Ihrem [FTP-Speicherplatz](/guides/web-cloud/web-hosting/ftp-connection) eingeloggt haben, sehen Sie alle Dateien, die derzeit auf diesem gespeichert sind. Bleiben Sie auf dem Wurzelverzeichnis Ihres Hostings (das mit „/“ angegeben werden kann). Dort finden Sie die Datei “.ovhconfig“.
...
|app.engine.version|Definiert die vom Hosting verwendete [PHP-Version](#php-versions) unter [den von OVHcloud angebotenen Versionen](https://www.ovhcloud.com/de/web-hosting/uc-programming-language/). Geben Sie die gewünschte Version ein (die der ausgewählten Laufzeitumgebung entspricht).|
|http.firewall|Hier können Sie die [OVHcloud Webhosting Firewall](https://www.ovhcloud.com/de/web-hosting/options/) aktivieren oder deaktivieren. Geben Sie **security** zum Aktivieren oder **none** zum Deaktivieren ein.|
...
Nachdem Sie die Datei “.ovhconfig“ bearbeitet haben, laden Sie sie auf Ihren [FTP-Speicherplatz](/guides/web-cloud/web-hosting/ftp-connection) hoch. Verbinden Sie sich hierzu erneut mit Ihrem [FTP-Speicherplatz](/guides/web-cloud/web-hosting/ftp-connection) und öffnen Sie dessen Wurzelverzeichnis (entspricht der Ebene `/`). Plazieren Sie die soeben bearbeitete “.ovhconfig“ an dieser Stelle in Ihrem [FTP-Speicherplatz](/guides/web-cloud/web-hosting/ftp-connection). Wenn die Datei bereits vorhanden ist, ersetzen Sie sie.
...
Erstellen Sie eine Datei namens „.ovhconfig“, die die gewünschte PHP-Version für die betreffenden Webseiten enthält. Nutzen Sie ggf. die Anweisungen im Abschnitt „[3.3 - Inhalt der Datei „.ovhconfig“ ändern](#update-ovhconfig)“ in dieser Anleitung. Wenn Sie die Datei „.ovhconfig“ auf Ihren [FTP-Speicherplatz](/guides/web-cloud/web-hosting/ftp-connection) hochladen, tun Sie dies im Wurzelverzeichnis, in dem sich die Dateien der betreffenden Webseite befinden. Sie finden das Wurzelverzeichnis Ihrer Webseite in Ihrem <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> im Tab <code className="action">Multisite</code> des betreffenden Hostings.

Wenn Sie weitere Informationen benötigen, konsultieren Sie unsere Anleitung zum [Konfigurieren mehrerer Websites auf einem Webhosting](/guides/web-cloud/web-hosting/multisites-configure-multisite).

:::warning
**Es ist nicht möglich, eine zweite [Laufzeitumgebung](#runtime-environment), einen zweiten [Entwicklungsmodus](#runtime-mod) und/oder eine zweite [PHP-Engine](#php-runtime)** auf demselben Webhosting einzurichten. Es werden nur die Werte berücksichtigt, die in der Datei “.ovhconfig“ im Wurzelverzeichnis Ihres [FTP-Speicherplatzes](/guides/web-cloud/web-hosting/ftp-connection) angegeben sind.

:::


<img className="thumbnail" alt="ovhconfig" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/multisite/root-folders.png" loading="lazy" />
