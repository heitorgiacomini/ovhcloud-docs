...
<Region zones={['eu']}>
- Sie verfügen über ein [OVHcloud Webhosting](/links/web/hosting), auf dem Sie 1-Klick-Module installieren können (ausgenommen [Kostenloses Hosting](/guides/web-cloud/web-hosting/activate-start10m)).
</Region>
<Region zones={['ca', 'apac']}>
- Sie verfügen über ein [OVHcloud Webhosting](/links/web/hosting), auf dem Sie 1-Klick-Module installieren können.
</Region>
- Sie haben ein 1-Klick-Modul auf Ihrem Hosting erstellt (Wenn Sie diese Installation noch nicht durchgeführt haben, folgen Sie den Anweisungen in unserer Anleitung "[Installation Ihrer Website mit 1-Klick-Modulen (CMS)](/guides/web-cloud/web-hosting/cms-install-1-click-modules)").
...
<Tabs>
  <Tab label="Schritt 1">
    Öffnen Sie die Seite <ManagerLink to="/#/web/hosting">Hosting-Pakete</ManagerLink> und wählen Sie das betreffende Webhosting aus.
  ...
  <Tab label="Schritt 2">
    Klicken Sie auf den Button <code className="action">...</code> rechts in der Zeile für Ihr Modul und dann auf <code className="action">Zugang zum Modul</code>.
  </Tab>
</Tabs>

:::info
Sollte Ihre Website nach diesem Vorgang nicht korrekt angezeigt werden, lesen Sie die zu den Webhosting-Paketen gehörigen OVHcloud Anleitungen unter [Diagnose](/guides/web-cloud/web-hosting/landing-page-web-hosting).
...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Klicken Sie auf den Tab <code className="action">1-Klick-Module</code>. Die Administrator-ID Ihres Moduls wird in der Spalte `Login` angezeigt.
  </Tab>
  <Tab label="Schritt 3">
    Sie können auch die bei der Erstellung Ihres Moduls erhaltene E-Mail über Ihr <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> abrufen: Klicken Sie oben rechts auf Ihren Namen und dann im Kontextmenü auf <code className="action">Meine Kommunikation</code>.
  </Tab>
</Tabs>
...
- PrestaShop: Der Herausgeber dieses Programms bietet keine Dokumentation zur Änderung des Zugangspassworts zum PrestaShop Verwaltungsinterface an. Kontaktieren Sie bitte den Herausgeber zu diesem Thema. Weitere Informationen finden Sie auf der offiziellen [PrestaShop-Seite](https://www.prestashop.com).

:::

Sie können auch das Passwort für den Zugang zum Verwaltungsinterface Ihres CMS direkt über Ihre Datenbank ändern.
...
<Tabs>
  ...
  <Tab label="Schritt 3">
    Andernfalls wechseln Sie zum Tab <code className="action">Meine Seiten</code>. Notieren Sie den Namen des `Wurzelverzeichnis`, das in der Zeile der betreffenden Webseite angezeigt wird: Dies ist das Verzeichnis, in dem sich die Dateien des 1-Klick-Moduls auf dem FTP-Server befinden.
  </Tab>
</Tabs>

Loggen Sie sich anschließend in den [FTP-Bereich](/guides/web-cloud/web-hosting/ftp-connection) Ihres Hostings ein. Öffnen Sie das entsprechende `Wurzelverzeichnis` aus dem Tab <code className="action">Meine Seiten</code> und suchen Sie die Konfigurationsdatei Ihres Moduls:
...
Folgen Sie den Anweisungen in unserer Anleitung "[Ihre Webseite exportieren](/guides/web-cloud/web-hosting/exporter-son-site-web)", um Dateien vom FTP-Bereich Ihres Hostings als auch Datenbanken zu exportieren und zu sichern.
...
<Tabs>
  ...
  <Tab label="Schritt 3">
    Klicken Sie auf den Button <code className="action">...</code> rechts neben der Zeile für Ihr Modul und dann auf den Befehl <code className="action">Das Modul löschen</code>.
...
Wenn Sie die maximale Anzahl an Datenbanken erreicht haben, die in Ihrem [Webhosting](/links/web/hosting) zulässig ist, müssen Sie **ZUERST** die im [ersten Schritt](#step1) identifizierte Datenbank löschen, um ein neues 1-Klick-Modul erstellen zu können.
...
<Tabs>
  ...
  <Tab label="Schritt 2">
    Klicken Sie auf <code className="action">...</code> rechts neben der Datenbank und wählen Sie <code className="action">Die Datenbank löschen</code> aus.
  </Tab>
  <Tab label="Schritt 3">
...
Sichern Sie Ihre Seite entsprechend den Anweisungen in unserer Anleitung "[Die Sicherheit Ihrer Website optimieren](/guides/web-cloud/web-hosting/secure-your-website)".
