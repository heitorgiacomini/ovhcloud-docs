...
description: Erfahren Sie hier, wie Sie die Software Cyberduck verwenden, um sich mit Ihrem OVHcloud Webhosting zu verbinden
...
- Der untere Bereich zeigt den Inhalt, den Sie anzeigen möchten. Wenn Sie z.B. auf das Symbol <code className="action">History</code> klicken, wird eine Liste Ihrer Aktionen angezeigt.
...
Klicken Sie im Hauptmenü ganz oben auf <code className="action">View</code> und dann auf <code className="action">Customize Toolbar...</code>.
...
Ziehen Sie im angezeigten Fenster die gewünschten Elemente auf die Symbolleiste. Wenn Sie beispielsweise die Funktion <code className="action">Download</code> in die Symbolleiste einfügen möchten, ziehen Sie das Symbol <code className="action">Download</code> auf die Symbolleiste. Um die Änderungen zu übernehmen, klicken Sie auf <code className="action">Done</code>.
...
**1.** Klicken Sie in der Symbolleiste auf <code className="action">Open Connection</code>.

**2.** Wählen Sie im Dropdown-Menü (orangefarbener Rahmen des Bildes) <code className="action">SFTP (SSH File Transfer Protocol)</code> aus.
...
:::tip[Success]
- Sie können Ihr Passwort in Cyberduck speichern, indem Sie <code className="action">Add to keychain</code> auswählen. Wenn Sie die Option nicht aktivieren, müssen Sie das Passwort eingeben, um sich erneut mit Ihrem Webhosting-Speicherplatz zu verbinden.
- Wenn Sie nicht alle Ihre FTP-Informationen (Server, Login-Daten, etc.) kennen, lesen Sie die Anleitung „[Mit dem FTP-Speicherplatz Ihres Webhostings verbinden](/guides/web-cloud/web-hosting/ftp-connection)“.

:::


Wenn Sie sich zum ersten Mal mit Ihrem Webhosting verbinden, erscheint ein Fenster mit dem Titel <code className="action">Modified fingerprint</code>. Aktivieren Sie die Option <code className="action">Always</code> und bestätigen Sie. So können Sie den Verbindungshost (OVHcloud) dauerhaft zertifizieren.
...
- Lesen Sie bei Bedarf die Anleitung „[Mit dem FTP-Speicherplatz Ihres Webhostings verbinden](/guides/web-cloud/web-hosting/ftp-connection)“.
...
2. Klicken Sie oben in der Symbolleiste auf den Tab <code className="action">Bookmarks</code> (orangefarbener Rahmen in der Abbildung unten).
3. Klicken Sie unten links im Fenster auf das Symbol <code className="action">+</code>, um ein neues Bookmark hinzuzufügen.
...
Klicken Sie im Menü von Cyberduck auf <code className="action">File</code> und dann auf <code className="action">Upload...</code>. Wählen Sie die Dateien aus, die Sie auf den Server hochladen möchten, und klicken Sie auf <code className="action">Upload</code>.
...
- Indem Sie auf <code className="action">Window</code> (im Hauptmenü), dann auf <code className="action">Transfers</code> klicken
...
Doppelklicken Sie auf eine Datei oder einen Ordner, und wählen Sie <code className="action">Info</code> aus. Das folgende Fenster wird angezeigt:

<img className="thumbnail" alt="hosting" src="/images/assets/screens/other/web-tools/cyberduck/transfert-files-list.png" loading="lazy" />

Klicken Sie auf den Tab <code className="action">Permissions</code> und nehmen Sie die gewünschten Änderungen vor:
...
Klicken Sie im Cyberduck-Menü auf <code className="action">Go</code> und dann auf <code className="action">Send Command...</code>.
...
Fügen Sie im neuen Fenster den Befehl `CHMOD 705 /` ein und klicken Sie auf <code className="action">Send</code>, um den Befehl auszuführen. Zur Bestätigung sollte im unteren Bereich die Meldung `200 Permissions changed on /` erscheinen.
...
[Tutorial - FileZilla mit Ihrem OVHcloud Hosting verwenden](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide)
