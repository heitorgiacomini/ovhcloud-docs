...
En complément au [modèle de responsabilité entre OVHcloud et le client sur le service Serveurs Dédiés](/guides/account-and-service-management/responsibility-sharing/dedicated-servers), cette fiche a pour objectif de présenter les particularités et fonctions de sécurité propres à ce service. Elle met aussi en avant des bonnes pratiques qui permettront au client de l'exploiter au mieux.
...
D'autres guides sont disponibles dans le corpus documentaire sur les Serveurs Dédiés pour assister le client à la prise 
...
| --- | --- | --- | --- | --- |
| - FTP backup ou Backup Storage est un espace de stockage de 500Go mis à disposition du client suite à la souscription au service.<br/> - Le service doit être activé par le client.<br/> - Aucune routine de backup n'est configurée par OVHcloud. | Au choix du client | Dépend du choix du client | N/A | [Utiliser Backup Storage sur un serveur dédié](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) |
| - Backup Storage est une option de stockage supplémentaire qui permet d'avoir un espace disque supplémentaire pouvant atteindre 10To pour déposer des sauvegardes. | Au choix du client | Dépend du choix du client |  N/A | [Utiliser Backup Storage sur un serveur dédié](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) |
...
A travers son espace client OVHcloud, le client a la possibilité de gérer le service à l'aide de [trois contacts types](/guides/account-and-service-management/account-information/managing-contacts#definition).
...
| Windows | [https://docs.microsoft.com/fr-fr/windows/security/threat-protection/windows-security-configuration-framework/windows-security-baselines](https://docs.microsoft.com/fr-fr/windows/security/threat-protection/windows-security-configuration-framework/windows-security-baselines) |
...
Les prérequis et modes d'emploi sont disponibles sur [ce lien](/guides/bare-metal-cloud/dedicated-servers/bring-your-own-image).
...
Afin de continuer à bénéficier du service de monitoring OVHcloud, le client doit configurer [des règles de filtrage](/guides/bare-metal-cloud/dedicated-servers/network-ip-monitoring) sur le pare-feu interne de ses serveurs et sélectionner les autres services dont il souhaite suivre l'état.

OVHcloud propose une fonctionnalité appelée [OVHcloud Link Aggrégation](/guides/bare-metal-cloud/dedicated-servers/ola-enable-manager) qui peut être activée par le client et qui lui permet de bénéficier d'un réseau privé à haut débit et redondé pour ses Serveurs Dédiés.
...
Les principe de portabilité d'OVHcloud sont décrits dans sa propre [politique de portabilité](/guides/account-and-service-management/reversibility/global-reversibility-policy) et ceux spécifiques au service Serveurs Dédiés sont indiqués dans sa [politique spécifique](/guides/account-and-service-management/reversibility/dedicated-servers-reversibility-policy).
