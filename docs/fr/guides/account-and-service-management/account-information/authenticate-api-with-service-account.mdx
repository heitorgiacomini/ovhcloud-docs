...
description: Comment se connecter aux API avec ses comptes de service OVHcloud grâce au protocole OAuth2
...
- Savoir configurer des [politiques d'accès via API](/guides/account-and-service-management/account-information/iam-policies-api).
- Savoir utiliser les [APIs de OVHcloud](/guides/manage-and-operate/api/first-steps).
- Avoir créé un [compte de service via API](/guides/manage-and-operate/api/manage-service-account).
...
Avant toute chose, il faut que notre compte de service puisse accéder aux APIs utiles. OVHcloud fournit des droits pour chaque appel d'API. Pour trouver quelles sont les autorisations nécessaires, vous pouvez vous rendre sur la console des API de OVHcloud :
