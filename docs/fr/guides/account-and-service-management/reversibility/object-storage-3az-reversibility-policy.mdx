...
| **Fonctionnalité** | **Description** | **Formats disponibles** | **Modèle de migration** | **Documentation disponible** |
| --- | --- | --- | --- | --- |
| Offsite Replication OVHcloud | Réplication automatique des données vers une région OVHcloud 1-AZ distincte | S3 (objets répliqués) | **Entrant** : Non applicable à l'importation directe, nécessite une configuration OVHcloud.<br/> **Sortant** : Restauration manuelle depuis la région répliquée, puis export vers la cible externe (nécessite adaptation si la cible n’a pas de mécanisme équivalent). | [Cross-region replication](/guides/storage-and-backup/object-storage/s3-asynchronous-replication) |
| Object Lock (verrouillage d’objet) | Immuabilité des objets (WORM), empêche suppression/modification pendant une période définie | S3 (object lock) | **Entrant** : Nécessite que la source supporte l’Object Lock S3. <br/>**Sortant** : Export possible, mais la cible doit supporter le verrouillage S3 ou adaptation manuelle des politiques d’immuabilité. | [Object storage Object Lock](/guides/storage-and-backup/object-storage/s3-managing-object-lock) |
| ACL et gestion avancée des accès | Gestion des droits d'accès via ACL S3, ou politiques spécifiques | S3 (ACL, politiques) | **Entrant** : Adaptation des droits en fonction de la structure cible.<br/> **Sortant** :Export des objets, mais reconfiguration des ACL/politiques nécessaire sur la plateforme de destination  | [Gestion des accès](/guides/storage-and-backup/object-storage/s3-bucket-acl) |

### Fonctionnalités spécifiques

| **Fonctionnalité** | **Description** | **Formats disponibles** | **Modèle de migration** | **Documentation disponible** |
| --- | --- | --- | --- | --- |
| Gestion via l'espace client OVHcloud / API OVHcloud | Interface graphique et API propriétaires OVHcloud pour la gestion du stockage | N/A | **Entrant** : Non applicable.<br/> **Sortant** : Scripts/API à réécrire pour l'environnement cible. | [Premiers pas avec Object Storage](/guides/storage-and-backup/object-storage/s3-getting-started-with-object-storage) |
...
Les partenaires OVHcloud sont répertoriés avec le mot-clé « Cloud Migration » sur notre [page dédiée](https://partner.ovhcloud.com/fr/directory/).
...
Un point d’attention doit être néanmoins porté à la fonctionnalité Object Lock. Retrouvez plus d’informations sur notre guide de [gestion de l'immuabilité des objets avec Object Lock (WORM)](/guides/storage-and-backup/object-storage/s3-managing-object-lock).
