...
description: "Découvrez comment configurer External Secret Operator pour stocker les secrets Kubernetes sur le Secret Manager d'OVHcloud"
...
- Avoir un cluster [Managed Kubernetes Service](https://www.ovhcloud.com/fr/public-cloud/kubernetes/).
...
Créez un [utilisateur local IAM](/guides/account-and-service-management/account-information/ovhcloud-users-management) avec les droits d'accès à votre domaine.

L'utilisateur doit appartenir à un groupe avec le rôle ADMIN, ou si vous utilisez des [politiques IAM](/guides/account-and-service-management/account-information/iam-policy-ui), il doit avoir au moins les droits suivants sur le domaine OKMS :
...
Vous aurez également besoin de la `region` et de l'`okms-id` du domaine OKMS que vous souhaitez utiliser. Cet ID et cette région peuvent être trouvés sur le <ManagerLink to="/#/dedicated/security/kms">dashboard de votre domaine OKMS</ManagerLink> ou via l'[OVHcloud CLI](https://github.com/ovh/ovhcloud-cli) :
...
<Api version="v1" section="/location" method="GET" route={"/location"} />
...
Pour plus d'informations sur la gestion de l'External Secret Operator, veuillez consulter la documentation dédiée, en utilisant le fournisseur HashiCorp Vault : [https://external-secrets.io/latest/](https://external-secrets.io/latest/).
