...
description: "Découvrez comment configurer Kubernetes pour chiffrer le stockage ETCD avec l'interface KMIP d'OVHcloud KMS"
...
- Vous devez avoir [commandé un domaine OKMS](/guides/manage-and-operate/kms/quick-start).
...
Créez un [utilisateur local IAM](/guides/account-and-service-management/account-information/ovhcloud-users-management) avec des droits d'accès sur votre domaine.

Si vous utilisez des [politiques IAM](/guides/account-and-service-management/account-information/iam-policy-ui) à la place, l'utilisateur doit avoir au moins les droits suivants sur le domaine OKMS :
...
Créez un [certificat d'accès OKMS](/guides/manage-and-operate/kms/okms-certificate-management) et liez l'utilisateur précédemment créé.
...
| `--kmip-addr`   | Adresse du serveur KMIP. Disponible dans le <ManagerLink to="/#/dedicated/security/kms">dashboard de votre domaine OKMS</ManagerLink>. (par exemple : `eu-west-rbx.okms.ovh.net:5696`). | `""` (obligatoire)               |
