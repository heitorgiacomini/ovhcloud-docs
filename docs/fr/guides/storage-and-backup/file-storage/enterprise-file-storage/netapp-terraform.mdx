...
description: Découvrez comment gérer votre service Enterprise File Storage avec Terraform
...
- Avoir accèss aux [API OVHcloud](https://eu.api.ovh.com/)
- Avoir un service Enterprise File Storage dans votre compte OVHcloud. Le service peut être commandé depuis la [page produit](https://www.ovhcloud.com/fr/storage-solutions/enterprise-file-storage/), depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>, ou en utilisant la ressource `ovh_storage_efs` (voir [Commander un service](#commander-un-service)).
...
Vous pouvez suivre le guide [Premiers pas avec les API OVHcloud](/guides/manage-and-operate/api/first-steps) pour générer votre token d'API.
...
:::tip
Si vous n'êtes pas familiarisé avec l'utilisation des API OVHcloud, veuillez consulter notre guide sur les [Premiers pas avec les API OVHcloud](/guides/manage-and-operate/api/first-steps).
...
- **Méthode via l'espace client OVHcloud** : Accédez à la section <ManagerLink to="/">Enteprise File Storage</ManagerLink>
...
:::warning
Pour commander un produit via Terraform, votre compte doit avoir un moyen de paiement par défaut défini. Cela se fait depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> ou via l'API avec l'endpoint [/me/payment/method](https://api.ovh.com/console/#/me/payment/method~GET).
