...
description: "Découvrez comment déployer la stack d'infrastructure NetApp Trident CSI avec Enterprise File Storage et Managed Kubernetes Service via Terraform."
...
**Découvrez comment déployer et configurer un ensemble de services OVHcloud avec Terraform pour provisionner des volumes EFS avec NetApp Trident CSI.**
...
<img className="thumbnail" alt="Architecture du module Terraform" src="/images/storage-and-backup/file-storage/enterprise-file-storage/netapp-trident-terraform/terraform-module-architecture.png" loading="lazy" />
...
| Passerelle requise | Le cluster MKS nécessite une passerelle pour joindre l'API OVHcloud |
...
Le provider Terraform OVHcloud doit être configuré avec un token d'API pour appeler l'API OVHcloud.

Votre token d'API doit disposer des droits suivants :
...
Suivez le guide [Premiers pas avec les API OVHcloud](/guides/manage-and-operate/api/first-steps/) pour générer votre token d'API.
...
:::info
Choisissez une `region` EFS et une `public_cloud_region` MKS aussi proches que possible.

La latence entre régions peut impacter les performances de vos charges de travail de stockage.
**Gardez votre stockage et vos ressources de calcul aussi proches que possible.**
...
Consultez la [documentation des régions OVHcloud](/links/bare-metal/regions) pour le mapping complet.
:::
...
module "ovh_efs_trident" {
  ...
  # Ignorer le binding entre vRack et vRack services s'il est déjà configuré
...
Après avoir déployé l'infrastructure avec ce module, configurez Trident CSI pour utiliser Enterprise File Storage. Suivez le guide [Premiers pas avec Trident CSI](/guides/storage-and-backup/file-storage/enterprise-file-storage/netapp-trident-csi/) pour des instructions détaillées.
...
- [Premiers pas avec Trident CSI](/guides/storage-and-backup/file-storage/enterprise-file-storage/netapp-trident-csi/)
- [Gérer Enterprise File Storage avec le provider Terraform OVHcloud](/guides/storage-and-backup/file-storage/enterprise-file-storage/netapp-terraform/)
