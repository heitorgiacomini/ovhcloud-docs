...
description: Découvrez comment configurer et gérer le service File Storage d’OVHcloud avec votre projet OpenStack. Ce guide couvre l’installation de la CLI, la création de shares, l’accès des clients et le montage sur vos machines virtuelles.
...
- Vous disposez déjà d’un [réseau privé](/guides/public-cloud/network-services/vrack) dans votre projet Public Cloud
- Une [instance Public Cloud](https://www.ovhcloud.com/fr/public-cloud/) dans votre compte OVHcloud
- Un [environnement CLI OpenStack prêt à l’emploi](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment)
...
<Tabs>
  <Tab label="Via l'espace client OVHcloud">
    ...
    **1\. Créer un share**
    ...
    Dans le menu latéral gauche, accédez à <code className="action">Storage & backup</code> > <code className="action">File Storage</code>, puis cliquez sur <code className="action">Créer un share</code>.
    
    Renseignez le nom du share, puis appliquez un filtre selon le mode de déploiement afin de sélectionner la région cible.
    
    Définissez le niveau de performance et la capacité (en Gio), associez un réseau privé, puis cliquez sur <code className="action">Valider</code> pour finaliser la création du share.
    
    **2\. Autoriser une machine virtuelle cliente (ACL)**
    
    :::info
    Récupérez l'[adresse IP privée](/guides/public-cloud/network-services/vrack) de votre machine virtuelle cliente et assurez-vous qu'elle se trouve sur le même réseau privé que le share.
    :::

    
    Une fois le share créé, cliquez dessus pour ouvrir son tableau de bord, puis accédez à l’onglet <code className="action">Liste de contrôle d’accès (ACL)</code>.
    
    Cliquez sur <code className="action">Ajouter un nouvel accès</code>, renseignez l’adresse IP privée ou une plage CIDR (par exemple, `10.1.0.123` ou `10.1.0.0/24`), puis sélectionnez les droits d’accès :
    
    - <code className="action">Lecture seule</code> : accès en lecture uniquement
    - <code className="action">Lecture et écriture</code> : accès complet en lecture/écriture
    ...
    **3\. Monter le share sur votre machine virtuelle cliente**
    ...
    Récupérez le chemin de montage (affiché sous le label `Chemin de montage` dans l’onglet <code className="action">Informations générales</code> de votre share), puis connectez-vous à votre machine virtuelle cliente et installez les utilitaires NFS :
  ...
  <Tab label="Via l’API OVHcloud">
    **1\. Créer un share**
    ...
    <Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/network"} />
    ...
    **Note :** Sélectionnez uniquement un réseau privé.
    ...
    <Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/network/\{networkId\}/subnet"} />
    ...
    <Api version="v1" section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share"} />
    ...
    **Note :** L'identifiant du share doit avoir le format `abc12345-def6-4abc-8def-123456abcdef`.
    ...
    <Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{id\}"} />
    ...
    **2\. Autoriser une machine virtuelle cliente**
    ...
    <Api version="v1" section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{id\}/acl"} />
    ...
    <Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{id\}/acl/\{aclId\}"} />
  ...
  <Tab label="Via la CLI OpenStack avec le plugin Manila">
  ...
  <Tab label="Via Manila CSI dans l'environnement K8s">
    ...
    **4\. Installation de la CLI OpenStack**
    ...
    Préparez votre environnement pour utiliser l'API OpenStack en installant python-openstackclient, en suivant [ce guide de préparation de l'environnement OpenStack](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment).
  ...
  <Tab label="Via Terraform">
    **1\. Prérequis additionnels**
    ...
    - Vous disposez déjà d'un environnement [Terraform](/guides/public-cloud/cross-functional/how-to-use-terraform) fonctionnel.
