...
description: "Découvrez comment étendre la capacité de stockage de votre Cloud Disk Array (CDA) OVHcloud à l’aide de l'API OVHcloud"
...
<summary>**Déroulement de la mise à niveau**</summary>
...
<summary>**Étape 1. Obtenez le nom de votre service (passez à l'étape suivante si vous le connaissez déjà)**.</summary>
...
Cette commande renvoie une liste de vos services CDA. Chaque service est identifié par un UUID, par exemple 48e5f77f-427b-4261-9799-7861033659fb
  ...
  <Tab label="Via l'espace client OVHcloud">
    Dans la page de votre service Cloud Disk Array, accédez à la section « Détails » et localisez le champ « ID ». Cette valeur est le nom de votre service CDA.
    
    <img className="thumbnail" alt="Ceph details" src="/images/storage-and-backup/block-storage/cloud-disk-array/ceph-grow-with-api/ceph_details.png" loading="lazy" />
  </Tab>
</Tabs>
...
<summary>**Étape 2. Identifiez le code de votre plan (passez à l'étape suivante si vous savez quel plan vous utilisez)**</summary>
...
<Api version="v1" section="/services?resourceName" method="GET" route={"/services?resourceName=\{serviceName\}"} />
...
<Api version="v1" section="/services/{serviceId}" method="GET" route={"/services/\{serviceId\}"} />
...
<Api version="v1" section="/order/upgrade/cephaas/{serviceName}/{planCode}" method="POST" route={"/order/upgrade/cephaas/\{serviceName\}/\{planCode\}"} />
...
Rendez-vous sur notre chaîne Discord dédiée : [https://discord.gg/ovhcloud](https://discord.gg/ovhcloud). Posez des questions, ajoutez des commentaires et interagissez directement avec l'équipe qui construit nos services de stockage et de sauvegarde.
