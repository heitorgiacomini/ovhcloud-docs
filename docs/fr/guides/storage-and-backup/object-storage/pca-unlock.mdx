...
- Dégel via python-swiftclient :
    - [Préparer l’environnement pour utiliser l’API OpenStack](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) en installant python-swiftclient.
...
Cliquez sur la rubrique <code className="action">Cloud Archive</code> dans le menu de gauche.

Pour dégeler une archive, cliquez sur le bouton <code className="action">...</code> à droite de celle-ci, puis sur <code className="action">Dégeler</code> afin d'entamer le processus de récupération.

<img className="thumbnail" alt="dégel" src="/images/storage-and-backup/object-storage/pca-unlock/unfreeze.png" loading="lazy" />
...
<img className="thumbnail" alt="délai avant dégel" src="/images/storage-and-backup/object-storage/pca-unlock/unfreeze_result.png" loading="lazy" />

Votre fichier sera disponible au téléchargement à l'issue de ce délai. Vous pourrez alors entammer le téléchargement directement par le navigateur ou via un [client Swift/SFTP/SCP](/guides/storage-and-backup/object-storage/pca-sftp).
