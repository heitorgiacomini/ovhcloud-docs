...
description: Restaurer des objets depuis la classe Cold Archive
...
- Avoir un [utilisateur Object Storage](/guides/storage-and-backup/object-storage/s3-identity-and-access-management) déjà créé
...
La classe de stockage **Cold Archive** est une classe de stockage d'archives. Existant également en tant que [produit dédié chez OVHcloud](/guides/storage-and-backup/object-storage/cold-archive-getting-started) (granularité au niveau du bucket), Cold Archive est désormais disponible en tant que classe Object Storage pour un bucket dit « general-purpose ».
...
- Si vous utilisez [l'upload multipart (MPU)](/guides/storage-and-backup/object-storage/s3-performance-optimization), vous devez savoir que les parties en cours sont facturées au tarif de la classe Standard jusqu'à ce que vous complétiez le téléchargement.
...
Vous pouvez restaurer un objet dans la classe de stockage Cold Archive en utilisant l' <ManagerLink to="/">espace client OVHcloud</ManagerLink>, les API OVHcloud ou en utilisant l'API REST Amazon S3 ou la CLI.

## Restaurer un objet archivé

<Tabs>
  <Tab label="Via l'espace client OVHcloud">
    Cliquez sur <code className="action">Object Storage</code> dans la barre de navigation de gauche, puis sur l'onglet <code className="action">Mes conteneurs</code>.
  ...
  <Tab label="Via AWS CLI">
    ...
    La classe Cold Archive est mappée avec le tier AWS S3 équivalent `DEEP_ARCHIVE`. Pour en savoir plus sur le mappage entre les niveaux OVHcloud Object Storage et les niveaux AWS S3, veuillez consulter le guide « [Object Storage - Endpoints et géo-disponibilité](/guides/storage-and-backup/object-storage/s3-location) ».
