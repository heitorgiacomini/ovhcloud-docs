...
description: Découvrez comment migrer un Object Storage Swift OVHcloud vers un Object Storage OVHcloud compatible S3 en utilisant Rclone
...
Si vous souhaitez en savoir plus sur les offres et classes d'Object Storage disponibles, référez-vous à notre guide « [Object Storage - Choisir une classe de stockage adaptée à vos besoins](/guides/storage-and-backup/object-storage/s3-choosing-the-right-storage-class-for-your-needs) ».
...
- Pour votre conteneur source : nous vous recommandons d'utiliser votre utilisateur OpenStack et le fichier de configuration Rclone associé depuis <ManagerLink to="/">l'espace client OVHcloud</ManagerLink>. Suivez les étapes de [ce guide](/guides/storage-and-backup/object-storage/pcs-sync-rclone-object-storage) pour accéder au fichier de configuration associé à votre utilisateur OpenStack.
...
:::info
Pour obtenir la liste des endpoints des régions OVHcloud, consultez notre guide « [Object Storage - Endpoints et géo-disponibilité de l’Object Storage](/guides/storage-and-backup/object-storage/s3-location) ».

:::


Vous pouvez ensuite tester vos deux connexions en utilisant la commande `rclone config` :
...
Vous pouvez utiliser la commande `rclone sync` pour migrer un ou tous les buckets. Comme indiqué dans la documentation, la commande `rclone sync` rendra la source et la destination identiques. Soyez prudent lors de son utilisation.

Vous pouvez également utiliser la commande `rclone copy` qui copiera les fichiers de votre source vers votre destination.

Dans les deux cas, n'oubliez pas de remplacer `<source_container_name>` et `<destination_bucket_name>` par le nom de votre conteneur source Swift OVHcloud et le nom de votre bucket de destination OVHcloud compatible S3, respectivement :
...
Nous vous recommandons de comparer les buckets source et destination après la migration. Vos buckets source et destination OVHcloud peuvent être comparés en ligne de commande ou directement depuis <ManagerLink to="/">l'espace client OVHcloud</ManagerLink>
