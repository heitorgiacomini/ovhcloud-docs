...
description: "Découvrez comment utiliser l'API Swift S3"
...
- [Obtenir le fichier Openstack RC v3 d'Horizon](/guides/public-cloud/cross-functional/compute-horizon-access-security)
...
<Tabs>
  <Tab label="Paquet Python">
    ```bash
    user@host:~$ pip install awscli
    ```
  </Tab>
  <Tab label="OS basé sur Debian">
    ```bash
    user@host:~$ sudo apt install awscli
    ```
  </Tab>
  <Tab label="OS basé sur RHEL">
    ```bash 
