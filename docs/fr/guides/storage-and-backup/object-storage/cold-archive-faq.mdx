...
description: FAQ sur la solution Cold Archive
...
Du point de vue de la restauration, la granularité est également différente : d'une part, avec Cold Archive v1, la restauration est effectuée pour l'ensemble du bucket (plus d'informations [ici](/guides/storage-and-backup/object-storage/cold-archive-overview)), tandis que pour Cold Archive v2, la restauration est disponible à l'objet (plus d'informations [ici](/guides/storage-and-backup/object-storage/s3-restoring-objects)).
...
| Compatibilité S3 | limitée, les détails peuvent être trouvés [ici](/guides/storage-and-backup/object-storage/cold-archive-getting-started) | complète (réplication asynchrone, lifecycle, versioning, object lock...) |
| Facturation | Object Storage Standard pour le stockage temporaire - frais de suppression anticipée (durée de stockage minimale de 180 jours) - frais de restauration - minimum 1 Tio par bucket - voir [tarification](https://www.ovhcloud.com/fr/public-cloud/old-prices/) | frais de suppression anticipée (durée de stockage minimale de 180 jours) - frais de restauration - voir [tarification](https://www.ovhcloud.com/fr/public-cloud/prices/) |
...
Retrouvez une présentation de l'offre sur [cette page](/guides/storage-and-backup/object-storage/cold-archive-overview).

### Comment utiliser le service pour la première fois ? 

Pour utiliser le service, les prérequis sont les suivants :

- Avoir un [projet Public Cloud](/guides/public-cloud/cross-functional/create-a-public-cloud-project) dans votre compte OVHcloud. 
- Etre connecté à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>.
- Avoir créé un [utilisateur Object Storage](/guides/storage-and-backup/object-storage/s3-getting-started-with-object-storage).

### Quelles sont les fonctionnalités disponibles avec Cold Archive ?

L'ensemble des fonctionnalités de nos classes de stockage Object Storage sont supportées par Cold Archive (v1 et v2). Vous trouverez la liste des fonctions sur [cette page](/guides/storage-and-backup/object-storage/s3-s3-compliancy).
...
Dans l'espace client OVHcloud, l'action est réalisée en activant le bouton <code className="action">Archiver</code>. 
...
Par API, les commandes sont décrites dans [notre documentation](/guides/storage-and-backup/object-storage/cold-archive-getting-started).
...
Dans l'interface utilisateur, cliquez sur <code className="action">Restaurer une archive</code>.  
...
Dans l'interface utilisateur, vous supprimez votre archive en sélectionnant les boutons <code className="action">Vider l'archive</code>.
...
Une fois les objets supprimés, le bucket peut être supprimé en sélectionnant <code className="action">Supprimer le conteneur</code>.

Retrouvez le descriptif des commandes par API dans notre guide « [Premiers pas avec Cold Archive](/guides/storage-and-backup/object-storage/cold-archive-getting-started) ».
...
Par exemple, vous installez un serveur, vous paramétrez Rclone pour synchroniser vos fichiers d'un Object Storage (cloud provider tiers) vers OVHcloud Object Storage, en suivant le guide : [Object Storage - Utiliser Object Storage avec Rclone](/guides/storage-and-backup/object-storage/s3-rclone).
...
Lors du téléchargement d'objets dans la classe Cold Archive, les objets sont facturés au tarif Cold Archive disponible sur [cette page](https://www.ovhcloud.com/fr/public-cloud/prices/).
...
En complément, nous vous recommandons de chiffrer vos données avant de les envoyer. Vous pouvez utiliser plusieurs options de chiffrement. Retrouvez-les en détails [ici](/guides/storage-and-backup/object-storage/s3-encrypt-your-objects-with-sse-c).
...
<img className="thumbnail" alt="redondance" src="/images/storage-and-backup/object-storage/cold-archive-faq/Cold-Archive-Architecture.png" loading="lazy" />
