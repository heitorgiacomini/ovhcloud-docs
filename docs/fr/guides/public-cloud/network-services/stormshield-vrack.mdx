...
description: Découvrez comment sécuriser votre infrastructure OVHcloud avec Stormshield Network Security déployé sur Public Cloud
...
Si vous éprouvez des difficultés à appliquer ces instructions, nous vous recommandons de faire appel à un [prestataire spécialisé](https://partner.ovhcloud.com/fr/directory/) et/ou discuter du problème avec notre communauté. Pour plus d'informations, consultez la section [Aller plus loin](#gofurther) de ce tutoriel.
...
- Un [utilisateur OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user) (facultatif).
...
- Un bloc d'adresses [Additional IP](https://www.ovhcloud.com/fr/network/additional-ip/) (/29) pour permettre le failover et la configuration de la haute disponibilité.
...
Reportez-vous au guide « [Configurer un bloc IP dans un vRack](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack) » pour plus d'informations.
...
Dans la section <code className="action">Public Cloud</code>, sélectionnez votre projet. Dans le menu de gauche, cliquez sur <code className="action">Instances</code> sous l'onglet **Compute**, puis retrouvez vos deux instances SNS EVA.
