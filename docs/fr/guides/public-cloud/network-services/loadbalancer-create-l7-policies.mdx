...
description: Découvrez comment configurer et gérer des Policies et règles de niveau 7 (L7) pour les Load Balencers Public Cloud OVHcloud
...
- Comprendre les [concepts du OVHcloud Load Balancer](/guides/public-cloud/network-services/load-balancer-concepts).
- Un [projet Public Cloud OVHcloud](/guides/public-cloud/cross-functional/create-a-public-cloud-project).
- Un Load Balancer [déjà configuré](/guides/public-cloud/network-services/getting-started-load-balancer) dans votre projet Public Cloud.
...
- **CLI OpenStack :** pour la gestion via la ligne de commande. Retrouvez plus d'informations pour préparer votre environnement API OpenStack sur [ce guide](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment).
- **Interface Horizon :** offre une gestion graphique via OpenStack pour les utilisateurs familiarisés avec cette plateforme. Retrouvez plus de détails supplémentaires sur [cette page](/guides/public-cloud/cross-functional/introducing-horizon).
...
Dans votre projet Public Cloud, sélectionnez <code className="action">Load Balancer</code>, vous verrez apparaître la liste de vos Load Balancers. Cliquez sur celui que vous souhaitez configurer.

#### Etape 3 - Gérer les Politiques L7

Après avoir sélectionné votre Load Balancer, vous accéderez à son interface de gestion. Cliquez sur l'onglet <code className="action">Listeners</code> pour afficher la liste des listeners.

Pour accéder aux options de gestion des politiques L7 d'un listener spécifique, cliquez sur le bouton <code className="action">...</code> situé à droite du listener que vous souhaitez configurer. 

Sélectionnez l'option <code className="action">Voir les L7 policies</code> dans le menu déroulant.

<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy9.png" loading="lazy" />
...
Une fois dans la section des politiques L7, vous pouvez voir les politiques existantes ou cliquer sur <code className="action">Ajouter une L7 policy</code> pour créer une nouvelle politique, selon les options disponibles dans l'interface.

<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy10.png" loading="lazy" />
...
<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy11.png" loading="lazy" />

Après avoir rempli tous les champs nécessaires, cliquez sur **Ajouter** pour créer votre politique L7.

<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy4.png" loading="lazy" />
...
Sur la page des politiques L7 de votre listener, localisez la politique pour laquelle vous souhaitez ajouter des règles. Cliquez sur le bouton <code className="action">...</code> à côté de la politique concernée et sélectionnez <code className="action">Gérer les L7 rules</code>.

<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy5.png" loading="lazy" />

2\. **Ajouter une nouvelle règle L7**

Sur la page des règles L7, cliquez sur le bouton <code className="action">Ajouter une L7 rule</code>.

<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy6.png" loading="lazy" />
...
<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy7.png" loading="lazy" />

5\. **Enregistrement de la règle**

Une fois tous les champs remplis selon vos critères, cliquez sur <code className="action">Ajouter</code> pour enregistrer la règle à votre politique L7.

<img className="thumbnail" alt="Schema 1" src="/images/public-cloud/network-services/technical-resources-04-lbaas-create-L7/Policy8.png" loading="lazy" />
...
- Pour vous connecter avec l'authentification unique OVHcloud : utilisez le lien <code className="action">Horizon</code> dans le menu de gauche sous « Interfaces de gestion » après avoir ouvert votre projet <code className="action">Public Cloud</code> dans l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>.

- Pour vous connecter avec un utilisateur OpenStack spécifique : ouvrez la [page de connexion à Horizon](https://horizon.cloud.ovh.net/auth/login/) et entrez les [identifiants de l'utilisateur OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user) précédemment créées, puis cliquez sur <code className="action">Se connecter</code>.

Dans le tableau de bord d'Horizon, sélectionnez votre <code className="action">Projet</code>.<br/>
Dans le menu de gauche, cliquez sur l'onglet <code className="action">Network</code> et sélectionnez <code className="action">Load Balancers</code>.<br/>
...
Dans les détails du listener, localisez et cliquez sur l'onglet <code className="action">L7 policies</code>. Cet onglet liste toutes les politiques existantes.

Pour ajouter une nouvelle politique, cliquez sur le bouton <code className="action">Create L7 Policy</code>.
...
Cliquez sur <code className="action">Add Rule</code> pour configurer les règles qui détermineront les conditions précises sous lesquelles la politique s'appliquera.
