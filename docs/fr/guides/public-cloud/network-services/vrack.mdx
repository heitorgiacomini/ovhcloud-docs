...
Vous pouvez accéder aux API depuis [notre page web](/links/api) ou créer vos scripts PHP ou Python pour y faire appel.
...
Le [CLI ovhcloud](https://github.com/ovh/ovhcloud-cli) permet de gérer vos réseaux privés vRack et les interfaces réseau de vos instances Public Cloud directement depuis le terminal.

Consultez la [documentation officielle](https://github.com/ovh/ovhcloud-cli) pour l’installation et la configuration.
...
Vous êtes redirigé vers une autre page pour valider la commande ; l'opération prend quelques minutes.
    ...
    Pour continuer depuis l'espace client OVHcloud, poursuivez à l'[étape 2 : Créer un réseau privé dans le vRack](#create-pn-in-vrack), onglet **Depuis l'espace client OVHcloud**.
  </Tab>
...
Sélectionnez l'interface à supprimer et validez :
  ...
  <Tab label="Depuis la CLI OpenStack">
    **Récupération des informations nécessaires**
    ...
    :::info
    Notez les ID des réseaux qui vous intéressent :
    
    - Ext-Net pour une IP publique.
    - Celui du ou des VLAN nécessaires à votre configuration.
    ...
    **Ajout d'une interface réseau privée**
    ...
    Pour attacher une nouvelle interface :
    ...
    **Détachement d'une interface réseau**

    :::warning
    Détacher une interface réseau entraîne sa suppression immédiate. Si vous détachez l'interface publique (Ext-Net), l'adresse IP sera relâchée et ne pourra pas être récupérée.
    :::

    Identifiez le port à supprimer :

    ```bash
    openstack port list --server <instance_id>
    ```

    Puis détachez l'interface :
  ...
  <Tab label="Depuis le CLI ovhcloud">
    Utilisez le [CLI ovhcloud](https://github.com/ovh/ovhcloud-cli) pour gérer les interfaces réseau d'une instance existante. Définissez votre projet cloud avec `--cloud-project <project_id>` ou configurez-le dans votre profil.

    **Lister les interfaces de l'instance**

    ```bash
    ovhcloud cloud instance interface list <instance_id> \
      --cloud-project <project_id>
    ```
    ...
    **Attacher une interface réseau privée**
    ...
    ```bash
    ovhcloud cloud instance interface create <instance_id> <network_id> \
      --cloud-project <project_id>
    ```

    Pour attacher avec une adresse IP fixe :

    ```bash
    ovhcloud cloud instance interface create <instance_id> <network_id> <ip_address> \
      --cloud-project <project_id>
    ```
    ...
    **Détacher une interface réseau**
    ...
    ```bash
    ovhcloud cloud instance interface delete <instance_id> <interface_id> \
      --cloud-project <project_id>
    ```
  </Tab>
</Tabs>
...
Si vous détachez l’interface « Ext-Net » (IP publique), cette adresse sera relâchée et remise en circulation. Vous ne pourrez pas vous la réattribuer.<br/>
Cette action n’est à effectuer que si vous souhaitez isoler votre serveur dans le vRack (réseau privé) ou le retirer d’un ou de plusieurs VLAN.

:::


Pour détacher une interface réseau via la CLI OpenStack, identifiez d’abord le port à supprimer :
...
+--------------------------------------+------+-------------------+-------------------------------------------------------------------------------------------+
| 12345678-abcd-ef01-2345-678910abcdef |      | fa:xx:xx:xx:xx:xx | ip_address=’192.168.0.xx’, subnet_id=’01234567-8901-abscdef12345678910abcd’               |
| 09876543-210a-bcde-f098-76543210abcd |      | fa:yy:yy:yy:yy:yy | ip_address=’2001:41d0:xxx:xxxx::xxxx’, subnet_id=’65432109-abcd-ef09-8765-43210abcdef1’   |
+--------------------------------------+------+-------------------+-------------------------------------------------------------------------------------------+
```

Puis supprimez le port :
