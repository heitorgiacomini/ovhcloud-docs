...
**Get existing OIDC configuration**
...
```bash
ovhcloud cloud managed-registry oidc create <registry_id> \
  --cloud-project <project_id> \
  --name keycloak \
  --endpoint https://xxx.yyy.com/realms/mpr \
  --client-id <client_id> \
  --client-secret <client_secret> \
  --scope "openid,profile,email,offline_access" \
  --groups-claim groups \
  --admin-group harbor-admin \
  --verify-cert \
  --delete-users
```

:::info
The `--delete-users` flag is required if local users already exist in the registry. Without it, the command will fail if any local users are present.
:::

After configuration, the <code className="action">Login via OIDC provider</code> button will appear in the Harbor login page.

**Get current OIDC configuration**

```bash
ovhcloud cloud managed-registry oidc get <registry_id> --cloud-project <project_id>
```

**Update OIDC configuration**

For example, to add or change the admin group:

```bash
ovhcloud cloud managed-registry oidc edit <registry_id> \
  --cloud-project <project_id> \
  --admin-group harbor-admin \
  --groups-claim groups
```

When an `adminGroup` is configured, all users belonging to this group will have admin rights on Harbor.

**List users**

```bash
ovhcloud cloud managed-registry users list <registry_id> --cloud-project <project_id>
```

**Set a user as admin**

```bash
ovhcloud cloud managed-registry users set-as-admin <registry_id> <user_id> \
  --cloud-project <project_id>
```
...
```bash
ovhcloud cloud managed-registry oidc delete <registry_id> --cloud-project <project_id>
```

:::info
You can generate new user credentials via the OVHcloud Control Panel or CLI and log in with these new local credentials after deleting the OIDC configuration.
:::
