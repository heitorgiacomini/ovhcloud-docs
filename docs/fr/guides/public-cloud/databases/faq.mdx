...
Since we bill per second, this operation can be repeated as much as you need (and without a strong billing increase).
...
You don't pay before using the service. At the end of the month, we summarize your spendings in one bill per project. Public Cloud Databases are billed per second, for the service range and flavor selected during DB instance creation, with a minimum billing period of 1 minute.
