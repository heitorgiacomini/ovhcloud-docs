...
Ce fonctionnement ressemble à celui de certains opérateurs téléphoniques qui facturent en fin de mois les minutes de communication consommées. Ici, OVHcloud facture le temps exact — à la seconde près — d'utilisation d'un serveur, d'un espace de stockage ou d'un autre élément du service.
...
Le but est de fournir une facturation qui correspond au mieux à l'utilisation qu'ont les clients de ce service. Pour répondre à cet objectif, il est nécessaire de rendre la facturation granulaire, c'est pourquoi l'unité est le temps, compté en secondes.

Pour chaque ressource sujette à facturation, un compteur d'usage (en secondes) démarre à la création de la ressource et se termine à sa suppression, selon le principe où chaque seconde d'utilisation est due au-delà d'une durée minimale de facturation d'une minute — sans arrondi à l'heure supérieure.
...
Chaque projet cloud a donc sa propre facture rassemblant l'ensemble des ressources facturées dans le mois, cette facture est éditée le 1er du mois suivant. Les prix sont généralement affichés sous forme de tarif horaire pour plus de lisibilité, mais la facturation réelle se fait à la seconde.
...
L'utilisation étant mesurée à la seconde (au-delà d'une minute minimale), les périodes d'utilisation plus courtes sont facturées avec la même précision : par exemple, si vous utilisez une ressource pendant seulement 20 minutes, vous ne serez facturé que pour ces 20 minutes, sans arrondi à l'heure supérieure.
