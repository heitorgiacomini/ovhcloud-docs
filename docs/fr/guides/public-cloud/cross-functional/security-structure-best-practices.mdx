...
description: Apprenez à sécuriser votre compte OVHcloud, à gérer les identités, à structurer vos projets Public Cloud et à utiliser les politiques IAM pour restreindre les droits d’accès
...
Le guide « [Sécuriser mon compte OVHcloud et gérer mes informations personnelles](/guides/account-and-service-management/account-information/all-about-username) » fournit des pratiques générales pour la gestion des comptes et constitue un bon point de départ.
...
- **Comment l'activer** : Connectez-vous à l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>, cliquez sur votre nom en haut à droite puis sur vos initiales. Accédez à l'onglet <code className="action">Sécurité</code>, et suivez les étapes pour configurer la 2FA via une application mobile ou une clé de sécurité. Assurez-vous de stocker vos codes de secours en toute sécurité. Pour plus de détails, référez-vous au [guide sur la 2FA](/guides/account-and-service-management/account-information/secure-ovhcloud-account-with-2fa).
...
Le [guide de gestion des mots de passe](/guides/account-and-service-management/account-information/manage-ovh-password) fournit des bonnes pratiques en matière de gestion des mots de passe.
...
- **Gestion des identités** : Trois types d’identités sont supportés et présentés dans cette [page de documentation](/guides/manage-and-operate/iam/identities-management) :
    - `Utilisateurs locaux` : identités représentant une personne dans votre organisation et associées à votre compte OVHcloud. Consultez le guide « [Créer et gérer des utilisateurs locaux sur un compte OVHcloud](/guides/account-and-service-management/account-information/ovhcloud-users-management) ».
    - `Comptes de service` : utilisés pour les interactions « machine à machine ». Les comptes de service sont également associés à votre compte OVHcloud.Pour plus d'informations, consultez [Gestion des comptes de service OVHcloud via l'API](/guides/manage-and-operate/api/manage-service-account) et [Comment utiliser des comptes de service pour se connecter aux API OVHcloud](/guides/account-and-service-management/account-information/authenticate-api-with-service-account).
    - `Identités fédérées` : plusieurs protocoles et fournisseurs de fédération sont supportés et documentés : [AD FS](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-adfs), [Google Workspace](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-google-workspace), [Entra ID](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-azure-ad) ou [OKTA](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-okta).
- **Groupes d'utilisateurs** : Créez des groupes comme "Développeurs", "Admins" et "Auditeurs" pour simplifier la gestion des permissions. Si vous avez choisi de créer des utilisateurs locaux, consultez ce [guide](/guides/account-and-service-management/account-information/ovhcloud-users-management) pour créer des groupes d’utilisateurs locaux. Si vous avez choisi la fédération d'identité, le groupe doit être défini dans votre fournisseur d'identité, fourni dans le ticket SAML et mappé à un groupe OVHcloud. Cette étape est détaillée dans chaque page de documentation du fournisseur d'identité.
- **Définition des politiques** : Attribuez des politiques aux identités et aux groupes pour restreindre ou accorder l'accès. Par exemple, un développeur pourrait avoir l’autorisation de déployer des applications mais pas de gérer les informations de facturation. Ce [guide](/guides/account-and-service-management/account-information/iam-policy-ui) définit comment utiliser les politiques IAM via le tableau de bord OVHcloud.
...
Pour fournir un accès à l'espace client OVHcloud, vous devez créer une politique spécifique comme décrit dans ce [guide](/guides/account-and-service-management/account-information/iam-control-panel-access). Dans la section `Ajouter des groupes d'utilisateurs`, choisissez les groupes `finance_developer_group` et `finance_SRE_group`.

<img className="thumbnail" alt="Ajouter des groupes d'utilisateurs" src="/images/public-cloud/cross-functional/securing-and-structuring-projects/Add_user_groups.png" loading="lazy" />
...
- Dans la page IAM, cliquez sur <code className="action">Créer une politique</code>, remplissez le nom `cloud_project_finance_fpa_dev-RW` et une description.
- Cliquez ensuite sur <code className="action">Ajouter des groupes d'utilisateurs</code>, choisissez le groupe `finance_developer_group`.
- Sous `Types de produits`, choisissez <code className="action">Projet Public Cloud</code>.
- Sous `Ressources`, sélectionnez le projet Public Cloud cible `cloud_project_finance_fpa_dev`.

<img className="thumbnail" alt="Ajouter types de produits & ressources" src="/images/public-cloud/cross-functional/securing-and-structuring-projects/Product_types_resources.png" loading="lazy" />

- Sous `Actions` dans <code className="action">Groupes de permissions gérées</code>, laissez `Autoriser toutes les actions` décoché et choisissez <code className="action">globalWriteAccess</code>.

<img className="thumbnail" alt="Actions" src="/images/public-cloud/cross-functional/securing-and-structuring-projects/actions2.png" loading="lazy" />
