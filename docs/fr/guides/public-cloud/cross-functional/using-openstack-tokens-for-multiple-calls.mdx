...
description: Découvrez comment créer et utiliser des jetons OpenStack pour vos actions
...
- Pour les utilisateurs de Windows, consultez ce guide pour exporter vos variables d'environnement : [Définir les variables d'environnement OpenStack](/guides/public-cloud/cross-functional/compute-set-openstack-environment-variables).
...
Cliquez sur <code className="action">Utilisateurs & Rôles</code> dans la section **Paramètres** puis sur le bouton <code className="action">...</code> à droite de votre utilisateur OpenStack.<br/>
Téléchargez le fichier OpenRC de cet utilisateur et spécifiez la région dans laquelle vous voulez effectuer des actions.

<img className="thumbnail" alt="télécharger le fichier openRC" src="/images/public-cloud/cross-functional/using-openstack-tokens-for-multiple-calls/openrc.png" loading="lazy" />
