...
Installez ensuite toutes les dépendances en exécutant dans un terminal (cliquez sur l'icône de menu > Terminal > New Terminal) :
...
- `--endpoints ~/ai-training-examples/notebooks/natural-language-processing/chatbot/conda/rasa_bot/endpoints.yml` : vous spécifiez le chemin de votre fichier `endpoints.yml`. Ce fichier permet d'indiquer où votre modèle s'exécute et où s'exécutent les ports de vos actions personnalisées.
