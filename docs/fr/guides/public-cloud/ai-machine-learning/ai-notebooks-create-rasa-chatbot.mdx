...
- Spécifie une version pour le framework conda (Python 3.9).
...
Avant d'entraîner un modèle AI dans le notebook, vous devez installer toutes les dépendances de Rasa. Pour cela, ouvrez un terminal en cliquant sur l'icône de menu > Terminal > New Terminal, et lancez la commande suivante :
...
Cela créera quelques dossiers avec des données de base pour le chatbot Rasa. Pendant la commande, vous pouvez également entraîner un modèle et parler avec le chatbot à la fin. Voici un petit exemple de conversation avec le chatbot.
