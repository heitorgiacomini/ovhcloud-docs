...
description: Découvrez comment importer votre propre image sur Public Cloud
...
- une [instance Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance) sur votre espace client OVHcloud
...
- un environnement [compatible avec la CLI OpenStack](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) (si vous utilisez CLI)
...
Il est conseillé d'utiliser des images compatibles cloud fournies par le distributeur ou de créer votre propre image en utilisant des solutions telles que [Packer OpenStack builder](/guides/public-cloud/compute/create-image-from-existing-image-with-packer).
...
3\. Dirigez-vous vers la section `Images` puis cliquez sur <code className="action">Create Image</code>.
...
5\. Entrez les métadonnées de l'instance (vous pouvez aussi entrer les métadonnées personnalisées de votre choix), puis cliquez sur <code className="action">Create Image</code>.
