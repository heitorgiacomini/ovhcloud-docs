...
description: "Dans le cas d'un incident de sécurité, réaliser des investigations numériques sur les données du disque et/ou la RAM d'une instance Public Cloud peut s'avérer nécessaire. Ce guide vous accompagnera pour récupérer les données nécessaires"
...
- Avoir la [OpenStack CLI](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) prête à être utilisée sur votre système.
- Un [utilisateur OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user) a été créé avec (a minima) les rôles _Compute Operator_ et _Backup Operator_.
...
Afin de récupérer le disque d'une instance Public Cloud, le moyen le plus simple est d'utiliser [l'utilitaire en ligne de commande OpenStack](https://docs.openstack.org/newton/user-guide/common/cli-overview.html). Si l'outil n'a pas été installé sur votre système, vous pouvez lire [notre guide](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) ainsi que [notre guide pour créer un utilisateur OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user).

#### Réaliser une copie de sauvegarde

Tout d'abord, nous allons réaliser une copie de sauvegarde de l'instance Public Cloud. Cette opération peut être faite soit [en utilisant l'espace client OVHcloud](/guides/public-cloud/compute/save-an-instance) ou bien en utilisant la OpenStack CLI. Dans ce guide, nous appliquerons la seconde option.
...
Vous devez commencer par récupérer le fichier `openrc.sh`. Dans le menu de gauche de l'univers Public Cloud, sous la catégorie "Paramètres", rendez-vous dans la section <code className="action">Utilisateurs & Roles</code>, puis, à droite de votre utilisateur OpenStack, cliquez sur le bouton <code className="action">...</code> et sélectionnez <code className="action">Télécharger le fichier OpenStack RC</code>. Une boîte de dialogue vous demandera de choisir la _Region_ afin de vous permettre de télécharger le bon fichier de configuration. Sélectionnez la même _Region_ que l'instance Public Cloud que vous souhaitez investiguer.

<img className="thumbnail" alt="rc-file" src="/images/public-cloud/compute/forensics/rc-file.png" loading="lazy" />
...
Cette [documentation](/guides/public-cloud/cross-functional/compute-set-openstack-environment-variables) vous aidera à configurer les bonnes variables d'environnement sous Windows.
:::
