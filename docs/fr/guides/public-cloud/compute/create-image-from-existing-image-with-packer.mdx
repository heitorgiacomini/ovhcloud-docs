...
description: Créer et personnaliser une image OpenStack à partir d’une image existante avec Packer
...
À partir de votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, récupérez votre fichier de configuration `openrc.sh`. 

Vous pouvez le retrouver via le menu OpenStack dans le panneau latéral gauche puis sous le bouton <code className="action">...</code> sur la droite, vous trouverez le bouton <code className="action">Télécharger un fichier de configuration OpenStack</code> 

La [création d'un utilisateur OpenStack](/guides/public-cloud/cross-functional/create-and-delete-a-user) peut s'avérer nécessaire au préalable.
