...
description: Découvrez comment faire évoluer les ressources de votre instance pour faire face à une activité accrue
...
- Avoir un [environnement OpenStack préparé pour le CLI](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment)
- Avoir défini les [variables d'environnement OpenStack](/guides/public-cloud/cross-functional/compute-set-openstack-environment-variables)
...
Lors d'un redimensionnement, l'instance est arrêtée pendant toute la durée de l'opération. Avant de procéder, il est donc recommandé de sauvegarder votre instance et d'arrêter tous les processus en cours d'exécution. Retrouvez plus d'informations sur les méthodes de sauvegarde dans le [guide dédié](/guides/public-cloud/compute/save-an-instance).
...
[Redimensionner une instance Public Cloud via l’espace client OVHcloud](/guides/public-cloud/compute/resize-instance-manager)
