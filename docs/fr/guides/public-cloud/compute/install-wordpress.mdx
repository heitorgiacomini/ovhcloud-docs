...
description: Découvrez comment utiliser une instance Public Cloud pour héberger des sites WordPress
...
- Avoir une [instance Public Cloud](/guides/public-cloud/compute/first-steps-with-public-cloud-instance) avec Debian ou Ubuntu installé
...
Afin d'accéder à votre installation via un nom de domaine, vous devez relier celui-ci à votre instance. Pour ce faire, vous devez éditer la zone DNS accessible depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, à condition que OVHcloud soit votre bureau d’enregistrement **et** que le nom de domaine utilise les serveurs DNS OVHcloud.

Consultez le guide [Éditer sa zone DNS](/guides/web-cloud/domains/dns-zone-edit) pour en savoir plus. Si le nom de domaine est actuellement utilisé, ne configurez les DNS qu'après l'installation du nouveau WordPress et le démarrage de votre site web.
...
Il est recommandé d'utiliser la méthode d'authentification proposée à la place de l'accès par mot de passe root. Appuyez sur <code className="action">y</code> puis sur <code className="action">Entrée</code>. (Si vous décidez d'utiliser l'accès utilisateur root, tapez <code className="action">n</code>, puis définissez un mot de passe root.)
...
Les invites suivantes concernant les mesures de sécurité, confirmez-les toutes avec <code className="action">y</code> jusqu'à la fin du script.
...
Ouvrez un navigateur Web et connectez-vous au site WordPress en saisissant l'adresse IP de votre instance (ou le nom de domaine si vous en avez déjà [relié un à l'instance](/guides/web-cloud/domains/dns-zone-edit)). Choisissez une langue sur la première page.
