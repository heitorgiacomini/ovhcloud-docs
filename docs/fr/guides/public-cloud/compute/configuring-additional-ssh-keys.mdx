...
description: Découvrez comment configurer des clés SSH supplémentaires pour les comptes utilisateurs et les ajouter à votre instance Public Cloud
...
- [Accès administratif à votre instance via SSH](/guides/public-cloud/compute/creating-ssh-keys#login-linux)
...
Si nécessaire, utilisez notre [guide sur les clés SSH](/guides/public-cloud/compute/creating-ssh-keys) pour créer une nouvelle paire de clés SSH.  
Vous y trouverez également des informations sur [la gestion de plusieurs clés](/guides/public-cloud/compute/creating-ssh-keys#multiplekeys) sur votre poste de travail local si votre installation l'exige.

### Étape 2 : configurer un nouveau compte utilisateur

[Connectez-vous à votre instance](/guides/public-cloud/compute/getting-started#connect-instance) et utilisez les commandes ci-dessous pour créer un nouveau compte utilisateur et un dossier `.ssh` :
...
Vous pouvez en savoir plus sur les autorisations des utilisateurs et les rubriques connexes dans notre [guide du compte utilisateur](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).
...
:::info
Pour des raisons de sécurité et de bonnes pratiques, une paire de clés ne doit pas être utilisée par plusieurs utilisateurs. Puisque chaque utilisateur sur les systèmes GNU/Linux a son propre fichier `authorized_keys` dans `~/.ssh/`, vous pouvez utiliser la commande `ssh-copy-id` comme indiqué ci-dessus et adapter `KeyFileName` et `user` après avoir [créé la paire de clés](/guides/public-cloud/compute/creating-ssh-keys#create-ssh-key).
:::


#### Ajout manuel de clés publiques à l'instance

[Connectez-vous à votre instance](/guides/public-cloud/compute/getting-started#connect-instance) et ouvrez, avec votre éditeur de texte préféré (`nano` est utilisé dans cet exemple), le fichier `authorized_keys` dans le dossier personnel du nouvel utilisateur :
...
Consultez le [guide sur les clés SSH](/guides/public-cloud/compute/creating-ssh-keys) pour en savoir plus sur l'utilisation des clés SSH avec les instances Public Cloud.

## Aller plus loin

[Comment créer des instances Public Cloud](/guides/public-cloud/compute/getting-started)
