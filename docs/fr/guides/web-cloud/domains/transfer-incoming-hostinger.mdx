...
description: Découvrez les démarches spécifiques pour transférer un nom de domaine depuis Hostinger vers OVHcloud
...
Pour les noms de domaine enregistrés chez Hostinger, vous pouvez gérer le statut de verrouillage de votre nom de domaine en ouvrant la section <code className="action">Domaines</code> et en sélectionnant le nom de domaine à transférer.
...
Pour vous familiariser avec le changement de titulaire d'un nom de domaine, nous vous consultez ce [guide](/guides/web-cloud/domains/trade-domain).

:::warning
Certains TLD ont un processus de transfert spécial. Cela est vrai notamment pour les noms de domaine **.uk** et **.eu**.
...
Une fois le code d'autorisation obtenu, vous pouvez procéder au transfert de votre nom de domaine en suivant les étapes de notre guide « [Transférer son nom de domaine vers OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain) ».
