...
description: "Découvrez comment utiliser l'API publique OVHcloud pour commander vos noms de domaine"
...
:::info
Pour suivre ce guide, vous devez d'abord vous connecter à l'API OVHcloud. Vous trouverez plus de détails sur la page d'[introduction à l'API](/guides/web-cloud/domains/api-domain-intro).
:::
...
- [Gestion des contacts d'un nom de domaine](/guides/web-cloud/domains/api-domain-contacts)
...
<Api version="v1" section="/order" method="GET" route={"/order/cart/\{cartId\}/domain"} />
...
<Api version="v1" section="/order" method="POST" route={"/order/cart/\{cartId\}/domain"} />
...
<Api version="v1" section="/order" method="GET" route={"/order/cart/\{cartId\}/summary"} />
...
:::info
**Un petit point sur la zone DNS**
...
Ces APIs correspondant à un usage avancé, et étant utilisées également en dehors de la commande (comme pour la mise à jour d'un contact), elles sont décrites en détail dans la section [Gestion des règles](/guides/web-cloud/domains/api-domain-rules).
:::


:::info
Ici, le `OWNER_CONTACT` représente une API de "ressource", à savoir `/me/contact` ou plus précisement `/domain/contact`.
Les APIs permettant de créer ces contacts sont décrites dans la section [Gestion des contacts](/guides/web-cloud/domains/api-domain-contacts).
:::
...
<Api version="v1" section="/order" method="GET" route={"/order/cart/\{cartId\}/item/\{itemId\}/configuration"} />


#### Récupération de la valeur d'une configuration

<Api version="v1" section="/order" method="GET" route={"/order/cart/\{cartId\}/item/\{itemId\}/configuration/\{configurationId\}"} />


#### Suppression d'une configuration

<Api version="v1" section="/order" method="DELETE" route={"/order/cart/\{cartId\}/item/\{itemId\}/configuration/\{configurationId\}"} />
...
<Api version="v1" section="/order" method="GET" route={"/order/cart/\{cartId\}"} />
...
<Api version="v1" section="/order" method="DELETE" route={"/order/cart/\{cartId\}/item/\{itemId\}"} />
...
<Api version="v1" section="/me" method="GET" route={"/me/order/\{orderId\}/availableRegisteredPaymentMean"} />
...
<Api version="v1" section="/me" method="GET" route={"/me/order/\{orderId\}/status"} />
