...
- **Depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>** où un assistant de configuration permet de paramétrer votre redirection.
...
Attention : une redirection créée depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> ne permet pas de rediriger une URL en `https://` vers un autre nom de domaine ou une autre URL.
...
Hormis les redirections de « pointage » vers les entrées DNS A, AAAA et CNAME, 3 options de redirections sont disponibles depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>.
