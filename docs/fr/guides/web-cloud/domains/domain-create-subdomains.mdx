...
- Disposer d'une <Tooltip term="dnsZone">zone DNS</Tooltip> active pour votre nom de domaine. Si besoin, consultez notre guide « [Créer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-create) ».
...
- [Associer, autoriser et configurer votre sous-domaine avec un service OVHcloud](#link-subdomain)
    - [Cas 1 - Afficher un site web présent sur mon hébergement web OVHcloud avec un sous-domaine](#link-subdomain-case-1)
    <Region zones={['eu', 'ca']}>
    - [Cas 2 - Créer des adresses e-mail Exchange avec un sous-domaine](#link-subdomain-case-2)
    </Region>
    <Region zones={['eu']}>
    - [Cas 3 - Créer des adresses E-mail Pro avec un sous-domaine](#link-subdomain-case-3)
    </Region>
...
<summary>Quelle est la composition d'un nom de domaine ?</summary>
...
<summary>Qu'est-ce qu'un sous-domaine ?</summary>
...
- *dns1.example.com* et *dns2.example.com* pour personnaliser vos serveurs DNS à l'aide des [GLUE records](/guides/web-cloud/domains/glue-registry) ;
...
Pour plus de détails sur les zones DNS, consultez notre guide « [Créer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-create) » et « [Éditer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit) ».

**Tous les sous-domaines se configurent dans la zone DNS active du nom de domaine, en y ajoutant des enregistrements DNS**

#### 1 - Identifier l'endroit où se trouve la zone DNS active de votre nom de domaine <a name="identification"></a>
...
1 : Pour identifier l'endroit où se trouve la zone DNS active d'un nom de domaine enregistré chez OVHcloud, vous pouvez vous aider de notre guide « [Modifier les serveurs DNS d'un nom de domaine OVHcloud](/guides/web-cloud/domains/dns-server-edit) ».

2 : Si votre nom de domaine n'est pas enregistré chez OVHcloud, rapprochez-vous du *bureau d'enregistrement* actuel de votre nom de domaine afin de connaître l'endroit où est hébergée sa zone DNS active. Sachez que vous pouvez utiliser notre guide « [Transférer son nom de domaine vers OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain) » si vous souhaitez réaliser cette action.

:::


Si les serveurs DNS déclarés pour votre nom de domaine ont l'une des trois formes suivantes :
...
#### 2 - Créer les enregistrements DNS pour vos sous-domaines <a name="dns-records-creation"></a>

Pour ajouter vos sous-domaines dans la zone DNS active de votre nom de domaine, consultez notre guide « [Éditer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit) ».
...
- Les serveurs e-mail (enregistrements DNS de type <Tooltip term="mxRecord">*MX*</Tooltip>) vers lesquels votre sous-domaine doit rediriger les e-mails qu'il reçoit. Cela vous permet de les consulter sur votre (vos) adresse(s) e-mail(s) personnalisée(s) avec votre sous-domaine.
- Des informations liées à la sécurité / l'authentification de vos services (hébergement web, serveur web, e-mail, etc.) associés à l'un de vos sous-domaines (enregistrements DNS de type <Tooltip term="spf">*SPF*</Tooltip>, <Tooltip term="dkim">*DKIM*</Tooltip>, <Tooltip term="dmarc">*DMARC*</Tooltip>, etc.).

:::info
La modification d'une zone DNS associée à un nom de domaine entraine un délai de <Tooltip term="dnsPropagation">propagation</Tooltip> de **4** à **24** heures maximum pour que celle-ci soit effective.
...
Pour les services qui ne seraient pas mentionnés, consultez la documentation relative au service concerné, afin d'identifier si ce dernier peut être utilisé avec un sous-domaine.

#### Cas 1 - Afficher un site web présent sur mon hébergement web OVHcloud avec un sous-domaine <a name="link-subdomain-case-1"></a>

Tout comme pour un nom de domaine, pour autoriser un sous-domaine à afficher le contenu d'un dossier « cible » présent sur un hébergement web, connectez-vous à l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> et sélectionnez <code className="action">Web Cloud</code>. Cliquez sur <code className="action">Hébergements</code> dans la colonne de gauche, sélectionnez l'offre concernée où se trouve votre site web puis choisissez l'onglet <code className="action">Multisite</code>.

C'est ici que vous autorisez l'accès de votre sous-domaine à votre hébergement web où se trouve votre site web.

Pour plus de détails sur la configuration d'un nom de domaine ou d'un sous-domaine sur un hébergement web, consultez notre guide « [Partager son hébergement entre plusieurs sites](/guides/web-cloud/web-hosting/multisites-configure-multisite) ». Qu'il s'agisse d'un nom de domaine ou d'un sous-domaine, la procédure est la même.
...
Pour débloquer la création d'adresses e-mail Exchange personnalisées avec un sous-domaine, connectez-vous à l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> et sélectionnez <code className="action">Web Cloud</code>. Cliquez sur <code className="action">Microsoft</code> dans la colonne de gauche puis sur <code className="action">Exchange</code>. Sélectionnez ensuite la plateforme Exchange que vous souhaitez utiliser avec votre sous-domaine. Sur la page qui s'affiche, rendez-vous dans l'onglet <code className="action">Domaine associés</code> puis cliquez sur le bouton <code className="action">Ajouter un nom de domaine</code> à droite.
...
- [Ajouter un nom de domaine sur une plateforme e-mail](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-adding-domain)
- [Ajouter un enregistrement CNAME pour valider votre nom de domaine sur votre offre e-mail](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-dns-cname)
...
Pour débloquer la création d'adresses E-mail Pro personnalisées avec un sous-domaine, connectez-vous à l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> et sélectionnez <code className="action">Web Cloud</code>. Cliquez sur <code className="action">E-mails Pro</code> puis sélectionnez la plateforme E-mail Pro que vous souhaitez utiliser avec votre sous-domaine. Sur la page qui s'affiche, rendez-vous dans l'onglet <code className="action">Domaine associés</code> puis cliquez sur le bouton <code className="action">Ajouter un domaine</code> à droite.
