...
L'enregistrement **D**omain-based **M**essage **A**uthentication, **R**eporting, and **C**onformance (DMARC) est un mécanisme de sécurité e-mail. Il s'appuie sur le résultat des vérifications [SPF](/guides/web-cloud/domains/dns-zone-spf) et [DKIM](/guides/web-cloud/domains/dns-zone-dkim).
...
- Un des mécanismes d'authentification, [SPF](/guides/web-cloud/domains/dns-zone-spf) et/ou [DKIM](/guides/web-cloud/domains/dns-zone-dkim) doit être configuré dans la zone DNS du nom de domaine de l'offre e-mail.
...
DMARC interroge les mécanismes d'authentification [SPF](/guides/web-cloud/domains/dns-zone-spf) et [DKIM](/guides/web-cloud/domains/dns-zone-dkim) pour vérifier les e-mails entrants.<br/>
...
Vous pouvez ajouter l'enregistrement DMARC à votre zone DNS depuis l'espace client OVHcloud. Pour cela, connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> puis rendez-vous dans la partie <code className="action">Web Cloud</code>. Dans la colonne de gauche, sélectionnez, dans la section <code className="action">Noms de domaine</code>, le nom de domaine concerné puis cliquez sur l'onglet <code className="action">Zone DNS</code> pour accéder à votre zone DNS.

Une fois votre zone DNS affichée, cliquez sur le bouton <code className="action">Ajouter une entrée</code> puis sur « Champs mails » <code className="action">DMARC</code>.

- **Sous-domaine** : cette entrée doit **obligatoirement débuter par** `_dmarc`. Si vous appliquez votre DMARC à l'ensemble du nom de domaine, n'inscrivez rien d'autre que `_dmarc` dans cette case. Si vous définissez votre DMARC à un sous-domaine de votre nom de domaine principal, ajoutez votre sous-domaine après `_dmarc`. Par exemple, si on doit appliquer le DMARC à un sous-domaine *subdomain.mydomain.ovh*,  il faut saisir `_dmarc.subdomain` dans la case « sous-domaine » pour le nom de domaine *mydomain.ovh*.
...
Vous pouvez ajouter l'enregistrement TXT à votre zone DNS depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> puis rendez-vous dans la partie <code className="action">Web Cloud</code>. Cliquez sur <code className="action">Noms de domaine</code> puis choisissez le nom de domaine concerné. Positionnez-vous enfin sur l'onglet <code className="action">Zone DNS</code>.

Une fois votre zone DNS affichée, cliquez sur le bouton <code className="action">Ajouter une entrée</code> puis sur « Champs étendus » <code className="action">TXT</code>.
