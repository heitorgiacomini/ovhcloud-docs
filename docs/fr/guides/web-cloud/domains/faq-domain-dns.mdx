...
<summary>Comment puis-je souscrire à un nom de domaine chez OVHcloud ?</summary>


Suivez ces étapes :

1. Rendez-vous sur notre site web [OVHcloud](https://www.ovhcloud.com/fr/).
2. Sur la page qui s'affiche et dans le champ prévu à cet effet, saisissez le nom de domaine que vous souhaitez réserver (par exemple : `domain.tld`), puis cliquez sur le bouton <code className="action">Rechercher</code>.
...
4. Une fois que vous avez trouvé un nom de domaine disponible, cliquez sur le bouton <code className="action">Acheter</code>, puis sur le bouton <code className="action">Poursuivre la commande</code> dans la colonne de droite.
5. Sélectionnez les éventuelles options ou services auxquels vous souhaitez souscrire en complément de votre nom de domaine, puis cliquez sur <code className="action">Suivant</code> jusqu'à ce que le tunnel de commande vous invite à vous authentifier ou à créer un compte client OVHcloud.
6. Dès que vous êtes authentifié avec votre compte client OVHcloud, vous pourrez personnaliser les informations des contacts (titulaire, administrateur, technique) pour votre nom de domaine. Cliquez ensuite sur le bouton <code className="action">Continuer</code> pour accéder au récapitulatif de votre commande.
7. Sur la page `Récapitulatif de votre commande` et si nécessaire, vous pourrez modifier la configuration DNS qui s'appliquera à votre nom de domaine en cliquant sur le lien intitulé <code className="action">Modifier la configuration</code>. Dès que vos modifications sont terminées, cliquez sur le bouton <code className="action">Payer</code> pour accéder à la dernière étape de votre commande.
...
Vous pourrez ensuite administrer votre nom de domaine en vous connectant à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>.
...
<summary>Comment puis-je acheter un nom de domaine sur le marché secondaire ?</summary>
...
<summary>Comment savoir si mon nom de domaine est enregistré chez OVHcloud ?</summary>


Pour cela, vous pouvez effectuer une requête [WHOIS](https://www.ovhcloud.com/fr/domains/whois/) pour savoir où est enregistré votre nom de domaine et pour vérifier que vous êtes bien déclaré en tant que titulaire du nom de domaine.
...
<summary>Comment connaître la date d'expiration d'un nom de domaine ?</summary>


La solution la plus rapide est d'effectuer une requête [WHOIS](https://www.ovhcloud.com/fr/domains/whois/) sur le nom de domaine. Une fois la requête effectuée, recherchez dans le résultat la ligne correspondant à la date d'expiration (par exemple : `Expiry Date: 2025-09-22T08:00:00Z`, `Registry Expiry Date: 2025-09-22T08:00:00Z`, etc.).

Si votre nom de domaine est enregistré chez OVHcloud, cliquez sur les onglets ci-dessous pour afficher successivement chacune des **2** étapes.

<Tabs>
  <Tab label="Étape 1">
    Accédez à la page <ManagerLink to="/#/billing/autorenew/services">Mes offres & services</ManagerLink>.
  </Tab>
  <Tab label="Étape 2">
    Dans le tableau qui apparaît, recherchez la ligne correspondant à votre nom de domaine, puis repérez la date présente dans la colonne `Date d'effet`. Cette date correspond à la date d'expiration de votre nom de domaine.
  </Tab>
</Tabs>
...
<summary>Comment changer la date annuelle d'expiration d'un nom de domaine ?</summary>
...
<summary>Comment puis-je corriger une faute de frappe dans mon nom de domaine ?</summary>
...
<summary>Comment modifier un nom de domaine déjà souscrit ?</summary>
...
<Tabs>
  ...
  <Tab label="Étape 2">
    Dans le tableau qui apparaît, recherchez la ligne correspondant à votre nom de domaine, cliquez sur le bouton <code className="action">...</code> à droite, puis sur <code className="action">Résilier mon service</code>.
  </Tab>
  <Tab label="Étape 3">
    Sur la page qui s'affiche, sélectionnez le mode de résiliation (immédiatement ou à la date d'expiration du service) puis cliquez en bas sur le bouton <code className="action">Oui, résilier</code>.
...
:::tip
Retrouvez tous les détails dans notre guide « [Comment résilier mes services OVHcloud](/guides/account-and-service-management/managing-billing-payments-and-services/how-to-cancel-services) ».
...
<summary>J'ai reçu un e-mail concernant la validation des informations du titulaire associée à mon nom de domaine, que faire ?</summary>


Tout d'abord, si vous avez un doute sur la légitimité de l'e-mail reçu, consultez notre guide « [Phishing - Comment reconnaître des e-mails ou SMS frauduleux ?](/guides/account-and-service-management/account-information/phishing-care) ».
...
<summary>Je n'ai pas reçu l'e-mail de validation des informations du titulaire associé à mon nom de domaine et celui-ci est suspendu, que faire ?</summary>


Si vous n'avez pas reçu l'e-mail de validation du titulaire de votre nom de domaine, vérifiez les points ci-dessous :
...
<summary>Qu'est-ce qu'un nom de domaine au format IDN ?</summary>
...
<summary>Comment corriger un nom de domaine au format IDN ?</summary>
...
<summary>Comment renouveler un seul nom de domaine présent dans un pack Alldom ?</summary>


Pour cela, vous devez être à minima déclaré en tant que [contact « Facturation »](/guides/account-and-service-management/account-information/managing-contacts) du nom de domaine concerné. Vous devrez ensuite modifier le mode de renouvellement du nom de domaine pour le passer en **renouvellement automatique**.
...
<Tabs>
  ...
  <Tab label="Étape 2">
    Dans le tableau qui apparaît et à droite du nom de domaine concerné, cliquez sur le bouton <code className="action">...</code> dans la colonne `Actions`, puis sur <code className="action">Configurer le renouvellement</code>. Vous pourrez ensuite configurer le renouvellement de ce nom de domaine en **renouvellement automatique**.
  </Tab>
</Tabs>
...
<summary>Est-ce que mon nom de domaine est transférable après un changement de titulaire ?</summary>


L'**I**nternet **C**orporation for **A**ssigned **N**ames and **N**umbers (**ICANN**) a mis en place des mesures de sécurité pour prévenir les transferts ou les changements de titulaires non autorisés ou abusifs des noms de domaine.

L'ICANN a notamment défini un délai incompressible de **60** jours entre chaque opération pouvant survenir sur un nom de domaine (création, changement de titulaire, transfert).

Les règles définies par l'ICANN doivent obligatoirement être respectées par les bureaux d'enregistrement (tel qu'OVHcloud).

Vous n'aurez donc pas d'autre choix que de patienter jusqu'à la fin du délai de 60 jours pour pouvoir transférer votre nom de domaine après avoir changé son titulaire.
...
<summary>Mon nom de domaine est bloqué contre le transfert pendant 60 jours, que faire ?</summary>
...
Vous n'aurez donc pas d'autre choix que de patienter jusqu'à la fin du délai de 60 jours pour réaliser une nouvelle opération (changement de titulaire ou transfert) sur votre nom de domaine.
...
<summary>Je ne retrouve pas mon nom de domaine dans mon espace client, que faire ?</summary>


Tout d'abord, effectuez une requête [WHOIS](https://www.ovhcloud.com/fr/domains/whois/) pour savoir où est enregistré votre nom de domaine et pour vérifier que vous êtes bien déclaré en tant que titulaire du nom de domaine.
...
Effectuez une [procédure de récupération des contacts](https://www.ovh.com/fr/cgi-bin/fr/procedure/procedureChangeContacts.cgi) pour que votre nom de domaine soit entièrement géré dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Ainsi, vous n'aurez plus besoin de contacter la personne qui gérait auparavant votre nom de domaine.
...
Cependant, vous pouvez tenter de contacter la personne ou organisation qui le gère en suivant les instructions de [ce formulaire](https://www.ovhcloud.com/fr/lp/request-ovhcloud-registered-domain/).
...
<summary>Je ne parviens pas à contacter la personne qui gère mon nom de domaine, que faire ?</summary>


Tout d'abord, effectuez une requête [WHOIS](https://www.ovhcloud.com/fr/domains/whois/) pour vérifier que vous êtes bien déclaré en tant que titulaire du nom de domaine.
...
<summary>Puis-je vendre mon nom de domaine ?</summary>
...
:::info
La modification d'une zone DNS est une manipulation sensible et peut engendrer une interruption des services associés à votre nom de domaine (hébergement web, e-mail, etc.). En cas de doute, n'hésitez pas à contacter un [prestataire spécialisé](https://partner.ovhcloud.com/fr/directory/).
...
<summary>Qu'est-ce qu'une zone DNS ?</summary>
...
:::tip
Retrouvez tous les détails dans notre guide « [Tout savoir sur la zone DNS](/guides/web-cloud/domains/dns-zone-general-information) ».
...
<summary>Qu'est-ce qu'un enregistrement DNS ?</summary>
...
- Enregistrements DNS de pointage :
    - [Ajouter un enregistrement DNS de type A pour un nom de domaine](/guides/web-cloud/domains/dns-zone-a-record-creation)
    - [Ajouter un enregistrement DNS de type AAAA pour un nom de domaine](/guides/web-cloud/domains/dns-zone-aaaa-record-creation)
    - [Ajouter un enregistrement DNS de type CNAME pour un nom de domaine](/guides/web-cloud/domains/dns-zone-cname-record-creation)
- Enregistrements DNS étendus :
    - [Ajouter un enregistrement DNS de type TXT pour un nom de domaine](/guides/web-cloud/domains/dns-zone-txt-record-creation)
- Enregistrements DNS e-mails :
    - [Configurer un enregistrement MX pour la gestion des emails](/guides/web-cloud/domains/dns-zone-mx)
...
<summary>Quels sont les enregistrements DNS disponibles dans une zone DNS OVHcloud ?</summary>
...
<summary>Puis-je changer les serveurs DNS déclarés dans ma zone DNS chez OVHcloud ?</summary>
...
<Tabs>
  ...
  <Tab label="Étape 2">
    Sélectionnez l'onglet <code className="action">Serveurs DNS</code> une fois positionné sur le nom de domaine concerné.
  </Tab>
  <Tab label="Étape 3">
...
<summary>Quelle est la différence entre un enregistrement DNS de type A (IPv4) et AAAA (IPv6) ?</summary>
...
- [Ajouter un enregistrement DNS de type A pour un nom de domaine](/guides/web-cloud/domains/dns-zone-a-record-creation)
- [Ajouter un enregistrement DNS de type AAAA pour un nom de domaine](/guides/web-cloud/domains/dns-zone-aaaa-record-creation)
- [Configurez une adresse IPv6 pour votre site web](/guides/web-cloud/web-hosting/configure-ipv6)
...
<summary>Comment configurer un enregistrement PTR pour mon adresse IP externe à OVHcloud ?</summary>
...
:::tip
Retrouvez tous les détails dans notre guide « [Tout savoir sur les enregistrements DNS](/guides/web-cloud/domains/dns-zone-records) ».
...
<summary>Comment changer le TTL par défaut dans ma zone DNS OVHcloud ?</summary>
...
<summary>Qu'est-ce qu'un enregistrement DNS de type SOA ?</summary>
...
<summary>Comment vérifier la configuration de ma zone DNS ?</summary>
...
<summary>Comment vérifier la propagation des modifications effectuées dans ma zone DNS ?</summary>
...
Une fois le numéro de série récupéré, cliquez sur les onglets ci-dessous pour afficher successivement chacune des **4** étapes.
...
:::tip
Retrouvez tous les détails dans notre guide « [Gérer l’historique d'une zone DNS](/guides/web-cloud/domains/dns-zone-history) ».
...
<summary>Comment récupérer une copie de ma zone DNS ?</summary>
...
<summary>Puis-je créer une zone DNS pour un sous-domaine ?</summary>


Vous pouvez créer une zone DNS pour un sous-domaine.

Pour ce faire, cliquez sur les onglets ci-dessous pour afficher successivement chacune des **4** étapes.
...
<Tabs>
  ...
  <Tab label="Étape 3">
    Dès que la vérification aboutit, choisissez d'activer ou non les entrées minimales pour la zone DNS que vous allez créer. Ce choix n'est pas définitif puisque vous pourrez toujours [éditer les enregistrements de la zone DNS](/guides/web-cloud/domains/dns-zone-edit) par la suite.
  </Tab>
  <Tab label="Étape 4">
...
Pour récupérer les noms des 2 serveurs DNS, cliquez sur les onglets ci-dessous pour afficher successivement chacune des **2** étapes.
...
<summary>Comment rediriger tous les sous-domaines d'un même nom de domaine vers la même adresse IP ?</summary>
...
<summary>Puis-je mettre en place un wildcard dans ma zone DNS ?</summary>
...
<summary>J'ai supprimé accidentellement ma zone DNS et je souhaite la restaurer, que faire ?</summary>
...
:::tip
Si vous n'avez pas reçu cet e-mail, vérifiez dans vos courriers indésirables ou rendez-vous sur la page <ManagerLink to="/#/account/useraccount/dashboard">Mon compte</ManagerLink>, puis cliquez sur l'onglet <code className="action">Emails reçus</code>.

:::


Pour restaurer votre zone DNS, téléchargez le fichier contenant la zone DNS depuis l'e-mail reçu.
...
<summary>Comment annuler une demande de suppression de ma zone DNS ?</summary>
...
<summary>Je ne parviens pas à activer une zone DNS pour mon nom de domaine, que faire ?</summary>
...
<summary>Pourquoi je ne retrouve pas l'onglet « GLUE » dans mon espace client OVHcloud ?</summary>


La fonctionnalité n'est pas disponible avec toutes les extensions de noms de domaine.
Si l'onglet n'apparaît pas dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, c'est que l'option « GLUE » est indisponible pour votre nom de domaine.

:::tip
Retrouvez tous les détails dans notre guide « [Personnaliser les serveurs DNS d'un nom de domaine (Glue records)](/guides/web-cloud/domains/glue-registry) ».
...
:::info
La modification des serveurs DNS est une manipulation sensible et peut engendrer une interruption des services associés à votre nom de domaine (hébergement web, e-mail, etc.). En cas de doute, n'hésitez pas à contacter un [prestataire spécialisé](https://partner.ovhcloud.com/fr/directory/).
...
<summary>Comment modifier mes serveurs DNS ?</summary>
...
<summary>Comment personnaliser mes serveurs DNS ?</summary>
...
<summary>Comment remplacer mes serveurs DNS par ceux d'OVHcloud ?</summary>
...
<summary>Dans mon espace client, j'ai un message d'erreur m'indiquant que je n'utilise pas les serveurs DNS d'OVHcloud pour mon nom de domaine, que faire ?</summary>


Dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, ce message indique uniquement que la zone DNS créée pour votre nom de domaine n'est pas sa zone DNS active.
...
<summary>Je ne parviens pas à modifier les serveurs DNS d'un nom de domaine depuis mon espace client OVHcloud, que faire ?</summary>
...
Pour le vérifier, cliquez sur les onglets ci-dessous pour afficher successivement chacune des **2** étapes.

<Tabs>
  <Tab label="Étape 1">
    Accédez à la page <ManagerLink to="/#/web-domains/domain">Noms de domaine</ManagerLink>, puis vérifiez si le nom de domaine concerné apparaît.
  ...
  <Tab label="Étape 2">
    **Cas n°1** - Le nom de domaine n'apparaît pas dans la liste :
    ...
    Cela signifie que le nom de domaine n'est pas géré depuis votre espace client OVHcloud. Effectuez une requête [WHOIS](https://www.ovhcloud.com/fr/domains/whois/) avec ce dernier pour connaître l'endroit où il est enregistré.
    
    Vous pourrez ensuite faire l'une des actions suivantes (si vous êtes le titulaire déclaré sur le WHOIS du nom de domaine) :
    
    - Le nom de domaine est enregistré chez OVHcloud : Vous pourrez effectuer une [procédure de récupération des contacts](https://www.ovh.com/fr/cgi-bin/fr/procedure/procedureChangeContacts.cgi) pour que votre nom de domaine soit géré dans votre espace client OVHcloud.
    - Le nom de domaine n'est pas enregistré chez OVHcloud : Vous pourrez réaliser une opération de [transfert entrant](/guides/web-cloud/domains/transfer-incoming-generic-domain) vers OVHcloud pour que votre nom de domaine soit géré dans votre espace client OVHcloud.
    
    **Cas n°2** - Le nom de domaine apparaît dans la liste :
    
    Cela signifie que vous ne disposez pas des droits suffisants pour gérer le nom de domaine depuis votre espace client OVHcloud. Effectuez une requête [WHOIS](https://www.ovhcloud.com/fr/domains/whois/) pour vérifier que vous êtes bien déclaré en tant que titulaire du nom de domaine.
    
    Vous pourrez ensuite effectuer une [procédure de récupération des contacts](https://www.ovh.com/fr/cgi-bin/fr/procedure/procedureChangeContacts.cgi) pour que votre nom de domaine soit entièrement géré dans votre espace client OVHcloud.
  </Tab>
</Tabs>
...
[Hébergement Web - FAQ](/guides/web-cloud/web-hosting/faq)
