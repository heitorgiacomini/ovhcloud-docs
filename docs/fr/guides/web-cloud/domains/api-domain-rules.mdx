...
description: "Description du format technique des règles d'éligibilité aux noms de domaine"
...
<Api version="v1" section="/domain" method="GET" route={"/domain/configurationRule"} />
...
<Tabs>
  ...
  <Tab label="Afficher : Exemple de règle au format JSON">
    ```json
    {
      "and": [
        {
...
{/* prettier-ignore */}
<Tabs>
  <Tab label="Règle d'acceptation des conditions particulières">
    ```json
    {
...
Avec cette règle, lors de la commande, le domaine doit obligatoirement avoir une [configuration](/guides/web-cloud/domains/api-domain-order#configurations-management) ayant pour label `ACCEPT_CONDITIONS` avec une valeur booléenne à `true`, `1` ou `"1"`.
...
{/* prettier-ignore */}
<Tabs>
  <Tab label="Règle de justification">
    ```json
    {
      "label": "REASON",
      "type": "text",
...
{/* prettier-ignore */}
<Tabs>
  <Tab label="Exemple de règle avec une condition">
    ```json
    {
      "label": "ACCEPT_CONDITIONS",
      "type": "bool",
...
<Tabs>
  ...
  <Tab label="Afficher : Règle de création d'un nom de domaine">
    ```json
    {
      "and": [
        {
...
<Tabs>
  ...
  <Tab label="Afficher : Règle de transfert d'un nom de domaine">
    ```json
    {
      "and": [
        {
...
<Tabs>
  ...
  <Tab label="Afficher : Règle de mise à jour du contact titulaire d'un nom de domaine">
    ```json
    {
        "label": "OWNER_CONTACT",
        "type": "contact",
...
<Tabs>
  ...
  <Tab label="Afficher : Règle de changement de titulaire">
    ```json
    {
      "label": "OWNER_CONTACT",
      "type": "contact",
...
- Une pour récupérer les règles d'éligibilité d'un nom de domaine pour une action (création, transfert...).
...
<Api version="v1" section="/domain" method="POST" route={"/domain/configurationRule/check"} />
...
:::info
Pour plus d'informations sur les différents types de contact, veuillez consulter la [documentation associée](/guides/web-cloud/domains/api-domain-contacts).
:::
...
{/* prettier-ignore */}
<Tabs>
  <Tab label="Retour d'erreur de validation">
    ```json
    {
