...
description: "Découvrez comment réutiliser le WiFi d'un modem Zyxel avec votre OverTheBox"
...
Dès lors, l'utilisation de ces réseaux WiFi présente nettement moins d'intérêt, et pourrait vous orienter vers l'utilisation d'un point d'accès WiFi dédié pour profiter du WiFi avec l'OverTheBox.
...
- Disposer d'un [accès Internet xDSL ou FTTH OVHcloud](https://www.ovhcloud.com/fr/internet/) et d'un modem Zyxel fourni par OVHcloud.
...
:::info
Ce guide ne concerne que les modems de marque Zyxel. Si vous possédez un modem Thomson ou Technicolor, vous pouvez demander un échange de confort facturé 70 € HT (84 € TTC). Pour cela, depuis votre <ManagerLink to="/#/telecom/pack">espace client OVHcloud</ManagerLink> partie « Télécom », cliquez sur <code className="action">Accès Internet</code>, puis sélectionnez le pack et l’accès à Internet concerné. Assurez-vous d'être positionné sur l'onglet <code className="action">Mon Accès</code>. En bas de la page à gauche, cliquez sur le bouton <code className="action">Échanger mon modem</code>.
...
<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step1-2022.png" loading="lazy" />
...
- Si la configuration à distance de votre modem était déjà désactivée et que vous avez effectué une réinitialisation, saisissez l'utilisateur **admin** ainsi que le mot de passe sur l'étiquette au dos du modem.
...
<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step2.png" loading="lazy" />

:::info
L'ensemble des e-mails envoyés par OVHcloud sont accessibles depuis votre espace client. Pour retrouver vos derniers identifiants de connexion reçus par e-mail, connectez-vous à votre <ManagerLink to="/#/communication">espace client OVHcloud</ManagerLink> depuis un autre accès à Internet ou un smartphone. Une fois connecté, cliquez sur votre nom puis sur <code className="action">E-mails de service</code> à droite de l'écran.
...
<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step3-1.png" loading="lazy" />

Sur cette page, cliquez sur le bouton <code className="action">+ Add New Interface Group</code>.

<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step3-2.png" loading="lazy" />
...
<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step3-3-2022.png" loading="lazy" />

### Étape 4 : modifier la gestion du DHCP pour le groupe OverTheBox

Accédez maintenant à la page <code className="action">Home Networking</code> qui se trouve également dans le menu `Network Settings`. Depuis cette page, il vous faut désactiver le DHCP du modem sur le groupe `OverTheBox`, afin que le serveur DHCP utilisé soit celui de l'OverTheBox.
...
Une fois les modifications effectuées, cliquez sur le bouton <code className="action">Apply</code> pour appliquer votre configuration.
...
À l'aide d'un appareil pouvant se connecter au WiFi, connectez-vous au WiFi partagé par votre modem Zyxel
...
<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step6-1.png" loading="lazy" />

Pour vérifier que votre appareil utilise bien le service OverTheBox, vous pouvez faire un test de débit sur notre site [proof.ovh.net](http://proof.ovh.net) et vérifier que le champ IPv4 correspond à l'adresse IP publique de votre service OverTheBox (151.127.x.y dans l'image).

<img className="thumbnail" alt="reutiliserWiFiOTB" src="/images/web-cloud/internet/internet-access/comment-reutiliser-wifi-zyxel-otb/reutiliserWiFiOTB-step6-2.png" loading="lazy" />
