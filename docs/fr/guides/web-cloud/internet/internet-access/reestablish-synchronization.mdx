...
description: Découvrez comment rétablir la synchronisation de votre ligne xDSL OVHcloud suite à une coupure
...
:::info
Pour vous accompagner au mieux dans le rétablissement de vos services, vous avez également la possibilité d'utiliser notre assistant de dépannage. Pour y accéder, connectez-vous à votre <ManagerLink to="/#/telecom/pack">espace client OVHcloud</ManagerLink> dans la section « Télécom ». Une fois positionné sur l'accès Internet concerné, cliquez sur <code className="action">Lancer le diagnostic</code>. 
...
Si cela est nécessaire, reportez-vous aux instructions décrites dans notre documentation « [Rétablir son service suite à une coupure complète ou partielle](/guides/web-cloud/internet/internet-access/interruption-de-service) » afin de vous en assurer. Dès lors, si la panne rencontrée est bien liée à une coupure de synchronisation, poursuivez la lecture de cette documentation afin de réaliser quelques vérifications préalables. 
...
Rendez-vous sur la page [https://status.isp.ovhcloud.com/](https://status.isp.ovhcloud.com/) et vérifiez si votre accès est affecté par incident en cours. Dès lors, deux possibilités :
...
Par exemple, si vous constatez que le poteau où se trouve le câble de votre ligne fixe est arraché ou mal attaché, il est impératif de le déclarer sur le site ci-dessous en y communiquant un maximum d'informations : [https://dommages-reseaux.orange.fr/](https://dommages-reseaux.orange.fr/). 
...
Une fois cette action effectuée, redémarrez votre modem. Si besoin, reportez-vous aux instructions décrites dans notre documentation « [Redémarrer ou réinitialiser un modem ADSL OVHcloud](/guides/web-cloud/internet/internet-access/restart-reboot-modem) ».
...
<img className="thumbnail" alt="adslsynchronisation" src="/images/web-cloud/internet/internet-access/reestablish-synchronization/rj11_filtre.png" loading="lazy" />
...
Le dernier test possible consiste à réinitialiser votre modem. Si besoin, reportez-vous aux instructions décrites dans notre documentation « [Redémarrer ou réinitialiser un modem ADSL OVHcloud](/guides/web-cloud/internet/internet-access/restart-reboot-modem) ».
