...
description: "Découvrez comment configurer la qualité de service (QoS) sur votre OverTheBox afin d'améliorer la gestion de votre bande passante"
...
<img className="thumbnail" alt="overthebox" src="/images/web-cloud/internet/overthebox/config-qos/step2-tc-1-selectInterface-2024.png" loading="lazy" />

- Rendez-vous sur l'onglet <code className="action">Traffic Control</code>.
- Modifiez le paramètre <code className="action">Traffic Control</code> en `Static` à l'aide du menu déroulant.
- Dans le paramètre <code className="action">Download bandwidth</code>, renseignez le débit descendant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 10Mbps, la valeur est `10000` Kbps.
- Dans le paramètre <code className="action">Upload bandwidth</code>, renseignez le débit montant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 1,2Mbps, la valeur est `1200` Kbps.
...
<img className="thumbnail" alt="overthebox" src="/images/web-cloud/internet/overthebox/config-qos/step2-tc-2-configureInterface-2024.png" loading="lazy" />
...
- Rendez-vous dans l'onglet <code className="action">OverTheBox > QoS Settings</code>.
- L'écran de configuration listant toutes les règles QoS s'affiche.
- Cliquez sur <code className="action">Add</code>.

:::info
Les règles sont testées dans l'ordre du tableau, utilisez le bouton <code className="action">≡</code> pour modifier la position d'une règle.
:::


<img className="thumbnail" alt="overthebox" src="/images/web-cloud/internet/overthebox/config-qos/step3-tc-1-addRule-2024.png" loading="lazy" />

- Rendez-vous sur l'onglet <code className="action">Traffic Control</code>.
- Modifiez le paramètre <code className="action">Direction</code> pour sélectionner le sens de la règle, montant ou descendant. Pour notre exemple, nous souhaitons que le débit montant émanant du **NAS** soit prioritaire, nous renseignons donc `upload`.
- Modifiez le paramètre <code className="action">Protocol</code> pour sélectionner le protocole sur lequel s'applique la règle. Pour notre exemple, nous souhaitons n'importe quel protocole, nous renseignons donc `all`.
- Modifiez le paramètre <code className="action">Source Host</code> pour sélectionner la source du trafic. Pour notre exemple, le **NAS** est l'hôte source, nous renseignons donc son adresse IP `192.168.100.80`.
- Modifiez le paramètre <code className="action">Destination Host</code> pour sélectionner la destination du trafic. Pour notre exemple, nous souhaitons n'importe quelle destination, nous renseignons donc `all`.
- Modifiez le paramètre <code className="action">Class</code> pour sélectionner la priorité du trafic. Pour notre exemple, nous souhaitons que le trafic soit prioritaire, nous renseignons donc `High priority`.
- Modifiez le paramètre <code className="action">Comment</code> pour nommer la règle. Pour notre exemple, nous nommons la règle `NAS`.
- Confirmez vos changements avec le bouton <code className="action">Save</code>.
- Appliquez vos changements avec le bouton <code className="action">Save & Apply</code>.

<img className="thumbnail" alt="overthebox" src="/images/web-cloud/internet/overthebox/config-qos/step3-tc-2-configureRule-2024.png" loading="lazy" />
...
Votre **OverTheBox** contient un service qui s'appelle <code className="action">otb-auto-sqm</code> qui est capable de lancer des tâches en arrière-plan afin de configurer automatiquement SQM sur toutes les interfaces WAN disponibles.
...
- Rendez-vous dans l'onglet <code className="action">Network > SQM QoS</code>.
- L'écran de configuration listant la configuration **SQM** s'affiche.
- Cliquez sur <code className="action">Add</code>.

<img className="thumbnail" alt="overthebox" src="/images/web-cloud/internet/overthebox/config-qos/step2-sqm-1-addConfig-2024.png" loading="lazy" />

- Cochez la case du paramètre <code className="action">Enable this SQM instance</code> pour activer la configuration **SQM**.
- Modifiez le paramètre <code className="action">Interface name</code> pour sélectionner l'interface sur laquelle **SQM** s'exécute. Pour notre exemple, notre lien **ADSL** est l'interface `eth2`, nous renseignons donc `eth2`.
- Dans le paramètre <code className="action">Download speed (ingress)</code>, renseignez le débit descendant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 25Mbps, la valeur est `25000` Kbps.
- Dans le paramètre <code className="action">Upload speed (egress)</code>, renseignez le débit montant maximum de la connexion en **Kbps**. Pour notre exemple, pour un débit de 3Mbps, la valeur est `3000` Kbps.
- Appliquez vos changements avec le bouton <code className="action">Save & Apply</code>.

<img className="thumbnail" alt="overthebox" src="/images/web-cloud/internet/overthebox/config-qos/step2-sqm-2-configure-2024.png" loading="lazy" />
...
:::warning
Cette section concerne la page <code className="action">OverTheBox > QoS Settings</code> disponible uniquement sur les versions v0.9 et antérieures.
:::
