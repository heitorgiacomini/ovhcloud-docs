...
description: Retrouvez les réponses aux questions les plus fréquemment posées sur le service SMS OVHcloud
...
<details>
<summary>Comment créer un compte SMS OVHcloud ?</summary>


Pour créer un compte SMS OVHcloud, rendez-vous sur la [page dédiée aux offres SMS OVHcloud](https://www.ovhcloud.com/fr/sms/) et choisissez le pack de crédits adapté à vos besoins. La commande créera automatiquement un compte SMS accessible depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, dans la <ManagerLink to="/#/telecom/sms">section SMS</ManagerLink>. Chaque compte SMS est identifié par un nom unique (ex : `sms-xx12345-1`). Vous pouvez avoir plusieurs comptes SMS sur un même identifiant client OVHcloud, ce qui permet de séparer les usages (transactionnel, marketing, notifications internes) et les budgets.

</details>


<details>
<summary>Comment fonctionne le système de crédits SMS ?</summary>
...
- **La destination :** un SMS vers la France métropolitaine consomme 1 crédit. Les destinations internationales consomment plus de crédits (consultez la [grille tarifaire OVHcloud](https://www.ovhcloud.com/fr/sms/prices/)).
...
Pour plus de détails, consultez le guide « [Gérer les crédits SMS et activer la recharge automatique](/guides/web-cloud/messaging/sms/activer-la-recharge-automatique-du-credit-sms) ».

</details>


<details>
<summary>Comment activer la recharge automatique de crédits SMS ?</summary>


La recharge automatique permet de recréditer automatiquement votre compte SMS lorsque le solde passe en dessous d'un seuil défini. <ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis accédez à <code className="action">Options</code> > <code className="action">Recharge automatique</code>. Configurez le seuil de déclenchement et le montant de la recharge. Un moyen de paiement valide doit être enregistré sur votre compte. Cette option est indispensable pour les envois de SMS transactionnels où une rupture de crédits bloquerait vos notifications critiques.
...
<details>
<summary>Comment consulter l'historique de mes SMS envoyés et reçus ?</summary>


<ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS puis l'onglet <code className="action">Message et campagne</code> > <code className="action">Gestion des SMS</code> > <code className="action">Historique des envois</code> ou <code className="action">SMS reçus</code>. Vous pouvez filtrer l'historique par date, expéditeur, destinataire ou message. L'historique est conservé pendant 6 mois. L'historique peut également être exporté au format CSV pour analyse.

Pour plus de détails, consultez le guide « [Gérer l'historique des SMS](/guides/web-cloud/messaging/sms/sms-history) ».

</details>


<details>
<summary>Comment paramétrer des alertes de seuil de crédits SMS ?</summary>


Pour les **utilisateurs API**, <ManagerLink to="/#/telecom/sms">accédez à la section SMS</ManagerLink>, sélectionnez votre compte, puis <code className="action">Utilisateurs API</code>. Cliquez sur <code className="action">...</code> > <code className="action">Limite</code> pour l'utilisateur concerné et configurez :
...
Pour plus de détails, consultez le guide « [Tout savoir sur les utilisateurs SMS](/guides/web-cloud/messaging/sms/sms-users) ».
...
<details>
<summary>Comment envoyer un SMS depuis l'espace client OVHcloud ?</summary>


<ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis cliquez sur <code className="action">Envoyer un SMS</code>. Renseignez :
...
Pour plus de détails, consultez le guide « [Envoyer des SMS depuis l'espace client OVHcloud](/guides/web-cloud/messaging/sms/send-sms-control-panel) ».

</details>


<details>
<summary>Comment envoyer des SMS via une URL (http2sms) ?</summary>
...
Pour plus de détails, consultez le guide « [Envoyer des SMS depuis une URL - http2sms](/guides/web-cloud/messaging/sms/send-sms-http2sms) ».

</details>


<details>
<summary>Comment envoyer des SMS depuis une adresse e-mail ?</summary>
...
Pour plus de détails, consultez le guide « [Envoyer des SMS depuis une adresse e-mail](/guides/web-cloud/messaging/sms/send-sms-email) ».

</details>


<details>
<summary>Comment envoyer des SMS en masse de manière performante via l'API ?</summary>
...
<details>
<summary>Comment créer ma première campagne SMS ?</summary>


<ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis cliquez sur <code className="action">Envoyer un SMS</code>. Les étapes sont :
...
Le suivi de la campagne est disponible dans <code className="action">Message et campagne</code> > <code className="action">Gestion des campagnes</code> > <code className="action">Statistiques et historique</code>.

Pour plus de détails, consultez le guide « [Ma première campagne SMS](/guides/web-cloud/messaging/sms/ma-premiere-campagne-sms) ».

</details>


<details>
<summary>Comment gérer mes listes de destinataires SMS ?</summary>


<ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis accédez à <code className="action">Contacts</code> > <code className="action">Créer une liste de contacts</code>. Vous pouvez :
...
Pour plus de détails, consultez le guide « [Liste de destinataires SMS](/guides/web-cloud/messaging/sms/liste-de-destinataire-sms) ».

</details>


<details>
<summary>Comment gérer mes carnets d'adresses SMS ?</summary>


Les carnets d'adresses SMS offrent une gestion plus riche des contacts (nom, prénom, numéro). <ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink>, sélectionnez votre compte SMS, puis accédez à <code className="action">Contacts</code> > <code className="action">Carnet d'adresses</code>. Vous pouvez :
...
Pour plus de détails, consultez le guide « [Gérer mes carnets d'adresses SMS](/guides/web-cloud/messaging/sms/sms-address-books) ».

</details>


<details>
<summary>Quelle est la différence entre un SMS standard (160 caractères) et un SMS long ?</summary>
...
<details>
<summary>Quels caractères spéciaux font basculer un SMS en encodage Unicode ?</summary>
...
<details>
<summary>Quel est le délai de livraison d'un SMS envoyé via OVHcloud ?</summary>
...
<details>
<summary>Mes SMS ne sont pas délivrés, comment diagnostiquer le problème ?</summary>


Procédez aux vérifications suivantes :

1. **Vérifiez votre solde de crédits :** un solde à zéro bloque immédiatement tous les envois.
2. **Consultez l'historique d'envoi :** dans <code className="action">Message et campagne</code> > <code className="action">Gestion des SMS</code> > <code className="action">Historique des envois</code>, vérifiez le statut de chaque SMS. Un code PTT indique la raison de l'échec.
...
<details>
<summary>Quels types d'expéditeurs SMS puis-je utiliser ?</summary>
...
Pour plus de détails, consultez le guide « [Tout savoir sur les expéditeurs SMS](/guides/web-cloud/messaging/sms/sms-senders) ».

</details>


<details>
<summary>Comment ajouter un expéditeur alphanumérique personnalisé ?</summary>


<ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis cliquez sur l'onglet <code className="action">Expéditeurs</code>. Cliquez sur <code className="action">Actions</code> > <code className="action">Ajouter</code> et choisissez « Ajouter manuellement des expéditeurs ». Renseignez :
...
<details>
<summary>Pourquoi ne puis-je pas envoyer un SMS contenant une URL avec un numéro court ?</summary>
...
<details>
<summary>Qu'est-ce qu'un numéro mobile virtuel (VLN) et comment l'obtenir ?</summary>
...
4. La réponse est consultable dans votre espace client OVHcloud (<code className="action">Message et campagne</code> > <code className="action">Gestion des SMS</code> > <code className="action">SMS reçus</code>).
...
Pour plus de détails, consultez le guide « [Envoyer des SMS permettant la réponse](/guides/web-cloud/messaging/sms/sms-with-reply) ».

</details>


<details>
<summary>Comment configurer une réponse automatique aux SMS reçus ?</summary>


<ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis accédez à <code className="action">Options</code> > <code className="action">Options des réponses</code>. Dans la section « Action à la réception », choisissez « Répondre un texte prédéfini » ou « Appeler un CGI » (URL d'un script web qui sera appelé à chaque réponse reçue, permettant un traitement dynamique). Vous pouvez également configurer des notifications à la réception (par e-mail ou SMS).
...
<details>
<summary>Qu'est-ce que le service Time2Chat OVHcloud ?</summary>
...
<details>
<summary>Comment créer et gérer des utilisateurs API pour les SMS ?</summary>


Les utilisateurs API SMS permettent de déléguer l'envoi de SMS via l'API ou la fonction http2sms sans exposer vos identifiants client OVHcloud. <ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte SMS, puis cliquez sur <code className="action">Utilisateurs API</code>. Ajoutez un nouvel utilisateur en définissant un identifiant et un mot de passe. Chaque utilisateur peut se voir attribuer :
...
<details>
<summary>Comment envoyer un SMS via l'API OVHcloud en PHP ?</summary>
...
Pour plus de détails, consultez le guide « [Envoyer des SMS avec l'API OVHcloud en PHP](/guides/web-cloud/messaging/sms/send-sms-api-php) ».

</details>


<details>
<summary>Quels sont les endpoints API disponibles pour le service SMS OVHcloud ?</summary>
...
<details>
<summary>Comment recevoir les accusés de réception (DLR) de mes SMS via un callback ?</summary>


Les accusés de réception (Delivery Reports / DLR) confirment la bonne livraison d'un SMS. Pour les recevoir automatiquement, configurez une URL de callback sur votre utilisateur API. <ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink>, puis <code className="action">Utilisateurs API</code>, cliquez sur <code className="action">...</code> > <code className="action">Callback</code> pour l'utilisateur concerné. Renseignez l'URL de votre endpoint web. À chaque mise à jour de statut, OVHcloud appellera cette URL avec les paramètres suivants :
...
<details>
<summary>Comment utiliser le protocole SMPP avec OVHcloud ?</summary>


Le protocole SMPP (Short Message Peer-to-Peer) est un protocole industriel pour l'envoi et la réception de SMS en volume. <ManagerLink to="/#/telecom/sms">Accédez à la section SMS</ManagerLink> de votre espace client OVHcloud, sélectionnez votre compte, puis accédez à <code className="action">Options</code> > <code className="action">Paramètres SMPP</code>. Vous obtiendrez les paramètres de connexion : adresse du serveur SMPP, port, system_id et password. Le protocole SMPP offre une connexion persistante, un débit d'envoi plus élevé, et un support natif des DLR en mode push.

Pour plus de détails, consultez le guide « [Gestion d'un compte SMS SMPP](/guides/web-cloud/messaging/sms/smpp-control-panel) ».

</details>


<details>
<summary>Quelles sont les spécifications techniques du service SMPP OVHcloud ?</summary>
...
Pour plus de détails, consultez le guide « [Spécifications techniques du SMPP](/guides/web-cloud/messaging/sms/smpp-specification) ».

</details>


<details>
<summary>Comment intégrer l'envoi de SMS OVHcloud dans mon application métier ou CRM ?</summary>
...
<details>
<summary>Quelles sont les limites de débit d'envoi de SMS ?</summary>
...
<details>
<summary>Puis-je envoyer des SMS contenant des variables personnalisées ?</summary>
...
<details>
<summary>Quelles sont les obligations légales pour l'envoi de SMS marketing en France ?</summary>
...
<details>
<summary>Quelle est la différence entre un SMS transactionnel et un SMS marketing ?</summary>
...
<details>
<summary>Comment optimiser le taux de délivrabilité de mes SMS ?</summary>
...
<details>
<summary>Comment envoyer des SMS vers les États-Unis ?</summary>
...
- Un coût en crédits supérieur à un SMS national (consultez la [grille tarifaire](https://www.ovhcloud.com/fr/sms/prices/) pour le tarif exact).

Le taux de délivrabilité peut varier en fonction des opérateurs américains et de leurs filtres anti-spam.

Pour plus de détails, consultez le guide « [Envoi de SMS aux États-Unis](/guides/web-cloud/messaging/sms/envoi-de-sms-aux-etats-unis) ».

</details>


<details>
<summary>Qu'est-ce qu'une requête HLR et à quoi sert-elle ?</summary>
...
Chaque requête HLR coûte **0,1 crédit SMS**. Le résultat indique : le statut du numéro (actif/inactif), l'opérateur d'enregistrement, la joignabilité, le statut de portabilité, et le statut d'itinérance. Les requêtes HLR sont accessibles depuis l'espace client OVHcloud (<code className="action">Message et campagne</code> > <code className="action">Gestion des SMS</code> > <code className="action">HLR</code>) ou via l'API.

Pour plus de détails, consultez le guide « [Tout ce qu'il faut savoir sur le HLR](/guides/web-cloud/messaging/sms/sms-hlr) ».

</details>


<details>
<summary>Comment nettoyer une base de contacts SMS avec le HLR ?</summary>


Le nettoyage HLR s'effectue depuis l'espace client OVHcloud :

1. **Importez votre fichier de contacts :** dans <code className="action">Contacts</code> et chargez votre fichier CSV (colonne : `numbers` au format international).
...
<details>
<summary>Quelles sont les restrictions géographiques pour l'envoi de SMS OVHcloud ?</summary>
...
- **Tarification variable :** le coût en crédits varie selon la destination. Les tarifs sont consultables dans votre espace client OVHcloud ou sur la [page tarifaire OVHcloud](https://www.ovhcloud.com/fr/sms/prices/).
