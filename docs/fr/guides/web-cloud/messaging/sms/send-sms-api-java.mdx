...
description: Découvrez comment configurer un environnement de développement Java et envoyer votre premier SMS avec l’API RESTful OVHcloud pas à pas
...
- Disposer d'un [compte SMS OVHcloud](/links/telecom/sms) avec des crédits SMS
...
Créez vos identifiants de Script (toutes les clés en une fois) sur cette page : [https://auth.eu.ovhcloud.com/api/createToken](https://auth.eu.ovhcloud.com/api/createToken?GET=/sms/&GET=/sms/*/jobs&POST=/sms/*/jobs) (Cette URL vous permet d'avoir automatiquement les bons droits pour les étapes décrites dans ce guide).
...
<img className="thumbnail" alt="récupération des tokens" src="/images/web-cloud/messaging/sms/send-sms-api-java/img_2480.jpg" loading="lazy" />
...
Pour envoyer des SMS, utilisez la méthode POST jobs :
