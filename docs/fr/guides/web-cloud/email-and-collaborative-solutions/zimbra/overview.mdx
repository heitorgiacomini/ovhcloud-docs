---
pageType: overview
title: Zimbra
description: "Configurez et exploitez votre solution d'e-mail collaborative OVHcloud Zimbra - création de boîtes, synchronisation mobile, dossiers partagés et administration."
text: "Découvrez comment gérer votre solution e-mail Zimbra"
essentialsTitle: "Les guides essentiels pour commencer"
essentials:
  - title: "Premiers pas avec l'offre Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/getting-started-zimbra
  - title: "Utiliser le webmail Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-zimbra
  - title: FAQ sur la solution Zimbra OVHcloud
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/faq-zimbra
  - title: "Configurer une adresse e-mail Zimbra sur un logiciel de messagerie"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/mail-apps
  - title: "Migrer une adresse e-mail MX Plan vers un compte Zimbra OVHcloud"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/migrate-mxplan-to-zimbra
  - title: "Zimbra - Configurer son compte e-mail sur l'application mobile Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/mail-app-zimbra-for-android-ios
goFurther:
  title: Aller plus loin
  items:
    - title: Consulter la communauté OVHcloud
      link: https://community.ovh.com/
    - title: Consulter la roadmap & le changelog
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: Rejoindre Discord
      link: https://discord.gg/ovhcloud
cta:
  title: Des questions ? Accédez au support OVHcloud
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: Accéder au support
      link: https://help.ovhcloud.com/
      theme: brand
availableIn: [eu]
---
