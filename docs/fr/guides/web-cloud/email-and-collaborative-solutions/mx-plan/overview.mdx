---
pageType: overview
title: MX Plan
description: "Démarrez avec la solution e-mail OVHcloud MX Plan - création de boîtes mail, configuration client, redirections, listes de diffusion et antispam."
text: "Découvrez comment bien débuter avec votre offre MX Plan"
essentialsTitle: "Les guides essentiels pour commencer"
essentials:
  - title: "Premiers pas avec l'offre MX Plan"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities
  - title: "Créer une adresse e-mail avec son offre MX Plan"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation
  - title: "Modifier le mot de passe d'une adresse e-mail"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-change-password
  - title: "Utiliser son adresse e-mail depuis le webmail Roundcube"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-roundcube
  - title: FAQ e-mails OVHcloud
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/faq-emails
  - title: "Utiliser le webmail Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-zimbra
goFurther:
  title: Aller plus loin
  items:
    - title: Consulter la communauté OVHcloud
      link: https://community.ovh.com/
    - title: Consulter la roadmap & le changelog
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: Rejoindre Discord
      link: https://discord.gg/ovhcloud
cta:
  title: Des questions ? Accédez au support OVHcloud
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: Accéder au support
      link: https://help.ovhcloud.com/
      theme: brand
availableIn: [eu, ca, apac]
---
