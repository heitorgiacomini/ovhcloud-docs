...
Pour réactiver votre abonnement, il suffit de reconfigurer sa [périodicité de facturation](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/manage-billing-exchange#periodicity) dans l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>.

### Le compte est bloqué par la politique de sécurité

Si une politique de sécurité est activée dans l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>, le compte peut être temporairement verrouillé.
...
Si nécessaire, vous pouvez modifier le mot de passe du compte concerné dans l'onglet <code className="action">Comptes e-mail</code> de votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Si le problème persiste, créez une demande d'assistance.
