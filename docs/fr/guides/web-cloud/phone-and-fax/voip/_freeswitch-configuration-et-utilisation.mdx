...
Pour plus d'informations sur Freeswitch : [https://freeswitch.org/confluence/display/FREESWITCH/FreeSWITCH+Explained](https://freeswitch.org/confluence/display/FREESWITCH/FreeSWITCH+Explained)
...
-   Une ligne SIP Trunk. Pour en commander une : **[https://www.ovhtelecom.fr/telephonie/sip_trunk/](https://www.ovhtelecom.fr/telephonie/sip_trunk/)**
...
Nous recommandons de compiler Freswitch sur votre machine directement. Vous obtiendrez plus d'informations via ce lien : **[https://freeswitch.org/confluence/display/FREESWITCH/Debian](https://freeswitch.org/confluence/display/FREESWITCH/Debian)**
...
**siptrunk.ovh.net.xml**
    ...
    `<include>` <gateway name="trunk-ovh"> 
        <param name="username" value="0033972320690"/> 
        <param name="password" value="2484fUfFGRgw"/> 
        <param name="realm" value="siptrunk.ovh.net"/>  <param
        name="proxy" value="siptrunk.ovh.net"/>  <param
        name="register" value="true"/> 
        <param name="caller-id-in-from" value="true"/>  <param name="expire-seconds" value="1800"/>  </gateway> </include>
...
**/usr/local/freeswitch/conf/directory/default/200.xml**
    ...
    `<include>` <user id="200">  `<params>` <param name="password" value="MyPassword"/>
         <param name="vm-password" value="3214"/>
        ...
        name="effective_caller_id_number" value="0033185450330"/> 
...
**/usr/local/freeswitch/conf/dialplan/default.xml**
    ...
    [..] <extension name="Local_Extension">
        <condition field="destination_number" expression="^(10[01][0-9]|200|300)$">  <action application="export"
        data="dialed_extension=$1"/>  <action application="bind_meta_app" data="1 b s execute_extension::dx XML
        ...
        data="hangup_after_bridge=true"/>  <action application="set" data="continue_on_fail=true"/> <action
        application="hash"
...
**/usr/local/freeswitch/conf/dialplan/default/01\_custom.xml**
    ...
    `<include>` <extension
        ...
        expression="^(0[1-7]\d{8})$">  <action application="bridge"
        ...
        expression="^(09\d{8})$">  <action application="bridge" data="sofia/gateway/trunk-ovh/$1"/>
...
**/usr/local/freeswitch/conf/dialplan/public/00\_inbound\_did.xml**
     ...
     `<include>` <extension name="public_did"> <condition field="$\{sip_to_user\}" expression="(366725520)$"> <action application="set" data="domain_name=$$\{domain\}"/> <action application="bridge" data="sofia/internal/200%$\{sip_profile\}"/> </condition> </extension> </include>  `<include>` <extension name="public_did"> <condition field="$\{sip_to_user\}" expression="(185450330)$"> <action application="set" data="domain_name=$$\{domain\}"/> <action application="bridge" data="sofia/internal/300%$\{sip_profile\}"/> </condition> </extension> </include>  `<include>` <extension name="public_did"> <condition field="$\{sip_to_user\}" expression="(972320690)$"> <action application="answer" /> <action application="playback" data="/tmp/son.wav"/>  <action application="set" data="domain_name=$$\{domain\}"/> </condition> </extension> </include>
