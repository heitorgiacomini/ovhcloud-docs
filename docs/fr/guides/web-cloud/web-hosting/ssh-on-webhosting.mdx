...
description: "Découvrez comment vous connecter et utiliser l'accès SSH de votre hébergement web OVHcloud"
...
- Disposer d'une offre d'[hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) bénéficiant d'un accès SSH.
...
Si l'accès SSH de l'utilisateur concerné est `Désactivé` dans le tableau, effectuez les opérations suivantes :
    ...
    - 2 : Dans la fenêtre qui s'affiche, section `Protocoles de connexion`, sélectionnez le choix <code className="action">FTP, SFTP et SSH</code>, puis cliquez sur <code className="action">Suivant</code>.
    - 3 : Vérifiez le résumé de la modification demandée, puis cliquez sur <code className="action">Valider</code>.
    
    > Si vous n'avez pas la possibilité de l'activer, assurez-vous que [votre offre d'hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) bénéficie bien d'un accès SSH.
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab label="Étape 3">
    ...
    |**Mot de passe de l'utilisateur SSH**| Si vous avez oublié ce mot de passe, cliquez sur le bouton <code className="action">...</code> à droite de la ligne correspondant à l'utilisateur concerné dans le tableau situé en bas de page, puis sur <code className="action">Changer le mot de passe</code>.|
  </Tab>
</Tabs>
...
:::info
Les terminaux de commande sont installés par défaut sur macOS, Linux et Windows 10. Un environnement Windows plus ancien nécessitera l’installation d’un logiciel comme [PuTTY](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows) ou l’ajout de la fonctionnalité OpenSSH.
...
<summary>Depuis un terminal</summary>
...
<summary>Depuis un logiciel</summary>
...
Retrouvez plus d'information sur l'utilisation de la commande `scp` avec nos hébergements web OVHcloud dans notre guide « [Hébergement Web - Copier des fichiers avec la commande SCP](/guides/web-cloud/web-hosting/using-scp-command) ».
