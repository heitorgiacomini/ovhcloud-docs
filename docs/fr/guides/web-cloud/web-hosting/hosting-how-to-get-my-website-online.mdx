...
- Disposer d'un [nom de domaine](https://www.ovhcloud.com/fr/domains/) qui sera l'adresse d'accès à votre site web.
...
- **utiliser un accès SSH** : vous devrez utiliser des commandes depuis un terminal pour interagir avec votre espace de stockage. Des connaissances plus avancées, ainsi qu'une offre d'[hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) spécifique sont nécessaires pour utiliser ce type d'accès.
...
Pour vérifier le dossier dans lequel doit être publié le site web, positionnez-vous sur l'onglet <code className="action">Multisite</code> depuis votre espace client OVHcloud. Dans le tableau qui s'affiche, pour le domaine souhaité, regardez le <code className="action">Dossier racine</code> qui s'affiche. Publiez alors les fichiers du site dans ce dernier.
...
Si vous disposez déjà d'une base de données que vous souhaitez utiliser (issue d'une offre d'[hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) ou [Web Cloud Databases](https://www.ovhcloud.com/fr/web-cloud/databases/)), munissez-vous du nom d'utilisateur et de son mot de passe, du nom de la base de données ainsi que de l'adresse du serveur. Poursuivez alors vers l'étape suivante.

Si vous souhaitez créer une nouvelle base de données chez OVHcloud, connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, puis cliquez sur <code className="action">Hébergements</code>. Choisissez le nom de l'hébergement concerné et positionnez-vous enfin sur l'onglet <code className="action">Base de données</code>.
...
Dès lors, cliquez sur le bouton <code className="action">Créer une base de données</code> ou, si ce dernier n'apparaît pas, sur <code className="action">Actions</code> puis sur <code className="action">Créer une base de données</code>. Suivez alors les informations qui s'affichent.
...
Pour rappel, si vous rencontrez des difficultés lors de la mise en ligne de votre site web, nous vous recommandons de faire appel à un [prestataire spécialisé](https://partner.ovhcloud.com/fr/directory/) et/ou de contacter l'éditeur du service (du CMS installé par exemple).
