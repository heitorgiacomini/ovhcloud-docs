---
title: Spécificités techniques liées aux hébergements web
description: Découvrez dans ce guide différentes informations et spécificités techniques liées aux hébergements Web
...
- Si vous rencontrez l'erreur d'accès « Échec de l'authentification de connexion 530 » lors de la connexion à votre espace de stockage FTP: Assurez-vous que les informations d'accès à votre espace FTP sont correctes. Pour cela, connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> puis rendez-vous dans la partie <code className="action">Web Cloud</code>. Dans la colonne de gauche, cliquez sur <code className="action">Hébergements</code> puis sélectionnez l'hébergement web concerné. Sur la page qui s'affiche, cliquez sur l'onglet <code className="action">FTP - SSH</code>.
...
Retrouvez plus d'informations sur le sujet dans notre guide « [Se connecter à l'espace de stockage FTP de son hébergement Web](/guides/web-cloud/web-hosting/ftp-connection) ».
...
|Offres|Hébergement gratuit|Starter|Perso|Startup|Pro|Performance|Agency|Agency Plus|Agency Max|
|---|---|---|---|---|---|---|---|---|---|
|Quantité maximale d'envoi d'e-mails par heure et par service|10|20|100|100|200|2000|2000|2000|2000|

</Region>

<Region zones={['ca', 'apac']}>

|Offres|Starter|Perso|Startup|Pro|Performance|Agency|Agency Plus|Agency Max|
|---|---|---|---|---|---|---|---|---|
|Quantité maximale d'envoi d'e-mails par heure et par service|20|100|100|200|2000|2000|2000|2000|
...
Pour en savoir plus, consultez les détails de nos [offres d'hébergement web](https://www.ovhcloud.com/fr/web-hosting/) et notre guide « [Premiers pas avec votre offre Web Cloud Databases](/guides/web-cloud/databases/db-getting-started) ».
...
Pour plus d'informations sur la gestion des bases de données et sur la connection à l'interface *phpMyAdmin*, reportez-vous au guide « [Créer une base de données sur son hébergement web](/guides/web-cloud/web-hosting/sql-create-database) ».
...
Retrouvez plus de détails sur ce sujet dans notre guide « [Hébergement web : environnement, version PHP, « .ovhconfig »](/guides/web-cloud/web-hosting/configure-your-web-hosting) »
...
Retrouvez et vérifiez les librairies, langages et versions disponibles pour votre offre d'hébergement web depuis cette page : [https://webhosting-infos.hosting.ovh.net](https://webhosting-infos.hosting.ovh.net)

<Region zones={['eu']}>
Pour connaitre les spécificités techniques de l'offre Cloud Web, dirigez-vous vers cette page : [https://cloudweb-infos.hosting.ovh.net/](https://cloudweb-infos.hosting.ovh.net/).
</Region>
...
Découvrez comment [se connecter à l’espace de stockage FTP de votre hébergement web](/guides/web-cloud/web-hosting/ftp-connection) ou [restaurer l’espace de stockage FTP de son hébergement web](/guides/web-cloud/web-hosting/ftp-save-and-backup) sur nos documentations.
...
Pour les bases de données mutualisées (comprises dans votre offre d'hébergement Web) ou les serveurs de base de données (Web Cloud Databases), proposés sur Gravelines (GRA), en France et Beauharnois (BHS), au Canada, la sauvegarde des bases est faite tous les jours. Ces sauvegardes sont accessibles (via <ManagerLink to="/">l'espace client OVHcloud</ManagerLink> ou via les [API OVHcloud](https://eu.api.ovh.com/)). Les sauvegardes sont également stockées sur une autre infrastructure. Ces données sont répliquées dans un centre de données situé à Strasbourg (SBG). La politique de rétention des sauvegardes est de 30 jours.

Découvrez comment [Récupérer la sauvegarde de la base de données d’un hébergement web](/guides/web-cloud/web-hosting/sql-database-export) sur notre documentation.
...
[Restaurer l’espace de stockage FTP de son hébergement web](/guides/web-cloud/web-hosting/ftp-save-and-backup)
