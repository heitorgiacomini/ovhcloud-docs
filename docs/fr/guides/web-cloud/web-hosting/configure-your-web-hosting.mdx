...
Les offres d'[hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) permettent d’héberger le site web que vous souhaitez, tant que celui-ci est compatible avec la [configuration de nos infrastructures mutualisées](https://webhosting-infos.hosting.ovh.net).

La modification du fichier **php.ini** est indisponible sur les offres d'hébergements web, car la configuration PHP est globale à l'ensemble de l'infrastructure mutualisée.
...
- depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> ;
...
En résumé, modifier la configuration de votre hébergement web depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> ou modifier les valeurs présentes dans le fichier « .ovhconfig » revient au même.
...
<iframe className="video" width="560" height="315" src="https://www.youtube-nocookie.com/embed/X31MNMLw064" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen={true}></iframe>

## Prérequis

<Region zones={['eu']}>
- Disposer d’une offre d’[hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/), à l'exception d'une offre d'hébergement Cloud Web.
</Region>
...
Voici la description technique de chacun des paramètres modifiables sur les hébergements web OVHcloud.
...
Voici les éléments contenus dans nos différents environnements d'exécution :
...
- Aidez-vous de la [documentation officielle PHP](https://www.php.net/manual/en/appendices.php) donnant plus d'informations sur les migrations de version.
...
**Cliquez sur les 2 titres ci-dessous pour afficher le contenu.**

<details>
<summary>Via l'espace client OVHcloud</summary>
...
<Tabs>
  ...
  <Tab label="Étape 2">
    Dans l'onglet <code className="action">Informations générales</code>, repérez la version en dessous de *Version PHP globale*.
...
<details>
<summary>Via un script</summary>

Créez un script **.php** contenant uniquement le code suivant :

```php
<?php phpinfo(); ?>
```

Mettez-le ensuite en ligne sur votre [espace de stockage FTP](/guides/web-cloud/web-hosting/ftp-connection), puis appelez-le en accédant à son adresse/URL complète.
...
En comparaison, l'accélérateur PHP (« PHP-FPM ») est jusqu'à 7 fois plus rapide que le moteur *phpcgi*.
...
Consultez notre guide « [Activation du pare-feu applicatif](/guides/web-cloud/web-hosting/multisites-activating-application-firewall) » pour plus de détails sur le sujet.
...
Cliquez sur le bouton <code className="action">...</code> à droite de la mention **Version PHP Globale**, puis sur <code className="action">Modifier la configuration</code>.
  ...
  <Tab label="Étape 3">
    Sur la fenêtre qui apparaît, deux choix sont possibles. Sélectionnez celui qui correspond à l'action que vous souhaitez réaliser puis cliquez sur <code className="action">Suivant</code>.
    ...
    Dès que vous êtes prêt, cliquez sur <code className="action">Valider</code> pour appliquer la modification. Patientez quelques instants le temps qu'elle se réalise.
...
<Tabs>
  ...
  <Tab label="Étape 3">
    ...
    <img className="thumbnail" alt="FTP - SSH tab" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/ftp-ssh/tab-perso.png" loading="lazy" />

Concernant le mot de passe de l'utilisateur FTP, référez-vous aux instructions décrites dans notre documentation « [Modifier le mot de passe d’un utilisateur FTP](/guides/web-cloud/web-hosting/ftp-change-password) » si nécessaire.
  </Tab>
</Tabs>

#### 3.2 - Récupérer ou créer le fichier « .ovhconfig »

Une fois connecté à votre [espace de stockage FTP](/guides/web-cloud/web-hosting/ftp-connection), vous visualisez l'ensemble des fichiers actuellement hébergés sur ce dernier. Restez positionné sur la racine de votre hébergement (que l'on peut symboliser par un « / »). Vous devriez y trouver le fichier « .ovhconfig ».
...
|app.engine.version|Permet de définir [la version de PHP](#php-versions) utilisée par l'hébergement parmi [celles qu'OVHcloud propose](https://www.ovhcloud.com/fr/web-hosting/uc-programming-language/). Renseignez la version de votre choix (en adéquation avec l'environnement d'exécution que vous avez choisi d'utiliser).|
|http.firewall|Permet d'activer ou de désactiver le [pare-feu fourni avec les hébergements web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/options/). Renseignez **security** pour l'activer ou **none** pour le désactiver.|
...
|container.image|Permet de modifier [l'environnement d'exécution](#runtime-environment) utilisé par l'hébergement. Renseignez l'environnement d'exécution (**legacy**, **stable** ou **stable64**) de votre choix. Si vous choisissez l'environnement d'exécution **stable64**, vérifiez que votre site est compatible avec l'architecture 64 bits.|
...
Si nécessaire, voici la description technique détaillée du fichier « .ovhconfig » :
...
Une fois le fichier « .ovhconfig » modifié, téléchargez-le sur votre [espace de stockage FTP](/guides/web-cloud/web-hosting/ftp-connection). Pour cela, reconnectez-vous à votre [espace de stockage FTP](/guides/web-cloud/web-hosting/ftp-connection) et positionnez-vous à la racine de votre [espace de stockage FTP](/guides/web-cloud/web-hosting/ftp-connection) (que l'on peut symboliser par un « / »). Chargez le fichier « .ovhconfig » que vous venez de modifier dans votre [espace de stockage FTP](/guides/web-cloud/web-hosting/ftp-connection). Si le fichier existe déjà, remplacez-le.
