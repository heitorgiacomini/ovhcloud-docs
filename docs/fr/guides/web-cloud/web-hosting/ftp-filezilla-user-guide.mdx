...
Le **SFTP** utilise, comme le SSH, le port 22 par défaut au lieu du port 21. Si vous utilisez une offre d'hébergement Cloud Web, vous devez utiliser le port qui s'affiche dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Le port 22 est par sécurité désactivé en SSH et en SFTP pour les hébergements Cloud Web.
...
Pour cela, retournez dans l'onglet <code className="action">FTP-SSH</code> de votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> comme indiqué dans la [première partie](#part-1) de ce guide.
...
Vérifiez vos identifiants afin de vous assurer qu'aucune erreur ne soit renseignée. Le cas échéant, vous pouvez [modifier le mot de passe de l'accès FTP](/guides/web-cloud/web-hosting/ftp-change-password) de votre hébergement web directement dans l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>.
...
Vérifiez alors ce dernier par rapport au nom d'hôte déclaré dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>.
