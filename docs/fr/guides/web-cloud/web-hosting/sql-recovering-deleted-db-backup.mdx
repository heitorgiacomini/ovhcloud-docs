...
description: "Découvrez comment retrouver la sauvegarde d'une base de données lorsque celle-ci a été supprimée depuis votre espace client OVHcloud"
...
La plupart de nos offres d'[hébergement web](https://www.ovhcloud.com/fr/web-hosting/) comprennent des bases de données. Si, accidentellement, vous supprimez une base de données associée à votre hébergement web, vous pouvez tenter d'en récupérer la sauvegarde via nos API.
...
- Disposer d'une [offre d'hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) active comprenant une (ou plusieurs) base(s) de données mutualisée(s) OVHcloud associée(s).
- La suppression de la base de données doit dater de moins de 30 jours.
...
<Tabs>
  ...
  <Tab label="Étape 2">
    En haut à gauche de la page qui s'affiche, retrouvez le nom de votre hébergement web à droite de la mention <code className="action">Hébergements /</code>.
    
    <img className="thumbnail" alt="Nom de l'hébergement web dans l'espace client OVHcloud" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/general-information/find-webhosting-name.png" loading="lazy" />
  </Tab>
</Tabs>
...
- Sur la nouvelle page qui apparaît et dans la partie gauche de la page, positionnez-vous sur le formulaire situé à droite du formulaire <code className="action">v1</code>, puis sélectionnez/saisissez le choix `/hosting/web`.
...
- L'interface de connexion à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> s'ouvre.
- Connectez-vous avec votre identifiant client, puis cliquez sur <code className="action">Authorize</code> pour utiliser les API OVHcloud avec les services présents dans votre espace client.
...
- Pour la section intitulée `QUERY-STRING PARAMETERS` :
    ...
    - `databaseName` : Saisissez le nom de la base de données supprimée accidentellement. (exemple : **deletedDatabase.mysql.db**).
...
Une fois les différents formulaires remplis, cliquez sur le bouton bleu <code className="action">EXECUTE</code> situé en bas à droite des deux sections préalablement remplies.

Si tout a été renseigné correctement et que des sauvegardes sont disponibles pour la base de données supprimée, une liste de numéros d'identifiants de sauvegarde apparaît dans la fenêtre <code className="action">RESPONSE</code> lorsque vous descendez sur la page en dessous du bouton <code className="action">EXECUTE</code>.
...
Pour cela, restez sur notre site [API OVHcloud](https://eu.api.ovh.com/) et effectuez les actions suivantes :

- Dans la partie gauche de la page, positionnez-vous sur le formulaire situé à droite du formulaire <code className="action">v1</code>, puis sélectionnez/saisissez le choix <code className="action">/hosting/web</code>.
...
Une fois les différents formulaires remplis, cliquez sur le bouton bleu <code className="action">EXECUTE</code> situé en bas à droite de la section préalablement remplie.

Si tout a été renseigné correctement, le résultat suivant apparaît dans la fenêtre <code className="action">RESPONSE</code> lorsque vous descendez sur la page en dessous du bouton <code className="action">EXECUTE</code> :
...
Dans ce résultat, copiez l'intégralité de l'URL en « HTTPS » **sans les guillemets** présente à droite de la mention `"url":`, puis collez-la dans la barre de recherche de votre navigateur internet pour initier le téléchargement de la sauvegarde.
...
Une fois la sauvegarde de votre base de données récupérée, vous devrez créer une nouvelle base de données. Pour cela, consultez notre guide « [Créer une base de données sur son hébergement web](/guides/web-cloud/web-hosting/sql-create-database) ».

Lorsque cette nouvelle base de données sera créée, importez la sauvegarde à l'aide de notre guide « [Importer une sauvegarde dans la base de données d'un hébergement web](/guides/web-cloud/web-hosting/sql-importing-mysql-database) ».

Pour terminer, liez votre base de données OVHcloud avec le fichier de configuration de votre site web présent dans l'[espace de stockage FTP de votre hébergement OVHcloud](/guides/web-cloud/web-hosting/ftp-connection).
...
[Modifier le mot de passe d'une base de données](/guides/web-cloud/web-hosting/sql-change-password).
