...
<Tabs>
  ...
  <Tab label="Étape 2">
    Dans l’onglet <code className="action">FTP - SSH</code>, le nom d’hôte est visible sous la mention `Serveur FTP et SFTP`.
...
<Tabs>
  ...
  <Tab label="Étape 2">
    Dans l'onglet <code className="action">FTP - SSH</code>, repérez le nom d'utilisateur dans la colonne `Login` du tableau.
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab label="Étape 2">
    Dans l'onglet <code className="action">FTP - SSH</code>, repérez le chemin sous la mention `chemin du répertoire home`. Si plusieurs utilisateurs sont configurés, le chemin indiqué peut être différent. Remplacez alors le nom d'utilisateur mentionné après `home/` par celui de votre choix dans la colonne `Login` du tableau.
...
[Utiliser l'accès SSH de son hébergement web](/guides/web-cloud/web-hosting/ssh-on-webhosting). N'oubliez pas que pour utiliser le SSH, vous devez disposer d'une offre d'[hébergement web OVHcloud](https://www.ovhcloud.com/fr/web-hosting/) **Pro**, **Performance**, **Agency**, **Agency Plus** ou **Agency Max**.
