...
<Tabs>
  ...
  <Tab label="Étape 2">
    Sur la page qui s’affiche, cliquez sur l’onglet <code className="action">Statistiques et logs</code>. Défilez jusqu’au bas de la page où se trouve le graphique lié à l’utilisation de votre hébergement.
  </Tab>
  <Tab label="Étape 3">
    Sélectionnez le **type** d’information ainsi que la **période** des données affichées.
    
    <img className="thumbnail" alt="Graphiques des statistiques de l'hébergement web" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/statistics-and-logs/infrastructure-statistics-graph.png" loading="lazy" />
...
_Cette étape est facultative si vous n’utilisez pas de CMS._
...
_Cette étape est facultative si vous n’utilisez pas de base de données._
...
**Comment accéder à la base de données par le biais de phpMyAdmin :** consultez la partie dédiée dans notre guide « [Créer une base de données sur son hébergement web](/guides/web-cloud/web-hosting/sql-create-database) ».
