...
description: "Découvrez comment récupérer la sauvegarde d'une base de données de votre hébergement web OVHcloud"
...
- Selon la méthode de sauvegarde utilisée, disposer d'un accès à la gestion de l'offre d'hébergement web depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> ou des informations permettant de se connecter à la base de données.
...
- **Utiliser l'outil de sauvegarde d'OVHcloud** : cette solution permet de récupérer des sauvegardes de vos bases de données depuis l'<ManagerLink to="/">espace client OVHcloud</ManagerLink>. Cette méthode ne nécessite pas de compétences techniques particulières.
...
:::tip
Si vous réalisez une sauvegarde de votre base de données car celle-ci est saturée / pleine, n'hésitez pas à consulter notre tutoriel « [Que faire lorsque ma base de données est saturée ?](/guides/web-cloud/web-hosting/sql-overquota-database) ».
...
Toujours depuis l'onglet <code className="action">Bases de données</code>, cliquez sur le bouton <code className="action">...</code> à droite de la base de données à sauvegarder, puis sur <code className="action">Créer une sauvegarde</code>.
...
Sur la fenêtre qui apparaît, sélectionnez la date souhaitée pour la sauvegarde, puis cliquez sur le bouton <code className="action">Suivant</code>. Assurez-vous que les informations dans le récapitulatif sont correctes, puis cliquez sur <code className="action">Valider</code> pour initier la manipulation.
...
Toujours depuis l'onglet <code className="action">Bases de données</code>, cliquez sur le bouton <code className="action">...</code> à droite de la base de données à sauvegarder, puis sur <code className="action">Restaurer une sauvegarde</code>.
...
Pour télécharger une sauvegarde, cliquez sur le bouton <code className="action">...</code> à droite de celle que vous souhaitez récupérer, puis sur <code className="action">Télécharger la sauvegarde</code>. Une fenêtre vous invitant à l'enregistrer sur votre machine apparaît. Acceptez, puis patienter le temps que la sauvegarde soit téléchargée.
...
Pour réaliser la manipulation, connectez-vous à *phpMyAdmin*. Pour connaître le lien d'accès à ce dernier, connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> puis rendez-vous dans la partie <code className="action">Web Cloud</code>. Dans la colonne de gauche, cliquez sur <code className="action">Hébergements</code>, puis choisissez le nom de l'hébergement concerné. Positionnez-vous enfin sur l'onglet <code className="action">Bases de données</code>.

Le tableau qui s'affiche contient toutes les bases de données créées dans le cadre de votre offre d'hébergement web. Cliquez dans ce dernier sur le bouton <code className="action">...</code> à droite de la base de données concernée puis sur <code className="action">Accéder à phpMyAdmin</code>.
...
Une fois sur l'interface de connexion à *phpMyAdmin*, renseignez les informations de la base de données, puis connectez-vous. Une fois connecté, rendez-vous à présent sur l'onglet <code className="action">Exporter</code> où deux méthodes d'exportation sont proposées :
...
[Tutoriel - Que faire lorsque ma base de données est saturée ?](/guides/web-cloud/web-hosting/sql-overquota-database)
