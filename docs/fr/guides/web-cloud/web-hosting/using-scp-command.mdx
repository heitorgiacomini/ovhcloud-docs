...
description: Découvrez comment utiliser la commande Secure Copy Protocol (SCP) en SSH pour copier des fichiers depuis ou vers votre hébergement web
...
Ce guide vous détaille de manière non exhaustive des fonctionnalités disponibles avec la commande `scp`. N'hésitez pas à échanger avec notre [communauté d'utilisateurs](https://community.ovhcloud.com/community/fr) si vous souhaitez approfondir vos connaissances sur cette commande.

### Étape 1 - Récupérer les accès SSH de votre hébergement web

Pour retrouver les accès SSH de votre hébergement web, consultez notre guide « [Utiliser l'accès SSH de son hébergement web](/guides/web-cloud/web-hosting/ssh-on-webhosting) ».
...
- `ssh.cluster0XX.hosting.ovh.net` : remplacez les `XX` par le numéro du cluster où se trouve votre hébergement web. Si besoin, consultez notre guide « [Utiliser l'accès SSH de son hébergement web](/guides/web-cloud/web-hosting/ssh-on-webhosting) » pour retrouver cette information.
...
[Se connecter à l'espace de stockage FTP de votre hébergement web](/guides/web-cloud/web-hosting/ftp-connection)
