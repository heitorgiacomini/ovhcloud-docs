...
description: Découvrez comment utiliser l’application Cyberduck pour vous connecter à votre hébergement web OVHcloud
...
- En-dessous se trouve le contenu que vous souhaitez afficher. Par exemple, si vous cliquez sur l'icône <code className="action">History</code>, la liste des vos actions y apparaitra.
...
Dans le menu principal, tout en haut de l'interface, cliquez sur <code className="action">View</code> puis sur <code className="action">Customize Toolbar...</code>.
...
Dans la fenêtre qui apparaît, faites glisser les éléments souhaités vers la barre d'outils. Par exemple, si vous voulez ajouter une icône <code className="action">Download</code> dans votre barre d'outils, glissez et déposez l'icône <code className="action">Download</code> vers la barre d'outils. Pour valider vos modifications, cliquez sur <code className="action">Done</code>.
...
**1.**  Dans la barre d'outils, cliquez sur <code className="action">Open Connection</code>

**2.**  Dans le menu déroulant (cadre orange de l'image), sélectionnez <code className="action">SFTP (SSH File Transfert Protocol)</code>
...
:::tip[Success]
- Vous avez la possibilité d'enregistrer votre mot de passe dans Cyberduck en cochant <code className="action">Add to keychain</code>. Si vous ne cochez pas la case, vous devrez renseigner le mot de passe afin de vous reconnecter à votre espace d'hébergement web.
- Si vous ne connaissez pas toutes vos informations FTP (serveur, identifiants etc.), reportez-vous au guide « [Se connecter à l’espace de stockage FTP de son hébergement web](/guides/web-cloud/web-hosting/ftp-connection) ».

:::


Lorsque vous effectuez une première connexion vers votre espace d'hébergement web, une fenêtre portant le titre <code className="action">Modified fingerprint</code> s'affiche. Cochez la case <code className="action">Always</code> puis validez. Ceci vous permettra de certifier définitivement l'hôte de connexion (OVHcloud).
...
- Si besoin, reportez-vous au guide « [Se connecter à l’espace de stockage FTP de son hébergement web](/guides/web-cloud/web-hosting/ftp-connection) ».
...
2. En haut de l'interface, dans la barre d'outils, cliquez sur l'onglet <code className="action">Bookmarks</code> (cadre orange dans l'image ci-dessous).
3. En bas à gauche de la fenêtre, cliquez sur l'icône <code className="action">+</code> pour ajouter un nouveau signet.
...
Dans le menu de Cyberduck, cliquez sur <code className="action">File</code> puis sur <code className="action">Upload...</code>. Sélectionnez les fichiers que vous souhaitez transférer sur le serveur puis cliquez sur <code className="action">Upload</code>.
...
- en cliquant sur <code className="action">Window</code> (dans le menu principal) puis <code className="action">Transfers</code>.
...
Double-cliquez sur un fichier ou un dossier puis sélectionnez <code className="action">Info</code>. La fenêtre suivante s'affiche :

<img className="thumbnail" alt="hosting" src="/images/assets/screens/other/web-tools/cyberduck/unix-permissions.png" loading="lazy" />

Cliquez sur l'onglet <code className="action">Permissions</code> puis effectuez les modifications souhaitées :
...
Dans le menu de Cyberduck, cliquez sur <code className="action">Go</code> puis sur <code className="action">Send command...</code>.
...
Dans la nouvelle fenêtre, insérez la commande `CHMOD 705 /` puis cliquez sur <code className="action">Send</code> pour éxécuter la commande. En confirmation, le message `200 Permissions changed on /` devrait apparaître dans l'encadré du dessous.
