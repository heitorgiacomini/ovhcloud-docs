...
description: Découvrez comment commander et installer un certificat SSL Sectigo EV sur votre hébergement Web OVHcloud
...
OVHcloud propose plusieurs types de certificats SSL sur nos offres d'[hébergement mutualisé OVHcloud](https://www.ovhcloud.com/fr/web-hosting/). Ils sont présentés dans notre guide « [Gérer un certificat SSL sur son hébergement web](/guides/web-cloud/web-hosting/ssl-on-webhosting) ». Les certificats SSL sont incontournables pour la sécurité de votre site web.
...
- [Activer un certificat SSL gratuit Let's Encrypt](/guides/web-cloud/web-hosting/ssl-letsencrypt) (si ce n'est pas déjà le cas par défaut).
- Activer un (ou plusieurs) autre(s) certificat(s) SSL payant(s) ([Sectigo DV](/guides/web-cloud/web-hosting/ssl-dv) ou [Sectigo EV](/guides/web-cloud/web-hosting/ssl-ev)).
- [Installer votre propre certificat SSL](/guides/web-cloud/web-hosting/ssl-custom).
...
<Tabs>
  ...
  <Tab label="Étape 3">
    Lorsque le contenu de l'onglet apparaît, cliquez sur le bouton <code className="action">Commander un certificat SSL Sectigo</code>.
  ...
  <Tab label="Étape 4">
    Dans la nouvelle fenêtre qui s'ouvre, sélectionnez le nom de domaine ou sous-domaine concerné à l'aide du menu déroulant, puis cliquez sur <code className="action">Valider</code> pour être redirigé vers le bon de commande de votre certificat SSL Sectigo EV.
    
    <img className="thumbnail" alt="SSL Sectigo sélection du domaine" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/ssl-certificates/order-a-sectigo-ssl-certificate-select-domain.png" loading="lazy" />
  </Tab>
  <Tab label="Étape 5">
...
Cliquez sur <code className="action">Continuer</code> une fois **tous les éléments** correctement renseignés.
...
Si vous n'avez pas encore commandé votre nom de domaine et l'hébergement associé, rendez-vous sur notre [page d'accueil OVHcloud](https://www.ovhcloud.com/fr/), saisissez un nom de domaine dans le **formulaire de recherche prévu à cet effet** puis cliquez sur <code className="action">Rechercher</code> pour démarrer votre commande.
...
Sélectionnez vos choix d'installation de <code className="action">module en 1 clic</code> et de <code className="action">CDN</code> puis descendez tout en bas de la page jusqu'à la section <code className="action">Sécurisez votre site web avec nos certificats SSL</code>.

<img className="thumbnail" alt="SSL EV order" src="/images/assets/screens/website/order/ssl-ev-selection.png" loading="lazy" />

Choisissez <code className="action">Sectigo EV SSL</code> puis cliquez sur <code className="action">Continuer</code>.
...
Nous vous recommandons aussi de **ne pas** activer ou laisser actif le [pare-feu applicatif](/guides/web-cloud/web-hosting/multisites-activating-application-firewall), disponible avec nos hébergements web, pendant toute la durée de l'installation de votre certificat SSL Sectigo EV.
...
Il ne vous restera plus qu'à [passer votre site en HTTPS](/guides/web-cloud/web-hosting/ssl-activate-https-website) pour utiliser pleinement votre certificat SSL Sectigo EV.
...
[Passer votre site en HTTPS](/guides/web-cloud/web-hosting/ssl-activate-https-website)
