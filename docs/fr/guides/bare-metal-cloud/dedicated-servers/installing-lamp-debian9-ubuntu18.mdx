...
description: Découvrez comment configurer un serveur web LAMP
...
Si une distribution Debian ou Ubuntu n'est pas déjà installée sur votre serveur, effectuez d'abord une réinstallation depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. C'est la meilleure façon d'avoir un système propre pour votre serveur web et les applications qui s'y exécutent.

Suivez le guide correspondant pour installer une distribution sur votre service OVHcloud et vous y connecter en [SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction) :
...
Confirmez la première invite en appuyant sur <code className="action">Entrée</code>.
...
Il est recommandé d'utiliser la méthode d'authentification proposée (*unix_socket*) à la place de l'accès par mot de passe root. Appuyez sur <code className="action">y</code> puis sur <code className="action">Entrée</code>. Si vous décidez d'utiliser l'accès utilisateur root à la place, choisissez <code className="action">n</code>, puis définissez un mot de passe root à l'invite suivante.

Entrez <code className="action">n</code> à l'invite suivante :

```console
Change the root password? [Y/n]
```

Les invites suivantes concernent les mesures de sécurité, confirmez-les toutes avec <code className="action">y</code> jusqu'à la fin du script.
...
Pour installer WordPress sur un serveur, vous pouvez suivre [ce tutoriel](/guides/public-cloud/compute/install-wordpress).
...
[La configuration d’un pare-feu](/guides/bare-metal-cloud/dedicated-servers/firewall-linux-iptable) (*iptables*) permettra d’améliorer la sécurité de votre serveur. Ce processus peut être simplifié en utilisant le frontend « Uncomplicated Firewall » (UFW) et son ensemble de profils prédéfinis. 
...
L'accès à l'installation de votre serveur web via un nom de domaine nécessite de l'attacher à votre service. Pour ce faire, vous devez éditer la zone DNS accessible depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, à condition qu’OVHcloud soit votre bureau d’enregistremente **et** que le nom de domaine utilise les serveurs DNS d’OVHcloud.

Consultez le guide « [Éditer une zone DNS](/guides/web-cloud/domains/dns-zone-edit) » pour en savoir plus. Si le nom de domaine est actuellement utilisé, configurez les DNS uniquement après que votre site web ou votre application soit prêt.
...
En alternative, OVHcloud vous propose la solution [SSL Gateway](https://www.ovhcloud.com/fr/security/ssl-gateway/). Référez-vous à [notre documentation](/guides/web-cloud/ssl-gateway/order) pour plus d'informations.
...
[Comment télécharger et récupérer des données sur un serveur via SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp)
