...
description: Découvrez les différentes méthodes pour sécuriser efficacement votre serveur dédié ESXi
...
Vous trouverez plus d'informations sur cette attaque dans [une FAQ complémentaire](/guides/bare-metal-cloud/dedicated-servers/faq-esxi).
:::
...
- Avoir souscrit à une offre compatible avec notre fonctionnalité [Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network) si vous souhaitez l'utiliser pour effectuer le filtrage.
...
Pour cela, il vous sera nécessaire de [redémarrer](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#redemarrage-de-votre-serveur-dedie) votre serveur ESXi à travers l'espace client OVHcloud.
:::
...
Toutes ces informations sont également disponibles à travers l'interface d'administration web. Cliquez sur le menu <code className="action">Host</code> et accédez à la section <code className="action">Monitor</code>, puis cliquez sur <code className="action">Logs</code>.
...
Nous vous proposons d'activer et d'utiliser notre solution de filtrage [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network).
...
Vous pouvez accéder aux paramètres du Edge Network Firewall depuis l'espace client OVHcloud, en sélectionnant <code className="action">Network</code>, puis en choisissant <code className="action">Adresses IP publiques</code> sous "Réseau public". Vous pouvez retrouver votre serveur dédié en recherchant son adresse IP.

<img className="thumbnail" alt="Page Adresses IP publiques dans l'espace client OVHcloud" src="/images/bare-metal-cloud/dedicated-servers/esxi-hardening/firewall_network_1.png" loading="lazy" />

Sur la droite du tableau, cliquez sur le bouton <code className="action">...</code> correspondant à votre serveur dédié, puis sélectionnez <code className="action">Configurer le Edge Network Firewall</code>.

<img className="thumbnail" alt="Option Configurer le Edge Network Firewall dans le menu contextuel" src="/images/bare-metal-cloud/dedicated-servers/esxi-hardening/firewall_network_2.png" loading="lazy" />
...
Cliquez sur le menu <code className="action">Host</code> et accédez à la section <code className="action">Manage</code> puis cliquez sur <code className="action">Services</code>.

Trouvez dans la liste le service `TSM-SSH` et faites un clic droit sur la ligne associée.

Arrêtez le service en cliquant sur <code className="action">Stop</code> :

<img className="thumbnail" alt="Arret du service TSM-SSH dans ESXi" src="/images/bare-metal-cloud/dedicated-servers/esxi-hardening/stop_service.png" loading="lazy" />

Selectionnez la `Policy` puis modifiez-la comme sur l'exemple présenté.

Choisissez l'option <code className="action">Start anc stop manually</code> afin d'éviter que le service ne soit actif au démarrage du serveur.

<img className="thumbnail" alt="Politique du service SSH definie sur Demarrer et arreter manuellement" src="/images/bare-metal-cloud/dedicated-servers/esxi-hardening/ssh_disabled_.png" loading="lazy" /> 
...
Cliquez sur le menu <code className="action">Networking</code> puis sur <code className="action">Firewall rules</code> et choisissez <code className="action">Edit settings</code> pour chacun des services à protéger :
...
[Installer VMware ESXi 8 sur un serveur dédié](/guides/bare-metal-cloud/dedicated-servers/esxi-partitioning)
