...
description: "Retrouvez les questions les plus fréquemment posées sur la vague d'attaques ciblant les serveurs ESXi"
...
Si vous souhaitez tenter de récupérer les données présentes sur votre serveur, vous pouvez vous appuyer sur les étapes de la documentation suivante : [Activer et utiliser le mode rescue - Montage d'un datastore](/guides/bare-metal-cloud/dedicated-servers/rescue-mode#montage-dun-datastore).
...
L'offre Serveur Dédié bénéficie en option de l'offre [Backup Storage](https://www.ovhcloud.com/fr/bare-metal/backup-storage/). La sauvegarde n'est cependant pas automatique. Retrouvez plus de détails sur le fonctionnement de cette solution sur notre guide « [Utiliser Backup Storage sur un serveur dédié](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) ».
...
1. Sécurisez l'accès à votre serveur dédié via le [Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network) ou votre propre système de pare-feu.
2. Installez la version VMware ESXi 7.0 U3c disponible [via votre interface d'administration du serveur dédié](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#installation-ou-reinstallation-de-votre-serveur-dedie).
...
4. [Sécurisez votre serveur dédié](/guides/bare-metal-cloud/dedicated-servers/securing-a-dedicated-server).
...
[Maîtriser et sécuriser votre serveur dédié ESXi dès son premier démarrage](/guides/bare-metal-cloud/dedicated-servers/esxi-hardening)
