...
description: Migrez les données entre serveurs dédiés OVHcloud avec rsync, SFTP ou le backup storage pour une transition fluide
...
- La plus simple est d'utiliser un logiciel adapté tel que [SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp).
- L'autre option est de [synchroniser les deux serveurs entres-eux](/guides/bare-metal-cloud/dedicated-servers/how-to-copy-data-from-one-dedicated-server-to-another-using-rsync).
...
Consultez notre guide « [utiliser backup storage sur un serveur dédié](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) » pour plus d'informations.
...
Si la réputation de vos adresses IP est importante, il est fortement conseillé d'utiliser des [Additional IP](https://www.ovhcloud.com/fr/network/additional-ip/), celles-ci pouvant être conservées en cas de migration.

Une fois ces IP en votre possession, il vous suffira de déplacer celles-ci vers le nouveau serveur.<br/>
À cet effet, consultez notre guide dédié : [Déplacer une Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip).
...
Pour plus d'informations, consultez notre documentation sur les domaines et DNS.
