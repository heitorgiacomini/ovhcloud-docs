...
description: Découvrez comment configurer le OVHcloud Game DDoS Protection firewall
...
:::info
Retrouvez plus d’informations sur notre [protection anti-DDoS Game sur notre site](https://www.ovhcloud.com/fr/security/game-ddos-protection/).
...
- Un [serveur dédié OVHcloud **Game**](https://www.ovhcloud.com/fr/bare-metal/game/)
...
- **Lien direct :** <ManagerLink to="/#/network/ip">IP publiques</ManagerLink>
...
:::warning
Le *Game Firewall* protège les adresses IP associées à un serveur. Par conséquent, si vous possédez un serveur avec plusieurs adresses IP (ex: des [adresses Additional IP](https://www.ovhcloud.com/fr/network/additional-ip/)), vous devez configurer chacune d’entre elles séparément.
...
- Cliquez sur <code className="action">Network</code> dans le menu situé à gauche de l'écran.
- Cliquez sur <code className="action">Adresses IP Publiques</code>.

Vous pouvez filtrer les adresses IP en utilisant le menu déroulant <code className="action">Tous les types de service</code>, ou entrer directement l'adresse IP désirée dans la barre de recherche. Renseignez le nom ou la catégorie du serveur correspondant :

#### Lister les adresses IP attachées à votre serveur Game

<Tabs>
  <Tab label="Depuis la page Serveurs dédiés">
    - Ouvrez la section <code className="action">Bare Metal Cloud</code> dans la barre latérale gauche.
    - Sélectionnez <code className="action">Serveurs dédiés</code>.
    ...
    - Cliquez sur le bouton <code className="action">...</code> et sélectionnez <code className="action">Configurer Game Protection</code>. Vous serez dirigé vers la liste des adresses IP attribuées à votre serveur.
  </Tab>
  <Tab label="Depuis la page Adresses IP publiques">
    ...
    - Dans le menu déroulant <code className="action">Tous les types de service</code>, recherchez et sélectionnez le serveur Bare Metal Game que vous souhaitez configurer.
...
Pour chaque adresse attachée à votre serveur nécessitant une protection, vérifiez que le statut du *Game Firewall* est <code className="action">Disponible</code>. Vous devrez configurer les règles du *Game Firewall* individuellement pour chacune d'entre elles.

- Cliquez sur le bouton <code className="action">⁝</code> à droite du tableau et sélectionnez <code className="action">Configurer le Game firewall</code>.
...
- Pour des raisons de sécurité, nous vous recommandons fortement d’activer l’option <code className="action">Appliquer la stratégie « Refus par défaut »</code>, en haut à droite du tableau de règles. Cette option bloque tout le trafic qui ne correspond pas aux règles que vous avez définies pour le Game Firewall, c’est-à-dire que toutes les applications de jeu répertoriées seront protégées et qu’aucune autre connexion ne pourra atteindre votre serveur. Cette option réduit considérablement la surface d’attaque exposée aux acteurs malveillants potentiels.
...
:::warning
La protection anti-DDoS Game prend effet après les règles définies dans le [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network). Pour que les deux fonctionnent correctement, le Edge Network Firewall ne peut pas être trop strict et doit laisser passer le trafic approprié à la protection anti-DDoS Game.
...
Tous les protocoles de protection anti-DDoS Game pris en charge pour un serveur spécifique sont visibles sur la page de configuration <code className="action">GAME firewall</code> pour toute adresse IP pointant vers ce serveur, dans le menu déroulant <code className="action">Game protocol</code> :
...
Si vous préférez l'automatisation, les détails du protocole peuvent être récupérés à l'aide de [l'APIv6 OVHcloud](/guides/manage-and-operate/api/first-steps) :
...
<summary>**Puis-je utiliser la protection anti-DDoS Game sur d’autres gammes que les serveurs Bare Metal Game ?**</summary>
...
<summary>**Comment puis-je m’assurer que l’automatisation fonctionne pour une Additional IP entre une nouvelle et une ancienne génération de serveurs Bare Metal Game ?**</summary>
...
<summary>**Puis-je désactiver la protection anti-DDoS Game ?**</summary>
...
<summary>**Mon jeu ne figure pas sur la liste des protocoles supportés, que puis-je faire ?**</summary>
...
<summary>**Après avoir configuré mon jeu avec les ports appropriés et la stratégie par défaut à supprimer, je continue à recevoir des attaques qui affectent mon serveur Game. Que faire ?**</summary>
