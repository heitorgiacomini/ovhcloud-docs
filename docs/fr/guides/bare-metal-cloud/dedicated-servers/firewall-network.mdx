...
- Un service OVHcloud exposé et utilisant une adresse IP publique dédiée ([Serveur Dédié](https://www.ovhcloud.com/fr/bare-metal/), [VPS](https://www.ovhcloud.com/fr/vps/),[instance Public Cloud](https://www.ovhcloud.com/fr/public-cloud/), [Hosted Private Cloud](https://www.ovhcloud.com/fr/hosted-private-cloud/vmware/), [Additional IP](https://www.ovhcloud.com/fr/network/additional-ip/), etc.)
...
<img className="thumbnail" alt="filtrer les services" src="/images/bare-metal-cloud/dedicated-servers/firewall-network/selectservice_cut_new.png" loading="lazy" />

Cliquez ensuite sur le bouton <code className="action">⁝</code> à droite de l'IPv4 concernée et sélectionnez <code className="action">Configurer le Edge Network Firewall</code> (ou cliquez sur l'icône de statut dans la colonne **Edge Firewall**).

<img className="thumbnail" alt="Activation du Edge Network Firewall" src="/images/bare-metal-cloud/dedicated-servers/firewall-network/firewall_config_new.png" loading="lazy" />
...
Pour plus d'informations, reportez-vous aux guides suivants : [Configuration du pare-feu sous Windows](/guides/bare-metal-cloud/dedicated-servers/activate-port-firewall-soft-win) et [Configuration du pare-feu sous Linux avec iptables](/guides/bare-metal-cloud/dedicated-servers/firewall-linux-iptable).

:::


**Pour ajouter une règle**, cliquez sur le bouton <code className="action">+ Ajouter une règle</code>, en haut à gauche de la page.

| ![add-rule-btn](/images/bare-metal-cloud/dedicated-servers/firewall-network/enf_add_rule_new.png) |
|:--:|
| Cliquez sur <code className="action">+ Ajouter une règle</code>. |
...
|:--|
| - Une priorité (de 0 à 19, 0 étant la première règle à appliquer, suivie des autres) <br/> - Une action (<code className="action">Accepter</code> ou <code className="action">Refuser</code>) <br/> - Le protocole <br/> - L'adresse IP source (facultatif) |
...
<img className="thumbnail" alt="Exemple de configuration" src="/images/bare-metal-cloud/dedicated-servers/firewall-network/exemple.png" loading="lazy" />
...
Pour un aperçu détaillé des attaques détectées et des résultats des activités du Scrubbing Center, nous vous encourageons à consulter notre guide sur le [Network Security Dashboard](/guides/bare-metal-cloud/dedicated-servers/network-security-dashboard).
...
- [Protéger un serveur GAME avec le pare-feu applicatif](/guides/bare-metal-cloud/dedicated-servers/firewall-game-ddos)
