...
Afin de simplifier l'installation des OS sur les serveurs dédiés, ainsi que pour simplifier la communication de ce changement, un nouvel appel API a été créé : `POST /dedicated/server/{serviceName}/reinstall`. Cet appel API est le successeur de l'ancienne route : `POST /dedicated/server/{serviceName}/install/start`. Cette ancienne route permettait de cibler une installation OS depuis un gabarit OVHcloud **ou un gabarit personnel**.
...
### Étape 1 : 17 Juin 2025 <a id="step-1"></a>
...
### Étape 2 : 7 Octobre 2025 <a id="step-2"></a>
...
### Espace client OVHcloud, SoYouStart ou Kimsufi <a id="ux"></a>

Le bouton `Installation à partir d'un de vos gabarits` a été supprimé de l'assistant d'installation OS des serveurs dédiés en Avril 2025. Si vous utilisiez ce bouton, vous disposez de 3 possibilités sur le long-terme :
...
Même si vous pouvez déjà explorer des exemples et schémas de la [nouvelle route API /reinstall dans la console API](https://eu.api.ovh.com/console/?section=%2Fdedicated%2Fserver&branch=v1#post-/dedicated/server/-serviceName-/reinstall) et lire la [documentation publique](/guides/bare-metal-cloud/dedicated-servers/api-os-installation), voici un contournement temporaire que vous pouvez suivre en attendant de recevoir les payloads de la nouvelle route API dans le courriel du 19 Mai 2025.

#### Contournement pour les comptes OVHcloud <a id="ux-ovh"></a>

Comme mentionné dans la [documentation API OVHcloud](/guides/manage-and-operate/api/first-steps#se-connecter-aux-api-ovhcloud), sur la [page des API OVHcloud](/links/console) :

- Cliquez sur `Authentication` en haut à gauche.
- Cliquez ensuite `Login with OVHcloud SSO`.
- Saisissez vos identifiants OVHcloud.
- Cliquez sur le bouton `Authorize` pour autoriser les appels aux API depuis ce site.

![API](/images/guides/bare-metal-cloud/dedicated-servers/end-of-life-for-personal-installation-templates/login.png)

Ensuite, exécutez l'appel API pour déclencher une installation OS depuis votre gabarit personnel :

- Sélectionnez `v1`.
- Choisissez la section `/dedicated/server`.
- Cherchez l'appel API `POST /dedicated/server/{serviceName}/install/start` (vous pouvez utiliser le filtre).
...
Si vous avez besoin de lister et d'afficher les détails de vos gabarits personnels, vous pouvez le faire dans la section `/me` dans les appels API dans `/me/installationTemplate`.
...
Cliquez ensuite sur le bouton `Execute` pour lancer la tâche d'installation OS.

![console API](/images/guides/bare-metal-cloud/dedicated-servers/end-of-life-for-personal-installation-templates/screenshot-api-console-workaround.png)

Vous pouvez revenir sur la page du tableau de bord du serveur dédié dans <ManagerLink to="/">l'espace client OVHcloud</ManagerLink>, afin de suivre la progression de la tâche d'installation OS.

#### Contournement pour les comptes SoYouStart ou Kimsufi <a id="ux-sys-ks"></a>
...
Cliquez sur le bouton `Login` en haut à droite et saisissez vos identifiants. Ensuite, cliquez sur `Log in`. Vous êtes alors authentifié sur la console API.

- Ouvrez la section `/dedicated/server`.
- Trouvez l'appel API `POST /dedicated/server/{serviceName}/install/start`.
...
### Utiliser les APIs OVHcloud, SoYouStart ou Kimsufi <a id="api"></a>

Si vous utilisez directement les appels APIs OVHcloud, SoYouStart ou Kimsufi pour déclencher les installations OS depuis un gabarit personnel, vous pouvez continuer à utiliser l'appel API `POST /dedicated/server/{serviceName}/install/start` jusqu'au **7 Octobre 2025**.
...
La comparaison suivante vous donne une idée de la réduction du nombre d'appels APIs apportée par le [nouvel appel API /reinstall](https://eu.api.ovh.com/console/?section=%2Fdedicated%2Fserver&branch=v1#post-/dedicated/server/-serviceName-/reinstall) dans le cas d'une personnalisation complexe impliquant du RAID matériel et une personnalisation du partitionnement : de `4+n` appels (où `n` représente le nombre de partitions définies dans le schéma de partitionnement) à un seul et unique appel API.

![Comparaison ancien vs nouveau call API](/images/guides/bare-metal-cloud/dedicated-servers/end-of-life-for-personal-installation-templates/api-calls-comparison.png)
