---
title: Créer des clés SSH pour accéder à un serveur dédié
description: Créez des paires de clés OpenSSH sur votre appareil local pour des connexions sécurisées sans mot de passe à votre serveur dédié
...
- Disposer d'un [serveur dédié](https://www.ovhcloud.com/fr/bare-metal/) ou d'un [VPS](https://www.ovhcloud.com/fr/vps/) sur votre compte OVHcloud
...
:::info
`Ed25519` est considéré comme la méthode la plus sûre mais `RSA` est une alternative valable. Les deux sont compatibles avec l’espace client OVHcloud si vous souhaitez [stocker les clés publiques dans votre compte client](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
:::
...
Si vous confirmez avec <code className="action">Entrée</code> sans entrer de nom, le nom de fichier standard sera utilisé (`id_rsa` dans cet exemple).
...
Copiez cette chaîne de clé pour [l'ajouter à un nouveau serveur](#getstarted) ou pour [l'importer dans votre espace client](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
...
[Introduction au protocole SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)

[Mode rescue sur serveur dédié](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)

[Mode Rescue sur VPS](/guides/bare-metal-cloud/virtual-private-servers/rescue)
