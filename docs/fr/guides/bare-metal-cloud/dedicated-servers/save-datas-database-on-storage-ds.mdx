...
description: Sécuriser ses données en 5 étapes
...
- Un [serveur de stockage OVHcloud](https://www.ovhcloud.com/fr/bare-metal/storage/).
- Une infrastructure de production ([VPS](https://www.ovhcloud.com/fr/vps/), [serveurs dédiés](https://www.ovhcloud.com/fr/bare-metal/), [Public Cloud](https://www.ovhcloud.com/fr/public-cloud/), etc.).
- Une connexion SSH configurée entre le serveur de stockage et l'infrastructure de production.
- Conseillé : un réseau privé entre vos serveurs ([OVHcloud vRack](https://www.ovhcloud.com/fr/network/vrack/)).

## En pratique

### Étape 1 : choisissez le mode RAID approprié

OVHcloud dispose d'une gamme de [serveurs de stockage](https://www.ovhcloud.com/fr/bare-metal/storage/) dont les configurations matérielles contiennent plusieurs disques durs. Dans notre exemple, nous possédons un RAID logiciel (ou _soft RAID_) de quatre disques d'une capacité de 6 To chacun.
...
Rendez-vous dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> et installez votre serveur. Comme indiqué au début, nous allons utiliser Debian 9.4. Référez-vous à notre guide des [premiers pas sur un serveur dédié](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#installation-ou-reinstallation-de-votre-serveur-dedie) pour plus d'informations.

Une fois le système sélectionné pour l'installation, cochez la case <code className="action">Personnaliser la configuration des partitions</code>.

<img className="thumbnail" alt="Personnaliser la configuration des partitions" src="/images/bare-metal-cloud/dedicated-servers/save-datas-database-on-storage-ds/custompartition.png" loading="lazy" />
...
<img className="thumbnail" alt="Modification de la /home" src="/images/bare-metal-cloud/dedicated-servers/save-datas-database-on-storage-ds/partition_customization_menu.png" loading="lazy" />
