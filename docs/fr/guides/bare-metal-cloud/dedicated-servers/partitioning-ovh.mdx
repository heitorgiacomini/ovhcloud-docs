...
description: "Découvrez comment l'API OVHcloud vous permet de personnaliser la configuration des disques, RAID matériels/logiciels et du partitionnement lors de la réinstallation de l'OS sur votre serveur"
...
Les [serveurs dédiés](https://www.ovhcloud.com/fr/bare-metal/) OVHcloud vous permettent de configurer les disques, le [RAID matériel](/guides/bare-metal-cloud/dedicated-servers/raid-hard), le [RAID logiciel](/guides/bare-metal-cloud/dedicated-servers/raid-soft), LVM, ZFS, etc. pendant [la réinstallation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server) de votre système d'exploitation depuis l’[API OVHcloud](https://eu.api.ovh.com/) ou depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Dans cet article, nous allons nous concentrer sur l'[API OVHcloud](https://eu.api.ovh.com/).<br/>
...
- [RAID logiciel](/guides/bare-metal-cloud/dedicated-servers/raid-soft) (SR)
...
<img className="thumbnail" alt="Tableau représentant les différentes couches du partitionnement" src="/images/bare-metal-cloud/dedicated-servers/partitioning-ovh/partitioning-layers-table.png" loading="lazy" />
...
Dans la section <code className="action">/dedicated/installationTemplate</code>, vous pouvez obtenir les détails tels que la compatibilité LVM, la disponibilité du système de fichiers pour un OS en particulier :
...
⁴ L'installateur ESXi ne prend pas en charge les schémas de partitionnement personnalisés. Le partitionnement est défini par l'éditeur du logiciel. Néanmoins, l’[API OVHcloud](https://eu.api.ovh.com/) peut vous donner une idée de ce à quoi ressemble le partitionnement : pour plus d'informations, consultez [OS & Compatibilité du partitionnement](#os-partitioning-compatibility).<br />
...
Depuis l'[API OVHcloud](https://eu.api.ovh.com/), cet état peut être obtenu avec l'appel API suivant :
...
|`Debian` does not provide a precompiled `ZFS` module and this server does not have enough memory to compile it (8 GiB minimum required). Please use a different file system.|Nous essayons de toujours trouver le meilleur équilibre entre intégration dans l'écosystème OVHcloud, compatibilité matérielle et taille réduite de l'image, tout en fournissant une image la plus proche de l'image "cloud-ready" officielle fournie par l'éditeur. Le module `ZFS` n'étant pas inclus dans l'image officielle, nous ne souhaitons pas l'alourdir avec ce module pour les clients qui n'utilisent pas `ZFS`. Nous souhaitons cependant offrir un OS avec `ZFS` clé en main pour les clients qui en font la demande. Pour cette raison, le module `ZFS` est compilé et installé lors de la réinstallation et seulement pour les clients qui le demandent. La compilation nécessitant de la mémoire vive en plus de celle déjà utilisée pour faire fonctionner l'OS du rescue en `RAM`, nous avons fixé le seuil à 8 GiB pour garantir un bon fonctionnement et une bonne expérience client.|- Supprimez toutes les partitions `ZFS` ou changez pour un type de système de fichiers autre que `ZFS`.<br />- Choisissez un OS qui supporte `ZFS` nativement tel qu'Ubuntu ou Proxmox.<br />- Utilisez un serveur avec au moins 8 GiB de mémoire vive.|
|This operating system doesn't support `md` metadata version 1.2 for the `/` partition. Fallback to `md` version 0.90 is not possible because the `/` partition requires `x` MiB on every disk, exceeding the supported maximum of 4 TiB.|Vous avez configuré une partition `/` sur un RAID logiciel avec un système d'exploitation qui ne supporte que la version 0.9 des métadonnées `md` pour la partition `/`. Malheureusement, cette version des métadonnées de `md` ne fonctionne pas lorsque la taille par disque dépasse 4 TiB.|- Réduisez la taille de cette partition de telle sorte qu'elle ne dépasse pas 4 TiB par disque.<br />- Désactivez le RAID logiciel, en restreignant l'installation OS sur 1 seul disque.<br />- Choisissez un autre système d'exploitation.|
