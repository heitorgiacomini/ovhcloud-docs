...
description: Découvrez quelles actions peuvent être à effectuer par vos soins sur votre serveur suite à une intervention de maintenance
...
1\. Redémarrez le serveur en [mode rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode).<br/>
...
1\. Redémarrez le serveur en [mode rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) et repérez la partition `/` :
...
<img className="thumbnail" alt="proxmox" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/proxmox_edited.jpg" loading="lazy" />

Effectuez les actions suivantes :

1\. Redémarrez le serveur en [mode rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode).
...
- La nouvelle adresse MAC (visible dans l'onglet <code className="action">Interfaces réseau</code> de l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> ou dans le retour de la commande `ip link`).
...
1\. Redémarrez le serveur en [mode rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) et utilisez la commande `chroot` :
...
<img className="thumbnail" alt="reset_network" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/restore.png" loading="lazy" />

#### Version 6.7 ou inférieure

La procédure décrite ci-dessous ne concerne que les **versions 6.7 ou inférieures**.

1\. Redémarrez le serveur en [mode rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) pour monter la partition `/` :
...
1\. Connectez-vous en tant qu'administrateur à partir de l'interface [IPMI/KVM](/guides/bare-metal-cloud/dedicated-servers/ipmi).<br/>
2\. Ouvrez l’utilitaire <code className="action">Exécuter</code> (touche de logo Windows + <code className="action">R</code>) puis, à travers l'invite `run`, exécutez la commande `devmgmt.msc` :

<img className="thumbnail" alt="win_device_manager" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/win_device_manager_edited.png" loading="lazy" />

3\. Ouvrez <code className="action">Network adapters</code> et sélectionnez l'adaptateur correspondant à `eth0`.<br/>
4\. Faites un clic-droit sur <code className="action">Properties</code> > <code className="action">Advanced</code>.<br/>
...
<img className="thumbnail" alt="win_advanced" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/win_advanced_edited.png" loading="lazy" />

#### Via WinPE

1\. Redémarrez le serveur en mode `WinPE`.<br/>
2\. Lancez la commande `regedit` via l'invite de commande `run`.<br/>
3\. Chargez la base de registre locale à travers celle disponible dans `WinPE` puis cliquez sur <code className="action">HKEY_LOCAL_MACHINE</code> :

<img className="thumbnail" alt="win_regedit_1" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/win_regedit_1_edited.png" loading="lazy" />

4\. Cliquez ensuite sur <code className="action">Load Hive...</code>.

<img className="thumbnail" alt="win_regedit_2" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/win_regedit_2_edited.png" loading="lazy" />
...
<img className="thumbnail" alt="win_load" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/win_load_edited.png" loading="lazy" />
...
<img className="thumbnail" alt="win_regedit_3" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/win_regedit_3_edited.png" loading="lazy" />

10\. Il sera nécessaire de décharger le registre pour appliquer les changements récents. Cliquez sur OVH_TEST (créé précédemment) puis cliquez sur <code className="action">File</code> (en haut à gauche) et sélectionnez <code className="action">Unload hive...</code>.
...
1\. Redémarrez le serveur en [mode rescue](/guides/bare-metal-cloud/dedicated-servers/rescue-mode), montez la partition `/` et utilisez ensuite la commande `chroot` :
...
- Rendez-vous dans le menu `boot` de votre BIOS :
    - Choisissez <code className="action">Add New Boot Option</code>.
    - Choisissez le chemin de votre partition <code className="action">Path for boot option</code> et sélectionnez le système de fichier qui contient votre partition de démarrage.
    - Sélectionnez le fichier `EFI\centos6\bootx64.efi` puis choisissez <code className="action">Create</code>.

L'image ci-dessous est un résumé des actions citées plus haut :

<img className="thumbnail" alt="generate_efi" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/generate_efi-v2.gif" loading="lazy" />
...
- Toujours dans le menu `boot` de votre BIOS : 
    - Choisissez <code className="action">UEFI Hard Disk BBS Priorities</code>.
    - Choisissez <code className="action">UEFI Boot Order #1</code> puis sélectionnez <code className="action">CENTOS6</code> afin de le positionner en tant que 1er choix de démarrage dans la liste active.

L'image ci-dessous est un résumé des actions citées plus haut :

<img className="thumbnail" alt="select_efi" src="/images/bare-metal-cloud/dedicated-servers/intervention-support/select_efi-v2.gif" loading="lazy" />
