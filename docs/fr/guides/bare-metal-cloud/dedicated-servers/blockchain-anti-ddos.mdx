...
description: "Utilisez l'infrastructure Anti-DDoS OVHcloud pour protéger vos nœuds Solana sur un serveur dédié sans nuire à leur fonctionnalité"
...
- Au moins un [serveur dédié ](https://www.ovhcloud.com/fr/bare-metal/) qui respecte les exigences matérielles spécifiques de la blockchain souhaitée.
...
Pour commencer, veuillez créer un [ticket de demande d'assistance](https://help.ovhcloud.com/csm?id=csm_get_help) avec les options suivantes : Incident -> IP publique -> [x] Ce service n'est pas répertorié -> Créer une demande d'assistance -> <code className="action">Suivant</code>.
