...
description: Déployez OpenClaw en production avec HTTPS automatique via Traefik et connectez-le aux AI Endpoints OVHcloud sur un VPS
...
- Disposer d'un [VPS OVHcloud](https://www.ovhcloud.com/fr/vps/) (Debian 11/12 ou Ubuntu 22.04+). Consultez notre guide « [Premiers pas avec un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps) ».
- Accès SSH à votre VPS
- Un nom de domaine pointant vers votre VPS. Consultez notre guide « [Ajouter un enregistrement DNS de type A](/guides/web-cloud/domains/dns-zone-a-record-creation) » si nécessaire.
...
Cela signifie que Docker est déjà installé sur votre machine. Dans le cas contraire, suivez le [guide d'installation de Docker](/guides/bare-metal-cloud/virtual-private-servers/install-docker-on-vps).
