...
description: "Découvrez les dates de fin de support des systèmes d'exploitation de votre VPS OVHcloud affectant les licences Plesk et cPanel"
...
Dans l'onglet <code className="action">Accueil</code>, retrouvez les détails de votre système d'exploitation dans la section `OS / Distribution` de l'encadré `Votre VPS`.
...
1. [Sauvegardez vos données](/guides/bare-metal-cloud/virtual-private-servers/using-automated-backups-on-a-vps) (contenu web, base de données, e-mails, etc.).
2. Réinstallez un OS compatible depuis l’espace client OVHcloud en suivant la section `Réinstaller votre VPS` de notre guide [Premiers pas avec un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps).
3. [Réinstallez cPanel](/guides/bare-metal-cloud/virtual-private-servers/cpanel) ou Plesk sur le nouveau système.
...
Commandez un nouveau VPS avec un OS compatible si ce n'est pas déjà fait. [Installez cPanel](/guides/bare-metal-cloud/virtual-private-servers/cpanel) ou Plesk.
...
Pour connaître les dates de fin de vie et de fin de support des images et des OS (VPS & Public Cloud), consultez notre guide « [Public Cloud & VPS - Cycle de vie et annonces de fin de vie/support des images et distributions](/guides/public-cloud/compute/image-life-cycle) ».
