...
<summary>Qu'est-ce qu'un VPS et à quoi sert-il ?</summary>
...
<summary>Quels sont les avantages du VPS OVHcloud ?</summary>
...
<summary>Une solution VPS est-elle le bon choix pour moi ?</summary>
...
<summary>Puis-je facilement faire évoluer mon VPS vers une gamme supérieure ou vers une configuration inférieure ?</summary>
...
<summary>Quelle région ou pays choisir pour mon VPS ?</summary>
...
<summary>Quel est l'avantage d'un VPS situé en Europe ?</summary>
...
<summary>Les sauvegardes sont-elles incluses avec mon VPS ?</summary>
...
<summary>Puis-je héberger plusieurs sites web sur un VPS ?</summary>
...
<summary>Un nom de domaine et un service e-mail sont-ils inclus avec mon VPS ?</summary>
...
<summary>Comment choisir entre un VPS et un hébergement web ?</summary>
...
<summary>Quelle est la différence entre un VPS et une solution Public Cloud ?</summary>
...
<summary>Quels sont les avantages d’un VPS par rapport à un serveur dédié ?</summary>
...
<summary>Quel SLA est appliqué à mon VPS ?</summary>
...
<summary>Quelles sont les spécificités d’un VPS Local Zone ?</summary>
...
<summary>Puis-je migrer mon VPS d’une Local Zone vers un datacentre, et vice versa ?</summary>
...
<summary>Quels sont les risques d'utiliser un VPS sans protection DDoS ?</summary>
...
<summary>Comment me connecter à mon VPS ?</summary>
...
Tous les détails sont présentés dans notre guide « [Premiers pas avec un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps) ».
...
<summary>Est-ce qu'OVHcloud fournit une assistance à la configuration logicielle de mon VPS ?</summary>
...
<summary>Quelles sont les bonnes pratiques pour l’envoi d’e-mails depuis un VPS ?</summary>
...
<summary>Puis-je installer plus d’un système d’exploitation sur mon VPS ?</summary>
...
<summary>Puis-je installer des machines virtuelles sur un VPS en utilisant un logiciel de virtualisation (comme Proxmox) ?</summary>
...
<summary>Est-il possible de choisir le matériel de mon VPS (GPU, CPU, etc.) ou de le faire évoluer ?</summary>
...
<summary>Mon VPS est trop lent, puis-je déplacer mon VPS sur un autre hôte ?</summary>
...
<summary>Puis-je ajouter des blocs d’adresses IP à mon VPS ?</summary>
...
<summary>Puis-je changer le VPS associé à ma licence (Plesk, cPanel) ?</summary>
...
<img className="thumbnail" alt="Licence API" src="/images/bare-metal-cloud/virtual-private-servers/vps-faq/getlicense.png" loading="lazy" />

Entrez les paramètres comme suit :

- `serviceName` : Renseignez le nom interne de votre licence (voir la section `Licences` dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>).
...
<img className="thumbnail" alt="Licence API" src="/images/bare-metal-cloud/virtual-private-servers/vps-faq/getlicense_response.png" loading="lazy" />
...
<summary>Comment sécuriser mon VPS ?</summary>
...
Pour cela, aidez-vous de notre guide « [Sécuriser un VPS](/guides/bare-metal-cloud/virtual-private-servers/secure-your-vps) ».
...
<summary>Comment puis-je télécharger mes fichiers ou une sauvegarde du VPS ?</summary>


Plusieurs possibilités s'offrent à vous, par exemple :

- Téléchargement via SFTP : Connectez-vous à votre VPS avec un logiciel client SFTP (par exemple [FileZilla](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp)), puis transférez tous les fichiers du répertoire racine du système.
...
- Téléchargement via l'option **Backup automatisé** : Suivez notre guide sur [l'utilisation des sauvegardes automatisées sur un VPS](/guides/bare-metal-cloud/virtual-private-servers/using-automated-backups-on-a-vps) pour télécharger des fichiers à partir d'une sauvegarde.
- Téléchargement via l'option **Snapshot** : Suivez notre guide sur [l'utilisation des snapshots sur un VPS](/guides/bare-metal-cloud/virtual-private-servers/using-snapshots-on-a-vps) pour télécharger un snapshot du VPS.
...
<summary>Comment puis-je télécharger mon VPS en tant que fichier VM ?</summary>
...
Suivez notre guide « [Utiliser les snapshots sur un VPS](/guides/bare-metal-cloud/virtual-private-servers/using-snapshots-on-a-vps) » pour créer et télécharger un snapshot du VPS.
...
<summary>Comment accéder à mon Backup Storage depuis une adresse IP extérieure à mon service ?</summary>
...
<summary>Mon VPS est-il protégé contre les attaques externes ?</summary>
...
- Suivez notre guide « [Comment configurer le Edge Network Firewall OVHcloud](/guides/bare-metal-cloud/dedicated-servers/firewall-network) », service intégré à notre infrastructure Anti-DDoS afin de limiter l'exposition de vos services aux attaques DDoS. 
- De plus, vous pouvez utiliser nos guides :
    - [Configurer le pare-feu sous Linux avec Iptables](/guides/bare-metal-cloud/virtual-private-servers/firewall-linux-iptable)
    - [Configurer le pare-feu sous Windows](/guides/bare-metal-cloud/virtual-private-servers/activate-port-firewall-soft-win).
- Pour les distributions Linux sur un VPS, vous pouvez suivre les instructions de notre guide « [Sécuriser un VPS](/guides/bare-metal-cloud/virtual-private-servers/secure-your-vps) » pour appliquer des mesures de sécurité supplémentaires.
...
<summary>Comment sécuriser mon VPS contre les attaques DDoS ?</summary>
...
<summary>Je souhaite utiliser un VPS comme serveur de jeu, bénéficiera-t-il du même pare-feu que celui qui protège les serveurs dédiés Game ?</summary>


La protection anti-DDoS Game d’OVHcloud est disponible uniquement pour nos serveurs dédiés Game. Si vous commandez un VPS pour l’hébergement de jeux, vous devrez configurer le pare-feu directement sur votre système, en fonction du jeu souhaité. Retrouvez plus d'informations dans notre guide sur [la configuration du pare-feu sous Linux avec Iptables](/guides/bare-metal-cloud/virtual-private-servers/firewall-linux-iptable).
