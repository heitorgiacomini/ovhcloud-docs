...
description: "Découvrez comment utiliser l'espace client OVHcloud pour gérer votre VPS : tableau de bord, réinstallation, redémarrage, sauvegardes et configuration du service"
...
Veuillez visiter notre [page Web des Local Zones](https://www.ovhcloud.com/fr/lp/vps-localzone/) pour obtenir plus d'informations.
...
L'onglet <code className="action">Accueil</code> constitue le **tableau de bord principal** de votre VPS.
...
<Tabs>
  <Tab label="Nom">
    Pour personnaliser le nom de votre VPS, cliquez sur le bouton <code className="action">...</code> et sélectionnez <code className="action">Modifier le nom</code>. Cette fonctionnalité est utile pour faciliter la navigation dans l'espace client lorsque vous gérez plusieurs services VPS. Toutefois, le nom interne du service reste au format *vps-XXXXXXX.vps.ovh.net*.
  </Tab>
  <Tab label="Boot">
    ...
    Utilisez le bouton <code className="action">...</code> pour [redémarrer le VPS](#rebootvps) ou démarrez-le en mode rescue si nécessaire.
    
    Si besoin, retrouvez plus d'informations dans notre guide sur le [mode rescue](/guides/bare-metal-cloud/virtual-private-servers/rescue).
  </Tab>
  <Tab label="OS / Distribution">
    Il s'agit du système d'exploitation actuellement installé. Utilisez le bouton <code className="action">...</code> pour [réinstaller le même système d'exploitation ou en sélectionner un autre parmi les options disponibles](#reinstallvps).
    ...
    Une fois le système installé, vous êtes responsable de l'application des mises à jour de sécurité du système d'exploitation. Vous trouverez plus d'informations dans la partie « [Réinstaller votre VPS](#reinstallvps) » ainsi que dans notre guide « [Sécuriser un VPS](/guides/bare-metal-cloud/virtual-private-servers/secure-your-vps) ».
  </Tab>
  <Tab label="Zone / Localisation">
...
<Tabs>
  ...
  <Tab label="vCores / Mémoire / Stockage">
    Les ressources actuelles de votre VPS sont affichées ici et peuvent être mises à jour séparément en cliquant sur le lien correspondant. À noter que les mises à niveau sont limitées par le modèle de VPS choisi et peuvent uniquement être disponibles en passant à une [gamme supérieure](https://www.ovhcloud.com/fr/vps/).
  </Tab>
  <Tab label="Disques additionnels">
...
<Tabs>
  <Tab label="IPv4">
    L'adresse IPv4 publique principale du VPS est configurée automatiquement à l'installation. Retrouvez plus d'informations sur la gestion des IP dans notre guide « [Configurer une adresse IP en alias](/guides/bare-metal-cloud/virtual-private-servers/configuring-ip-aliasing) ».
  </Tab>
  <Tab label="IPv6 / Gateway">
    Retrouvez ici l'adresse IPv6 publique et l'adresse de la passerelle associée. Celles-ci sont automatiquement attachées au VPS lors de l'installation. Retrouvez plus d'informations dans notre guide « [Configurer l'IPv6 sur un serveur VPS](/guides/bare-metal-cloud/virtual-private-servers/configure-ipv6) ».
  </Tab>
  <Tab label="Secondary DNS">
    Cette fonctionnalité est utile pour héberger des services DNS. Consultez notre guide « [Configurer un DNS secondaire OVHcloud sur un VPS](/guides/bare-metal-cloud/virtual-private-servers/adding-secondary-dns-on-vps) » pour plus de détails à ce sujet.
  </Tab>
</Tabs>
...
Retrouvez toutes les informations sur les solutions de sauvegarde disponibles pour votre service sur la [page produit VPS](https://www.ovhcloud.com/fr/vps/options/) et dans [nos guides respectifs](/guides/bare-metal-cloud/virtual-private-servers/overview).

#### Mon offre <a name="myoffer"></a>

Cette section présente les informations les plus importantes concernant la facturation de votre service. Retrouvez toutes les informations sur ce sujet dans [nos guides respectifs](/guides/account-and-service-management/managing-billing-payments-and-services/best-practices).
...
Cependant, vous pouvez effectuer un redémarrage forcé à tout moment dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Depuis l'onglet <code className="action">Accueil</code>, cliquez sur le bouton <code className="action">...</code> à côté de `Boot` dans la section **Votre VPS**. Sélectionnez <code className="action">Redémarrer mon VPS</code> et cliquez sur <code className="action">Confirmer</code> dans la fenêtre qui s'affiche.
...
Cliquez sur le bouton <code className="action">...</code> à droite de <code className="action">OS / Distribution</code>, puis sur <code className="action">Réinstaller mon VPS</code>.
...
Dans la fenêtre qui apparaît, choisissez un système d'exploitation dans la liste déroulante. Les options proposées sont [des images compatibles avec un VPS OVHcloud](/guides/public-cloud/compute/image-life-cycle) et sont immédiatement fonctionnelles après l'installation.
...
- Si vous avez précédemment [stocké une clé publique](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel) dans votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, sélectionnez la clé souhaitée dans le menu déroulant `Clé SSH à pré-installer`.
...
- [Comment créer et utiliser des clés d'authentification pour les connexions SSH aux serveurs OVHcloud](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)
...
Certains systèmes d'exploitation ou plateformes propriétaires, comme Plesk ou cPanel, nécessitent des licences qui génèrent des frais supplémentaires. Les licences sont administrables depuis votre espace client : rendez-vous dans la section <code className="action">Bare Metal Cloud</code>, puis cliquez sur <code className="action">Licences</code> dans la barre de navigation à gauche.
