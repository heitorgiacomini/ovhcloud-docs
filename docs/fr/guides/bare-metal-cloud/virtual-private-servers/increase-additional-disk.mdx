...
description: "Découvrez comment augmenter la taille d'un volume additionnel et agrandir sa partition principale"
...
- Un [disque additionnel](/guides/bare-metal-cloud/virtual-private-servers/config-additional-disk) configuré sur le VPS.
...
Les étapes ci-dessous supposent que vous avez configuré un disque additionnel en suivant les instructions de [ce guide](/guides/bare-metal-cloud/virtual-private-servers/config-additional-disk).

### Modifier la taille du disque <a name="extend"></a>

Depuis l'onglet <code className="action">Accueil</code>, dans la section **Votre configuration**, cliquez sur <code className="action">...</code> à côté de l'option `Disques additionnels` puis cliquez sur <code className="action">Augmenter la taille du disque</code>.
...
Choisissez la nouvelle taille de disque dans la fenêtre qui apparaît, puis cliquez sur <code className="action">Augmenter</code>.
...
L'augmentation de capacité du disque prendra quelques minutes après la validation du paiement. Vous pouvez vérifier la progression dans l'onglet <code className="action">Disques additionnel</code> : si la nouvelle taille choisie est affichée, le disque est prêt.
...
Nous mettons ce guide à votre disposition afin de vous accompagner au mieux sur les tâches courantes. Néanmoins, nous vous recommandons de faire appel à un [prestataire spécialisé](https://partner.ovhcloud.com/fr/directory/) ou de faire appel à [notre communauté](https://community.ovhcloud.com/community/fr) si vous éprouvez des difficultés.
...
Une fois connecté, faites un clic droit sur le bouton <code className="action">Menu Démarrer</code> et ouvrez <code className="action">Gestion des disques</code>.
...
Le [disque étendu](#extend) affiche la capacité supplémentaire sous forme d'espace non alloué. Faites un clic droit sur le volume de votre disque additionnel et sélectionnez <code className="action">Étendre le volume</code> dans le menu contextuel.
...
Vous pourrez modifier l'espace disque à cette étape si cela est nécessaire. Cliquez sur <code className="action">Suivant</code>.
