...
description: "Apprenez à configurer l'IPv6 sur votre serveur VPS OVHcloud"
...
:::info
Cet article concerne la configuration d'adresses IPv6 principales sur une interface publique. Vous pouvez également configurer des adresses Additional IP sur votre VPS en utilisant [ce guide](/guides/bare-metal-cloud/virtual-private-servers/configuring-ip-aliasing).
...
- Disposer d'un [serveur VPS OVHcloud](https://www.ovhcloud.com/fr/vps/).
...
<Tabs>
  ...
  <Tab label="Via les API OVHcloud">
    Rendez-vous sur la page des [API OVHcloud](https://eu.api.ovh.com/console/) :
    
    - Cliquez sur <code className="action">Authentication</code> en haut à gauche.
    - Cliquez ensuite <code className="action">Login with OVHcloud SSO</code>.
...
Une fois les informations nécessaires pour la configuration IPv6 en votre possession, connectez-vous en SSH à votre VPS. Si besoin, aidez-vous des informations de notre documentation « [Introduction au SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction) ».
...
- [Application persistante sur Debian et dérivés (Ubuntu, Crunchbang, SteamOS, etc.)](#persistentdebian).
- [Application persistante sur Redhat et dérivés (CentOS, Rocky Linux & AlmaLinux, etc.)](#persistentredhat).
- [Application persistante sur Fedora 42 et versions ultérieures](#persistentfedora).
...
#### Application persistante sur Debian et dérivés (Ubuntu, Crunchbang, SteamOS, etc.) <a name="persistentdebian"></a>
...
- **pour Debian 12, Ubuntu 22.04 et versions ultérieures** : utilisez la méthode basée sur la fonction *Netplan*.

Dans certains cas, il se peut que la méthode à utiliser ne soit pas celle spécifiée ci-dessus. Pour vous en assurer, naviguez dans votre système pour vérifier la méthode active dans votre cas. Visitez le site [https://netplan.io/](https://netplan.io/) pour plus d'informations, si nécessaire.
...
<Tabs>
  <Tab label="Configuration des fichiers *interfaces*">
  ...
  <Tab label="Configuration à l'aide de *Netplan*">
    ...
    **Exemple de configuration** (avec préfixe /128) :
...
<Tabs>
  <Tab label="Configuration des fichiers *network-scripts*">
  ...
  <Tab label="Configuration des fichiers *system-connections*">
...
#### Application persistante sur Fedora 42 et versions ultérieures <a name="persistentfedora"></a>
...
Par défaut, l'adresse IPv6 n'est pas configurée sur les serveurs Windows. Pour l'activer, ouvrez le <code className="action">Panneau de configuration</code> et cliquez sur <code className="action">Afficher l'état et les tâches du réseau</code>, puis sur <code className="action">Modifier les paramètres de la carte</code>.
...
Cliquez sur <code className="action">Ethernet</code> pour ouvrir les paramètres et cliquez sur le bouton <code className="action">Propriétés</code> pour afficher `Propriétés Ethernet`.

Sélectionnez <code className="action">Protocol Internet version 6 (TCP/IPv6)</code>, puis cliquez sur le bouton <code className="action">Propriétés</code>.
...
Dans la fenêtre Propriétés IPv6, sélectionnez <code className="action">Utiliser l'adresse IPv6 suivante</code>. Entrez les adresses IP que vous avez récupérées à la première étape.

Vous avez également la possibilité d'entrer les résolutions DNS IPv6 de votre choix sous <code className="action">Utiliser l'adresse de serveur DNS suivante</code>. Cela n'est pas obligatoire si les résolveurs DNS de la configuration IPv4 sont déjà fonctionnels.

Enfin, cochez la case `Valider les paramètres en quittant` et cliquez sur le bouton <code className="action">OK</code> pour valider vos modifications. Un message d'erreur peut s'afficher si la passerelle spécifiée ne se trouve pas sur le même sous-réseau IPv6 (/128 et /64, par exemple). Vous pouvez ignorer ce message et passer à l'étape suivante.
...
<summary>**Étape 4 : Désactiver la gestion du réseau Cloud-init (en option)**</summary>
