...
description: Découvrez comment mettre en place des mesures de sécurité basiques pour protéger votre VPS des attaques et des accès non autorisés
...
S'il s'agit de votre première configuration d'un VPS OVHcloud, nous vous invitons à consulter en premier lieu notre guide [Débuter avec un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps).
:::


Les exemples suivants supposent que vous êtes connecté en tant qu'[utilisateur avec des autorisations élevées](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).
...
L’authentification par clé SSH est l’une des méthodes les plus efficaces pour sécuriser l’accès à votre VPS.<br/>
...
N'oubliez pas que vous devrez indiquer le nouveau port à chaque demande de [connexion SSH à votre serveur](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction) :
...
Si vous êtes bloqué hors de votre système, vous pouvez utiliser notre [mode rescue](/guides/bare-metal-cloud/virtual-private-servers/rescue) pour annuler vos modifications.

### Créer un utilisateur avec des droits restreints <a name="createuser"></a>

En général, les tâches qui ne requièrent pas de privilèges root doivent être effectuées via un utilisateur standard. Pour plus d'informations, consultez [ce guide](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds).

### Configurer le pare-feu interne (iptables) <a name="iptables"></a>
...
Pour plus d'informations sur iptables, consultez notre [guide dédié](/guides/bare-metal-cloud/virtual-private-servers/firewall-linux-iptable).
...
Consultez le guide « [Configurer le Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network) » si vous souhaitez l'activer.

### Sauvegarder votre système et vos données <a name="backup"></a>
...
Vous trouverez toutes les informations sur les solutions de sauvegarde disponibles pour votre service sur la [page produit](https://www.ovhcloud.com/fr/vps/options/) et dans les [guides respectifs](/guides/bare-metal-cloud/virtual-private-servers/overview).

## Aller plus loin

[Débuter avec un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)

[Créer et utiliser des clés SSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)
