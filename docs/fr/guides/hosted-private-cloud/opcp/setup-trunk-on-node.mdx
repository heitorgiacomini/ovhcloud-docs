...
Les ports Trunk permettent à une seule instance (bare metal ou machine virtuelle) d'envoyer et de recevoir du trafic sur plusieurs réseaux Neutron via du vlan tagging, à travers une seule interface physique ou un [bond LACP](/guides/hosted-private-cloud/opcp/how-to-setup-lacp-on-node).
...
:::warning
Le réseau d'un sous-port doit être **différent** de celui du port parent. Le sous-port hérite de l'adresse MAC du port parent (la sous-interface vlan de l'OS invité en hérite également, ce qui est nécessaire au DHCP). Deux ports d'un même réseau ne pouvant pas partager une adresse MAC, un sous-port créé sur le réseau du port parent fait échouer le déploiement du nœud.
:::
...
:::info
Si votre instance utilise le bonding LACP (voir le [guide LACP](/guides/hosted-private-cloud/opcp/how-to-setup-lacp-on-node)), remplacez `ens3` par le nom de votre interface bond (par exemple `bond0`). Les sous-interfaces vlan deviennent alors `bond0.100`, `bond0.200`, etc.
:::
