...
description: "Redéploiement personnalisé d'un Cluster Nutanix via les outils OVHcloud"
...
Depuis l'espace client OVHcloud, cliquez sur l'onglet <code className="action">Hosted Private Cloud</code>, sélectionnez votre cluster en dessous de la catégorie `Nutanix` à gauche et cliquez sur <code className="action">Redéployer mon cluster</code>.

![00 Redeploy cluster from OVHcloud control panel 01](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager01.png)

Si vous avez souscrit l'offre Nutanix BYOL, un rappel vous indique ne pas oublier de désinstaller vos licences.<br/>
Vous pouvez vous aider de ce guide pour gérer vos licences : [Gestion des licences dans votre cluster Nutanix on OVHcloud BYOL](/guides/hosted-private-cloud/nutanix-on-ovhcloud/activate-licenses-on-byol).<br/>
Lorsque vos license seront désinstallées, cliquez sur <code className="action">Continuer</code>.

![00 Redeploy cluster from OVHcloud control panel 02](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager02.png)

Cliquez sur <code className="action">Personnaliser la configuration</code> et cliquez sur <code className="action">Suivant</code>

![00 Redeploy cluster from OVHcloud control panel 03](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager03.png)
...
![00 Redeploy cluster from OVHcloud control panel 04](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager04.png)
...
Cliquez ensuite sur <code className="action">Redéployer</code>.

![00 Redeploy cluster from OVHcloud control panel 05](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager05.png)

Saisissez `REDEPLOY` et cliquez sur <code className="action">Confirmer</code>.

![00 Redeploy cluster from OVHcloud control panel 06](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager06.png)
...
![00 Redeploy cluster from OVHcloud control panel 07](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/00-cluster-redeployment-through-manager07.png)

### Redéploiement du cluster au travers de l'API OVHcloud

Connectez-vous à l'[API OVHcloud](https://api.ovh.com). Pour plus de détails sur le fonctionnement de l'API OVHcloud, consultez notre guide [Premiers pas avec les API OVHcloud](/guides/manage-and-operate/api/first-steps)

:::warning
Si vous utilisez l'API OVHcloud pour reconditionner votre cluster avec une offre Nutanix BYOL, vous n'aurez pas de message pour vous avertir de désinstaller vos licences. N'oubliez pas de le faire. Vous pouvez vous aider de ce guide pour gérer vos licences : [Gestion des licences dans votre cluster Nutanix on OVHcloud BYOL](/guides/hosted-private-cloud/nutanix-on-ovhcloud/activate-licenses-on-byol). Après avoir desinstalllé vos licences, vous pourrez redéployer votre cluster avec l'API OVHcloud.
...
<Api version="v1" section="/nutanix" method="PUT" route={"/nutanix/\{serviceName\}"} />
...
Cliquez ensuite sur le bouton <code className="action">Plus</code>.

![Cluster Redeployment 03](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/01-cluster-redeployment03.png)
...
Cliquez à nouveau sur le bouton <code className="action">Plus</code>.

![Cluster Redeployment 04](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/01-cluster-redeployment04.png)
...
![Cluster Redeployment 04](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/01-cluster-redeployment05.png)

Dans la rubrique **Prism Central Configuration** cochez <code className="action">Define property</code> et <code className="action">Empty array</code>.<br/>
...
Cliquez sur <code className="action">Execute</code> pour lancer le redéploiement du cluster.

![Cluster Redeployment 05](/images/hosted-private-cloud/nutanix-on-ovhcloud/00-cluster-custom-redeployment/01-cluster-redeployment06.png)
...
[Utilisation de l'API OVHcloud](/guides/manage-and-operate/api/first-steps).

[Gestion des licences dans votre cluster Nutanix on OVHcloud BYOL](/guides/hosted-private-cloud/nutanix-on-ovhcloud/activate-licenses-on-byol).
