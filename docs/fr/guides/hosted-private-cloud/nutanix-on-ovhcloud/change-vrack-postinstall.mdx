...
description: "Découvrez comment modifier le vRack d'un cluster Nutanix"
...
- Disposer d'un service [vRack](https://www.ovhcloud.com/fr/network/vrack/) supplémentaire activé dans votre compte OVHcloud
...
Connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Ouvrez la page de configuration du Nutanix Cluster vRack, sélectionnez tous les services (par exemple, serveur dédié, IP, load balancer) et cliquez sur <code className="action">Supprimer</code>.
...
Une fois le processus terminé, rendez-vous sur le vRack souhaité et ajoutez tous les services précédemment supprimés avec le bouton <code className="action">Ajouter</code>.
