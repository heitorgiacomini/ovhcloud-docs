...
description: "Ce guide fournit des instructions pour le déploiement de l'image SLES 15 for SAP sur un serveur dédié OVHcloud et sa préparation pour SAP HANA"
...
Depuis l'espace client OVHcloud, vous pouvez lancer le déploiement de l'image SLES 15 for SAP que OVHcloud met à disposition en cliquant sur le bouton <code className="action">Installer</code>.

<img className="thumbnail" alt="install-manager" src="/images/hosted-private-cloud/sap-on-ovhcloud/cookbook-install-sles-sap-hana-dedicated-server/install-manager.png" loading="lazy" />

La première étape consiste à renseigner des informations sur le système d'exploitation que vous souhaitez installer.<br/>
&ensp;&thinsp;a. Dans le menu déroulant <code className="action">Type de système d'exploitation</code>, sélectionnez <code className="action">ERP</code>.<br/>
&ensp;&thinsp;b. Dans le menu <code className="action">LINUX</code>, sélectionnez <code className="action">SUSE Linux Enterprise Server 15 SP3 for SAP Applications - sles-sap15sp3</code>.

Nous vous recommandons de réaliser l'installation du système d'exploitation sur la grappe de disque cible <code className="action">2 X Disk SSD 480 GB, JBOD</code>.
...
<img className="thumbnail" alt="sles-template" src="/images/hosted-private-cloud/sap-on-ovhcloud/cookbook-install-sles-sap-hana-dedicated-server/sles-template.png" loading="lazy" style={{ maxWidth: '593px' }} />

Si vous ne personnalisez pas la configuration des partitions, vous accéderez directement à la dernière étape.

Vous pouvez indiquer un nom de serveur qui sera visible via la commande `hostname`. Vous avez également la possibilité d'[ajouter votre clé SSH](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#ajout-dune-cle-ssh-facultatif).
...
<img className="thumbnail" alt="sles-partitions" src="/images/hosted-private-cloud/sap-on-ovhcloud/cookbook-install-sles-sap-hana-dedicated-server/sles-partitions.png" loading="lazy" />

Une fois ces derniers paramètres configurés, cliquez sur <code className="action">Valider</code> pour lancer l'installation.
...
Une fois l'installation de l'image SLES 15 for SAP réalisée, vous pouvez [vous connecter à votre serveur dédié](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#connexion-a-votre-serveur).
