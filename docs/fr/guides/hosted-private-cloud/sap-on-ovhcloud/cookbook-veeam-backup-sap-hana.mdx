...
description: Ce guide fournit des instructions générales pour sauvegarder des bases de données SAP HANA avec Veeam Backup and Replication et Veeam Plug-in for SAP HANA
...
Si vous souhaitez être guidé sur l'installation de Veeam Backup and Replication sur votre serveur Windows, [notre guide](/guides/storage-and-backup/backup-and-disaster-recovery-solutions/veeam/veeam-veeam-backup-replication) vous détaille l'ensemble des étapes.
...
<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez <code className="action">Backup Infrastructure</code>, puis <code className="action">Backup Repositories</code>.
    
    Cliquez sur <code className="action">Add Repository</code> pour démarrer l'assistant de configuration.
  ...
  <Tab label="Étape 2">
    Sélectionnez <code className="action">Direct attached storage</code> ou <code className="action">Network attached storage</code>, en fonction de votre situation.
    
    - <code className="action">Direct attached storage</code> permet d'ajouter un serveur physique ou virtuel comme espace de stockage sans protocole NFS ou SMB.
    - <code className="action">Network attached storage</code> permet d'ajouter un partage de fichiers via les protocoles NFS ou SMB (CIFS).
...
Pour découvrir les étapes de création d'un bucket Object Storage, veuillez prendre connaissance des étapes détaillées dans [notre guide](/guides/storage-and-backup/object-storage/s3-getting-started-with-object-storage).

<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez <code className="action">Backup Infrastructure</code>, puis <code className="action">Backup Repositories</code>.
    
    Cliquez à présent sur <code className="action">Add Repository</code> pour démarrer l'assistant de configuration.
  ...
  <Tab label="Étape 2">
    Sélectionnez <code className="action">Object storage</code>.
  ...
  <Tab label="Étape 5">
    Entrez le <code className="action">Service point</code> qui correspond à l'endpoint du bucket Object Storage, ainsi que la région (en minuscule) de votre bucket Object Storage.
    
    Ajoutez les identifiants de l'utilisateur Object Storage qui possède le rôle <code className="action">Administrator</code> sur ce bucket Object Storage. Il s'agit de la clé d'accès et de la clé secrète de l'utilisateur Object Storage associée au bucket Object Storage. Retrouvez plus d'informations sur l'utilisateur Object Storage dans [notre guide](/guides/storage-and-backup/object-storage/s3-identity-and-access-management).
...
<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez <code className="action">Backup Infrastructure</code>, puis <code className="action">Scale-out Repositories</code>.
    
    Cliquez à présent sur <code className="action">Add Scale-out Repository</code> pour démarrer l'assistant de configuration.
  ...
  <Tab label="Étape 3">
    Cliquez sur <code className="action">Add</code> et sélectionnez votre Backup Repository précédemment créé.
  ...
  <Tab label="Étape 4">
    Sélectionnez la politique de localisation souhaitée. Nous conseillons la politique <code className="action">Data locality</code>.
  ...
  <Tab label="Étape 5">
    Cochez <code className="action">Extend scale-out backup repository capacity with object storage</code>, puis sélectionnez le Backup Repository Object Storage précédemment créé.
    
    Cochez l'option <code className="action">Copy backups to object storage as soon as they are created</code> afin de répliquer les sauvegardes du premier Backup Repository sur le second Backup Repository immédiatement après leur création. Cette configuration permet une protection rapide de vos sauvegardes.
  ...
  <Tab label="Étape 6">
    ...
    Sélectionnez votre Scale-out Repository et cliquez sur <code className="action">Set Access Permissions</code>.
...
<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez le menu <code className="action">Home</code>, <code className="action">Backup Copy</code>, puis <code className="action">Application-level backup...</code>.
  ...
  <Tab label="Étape 4">
    ...
    Prenez connaissance des paramètres disponibles dans le menu <code className="action">Advanced</code>.
...
<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez <code className="action">Inventory</code>, puis <code className="action">Physical Infrastructure</code>.
    
    Cliquez sur <code className="action">Create Protection Group</code>.
  ...
  <Tab label="Étape 2">
    Cliquez sur <code className="action">Individual computers</code>.
  ...
  <Tab label="Étape 3">
    Donnez un nom à votre groupe de protection. Puis cliquez sur <code className="action">Next</code>.
  ...
  <Tab label="Étape 4">
    Ajoutez le ou les bases de données SAP HANA que vous souhaitez ajouter à ce groupe de protection. Vous pouvez par exemple regrouper vos bases de données de production. Puis cliquez sur <code className="action">Next</code>.
  ...
  <Tab label="Étape 5">
    ...
    Cochez <code className="action">Install application plug-ins</code>. Cliquez sur <code className="action">Configure</code>, puis cochez <code className="action">SAP HANA</code>.
    
    Cliquez sur <code className="action">Next</code> pour continuer.
...
<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez le menu <code className="action">Home</code>, <code className="action">Backup job</code>, <code className="action">Application</code>, puis <code className="action">SAP HANA</code>.
  ...
  <Tab label="Étape 2">
    Donnez un nom à votre job de sauvegarde SAP HANA. Puis cliquez sur <code className="action">Next</code>.
  ...
  <Tab label="Étape 3">
    Sélectionnez votre ou vos bases de données SAP HANA, ainsi que les TENANT associés (SYSTEMDB et/ou TENANTDB) que vous souhaitez gérer avec ce job de sauvegarde. Puis cliquez sur <code className="action">Next</code>.
  ...
  <Tab label="Étape 4">
    ...
    Cliquez sur <code className="action">Advanced...</code> afin de planifier les sauvegardes complètes, sélectionnez le type de sauvegardes incrémentales ou différentielles. Dans l'onglet <code className="action">SAP HANA</code>, vous avez la possibilité de paramétrer le nombre de canaux pour optimiser la vitesse de sauvegarde SAP HANA. <a name="canaux"></a>
  ...
  <Tab label="Étape 5">
    Configurez les identifiants OS et de la base de données SAP HANA. Puis cliquez sur <code className="action">Next</code>.
  ...
  <Tab label="Étape 7">
    Configurez la planification de l'exécution du Backup job. Puis cliquez sur <code className="action">Apply</code>.
  ...
  <Tab label="Étape 8">
    Veillez à cocher <code className="action">Enable the backup policy when I click Finish</code> afin d'activer le Backup job. Cliquez sur <code className="action">Finish</code> pour terminer la configuration du Backup job.
...
Pour cela, il suffit de vous rendre dans le menu <code className="action">Home</code>, de sélectionner <code className="action">Jobs</code> puis <code className="action">Applications</code>, puis de sélectionner votre Backup job et de cliquer sur <code className="action">Full</code>. Cela aura pour effet de lancer une sauvegarde complète sur la ou les bases de données SAP HANA configurées dans ce Backup job.
...
<Tabs>
  <Tab label="Étape 1">
    Ouvrez la console Veeam Backup and Replication, sélectionnez <code className="action">Home</code>, <code className="action">Backups</code>, <code className="action">Disk</code>, puis cliquez sur <code className="action">Restore from SAP HANA backups...</code> en faisant un clic droit sur la sauvegarde de votre base de données SAP HANA.
