...
description: "Découvrez comment réenregistrer des VM sur un nouveau service depuis d'anciens datastores"
...
Dans votre [interface vSphere](/guides/hosted-private-cloud/powered-by-vmware/vsphere-interface-connexion), placez-vous dans la vue <code className="action">Stockage</code>.

<img className="thumbnail" alt="vue stockage" src="/images/hosted-private-cloud/powered-by-vmware/vmware-register-vmx/register-vmx-01.png" loading="lazy" />

Sélectionnez un datastore dans la liste.

<img className="thumbnail" alt="sélectionner datastore" src="/images/hosted-private-cloud/powered-by-vmware/vmware-register-vmx/register-vmx-02.png" loading="lazy" />

Dans les dossiers de ce datastore, sélectionnez le fichier `.vmx` et cliquez sur <code className="action">Enregistrer la VM</code>.

<img className="thumbnail" alt="enregistrer VM" src="/images/hosted-private-cloud/powered-by-vmware/vmware-register-vmx/register-vmx-03.png" loading="lazy" />

Complétez les informations requises puis cliquez sur <code className="action">Terminer</code>.

<img className="thumbnail" alt="enregistrer VM" src="/images/hosted-private-cloud/powered-by-vmware/vmware-register-vmx/register-vmx-04.png" loading="lazy" />

Ces mêmes opérations sont à répéter sur chaque datastore et pour chaque VM à re-enregister.

Vérifiez les paramètres de vos VM (nom, [portgroup](/guides/hosted-private-cloud/powered-by-vmware/create-vlan), etc...) en faisant un clic droit sur chacune d'elles puis en sélectionnant <code className="action">Edit Settings</code>.

<img className="thumbnail" alt="modifier paramètres" src="/images/hosted-private-cloud/powered-by-vmware/vmware-register-vmx/register-vmx-06.png" loading="lazy" />

En cas d'erreur de paramétrage, un message d'erreur vous sera affiché lorsque vous rallumerez la VM.

Vous pourrez rallumer une VM en faisant un clic droit sur celle-ci puis en cliquant sur <code className="action">Power On</code>

<img className="thumbnail" alt="enregistrer VM" src="/images/hosted-private-cloud/powered-by-vmware/vmware-register-vmx/register-vmx-05.png" loading="lazy" />
