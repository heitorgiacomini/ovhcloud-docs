...
description: Comprendre le fonctionnement des snapshots horaires OVHcloud
...
Les fichiers sont présents maintenant il suffit d'ajouter cette machine dans votre **inventaire** en cliquant sur le fichier `.vmx`, puis sur <code className="action">register VM</code> au dessus.
...
Nous vous recommandons l'utilisation d'une solution complète de sauvegarde comme notre service [Veeam Backup](/guides/hosted-private-cloud/powered-by-vmware/veeam-backup-as-a-service) ou tout autre système réalisant un backup complet de vos machines virtuelles.
