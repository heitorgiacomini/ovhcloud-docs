...
description: Apprenez à mettre en œuvre une solution de contournement avec le mécanisme VMware DRS
...
- Activer l'option [Veeam Backup Managed](https://www.ovhcloud.com/fr/enterprise/products/hosted-private-cloud/veeam-backup-managed/).
...
Pour mettre en œuvre cette solution, cliquez sur le cluster approprié, rendez-vous sur l'onglet <code className="action">Configurer</code> puis la section <code className="action">Règles de VM/hôte</code>.

<img className="thumbnail" alt="vSphere" src="/images/hosted-private-cloud/powered-by-vmware/avoid-vm-freeze-with-veeam-backup/en01add.png" loading="lazy" />
...
<img className="thumbnail" alt="proxy" src="/images/hosted-private-cloud/powered-by-vmware/avoid-vm-freeze-with-veeam-backup/en02proxy.png" loading="lazy" />

Créez une autre règle DRS pour **séparer les machines virtuelles**, afin de conserver les backups proxy sur différents hôtes :

<img className="thumbnail" alt="proxy" src="/images/hosted-private-cloud/powered-by-vmware/avoid-vm-freeze-with-veeam-backup/en03proxy2.png" loading="lazy" />
