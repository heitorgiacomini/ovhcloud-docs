...
description: Apprenez à mettre en place la double authentification pour protéger votre infrastructure
...
<img className="thumbnail" alt="Gateway Private Cloud" src="/images/hosted-private-cloud/powered-by-vmware/use-2fa/gatewayPCC.jpg" loading="lazy" />

- Directement via l'URL `https://pcc-xxx-xxx-xxx-xxx.ovh.com/secure/` ( attention à ne pas oublier le "/" final de l'adresse).

Une fois connecté à l'interface de gestion, cliquez sur <code className="action">Change Password</code>

<img className="thumbnail" alt="Change Password" src="/images/hosted-private-cloud/powered-by-vmware/use-2fa/selectChangePassword.png" loading="lazy" />

Au sein de l'interface, vous devez alors :
	
* Sélectionner  <code className="action">Password and 2FA Shared Secret</code> ,
...
<img className="thumbnail" alt="Scan QRcode" src="/images/hosted-private-cloud/powered-by-vmware/use-2fa/scanQRcode.png" loading="lazy" />

Une tâche se créera alors, et un token vous sera envoyé.

Rendez-vous dans la partie <code className="action">Operation validation</code> , chargez l'opération reçue par SMS, et confirmez avec le token reçu dans ce même SMS.
...
<img className="thumbnail" alt="Connexion 2FA" src="/images/hosted-private-cloud/powered-by-vmware/use-2fa/2FAtoken.png" loading="lazy" />
...
Ces applications doivent être préalablement autorisées au travers de la politique d'accès au vCenter qui est paramétrable dans votre [espace client](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud#securite)
