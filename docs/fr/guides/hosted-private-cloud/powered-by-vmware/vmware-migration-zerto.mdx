...
description: Découvrez comment migrer vos charges de travail VMware on-premises vers un environnement Hosted Private Cloud OVHcloud à l’aide de Zerto Virtual Replication
...
[Move2Cloud - Migration de charges de travail VMware vers le Hosted Private Cloud SecNumCloud avec Zerto](/guides/hosted-private-cloud/powered-by-vmware/vmware-migration-zerto-secnumcloud)
...
<img className="thumbnail" alt="Move2CloudZerto" src="/images/assets/screens/other/zerto/move2cloud-zerto.png" loading="lazy" />
...
Retrouvez plus d'informations sur la planification du réseau dans notre guide « [Premiers pas avec NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps) ».
...
- Migrer vos plages d'adresses IP existantes grâce à la fonctionnalité [Bring Your Own IP (BYOIP)](https://www.ovhcloud.com/fr/network/byoip/).
...
2. Sélectionnez votre <code className="action">Hosted Private Cloud</code>.
3. Accédez à l'onglet <code className="action">Sécurité</code>
4. Cliquez sur <code className="action">Ajouter une nouvelle plage d'adresses IP</code> pour autoriser vos adresses IP d'infrastructure source et vos composants Zerto.

Pour des instructions détaillées, référez-vous à notre guide « [Autoriser des IP à se connecter au vCenter](/guides/hosted-private-cloud/powered-by-vmware/authorise-ip-vcenter-access) ».
...
Configurez les rôles et permissions dans votre <code className="action">Hosted Private Cloud</code> via la solution **IAM** d'OVHcloud.
...
Pour des instructions détaillées, référez-vous au [guide de configuration IAM](/guides/hosted-private-cloud/powered-by-vmware/vmware-iam-getting-started).
...
Pour cela, suivez notre guide « [Activer les connexions Active Directory Federation Services (AD FS) SSO avec votre compte OVHcloud](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-adfs) ».
...
Référez-vous à notre guide « [Premiers pas avec NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps) » pour obtenir plus de détails sur la création de segments et leur affectation aux machines virtuelles.
...
Vous retrouverez un aperçu de la façon dont NSX gère ces fonctionnalités dans notre guide « [Premiers pas avec NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps) ».
...
Dans votre interface <code className="action">Hosted Private Cloud</code> :

1. Accédez à <code className="action">Reprise d'activité</code>.
2. Sélectionnez <code className="action">Activer la réplication virtuelle Zerto</code>.
...
Tous les détails sont disponibles dans notre guide « [Utiliser Zerto entre OVHcloud et une plateforme tierce](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-customer-to-ovh) ».
...
Les détails et les paramètres sont disponibles dans notre guide « [Utiliser Zerto entre OVHcloud et une plateforme tierce](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-customer-to-ovh) ».
...
Utilisez <code className="action">Storage vMotion</code> via <code className="action">vSphere Client</code> :

1. Faites un clic droit sur la VM puis cliquez sur <code className="action">Migrer</code>.
2. Sélectionnez <code className="action">Modifier le stockage uniquement</code>.
3. Choisissez le datastore de destination ou la stratégie vSAN.

Pour plus de détails, consultez notre guide « [VMware Storage vMotion](/guides/hosted-private-cloud/powered-by-vmware/storage-vmotion) ».
