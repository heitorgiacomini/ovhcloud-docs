...
description: "Découvrez comment déplacer vos VMs d'un vDC existant vers un nouveau vDC dans la même infrastructure VMware"
...
- Être connecté à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> dans la partie <code className="action">Hosted Private Cloud</code>.
...
<img className="thumbnail" alt="decision tree" src="/images/hosted-private-cloud/powered-by-vmware/service-migration-vdc/tree.png" loading="lazy" />
...
<Api version="v1" section="/dedicatedCloud" method="GET" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/filer/\{filerId\}/checkGlobalCompatible"} />
...
<img className="thumbnail" alt="add a Virtual Datacenter" src="/images/hosted-private-cloud/powered-by-vmware/service-migration-vdc/add-vDC.gif" loading="lazy" />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/filer/\{filerId\}/convertToGlobal"} />
...
Pour vous connecter à votre infrastructure VMware, vous pouvez choisir de bloquer l'accès au vSphere par défaut. Pour cela, consultez notre guide sur la [politique d'accès au vCenter](/guides/hosted-private-cloud/powered-by-vmware/authorise-ip-vcenter-access).
...
Consultez à cet effet nos guides pour [changer les droits d'un utilisateur](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud), [modifier le mot de passe d'un utilisateur](/guides/hosted-private-cloud/powered-by-vmware/change-user-password) et [associer un e-mail à un utilisateur](/guides/hosted-private-cloud/powered-by-vmware/vsphere-edit-user).
...
Par défaut, dans le cadre d'une migration, le nouveau vDC sera lié au même vRack que le vDC d'origine. Consultez notre guide sur [l'utilisation de Hosted Private Cloud au sein d'un vRack](/guides/hosted-private-cloud/powered-by-vmware/using-private-cloud-in-vrack).
...
L'installation d'un nouveau vDC de destination implique de refaire la configuration du VMware High Availability (HA), notamment l'ordre et la priorité de boot. Consultez [notre guide sur sa configuration](/guides/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability).
...
Pour plus d'informations, consultez le guide OVHcloud sur [comment créer un V(x)LAN dans un vRack](/guides/hosted-private-cloud/powered-by-vmware/create-vlan#vlan-vrack) et la documentation de VMware sur [comment modifier les paramètres des groupes de ports distribués](https://docs.vmware.com/fr/VMware-vSphere/7.0/com.vmware.vsphere.networking.doc/GUID-FCA2AE5E-83D7-4FEE-8DFF-540BDB559363.html).
...
- Cependant, si le « *Promiscuous Mode* » doit être activé sur le portgroup « VM Network » du nouveau vDC, suivez les instructions de [ce guide](/guides/hosted-private-cloud/powered-by-vmware/vmware-carp).
...
Dans un premier temps, nous vous invitions à consulter notre documentation sur [les premiers pas avec NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps).
...
Pour plus d'informations, consultez notre documentation sur la [gestion du pare-feu distribué dans NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-manage-distributed-firewall).
...
Pour créer les T1, suivez [cette documentation](/guides/hosted-private-cloud/powered-by-vmware/nsx-add-new-tier1-gateway). Ce guide vous aiguillera aussi sur la façon de créer les segments. Avant de créer les segments, il est nécessaire de faire l'inventaire des VXLANs utilisés dans le vDC source et de créer un segment pour chaque VXLAN utilisé sur votre infrastructure.

Il suffit ensuite de les relier à la T0 fournie dans NSX.

Pour plus d'informations, consultez notre documentation sur la [gestion des segments dans NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-segment-management).
...
Pour recréer les DHCP et les associer à vos segments puis à vos T1, consultez notre guide sur la [configuration du DHCP dans NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-configure-dhcp-onsegment).

<a name="dns"></a>
##### Etape 4.8.3.3 DNS

Pour recréer les DNS et les associer à vos T1, consultez notre guide sur la [configuration du redirecteur DNS dans NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-configure-dns-forwarder).

<a name="nat"></a>
##### Etape 4.8.3.4 Règles NAT

Pour recréer vos règles NAT et les associer aux T1, consultez notre guide sur la [mise en place du NAT pour des redirections de ports avec NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-configure-nat-redirection).

<a name="lb"></a>
##### Etape 4.8.3.5 Load Balancing NSX

Pour recréer vos Load Balancer, suivez les indications de notre guide sur la [configuration du Load Balancing dans NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-configure-loadbalancing).

<a name="fwgw"></a>
##### Etape 4.8.3.6 Pare-feu des passerelles T0 T1

Pour recréer les règles de pare-feu associées à vos anciennes edges, consultez notre guide sur la [gestion du pare-feu des passerelles dans NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-manage-gateway-firewall).
...
Pour recréer vos sessions IPsec, consultez notre guide sur la [mise en place d'un tunnel IPsec avec NSX](/guides/hosted-private-cloud/powered-by-vmware/nsx-configure-ipsec).
...
- l'IP publique de la VIP associée à la T0 de NSX (visible dans <code className="action">Networking</code> > <code className="action">Tier-0 Gateways</code> > <code className="action">ovh-T0-XXXX</code> > dépliez > <code className="action">HA VIP Configuration</code> > cliquez sur <code className="action">1</code> > section <code className="action">IP Address / Mask</code>)

Par la suite, depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>, suivez les indications de notre guide « [Déplacer une Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip) »  pour déplacer le bloc initial NSX-V sur le service PCC que vous migrez, tout en précisant comme *next hop* l'IP de la VIP de la T0 récupérée précédemment, comme sur l'exemple ci-dessous :

<img className="thumbnail" alt="Migration NSX IP" src="/images/hosted-private-cloud/powered-by-vmware/service-migration-vdc/MoveIPNextHop.png" loading="lazy" />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/disasterRecovery/zerto/startMigration"} />
...
L'étape suivante dépend de la configuration actuelle par [groupe de protection virtuelle](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-vmware-vsphere-drp) :
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/checkBackupJobs"} />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/backup/disable"} />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/disasterRecovery/zerto/endMigration"} />
...
Pour changer la Private Gateway de virtual datacenter (vDC), pour devez d'abord la désactiver en suivant les étapes du guide pour [désactiver la private gateway](/guides/hosted-private-cloud/powered-by-vmware/gateway#disablegw).
...
Puis activez de nouvelle celle-ci en en suivant les étapes du guide pour [activer la private gateway](/guides/hosted-private-cloud/powered-by-vmware/gateway#enablegw) et en choisissant le datacenterId du nouveau vDC.
...
1. Dans le client vSphere, accédez à <code className="action">Hosts and Clusters</code>.
2. Accédez à un <code className="action">Host</code>.
3. Faites un clic droit sur le Host.
4. Accédez à <code className="action">Maintenance Mode</code>.
5. Cliquez sur <code className="action">Enter Maintenance Mode</code>.
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/filer/\{filerId\}/remove"} />

Dans le cas d'un datastore global, vous pouvez utiliser l'appel API suivant :

<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/filer/\{filerId\}/remove"} />

Une tâche est créée pour chaque appel, vous pouvez suivre l'avancement avec :

<Api version="v1" section="/dedicatedCloud" method="GET" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/filer/\{filerId\}/task/\{taskId\}"} />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/host/\{hostId\}/remove"} />

Une tâche est créée pour chaque appel, vous pouvez suivre l'avancement avec :

<Api version="v1" section="/dedicatedCloud" method="GET" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/host/\{hostId\}/task/\{taskId\}"} />
...
<Api version="v1" section="/dedicatedCloud" method="DELETE" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}"} />
...
<summary>Quels sont les impacts lors du partage de mes datastores entre mes vDC ?</summary>
...
<summary>Est-ce que les VMs (avec IP publiques) seront accessibles depuis l'extérieur si elles sont dans le nouveau vDC quand les PFSENSE sont dans l'ancien vDC ?</summary>
...
<summary>Est-il possible de mettre en place un PFSENSE dans l'ancien vDC et un autre dans le nouveau vDC ?</summary>
...
<summary>Les vxlan sont-ils disponibles sur les deux vDC ?</summary>
...
<summary>Nous n'utilisons pas NSX. La procédure de migration indique que les vDS source/destination doivent avoir la même version. Sur la source, notre unique vDS est en 6.0.0 donc j'imagine qu'il faut le mettre à jour. La documentation / la video / et l'interface indiquent qu'on peut le faire nous-mêmes sans coupure si c'est du vRack. Je pensais que c'était du vRack mais nous ne pouvons pas mettre à jour (le menu est grisé). Est-ce que ça signifie que c'est du vxlan ? Comment fait-on la différence entre vRack et vxlan ?</summary>
...
<summary>Comment savoir si mes adaptateurs réseau sont VLAN ou VxLAN et compatibles avec Essentials ? Dans vSphere, je vois par exemple et sans plus de détails : vxw-dvs-74-virtualwire-20-sid-....</summary>
...
<summary>Si j'ai plusieurs VM qui passent par le même EDGE NSX, faudra-t-il faire la migration de l'ensemble des VM et du EDGE en même temps, au risque de ne plus avoir de liaison Internet sur certaines VM dans le cas contraire ?</summary>
...
<summary>Est-ce qu'on peut créer un pool DRS pour les datastores globaux ? Je crois avoir déjà essayé sans succès entre 2 vDC 2014 / 2016.</summary>
...
<summary>Nous avons un SDDC 2016 avec 6 x 6 To SSD Acceleraded (commandés en 2021) avec "convert to global" disponible dans l'espace client OVHcloud. Peut-on les convertir en global et les garder en l'état dans le nouveau vDC (pour éviter la phase de storage vMotion) ? Note: les 6 DS sont dans un cluster de stockage.</summary>
...
<summary>Quelles sont les limitations/différences au niveau de la migration selon la gamme choisie (Essentials ou Premier)?</summary>
...
<summary>Combien de temps faut-il prévoir pour cette migration (en fonction du nombre de VM)?</summary>
...
<summary>Quelles sont les licences Microsoft disponibles en mode SPLA ?</summary>
...
<summary>Je dois upgrader 2 infrastructures VMware, actuellement utilisées dans le cadre d'un PRA zerto avec la réplication des données. Est-il nécessaire de faire d'abord un upgrade de mon infrastructure secondaire ou primaire ?</summary>
...
<summary>Le plafond historique sur les ressources horaires sera-t-il toujours déployé ?</summary>
...
<summary>Le prix des anciennes offres va-t-il évoluer?</summary>
...
<summary>Dans quelle langue les Services Professionnels d'OVHcloud sont-ils disponibles ?</summary>
...
<summary>Est-ce que les Services Professionnels d'OVHcloud peuvent recréer mes comptes utilisateurs & configurations NSX pour moi ?</summary>
...
<summary>Quelle est la durée de vie des crédits du Pack of Technical Advice Services ?</summary>
...
<summary>Comment savoir combien d'heures de Crédits ont été utilisées et sont restantes ?</summary>
...
<summary>Que se passe-t-il si la session du service de conseil prend moins de temps que prévu ?</summary>
