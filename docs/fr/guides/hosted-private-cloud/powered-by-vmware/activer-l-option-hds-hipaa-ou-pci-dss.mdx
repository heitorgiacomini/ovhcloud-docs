...
Pour adapter votre Private Cloud à cette certification, l'une des options de sécurité correspondantes doit être activée. Pour le vérifier, connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> et dirigez-vous dans la section `Serveur`. Cliquez sur <code className="action">Private Cloud</code> et sélectionnez le service concerné.
...
- **La politique d'accès à vCenter doit être restreinte**: depuis l'onglet <code className="action">Sécurité</code> , vous pouvez vérifier l'état de la politique de sécurité. S'il n'est pas restreint, effectuez la modification en cliquant sur <code className="action">Modifier la politique d'accès à vCenter</code>, puis suivez les étapes. N'hésitez pas à consulter notre guide [Présentation de l’espace client Private Cloud OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud).

- **Vous devez disposer d'au moins une adresse IP autorisée pour vous connecter à vCenter**: depuis l'onglet <code className="action">Sécurité</code> , assurez-vous d'avoir autorisé au moins une adresse IP. Si nécessaire, cliquez sur <code className="action">Ajouter une nouvelle plage d'adresses IP</code>. N'hésitez pas à consulter notre guide [Présentation de l’espace client Private Cloud OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud).
...
Pour plus de détails, consultez [notre guide dédié au fonctionnement de l'interface sécurisée](/guides/hosted-private-cloud/powered-by-vmware/interface-secure)

## Aller plus loin

[Présentation de l’espace client Private Cloud OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud)
