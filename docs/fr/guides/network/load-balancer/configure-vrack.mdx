...
description: Voici comment configurer le vRack sur un Load Balancer
...
Le vRack est un réseau privé dédié qui crée un lien entre tous vos produits cloud. Pour plus d'informations n'hésitez pas à consulter la page sur le [vRack](https://www.ovhcloud.com/fr/network/vrack/)

Avant de vous lancer, si vous ne l’avez pas encore lue, nous vous conseillons de consulter la [présentation générale du service Load Balancer](/guides/network/load-balancer/use-presentation).
...
Connectez-vous à l'[interface API OVHcloud](https://eu.api.ovh.com/) et suivez les étapes ci-dessous.
...
Pour rappel, un guide de configuration du Load Balancer est disponible : [Configurer un service OVHcloud Load Balancer HTTP/HTTPS](/guides/network/load-balancer/create-http-https).
