...
description: Dirigez dynamiquement vos requêtes vers une ferme spécifique
...
- Disposer d'un [Load Balancer OVHcloud](https://www.ovhcloud.com/fr/network/load-balancer/) sur une offre autorisant la création des routes.
...
La gestion des routes n'est accessible qu'au travers de l'[API OVHcloud](https://eu.api.ovh.com/). Elle n'est valable que pour les protocoles **http** et **tcp**, et le chemin <code className="action">/ipLoadbalancing/&#123;serviceName&#125;/&#123;protocol&#125;/route/</code> expose l'API dédiée aux routes.
...
:::info
Pour afficher uniquement les appels API liés aux routes dans la console de l'API OVHcloud, vous pouvez utiliser le champ <code className="action">filter</code> avec le mot-clé "*[a-z]*".
:::
