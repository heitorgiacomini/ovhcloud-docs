...
description: Intégrez vos services TCP derrière un Load Balancer avec le proxyProtocol
...
Si vous utilisez un frontend HTTP, nous vous recommandons le guide « [Configuration d'un service OVHcloud Load Balancer - Les en-têtes HTTP](/guides/network/load-balancer/create-headers) » qui décrit la manière standard d'utiliser les en-têtes HTTP pour retrouver l'adresse IP, le port ainsi que le protocole source.
...
- **Lien direct :** <ManagerLink to="/#/dedicated/iplb">Load Balancer</ManagerLink>
...
<img className="thumbnail" alt="Adresse IPv4 de sortie de votre service OVHcloud Load Balancer" src="/images/network/load-balancer/create-proxyprotocol/iplb_service.png" loading="lazy" />
...
Dans la section <code className="action">Fermes</code>, sélectionnez la ferme contenant le serveur sur lequel activer le ProxyProtocol puis cliquez sur le bouton d'édition du serveur concerné.

Le ProxyProtocol se configure via l'option <code className="action">Version du ProxyProtocol</code>. Vous y retrouverez les 4 modes décrits ci-dessus.

<img className="thumbnail" alt="Activation du ProxyProtocol sur un serveur d'une ferme" src="/images/network/load-balancer/create-proxyprotocol/edit_server.png" loading="lazy" />

Une fois le mode souhaité sélectionné, cliquez sur <code className="action">Mettre à jour</code> puis sur <code className="action">Déployer la zone: VOTRE ZONE</code> pour appliquer vos changements dans la zone concernée.
...
- Modifier un <code className="action">Serveur</code> existant :
