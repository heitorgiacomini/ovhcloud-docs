...
<img className="thumbnail" alt="Vue d'ensemble — schéma" src="/images/network/load-balancer-revamp/high-availability/fig-1.fr.svg" loading="lazy" />
...
Une zone de disponibilité est un emplacement isolé dans lequel un service Load Balancer peut s'exécuter. La plupart des régions OVHcloud exposent une seule zone ; utiliser plusieurs zones revient donc généralement à utiliser plusieurs régions. OVHcloud déploie également des régions multi-zones de disponibilité (multi-AZ), où plusieurs zones coexistent au sein d'une même région et sont reliées par des liens à faible latence.
...
<img className="thumbnail" alt="Routage Anycast — schéma" src="/images/network/load-balancer-revamp/high-availability/fig-2.fr.svg" loading="lazy" />

:::info
En raison de restrictions techniques, lorsqu'un Load Balancer est configuré avec deux zones dont l'une se trouve dans une région APAC et l'autre non, le trafic est acheminé en priorité par la zone non-APAC, même lorsque le service y est hors service. Ce comportement est spécifique aux configurations intercontinentales impliquant des zones APAC, et OVHcloud ne recommande pas de configurer un Load Balancer de cette manière.
:::

### Sondes et failover

Le failover consiste à retirer une cible non saine de la rotation et à rediriger son trafic ailleurs. Le Load Balancer vérifie en continu l'état de santé de chaque serveur backend d'une ferme à l'aide de sondes configurables. Lorsqu'un serveur échoue à sa sonde, le Load Balancer cesse de lui envoyer de nouvelles requêtes et répartit le trafic sur les serveurs sains restants de la ferme.
...
Les sondes sont essentielles à ce comportement. Sans sondes, le Load Balancer ne peut pas détecter un backend qui ne répond pas et peut continuer à lui envoyer du trafic. Pour les configurer, consultez le guide [Configurer les sondes](/guides/network/load-balancer-revamp/configure-health-probes).

<img className="thumbnail" alt="Sondes et failover — schéma" src="/images/network/load-balancer-revamp/high-availability/fig-3.fr.svg" loading="lazy" />

### Anti-affinité entre zones

L'anti-affinité consiste à placer des composants redondants dans des domaines de défaillance distincts, de sorte qu'une seule panne ne puisse pas en mettre plusieurs hors service. Pour le Load Balancer, cela signifie répartir vos frontends et vos fermes de serveurs sur plusieurs zones plutôt que de les concentrer dans une seule.
...
<img className="thumbnail" alt="Anti-affinité entre zones — schéma" src="/images/network/load-balancer-revamp/high-availability/fig-4.fr.svg" loading="lazy" />
...
### Toujours configurer des sondes

Le failover repose sur les sondes pour détecter les backends qui ne répondent pas. Configurez une sonde sur chaque ferme afin que les serveurs défaillants soient retirés automatiquement de la rotation. Sans sondes, le trafic peut continuer d'atteindre des serveurs qui ne sont plus en mesure de répondre.
...
Pour les workflows d'infrastructure-as-code, le provider Terraform OVHcloud expose des ressources pour le service et ses fermes, telles que `ovh_iploadbalancing`, `ovh_iploadbalancing_http_farm` et `ovh_iploadbalancing_tcp_farm`. Les frontends sont gérés avec `ovh_iploadbalancing_http_frontend`, `ovh_iploadbalancing_tcp_frontend` et `ovh_iploadbalancing_udp_frontend`, et l'opération d'application (refresh) avec `ovh_iploadbalancing_refresh`.
...
- Le comportement du failover dépend de sondes correctement configurées ; des sondes mal configurées peuvent soit ne pas détecter les pannes, soit retirer prématurément des serveurs sains.
