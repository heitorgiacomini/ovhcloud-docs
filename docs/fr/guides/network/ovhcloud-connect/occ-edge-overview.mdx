...
| **Lien double** | Deux boucles locales indépendantes, délivrées par deux fournisseurs différents vers deux PoP différents. | Une connectivité résiliente pour les sites critiques qui doivent survivre à la perte d'un accès. |
...
Un lien simple utilise une seule boucle locale vers un PoP OVHcloud. Un lien double utilise deux boucles locales indépendantes, délivrées par deux fournisseurs différents vers deux PoP différents, de sorte que la panne d'un accès n'interrompt pas la connectivité. Un lien double nécessite que votre site soit éligible auprès de deux fournisseurs distincts.
...
OVHcloud Connect Edge est proposé en deux niveaux de service.

{/* DRAFT: To verify — confirm the SLA percentages and restoration-time commitments per tier before GA. */}

| Niveau | SLA de disponibilité | Garantie de temps de rétablissement (GTR) | Support |
|---|---|---|---|
| **Basic** | 99,9 % | 4 heures, heures ouvrées | Support complet à l'installation, puis support en heures ouvrées. |
| **Advanced** | 99,95 % | 4 heures, 24/7 | Support complet à l'installation, puis support 24/7. |

### Capacités du uCPE

Le uCPE en option fournit les éléments suivants, gérés par OVHcloud :

- Session BGP automatique avec OVHcloud Connect Edge.
- Routage LAN, avec un client DHCP sur le port LAN.
- Un serveur DHCP sur le port LAN.
- Une politique de pare-feu basique (filtrage IP et ports).
- Configuration à distance, logs et supervision.

{/* DRAFT: To verify — some device capabilities (for example static routes and 4G backup) are on the roadmap. Confirm which are available at launch before publishing. */}
...
| Limitation | Détails |
|---|---|
| Géographie | Disponible en France. |
