...
| 2 | **Auto-négociation activée** | L’auto-négociation **n’est pas prise en charge** sur OVHcloud Connect. Désactivez-la sur votre routeur (voir ci-dessous). |
...
Désactivez l’auto-négociation sur l’interface de votre routeur, en suivant la documentation de votre constructeur. La vitesse du port doit être forcée pour correspondre au lien commandé (1 Gbps ou 10 Gbps).
...
| 2 | **ASN incorrect** | Votre ASN et l’ASN côté OVHcloud doivent tous deux être correctement configurés, différer l’un de l’autre et ne pas utiliser les valeurs réservées (**65501** Europe, **65502** Canada, **65519** Asie) ; tout autre ASN est autorisé. |
| 3 | **ID de VLAN incompatible** | Le tag VLAN sur votre interface doit correspondre à l’ID de VLAN configuré dans la configuration du PoP de l’espace client OVHcloud. Vérifiez-le sur votre routeur. |
...
| # | Cause possible | Solution |
|---|---|---|
| 1 | **Instruction `network` ou politique d’export manquante** | Sur votre routeur, assurez-vous d’annoncer les bons préfixes. Consultez [Configurer OCC L3 avec BGP](/guides/network/ovhcloud-connect/l3-bgp). |
...
| # | Cause possible | Solution |
|---|---|---|
| 1 | **Lien physique instable** | Vérifiez les compteurs d’erreurs des interfaces sur votre routeur pour les erreurs CRC, les erreurs d’entrée ou les drops de sortie. Inspectez le SFP, le câble cross-connect et le panneau de brassage. |
| 2 | **MTU incompatible** | Une incompatibilité de MTU entre votre équipement et OVHcloud peut entraîner la perte des grands paquets de mise à jour BGP. Les jumbo frames jusqu’à **9000 octets** sont pris en charge ; assurez-vous que le MTU est cohérent des deux côtés. |
| 3 | **Expiration du hold timer BGP** | Si des keepalives sont perdus à cause de l’instabilité du lien, la session BGP tombe à l’expiration du hold timer. Envisagez d’ajuster les timers BGP, mais corrigez d’abord la cause racine. |
...
| 3 | **Erreurs d’interface (CRC, FCS, drops)** | Vérifiez les compteurs d’erreurs des interfaces sur votre routeur. Les problèmes physiques (fibre encrassée, SFP défectueux, connecteur mal serré) provoquent des retransmissions et dégradent les performances. |
...
| 5 | **Congestion chez le fournisseur** | Si vous utilisez OVHcloud Connect Provider, vérifiez le portail de votre fournisseur pour l’utilisation et les alertes sur le circuit virtuel ou la connexion. |
...
Vous avez commandé un service OVHcloud Connect Provider, mais la connexion via votre fournisseur ne monte pas.

### Causes possibles et solutions

| # | Cause possible | Solution |
|---|---|---|
| 1 | **Clé de service incorrecte** | Vérifiez que la clé de service saisie dans le portail de votre fournisseur correspond exactement à la clé issue de l’espace client OVHcloud. |
| 2 | **Circuit virtuel du fournisseur non provisionné** | Vérifiez le statut du circuit virtuel ou de la connexion vers OVHcloud dans le portail de votre fournisseur. Il doit être affiché « Active » ou « Provisioned ». |
...
:::tip
**Astuce :** si le portail du fournisseur affiche la connexion comme « Active » mais qu’OVHcloud l’affiche encore comme « Pending », attendez quelques minutes que le provisionnement se termine. Si le statut ne se met pas à jour après un délai raisonnable, contactez le support OVHcloud.
:::
...
| 3 | **Circuit du fournisseur non encore relié (Provider)** | Assurez-vous que le circuit virtuel du fournisseur est provisionné et relié au service OVHcloud à l’aide de la clé de service correcte. |
...
| 3 | **BFD ne détecte pas la panne** | **BFD (Bidirectional Forwarding Detection)** est activé par défaut sur toutes les sessions BGP et réduit le temps de bascule à moins d’une seconde. Assurez-vous que BFD est également activé et correctement configuré sur votre propre routeur afin que les deux côtés puissent détecter la panne. |
...
Sur votre routeur, vérifiez que les deux sessions BGP sont à l’état `Established` et que les routes sont reçues des deux pairs avec des attributs différents (Local Preference, longueur de l’AS-path).
...
| 3 | **ASN en doublon** | Votre ASN et l’ASN côté OVHcloud doivent tous deux être correctement configurés, différer l’un de l’autre et ne pas utiliser les valeurs réservées (65501 Europe, 65502 Canada, 65519 Asie) ; tout autre ASN est autorisé. |
...
2. Créez une nouvelle demande de support et sélectionnez votre service **OVHcloud Connect** comme service concerné.
3. Incluez :
