...
description: Découvrez comment ajouter un certificat SSL sur le CDN
lastUpdated: 2018-02-22
---

## Objectif

Il est possible d'ajouter un [certificat SSL](https://www.ovhcloud.com/fr/web-hosting/options/ssl/) à votre offre Content Delivery Network (CDN) afin que vos utilisateurs puissent effectuer des connexions sécurisées, même en passant par le CDN.

**Dans ce guide, nous allons vous détailler le fonctionnement du SSL Let's Encrypt fourni par OVH.**

## Prérequis

- Posséder l'[offre CDN d'OVH](https://www.ovhcloud.com/fr/network/cdn/).
...
<img className="thumbnail" alt="Certificat SSL en cours" src="/images/network/content-delivery-network-infrastructure/ssl-certificate/ssl_in_progress.png" loading="lazy" />
...
<img className="thumbnail" alt="SSL validé" src="/images/network/content-delivery-network-infrastructure/ssl-certificate/ssl_validated.png" loading="lazy" />

### Ajouter son propre certificat

- Si vous n'avez pas encore ajouté de nom de domaine ou que vous ne possédez aucun certificat, vous pouvez utiliser l'option <code className="action">Ajouter mon certificat</code> depuis l'onglet SSL du votre CDN :

<img className="thumbnail" alt="Ajout d'un certificat SSL" src="/images/network/content-delivery-network-infrastructure/ssl-certificate/add_ssl.png" loading="lazy" />

- Si vous possédez déjà un certificat Let's Encrypt, vous pouvez simplement utiliser l'option <code className="action">Remplacer par mon certificat</code> :

<img className="thumbnail" alt="Changement d'un certificat SSL" src="/images/network/content-delivery-network-infrastructure/ssl-certificate/change_ssl.png" loading="lazy" />
