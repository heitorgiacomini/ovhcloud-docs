...
description: Create your account and your first stream, then send your first logs to Logs Data Platform.
...
- Open the <ManagerLink to="/#/dedicated/dbaas/logs">Logs Data Platform</ManagerLink> page in the OVHcloud Control Panel and select your platform.

To configure your account you will have two choices:

- Use a username/password pair
- Use OVHcloud IAM to enable [IAM identities](/guides/account-and-service-management/account-information/ovhcloud-users-management) and [IAM policies](/guides/manage-and-operate/observability/logs-data-platform/overview).
...
<img className="thumbnail" alt="Main interface" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/manager_start.png" loading="lazy" />
...
3. List your number of PGP encryption keys for [cold storage archive](/guides/manage-and-operate/observability/logs-data-platform/archive-cold-storage) and edit them. More on that in the [dedicated documentation](/guides/manage-and-operate/observability/logs-data-platform/cold-storage-encryption).
...
- First, create a stream in order to get your token using: <code className="action">Add data stream</code> in the data stream panel. You will be redirected to a page where one can add a name and a description for the stream:

<img className="thumbnail" alt="new Stream" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/newStream.png" loading="lazy" />
...
- Once you have done this, click on the blue button <code className="action">Save</code> and that's it! You have created your first stream. The button will redirect you to the stream page where you will be able to copy the X-OVH-TOKEN token. This value is the only token you will need to route logs to your stream. Under this token, you will have a list of your created streams.

<img className="thumbnail" alt="new Stream" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/streamMenu.png" loading="lazy" />
...
- **Monitor in real-time** allows you to see the logs incoming into your stream in real-time. Note that Graylog also provides this functionality. On this page you can also **Test** different log formats from your computer to your stream. [More about](/guides/manage-and-operate/observability/logs-data-platform/ldp-tail)
- **Manage alerts** allows you to define your alert conditions on the logs routed to the stream. [More about](/guides/manage-and-operate/observability/logs-data-platform/alerting)
- **Archives** allows you to download the cold stored archives. [More about](/guides/manage-and-operate/observability/logs-data-platform/archive-cold-storage)
...
- **Beats**: A secure and reliable protocol used by the beats family in the Elasticsearch ecosystem (Ex: [Filebeat](/guides/manage-and-operate/observability/logs-data-platform/filebeat-logs), [Metricbeat](https://www.elastic.co/beats/metricbeat), [Winlogbeat](https://www.elastic.co/beats/winlogbeat)).
...
<img className="thumbnail" alt="About page" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/about.png" loading="lazy" />
...
- To see your logs in Graylog, click on the Menu button <code className="action">...</code> located at the right of your stream in the stream list. Click on the **Graylog access** link to jump straight to Graylog.

<img className="thumbnail" alt="Graylog access" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/graylog-access.png" loading="lazy" />

The Graylog login page looks like this:

<img className="thumbnail" alt="login Graylog" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/login.png" loading="lazy" />
...
<img className="thumbnail" alt="Graylog Stream" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/graylog-stream.png" loading="lazy" />
...
Press <code className="action">Enter</code> or click on the Green button to launch the search and wait for your results.
...
Let's go back to the Logs Data Platform control panel, we will now create a Dashboard that will allow you to explore your data in a graphical manner. It is even simpler to create a Dashboard, just click on the <code className="action">Add a dashboard</code> button and on the next page, add a description and a title for your Dashboard. Once created, you can use the <code className="action">...</code> menu to access it immediately.

<img className="thumbnail" alt="Dashboard access" src="/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/dashboard-access.png" loading="lazy" />

Your dashboard is empty at first; we will now add some widgets to it. To do that, get back to your stream: you can use the link on graylog (under the Stream tab) or the link on your console as you wish. Let's say you want all the user Ids for which the value `some_metric_num` is above `30`, first. You search for this data:
...
You can <code className="action">edit</code> the widget by using its top right menu arrow. For example you can change the visualization type by choosing **Pie Chart** in the top left _Visualization Type_ option.

![Quick Values](/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/quick-values.png)

This widget gives you the most frequent `user_id` in the logs of the last hour that have a `some_metric_num` value above `30`.

- To add this really critical information to your dashboard, click on the <code className="action">Copy to Dashboard</code> menu button and select your newly created Dashboard. After that, you will be redirected to the Dashboard with your newly created widget in it.

![critical Dashboard](/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/critical-dashboard.png)
...
![widget creation](/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/widget-create.png)
...
![critical Dashboard 2](/images/manage-and-operate/observability/logs-data-platform/getting-started-quick-start/critical-dashboard-2.png)

Note that dashboards are interactive and you can use the top search bar and the date picker to display only the widgets matching a particular type of event.

## Go further

This guide covers only part of what Logs Data Platform can do. You will also find how to:

- [Send correctly formatted logs](/guides/manage-and-operate/observability/logs-data-platform/field-naming-conventions) to use custom types as number, boolean and other stuff.
- [Configure your syslog-ng](/guides/manage-and-operate/observability/logs-data-platform/log-linux) to send your Linux logs to Logs Data Platform.
- [Manage IAM Policies](/guides/manage-and-operate/observability/logs-data-platform/overview) to allow other users of the platform to view your dashboards or explore your streams themselves, rather than doing it for them.
- [Using OpenSearch Dashboards and aliases to exploit the power of OpenSearch](/guides/manage-and-operate/observability/logs-data-platform/opensearch-dashboards)
