...
description: Use the power of OpenSearch without managing a cluster.
...
- You have created a [Logs Data Platform account](/guides/manage-and-operate/observability/logs-data-platform/getting-started-quick-start)
...
To create an OpenSearch index with the Logs Data Platform manager, you need to go to the index page and click on the <code className="action">Add a new index</code> on the OpenSearch index section

<img className="thumbnail" alt="add index option" src="/images/manage-and-operate/observability/logs-data-platform/opensearch-index/add_index.png" loading="lazy" />
...
If you have enabled IAM on your account you first need to create a policy and give the creation rights to your preferred user/account. See the [IAM for LDP documentation policy guide](/guides/manage-and-operate/observability/logs-data-platform/iam-access-management).
...
To interact with the OpenSearch API, you can either create [service accounts](/guides/account-and-service-management/account-information/authenticate-api-with-service-account) and leverage the OpenID workflow, or create [local users](/guides/account-and-service-management/account-information/ovhcloud-users-management) with their Personal Access Token with the API:
...
If you don't know how to create a Logstash collector, please refer to the [Logstash guide](/guides/manage-and-operate/observability/logs-data-platform/logstash-input). Edit the configuration of Logstash. For this example we will use a SSL TCP input with the GELF codec. Here is the input configuration.
...
- **username**: This is the username to authenticate yourself against the API. Again, we recommend that you use [tokens](/guides/manage-and-operate/observability/logs-data-platform/tokens) for that.
...
- **fields**: This is where the magic happens. The field of the document found will be added to the event. The field of the document is on the left and the new (or updated) field of the event is on the right. Be sure to follow the [field naming conventions](/guides/manage-and-operate/observability/logs-data-platform/field-naming-conventions).
...
<img className="thumbnail" alt="Log in the stream" src="/images/manage-and-operate/observability/logs-data-platform/opensearch-index/stream_1.png" loading="lazy" />
...
<img className="thumbnail" alt="A song of Ice And Fire" src="/images/manage-and-operate/observability/logs-data-platform/opensearch-index/dashboard.png" loading="lazy" />
...
On Logs Data Platform, we allow users to use OpenSearch API to handle the lifecycle of their indices. You can create and delete indices directly with the OpenSearch API. You can also create aliases and delete them. We even support templates to allow users to create their mapping a the creation of the index automatically!
