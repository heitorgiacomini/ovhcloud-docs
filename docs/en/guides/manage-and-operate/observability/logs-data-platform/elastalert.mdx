...
1. In the <code className="action">Alias</code> tab, click the <code className="action">Add an alias</code> button.
...
4. Once the alias has been created. Use the **...** menu on the right side and select the <code className="action">Attach content to the alias</code> option.
5. Define there the data streams you want to associate to your alias.
6. The alias is now ready to use.

<img className="thumbnail" alt="Alias creation" src="/images/manage-and-operate/observability/logs-data-platform/alerting-elastalert/alias.png" loading="lazy" />

If you only have [indices](/guides/manage-and-operate/observability/logs-data-platform/index-as-a-service), you can use them directly in the ElastAlert configuration.
...
- `<password>` is the associated password. You can use [tokens](/guides/manage-and-operate/observability/logs-data-platform/tokens) in place of the username/password couple for your credentials.
...
Here is a sample **config.yml** file you can use for your configuration in your configuration directory:
...
To test your alert you can use the following curl command sending logs to our [OpenSearch endpoint](/guides/manage-and-operate/observability/logs-data-platform/ldp-index):
