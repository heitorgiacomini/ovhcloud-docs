...
description: This super-powered tail will follow and format your logs with flying colors.
...
- A LDP account with [an active stream](/guides/manage-and-operate/observability/logs-data-platform/getting-started-quick-start) and WebSocket option enabled
...
Let's retrieve the WebSocket address that will allow you to follow your logs. For this you will need first to connect to the manager and go to the streams page. From there, open the menu of the stream you want the address of and click on <code className="action">Monitor in real-time</code>

<img className="thumbnail" alt="Follow" src="/images/manage-and-operate/observability/logs-data-platform/cli-ldp-tail/follow.png" loading="lazy" />

You will land on a new page where you will see all your logs in real-time as soon as they arrive. On this page click on the button <code className="action">Copy WebSocket address</code> to copy the WebSocket address in your clipboard. Just use the address as in the example and you will see your logs flowing in your terminal right away.
...
**ldp-tail** is not just a plain tail (as its name suggests). It comes with advanced formatting and filtering capabilities. The full documentation of these capabilities are all available at the [github website](https://github.com/ovh/ldp-tail#parameters). Here are the two main options that you can use to enhance your output.
...
You can of course combine multiple matches by issuing **ldp-tail --match `<COND_1>` --match `<COND_2>` --pattern ...**. A pattern and match combination can be really difficult to read, that is why ldp-tail supports a TOML configuration file.
