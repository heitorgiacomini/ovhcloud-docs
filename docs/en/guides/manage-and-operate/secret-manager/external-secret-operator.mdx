...
description: Configure External Secret Operator to store Kubernetes secrets on the OVHcloud Secret Manager
...
- A [Managed Kubernetes Service](https://www.ovhcloud.com/en-gb/public-cloud/kubernetes/) cluster.
...
Create an [IAM local user](/guides/account-and-service-management/account-information/ovhcloud-users-management) with access rights on your domain.
...
You will also need the `region` and the `okms-id` of the OKMS domain you want to use. This ID and this region can be found in the <ManagerLink to="/#/dedicated/security/kms">OKMS domain dashboard</ManagerLink> or via the [OVHcloud CLI](https://github.com/ovh/ovhcloud-cli):
...
For any additional information on how to manage the External Secret Operator, refer to the dedicated documentation, using the HashiCorp Vault provider: [https://external-secrets.io/latest/](https://external-secrets.io/latest/).
