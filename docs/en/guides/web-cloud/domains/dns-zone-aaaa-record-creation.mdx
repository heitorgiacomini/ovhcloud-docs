...
description: Find out how to add an AAAA record to an OVHcloud DNS zone for your domain name
...
:::info
To modify or delete an AAAA record of an OVHcloud DNS zone, follow [this guide](/guides/web-cloud/domains/dns-zone-edit).
...
<Tabs>
  ...
  <Tab label="Step 3">
    In the window that opens, select the <code className="action">AAAA</code> record type.
  </Tab>
  <Tab label="Step 4">
    Enter the IP address (e.g.: `2001:db8:1:1b00:203:0:113:0`) of the service your website is hosted on (web hosting plan, dedicated server, VPS, etc.) in the `Target *` field, then click <code className="action">Next</code>.
  </Tab>
  <Tab label="Step 5">
...
<Tabs>
  ...
  <Tab label="Step 4">
    Enter the subdomain in the `Sub-domain` field (e.g.: `www` for the subdomain `www.domain.tld`), and the IP address (e.g.: `2001:db8:1:1b00:203:0:113:0`) of the service your website is hosted on (web hosting plan, dedicated server, VPS, etc.) in the `Target *` field. Click <code className="action">Next</code>.
  </Tab>
  <Tab label="Step 5">
    Check the summary, then click <code className="action">Confirm</code>. It takes up to **24** hours for the change to propagate fully over the DNS network.
  </Tab>
