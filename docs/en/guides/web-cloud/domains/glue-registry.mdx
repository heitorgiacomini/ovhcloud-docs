...
To do this, go to [https://zonemaster.net](https://zonemaster.net/en), enter your domain name without *www* (example: *domain.tld*), then tick the <code className="action">Options</code> button located just below the domain name entry form.

In the available options, click on the <code className="action">Fetch NS from parent zone</code> button.
...
If you need more information, please read our tutorial on the [Zonemaster](/guides/web-cloud/domains/dns-zonemaster) tool.
...
<summary>Your domain name does not use an active DNS zone at OVHcloud</summary>
...
<summary>Your domain name uses an active DNS zone at OVHcloud</summary>
...
<Tabs>
  ...
  <Tab label="Step 4">
    Enter the *subdomain* and the *IPv4* (A) or *IPv6* (AAAA) address, then proceed until the addition has been validated. If necessary, refer to the instructions in our guide "[How to edit an OVHcloud DNS zone](/guides/web-cloud/domains/dns-zone-edit)".
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab label="Step 3">
    In this window, replace **only in the *NS* type records** the DNS server names with your own custom DNS server names, **making sure** to increment the first numeric value of the *SOA* line by "1". Once you have made your changes, click <code className="action">Next</code> then <code className="action">Confirm</code>.
...
If necessary, refer to the instructions in our guide "[How to edit an OVHcloud DNS zone](/guides/web-cloud/domains/dns-zone-edit)".
...
<Tabs>
  <Tab label="Step 1">
    Go to the <ManagerLink to="/#/web-domains/domain">Domain names</ManagerLink> page, then choose *the domain name for which you want to customise the DNS servers*.
  ...
  <Tab label="Step 4">
    Finalise the steps and, if necessary, refer to the instructions in our guide "[Modifying an OVHcloud domain name's DNS servers](/guides/web-cloud/domains/dns-server-edit)".
