...
You will need the cluster name of your web hosting plan which you can find in the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, as well as the FTP username and password. Consult [the corresponding guide](/guides/web-cloud/web-hosting/ftp-connection) if you need further information on this access method.
...
You will need the IP address of your server which you can find in the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> and the name of the user account you want to use for this login session. Consult our "Getting started" guides if you require further details on this topic:
...
:::info
PuTTY stores key files in a specific format which makes them incompatible with SSH key files created with the **OpenSSH** client. If you want to use a **private key** previously created with the command line SSH client (e.g. for a [server](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys) or a [Public Cloud instance](/guides/public-cloud/compute/creating-ssh-keys)), you will need to [convert it to the PuTTY format](https://www.chiark.greenend.org.uk/~sgtatham/putty/faq.html#faq-ssh2-keyfmt) first.
...
- [How to replace an SSH key pair on a Public Cloud instance](/guides/public-cloud/compute/replacing-lost-ssh-key-pair)
...
[How to create SSH keys with OpenSSH](/guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys)
