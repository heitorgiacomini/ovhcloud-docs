...
description: Find out how to log in to your OVHcloud Web Hosting plan’s storage space, and manage the data stored on it, using FileZilla software
...
You can use it to put files or your website online by [connecting to your web hosting plan’s storage space](/guides/web-cloud/web-hosting/ftp-connection).
...
<summary>Click here to see the contents of this section.</summary>
...
3. On the page that pops up, click on the <code className="action">FTP - SSH</code> tab.
...
:::info
For security reasons, a user’s password does not appear on the page of the <code className="action">FTP - SSH</code> tab. If you have forgotten it, please refer to [this guide](/guides/web-cloud/web-hosting/ftp-change-password) to modify it.
...
<summary>Log in to your Web Hosting plan’s storage space via SFTP using FileZilla. <a name="sftp"></a></summary>


The **SFTP** uses, like SSH, port 22 by default instead of port 21. If you are using a Cloud Web hosting plan, you will need to use the port that appears in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. Port 22 is disabled in SSH and SFTP for Cloud Web hosting.
...
To do this, go back to the <code className="action">FTP - SSH</code> tab in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, as detailed in the [first part](#part-1) of this guide.
...
- For Professional and Performance plans:
    ...
    - 1: Click the <code className="action">...</code> button to the right of the line corresponding to the user, then <code className="action">Edit</code>.
    - 2: In the window that pops up, in the `Connection protocols` section, select the <code className="action">FTP and SFTP</code> choice, then click <code className="action">Next</code>.
    - 3: Check the summary of the requested modification, then click <code className="action">Confirm</code>.
...
Once you have entered everything correctly in the **1** box in the image below, click <code className="action">Quick Connect</code>.
...
<summary>Log in to your Web Hosting plan’s storage space via FTP using FileZilla.</summary>
...
Check your login details to ensure that no errors are entered. If applicable, you can [change the FTP access password](/guides/web-cloud/web-hosting/ftp-change-password) of your web hosting plan directly in the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>.
...
Check that it is linked to the host name declared in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>.
...
<summary>Server context menu</summary>
...
<summary>File and folder access rights (CHMOD)</summary>


Right-click on one of the files on the server, then select <code className="action">File permissions...</code>.
...
<summary>Site reopening</summary>
...
You must then [secure your services](/guides/web-cloud/web-hosting/diagnostic-403-forbidden#step-2) while correcting the security vulnerabilities mentioned in the notification received via email.

:::


Then click <code className="action">Server</code> and select <code className="action">Enter custom command</code>. (This option may also be called <code className="action">Enter FTP command</code>).
...
<summary>Binary file transfer</summary>


For binary type files, such as **CGI** files, it may be of interest to choose how the transfer will be done.

To change the transfer type, select <code className="action">Transfer</code> from the main menu, then <code className="action">Transfer type</code>.
...
<summary>Folder comparison</summary>
...
Consult our documentation for [resolving recurring errors when using FTP software](/guides/web-cloud/web-hosting/ftp-recurring-ftp-problems).

Generally, find all our [guides related to shared hosting here](/guides/web-cloud/web-hosting/overview).
