...
When you do a security backup for your site and you are using a database, also create a backup of your database. Please refer to our guide on [retrieving the backup of a Web Hosting plan’s database](/guides/web-cloud/web-hosting/sql-database-export).
...
- Depending on which method you use, you must be able to manage the Web Hosting plan from the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, or have the FTP user password required to access your storage space. 
...
If the space in use is more than half of the total FTP storage space of your hosting plan, you can free up space by removing large files from your website (you can use [Filezilla](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide)).
...
To determine whether your website has been placed in "maintenance" mode, please refer to our guide [What do I do if I have a 403 forbidden page?](/guides/web-cloud/web-hosting/diagnostic-403-forbidden). 

In this case:

- Our teams send an email to the hosting [administrator contact](/guides/account-and-service-management/account-information/managing-contacts#access-contact-management).
- The “maintenance” status will appear in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. In the <code className="action">Web Cloud</code> section, click on your service in the <code className="action">Hosting plans</code> section, then on the <code className="action">General information</code> tab.
...
<Tabs>
  ...
  <Tab label="Step 5">
    ...
    Once you have selected a date, click <code className="action">Next</code>. 
  </Tab>
  <Tab label="Step 6">
...
Take a few minutes to check that none of your files will be lost after the restoration, e.g. any files saved on your storage space after the restore date you have selected. As a reminder, the restoration will effectively overwrite all of your current data, and replace it with the backup data.
    ...
    Once you are ready to start restoring the backup, click <code className="action">Confirm</code>.
  </Tab>
</Tabs>
...
You can find this information in the <code className="action">FTP - SSH</code> tab of your Web Hosting plan.

<img className="thumbnail" alt="FTP- SSH" src="/images/assets/screens/control-panel/product-selection/web-cloud/web-hosting/ftp-ssh.png" loading="lazy" />

If you no longer have the FTP user password, please refer to our guide on instructions set out in our documentation on [Modifying a FTP user password](/guides/web-cloud/web-hosting/ftp-change-password).
...
[Using FileZilla software with your Web Hosting plan](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide)
