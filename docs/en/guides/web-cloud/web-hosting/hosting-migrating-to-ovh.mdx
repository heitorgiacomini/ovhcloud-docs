...
description: Find out how to migrate your website, domain name, database and emails to OVHcloud without any service interruptions
...
Several [OVHcloud web hosting plans](https://www.ovhcloud.com/en-gb/web-hosting/) contain an [MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities) email solution. With this email offer, you can create email accounts with a maximum storage space of 5 GB each. Choose from the following hosting plans, considering the PHP version, SQL version, number of email accounts you need, and the size of your website to migrate:

**Eco** range:

- **Starter** — 2 MX Plan email accounts
- **Personal** — 10 MX Plan email accounts

**Business** range:

- **Startup** — 10 MX Plan email accounts
- **Professional** — 100 MX Plan email accounts
- **Performance** — 1,000 MX Plan email accounts

**Agencies** range:

- **Agency**, **Agency Plus** and **Agency Max** — 1,000 MX Plan email accounts each

Once you have chosen your hosting plan, click the <code className="action">Order</code> button on the commercial page of your chosen plan. Follow the steps for the **order without requesting the transfer of your domain name**. (This action will be performed in part 10 of this guide.)
...
<Tabs>
  <Tab label="Step 1">
    Go to the <ManagerLink to="/#/web/hosting">Hosting plans</ManagerLink> page.
  ...
  <Tab label="Step 2">
    At the top left of the interface, click the <code className="action">Order</code> button, then <code className="action">Hosting plans</code>.
  </Tab>
  <Tab label="Step 3">
...
<Region zones={['ca']}>
:::info
OVHcloud offers other email services in addition to the MX Plan solution. For example, you can combine email accounts of the [Exchange offer](https://www.ovhcloud.com/en-gb/emails/hosted-exchange/) with MX Plan email accounts.
:::
</Region>
...
After your hosting service is installed, log in to your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> to create a DNS zone for your domain name. Do not use "**www**" when doing this. You can refer to our guide on [Creating a DNS zone at OVHcloud](/guides/web-cloud/domains/dns-zone-create).

Once you have created a DNS zone, you can manage it using our guide on [Editing an OVHcloud DNS zone](/guides/web-cloud/domains/dns-zone-edit).
...
To retrieve the correct target IP address for your OVHcloud hosting plan, please refer to our guide listing the [IP addresses of the different shared hosting clusters](/guides/web-cloud/web-hosting/clusters-and-shared-hosting-ip).
...
{/* TODO zone-adapt (CA/APAC): produce a localized screenshot `dashboard-mx-a-cname-ca.png` showing `.ovh.ca` nameservers (dnsXX/nsXX.ovh.ca) and `mx*.mail.ovh.ca`, then wrap it in <Region zones={['ca','apac']}>. The EU screenshot above is gated so CA/APAC readers are not shown `.ovh.net`. */}
...
<Region zones={['ca', 'apac']}>
:::tip
Note the two target values with the record type "NS", `dnsXX.ovh.ca` and `nsXX.ovh.ca` (or `dns200.anycast.me` and `ns200.anycast.me`). They correspond to the DNS servers associated with this DNS zone for your domain name. They will be used in [part 9](#step9) of this guide.
:::
</Region>
...
To import the backup of your old service provider's FTP storage space, [log in to the FTP storage space of your OVHcloud hosting](/guides/web-cloud/web-hosting/ftp-connection) and upload the backup to the root folder 'www' (or another root folder you have already created).

We recommend that you use [FileZilla](/guides/web-cloud/web-hosting/ftp-filezilla-user-guide) to upload your FTP backup to your hosting.
...
[Create a new database](/guides/web-cloud/web-hosting/sql-create-database) for your hosting, then [import your backup into your new database](/guides/web-cloud/web-hosting/sql-importing-mysql-database).

:::info
OVHcloud offers Web Cloud Databases database servers. If you would like to use this solution with your website, you can find all of our documentation for this product on our dedicated page: [Web Cloud Databases](/guides/web-cloud/databases/overview).
:::
...
:::tip
If you are using a Content Management System (CMS) like WordPress, Joomla!, Drupal or PrestaShop, you can find the information in their configuration files. See **part 2** of the guide "[Changing a database password](/guides/web-cloud/web-hosting/sql-change-password)".
:::

Declare and authorise your external domain name on your OVHcloud web hosting via our guide [Managing multiple websites on an OVHcloud web hosting plan](/guides/web-cloud/web-hosting/multisites-configure-multisite). Enter the name of the folder you chose at the beginning of [part 4](#step4) as the Multisite "Root folder". As a reminder, this is the folder in which you have placed your files in your FTP storage space.
...
{/* TODO zone-adapt (CA/APAC): produce a localized screenshot `dashboard-a-txt-cname-ca.png` showing `.ovh.ca` nameservers (dnsXX/nsXX.ovh.ca), then wrap it in <Region zones={['ca','apac']}>. The EU screenshot above is gated so CA/APAC readers are not shown `.ovh.net`. */}
...
Create new email accounts and name them according to your current email addresses hosted by your email provider. Use our guide on [Creating MX Plan email addresses](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).
...
- [For Email Pro](/guides/web-cloud/email-and-collaborative-solutions/email-pro/first-config)
- [For Exchange](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/exchange-starting-hosted)
</Region>

<Region zones={['ca']}>
If you have opted for an Exchange solution, please read our documentation on this topic to create your email accounts:
...
**Option 1**: Use our [OVHcloud Mail Migrator (OMM)](https://omm.ovhcloud.com/) tool, which allows you to copy the content of email accounts at your old service provider to your new OVHcloud accounts. You can refer to our guide [Migrating email accounts using OVHcloud Mail Migrator](/guides/web-cloud/email-and-collaborative-solutions/migrating/omm-migrate-email-account-to-ovhcloud).

We recommend that you do not use the <code className="action">Server type</code> **POP** in the <code className="action">Source account</code> section. This protocol deletes emails from your old server, and sends them to the destination OVHcloud server. You will no longer be able to compare the content of the old accounts with the new email accounts.

As for <code className="action">Destination account</code>, enter only the relevant OVHcloud email address and password. Keep the <code className="action">Server type</code> in <code className="action">Hosted by OVH (Autodetect)</code>.
...
**Option 2**: Back up the content of your email accounts using an email client (Outlook, Mac Mail, etc.), reconfigure your email software, then import the backup into your new OVHcloud email accounts. You can find more information in our guide "[Migrating your email address manually](/guides/web-cloud/email-and-collaborative-solutions/migrating/manual-email-migration)".
...
- You can find all of the configuration settings in our guide "[Getting started with the MX Plan solution](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities#consult-emails-client)". You will also find links to custom configuration guides for major email software.

<Region zones={['eu']}>
#### For Email Pro accounts:

- You can find all of our configuration guides in the section `Email client configuration` of [our documentation on the Email Pro offer](/guides/web-cloud/email-and-collaborative-solutions/email-pro/overview).
</Region>

<Region zones={['eu', 'ca']}>
#### For Exchange email accounts:

- You can find all of our configuration guides in the `Exchange configuration on computer` and `Exchange configuration on smartphone` sections of [our Exchange documentation](/guides/web-cloud/email-and-collaborative-solutions/microsoft-exchange/overview).
</Region>
...
<Region zones={['ca', 'apac']}>
Replace the current DNS servers (of the original registrar) with the two DNS servers declared in the OVHcloud DNS zone (format `dnsXX.ovh.ca` and `nsXX.ovh.ca` or `dns200.anycast.me` and `ns200.anycast.me`). You can do this in the management interface of the original registrar.
</Region>
...
Then transfer your domain name using our guide on [transferring a domain name to OVHcloud](/guides/web-cloud/domains/transfer-incoming-generic-domain).
...
[General information on DNS servers](/guides/web-cloud/domains/dns-server-general-information)
...
[Importing a MySQL database](/guides/web-cloud/web-hosting/sql-importing-mysql-database)
