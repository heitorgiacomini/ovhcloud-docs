...
- Être connecté à <ManagerLink to="/">l'espace client OVHcloud</ManagerLink>, partie `Télécom` puis `SMS`.
...
Vous pouvez également acheter un pack de crédits SMS depuis votre [espace client OVHcloud](/links/manager-telecom), rubrique `Télécom` puis section `SMS`. Cliquez sur votre compte SMS puis sur `Créditer mon compte` depuis l'onglet `Accueil`.
:::

### Commander un numéro Time2Chat

Connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink> puis sélectionnez `Télécom`. Cliquez ensuite sur `SMS` et sur votre compte SMS. Dans l'onglet `Accueil`, cliquez sur le bouton `Commander un numéro Time2Chat`.

![time2chat](/images/guides/web-cloud/messaging/sms/time-2-chat/time2chat_order_number_button.png)

Dans le formulaire qui s'affiche, remplissez tous les champs puis cliquez sur `Continuer`.

![time2chat](/images/guides/web-cloud/messaging/sms/time-2-chat/time2chat_order_number_form.png)
...
Une fois votre numéro Time2Chat commandé, cliquez sur l'onglet `Expéditeurs` de votre compte SMS. Le numéro Time2Chat apparaît automatiquement dans la liste des expéditeurs avec le type « Numéro virtuel ». Pour le configurer, cliquez à droite du numéro sur le bouton `...` puis sélectionnez `Configurer`. Vous accédez alors à la page de configuration du SMS conversationnel Time2Chat.

![time2chat](/images/guides/web-cloud/messaging/sms/time-2-chat/time2chat_configuration_number.png)
