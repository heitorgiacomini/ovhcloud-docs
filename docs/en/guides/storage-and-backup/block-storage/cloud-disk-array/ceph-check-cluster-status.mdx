...
description: This guide shows you how to check your Ceph cluster status and health.
...
## Using web interface
First, connect to the [Cloud Disk Array manager](https://www.ovh.com/manager/cloud/index.html). Under 'Platforms and services' select your Ceph cluster. You will see two different parts: **cluster details** and **cluster health**.

### Cluster details

<img className="thumbnail" alt="Cluster details" src="/images/storage-and-backup/block-storage/cloud-disk-array/ceph-check-cluster-status/check_cluster_status_1.png" loading="lazy" />

Additional explanations:

| Field | Description |
|-------|-------------|
| **Label** | Name given to your cluster |
...
<img className="thumbnail" alt="Cluster health" src="/images/storage-and-backup/block-storage/cloud-disk-array/ceph-check-cluster-status/check_cluster_status_2.png" loading="lazy" />

Additional explanations:

| Field | Description |
|-------|-------------|
| **Status** | HEALTH_OK means that every object is at the right place and have 3 replicates. HEALTH_WARN means that some objects are misplaced (like after a growth) or that some object are not replicated three times. In this case cluster is recovering |
| **Healthy** | Boolean indicating if everything is ok with Ceph cluster or not |
| **Available bytes** | Free bytes available in the cluster (before replication) |
