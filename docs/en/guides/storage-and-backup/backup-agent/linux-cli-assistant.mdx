...
description: Find out how to use the ovh-ba-install.sh script provided by OVHcloud to install and manage the Veeam backup agent on your Linux server
...
| Item | Detail |
|--------|--------|
| **OS** | Linux **compatible** with Veeam Agent for Linux (see the [Veeam system requirements](https://helpcenter.veeam.com/docs/agentforlinux/userguide/system_requirements.html?ver=13) and [Backup Agent restrictions](/guides/storage-and-backup/backup-agent/restrictions)). |
...
In the OVHcloud Control Panel, open your <ManagerLink to="/#/bmc-backup-agent-baremetal/service">Backup Agent</ManagerLink>, go to the <code className="action">Agents</code> tab, then click the <code className="action">Download</code> button. In the window that opens, select **Linux** to display the command containing both URLs (script and Linux package).

<img className="thumbnail" alt="Backup Agent download — Linux installation commands" src="/images/storage-and-backup/backup-agent/backup-agent-linux-cli-assistant/01-backup-agent-download-linux-en.png" loading="lazy" />
...
For advanced diagnostics, see our [Backup Agent troubleshooting guide](/guides/storage-and-backup/backup-agent/troubleshooting).
...
- [Configure your first backup](/guides/storage-and-backup/backup-agent/first-configuration)
