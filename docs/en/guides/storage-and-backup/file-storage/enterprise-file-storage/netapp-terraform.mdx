...
description: Find out how to manage your Enterprise File Storage offer using Terraform
...
- [OVHcloud Terraform provider >= v2.5.0](https://github.com/ovh/terraform-provider-ovh/releases/) (>= v2.12.0 to order a service using Terraform)
- Access to the [OVHcloud API](https://eu.api.ovh.com/)
- An Enterprise File Storage service. A service can be ordered from the [product page](https://www.ovhcloud.com/en-gb/storage-solutions/enterprise-file-storage/), from the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, or using the `ovh_storage_efs` resource (see [Order a service](#order-a-service)).
...
If you plan to order a service using Terraform (see [Order a service](#order-a-service)), you will also need the following rights:
...
Follow the [First steps with the OVHcloud APIs](/guides/manage-and-operate/api/first-steps) guide to generate your API token.
...
- **OVHcloud Control Panel method**: Navigate to the <ManagerLink to="/">Enteprise File Storage</ManagerLink> section.
...
Use the [ovh_storage_efs](https://registry.terraform.io/providers/ovh/ovh/latest/docs/resources/storage_efs) resource to order and manage an Enterprise File Storage service.

:::warning
To order a product through Terraform, your account needs to have a default payment method defined. This can be done in the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> or via API with the [/me/payment/method](https://api.ovh.com/console/#/me/payment/method~GET) endpoint.
...
Once you reviewed the execution plan, run [terraform apply](https://developer.hashicorp.com/terraform/cli/commands/apply) to create the service:
...
A created or imported service can be deleted using the [terraform destroy](https://developer.hashicorp.com/terraform/cli/commands/destroy) command.
