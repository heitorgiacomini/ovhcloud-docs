...
description: Find out how to unfreeze your archives
...
- Unfreezing via python-swiftclient:
    - [Preparing an environment for using the OpenStack API](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) by installing python-swiftclient
...
Click on <code className="action">Cloud Archive</code> in the left-hand menu.

To unfreeze the archive, click on the button <code className="action">...</code> to the right of your archive, then <code className="action">Unfreeze</code> to start the retrieval process.
...
<img className="thumbnail" alt="Unfreeze result" src="/images/storage-and-backup/object-storage/pca-unlock/unfreeze_result.png" loading="lazy" />

Your file will be ready for download after this time period. You can then start the download directly in your browser or via a [Swift/SFTP/SCP client](/guides/storage-and-backup/object-storage/pca-sftp).
