...
description: Find out how to activate the OVHcloud rescue mode to troubleshoot your VPS and run system checks
...
- [Resetting your user password to regain access](/guides/bare-metal-cloud/dedicated-servers/replacing-user-password)
...
On the <code className="action">Home</code> tab, click on <code className="action">...</code> next to "Boot" in the section **Your VPS**.
...
Select <code className="action">Reboot in rescue mode</code> from the menu and click <code className="action">Confirm</code> in the popup window.
...
:::info
You will receive an automated email with the SSH credentials for rescue mode access. Please wait for the email to arrive before taking any further action. This email can also be viewed in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>: Click on the name associated with your NIC handle (Customer ID) in the menu bar in the top right-hand corner, then select <code className="action">Service emails</code>.

:::


:::warning
Please note that if you are no longer the technical contact of the server, you will not receive the email. For more information, refer to our guide on [Managing contacts for your services](/guides/account-and-service-management/account-information/managing-contacts).

:::


You will then need to [access your server via SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction), using the temporary password generated for the rescue mode.
...
- Check the KVM in the OVHcloud Control Panel for relevant information as to why the VPS is unable to start. Consult our [KVM guide](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps) for help with this feature.
- If the KVM shows that the VPS is stuck in boot or unable to find the disk, ensure you have [boot logs enabled](/guides/bare-metal-cloud/virtual-private-servers/bootlog-display-kvm). Relay the pertinent logs to our support teams for further investigations by [creating a support request](https://help.ovhcloud.com/csm?id=csm_get_help).

<a name="gofurther"></a>

## Go further

[Getting started with SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)

[Recovering server access if your user password is lost](/guides/bare-metal-cloud/dedicated-servers/replacing-user-password)

[How to replace an SSH key pair](/guides/bare-metal-cloud/dedicated-servers/replacing-lost-ssh-key-pair)
