...
description: Find out how to send SMS from n8n using the OVHcloud API
...
- An [OVHcloud SMS account](https://www.ovhcloud.com/en-gb/sms/) and a valid [SMS sender](/guides/web-cloud/messaging/sms/sms-senders).
- An [OVHcloud VPS](https://www.ovhcloud.com/en-gb/vps/), a server, or a local machine with [n8n](https://n8n.io/) installed and accessible.

## Instructions

If you have not yet installed [n8n](https://n8n.io/) on your VPS, follow the instructions in our guide "[How to install n8n on an OVHcloud VPS](/guides/bare-metal-cloud/virtual-private-servers/install-n8n-on-vps)".
...
To do this, consult the section **Advanced usage: pair OVHcloud APIs with an application** in our guide "[First Steps with the OVHcloud APIs](/guides/manage-and-operate/api/first-steps)", then copy and save the three identifiers `Application key`, `Application secret` and `Consumer key`.
...
- [The sender must be defined in your OVHcloud account](/guides/web-cloud/messaging/sms/sms-senders). To perform a test without declaring a sender and using a short number, replace `"sender": "Your_sender"` with `"senderForResponse": true`.
:::
