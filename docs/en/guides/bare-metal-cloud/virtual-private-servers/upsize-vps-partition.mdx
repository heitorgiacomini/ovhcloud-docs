...
description: Find out how to increase the usable disk space following an upgrade
...
- [Rescue mode](/guides/bare-metal-cloud/virtual-private-servers/rescue) activated on the VPS (Linux only)
...
If the VPS is not already in rescue mode, activate it by using [our guide](/guides/bare-metal-cloud/virtual-private-servers/rescue).
...
- `bad magic number in superblock`: Do not continue. Please read and follow our instructions on [How to fix a **bad magic number in superblock** error](/guides/bare-metal-cloud/virtual-private-servers/upsize-vps-partition#how-to-fix-a-bad-magic-number-in-superblock-error).
...
Before deleting the old partition, it is recommended that you write down the number corresponding to the first sector of the partition. You can find this information through the command <code className="action">p</code>. The information is listed under the `Start` field. Save this data for later.
...
Then delete the partition with the command <code className="action">d</code>.
...
You now need to create a new partition with the command <code className="action">n</code>. It is recommended that you use the default values.
...
You now need to ensure that the partition is bootable. You can do this using the command <code className="action">a</code>.
...
Save your changes and exit the application with the command <code className="action">w</code>:
...
If the command <code className="action">e2fsck</code> returns the error message `bad magic number in superblock`, you should check and repair the filesystem by using a backup of the superblock. To see which backups of the superblock are available, enter the following command:
...
Right click on the C: volume and select <code className="action">Extend Volume...</code>.
...
Enter your desired size and click <code className="action">OK</code>. Your volume will now be extended.
