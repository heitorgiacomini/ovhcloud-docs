...
description: Find out how to configure IPv6 on your OVHcloud VPS
...
:::info
This article is about Primary IPv6 configuration on a public interface. You can also configure Additional IP addresses on your VPS using [this guide](/guides/bare-metal-cloud/virtual-private-servers/configuring-ip-aliasing).
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    The IPv6 address and the IPv6 gateway assigned to your server will appear in the `IP` section of the <code className="action">Home</code> tab. Once you have copied them, continue with [applying the IPv6 configuration](#applyipv6).
  ...
  <Tab label="Via the OVHcloud API">
    Go to the [OVHcloud console API](https://eu.api.ovh.com/console/) page:
    
    - Click <code className="action">Authentication</code> in the upper left corner.
...
Once you have gathered the necessary information for the IPv6 configuration, log in to your VPS via SSH. If you require assistance, please refer to this guide: [Introduction to SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction).
...
<Tabs>
  <Tab label="Configuring *interfaces* files">
  ...
  <Tab label="Configuration using Netplan">
...
#### Persistent application on Red Hat and its derivatives (CentOS, Rocky Linux & AlmaLinux, etc.) <a name="persistentredhat"></a>
...
<Tabs>
  <Tab label="Configuring *network-scripts* files">
  ...
  <Tab label="Configuring *system-connections* files">
    ...
    We have omitted the IPv4 configuration to avoid confusion, the IPv6 address is configured in the same file.
...
#### Persistent application on Fedora 42 and later <a name="persistentfedora"></a>
...
By default, IPv6 is not configured on Windows Servers. To enable it, open the <code className="action">Control Panel</code> and click on <code className="action">View network status and tasks</code>, then on <code className="action">Change adapter settings</code>.
...
Click on <code className="action">Ethernet</code> to open the settings and click on the <code className="action">Properties</code> button to view the `Ethernet Properties`.

Select <code className="action">Internet Protocol Version 6 (TCP/IPv6)</code>, then click on the <code className="action">Properties</code> button.
...
In the IPv6 Properties window, select <code className="action">Use the following IPv6 address</code>. Enter the IP addresses which you have retrieved in the first step.

You also have the option of entering the IPv6 DNS resolvers of your choice under <code className="action">Use the following DNS server addresses</code>. This is not mandatory if the DNS resolvers of the IPv4 configuration are already functional.

Finally, tick the `Validate settings upon exit` box and click the <code className="action">OK</code> button to validate your changes. An error message may appear if the specified gateway is not on the same IPv6 subnet (/128 and /64 for example). You can ignore this message and continue to the next step regardless.
...
<summary>**Step 4: Disable Cloud-init network management (optional)**</summary>
