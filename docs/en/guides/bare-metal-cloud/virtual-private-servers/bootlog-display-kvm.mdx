...
description: Find out how to diagnose a VPS by checking the boot log
lastUpdated: 2021-07-05
---

## Objective

If your VPS has become unresponsive, you still have the possibility to access it from your Control Panel via [KVM](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps). The quickest way to diagnose the problem is to check the boot log of the server. However, the GRUB configuration needs to be modified in order for these logs to appear. 
...
Initiate a server reboot in rescue mode from the Control Panel. Refer to our [rescue mode guide](/guides/bare-metal-cloud/virtual-private-servers/rescue) if necessary.
...
Once you have done the modifications, reboot your VPS in 'normal' mode in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. Boot log information should now appear when using the [KVM console](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps).

## Go further

[Using the KVM for a VPS](/guides/bare-metal-cloud/virtual-private-servers/using-kvm-for-vps)
