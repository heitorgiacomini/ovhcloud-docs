...
description: Find out how to configure a new password for a user account on a GNU/Linux operating system with the OVHcloud rescue mode
...
:::info
To recover access to a server that you log in to with an SSH key, refer to our guide on [How to replace an SSH key pair](/guides/bare-metal-cloud/dedicated-servers/replacing-lost-ssh-key-pair) instead.
:::
...
:::info
This guide is not applicable for **Windows** server installations. Please refer to our guides on [How to change the admin password on a Windows dedicated server](/guides/bare-metal-cloud/dedicated-servers/rcw-changing-admin-password-on-windows) and [How to change the admin password on a Windows VPS](/guides/bare-metal-cloud/virtual-private-servers/resetting-a-windows-password).
...
Remember to use the regular boot mode of your server when restarting it in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>.
