...
description: Replace a defective disk on a dedicated server with software RAID without downtime using the hot-swap procedure.
...
The defective disk is now ready to be replaced by a datacentre technician. Once the operation is complete, you just have to resync the RAID. To do this, you can use the following guide: [Configuring software RAID](/guides/bare-metal-cloud/dedicated-servers/raid-soft)
...
[Managing Software RAID (UEFI) on Dedicated Servers](/guides/bare-metal-cloud/dedicated-servers/raid-soft-uefi)
