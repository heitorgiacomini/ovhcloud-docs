...
description: Find out what the OVHcloud API provides in order to customise the disks, hardware/software RAID and partitioning configuration during the server OS reinstallation
...
With [OVHcloud Dedicated Servers](/links/bare-metal/bare-metal), you can configure Disks, [hardware RAID](/guides/bare-metal-cloud/dedicated-servers/raid-hard), Partitions, [software RAID](/guides/bare-metal-cloud/dedicated-servers/raid-soft), LVM, ZFS, etc. during [OS reinstallation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server) from the <ApiLink>OVHcloud API</ApiLink> as well as the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. In this article, we will focus on the <ApiLink>OVHcloud API</ApiLink>. This will give us more details about the engine that is running in the background in order to create the storage customisation on the dedicated server from the input data passed on to the OVHcloud API.
...
In this page we are focusing only on the `storage` sub-hash of the API call used to reinstall an OS on a dedicated server. For other customisations not related to storage, please read [OVHcloud API & OS installation](/guides/bare-metal-cloud/dedicated-servers/api-os-installation) for more details.
...
<Api version="v1" section="/dedicated/server" method="GET" route={"/dedicated/server/\{serviceName\}/specifications/hardware"} />
...
<Api version="v1" section="/dedicated/server" method="GET" route={"/dedicated/server/\{serviceName\}/install/hardwareRaidProfile"} />
...
- [software RAID](/guides/bare-metal-cloud/dedicated-servers/raid-soft) (SR),
...
<img className="thumbnail" alt="Partitioning Layers Table" src="/images/bare-metal-cloud/dedicated-servers/partitioning-ovh/partitioning-layers-table.png" loading="lazy" />
...
In the <code className="action">/dedicated/installationTemplate</code> section, you can display storage details such as LVM compatibility and filesystem availability for a specific OS:
...
² The RAID level for swap can only be set to 1 within the <ApiLink>OVHcloud API</ApiLink>. In reality, the swap partitions will not use RAID. When a swap partition with size `s` is defined on a server with `n` disks, this will create `n` partitions of size `s` on every disk without any software RAID device underneath.<br />
...
⁴ The ESXi installer does not support custom partitioning schemes. Partitioning is defined by the software publisher. Nevertheless, the <ApiLink>OVHcloud API</ApiLink> can give you an idea of what the partitioning looks like: see [OS & Partitioning Compatibility](#os-partitioning-compatibility) for more details.<br />
...
:::warning
A custom partitioning layout, while valid and accepted by the <ApiLink>OVHcloud API</ApiLink>, can still prevent the server from booting properly (for example an unusual filesystem/bootloader combination, or a RAID/LVM/ZFS assembly issue specific to your layout). In that case, no support intervention will automatically be created on the server: an explicit customer message will appear after the last reboot. See [Common customer errors](#errors) for more details.

:::

### Data erasure <a name="data-erasure"></a>

By default, if the `erase` parameter is not specified for a disk group involved in the `storage` request, it is set to `true`: all disks of that disk group are erased during the OS installation.
...
The OS is installed on the disk group carrying the installation attributes (`partitioning`, `hardwareRaid`), the other declared disk groups carrying only `erase`. If none of them carries an installation attribute, the OS is installed on the default disk group, as described in the [Disk Groups](#disk-group) section.
...
- It is not possible to set `erase: false` for the disk group on which the OS is installed.
- It is not possible to declare the same `diskGroupId` more than once.
- It is not possible to declare more than one disk group on which to install the OS. For any additional disk group, only the `erase` attribute can be set — without OS installation, partitioning, or hardware RAID configuration.
...
If the installation fails for one of these reasons, no support ticket is created automatically. Instead, an explicit message is displayed to the customer, either during partitioning or at first boot into the installed OS. See [Common customer errors](#errors) for more details.
...
Within the <ManagerLink to="/#/dedicated-servers/server">OVHcloud Control Panel</ManagerLink>, this is visible on the progress bar
From the <ApiLink>OVHcloud API</ApiLink>, this status can be obtained with the following API call:
...
|---|---|---|
|Partition of type `t` with mountpoint `m` cannot fill the disk.|You have chosen the `swap` partition to fill the disk (or partition with size 0 when defined within the <ApiLink>OVHcloud API</ApiLink>), we disallow this to avoid creating unnecessarily large `swap` partitions.|- Set a fixed size for the `swap` partition|
|`message`. Please adjust partitions so that the `p` partition fits on `n` disk(s)|You have chosen a partition with a RAID that requires a number of disks that your server can provide, but some disks are already full because of other partitions and/or this current partition.|- If it is not already set on another partition, set the partition size to fill the disk (or partition with size 0 when defined within the <ApiLink>OVHcloud API</ApiLink>)<br />- Reduce the size of this partition so that it fits the disks<br />- Reduce the size of other partitions so that this partition fits the disks|
...
|Partitioning failed. Leftover partitions from a previous install (on disk groups excluded from cleaning) are likely conflicting with the new layout.|Leftover metadata (partition table, `mdadm` superblocks, LVM or ZFS signatures) on a disk group declared with `erase: false` conflicts with the requested layout.|- Set `erase: true` on this disk group if you no longer need to keep its data<br />- Manually erase the disk group before reinstalling|
|The server does not seem to have booted properly into the newly installed operating system. This is likely related to leftover partitioning from a previous installation on disk group(s) excluded from cleaning; please check the KVM, if available.|Leftover metadata on a disk group declared with `erase: false` interferes with the boot of the newly installed system.|- Check the KVM to identify the exact boot issue<br />- Set `erase: true` on this disk group if you no longer need to keep its data<br />- Manually erase the disk group before reinstalling|
|The server does not seem to have booted properly into the newly installed operating system. This is likely related to the custom partitioning you requested; please check the KVM, if available.|The requested layout is accepted by the <ApiLink>OVHcloud API</ApiLink> but produces a bootloader/filesystem/RAID assembly the server cannot boot from.|- Check the KVM to identify the exact boot issue<br />- Adjust your partitioning layout (e.g. filesystem choice, RAID level, `/boot` configuration) and reinstall|
