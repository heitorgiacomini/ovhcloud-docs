...
|Partitioning failed. Leftover partitions from a previous install (on disk groups excluded from cleaning) are likely conflicting with the new layout.|You declared `erase: false` on a disk group that was previously used with a different storage configuration, and the leftover partition table, `mdadm` superblocks, LVM or ZFS signatures on that disk group conflict with the new layout being applied.|- Set `erase: true` on this disk group if you no longer need to keep its data<br />- Manually erase the disk group's existing partitions/metadata yourself and retry the installation|
|The server does not seem to have booted properly into the newly installed operating system. This is likely related to leftover partitioning from a previous installation on disk group(s) excluded from cleaning; please check the KVM, if available.|You declared `erase: false` on a disk group, and leftover boot metadata, `mdadm` superblocks, LVM or ZFS signatures from a previous installation on that disk group are conflicting with the newly installed bootloader/partitioning.|- Check the KVM to identify the exact boot issue<br />- Set `erase: true` on this disk group if you no longer need to keep its data<br />- Manually erase the disk group before reinstalling|
|The server does not seem to have booted properly into the newly installed operating system. This is likely related to the custom partitioning you requested; please check the KVM, if available.|Your custom partitioning layout, while valid and accepted by the <ApiLink>OVHcloud API</ApiLink>, resulted in a configuration (bootloader, filesystem, RAID/LVM/ZFS assembly) that prevents the server from booting properly.|- Check the KVM to identify the exact boot issue<br />- Adjust your partitioning layout (e.g. filesystem choice, RAID level, `/boot` configuration) and reinstall|
