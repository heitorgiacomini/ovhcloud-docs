...
Log in to the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, open the <code className="action">Network</code> menu in the left-hand sidebar and click <code className="action">Public IP Addresses</code>. Select the correct server, so that the Additional IP (or IP block) attached to it appears.
