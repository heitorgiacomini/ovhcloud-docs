...
description: Rebuild the software RAID mirror configuration on a Windows dedicated server after a disk replacement.
...
Once logged in, right-click on the <code className="action">Start Menu</code> button and open <code className="action">Run</code>.

<img className="thumbnail" alt="Windows Start menu with Run option" src="/images/bare-metal-cloud/dedicated-servers/raid-soft-windows/raid-soft-windows-01.png" loading="lazy" />

Enter "cmd" and click on <code className="action">OK</code>.

<img className="thumbnail" alt="Run dialog box with cmd command entered" src="/images/bare-metal-cloud/dedicated-servers/raid-soft-windows/raid-soft-windows-02.png" loading="lazy" />
...
In this example, `Disk 1` is a replacement drive that has been installed in order to replace the defective `Disk M0` which had been [physically removed](/guides/bare-metal-cloud/dedicated-servers/disk-replacement) previously.
...
[Managing Software RAID (BIOS) on Dedicated Servers](/guides/bare-metal-cloud/dedicated-servers/raid-soft)
