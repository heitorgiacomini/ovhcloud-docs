...
description: Learn how OVHcloud dedicated servers boot and why PXE must remain first in the boot order
...
OVHcloud dedicated servers use a network-based boot process (netboot) that provides flexibility and security benefits. This guide explains how the boot process works and why you should not change the default boot order directly in the BIOS/UEFI settings, but instead use the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> or the [OVHcloud API](https://eu.api.ovh.com/).
...
<img className="thumbnail" alt="Boot process overview" src="/images/bare-metal-cloud/dedicated-servers/boot-process/boot-process.svg" loading="lazy" />
...
- **UEFI boot servers**: iPXE uses [`sanboot`](https://ipxe.org/cmd/sanboot) to boot the EFI bootloader path (`efiBootloaderPath`) specified during [OS installation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#install) (e.g. `\efi\debian\grubx64.efi`).
    ...
    The `efiBootloaderPath` can be changed via the [OVHcloud API](https://eu.api.ovh.com/):

    <Api version="v1" section="/dedicated/server" method="PUT" route={"/dedicated/server/\\{serviceName\\}"} />

#### Customer rescue mode

A temporary operating system for troubleshooting, recovery, and initial configuration. Microcode updates are applied before booting into rescue, just as with boot to disk. For more information, see [Activating and using rescue mode](/guides/bare-metal-cloud/dedicated-servers/rescue-mode).

#### Custom boot scripts

For advanced use cases, you can configure a custom iPXE script via the [OVHcloud API](https://eu.api.ovh.com/). For more information, see [Configuring a custom iPXE script](/guides/bare-metal-cloud/dedicated-servers/ipxe-scripts).
...
If your server uses **[OLA (OVHcloud Link Aggregation)](/guides/bare-metal-cloud/dedicated-servers/ola-enable-manager)** — a vRack-only configuration with no public interface — the standard netboot process will not work. Private networks cannot reach the OVHcloud DHCP and boot infrastructure.

In this case, you must deploy your own DHCP, TFTP, and boot services within your private network. For detailed instructions, see the guide on [Managing server reboot with OVHcloud Link Aggregation](/guides/bare-metal-cloud/dedicated-servers/pxe-with-full-private-dedicated).
...
[OS installation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#install)

[Activating and using rescue mode](/guides/bare-metal-cloud/dedicated-servers/rescue-mode)

[Configuring a custom iPXE script](/guides/bare-metal-cloud/dedicated-servers/ipxe-scripts)

[Managing server reboot with OVHcloud Link Aggregation](/guides/bare-metal-cloud/dedicated-servers/pxe-with-full-private-dedicated)
