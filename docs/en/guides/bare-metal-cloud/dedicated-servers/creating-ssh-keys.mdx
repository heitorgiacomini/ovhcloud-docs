...
description: Create OpenSSH key pairs on your local device and use them for secure, passwordless connections to your dedicated server.
...
**This guide explains how to create and manage authentication key pairs on your local device and use them to connect to remote servers.**
...
:::info
`Ed25519` is considered the most secure method but `RSA` is a valid alternative. Both are compatible with the OVHcloud Control Panel if you want to [store public keys in your customer account](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
:::
...
If you confirm with <code className="action">Enter</code> without entering a name, the standard file name will be used (`id_rsa` in this example).
...
:::warning
When using authentication keys, remote access to your server is only as secure as the client device storing the private key. Protecting your device and the key files stored on it from unauthorized access is therefore crucial.
...
Copy this key string to [add it to a new server](#getstarted) or to [import it into the OVHcloud Control Panel](/guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel).
...
Since all key files should be placed inside the folder `.ssh` of your user's `home` directory, the file names have to be different. When you create a new key pair and you are asked to provide a file name, enter a name of your choice, for example the name of your server. 
...
When connecting to the corresponding server, specify the name of the private key file in addition to the connecting user and server details:
...
Following is a simple example to explain how to configure an SSH connection to a server.  
...
Be sure to use the correct IP address and key file name. The first line, beginning with `Host`, defines the name of this connection (`dedicated_server` in this example).

You can then log in to the server by replacing the server IP address with the alias name identifying this connection (`Host`):
...
To configure an SSH connection to a second remote host with the username "rocky", the modified SSH port "49160" and the private key in the file "myVPS_rsa", extend the file content as shown in this example:
...
When working from a **Windows** command line, you can use a right-click to **paste** the content of your clipboard into the command line window. To **copy** a string from the command line window, highlight it, then press <code className="action">Enter</code>. You can also find these functions via a right-click on the menu bar of the command line window.
:::
...
Since all key files should be placed inside the folder `.ssh` of your Windows user directory, the file names have to be different. When you create a new key pair and you are asked to provide a file name, enter a name of your choice, for example the name of your server. 
...
To add key authentication for other users accessing your server, create a new key pair but use the appropriate `$HOME` folder or **Windows** `Users` directory of the user in question to store the authentication keys (or execute the commands on this person's dedicated device).  
Then add the new public key string to the server in the file `authorized_keys` as described above.

#### Removing public keys from your server

Open the file `authorized_keys` on your server as described above and remove the key string that corresponds to the user account having its access revoked.

<a name="gofurther"></a>

## Go further

[How to get started with SSH](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)
