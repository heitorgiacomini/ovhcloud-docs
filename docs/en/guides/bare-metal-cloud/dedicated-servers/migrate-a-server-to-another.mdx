...
description: Migrate data between OVHcloud dedicated servers using rsync, SFTP, or backup storage for a seamless server transition.
...
- The easiest way is to use a suitable software such as [SFTP](/guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp).
- The other option is to [synchronise the two servers with each other](/guides/bare-metal-cloud/dedicated-servers/how-to-copy-data-from-one-dedicated-server-to-another-using-rsync).
...
See our guide on [using Backup Storage on a dedicated server](/guides/bare-metal-cloud/dedicated-servers/services-backup-storage) for more information.
...
If the reputation of your IP addresses is important, we strongly recommend using [Additional IPs](https://www.ovhcloud.com/en-gb/network/additional-ip/), as they can be kept if you migrate.

Once you have these IPs, you simply need to move them to the new server.
To do this, please refer to our dedicated guide: [Moving an Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip).
...
For more information, please read our documentation on domains and DNS.

## Go further

[Getting Started with SSH on a Dedicated Server](/guides/bare-metal-cloud/dedicated-servers/ssh-introduction)
