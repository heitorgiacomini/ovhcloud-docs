...
description: Learn how to configure the OVHcloud Game DDoS Protection firewall
...
- An [OVHcloud **Game** dedicated server](https://www.ovhcloud.com/en-gb/bare-metal/game/)
...
:::warning
The *Game firewall* protects the IP associated with a server. As a result, if you have a server with multiple IP addresses (i.e. [Additional IP addresses](https://www.ovhcloud.com/en-gb/network/additional-ip/)), you need to configure each of them separately.
...
- Open <code className="action">Network</code> in the left-hand sidebar.
- Open <code className="action">Public IP Addresses</code>.

You can filter IP addresses by using the <code className="action">All service types</code> drop-down menu, or directly enter the desired IP address in the search bar. Enter the name or category of the corresponding server:

#### List the IP addresses attached to your Game Server

<Tabs>
  <Tab label="From the Dedicated Servers page">
    - Open the <code className="action">Bare Metal Cloud</code> section in the left-hand sidebar.
    - Select <code className="action">Dedicated servers</code>.
    ...
    - Click the <code className="action">...</code> button and select <code className="action">Configure Game Protection</code>. You will be taken to the list of IP addresses assigned to your server.
  </Tab>
  <Tab label="From the Public IP Addresses page">
    ...
    - In the <code className="action">All service types</code> drop-down menu, find and select the Bare Metal Game server you wish to configure.
...
For each address attached to your server that requires protection, verify that the *Game Firewall* status is set to <code className="action">Available</code>. You will have to configure the *Game Firewall* rules separately for each.

- Click the <code className="action">⁝</code> button to the right of the table and select <code className="action">Configure Game firewall</code>.
...
- For security reasons, we strongly recommend you enable the <code className="action">Apply "Default Deny" policy</code> option, at the top right of the rule table. This option blocks all traffic that does not match the rules you set up for the Game Firewall, i.e. all listed game applications will be protected and no other connections will be able to reach your server. This option significantly reduces the attack surface exposed to potential malicious actors.
...
Please note that supported gaming protocols (game titles and versions that can be protected) may change over time. Moreover, they can be different between older Bare Metal Game server ranges and the newer ones. The most recent list of supported game profiles can be found on the [Game DDoS Protection page](https://www.ovhcloud.com/en-gb/security/game-ddos-protection/).
...
:::warning
Game DDoS Protection takes effect after the rules defined in the [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network). For both to work properly, the Edge Network Firewall cannot be too strict and needs to pass traffic to the Game DDoS Protection.
...
All supported Game DDoS Protection protocols for a specific server are visible on the <code className="action">GAME firewall</code> configuration page for any IP address pointing to that server, in the <code className="action">Game protocol</code> drop-down menu:
...
<summary>**How can I ensure automation to work for an Additional IP between a new and an old generation of Bare Metal Game servers?**</summary>
...
<summary>**While having configured my game with appropriate ports and default policy to drop, I still receive attacks that are impacting my Game server. What to do?**</summary>
