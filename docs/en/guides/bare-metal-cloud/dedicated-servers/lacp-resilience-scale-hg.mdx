...
description: Reinforce network resilience on your OVHcloud Bare Metal server by configuring public and private interface aggregation.
...
- A [dedicated server](https://www.ovhcloud.com/en-gb/bare-metal/) with 4 network interfaces (High Grade & SCALE ranges).
...
<img className="thumbnail" alt="Before/after resilience comparison" src="/images/bare-metal-cloud/dedicated-servers/lacp-resilience-scale-hg/resilience_comparison.svg" loading="lazy" />
...
To activate the new rule, you need to [switch to **OLA** mode](/guides/bare-metal-cloud/dedicated-servers/ola-enable-manager), then revert to the default mode.
...
[Dedicated Server - LACP Link Aggregation on Debian/Ubuntu](/guides/bare-metal-cloud/dedicated-servers/lacp-enable-netplan)
