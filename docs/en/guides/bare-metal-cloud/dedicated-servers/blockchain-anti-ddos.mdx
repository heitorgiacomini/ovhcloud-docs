...
description: Leverage the OVHcloud Anti-DDoS infrastructure to protect your Solana nodes on dedicated servers without harming functionality.
...
- At least one [dedicated server](https://www.ovhcloud.com/en-gb/bare-metal/) which respects the desired blockchain's specific hardware requirements.
...
To get started, create a [support request ticket](https://help.ovhcloud.com/csm?id=csm_get_help) with the following options: Incident -> Public IP -> [x] This service is not listed -> Create a support request -> <code className="action">Next</code>.
...
[Edge Network Firewall for Dedicated Servers](/guides/bare-metal-cloud/dedicated-servers/firewall-network)

[Network Security Dashboard for Dedicated Servers](/guides/bare-metal-cloud/dedicated-servers/network-security-dashboard)
