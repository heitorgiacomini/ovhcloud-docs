...
description: Deploy a certified OpenNebula Hosted Cloud using OVHcloud Bare Metal servers and Ansible playbooks.
...
- **Two** [dedicated servers](https://www.ovhcloud.com/en-gb/bare-metal/) from the Scale or High Grade ranges.
- An active [vRack](https://www.ovhcloud.com/en-gb/network/vrack/) service.
- A public block of [Additional IP](https://www.ovhcloud.com/en-gb/network/additional-ip/) addresses, sized according to your needs.
...
Servers from the [Scale range](https://www.ovhcloud.com/en-gb/bare-metal/scale/) are suitable for small to medium cloud environments.

Servers from the [High Grade range](https://www.ovhcloud.com/en-gb/bare-metal/high-grade/) are more appropriate for heavier production workloads.
...
First, you need to install Ubuntu 24.04 LTS on both of your dedicated servers, by following the instructions in [this guide](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server).

Subsequently, add both servers to your vRack service by following step 2 of [this vRack configuration guide](/guides/bare-metal-cloud/dedicated-servers/vrack-configuring-on-dedicated-server).

Finally, from the OVHcloud Control Panel, open the <code className="action">Network</code> section, then select <code className="action">Public IP Addresses</code> under **Public Network**. Once you have reached the IP management interface, click on the <code className="action">Order IPs</code> button near the top of the page. Choose the IP version, then **select the vRack your servers are attached to**, and the region where those servers are hosted.
...
If you are unsure or need more information, please consult the following guide: [Configuring an Additional IP block in a vRack](/guides/bare-metal-cloud/dedicated-servers/configuring-an-ip-block-in-a-vrack).
...
For more information about the link aggregation settings, please consult the following guide : [Improving Network Resilience on Bare Metal servers](/guides/bare-metal-cloud/dedicated-servers/lacp-resilience-scale-hg).
...
<img className="thumbnail" alt="dedicated servers administration page" src="/images/bare-metal-cloud/dedicated-servers/opennebula-deployment/bare_metal_network.png" loading="lazy" />
...
<img className="thumbnail" alt="dedicated servers ip address" src="/images/bare-metal-cloud/dedicated-servers/opennebula-deployment/bare_metal_ip_address.png" loading="lazy" />
