...
Fully automating OS installation or reinstallation on a [dedicated server](https://www.ovhcloud.com/en-gb/bare-metal/) with the [OVHcloud API](/guides/manage-and-operate/api/first-steps) can be interesting in various situations.
...
Open the <ApiLink>API console</ApiLink> and go to the <code className="action">/dedicated/server</code> section.
...
In the <code className="action">/dedicated/installationTemplate</code> section, you can display the details for a specific OS:
...
¹ Customers that don't use images from the OVHcloud catalogue (installation from a custom image ([BYOI](/guides/bare-metal-cloud/dedicated-servers/bring-your-own-image)/[BYOLinux](/guides/bare-metal-cloud/dedicated-servers/bring-your-own-linux)), installation over the network, or manually via IPMI) are not affected by this limitation.<br />
² For more details, see [partitioning customization](/guides/bare-metal-cloud/dedicated-servers/partitioning-ovh).<br />

:::info
If you need to see this information for all available OSs in the catalog, you should consider using the API call <code className="action">/dedicated/installationTemplate/templateInfos</code> instead.
:::
...
Example of specific questions for Debian 13 (Trixie):
...
```json
{
    "inputs": [
        ...
        {
            "name": "configDriveMetadata",
            "description": "Cloud-init Config Drive Metadata",
            "default": "",
            "mandatory": false,
            "type": "keyValue",
            "enum": []
...
Example of specific questions for Windows Server 2025 Standard (Core):
...
:::info
Answers must use the JSON type declared by the question:

- `boolean`: use a JSON boolean, not a string, i.e. `true` and not `"true"`. Note that, in the question description above, `default` is always returned as a string.
- `keyValue`: use a JSON object with string values, for example `{"foo": "bar"}`.
:::
...
`storage` contains customization related to disks & hardware RAID, partitions, software RAID, LVM, ZFS, etc. See [OVHcloud API and Storage](/guides/bare-metal-cloud/dedicated-servers/partitioning-ovh) for more details.

#### Linux payload example

The following payload will install Debian 13 (Trixie) with SSH key-based authentication, a custom bash post-installation script, and LACP bonding enabled:
...
The following payload will install Windows Server 2025 Standard (Core) in French with a custom PowerShell post-installation script.

```json
{
  "operatingSystem": "win2025core-std_64",
  "customizations": {
    "hostname": "ma-fenetre",
    "language": "fr-fr",
...
[Installing CUDA on a Dedicated Server with GPU](/guides/bare-metal-cloud/dedicated-servers/cuda-installation-on-gpu)
