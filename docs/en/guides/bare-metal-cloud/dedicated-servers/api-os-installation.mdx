...
:::info
Questions of the `boolean` type are answered with a JSON boolean, not with a string: use `true`, not `"true"`. Note that in the question description above, `default` is always returned as a string.
:::
