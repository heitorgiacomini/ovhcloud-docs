...
- **Direct link:** <ManagerLink to="/#/dedicated/network-security/scrubbing-center">Security Dashboard</ManagerLink>
- **Navigation path:** <code className="action">Network</code> > <code className="action">Network Security Dashboard</code>
...
OVHcloud's Anti-DDoS infrastructure is a multi-layered, distributed defense system to fight against cyber attacks. It consists of multiple edge locations and scrubbing centres that can analyse and clean up malicious traffic. Together with the [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network) and the [GAME DDoS protection](/guides/bare-metal-cloud/dedicated-servers/firewall-game-ddos), it offers quality protection services for various cases.
...
During an attack, an active mitigation action will be indicated by a warning icon on the IP listing page (in the <code className="action">Manage IPs</code> section of your Control Panel).
...
- The **Firewall** column indicates the state of the Edge Network Firewall which can be:
    ...
    - **(no status)** - Firewall configuration is not created. To configure rules, click on the <code className="action">...</code> button then select <code className="action">Create Firewall</code>.
...
For more information how to configure Edge Network Firewall rules, please see our [Edge Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network) guide.
