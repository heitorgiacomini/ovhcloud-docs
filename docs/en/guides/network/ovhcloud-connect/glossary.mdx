...
description: 'Discover the vocabulary related to the technologies used by OVHcloud Connect'
...
A unique number that identifies a network on the Internet for the purpose of exchanging routing information via BGP. Your network has its own ASN, and the OVHcloud-side routing instance has its own ASN. These numbers are used when configuring BGP sessions, and the two sides must differ (eBGP). You can use any ASN except the OVHcloud per-region reserved values (65501 Europe, 65502 Canada, 65519 Asia).

**AZ (Availability Zone)**
A physically separate data centre within a region, containing the one or more data centres hosting your services. AZs are both sufficiently geographically distant from each other to be isolated in the event of a disaster, and sufficiently close to ensure low latency.

### B

**Bandwidth**
The subscribed throughput capacity for your link. The available options depend on the offer: OVHcloud Connect Direct: 1, 10 or 100 Gbps. OVHcloud Connect Provider: from 50 Mbps up to 50 Gbps. Unlike a connection established over the Internet, this throughput is guaranteed and ensures consistent performance.
...
A physical location (third-party data centre) where the OVHcloud network is available for connection. It is also the entry point of your data into OVHcloud's private network. The accessible PoPs are listed on [this link on our website](/links/network/ovhcloud-connect).
...
**Region**
A geographical area where OVHcloud operates one or more data centres and Availability Zones. OVHcloud regions are deployed around the world, including in Europe, North America, and Asia-Pacific. Find out more on OVHcloud regions on [this link to our website](/links/bare-metal/regions).
...
A contractual commitment from OVHcloud that defines the guaranteed level of service, expressed as a percentage of monthly availability. The level depends on your configuration: for example, a redundant setup with two connections across two PoPs qualifies for a 99.9% availability commitment. See the [SLAs](/guides/network/ovhcloud-connect/slas) guide for the full config-based tiering.
...
A protocol that provides gateway redundancy for the OVHcloud L3 routing instance in an availability zone. OVHcloud runs two devices (A and B) that share a virtual gateway IP (the first usable IP of the AZ subnet). There is one VRRP instance per AZ; the VRID is provided by OVHcloud; device A is the master by default. Enabling BGP on the AZ disables VRRP.
