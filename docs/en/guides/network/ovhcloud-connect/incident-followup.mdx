---
title: 'Declare and follow up an OVHcloud Connect incident'
description: 'Find out how to report an OVHcloud Connect incident, provide diagnostic information, and track resolution'
lastUpdated: 2026-06-15
---

## Objective

When an OVHcloud Connect link degrades or goes down, knowing how to report, track, and escalate the incident shortens resolution and clarifies what OVHcloud covers (the PoP-to-datacentre segment) versus what you or your provider own. It also determines how the availability SLA and any service credits apply, since credits are tied to an open ticket. If you experience a service degradation or outage, follow this guide to report the incident, provide the right diagnostic information, and track resolution through to a post-incident review.

## Step 1 — Gather diagnostic information

Before contacting support, collect the following information to speed up diagnosis:

### Incident triage checklist

| Information | How to collect it |
|---|---|
| **Timestamp (UTC)** | Note exactly when the issue started and its duration |
| **Affected service** | OVHcloud Connect service name/ID, PoP, VLAN ID |
| **Symptoms** | Link down, BGP session drop, packet loss, high latency, etc. |
| **Interface status** | Check interface/BGP status on your router (vendor-specific commands) |
| **BGP status** | Check interface/BGP status on your router (vendor-specific commands) |
| **Traceroute** | Run from your side to an OVHcloud resource, and vice versa |
| **MTR output** | If available, run `mtr` for a combined ping + traceroute with loss % |
| **Recent changes** | Any configuration changes, maintenance, or updates before the incident |

### Example diagnostic commands

Collect the equivalent BGP, interface, and routing status from your own edge router using your vendor's commands.

**From a Linux host in OVHcloud:**

```bash
ping -c 10 10.0.0.1
traceroute 10.0.0.1
mtr -r -c 50 10.0.0.1
```

## Step 2 — Check for known issues

Before opening a ticket, verify whether the issue is already known:

- **OVHcloud status page** — Check the OVHcloud status and scheduled-maintenance pages.
- **Provider status page** — If using a provider, check your provider's status/maintenance page.
- **Your own monitoring** — Confirm the issue is not on your side (local router failure, ISP outage, etc.).

Log in to your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> and open the <code className="action">Bare Metal Cloud</code> section, then go to <code className="action">Network</code> > <code className="action">OVHcloud Connect</code>.

## Step 3 — Open a support ticket

1. Open the **Help Centre** from your Control Panel dashboard and start a new support request.
2. Select your **OVHcloud Connect** service as the affected service.
3. Provide a clear **subject line** (e.g. "OVHcloud Connect — BGP session down since 14:30 UTC").
4. In the description, include:
   - Your OVHcloud Connect **service name/ID**
   - **Timestamp** of the issue (UTC)
   - **Symptoms** observed
   - **Diagnostic outputs** (paste BGP summary, interface status, traceroute)
   - **Impact** on your business (number of users affected, critical services down)
5. Attach any **screenshots** or log files.
6. Submit the ticket and note the **ticket number**.

:::tip
**Tip:** The more detailed your initial report, the faster the support team can diagnose the issue.
:::

## Step 4 — Follow up

| Action | When |
|---|---|
| **Check ticket status** | Regularly, in the Help Centre where you opened the request |
| **Respond to requests** | If support asks for additional information, provide it promptly |
| **Escalate if needed** | For critical business impact, request priority handling or escalation |
| **Keep a chronology** | Maintain a timeline of events, communications, and status changes |

### Escalation

If the issue is **business-critical** and you are not getting a timely response:

1. Reference the SLA terms in your contract (see [SLAs](/guides/network/ovhcloud-connect/slas)).
2. Request an escalation through the support portal.
3. Contact your OVHcloud account manager for priority handling.

## Step 5 — Post-incident

After the incident is resolved:

1. **Request a Root Cause Analysis (RCA)** — Ask OVHcloud support for a post-incident report explaining what happened and what measures are being taken to prevent recurrence.
2. **Update your runbooks** — Document the incident, resolution steps, and lessons learned.
3. **Implement mitigations** — If the incident revealed a gap (e.g. missing redundancy, inadequate monitoring), plan improvements.
4. **Check for service credits** — Service credits apply only to configurations covered by an availability SLA (2 connections on 2 PoPs = 99.9%, or 4 connections on 2 PoPs to 2 datacentres = 99.99%). If eligible, the credit is 10%, 20% or 30% of the impacted service's monthly cost depending on the availability reached, capped at 30% per month, and must be claimed within one month of the ticket being closed. See [SLAs](/guides/network/ovhcloud-connect/slas) for the exact conditions.

## Summary: incident response flow

<img className="thumbnail" alt="Summary: incident response flow" src="/images/network/ovhcloud-connect/incident-followup/fig-1.svg" loading="lazy" />

## What's next?

- Set up proactive [monitoring](/guides/network/ovhcloud-connect/monitor) to detect issues before they impact your users
- Review your [SLA](/guides/network/ovhcloud-connect/slas) commitments

## Go further

For training or technical assistance implementing our solutions, contact your sales representative or visit our [Professional Services](/links/professional-services) page to request a quote and have your project analyzed by our experts.

Join our [community of users](/links/community).
