...
| 2 | **Incorrect ASN** | Check that you are peering with the OVHcloud-side ASN assigned for your region (**65501** Europe, **65502** Canada, **65519** Asia) and that your own ASN is correctly configured (private ASN range 64512–65534 recommended). Note the known issue with **AS65501** (routes are not propagated). |
...
| 7 | **MD5 authentication mismatch** | If you have configured MD5 on your side, the password must match on both sides. Remove or align the MD5 configuration if in doubt. |
...
| 3 | **Duplicate ASN** | Your BGP ASN must differ from the OVHcloud-side region ASN (65501 Europe, 65502 Canada, 65519 Asia). Use a private ASN in the 64512–65534 range. |
