...
| 2 | **Auto-negotiation enabled** | Auto-negotiation is **not supported** on OVHcloud Connect. Disable it on your router (see below). |
...
Disable autonegotiation on your router's interface, following your vendor's documentation. The port speed must be forced to match the ordered link (1 Gbps or 10 Gbps).
...
| 2 | **Incorrect ASN** | Your ASN and the OVHcloud-side ASN must both be set correctly, must differ from each other, and must not use the reserved values (**65501** Europe, **65502** Canada, **65519** Asia); any other ASN is allowed. |
| 3 | **VLAN ID mismatch** | The VLAN tag on your interface must match the VLAN ID configured in the OVHcloud Control Panel PoP configuration. Verify this on your router. |
...
| # | Possible cause | Solution |
|---|---|---|
| 1 | **Missing `network` statement or export policy** | On your router, ensure you are advertising the correct prefixes from your router. See [Configure OCC L3 with BGP](/guides/network/ovhcloud-connect/l3-bgp). |
...
| # | Possible cause | Solution |
|---|---|---|
| 1 | **Unstable physical link** | Check the interface error counters on your router for CRC errors, input errors, or output drops. Inspect the SFP, cross-connect cable, and patch panel. |
| 2 | **MTU mismatch** | An MTU mismatch between your equipment and OVHcloud can cause large BGP update packets to be dropped. Jumbo frames up to **9000 bytes** are supported; make sure the MTU is consistent on both sides. |
| 3 | **BGP hold timer expiry** | If keepalives are lost due to link instability, the BGP session drops when the hold timer expires. Consider adjusting BGP timers, but fix the root cause first. |
...
| 3 | **Interface errors (CRC, FCS, drops)** | Check the interface error counters on your router. Physical issues (dirty fibre, bad SFP, loose connector) cause retransmissions and degrade performance. |
...
| 5 | **Congestion at the provider** | If using OVHcloud Connect Provider, check your provider's portal for utilisation and alerts on the virtual circuit / connection. |
...
You have ordered an OVHcloud Connect Provider service, but the connection through your provider is not coming up.

### Possible causes and solutions

| # | Possible cause | Solution |
|---|---|---|
| 1 | **Incorrect service key** | Verify that the service key entered in your provider's portal exactly matches the key from the OVHcloud Control Panel. |
| 2 | **Provider virtual circuit not provisioned** | Check the status of the virtual circuit / connection to OVHcloud in your provider's portal. It should show as "Active" or "Provisioned". |
...
:::tip
**Tip:** If the provider portal shows the connection as "Active" but OVHcloud still shows it as "Pending", wait a few minutes for the provisioning to complete. If the status does not update after a reasonable delay, contact OVHcloud support.
:::
...
| 3 | **Provider circuit not yet linked (Provider)** | Ensure the provider virtual circuit is provisioned and linked to the OVHcloud service using the correct service key. |
...
| 3 | **BFD not detecting the failure** | **BFD (Bidirectional Forwarding Detection)** is enabled by default on all BGP sessions and reduces failover time to under 1 second. Make sure BFD is also enabled and correctly configured on your own router so both sides can detect the failure. |
...
On your router, verify that both BGP sessions are `Established` and that routes are received from both peers with different attributes (Local Preference, AS-path length).
...
| 3 | **Duplicate ASN** | Your ASN and the OVHcloud-side ASN must both be set correctly, must differ from each other, and must not use the reserved values (65501 Europe, 65502 Canada, 65519 Asia); any other ASN is allowed. |
...
2. Start a new support request and select your **OVHcloud Connect** service as the affected service.
3. Include:
