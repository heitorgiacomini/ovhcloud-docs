...
|---|---|---|
| resourceId in POST /config/pop response is the new popId | 97 | Response-body field mapping not captured in SOT api.json (routes only); needs live API call |
| Example verify response fields id/interfaceId/customerBgpArea/ovhBgpArea/subnet/status | 108–116 | PoP GET response schema not in SOT cache; illustrative — needs live API confirmation |
| Extra (BGP) response fields id/type/bgpNeighborArea/bgpNeighborIp/nextHop/subnet/status | 171–179 | Extra GET response schema not in SOT cache; needs live API confirmation |
| Diagnostic enum names: diagPeering, diagPeeringExtra, diagRoutes, diagMacs | 214 | Not in api.json, manager-beta, or docs; previous diagnostics guide uses Default/Routes/Advertised-Routes/MAC Address — enum spellings need live API/PM validation |

## Summary
- Category 1: 25 · Category 2: 12 · Category 3: 4
- Notes: Well-grounded guide — the CP procedure, ASN rules, IP/subnet rules, VRRP-vs-BGP interaction, BFD, ECMP, and MED/AS-path facts all carry over cleanly and match the authoritative facts. All API routes exist in api.json. The only category-3 items are API response-body example schemas (SOT cache stores routes, not schemas) and the diagnostic enum spellings, which diverge from the previous diagnostics guide's UI labels and need live confirmation.
