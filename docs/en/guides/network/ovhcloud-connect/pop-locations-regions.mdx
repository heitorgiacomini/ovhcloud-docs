...
description: 'Understand how OCC works with PoPs and Regions, and find out which ones are the most suitable for your infrastructure'
...
:::info
OVHcloud regularly adds new locations. Always check the [OVHcloud Connect webpage](/links/network/ovhcloud-connect) for the most up-to-date list.
:::

## How to choose a PoP

When selecting a PoP for your OVHcloud Connect service, consider the following:
...
- [On-premises](/guides/network/ovhcloud-connect/resilient-architecture#on-premises)
...
The following tables list the regions accessible from each PoP and the corresponding low-latency OVHcloud region, by geographic zone.

:::info
The low-latency region shown for each PoP is indicative. Region reachability depends on the PoP's zone, so treat these single-region mappings as guidance rather than a guarantee.
:::
...
| City | PoP | Low-latency OVHcloud Region |
| :--- | :--- | :--- |
| **Frankfurt** | Equinix - FR5 | Limburg (`eu-west-lim`) |
| **Lille** | ETIX - ETX2 | Roubaix (`eu-west-rbx`) |
| **London** | Equinix - LD5 | Erith (`eu-west-eri`) |
| **London** | Telehouse - West | Erith (`eu-west-eri`) |
| **Madrid** | Digital Realty - MAD2 | - |
| **Paris** | Equinix - PA3 | Paris (`eu-west-par`) |
| **Paris** | GlobalSwitch | Paris (`eu-west-par`) |
| **Paris** | Telehouse - TH2 | Paris (`eu-west-par`) |
| **Warsaw** | Equinix - WA2 | Warsaw (`eu-central-waw`) |
...
- Understand [Multi-AZ](/guides/network/ovhcloud-connect/multi-az) for resilient architectures
- Review [Configure OCC L3 with BGP](/guides/network/ovhcloud-connect/l3-bgp) for technical setup
