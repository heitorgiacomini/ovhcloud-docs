...
2. **Configure BGP on both links** with appropriate route priorities (using BGP attributes such as MED and/or AS-path prepending for path selection) so that traffic prefers one path but can fall back to the other.
...
- **AS-path prepending** — Make the backup path's AS-path longer so it is less preferred. AS-path prepending is only configurable on your own devices, not on the OVHcloud side.
- **MED** — The OVHcloud-side alternative to AS-path prepending for active/passive path selection with two PoPs (a lower MED is preferred). AS-path prepending itself is customer-side only.
...
| Regulated / compliance workloads | **Multi-AZ recommended** |
