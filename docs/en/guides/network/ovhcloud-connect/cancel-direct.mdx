...
An OVHcloud Connect Direct service renews automatically until you disable auto-renewal or request cancellation, so stopping it is a deliberate action. Cancelling the subscription ends the OVHcloud side of the service, but the physical cross-connect at the data centre remains yours to decommission separately with the PoP operator. This guide covers both parts and the clean-up needed to fully retire the connection.
...
- **Committed term** — You commit to the initial term selected when ordering, after which the service renews automatically. Cancelling stops the renewal; the service is not terminated mid-term.
- **Cross-connect removal** — The physical cross-connect in the data centre needs to be decommissioned separately.
- **Effective date** — Cancelling (disabling auto-renewal) takes effect at the **end of the current term**: the service keeps running until its expiration date, then stops.
...
:::info
To prevent the next renewal, cancel at least **24 hours before the end of the current term**. The service then stops at the end of that term (its expiration date), not immediately.
:::
