...
| **Dual link** | Two independent local loops, delivered by two different providers to two different PoPs. | Resilient connectivity for critical sites that must survive the loss of one access. |
...
A **single link** uses one local loop to one OVHcloud PoP. A **dual link** uses two independent local loops, delivered by two different providers to two different PoPs, so that the failure of one access does not interrupt connectivity. A dual link requires that your site is eligible with two distinct providers.
...
OVHcloud Connect Edge is offered in two service tiers.

{/* DRAFT: To verify — confirm the SLA percentages and restoration-time commitments per tier before GA. */}

| Tier | Availability SLA | Guaranteed restoration time (GTR) | Support |
|---|---|---|---|
| **Basic** | 99.9% | 4 hours, business hours | Full support during setup, then business-hours support. |
| **Advanced** | 99.95% | 4 hours, 24/7 | Full support during setup, then 24/7 support. |

### uCPE capabilities

The optional uCPE provides the following, managed by OVHcloud:

- Automatic BGP session with OVHcloud Connect Edge.
