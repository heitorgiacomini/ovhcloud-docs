...
<img className="thumbnail" alt="Reference topology <a name=reference-topology></a> diagram" src="/images/network/load-balancer-revamp/architecture-resilient-multi-zone/fig-1.svg" loading="lazy" />
...
For an automated, repeatable deployment of this topology, refer to [Automation](/guides/network/load-balancer-revamp/automation). The OVHcloud Terraform provider can manage the service and its farms and servers (for example `ovh_iploadbalancing_http_farm` and `ovh_iploadbalancing_http_farm_server`). Frontends and applying the configuration are handled through the API or the Control Panel, not Terraform.
