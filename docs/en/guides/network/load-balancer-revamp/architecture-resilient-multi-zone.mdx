---
title: Deploy a resilient multi-zone architecture on a Load Balancer
...
<img className="thumbnail" alt="Reference topology diagram" src="/images/network/load-balancer-revamp/architecture-resilient-multi-zone/fig-1.svg" loading="lazy" />
...
For an automated, repeatable deployment of this topology, refer to [Automation](/guides/network/load-balancer-revamp/automation). The OVHcloud Terraform provider can manage the service, its farms and servers (for example `ovh_iploadbalancing_http_farm` and `ovh_iploadbalancing_http_farm_server`), the per-zone frontends (`ovh_iploadbalancing_http_frontend`, `ovh_iploadbalancing_tcp_frontend`, `ovh_iploadbalancing_udp_frontend`), and applying the staged configuration with `ovh_iploadbalancing_refresh`. You can still apply the configuration through the API or the Control Panel instead.
