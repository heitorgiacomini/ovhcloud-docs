---
title: Configure the vRack on a Load Balancer
...
:::warning
vRack support depends on your Load Balancer offer. If the offer does not support vRack, the <code className="action">Network</code> tab displays "vRack is not supported for this load balancer offer." and the vRack steps in this guide cannot be completed.
:::
...
You attach the vRack through the OVHcloud API and configure the private networks from the <code className="action">Network</code> tab in the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. The same tab lets you review and manage the private subnets once they exist.
...
Three pieces of configuration connect the Load Balancer to your private backends. First, attach the Load Balancer service to the vRack. Second, declare a private network on the Load Balancer from the <code className="action">Network</code> tab: this tells it which subnet your servers live in and which NAT IP range it may use as the source of backend traffic. Third, bind each server farm to that private network so its servers are addressed by their private IPs.
...
You can also attach a vRack from the <code className="action">Network</code> tab in the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> with the <code className="action">Attach vRack</code> button, and remove it later with <code className="action">Detach</code>.
...
Go to the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> and open your Load Balancer service. Select the <code className="action">Network</code> tab, then click <code className="action">Add Private Network</code> and fill in the following fields:
...
Go to the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, open your Load Balancer service, and select the <code className="action">Farms</code> tab. Click <code className="action">Create farm</code> and configure the farm as usual, selecting the private network you created so that backend servers are addressed by their private IP.
...
:::info
Apply the staged configuration with the `ovh_iploadbalancing_refresh` resource (arguments `service_name` and `keepers`). After your Terraform changes, you can also apply the pending configuration through the OVHcloud API or the Control Panel as described below.
:::
