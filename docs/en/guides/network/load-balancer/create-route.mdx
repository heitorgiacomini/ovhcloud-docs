...
description: Find out how to dynamically direct your requests to a specific farm
...
- Have an [OVHcloud Load Balancer](https://www.ovhcloud.com/en-gb/network/load-balancer/) on an offer allowing the creation of routes.
...
Route management is only available through the [OVHcloud API](https://eu.api.ovh.com/). It is only valid for the **http** and **tcp** protocols, and the path <code className="action">/ipLoadbalancing/&#123;serviceName&#125;/&#123;protocol&#125;/route/</code> exposes the dedicated route API.
...
:::info
To display only the API calls related to routes in the OVHcloud API console, you can use the <code className="action">filter</code> field with the keyword "[a-z]*".
:::
