...
description: Integrate your TCP services behind a Load Balancer with ProxyProtocol
...
If you are using an HTTP frontend, we recommend the guide "[Configuring an OVHcloud Load Balancer service - HTTP Headers](/guides/network/load-balancer/create-headers)" which describes the standard way to use HTTP headers to retrieve the IP address, port, and source protocol.
...
<img className="thumbnail" alt="Outbound IPv4 address of your OVHcloud Load Balancer service" src="/images/network/load-balancer/create-proxyprotocol/iplb_service.png" loading="lazy" />
...
In the <code className="action">Clusters</code> section, select the farm containing the server on which to enable ProxyProtocol, then click the edit button for the relevant server.

ProxyProtocol is configured via the <code className="action">ProxyProtocol version</code> option. You will find the 4 modes described above.

<img className="thumbnail" alt="Activating ProxyProtocol on a server in a farm" src="/images/network/load-balancer/create-proxyprotocol/edit_server.png" loading="lazy" />

Once the desired mode is selected, click <code className="action">Update</code>, then <code className="action">Deploy zone: YOUR ZONE</code> to apply your changes in the relevant zone.
...
- Modify an existing <code className="action">Server</code>:
