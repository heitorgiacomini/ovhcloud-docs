...
To order an IP block for your **Hosted Private Cloud**, go to the OVHcloud Control Panel. In the `Hosted Private Cloud` section, click on the `IP` section in the left-hand column, then click <code className="action">Order additional IP addresses</code>. Then select your **Hosted Private Cloud** in the dropdown menu before moving on to the next step.
...
:::info
Please feel free to refer to our guide on the [OVHcloud Network plugin](/guides/hosted-private-cloud/powered-by-vmware/plugin-ovh-network) to find out which IPs are reserved for your block, as well as how they are used.
...
<img className="thumbnail" alt="nexthop field" src="/images/hosted-private-cloud/powered-by-vmware/ajout-de-bloc-ip/move-api.png" loading="lazy" />

The result should look like this:

<img className="thumbnail" alt="nexthop field" src="/images/hosted-private-cloud/powered-by-vmware/ajout-de-bloc-ip/api-result.png" loading="lazy" />
...
You can track the movement of your IP block via the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> in the <code className="action">Hosted Private Cloud</code> section, then <code className="action">Private Cloud</code>. Click on your Hosted Private Cloud service, then on the <code className="action">Operations</code> tab.
...
The IP will then appear in the <code className="action">IP parking</code>.
