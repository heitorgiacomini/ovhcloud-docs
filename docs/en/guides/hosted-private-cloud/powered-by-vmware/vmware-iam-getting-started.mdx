...
description: Find out how IAM works with vSphere
...
- One or more [Hosted Private Cloud products - VMware on OVHcloud](https://www.ovhcloud.com/en-gb/hosted-private-cloud/vmware/) linked to this account
...
An IAM role can only be added through manual actions in a global policy (action: assumerole -> role_iam). For more information, see the guide "[How to create an IAM role in vSphere](/guides/hosted-private-cloud/powered-by-vmware/vmware-iam-role-policy)".


### Can I activate IAM easily?

Yes, you can activate IAM via a single button in the OVHcloud Control Panel. For more information, see the guide [How to enable IAM](/guides/hosted-private-cloud/powered-by-vmware/vmware-iam-activation).

When IAM is disabled in your Control Panel:

<img className="thumbnail" alt="IAM Activation Not Enabled" src="/images/hosted-private-cloud/powered-by-vmware/vmware-iam-getting-started/iam_enabled.png" loading="lazy" />
...
**Associated identities**, **Resources**, **Resource groups** and their permissions in policies are managed from <ManagerLink to="/">the OVHcloud Control Panel</ManagerLink>.<br/>
Click on your name in the top right-hand corner of the OVHcloud Control Panel, then click on your initials to go to the <code className="action">My account</code> section.<br/>
Under <code className="action">My account</code>, click `Identities and access (IAM)`&#123;.action].

You can manage IAM roles and local vSphere users in the <code className="action">Hosted Private Cloud</code> section of the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>.
...
You can now follow the steps in the guide [IAM for VMware on OVHcloud - How to activate IAM](/guides/hosted-private-cloud/powered-by-vmware/vmware-iam-activation).
