...
description: Learn how to enable Private Gateway on a Hosted Private Cloud infrastructure
...
* Being connected to [OVHcloud API](https://eu.api.ovh.com/).
* Creating [OVHcloud API credentials](/guides/manage-and-operate/api/first-steps).
...
* Having added the private gateway network to [restrictions per source IP](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud).
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/privateGateway/enable"} />
...
<Api version="v1" section="/dedicatedCloud" method="POST" route={"/dedicatedCloud/\{serviceName\}/datacenter/\{datacenterId\}/privateGateway/disable"} />
