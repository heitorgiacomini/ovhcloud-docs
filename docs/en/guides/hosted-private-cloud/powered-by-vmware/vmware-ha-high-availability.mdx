...
description: Manage restart policy with HA function
...
<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HA3.png" loading="lazy" />
...
If HA is not enabled in your cluster, go to the <code className="action">Configure</code> tab of your cluster and then to the <code className="action">vSphere Availability</code> tab available in <code className="action">Services</code>.

Click <code className="action">Edit</code> and slide the cursor to enable the HA feature.
...
Disabling it is required to perform update operations with [Update Manager](/guides/hosted-private-cloud/powered-by-vmware/use-vmware-update-manager).

<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HA.png" loading="lazy" />
...
Default restart management on the cluster is also possible. You can refine this by virtual machine in the tab <code className="action">VM Overrides</code>.
...
<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HAparam1.PNG" loading="lazy" />
...
<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HAparam2.PNG" loading="lazy" />
...
<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HAparam3.PNG" loading="lazy" />
...
<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HAparam4.PNG" loading="lazy" />

##### VM Monitoring

VM monitoring is available after [VMware tools](/guides/hosted-private-cloud/powered-by-vmware/vmware-tools-install) is installed, in case of non-response via the **tools** (heartbeat signals) the virtual machine will be automatically restarted. Advanced configuration is possible for this feature (for example, reboot interval).

<img className="thumbnail" alt="" src="/images/hosted-private-cloud/powered-by-vmware/vmware-ha-high-availability/HAparam5.PNG" loading="lazy" />
...
In the <code className="action">configuration</code> section and then in the <code className="action">VM/Host Rules</code> tab, you can create a rule of type "VMs".
...
This rule can be added to the configurable restart priorities in the <code className="action">VM Overrides</code>.
