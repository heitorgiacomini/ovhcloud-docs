...
description: Find out how to postpone a planned maintenance on your VMware on OVHcloud solution
...
:::tip
Messages sent by OVHcloud are available in your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> as well.<br/>
Click on your name in the top right corner, then click on <code className="action">Service emails</code> in the menu on the right.
...
Log in to the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> with an administrator account.

In the <code className="action">Hosted Private Cloud</code> section, select your service and click on the <code className="action">Operations</code> tab. Select <code className="action">Available</code> from the drop-down menu to filter the operations.

Next, click on the <code className="action">...</code> button and select <code className="action">Modify the processing date</code>.

<img className="thumbnail" alt="change maintenance time" src="/images/hosted-private-cloud/powered-by-vmware/maintenance-rescheduling/maintenance-date-edition01.png" loading="lazy" />

:::info
If the option <code className="action">Modify the processing date</code> is greyed out, this maintenance cannot be postponed.
...
Finally, click the <code className="action">Modify</code> button to confirm your changes.

<img className="thumbnail" alt="change maintenance time" src="/images/hosted-private-cloud/powered-by-vmware/maintenance-rescheduling/maintenance-date-edition02.png" loading="lazy" />

### Using the OVHcloud API

Log in to the [API service administration console](https://eu.api.ovh.com/). You can refer to our guide on [Getting started with the OVHcloud API](/guides/manage-and-operate/api/first-steps).
