...
description: Learn how to migrate your on-premises VMware workloads to an OVHcloud Hosted Private Cloud environment using Zerto Virtual Replication
...
[Move2Cloud - Migrating VMware Workloads to SecNumCloud Hosted Private Cloud with Zerto](/guides/hosted-private-cloud/powered-by-vmware/vmware-migration-zerto-secnumcloud)
...
You can find more about network planning in [NSX-T - First steps](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps).
...
- Migrate your existing IP ranges using the [Bring Your Own IP (BYOIP)](https://www.ovhcloud.com/en-gb/network/byoip/) feature.
...
2. Select your <code className="action">Hosted Private Cloud</code>.
3. Navigate to the <code className="action">Security</code> tab.
4. Click <code className="action">Add a new IP address range</code> to authorize your source infrastructure IPs and Zerto components.

For step-by-step instructions, refer to [Authorise IPs to connect to vCenter](/guides/hosted-private-cloud/powered-by-vmware/authorise-ip-vcenter-access).
...
To do so, follow [Connect OVHcloud IAM to ADFS](/guides/account-and-service-management/account-information/ovhcloud-account-connect-saml-adfs).
...
Refer to [NSX-T – First steps](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps) for details on creating segments and assigning them to VMs.
...
You can find an overview of how NSX handles these features in [NSX-T – First steps](/guides/hosted-private-cloud/powered-by-vmware/nsx-first-steps).
...
In your <code className="action">Hosted Private Cloud</code> interface:

1. Go to <code className="action">Disaster Recovery</code>.
2. Select <code className="action">Enable Zerto Virtual Replication</code>.
...
Full details can be found in [Zerto Virtual Replication on OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-customer-to-ovh).
...
Details and parameters are available in [Zerto VPN Setup on OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-customer-to-ovh).
...
Use <code className="action">Storage vMotion</code> via the <code className="action">vSphere Client</code>:

1. Right-click on the VM > <code className="action">Migrate</code>.
2. Select <code className="action">Change storage only</code>.
3. Choose the destination datastore or vSAN policy.

See [VMware Storage vMotion](/guides/hosted-private-cloud/powered-by-vmware/storage-vmotion) for full details.
