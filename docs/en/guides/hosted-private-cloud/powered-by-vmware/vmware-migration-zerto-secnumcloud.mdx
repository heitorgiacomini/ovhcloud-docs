...
description: Learn how to migrate on-premises VMware workloads to a SecNumCloud-qualified Hosted Private Cloud using Zerto Virtual Replication
...
:::info
The [Bring Your Own IP (BYOIP)](https://www.ovhcloud.com/en-gb/network/byoip/) feature is not available in SecNumCloud environments.
...
Access to <code className="action">vCenter</code> is restricted by default in SecNumCloud environments.

You must request access from OVHcloud Support.

1. Open a support ticket from the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>.
2. Select your <code className="action">Hosted Private Cloud</code>.
3. Provide the list of source IPs that require access.

For step-by-step instructions, refer to [Authorize IPs to connect to vCenter](/guides/hosted-private-cloud/powered-by-vmware/authorise-ip-vcenter-access).
...
[Enabling secure access to the vSphere interface](/guides/hosted-private-cloud/powered-by-vmware/interface-secure)
...
1. In the <code className="action">Hosted Private Cloud</code> interface, open a support ticket.
...
Full details: [Zerto Virtual Replication on OVHcloud](/guides/hosted-private-cloud/powered-by-vmware/zerto-virtual-replication-customer-to-ovh)
