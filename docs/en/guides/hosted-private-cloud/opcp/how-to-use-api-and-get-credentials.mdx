...
- Be an administrator of the [OPCP](https://www.ovhcloud.com/en-gb/hosted-private-cloud/onprem-cloud-platform/) infrastructure and have access to the administration interface (admin.dashboard).
...
- Go to the `Clients` section and click on <code className="action">Create a client</code>.  
...
- Click on <code className="action">Next</code>, then on <code className="action">Save</code>.
...
- Click on <code className="action">Configure a new mapper</code>.
