...
:::warning
A sub-port's network must be **different** from the parent port's network. The sub-port inherits the parent port's MAC address (the guest OS vlan sub-interface inherits it too, which is required for DHCP). Since two ports on the same network cannot share a MAC address, a sub-port created on the parent port's network causes the node deployment to fail.
:::
