---
title: How to set up Trunk ports on a Node
description: Learn how to configure Neutron Trunk ports in OPCP for multi-network vlan connectivity on bare metal or virtual machine instances
lastUpdated: 2026-08-24
---

## Objective

Trunk ports allow a single instance (bare metal or virtual machine) to send and receive traffic on multiple Neutron networks using vlan tagging, through a single physical interface or an [LACP bond](/guides/hosted-private-cloud/opcp/how-to-setup-lacp-on-node).
...
- An active [OPCP service](https://www.ovhcloud.com/en-gb/hosted-private-cloud/onprem-cloud-platform/).
- **[Configured OpenStack CLI access](/guides/hosted-private-cloud/opcp/how-to-use-api-and-get-credentials)** with the necessary permissions (`clouds.yaml` or environment variables).
...
:::warning
A sub-port's network must be **different** from the parent port's network. The sub-port inherits the parent port's MAC address, and so does the guest OS vlan sub-interface, which is required for DHCP. Since two ports on the same network cannot share a MAC address, a sub-port created on the parent port's network causes the instance deployment to fail.
:::
...
:::info
If your instance uses LACP bonding (see [LACP guide](/guides/hosted-private-cloud/opcp/how-to-setup-lacp-on-node)), replace `ens3` with your bond interface name (e.g., `bond0`). The vlan sub-interfaces then become `bond0.100`, `bond0.200`, etc.
:::
