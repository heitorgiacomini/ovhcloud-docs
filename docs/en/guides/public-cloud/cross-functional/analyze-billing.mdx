...
This system is similar to the one used by certain telephone operators, where they bill at the end of the month for the number of minutes used. Here, OVHcloud bills for the exact amount of time — down to the second — for which the server, storage space or another element of the service is used.
...
Our goal is to provide a billing system that matches the way our customers use this service as closely as possible. To achieve this, we needed to make the billing granular. This is why the unit system is the amount of time, counted in seconds.

For each resource to be billed, a usage counter (in seconds) starts when the resource is created, and stops when the resource is removed. This works in accordance with the principle of every second of use being billed beyond an initial 1-minute minimum — usage is never rounded up to the next hour.
...
At the end of the month, each counter is converted into the corresponding cost for the resource. To get the overall bill total, simply add up all of the counters charged for. Prices are usually displayed as an hourly rate for readability, but the actual billing granularity is per second.
...
Because usage is metered to the second (beyond an initial 1-minute minimum), shorter usage periods are billed just as precisely: for example, if you use a resource for only 20 minutes, you are billed only for those 20 minutes — never rounded up to a full hour.
