...
description: Learn how to use S3* compatible Object Storage buckets with AI Tools
...
- A user account created on this Public Cloud project, with the **AI** and **Object Storage** roles assigned. For more information on how to create such a user, please consult the [Manage AI users and roles](/guides/public-cloud/ai-machine-learning/ai-users) documentation.
- The `ovhai` CLI installed. Please refer to our guide on [how to install ovhai CLI](/guides/public-cloud/ai-machine-learning/ai-cli-install-client) for assistance.
...
To create your first **S3 compatible Object Storage bucket**, click <ManagerLink to="/#/pci/projects">this link</ManagerLink> to access your Public Cloud project, then click the <code className="action">Object Storage</code> button in the left-hand side bar, in the <code className="action">Storage</code> section:

![S3 compatible bucket creation](/images/public-cloud/ai-machine-learning/gi-08-s3-compliance/s3-bucket-object-storage.png)

This should take you back to the page shown above, where you can see all the buckets you have already created on your Public Cloud project. By going to the <code className="action">My Containers</code> section, where you are by default, you can create a new bucket by clicking the <code className="action">Create an object container</code> button, just above the bucket list.

**If you haven't created a bucket yet**, the interface will be slightly different, but will also prompt you to <code className="action">Create an object container</code>.
...
Then, specify the deployment mode you want to use (*3-AZ*, *1-AZ* or *Local Zone*). Find out more by reading the [deployment modes comparison](/guides/storage-and-backup/object-storage/s3-regions-comparison):
...
Once you have named your bucket and configured its settings, click the <code className="action">Create the container</code> button to confirm S3 compatible bucket creation.
...
To do that, click on <code className="action">Users & Roles</code> in the <code className="action">Project management</code> category, in the OVHcloud Control Panel (UI) left-hand side bar.
...
For more information about editing user rights, you can check the [dedicated documentation](/guides/public-cloud/ai-machine-learning/ai-users).
...
If you have lost the password associated with this username, you can **regenerate it** by clicking the <code className="action">...</code>, then on <code className="action">Generate a password</code>, which will revoke the old one.

![user credentials](/images/public-cloud/ai-machine-learning/gi-08-s3-compliance/s3-user-retrieve-credentials.png)

Then, you need to retrieve the **access key** and **secret key** associated to this user. To do that, return to the <code className="action">Object Storage</code> section. By default, you will redirected to the <code className="action">My containers</code> section. Click on the <code className="action">Object Storage Users</code> category. Here you will find the **access key** for each of your existing users. You can also view the **secret key** by clicking the <code className="action">...</code> button, then on <code className="action">view the secret key</code>, as showed on the screnshot below:
...
To grant access to your bucket for your user, click the <code className="action">...</code> button located next to your bucket name. Then, click the `Add a user to my container` option and select the same user you used during bucket creation.
...
Now that your bucket has been created, and the associated user has the necessary permissions, you can move on to the `ovhai` CLI. If you have not installed it yet, please refer to the [ovhai CLI installation guide](/guides/public-cloud/ai-machine-learning/ai-cli-install-client).
...
For more information on storage classes, please consult this [documentation](/guides/storage-and-backup/object-storage/s3-choosing-the-right-storage-class-for-your-needs).
...
<Tabs>
  <Tab label="Using the Control Panel (UI)">
    Go to the <code className="action">Object Storage</code> section (in the Storage category) and click on your bucket name. Then, click the <code className="action">+ Add objects</code> button:
    
    ![S3 compatible bucket upload](/images/public-cloud/ai-machine-learning/gi-08-s3-compliance/s3-bucket-upload.png)
    
    Select the files you want to upload to your bucket. If needed, you can prefix their names. Then, choose between Standard or High performance storage (find more information about storage classes [here](/guides/storage-and-backup/object-storage/s3-choosing-the-right-storage-class-for-your-needs)). Once you have made your selection, confirm by clicking on `Import`.
...
<Tabs>
  <Tab label="AI Notebooks">
    ...
    You can also consult the dedicated [CLI - Launch an AI notebook documentation](/guides/public-cloud/ai-machine-learning/ai-cli-run-notebook) to know more about the possible options, frameworks, and editors available.
  </Tab>
  <Tab label="AI Training">
    ...
    You can also consult the dedicated [CLI - Launch an AI Training job documentation](/guides/public-cloud/ai-machine-learning/ai-cli-run-job) to know more about the possible job commands.
  </Tab>
  <Tab label="AI Deploy">
    ...
    You can also consult the dedicated [CLI - Launch an AI Deploy app documentation](/guides/public-cloud/ai-machine-learning/ai-cli-deploy-app) to know more about the possible job commands.
  </Tab>
</Tabs>
