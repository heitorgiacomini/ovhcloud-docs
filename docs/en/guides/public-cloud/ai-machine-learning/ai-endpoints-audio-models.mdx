...
description: Learn how to transcribe audio files with OVHcloud AI Endpoints
...
This documentation provides an overview of the Speech to Text endpoints offered on [AI Endpoints](https://www.ovhcloud.com/en-gb/public-cloud/ai-endpoints/). 

Visit our [Catalog](https://www.ovhcloud.com/en-gb/public-cloud/ai-endpoints/catalog/) to find out which models are compatible with Audio Analysis.
...
Follow the instructions in the [AI Endpoints - Getting Started](/guides/public-cloud/ai-machine-learning/ai-endpoints-getting-started) guide for more information on authentication.
...
| **chunking_strategy**        | No       | `string`/`server_vad object`/`null`   | -                                                                                     | null    | Controls how audio is segmented before transcription. Setting it speeds up transcription of longer files (around 1.5× from a couple of minutes of audio, up to ~2× on long audio recordings) while keeping quality very close to the default. Recommended for any file longer than ~30 seconds. More details [here](#chunking-strategy).                                                                                                                                                     |
...
| **model**                    | No       | `string`/`null`   | ID of the model to use                                                                | -       | Specifies the model to use for transcription. Useful when using our [unified endpoint](/guides/public-cloud/ai-machine-learning/ai-endpoints-virtual-models).                                                                                                                                                            |
...
<Tabs>
  ...
  <Tab label="cURL">
    ...
    To [**authenticate with your API key**](/guides/public-cloud/ai-machine-learning/ai-endpoints-getting-started), add an Authorization header:
...
<Tabs>
  <Tab label="Correcting acronyms and names">
  ...
  <Tab label="Translating transcript into English">
  ...
  <Tab label="Maintaining context across segments">
  ...
  <Tab label="Preserving filler words">
  ...
  <Tab label="Enforcing writing style">
...
<Tabs>
  <Tab label="Segment-level timestamps">
  ...
  <Tab label="Word-level timestamps">
  ...
  <Tab label="Word and segment timestamps">
...
<Tabs>
  <Tab label="Verbose JSON (default)">
...
Setting `chunking_strategy` splits the audio with voice activity detection (VAD) and transcribes the resulting segments in batches. This **speeds up transcription of longer files** (around **1.5× from a couple of minutes of audio, growing to about 2× on long recordings**), while keeping transcription quality **very close** to single block mode. Chunking brings no speed gain on very short files (under ~30 seconds).

This parameter supports two values, `auto` and `server_vad`.
...
- **Authenticated requests** (using an [API key](/guides/public-cloud/ai-machine-learning/ai-endpoints-getting-started)): Up to `2048 MB` or `10 800 seconds` of audio per request.
...
In this guide, we have explained how to use Speech to Text models available on [AI Endpoints](https://www.ovhcloud.com/en-gb/public-cloud/ai-endpoints/) models. We have provided a comprehensive overview of the feature which can help you perfect your integration of model for your own application.

## Go Further

Browse the full [AI Endpoints documentation](/guides/public-cloud/ai-machine-learning/overview) to further understand the main concepts and get started.
