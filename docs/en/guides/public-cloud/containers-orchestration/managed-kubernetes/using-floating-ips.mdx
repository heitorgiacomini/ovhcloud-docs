...
Floating IPs are billed per second according to OVHcloud Public Cloud pricing.
...
:::info
Floating IPs are charged per IP address for the exact time they are assigned to a node, with a minimum billing period of 1 minute. They are automatically deleted (and billing stops) when nodes are deleted.
:::
...
- <ApiLink>OVHcloud API console</ApiLink>: Explore the full OVHcloud API.
