...
description: Find out how to configure a multi-attach persistent volume using OVHcloud NAS-HA
...
OVHcloud Managed Kubernetes natively integrates Block Storage as persistent volumes. This technology may however not be suited to some legacy or non cloud-native applications, often requiring to share this persistent data accross different pods on multiple worker nodes (ReadWriteMany or RWX). If you would need to do this for some of your workloads, one solution is to use NFS volumes. [OVHcloud NAS-HA](https://www.ovhcloud.com/en-gb/storage-solutions/nas-ha/) is a managed solution that lets you easily configure an NFS server and multiple NFS volumes. In this tutorial we are going to see how to configure your OVHcloud Managed Kubernetes cluster to use [OVHcloud NAS-HA](https://www.ovhcloud.com/en-gb/storage-solutions/nas-ha/) as an NFS provider for [Kubernetes Persistent Volumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/).
...
Click on your NAS, then on the <code className="action">Partitions</code> tab, then on the <code className="action">Create a partition</code> button and create the new NFS partition with the following content:

<img className="thumbnail" alt="Create an NFS partition" src="/images/public-cloud/containers-orchestration/managed-kubernetes/configuring-multi-attach-persistent-volumes-with-ovh-nas-ha/create-nfs-partition.png" loading="lazy" />
...
You can find more details about how to use OVHcloud APIs with this guide: [First Steps with the OVHcloud APIs](/guides/manage-and-operate/api/first-steps)
...
Click on the <code className="action">Manage Access</code> menu of our newly created partition:

<img className="thumbnail" alt="Manage Access of the NFS partition" src="/images/public-cloud/containers-orchestration/managed-kubernetes/configuring-multi-attach-persistent-volumes-with-ovh-nas-ha/manage-nfs-partition-access.png" loading="lazy" />

Add either the nodes' IPs one by one or the Gateway's Public IP depending on your configuration:

<img className="thumbnail" alt="Allow nodes IP to access the NFS partition" src="/images/public-cloud/containers-orchestration/managed-kubernetes/configuring-multi-attach-persistent-volumes-with-ovh-nas-ha/manage-nfs-partition-access-ip.png" loading="lazy" />

You should now have something similar to this:

<img className="thumbnail" alt="Zpool-configuration" src="/images/public-cloud/containers-orchestration/managed-kubernetes/configuring-multi-attach-persistent-volumes-with-ovh-nas-ha/manage-nfs-zpool.png" loading="lazy" />
