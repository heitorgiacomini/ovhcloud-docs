...
OVHcloud [vRack](https://www.ovhcloud.com/en-gb/network/vrack/) is a private network solution that enables our customers to route traffic between OVHcloud dedicated servers as well as other OVHcloud services. At the same time, it allows you to add Public Cloud instances and Managed Kubernetes clusters to your private network to create an infrastructure of physical and virtual resources.
...
**In this tutorial we are going to give you an example of how to use the OVHcloud [vRack](https://www.ovhcloud.com/en-gb/network/vrack/) to connect a Managed Kubernetes cluster with a Public Cloud instance inside the same private network.**
...
It also supposes that you already have followed the [Using vRack](/guides/public-cloud/containers-orchestration/managed-kubernetes/using-vrack) guide to activate the vRack on your Public Cloud project and put your OVHcloud Managed Kubernetes cluster inside the vRack.

## Instructions

### Setting up the vRack

First of all, we will need to set up vRack Private Network for our Public Cloud. To do it, we follow the [Configuring vRack for Public Cloud](/guides/public-cloud/network-services/vrack) guide. Once you have created a vRack and added it into a Private Network, you can continue. 

<img className="thumbnail" alt="Setting-up the vRack" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-01.png" loading="lazy" />

:::warning
As explained in the [Known limits](/guides/public-cloud/containers-orchestration/managed-kubernetes/known-limits) guide, the following subnets are not compliant with the `vRack` feature and can generate some incoherent behaviours with our used overlay networks:
:::
...
Then we create a Kubernetes cluster, as explained in the [Create a cluster](/guides/public-cloud/containers-orchestration/managed-kubernetes/create-cluster) guide. Integrating a cluster into a vRack Private Network must be done at the third step on cluster creation, when we can choose an existing private network for the cluster:

<img className="thumbnail" alt="Choose a private network for this cluster" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-02.png" loading="lazy" />
...
<img className="thumbnail" alt="Private network information in Attached network column" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-03.png" loading="lazy" />

### Setting up the PCI

Now we can create a new Public Cloud instance inside the vRack, by following the [Integrating an instance into vRack](/guides/public-cloud/network-services/vrack#step-3-integrating-an-instance-into-vrack) guide.

We are going to create an Ubuntu instance:

<img className="thumbnail" alt="Setting-up PCI - Ubuntu instance" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-04.png" loading="lazy" />

In the fourth step of creation, we attach it to the private network we created before:

<img className="thumbnail" alt="Setting-up PCI - Attaching to a private network" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-05.png" loading="lazy" />
...
<img className="thumbnail" alt="Setting-up PCI - SSH connection information" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-05-bis.png" loading="lazy" />
...
Now we have set up the database, we can deploy WordPress on the Kubernetes cluster. We are following a similar process as in our [Installing WordPress](/guides/public-cloud/containers-orchestration/managed-kubernetes/install-wordpress) tutorial, but adapting it to use an external database.
...
<img className="thumbnail" alt="Setting-up WordPress" src="/images/public-cloud/containers-orchestration/managed-kubernetes/vrack-example-k8s-and-pci/vrack-example-06.png" loading="lazy" />
