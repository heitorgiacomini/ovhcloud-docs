...
```bash
ovhcloud cloud managed-registry iam enable <registry_id> \
  --cloud-project <project_id>
```

:::warning
All existing Harbor users will be removed when IAM is enabled. Use `--delete-users` to explicitly confirm this, or omit it if the registry has no local users.
:::

**3. Disable IAM authentication:**

```bash
ovhcloud cloud managed-registry iam disable <registry_id> \
  --cloud-project <project_id>
```
