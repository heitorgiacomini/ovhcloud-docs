...
However, you can also configure an Open ID Connect (OIDC) provider, for the OVHcloud Managed Private registry authentication, such as Keycloak.
...
Several OpenID Connect providers exists like Dex, Keycloak, Okta or a SaaS provider. Harbor supports all of them. In this tutorial we will use Keycloak.
...
- an OIDC provider, like a Keycloak instance and credentials to access it.

Follow the official guide if you want to [install Keycloak on an OVHcloud Managed Kubernetes cluster](/guides/public-cloud/containers-orchestration/managed-kubernetes/install-keycloak).

## Instructions

### Configure the OIDC provider through the API
...
Do a POST HTTP request and fill the input fields with the Keycloak/OIDC provider information.
...
When you click the <code className="action">Login via OIDC provider</code> button, you are redirected to the Keycloak interface. 

<img className="thumbnail" alt="Keycloak login" src="/images/public-cloud/containers-orchestration/managed-private-registry/configure-oidc-provider-authentication/keycloack-login.png" loading="lazy" />
...
First, retrieve information about groups in Keycloak:

<img className="thumbnail" alt="Keycloak group" src="/images/public-cloud/containers-orchestration/managed-private-registry/configure-oidc-provider-authentication/keycloack_groups.png" loading="lazy" />

Check you have at least one member in the group:

<img className="thumbnail" alt="Keycloak group member" src="/images/public-cloud/containers-orchestration/managed-private-registry/configure-oidc-provider-authentication/keycloack_group_member.png" loading="lazy" />
