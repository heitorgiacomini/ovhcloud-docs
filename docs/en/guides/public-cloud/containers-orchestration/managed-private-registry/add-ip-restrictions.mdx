...
<img className="thumbnail" alt="Log in to the API Explorer" src="/images/public-cloud/containers-orchestration/managed-private-registry/add-ip-restrictions/kubernetes-quickstart-api-ovh-com-001.png" loading="lazy" />
...
<img className="thumbnail" alt="Kubernetes section of the API Explorer" src="/images/public-cloud/containers-orchestration/managed-private-registry/add-ip-restrictions/kubernetes-quickstart-api-ovh-com-002.png" loading="lazy" />

**Management endpoint (Harbor UI access)**
...
**Registry endpoint (Docker/Helm access)**
...
```bash
ovhcloud cloud managed-registry ip-restrictions management list <registry_id> \
...
```bash
ovhcloud cloud managed-registry ip-restrictions management add <registry_id> \
  --cloud-project <project_id> \
  --ip-block 192.0.2.0/24 \
  --description "production environment"
```
...
```bash
ovhcloud cloud managed-registry ip-restrictions management delete <registry_id> <ip_block_id> \
  --cloud-project <project_id>
```

After adding a restriction, an unauthorized IP will have an "UNAUTHENTICATED" error on the Harbor user interface.
...
```bash
ovhcloud cloud managed-registry ip-restrictions registry list <registry_id> \
  --cloud-project <project_id>
```
...
```bash
ovhcloud cloud managed-registry ip-restrictions registry add <registry_id> \
  --cloud-project <project_id> \
  --ip-block 192.0.1.0/24 \
  --description "Q/A environment"
```
...
```bash
ovhcloud cloud managed-registry ip-restrictions registry delete <registry_id> <ip_block_id> \
  --cloud-project <project_id>
```
