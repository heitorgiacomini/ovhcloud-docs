...
description: Find out how to install Prometheus Node Exporter or Windows Exporter on an OVHcloud Public Cloud instance to gather metrics
...
- [Administrative access to the instance](/guides/public-cloud/compute/getting-started)
...
:::info
On Windows, use PowerShell with SSH or an SSH client such as [PuTTY](/guides/web-cloud/web-hosting/ssh-using-putty-on-windows) if you prefer using a command-line interface.
...
<Tabs>
  <Tab label="For Debian/Ubuntu">
    ```bash
    sudo apt update && sudo apt upgrade -y
    ```
  </Tab>
  <Tab label="For CentOS/RHEL">
    ```bash
    sudo yum update -y
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab label="For Windows (via SSH/PowerShell on the VM)">
    :::info
...
<Tabs>
  ...
  <Tab label="For Windows (via SSH/PowerShell)">
    ```powershell
...
<Tabs>
  <Tab label="For Linux (Debian/Ubuntu with UFW)">
    ```bash
