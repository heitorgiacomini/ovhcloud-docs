...
description: Learn how to perform forensics on the disk(s) and/or the RAM of a Public Cloud instance in case of a security incident
...
- The [OpenStack CLI](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) ready to be used on your system.
- An [OpenStack user](/guides/public-cloud/cross-functional/create-and-delete-a-user) has been created with (at least) the _Compute Operator_ and _Backup Operator_ roles.
...
In order to recover the disk of a Public Cloud instance, the easiest way is to use the [OpenStack Command Line Interface](https://docs.openstack.org/newton/user-guide/common/cli-overview.html). If the tool is not installed on your system, you can read the [following guide](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) as well as [the guide to create an OpenStack user](/guides/public-cloud/cross-functional/create-and-delete-a-user).

#### Perform a backup

First, we are going to perform a backup of the Public Cloud instance. The operation can be done either [using the OVHcloud Control Panel](/guides/public-cloud/compute/save-an-instance) or using the OpenStack CLI. We are going to use the latter one.
...
You need to recover the `openrc.sh` file. In the <code className="action">Users & Roles</code> section of the left-hand menu under "Settings", click the <code className="action">...</code> button and then <code className="action">Download OpenStack's RC file</code>. A popup will ask you to choose the _Region_ the configuration file should be configured with. Pick the same _Region_ as the Public Cloud instance you want to investigate.
...
This [documentation](/guides/public-cloud/cross-functional/compute-set-openstack-environment-variables) can help you deal with the required environment variables on Windows.
:::
