...
description: Find out how to install Plesk on an OVHcloud Public Cloud instance
...
- [An instance created via the OVHcloud Control Panel](/guides/public-cloud/cross-functional/create-a-public-cloud-project)
- [Administrative access to the instance](/guides/public-cloud/compute/getting-started#connect-instance)
...
- A message states: "You can also log in as 'root' using your 'root' password.". However, by default, no root password is generated. If needed, customers can follow [this guide](/guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds) to enable the root user and set a password.
...
Want to change your licence, to replace a test key or change your solution, for example? From the Plesk interface, go to the section <code className="action">Tools & Settings</code>. Then go to the **Plesk** section, and select <code className="action">License information</code>.
