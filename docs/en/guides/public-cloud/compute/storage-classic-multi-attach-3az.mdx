...
description: Learn how to safely use Classic Multi-Attach Block Storage in 3AZ regions, understand its limitations, and avoid data corruption
...
To better understand the underlying architecture for multi-zone deployments using regional Block Storage, refer to the following [documentation](/guides/public-cloud/cross-functional/deployments-modes-reference-architecture#2az-with-regional-block-storage).
...
For a step-by-step OCFS2 setup example, see [Configure Classic Multi-Attach Block Storage with OCFS2](/guides/public-cloud/compute/storage-classic-multi-attach-ocfs2).
:::


## Usage

Creating and attaching a Classic Multi-Attach volume follows the same process as described in the [OVHcloud documentation on volume creation](/guides/public-cloud/compute/starting-with-managing-volumes-openstack-api). The key difference is that the volume type is "classic-multiattach", not just "classic".
