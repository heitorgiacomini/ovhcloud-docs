...
description: How to connect to APIs with OVHcloud service accounts using the OAuth2 protocol
...
To carry out automations on your infrastructure, you need to provide credentials to the code that does it. With OVHcloud service accounts, it is possible to have a single ID per script for the use of the different APIs offered by OVHcloud products ([OVHcloud API](/guides/manage-and-operate/api/console-preview), [OpenStack API](/guides/public-cloud/compute/starting-with-nova), etc.)
...
- You know [how to configure access policies via API](/guides/account-and-service-management/account-information/iam-policies-api).
- You know [how to use the OVHcloud APIs](/guides/manage-and-operate/api/first-steps).
- You must have created a [service account via API](/guides/manage-and-operate/api/manage-service-account).
...
First of all, our service account must be able to access the required APIs. OVHcloud provides rights for each API call. To find out what permissions are required, you can go to the OVHcloud API console:
...
<img className="thumbnail" alt="API" src="/images/account-and-service-management/account-information/authenticate-api-with-service-account/actions-in-console.png" loading="lazy" />
...
To retrieve an API token, use the following HTTP call:
...
<Api version="v1" section="/hosting/web" method="GET" route={"/hosting/web/\{serviceName\}"} />
